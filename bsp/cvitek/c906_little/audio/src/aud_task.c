...
rt_mq_t xQueueAudio;
rt_mq_t xQueueAudioCmdqu;
int unit_test_triggered;
unsigned char ssp_running;
void *paudio_ssp_handle;
void *paudio_ssp_spk_handle;
//for ssp parameters after ssp_init
AI_TALKVQE_CONFIG_S *pstVqeConfig;
ST_SSP_RTOS_INDICATOR *pindicator;
ST_SSP_BUFTBL	*pstSspBufTbl;
//for block mode global variables
void *paudio_ssp_block;
...
void prvAudioRunTask(void *pvParameters)
{
	...
	rt_tick_t xTimer3AProcStart;
	rt_tick_t xTimer3AProcEnd;
	...
	for (;;) {
		rt_mq_recv(xQueueAudio, &rtos_cmdq,
		...
		if (rtos_cmdq.ip_id != IP_AUDIO) {
			...
			rt_mq_send(xQueueAudioCmdqu, &rtos_cmdq, sizeof(rtos_cmdq));
		...
		case CVIAUDIO_RTOS_CMD_SSP_INIT:
			...
			rt_hw_cpu_dcache_invalidate((uintptr_t)gpstCviaudioMailBox, sizeof(ST_CVIAUDIO_MAILBOX));
			...
			if (gpstCviaudioMailBox->u64RevMask == CVIAUDIO_RTOS_MAGIC_WORD_KERNEL_BIND_MODE) {
				...
				rt_hw_cpu_dcache_invalidate((uintptr_t)gpstCviaudioMailBox->AinVqeCfgPhy,
							sizeof(AI_TALKVQE_CONFIG_S));
				rt_hw_cpu_dcache_invalidate((uintptr_t)gpstCviaudioMailBox->buffertblPhy,
							sizeof(ST_SSP_BUFTBL));
				rt_hw_cpu_dcache_invalidate((uintptr_t)gpstCviaudioMailBox->indicatorPhy,
			...
			if (paudio_ssp_handle == NULL || pstSspBufTbl == NULL) {
				...
				do {
					...
					if (pindicator->msg_counter == prev_msg_counter) {
						...
						rt_hw_cpu_dcache_invalidate((uintptr_t)pindicator, sizeof(ST_SSP_RTOS_INDICATOR));
						rt_hw_cpu_dcache_invalidate((uintptr_t)pstSspBufTbl,
					...
					if (bCheck_input_full) {
						...
						rt_hw_cpu_dcache_clean((uintptr_t)pindicator,
							sizeof(ST_SSP_RTOS_INDICATOR));
						rt_hw_cpu_dcache_clean((uintptr_t)pstSspBufTbl,
							sizeof(ST_SSP_BUFTBL) * CVIAUDIO_SSP_CHUNK_NUMBERS);
						//xQueueSend(xQueueAudioCmdqu, &rtos_cmdq, 0U);
						rt_mq_send(xQueueAudioCmdqu, &rtos_cmdq, sizeof(rtos_cmdq));
					}
		...
		case CVIAUDIO_RTOS_CMD_SSP_UNIT_TEST_BLOCK_MODE_GET:
			...
			rt_hw_cpu_dcache_invalidate((uintptr_t)output_addr, 320);
			...
			taskEXIT_CRITICAL();
			#endif
			rt_hw_cpu_dcache_clean(output_addr, 320);
			//xQueueSend(xQueueAudioCmdqu, &rtos_cmdq, 0U);
			rt_mq_send(xQueueAudioCmdqu, &rtos_cmdq, sizeof(rtos_cmdq));
			}
			break;
				...
				do {
					for (index = 0; index < CVIAUDIO_SSP_CHUNK_NUMBERS; index++) {
						...
						if (!bCheck_input_full) {
							...
							rt_hw_cpu_dcache_invalidate((uintptr_t)pSspRtosCfg, sizeof(ST_SSP_RTOS_INIT));
							break;
						}
					}
					if (bCheck_input_full) {
						...
						rt_hw_cpu_dcache_clean(pSspRtosCfg, sizeof(ST_SSP_RTOS_INIT));
		...
		case CVIAUDIO_RTOS_CMD_SSP_INIT_BLOCK:
			...
			rt_hw_cpu_dcache_invalidate((uintptr_t)pstAudBlockMailBox, sizeof(ST_CVIAUDIO_MAILBOX_BLOCK));
			...
			if (pstAudBlockMailBox->u64RevMask != CVIAUDIO_RTOS_MAGIC_WORD_USERSPACE_BLOCK_MODE) {
				...
				rt_hw_cpu_dcache_clean((uintptr_t)pstAudBlockMailBox, sizeof(ST_CVIAUDIO_MAILBOX_BLOCK));
				//xQueueSend(xQueueAudioCmdqu, &rtos_cmdq, 0U);
				rt_mq_send(xQueueAudioCmdqu, &rtos_cmdq, sizeof(rtos_cmdq));
				break;
			} else
			...
			rt_hw_cpu_dcache_invalidate((uintptr_t)_pstVqeConfig, sizeof(AI_TALKVQE_CONFIG_S_RTOS));
		...
		case CVIAUDIO_RTOS_CMD_SSP_PROC_BLOCK:
			...
			rt_hw_cpu_dcache_invalidate((uintptr_t)pstBlockFrm, sizeof(ST_CVIAUDIO_BLOCK_FRAME));
			rt_hw_cpu_dcache_invalidate((uintptr_t)pstBlockFrm->mic_in_addr, 1280);
			rt_hw_cpu_dcache_invalidate((uintptr_t)pstBlockFrm->output_addr, 1280);
			rt_hw_cpu_dcache_invalidate((uintptr_t)pstBlockFrm->ref_in_addr, 1280);
			if (pstBlockFrm->u64RevMask != CVIAUDIO_RTOS_MAGIC_WORD_USERSPACE_BLOCK_MODE) {
				...
				rtos_cmdq.param_ptr = CVIAUDIO_RTOS_BLOCK_MODE_FAILURE_FLAG;
				rt_hw_cpu_dcache_clean((uintptr_t)pstBlockFrm, sizeof(ST_CVIAUDIO_BLOCK_FRAME));
				//xQueueSend(xQueueAudioCmdqu, &rtos_cmdq, 0U);
				rt_mq_send(xQueueAudioCmdqu, &rtos_cmdq, sizeof(rtos_cmdq));
				break;
				...
				if (frame_count == 8000/CVIAUDIO_BASIC_AEC_LENGTH*3) {
					aud_info("total_diff = %ld, cpu = %ld, portTICK_PERIOD_MS=%d\n",
							total_diff, total_diff*1000u / RT_TICK_PER_SECOND / 30,
							RT_TICK_PER_SECOND / 1000u);
					frame_count = 0;
					total_diff = 0;
				}
				#endif
			...
			rt_hw_cpu_dcache_clean((uintptr_t)pstBlockFrm, sizeof(ST_CVIAUDIO_BLOCK_FRAME));
			rt_hw_cpu_dcache_clean((uintptr_t)pstBlockFrm->mic_in_addr, 1280);
			rt_hw_cpu_dcache_clean((uintptr_t)pstBlockFrm->output_addr, 1280);
			rt_hw_cpu_dcache_clean((uintptr_t)pstBlockFrm->ref_in_addr, 1280);
			//xQueueSend(xQueueAudioCmdqu, &rtos_cmdq, 0U);
			rt_mq_send(xQueueAudioCmdqu, &rtos_cmdq, sizeof(rtos_cmdq));
			aud_info("[RTOS]Leaving\n");
			}
