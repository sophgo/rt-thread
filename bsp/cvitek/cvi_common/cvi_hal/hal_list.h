 ...
 * @param[in]   member  the name of the variable within the struct.
 */
#define hal_container_of(ptr, type, member) ((type *)((char *)(ptr) - hal_offsetof(type, member)))
 ...
 */
#define dlist_entry(addr, type, member) ((type *)((long)addr - hal_offsetof(type, member)))
...
static inline void INIT_HAL_DLIST_HEAD(dlist_t *list)
{
    list->next = list;
    list->prev = list;
}
 ...
 */
#define HAL_DLIST_INIT(list) {&(list), &(list)}
...
#define dlist_for_each_entry(queue, node, type, member)                                  \
    for (node = hal_container_of((queue)->next, type, member); &node->member != (queue); \
         node = hal_container_of(node->member.next, type, member))
...
#define dlist_for_each_entry_safe(queue, n, node, type, member) \
    for (node = hal_container_of((queue)->next, type, member),  \
        n     = (queue)->next ? (queue)->next->next : NULL;     \
         &node->member != (queue);                              \
         node = hal_container_of(n, type, member), n = n ? n->next : NULL)
 ...
 * @param[in]   member  the name of the variable within the struct.
 */
#define list_entry(ptr, type, member) hal_container_of(ptr, type, member)
...
#define HAL_DLIST_HEAD_INIT(name) {&(name), &(name)}
 ...
 */
#define HAL_DLIST_HEAD(name) dlist_t name = HAL_DLIST_HEAD_INIT(name)
...
#define slist_for_each_entry(queue, node, type, member)                                     \
    for (node = (queue)->next ? hal_container_of((queue)->next, type, member) : NULL; node; \
         node = node->member.next ? hal_container_of(node->member.next, type, member) : NULL)
...
#define slist_for_each_entry_safe(queue, tmp, node, type, member)                     \
    for (node = (queue)->next ? hal_container_of((queue)->next, type, member) : NULL, \
        tmp   = (queue)->next ? (queue)->next->next : NULL;                           \
         node;                                                                        \
         node = tmp ? hal_container_of(tmp, type, member) : NULL, tmp = tmp ? tmp->next : NULL)
 ...
 * @param[in]   name    the list to be initialized.
 */
#define HAL_SLIST_HEAD_INIT(name) {0}
 ...
 * @param[in]   name    the list to be initialized.
 */
#define HAL_SLIST_HEAD(name) slist_t name = HAL_SLIST_HEAD_INIT(name)
...
#define slist_entry(addr, type, member) \
    (addr ? (type *)((long)addr - hal_offsetof(type, member)) : (type *)addr)
 ...
 */
#define slist_tail_entry(ptr, type, member) slist_entry(slist_tail(ptr), type, member)
...
static inline void __hal_list_splice(const dlist_t *list, dlist_t *prev, dlist_t *next)
{
...
/**
 * hal_list_splice - join two lists, this is designed for stacks
 * @list: the new list to add.
 * @head: the place to add it in the first list.
 */
static inline void hal_list_splice(const dlist_t *list, dlist_t *head)
{
    if (!dlist_empty(list))
        __hal_list_splice(list, head, head->next);
}

/**
 * hal_list_splice_init - join two lists and reinitialise the emptied list.
 * @list: the new list to add.
 * @head: the place to add it in the first list.
 *
 * The list at @list is reinitialised
 */
static inline void hal_list_splice_init(dlist_t *list, dlist_t *head)
{
    if (!dlist_empty(list)) {
        __hal_list_splice(list, head, head->next);
        INIT_HAL_DLIST_HEAD(list);
    }
}
