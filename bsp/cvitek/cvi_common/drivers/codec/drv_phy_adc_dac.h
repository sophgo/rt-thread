...
#ifndef __DRV_PHY_ADC_DAC_H__
#define __DRV_PHY_ADC_DAC_H__
#include <stdbool.h>
#include <stdio.h>
#include <unistd.h>

#define DBG_TAG "CVI.CODEC"
#define DBG_LVL DBG_WARNING
#include <rtdbg.h>

#define ADC_BASE_OFFSET 0x100
...
#define AUDIO_PHY_RXADC_CTRL0 (0x100 - ADC_BASE_OFFSET)
#define AUDIO_PHY_RXADC_CTRL1 (0x104 - ADC_BASE_OFFSET)
#define AUDIO_PHY_RXADC_STATUS (0x108 - ADC_BASE_OFFSET)
#define AUDIO_PHY_RXADC_CLK (0x10c - ADC_BASE_OFFSET)
#define AUDIO_PHY_RXADC_ANA0 (0x110 - ADC_BASE_OFFSET)
#define AUDIO_PHY_RXADC_ANA1 (0x114 - ADC_BASE_OFFSET)
#define AUDIO_PHY_RXADC_ANA2 (0x118 - ADC_BASE_OFFSET)
#define AUDIO_PHY_RXADC_ANA3 (0x11c - ADC_BASE_OFFSET)
#define AUDIO_PHY_RXADC_ANA4 (0x120 - ADC_BASE_OFFSET)
#define AUDIO_PHY_RXADC_ANA5 (0x124 - ADC_BASE_OFFSET)
#define AUDIO_PHY_RXADC_ANA6 (0x128 - ADC_BASE_OFFSET)
#define AUDIO_PHY_GPIO_ANA (0x12c - ADC_BASE_OFFSET)
#define AUDIO_PHY_SPARE_0 (0xff0 - ADC_BASE_OFFSET)
#define AUDIO_PHY_SPARE_1 (0xff4 - ADC_BASE_OFFSET)
#define AUDIO_PHY_SPARE_RO (0xff8 - ADC_BASE_OFFSET)
#define AUDIO_PHY_DATE_CODE (0xffc - ADC_BASE_OFFSET)
...
#define DAC_VOL_STEP 0x8
#define DAC_VOL_L(V) (V == 0 ? 0 : (V * DAC_VOL_STEP) - 1)
#define DAC_VOL_R(V) ((V == 0 ? 0 : (V * DAC_VOL_STEP) - 1) << 16)
...
#define DAC_RESET 0xF7FFFFFF
#define DAC_APB_RESET 0xEFFFFFFF

#define ADC_RESET 0xDFFFFFFF
#define ADC_APB_RESET 0xBFFFFFFF
...
#define CVI_ADC_POWER_MASK                                                     \
	0xFFFCFF0C /* use to reset power on/off related bits */
...
struct cvi_vol_ctrl {
/* volume control, adc range: 0x00~0x1f, 0x17F:mute. dac range: 0x00~0x0f,
 * 0x0f:mute
 */
	unsigned int vol_l;
	unsigned int vol_r;
...
static inline void dac_write_reg(uint32_t *io_base, uint32_t reg,
								 uint32_t val)
{
	*(io_base + (reg / sizeof(uint32_t))) = val;
}

static inline uint32_t dac_read_reg(uint32_t *io_base, uint32_t reg)
{
	return *(io_base + (reg / sizeof(uint32_t)));
}

static inline void adc_write_reg(uint32_t *io_base, uint32_t reg,
								 uint32_t val)
{
	*(io_base + (reg / sizeof(uint32_t))) = val;
}

static inline uint32_t adc_read_reg(uint32_t *io_base, uint32_t reg)
{
	uint32_t val;

	val = *(io_base + (reg / sizeof(uint32_t)));
	return val;
}

static inline void reset_write_reg(uint32_t val, uint32_t *io_base)
{
	*(io_base) = val;
}

static inline uint32_t reset_read_reg(uint32_t *io_base)
{
	return *(io_base);
}

static inline void arch_usleep(unsigned long useconds) { usleep(useconds); }

/* while codecs transfer CIC between 64 and 128, need to reset codec first */
static inline void _dac_reset(void)
{
	uint32_t *reset_reg = (uint32_t *)(0x03003008);

	reset_write_reg((reset_read_reg(reset_reg) & DAC_RESET), reset_reg);
	arch_usleep(1000 * 10);
	reset_write_reg((reset_read_reg(reset_reg) | ~DAC_RESET), reset_reg);
}

static inline void _adc_reset(void)
{
	uint32_t *reset_reg = (uint32_t *)(0x03003008);

	reset_write_reg((reset_read_reg(reset_reg) & ADC_RESET), reset_reg);
	arch_usleep(1000 * 10);
	reset_write_reg((reset_read_reg(reset_reg) | ~ADC_RESET), reset_reg);
}
