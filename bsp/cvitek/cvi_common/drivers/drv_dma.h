...
#define DW_DMA_MAX_NR_MASTERS  1
...
#define DWC_CTL_SHADOWREG_OR_LLI_VALID \
    (1ULL << 63) /* Shadow Register content/Linked List Item valid */
...
enum dwc_multblk_type { CONTIGUOUS, RELOAD, SHADOW_REGISTER, LINK_LIST };
...
#define DW_DMA_BASE 0x4350000U
#define DW_DMA_IRQn 45U

#define DMA_CLK_EN_REG		0x030020f4
#define CLK_SDMA_AXI_BIT	15

#define REG_SDMA_DMA_CH_REMAP0	0x030002B0U
#define REG_SDMA_DMA_CH_REMAP1	0x030002B4U
...
typedef struct dw_dma_channel {
    void (*callback)(uint32_t ch_id, void *arg);
    void             *args;
    uint8_t           ctrl_id;
    uint8_t           ch_id;
    uint8_t           ch_mask;
...
#define dma_writeq_native(__value, __reg) (*(volatile uint64_t *)(__reg) = (__value));

#define dma_writeq(dw, name, val) dma_writeq_native((val), &(_dw_regs(dw)->name))

#define channel_readq(dwc, name) dma_readq_native(&(_dwc_regs(dwc)->name))

#define channel_writeq(dwc, name, val) dma_writeq_native((val), &(_dwc_regs(dwc)->name))
...
#define dma_readq(dw, name) dma_readq_native(&(_dw_regs(dw)->name))

#define dma_set_bit(dw, name, mask) \
    dma_writeq_native((mask) | dma_readq_native(&(_dw_regs(dw)->name)), &(_dw_regs(dw)->name))
...
{
    return (cfg->trans_dir == DMA_MEM2DEV || cfg->trans_dir == DMA_DEV2MEM);
}

static inline struct dw_dma_regs *_dw_regs(dw_dma_t *dw_dma)
{
    return dw_dma->regs;
}
...
#ifdef __riscv
#define SDMA_DMA_INT_MUX       0x030002c0
#define SDMA_DMA_INT_MUX_C906B (0xff << 10)
#define SDMA_DMA_INT_MUX_C906L (0x1ff << 10)
