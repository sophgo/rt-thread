...
static rt_err_t rt_dw_eth_init(rt_device_t dev)
{
    ...
    link_detect = rt_thread_create("link_detect", phy_link_detect, (void *)&dw_eth_device, DETECT_THREAD_STACK_SIZE,
                                   DETECT_THREAD_PRIORITY, 2);
...
const static struct rt_device_ops dw_eth_ops = {rt_dw_eth_init, RT_NULL, RT_NULL, RT_NULL, RT_NULL, rt_dw_eth_control};
