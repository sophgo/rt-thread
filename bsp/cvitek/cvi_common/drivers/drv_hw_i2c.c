...
static void dw_iic_set_target_address(dw_iic_regs_t* iic_base, rt_uint32_t address)
{
    ...
    iic_base->IC_TAR =
        (iic_base->IC_TAR & ~0x3ff) | address; /* this register can be written only when the I2C is disabled*/
...
static void dw_iic_set_master_mode(dw_iic_regs_t* iic_base)
{
    ...
    dw_iic_disable(iic_base);
    rt_uint32_t val =
        DW_IIC_CON_MASTER_EN | DW_IIC_CON_SLAVE_EN;  ///< set 1 to enabled master mode; set 1 to disabled slave mode
    iic_base->IC_CON |= val;
...
static rt_err_t dw_iic_check_addr(dw_iic_regs_t* iic_base)
{
...
static rt_err_t dw_iic_recv(dw_iic_regs_t* iic_base, struct rt_i2c_msg* msg, rt_uint32_t timeout)
{
    rt_err_t ret = RT_EOK;
    rt_uint32_t timecount, size;
    rt_uint8_t* data;
    ...
    for (int i = 0; i < size; i++) {
        if (i == (size - 1) && !(msg->flags & RT_I2C_NO_STOP))
    ...
    while (size > 0) {
        ret = dw_iic_check_addr(iic_base);
        if (ret) {
            if (!(msg->flags & RT_I2C_IGNORE_NACK))
                LOG_E("Rmote I/O ERROR\n");
            return ret;
        }
...
static rt_err_t dw_iic_send(dw_iic_regs_t* iic_base, struct rt_i2c_msg* msg, rt_uint32_t timeout)
{
    rt_err_t ret = RT_EOK;
    rt_uint32_t timecount, size;
    const uint8_t* data;
    ...
    while (size > 0) {
        ...
        if (iic_base->IC_STATUS & DW_IIC_TXFIFO_NOT_FULL_STATE) {
            if (--size == 0 && !(msg->flags & RT_I2C_NO_STOP)) {
...
static rt_ssize_t dw_iic_master_xfer(struct rt_i2c_bus_device* bus, struct rt_i2c_msg msgs[], rt_uint32_t num)
{
    ...
    if (dw_iic_xfer_init(iic_base, msgs[0].addr)) {
        ret = -RT_ERROR;
        goto ERR_EXIT;
    }
    ...
    for (i = 0; i < num; i++) {
        ...
        if (msg->flags & RT_I2C_RD) {
            timeout = 100;
            ret     = dw_iic_recv(iic_base, msg, timeout);
            if (ret != RT_EOK)
                goto ERR_EXIT;
        } else {
            timeout = 100;
            ret     = dw_iic_send(iic_base, msg, timeout);
            if (ret != RT_EOK)
                goto ERR_EXIT;
        }
    }
...
static rt_err_t dw_iic_set_speed(dw_iic_regs_t* iic_base, rt_uint32_t speed)
{
    rt_uint32_t tf = 300;  // falling time = 300ns
...
static rt_err_t dw_iic_bus_control(struct rt_i2c_bus_device* bus, int cmd, void* args)
{
    ...
    case RT_I2C_DEV_CTRL_CLK:
        speed = *(rt_uint32_t*)args;
        if (dw_iic_set_speed(iic_base, speed))
            return RT_ERROR;
        break;
...
static void dw_iic_init(dw_iic_regs_t* iic_base)
{
    ...
    dw_iic_set_speed(iic_base, 400 * 1000);
}
...
#elif defined (SOC_TYPE_CV181XH_BGA) || defined (SOC_TYPE_CV181XH_QFN)
...
#elif defined (SOC_TYPE_CV184XC_QFN) || defined (SOC_TYPE_CV184XH_BGA)
