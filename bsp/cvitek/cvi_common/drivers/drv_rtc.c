...
static rt_err_t _rtc_set_timeval(struct timeval *tv)
{
	...
	// Convert timeval to tm structure
	gmtime_r(&tv->tv_sec, &tm_new);

	t.tm_sec     = tm_new.tm_sec;
	t.tm_min     = tm_new.tm_min;
	t.tm_hour    = tm_new.tm_hour;
	t.tm_mday    = tm_new.tm_mday;
	t.tm_mon     = tm_new.tm_mon;
	t.tm_year    = tm_new.tm_year;
	t.tm_wday    = tm_new.tm_wday;
