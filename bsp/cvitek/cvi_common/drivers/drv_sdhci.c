...
uint32_t sdhci_prepare_data(struct rthw_sdhci *sdhci, struct rt_mmcsd_cmd *cmd, struct rt_mmcsd_data *data, sdhci_dma_config_t *dma_config)
{
    ...
    rt_hw_cpu_dcache_ops(RT_HW_CACHE_FLUSH, (void *)load_addr, dma_config->dma_des_buffer_len);
...
static rt_err_t sdhci_send_data_cmd(struct rthw_sdhci *sdhci, struct rt_mmcsd_cmd *cmd, struct rt_mmcsd_data *data)
{
    ...
    if (data)
    {
        ...
        if (data->flags & DATA_DIR_READ)
        {
            rt_hw_cpu_dcache_ops(RT_HW_CACHE_INVALIDATE, (void *)dma_config.dma_des_buffer_start_addr, dma_config.dma_des_buffer_len);
            if (src_unalign)
            {
...
static uint32_t sdhci_set_clock(rt_uint32_t *base, uint32_t target_hz)
{
    ...
    source_clock_hz = 400 * 1000 * 1000; // Mars3 = 400M, Mars_Phobos = 375M
...
static int rthw_sdhci_init(void)
{
    ...
    // Only SD card supported here.
    mmio_clrbits_32(0x03002108, BIT(3) | BIT(4)); // Set SD0 to PLL
    sdhci->base = (rt_uint32_t *)SDIO1_BASE;
    sdhci->irq = SDIO1_IRQ;
    strcpy(sdhci->name, "cvi_sd");
...
void sdhci_reg_dump(uint8_t argc, char **argv)
{
    ...
    if (argc < 2)
    {
        rt_kprintf("Usage: sdhci_reg_dump 0/1/2; EMMC-0, SD-1, WIFI-2\n");
        return;
    }
