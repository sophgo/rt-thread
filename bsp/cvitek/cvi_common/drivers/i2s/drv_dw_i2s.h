 ...
 * SOPHGO I2s driver on SOPHGO
 *
 * Copyright 2025 SOPHGO
 *
 * Author: Rachel.jiang
 *
 */
...
// shit 1/2 T for inv, ex master generate data at falling edge and let codec
...
struct i2s_tdm_regs {
	...
	u32 i2s_lrck_master;  // dummy1[1];	/* 0x2c */
	u32 fifo_reset;       /* 0x30 */
	...
	u32 i2s_pcm_synth;    /* 0x68 */
	u32 dummy5[5];        /* 0x70 ~ 0x7c */
...
struct i2s_config_info {
	...
	unsigned char direction; /*	rx or tx*/
...
#define FRAME_LENGTH(l)                                                        \
	(((l - 1) << 0) &                                                            \
	 FRAME_LENGTH_MASK) /* frame length between 0~511 = 1~512 bits */
...
#define FS_ACT_LENGTH(l)                                                       \
	(((l - 1) << 16) &                                                           \
	 FS_ACT_LENGTH_MASK) /* frame active length between 0~255 = 1~256 bits*/
...
int i2s_init(struct i2s_config_info *pi2s_tx);
