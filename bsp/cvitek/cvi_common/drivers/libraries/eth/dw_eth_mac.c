...
static int32_t designware_write_hwaddr(eth_mac_handle_t handle)
{
    ...
    macid_lo = mac_id[0] + (mac_id[1] << 8) + (mac_id[2] << 16) + (mac_id[3] << 24);
...
static void tx_descs_init(eth_mac_handle_t handle)
{
    ...
    for (idx = 0; idx < CVI_CONFIG_TX_DESCR_NUM; idx++) {
        ...
        desc_p->txrx_status &=
            ~(CVI_DESC_TXSTS_TXINT | CVI_DESC_TXSTS_TXLAST | CVI_DESC_TXSTS_TXFIRST
              | CVI_DESC_TXSTS_TXCRCDIS | CVI_DESC_TXSTS_TXCHECKINSCTRL | CVI_DESC_TXSTS_TXRINGEND
              | CVI_DESC_TXSTS_TXPADDIS);
...
static void rx_descs_init(eth_mac_handle_t handle)
{
    ...
    for (idx = 0; idx < CVI_CONFIG_RX_DESCR_NUM; idx++) {
        ...
        desc_p->dmamac_cntl =
            (CVI_MAC_MAX_FRAME_SZ & CVI_DESC_RXCTRL_SIZE1MASK) | CVI_DESC_RXCTRL_RXCHAIN;
...
static int32_t designware_eth_send(eth_mac_handle_t handle, const uint8_t *frame, uint32_t length)
{
    ...
    desc_p->dmamac_cntl |= (length << CVI_DESC_TXCTRL_SIZE1SHFT) & CVI_DESC_TXCTRL_SIZE1MASK;
    ...
    desc_p->dmamac_cntl &= ~CVI_DESC_TXCTRL_SIZE1MASK;
    desc_p->dmamac_cntl |= ((length << CVI_DESC_TXCTRL_SIZE1SHFT) & CVI_DESC_TXCTRL_SIZE1MASK)
                           | CVI_DESC_TXCTRL_TXLAST | CVI_DESC_TXCTRL_TXFIRST;
...
/**
  ...
  \return      error code
*/
int32_t dw_eth_mac_phy_read(eth_mac_handle_t handle, uint8_t phy_addr, uint8_t reg_addr,
                            uint16_t *data)
{
    RT_ASSERT(handle);
    RT_ASSERT(data);
...
/**
  ...
  \return      error code
*/
int32_t dw_eth_mac_phy_write(eth_mac_handle_t handle, uint8_t phy_addr, uint8_t reg_addr,
                             uint16_t data)
{
    RT_ASSERT(handle);
    gmac_dev_t              *mac_dev = (gmac_dev_t *)handle;
