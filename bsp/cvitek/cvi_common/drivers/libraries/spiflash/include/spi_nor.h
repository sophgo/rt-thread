...
#define RD_FULL (RD_SINGLE | RD_DUAL | RD_DUALIO | RD_QUAD | RD_QUADIO)
...
struct spi_nor {
    ...
    int (*read)(struct spi_nor *nor, uint64_t from, uint32_t len, void *read_buf);
    int (*write)(struct spi_nor *nor, uint32_t to, uint32_t len, const void *write_buf);
...
enum write_type { ERASE = 0, WRITE = 1 };
