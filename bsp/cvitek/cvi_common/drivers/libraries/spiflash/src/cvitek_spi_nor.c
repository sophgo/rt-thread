                                     ...
                                     uint32_t data_bytes, uint8_t bus_width)
{
    ...
    tran_csr &= ~(BIT_SPI_TRAN_CSR_TRAN_MODE_MASK | BIT_SPI_TRAN_CSR_ADDR_BYTES_MASK |
                  BIT_SPI_TRAN_CSR_FIFO_TRG_LVL_MASK | BIT_SPI_TRAN_CSR_BUS_WIDTH_MASK |
    ...
    /* clear interrupts */
    writeb(readb(spif->io_base + REG_SPI_INT_STS) & ~BIT_SPI_INT_TRAN_DONE,
           spif->io_base + REG_SPI_INT_STS);
    writeb(readb(spif->io_base + REG_SPI_INT_STS) & ~BIT_SPI_INT_WR_FIFO,
           spif->io_base + REG_SPI_INT_STS);

    return 0;
}

static int cvi_spi_data_in_tran(struct cvi_spif *spif, uint8_t *dst_buf, int data_bytes,
                                uint8_t bus_width)
{
    uint32_t tran_csr = 0;
    int      i, xfer_size, off;
    ...
    writeb(readb(spif->io_base + REG_SPI_INT_STS) & ~BIT_SPI_INT_TRAN_DONE,
           spif->io_base + REG_SPI_INT_STS);
    writeb(readb(spif->io_base + REG_SPI_INT_STS) & ~BIT_SPI_INT_RD_FIFO,
           spif->io_base + REG_SPI_INT_STS);
    return 0;
}
...
static unsigned int match_value_for_read(uint8_t opcode)
{
    ...
    const struct dmmr_reg_t dmmr_reg_set[] = {
        {0x03, 0x0, 0x003B81}, {0x0B, 0x8, 0x003B89}, {0x3B, 0x8, 0x003B91}, {0xBB, 0x4, 0x003B99},
        {0x6B, 0x8, 0x003BA1}, {0xEB, 0x6, 0x003BA9}, {0x13, 0x0, 0x303C81}, {0x0C, 0x8, 0x303C89},
        {0x3C, 0x8, 0x303C91}, {0xBC, 0x4, 0x303C99}, {0x6C, 0x8, 0x303CA1}, {0xEC, 0x6, 0x303CA9},
        {0x0, 0x0, 0x0}};
...
static void cvi_spif_set_memory_mode_on(struct cvi_spif *spif, spi_tran_conf_t *read)
{
...
static int cnt = 0;
void       memcpy_fromio(void *to, const volatile void *from, volatile long count)
{
...
int cvi_spif_write_reg(struct spi_nor *nor, uint8_t opcode, uint8_t *buf, int len)
{
    ...
    if (len) cvi_spi_data_out_tran(spif, buf, len, bus_width);
