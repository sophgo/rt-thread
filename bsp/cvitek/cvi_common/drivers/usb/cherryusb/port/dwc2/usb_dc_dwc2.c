...
// #define CONFIG_USB_DWC2_TX0_FIFO_SIZE (1024)
// #define CONFIG_USB_DWC2_TX1_FIFO_SIZE (2048)
// #endif


#if (USBD_UVC_NUM > 1)
#define CONFIG_USB_DWC2_TX1_FIFO_SIZE (2048)
#define CONFIG_USB_DWC2_TX2_FIFO_SIZE (2048)
#define CONFIG_USB_DWC2_TX3_FIFO_SIZE (2048)
#endif
...
#ifndef CONFIG_USB_DWC2_TX1_FIFO_SIZE
#define CONFIG_USB_DWC2_TX1_FIFO_SIZE (3072)
#endif
...
#define CONFIG_USB_DWC2_TX6_FIFO_SIZE (256)
#endif

#ifndef USB_NUM_BIDIR_ENDPOINTS
#define USB_NUM_BIDIR_ENDPOINTS 7 /* define with minimum value*/
#endif
...
#define EP_WRITE_BUFFER_SIZE    3072

static uint32_t SystemCoreClock = 300000000U;

USB_NOCACHE_RAM_SECTION USB_MEM_ALIGNX uint8_t read_buffer[3072];
USB_NOCACHE_RAM_SECTION USB_MEM_ALIGNX uint8_t ep_write_buffer[EP_WRITE_BUFFER_SIZE];
...
/* Driver state */
struct dwc2_udc {
    __attribute__((aligned(CONFIG_USB_ALIGN_SIZE))) struct usb_setup_packet setup;
...
#endif
static void USBD_IRQHandler(int vector, void *param);

void usbd_dump_reg_info()
{
    uint32_t ep_idx = 2;
    USB_LOG_ERR("doepctl:%08x\n", USB_OTG_OUTEP(ep_idx)->DOEPCTL);
    USB_LOG_ERR("ods:%08x ids:%08x sp:%08x\n",USB_OTG_OUTEP(ep_idx)->DOEPTSIZ,USB_OTG_OUTEP(3)->DOEPTSIZ,USB_OTG_GLB->GRXSTSR);
    USB_LOG_ERR("GINTSTS :%08x \r\n",USB_OTG_GLB->GINTSTS);

    // USB_LOG_DBG("rx fifo info:%08x\n", USB_OTG_GLB->GRXSTSP);
}
...
static inline int dwc2_reset(void)
{
    ...
    do {
        rt_hw_us_delay(10);
        if (++count > 100000U) {
            USB_LOG_DBG("ahb master idle failed!\n");
            return -1;
        }
    ...
    /* Core Soft Reset */
    count = 0U;
   // USB_LOG_DBG("USB_OTG_GLB->GRSTCTL:%08x\n", USB_OTG_GLB->GRSTCTL);
    USB_OTG_GLB->GRSTCTL |= USB_OTG_GRSTCTL_CSRST;

    do {
        rt_hw_us_delay(1);
        if (++count > 10000U) {
            USB_LOG_DBG("core soft reset failed!\n");
            return -1;
        }
...
{
    // if(USB_OTG_OUTEP(0U)->DOEPCTL & USB_OTG_DOEPCTL_EPENA){
    //     USB_LOG_DBG("EER:ep0 start read, 0x%08x\n", USB_OTG_OUTEP(0U)->DOEPCTL);
    // }
    ...
    if(fifo_dma_mode == ON){
        USB_OTG_OUTEP(0U)->DOEPDMA = (intptr_t)psetup;
        /* EP enable */
        //USB_OTG_OUTEP(0U)->DOEPCTL |= USB_OTG_DOEPCTL_CNAK;
        USB_OTG_OUTEP(0U)->DOEPCTL |= USB_OTG_DOEPCTL_EPENA;
        USB_OTG_OUTEP(0U)->DOEPCTL |= USB_OTG_DOEPCTL_USBAEP;
    }else{
        USB_OTG_OUTEP(0U)->DOEPDMA = (intptr_t)0;
    }
#endif
    // USB_LOG_DBG("ep0 read setup, 0x%08x\n", USB_OTG_OUTEP(0U)->DOEPCTL);
}
...
static inline uint32_t dwc2_get_outep_intstatus(uint8_t epnum)
{
    ...
    tmpreg &= USB_OTG_DEV->DOEPMSK | USB_OTG_DOEPINT_STPKTRX_Msk;
...
{
    rt_hw_interrupt_install(30, USBD_IRQHandler, NULL, "usbd");
...
int usb_dc_init(void)
{
    ...
    // reg =  (volatile uint32_t *)(0x3006040); *reg |= 0x1F000000;
    // reg =  (volatile uint32_t *)(0x3006044); *reg |= 0x30;
    ...
    USB_OTG_GLB->GINTMSK = USB_OTG_GINTMSK_USBRST | USB_OTG_GINTMSK_ENUMDNEM |
                        ...
                        #if CONFIG_USB_BULK_UVC
                           USB_OTG_GINTMSK_USBSUSPM | USB_OTG_GINTMSK_WUIM | USB_OTG_GINTMSK_FSUSPM |
                        #endif
...
int usbd_set_address(const uint8_t addr)
{
    ...
    //USB_LOG_DBG("after set address, DCFG:0x%08x\n", USB_OTG_DEV->DCFG);
    return 0;
}
...
int usbd_ep_open(const struct usbd_endpoint_cfg *ep_cfg)
{
...
#if CONFIG_USB_BULK_UVC
    uint8_t ep_next = (ep_idx + 1) % USB_NUM_BIDIR_ENDPOINTS;
#endif
    uint16_t ep_mps;

    if (ep_idx > (USB_NUM_BIDIR_ENDPOINTS - 1)) {
        USB_LOG_ERR("Ep addr %d overflow\r\n", ep_cfg->ep_addr);
        USB_LOG_ERR("ep_idx %d, USB_NUM_BIDIR_ENDPOINTS %d\r\n", ep_idx, USB_NUM_BIDIR_ENDPOINTS);
        return -1;
    }
    ...
    if (USB_EP_DIR_IS_OUT(ep_cfg->ep_addr)) {
        ...
        g_dwc2_udc.out_ep[ep_idx].ep_interval = ep_cfg->ep_interval;
        ...
        if ((USB_OTG_OUTEP(ep_idx)->DOEPCTL & USB_OTG_DOEPCTL_USBAEP) == 0) {
            USB_OTG_OUTEP(ep_idx)->DOEPCTL |= (ep_mps & USB_OTG_DOEPCTL_MPSIZ) |
                                            ((uint32_t)ep_cfg->ep_type << USB_OTG_DIEPCTL_EPTYP_Pos) |
        ...
        g_dwc2_udc.in_ep[ep_idx].ep_interval = ep_cfg->ep_interval;
        ...
        USB_OTG_INEP(ep_idx)->DIEPCTL |= (ep_cfg->ep_mps & USB_OTG_DIEPCTL_MPSIZ) |
                                         ((uint32_t)ep_cfg->ep_type << USB_OTG_DIEPCTL_EPTYP_Pos) |
                                         (ep_idx << USB_OTG_DIEPCTL_TXFNUM_Pos) |
                                         (ep_next << USB_OTG_DIEPCTL_NEXTEP_Pos) |
                                         USB_OTG_DIEPCTL_SD0PID_SEVNFRM |
...
int usbd_ep_close(const uint8_t ep)
{
    ...
    if (USB_EP_DIR_IS_OUT(ep)) {
        ...
        if (USB_OTG_OUTEP(ep_idx)->DOEPCTL & USB_OTG_DOEPCTL_EPENA) {
           // USB_LOG_DBG("set global out nak\n");

            USB_OTG_GLB->GINTSTS |= USB_OTG_GINTSTS_BOUTNAKEFF;
            ...
            do {
                rt_hw_us_delay(10);
                if (++count > 1000) {
                    USB_LOG_DBG("failed goutnakeff\n");
                    break;
                }
            } while ((USB_OTG_GLB->GINTSTS & USB_OTG_GINTSTS_BOUTNAKEFF) == 0);

           // USB_LOG_DBG("goutnakeff success?\n");

            /* Clear Global OUT NAK effective interrupt */
            USB_OTG_GLB->GINTSTS |= USB_OTG_GINTSTS_BOUTNAKEFF;

            /* Disable endpoint */
            USB_OTG_OUTEP(ep_idx)->DOEPCTL |= USB_OTG_DOEPCTL_EPENA;
            ...
            do {
                rt_hw_us_delay(10);
                if (++count > 50000) {
                    USB_LOG_DBG("OUT ep disabled failed\n");
                    break;
                }
            } while ((USB_OTG_OUTEP(ep_idx)->DOEPINT & USB_OTG_DOEPINT_EPDISD) == 0);
          //  USB_LOG_DBG("OUT EP disabled success?\n");
            ...
            USB_OTG_OUTEP(ep_idx)->DOEPCTL &= ~(USB_OTG_DOEPCTL_EPDIS | USB_OTG_DOEPCTL_EPENA);
        }
        ...
        if (USB_OTG_INEP(ep_idx)->DIEPCTL & USB_OTG_DIEPCTL_EPENA) {
            ...
            do
            {
                ...
                if (++count > 200000U)
                {
                    USB_LOG_DBG("IN ep(%02x) close timeout\n", ep_idx);
                    return -1;
                }
            }
...
int usbd_ep_start_write(const uint8_t ep, const uint8_t *data, uint32_t data_len)
{
...
#if CONFIG_USB_DWC2_DMA_ENABLE
    if(fifo_dma_mode == ON){
        if ((intptr_t)write_data & (CONFIG_USB_ALIGN_SIZE - 1)) {
            if (data_len > EP_WRITE_BUFFER_SIZE) {
                USB_LOG_ERR("ep write data_len too large (%d > %d)\n", EP_WRITE_BUFFER_SIZE, data_len);
                return -3;
            }
            USB_LOG_INFO("DWC2 DMA write address NOT aligned, original address:%p, use tmp buffer:%p\n", write_data, ep_write_buffer);
            memcpy(ep_write_buffer, write_data, data_len);
            write_data = ep_write_buffer;
        }
    }
#endif

    g_dwc2_udc.in_ep[ep_idx].xfer_buf = (uint8_t *)write_data;
    ...
    if (ep_idx == 0) {
        ...
        USB_OTG_INEP(ep_idx)->DIEPTSIZ |= (1 << USB_OTG_DIEPTSIZ_MULCNT_Pos);
    } else {
    ...
    if (g_dwc2_udc.in_ep[ep_idx].ep_type == USB_ENDPOINT_TYPE_ISOCHRONOUS
        && g_dwc2_udc.in_ep[ep_idx].ep_interval == 1) {
...
#if CONFIG_USB_DWC2_DMA_ENABLE
    if(fifo_dma_mode == ON){
        USB_OTG_INEP(ep_idx)->DIEPDMA = (intptr_t)write_data;
        dcache_clean_range( (uint64_t *)write_data,
                            roundup(data_len, 64 /* ARCH_DMA_MINALIGN */));
    }
#endif

    USB_OTG_INEP(ep_idx)->DIEPCTL &= ~(USB_OTG_DIEPCTL_EPDIS | USB_OTG_DIEPCTL_SNAK);
    USB_OTG_INEP(ep_idx)->DIEPCTL |= (USB_OTG_DIEPCTL_CNAK | USB_OTG_DIEPCTL_EPENA);
...
int usbd_ep_start_read(const uint8_t ep, uint8_t *data, uint32_t data_len)
{
...
#if CONFIG_USB_DWC2_DMA_ENABLE
     if(fifo_dma_mode == ON){
        if (((intptr_t)data) & (CONFIG_USB_ALIGN_SIZE - 1)) {
            USB_LOG_ERR("DWC2 DMA read address NOT aligned, original address:%p, temp buffer:%p\n", data, read_buffer);
            data = read_buffer;
        }
        USB_OTG_OUTEP(ep_idx)->DOEPDMA = (intptr_t)data;
        dcache_invalid_range( (uint64_t *)data,
                    roundup(data_len, 64 /* ARCH_DMA_MINALIGN */));
    }
#endif

    if (g_dwc2_udc.out_ep[ep_idx].ep_type == USB_ENDPOINT_TYPE_ISOCHRONOUS
        && g_dwc2_udc.out_ep[ep_idx].ep_interval == 1) {
        if ((USB_OTG_DEV->DSTS & (1U << 8)) != 0U) {
    ...
    USB_OTG_OUTEP(ep_idx)->DOEPCTL &= ~(USB_OTG_DOEPCTL_EPDIS | USB_OTG_DOEPCTL_SNAK);
    USB_OTG_OUTEP(ep_idx)->DOEPCTL |= (USB_OTG_DOEPCTL_CNAK | USB_OTG_DOEPCTL_EPENA);
...
//#if (!CONFIG_USB_DWC2_DMA_ENABLE || fifo_dma_mode != ON)
#if CONFIG_USB_DWC2_DMA_ENABLE
    if (fifo_dma_mode != ON)
#endif
    {
        ...
        if (gint_status & USB_OTG_GINTSTS_OEPINT) {
            ...
            while (ep_intr != 0U) {
                if ((ep_intr & 0x1U) != 0U) {
                    ...
                    USB_OTG_OUTEP(ep_idx)->DOEPINT = DoepintReg | epint;
                    if ((epint & USB_OTG_DOEPINT_XFRC) == USB_OTG_DOEPINT_XFRC) {
                        if (ep_idx == 0) {
                            if (g_dwc2_udc.out_ep[ep_idx].xfer_len == 0) {
                                /* Out status, start reading setup */
                                //rt_hw_us_delay(3 * 1000);
                                // USB_LOG_DBG("A\n");
                               ...
                               if(fifo_dma_mode == ON){
                                    if (USB_OTG_OUTEP(ep_idx)->DOEPDMA !=
                                        ...
                                        dcache_invalid_range((void *)(uint64_t)USB_OTG_OUTEP(ep_idx)->DOEPDMA,roundup(g_dwc2_udc.out_ep[ep_idx].actual_xfer_len, 64 /* ARCH_DMA_MINALIGN */));
                                        ...
                                        USB_LOG_INFO("[%s:%d] %p, %08x, %u\n", __FUNCTION__, __LINE__,
                                            g_dwc2_udc.out_ep[ep_idx].xfer_buf, USB_OTG_OUTEP(ep_idx)->DOEPDMA,
                                            g_dwc2_udc.out_ep[ep_idx].actual_xfer_len);
                                    } else {
                                        dcache_invalid_range( (uint64_t *)g_dwc2_udc.out_ep[ep_idx].xfer_buf,
                                                        roundup(g_dwc2_udc.out_ep[ep_idx].actual_xfer_len, 64 /* ARCH_DMA_MINALIGN */));
                                    }
                                }
                                #endif
                    ...
                    if (epint & USB_OTG_DOEPINT_STUP) {
                        #if CONFIG_USB_DWC2_DMA_ENABLE
                        if(fifo_dma_mode == ON){
                            dcache_invalid_range( (uint64_t *)&g_dwc2_udc.setup,
                                            roundup(sizeof(g_dwc2_udc.setup), 64 /* ARCH_DMA_MINALIGN */));
                        }
                        #endif
            ...
            while (ep_intr != 0U) {
                if ((ep_intr & 0x1U) != 0U) {
                    ...
                    if ((epint & USB_OTG_DIEPINT_EPDISD) == USB_OTG_DIEPINT_EPDISD) {
                        USB_LOG_DBG("ep(%d) disabled interrupt! \n", ep_idx);
                        USB_OTG_INEP(ep_idx)->DIEPCTL &= ~(USB_OTG_DIEPCTL_EPDIS | USB_OTG_DIEPCTL_EPENA);
                        dwc2_flush_txfifo(ep_idx);
                    }

                    USB_OTG_INEP(ep_idx)->DIEPINT = DiepintReg | epint;
                }
                ep_intr >>= 1U;
                ep_idx++;
            }
