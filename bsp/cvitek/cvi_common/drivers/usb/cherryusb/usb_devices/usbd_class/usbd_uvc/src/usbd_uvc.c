...
static int av_session_init_flag = false;
static rt_atomic_t uvc_pause_flag  = false;
static rt_atomic_t uvc_pause_done  = false;
...
static int is_media_info_update(struct uvc_device_info* info)
{
...
int uvc_index_of_vs_intf(uint8_t intf)
{
...
static void uvc_parse_media_info(uint8_t intf, uint8_t bFormatIndex, uint8_t bFrameIndex)
{
...
#endif
    default:
        rt_kprintf(" cur uvc_index not suppert!\n");
        break;
    }
    ...
    if (bFormatIndex < 0 || bFormatIndex > uvcout_format_cnt) {
        rt_kprintf("format_cnt =%d, format %d error!\r\n", uvcout_format_cnt, bFormatIndex);
        return;
    }
    ...
    if (bFrameIndex < 0 || bFrameIndex > nframes) {
        rt_kprintf("nframes = %d, frame %d error!\r\n", nframes, bFrameIndex);
        return;
    }
...
{
    // rt_kprintf("%d bytes of data sent at ep(%d)\n", nbytes, ep);
    struct uvc_device_info* uvc = uvc_container_of_ep(ep);
    uvc->tx_frm_idx++;
    uvc->xfer_flag = false;
}

void usbd_video_frame_submmit(struct rt_work *work, void *work_data)
{
    struct uvc_device_info* uvc = (struct uvc_device_info*)work_data;
...
void usbd_video_commit_set_cur(uint8_t intf, struct video_probe_and_commit_controls* commit)
{
    ...
    rt_kprintf("commit format idx:%d, frame idx:%d\n", commit->bFormatIndex,
                     commit->bFrameIndex);
}
...
// #endif /* (CONFIG_APP_VENC_SUPPORT) */
    ...
    if (uvc->streaming_on && packets > 0) {
        ...
        uvc->tx_cnt++;
#if CONFIG_USB_BULK_UVC
        rt_workqueue_submit_work(uvc->uvc_workqueue, &uvc->uvc_frame_submmit, 0);
#else
...
static void* send_to_uvc(void* arg)
{
    ...
    while (av_session_init_flag) {
        if (ret = rt_hw_atomic_load(&uvc_pause_flag)) {
            rt_atomic_add(&uvc_pause_done,1);
            while (rt_hw_atomic_load(&uvc_pause_done)) {
                rt_thread_mdelay(1);
            }
...
int uvc_init(void)
{
    ...
    for (uint8_t i = 0; i < USBD_UVC_NUM; i++) {
        ...
        uvc[i].uvc_workqueue = rt_workqueue_create(workqueuename[i], 4096, 15);
        RT_ASSERT(uvc[i].uvc_workqueue);
        rt_work_init(&uvc[i].uvc_frame_submmit, usbd_video_frame_submmit, &uvc[i]);
...
void uvc_switch(int argc, char** argv)
{
    ...
    rt_atomic_add(&uvc_pause_flag,1);
    while (!rt_hw_atomic_load(&uvc_pause_done)) {
        rt_thread_mdelay(1);
    }
    ...
    rt_atomic_sub(&uvc_pause_flag, 1);
    rt_atomic_sub(&uvc_pause_done, 1);
}
MSH_CMD_EXPORT_ALIAS(uvc_switch, uvc_switch, uvc_switch);
...
MSH_CMD_EXPORT_ALIAS(_uvc_dbg_proc_show, proc_uvc, dump uvc debug info);