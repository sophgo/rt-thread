...
CVI_S32 CVI_GDC_AddCorrectionTaskCNV(GDC_HANDLE hHandle, const GDC_TASK_ATTR_S *pstTask, const FISHEYE_ATTR_S *pstFishEyeAttr, uint8_t *p_tbl, uint8_t *p_idl, uint32_t *tbl_param)
{
    CHECK_GDC_FORMAT(pstTask->stImgIn, pstTask->stImgOut);
    UNUSED(hHandle);
                           ...
                           const char *name, CVI_U64 *pu64PhyAddr, CVI_VOID **ppVirAddr)
{
...
#endif
    vaddr = rt_malloc_align(mesh_size + DEFAULT_ALIGN, DEFAULT_ALIGN);
    if (vaddr == NULL)
    {
...
#if TEST_LOAD_MESH
    FILE *fp = fopen("/mnt/sd/mesh_1920_1088_100_100_0_0_-300.bin", "rb");
    if (!fp)
    {
    ...
    fread((void *)ALIGN((CVI_U64)vaddr, DEFAULT_ALIGN), mesh_size, 1, fp);
    fflush(fp);
    fclose(fp);
#else
    memset(vaddr, 0, mesh_size + DEFAULT_ALIGN);

    if (mesh_gen_ldc(in_size, out_size, pstLDCAttr, ALIGN(paddr, DEFAULT_ALIGN), (void *)ALIGN((CVI_U64)vaddr, DEFAULT_ALIGN), enRotation) != CVI_SUCCESS)
    {
        CVI_SYS_IonFree(paddr, vaddr);
        CVI_TRACE_GDC(CVI_DBG_ERR, "Can't generate ldc mesh.\n");
        return CVI_ERR_GDC_ILLEGAL_PARAM;
    }
    CVI_SYS_IonInvalidateCache(paddr, vaddr, mesh_size + DEFAULT_ALIGN);
#endif

    CVI_TRACE_GDC(CVI_DBG_INFO, "load mesh size:%d, mesh phy addr:%llx, vir addr:%p.\n", mesh_size, paddr, vaddr);
...
CVI_S32 CVI_GDC_DumpMesh(MESH_DUMP_ATTR_S *pMeshDumpAttr)
{
    ...
    case CVI_ID_VI:
        ...
        // phyMesh = vi_mesh[viChn].paddr;
        // virMesh = vi_mesh[viChn].vaddr;
        // s32Ret = CVI_VI_GetChnAttr(0, viChn, &stChnAttr);
        // if (s32Ret != CVI_SUCCESS) {
        // 	CVI_TRACE_GDC(CVI_DBG_ERR, "vi_sdk_get_chn_attr ioctl failed. errno 0x%x\n", s32Ret);
        // 	return s32Ret;
        // }
    ...
    virMesh = (CVI_VOID *)ALIGN((CVI_U64)virMesh, DEFAULT_ALIGN);

    CVI_TRACE_GDC(CVI_DBG_ERR, "dump mesh size:%d, mesh phy addr:%#" PRIx64 ", vir addr:%p.\n",
                  meshSize, phyMesh, virMesh);

    fp = open(filePath, O_CREAT | O_TRUNC | O_RDWR);
    if (!fp)
    {
        CVI_TRACE_GDC(CVI_DBG_ERR, "open file:%s failed.\n", filePath);
        return CVI_ERR_GDC_ILLEGAL_PARAM;
    }
    write(fp, virMesh, meshSize);
    fsync(fp);
    close(fp);
    return CVI_SUCCESS;
}
...
CVI_S32 CVI_GDC_LoadMesh(MESH_DUMP_ATTR_S *pMeshDumpAttr, const LDC_ATTR_S *pstLDCAttr)
{
    ...
    off_t fileSize = lseek(fp, 0, SEEK_END); // Move to the end of the file
    lseek(fp, 0, SEEK_SET);                  // Move back to the beginning of the file
    ...
    if (pmesh->vaddr)
    {
        rt_free_align(pmesh->vaddr);
    }

    // acquire memory space for mesh.
    virMesh = rt_malloc_align(mesh_size + DEFAULT_ALIGN, DEFAULT_ALIGN);
    if (virMesh == NULL)
    {
        CVI_TRACE_GDC(CVI_DBG_ERR, "Can't acquire memory for gdc mesh.\n");
        return CVI_ERR_GDC_NOMEM;
    }
    phyMesh = (CVI_U64)virMesh;
    ...
    read(fp, virMesh, mesh_size);
    CVI_SYS_IonFlushCache(phyMesh, virMesh, mesh_size);
    CVI_SYS_IonInvalidateCache(phyMesh, virMesh, mesh_size + DEFAULT_ALIGN);

    switch (mod)
    {
    case CVI_ID_VI:
        // vi_mesh[viChn].meshSize = mesh_size;
        ...
        // if (vi_sdk_set_chn_ldc(0, &vi_cfg) != CVI_SUCCESS) {
    ...
    case CVI_ID_VPSS:
        ...
        if (vpss_set_chn_ldc(&vpss_cfg) != CVI_SUCCESS)
        {
