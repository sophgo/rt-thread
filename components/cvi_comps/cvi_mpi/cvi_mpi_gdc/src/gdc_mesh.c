...
#define minmax(a, b, c) (((a) < (b)) ? (b) : ((a) > (c)) ? (c) \
                                                         : (a))
...
static void mesh_gen_get_1st_size(SIZE_S in_size, CVI_U32 *mesh_1st_size)
{
    if (!mesh_1st_size)
    {
        CVI_TRACE_GDC(CVI_DBG_ERR, "invalid param for 1st\n");
        return;
    }
    ...
    *mesh_1st_size = sizeof(struct COORD2D_INT_HW) * MESH_NUM_ATILE * MESH_NUM_ATILE * num_tilex_s1 * num_tiley_s1 * 4; // 4 = 4 knots in a mesh
}
...
void mesh_gen_get_2nd_size(SIZE_S in_size, CVI_U32 *mesh_2nd_size)
{
    ...
    *mesh_2nd_size = sizeof(struct COORD2D_INT_HW) * MESH_NUM_ATILE * MESH_NUM_ATILE * num_tilex_s2 * num_tiley_s2 * 4; // 4 = 4 knots in a mesh
}
                       ...
                       CVI_U32 *mesh_2nd_size)
{
    ...
    {
        CVI_TRACE_GDC(CVI_DBG_ERR, "invalid param for 1st/2nd\n");
        return;
    }
    (void)out_size;
                                      ...
                                      uint32_t *offset_p)
{
    ...
    for (int midy = 0; midy < MESH_NUM_ATILE; midy++)
    {
        for (int midx = 0; midx < MESH_NUM_ATILE; midx++)
        {
            ...
            {
                CVI_TRACE_GDC(CVI_DBG_ERR, "offset:%d invalid, large than :%d\n", offset, mesh_1st_size - 2);
                                          ...
                                          CVI_U32 mesh_1st_size)
{
    ...
    for (int tidy = 0; tidy < num_tiley_s1; tidy++)
    {
        for (int tidx = 0; tidx < num_tilex_s1; tidx++)
        {
            ...
            if (ret != CVI_SUCCESS)
            {
                CVI_TRACE_GDC(CVI_DBG_ERR, "_convert_1st_src_midxy fail, ret:%d\n", ret);
                return ret;
            }
            // # 256 bytes per tile
                                  ...
                                  int tidx, int tidy, int mesh_horcnt, int mesh_vercnt, const LDC_ATTR_S *pstLDCAttr)
{
    ...
    for (int midy = 0; midy < MESH_NUM_ATILE; midy++)
    {
        for (int midx = 0; midx < MESH_NUM_ATILE; midx++)
        {
            ...
            if (pstLDCAttr->stGridInfoAttr.bEnable)
            {
            ...
            else
            {
                ...
                for (int knotidx = 0; knotidx < 4; knotidx++)
                {
                    ...
                    uint32_t offset =
                        (yidx * (num_tilex_s1 * MESH_NUM_ATILE) + xidx) * 4 + knotidx;
...
static CVI_S32 _offline_get_1st_src_mesh_table(int num_tiley_s1, int num_tilex_s1, LDC_ATTR *cfg, COORD2D_INT_HW *src_1st_list,
    ...
    for (CVI_U32 meshidx = 0; meshidx < max_mesh_num; meshidx++)
    {
        for (int knotidx = 0; knotidx < 4; knotidx++)
        {
            dstMesh[meshidx].knot[knotidx].xcor =
                RMATRIX[0][0] * (cfg->DstRgnMeshInfoExt[meshidx].knot[knotidx].xcor) + RMATRIX[0][1] * (cfg->DstRgnMeshInfoExt[meshidx].knot[knotidx].ycor);

            dstMesh[meshidx].knot[knotidx].ycor =
                RMATRIX[1][0] * (cfg->DstRgnMeshInfoExt[meshidx].knot[knotidx].xcor) + RMATRIX[1][1] * (cfg->DstRgnMeshInfoExt[meshidx].knot[knotidx].ycor);

            srcMesh[meshidx].knot[knotidx].xcor =
                RMATRIX[0][0] * (cfg->SrcRgnMeshInfoExt[meshidx].knot[knotidx].xcor) + RMATRIX[0][1] * (cfg->SrcRgnMeshInfoExt[meshidx].knot[knotidx].ycor);

            srcMesh[meshidx].knot[knotidx].ycor =
                RMATRIX[1][0] * (cfg->SrcRgnMeshInfoExt[meshidx].knot[knotidx].xcor) + RMATRIX[1][1] * (cfg->SrcRgnMeshInfoExt[meshidx].knot[knotidx].ycor);
            ...
            for (int midy = 0; midy < MESH_NUM_ATILE; midy++)
            {
                for (int midx = 0; midx < MESH_NUM_ATILE; midx++)
                {
                    ...
                    ret = _fill_src_2nd_list(num_tilex_s2, cfg, src_2nd_list,
                                             tidx, tidy, midx, midy, dstMesh, cur_dst_mesh,
                                             swmesh_hit_index, mesh_horcnt, mesh_vercnt);
                    if (ret != CVI_SUCCESS)
                    {
                        CVI_TRACE_GDC(CVI_DBG_ERR, "_fill_src_2nd_list fail, ret=%d\n", ret);
                        return ret;
                    }
                }
            }
                                         ...
                                         int view_h, int mesh_horcnt, int mesh_vercnt, int rgn_idx)
{
    ...
    for (int j = 0; j < mesh_vercnt; j++)
    {
        ...
        if (j && 0 != (rgn_attr[rgn_idx].DstRgnMeshInfo[j * mesh_horcnt].knot[0].ycor - rgn_attr[rgn_idx].DstRgnMeshInfo[(j - 1) * mesh_horcnt].knot[3].ycor))
        {
    ...
    for (int reorder_meshidx = 0; reorder_meshidx < 9 * (mesh_horcnt * mesh_vercnt); reorder_meshidx++)
    {
        ...
        for (int knotidx = 0; knotidx < 4; knotidx++)
        {
            ...
            rgn_attr[0].DstRgnMeshInfoExt[reorder_meshidx].knot[knotidx].xcor =
                rgn_attr[0].DstRgnMeshInfo[ori_idy * mesh_horcnt + ori_idx].knot[knotidx].xcor + ext_idx * (view_w - 1);

            rgn_attr[0].DstRgnMeshInfoExt[reorder_meshidx].knot[knotidx].ycor =
                rgn_attr[0].DstRgnMeshInfo[ori_idy * mesh_horcnt + ori_idx].knot[knotidx].ycor + ext_idy * (view_h - 1);
        }
    }
                                  ...
                                  int swmesh_hit_index, int stageID)
{
    ...
    INTER_01.xcor = cur_dst_sw_mesh.knot[(0 + stageID) % 4].xcor + (cur_dst_sw_mesh.knot[(1 + stageID) % 4].xcor - cur_dst_sw_mesh.knot[(0 + stageID) % 4].xcor) * (x01_a) / (x01_a + x01_b);
    INTER_01.ycor = cur_dst_sw_mesh.knot[(0 + stageID) % 4].ycor + (cur_dst_sw_mesh.knot[(1 + stageID) % 4].ycor - cur_dst_sw_mesh.knot[(0 + stageID) % 4].ycor) * (x01_a) / (x01_a + x01_b);
    INTER_32.xcor = cur_dst_sw_mesh.knot[(3 + stageID) % 4].xcor + (cur_dst_sw_mesh.knot[(2 + stageID) % 4].xcor - cur_dst_sw_mesh.knot[(3 + stageID) % 4].xcor) * (x01_a) / (x01_a + x01_b);
    INTER_32.ycor = cur_dst_sw_mesh.knot[(3 + stageID) % 4].ycor + (cur_dst_sw_mesh.knot[(2 + stageID) % 4].ycor - cur_dst_sw_mesh.knot[(3 + stageID) % 4].ycor) * (x01_a) / (x01_a + x01_b);
    ...
    INTER_01_SRC.xcor = cur_src_sw_mesh.knot[(0 + stageID) % 4].xcor + (cur_src_sw_mesh.knot[(1 + stageID) % 4].xcor - cur_src_sw_mesh.knot[(0 + stageID) % 4].xcor) * (x01_a) / (x01_a + x01_b);
    INTER_32_SRC.xcor = cur_src_sw_mesh.knot[(3 + stageID) % 4].xcor + (cur_src_sw_mesh.knot[(2 + stageID) % 4].xcor - cur_src_sw_mesh.knot[(3 + stageID) % 4].xcor) * (x01_a) / (x01_a + x01_b);
                                      ...
                                      double x0, double y0, meshdata_all *meshdata, const LDC_ATTR_S *pstLDCAttr)
{
    ...
    else
    {
        for (int i = 0; i < 9 * (mesh_horcnt * mesh_vercnt); i++)
        {
            ...
            for (int knotidx = 0; knotidx < 4; knotidx++)
            {
                double x = rgn_attr[rgn_idx].DstRgnMeshInfoExt[i].knot[knotidx].xcor - rgn_attr[rgn_idx].OutX - rgn_attr[rgn_idx].OutW / 2;
                double y = rgn_attr[rgn_idx].DstRgnMeshInfoExt[i].knot[knotidx].ycor - rgn_attr[rgn_idx].OutY - rgn_attr[rgn_idx].OutH / 2;
                                 ...
                                 int y, int inc_zero)
{
    ...
    // A(x1, y1) to B(x2, y2), P(x0, y0) outsize the line
                     ...
                     void *mesh_vir_addr, ROTATION_E rot)
{
    ...
    if (pstLDCAttr->stGridInfoAttr.bEnable)
    {
        printf("pstLDCAttr->stGridInfoAttr.gridFileName is %s\n", pstLDCAttr->stGridInfoAttr.gridFileName);
        load_meshdata(pstLDCAttr->stGridInfoAttr.gridFileName, &gdc_meshdata, pstLDCAttr->stGridInfoAttr.gridBindName);
    ...
    if (!src_1st_list || !src_2nd_list || !cfg || !rgn_attr || !cfg->DstRgnMeshInfo || !cfg->SrcRgnMeshInfo || !cfg->DstRgnMeshInfoExt || !cfg->SrcRgnMeshInfoExt || !cfg->DstRgnMeshInfoExt2ND || !cfg->SrcRgnMeshInfoExt2ND || !cfg->DstRgnMeshInfo_1st || !cfg->SrcRgnMeshInfo_1st || !cfg->DstRgnMeshInfo_2nd || !cfg->SrcRgnMeshInfo_2nd || !rgn_attr->DstRgnMeshInfo || !rgn_attr->DstRgnMeshInfoExt || !rgn_attr->SrcRgnMeshInfo || !rgn_attr->SrcRgnMeshInfoExt)
    {
        free(src_1st_list);
        free(src_2nd_list);
    ...
    for (int rgn_idx = 0; rgn_idx < cfg->RgnNum; rgn_idx++)
    {
        ...
        if (ret)
        {
            CVI_TRACE_GDC(CVI_DBG_ERR, "_get_region_dst_mesh_list fail, ret:%d\n", ret);
            return CVI_FAILURE;
        }
    ...
    if (ret)
    {
        CVI_TRACE_GDC(CVI_DBG_ERR, "_offline_get_1st_src_mesh_table fail, ret:%d\n", ret);
        return CVI_FAILURE;
    }
#ifdef ENABLE_PROFILE
    clock_gettime(CLOCK_MONOTONIC, &end);
    ...
    if (ret)
    {
        CVI_TRACE_GDC(CVI_DBG_ERR, "_offline_get_2nd_src_mesh_table fail, ret:%d\n", ret);
        return CVI_FAILURE;
    }
    ...
    clock_gettime(CLOCK_MONOTONIC, &start);
#endif
    ret = convert_1st_src_mesh_table(num_tiley_s1, num_tilex_s1, src_1st_list, src_1st_list_1d, mesh_1st_size);
    if (ret)
    {
        CVI_TRACE_GDC(CVI_DBG_ERR, "convert_1st_src_mesh_table fail, ret:%d\n", ret);
        return CVI_FAILURE;
    }
...
#ifdef ENABLE_PROFILE
    clock_gettime(CLOCK_MONOTONIC, &end);
    CVI_TRACE_GDC(CVI_DBG_WARN, "    convert_2nd_src_mesh_table: %ldms\n",
                  get_diff_in_us(start, end) / 1000);
#endif
