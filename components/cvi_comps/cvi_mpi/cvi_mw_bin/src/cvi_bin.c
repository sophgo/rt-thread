...
// following info is V1.0 and V1.1 bin header field. difference of V1.0 and V1.1 is end of  V1.1's
// file that contains the whole file PQbin's md5 value.
...
//[sensorName3 55]  + [sensorName4 55] + [isp branch 20] + [version 4] + [generate mode - A:auto
//M:Manual 1]
...
#define SUPPORT_VI_MAX_PIPE_NUM \
    4  // Force change to 4 Instead of using VI_MAX_PIPE_NUM (now it's 6).
/****************************************************************************/
struct BIN_BUF_INFO {
...
static CVI_U32             g_u32CurIspBinSize        = 0;
static CVI_U32             g_u32CurIspJsonInitSize   = 0;
static CVI_U32             g_u32CurIspJsonCompreSize = 0;

static char userBinName[WDR_MODE_MAX][BIN_FILE_LENGTH] = {
    "/mnt/sd/cvi_sdr_bin", "/mnt/sd/cvi_sdr_bin", "/mnt/sd/cvi_sdr_bin", "/mnt/sd/cvi_wdr_bin",
    "/mnt/sd/cvi_wdr_bin", "/mnt/sd/cvi_wdr_bin", "/mnt/sd/cvi_wdr_bin", "/mnt/sd/cvi_wdr_bin",
    "/mnt/sd/cvi_wdr_bin", "/mnt/sd/cvi_wdr_bin", "/mnt/sd/cvi_wdr_bin", "/mnt/sd/cvi_wdr_bin"};
...
static CVI_S32 get_bin_Info_from_buf(CVI_U8 *buf, struct BIN_BUF_INFO *pstBufInfo)
{
    ...
    if (strncmp(pstBufInfo->achBinVersion, PQBIN_VERSION_V11, PQBINVERSION_SIZE) >= 0) {
        strncpy(pstBufInfo->achTotalParaMD5Value, (CVI_CHAR *)buf + pstBufInfo->u32ParaTotalSize,
                MD5_STRING_LEN);
...
static CVI_S32 check_sensor_num_is_exceeded(void)
{
    ...
    for (CVI_U32 idx = CVI_BIN_ID_ISP0; idx <= CVI_BIN_ID_ISP3; idx++) {
        if (CVI_VI_GetDevState(idx - CVI_BIN_ID_ISP0) == CVI_SUCCESS) {
            u32CurSensorNum++;
        }
    }
...
static CVI_S32 check_is_register_id(enum CVI_BIN_SECTION_ID id)
{
    if (id_is_valid_IspID(id) == CVI_SUCCESS) {
        if (CVI_VI_GetDevState(id - CVI_BIN_ID_ISP0) != CVI_SUCCESS) {
...
static enum CVI_BIN_SECTION_ID get_existent_id_in_bin(enum CVI_BIN_SECTION_ID idx,
                                                      CVI_BIN_HEADER         *pstHeader)
{
...
static CVI_S32 reset_id_size_in_bin(enum CVI_BIN_SECTION_ID src_id, enum CVI_BIN_SECTION_ID dst_id,
                                    const CVI_U8 *buf)
{
    CVI_BIN_HEADER *pstHeader = (CVI_BIN_HEADER *)buf;
...
static CVI_S32 restore_init_id_size_in_bin(enum CVI_BIN_SECTION_ID src_id,
                                           enum CVI_BIN_SECTION_ID dst_id, const CVI_U8 *buf)
{
    CVI_BIN_HEADER *pstHeader = (CVI_BIN_HEADER *)buf;
    CVI_U8         *addr      = (CVI_U8 *)buf;
...
CVI_U32 CVI_BIN_GetBinTotalLen(void)
{
    ...
    CVI_S32         bin_is_single_mode = isp_bin_is_single_mode();
    ...
    for (CVI_U32 idx = CVI_BIN_ID_MIN; idx < CVI_BIN_ID_MAX; idx++) {
        if (idx == CVI_BIN_ID_HEADER) {
            ...
            if ((getBinSizeFunc[idx] != NULL) && (check_is_register_id(idx) == CVI_SUCCESS)) {
                CVI_JSON_SaveParamToBuffer(NULL, idx, &stJsonHeader.size[idx],
                                           setParamToJsonBuf[idx], 0);
            }
#endif
        }
    }
    ...
    for (CVI_U32 idx = CVI_BIN_ID_MIN; idx < CVI_BIN_ID_MAX; idx++) {
        if (bin_is_single_mode && (idx > CVI_BIN_ID_ISP0) && (idx <= CVI_BIN_ID_ISP3)) {
            continue;
        }
        if ((getBinSizeFunc[idx] != NULL) && (check_is_register_id(idx) == CVI_SUCCESS)) {
...
CVI_S32 CVI_BIN_ImportBinData(CVI_U8 *pu8Buffer, CVI_U32 u32DataLength)
{
    ...
    CVI_BOOL        bBlkSizeInvalid  = CVI_TRUE;
    ...
    for (CVI_U32 idx = CVI_BIN_ID_MIN; idx < CVI_BIN_ID_MAX; idx++) {
        if ((getParamFromBinFunc[idx] != NULL) && (check_is_register_id(idx) == CVI_SUCCESS)) {
            ...
            idx_t =
                bin_is_single_mode
                    ? (idx > CVI_BIN_ID_ISP0) && (idx <= CVI_BIN_ID_ISP3) ? CVI_BIN_ID_ISP0 : idx
                    : idx;
            bBlkSizeInvalid = pstHeader->size[idx_t] == 0 ? CVI_TRUE : CVI_FALSE;
            if (bBlkSizeInvalid) {
#if CONFIG_PQBIN_USE_JSON
                ...
                ret = getParamFromBinFunc[idx](pu8BlockAddr, pstHeader->size[idx_t]);
                if (ret != CVI_SUCCESS) {
#if CONFIG_PQBIN_USE_JSON
...
CVI_S32 CVI_BIN_LoadParamFromBin(enum CVI_BIN_SECTION_ID id, CVI_U8 *buf)
{
    ...
    const CVI_CHAR *pchModuleName[CVI_BIN_ID_MAX] = {
        "Header", "Sensor_0", "Sensor_1", "Sensor_2", "Sensor_3", "Vpss", "Vdec", "Venc", "Vo"};
    CVI_BIN_HEADER *pstHeader = (CVI_BIN_HEADER *)buf;
    CVI_U32         secSize =
        pstHeader->size[CVI_BIN_ID_HEADER] - sizeof(CVI_U32) - sizeof(CVI_BIN_EXTRA_S);
    ...
    if (secCnt < id) {
        ...
        if ((getParamFromBinFunc[id] != NULL) && (ret == CVI_SUCCESS)) {
            ...
            idx_t         = bin_is_single_mode
                                ? (id > CVI_BIN_ID_ISP0) && (id <= CVI_BIN_ID_ISP3) ? CVI_BIN_ID_ISP0 : id
                                : id;
            bBlkSizeValid = (pstHeader->size[idx_t] == 0 ? CVI_FALSE : CVI_TRUE);
            if (bBlkSizeValid) {
                ret = getParamFromBinFunc[id](addr, pstHeader->size[idx_t]);
                if (ret != CVI_SUCCESS) {
#if CONFIG_PQBIN_USE_JSON
                    CVI_TRACE_SYS(CVI_DBG_WARN, "Get cur block(%d) para from json!\n", id);
                    ret = get_json_para_from_buffer(id, buf);
#else
                    CVI_TRACE_SYS(CVI_DBG_WARN, "Get cur block(%d) err=%08x & json inexistence!\n",
                                  id, ret);
...
CVI_S32 CVI_BIN_ExportSingleISPBinData(enum CVI_BIN_SECTION_ID id, CVI_U8 *pu8Buffer,
...
{
    isp_bin_en_single_mode();
    return CVI_SUCCESS;
}

CVI_S32 CVI_BIN_DisSingleMode(void)
{
    isp_bin_dis_single_mode();
    return CVI_SUCCESS;
}
