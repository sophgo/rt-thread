                                     ...
                                     pfn_cvi_json_getparamfrombuf pFuncgetParam)
{
    ...
    if (ret != CVI_SUCCESS) {
        ...
        CVI_TRACE_SYS(CVI_DBG_ERR, "Invalid id(%d)\n", id);
        goto ERROR_HANDLER;
    }

    if (buf == NULL) {
        ret = CVI_BIN_NULL_POINT;
        CVI_TRACE_SYS(CVI_DBG_ERR, "input buffer pointer is null!");
        goto ERROR_HANDLER;
    }
    if (pFuncgetParam == NULL) {
        ret = CVI_BIN_NULL_POINT;
        CVI_TRACE_SYS(CVI_DBG_ERR, "get json callback isn't registered!");
        goto ERROR_HANDLER;
    }
    ...
    if (iResult != Z_OK) {
        ...
        CVI_TRACE_SYS(CVI_DBG_ERR, "block(%d) Uncompress abnormally! abnormal value is:%d\n", id,
                      iResult);
        goto ERROR_HANDLER;
    }
...
CVI_S32 CVI_JSON_SaveParamToBuffer(unsigned char *buffer, enum CVI_BIN_SECTION_ID id,
                                   CVI_JSON_INFO             *pJsonInfo,
                                   pfn_cvi_json_setparamtobuf pFuncsetParam,
                                   CVI_U32                    u32FreeSpaceSize)
{
    CVI_S32 ret                = CVI_SUCCESS;
    CVI_S8 *buf                = NULL;
    ...
    if (pJsonInfo == NULL) {
        ret = CVI_BIN_NULL_POINT;
        CVI_TRACE_SYS(CVI_DBG_ERR, "input pJsonInfo pointer is null!");
        goto ERROR_HANDLER;
    }
    if (pFuncsetParam == NULL) {
        ret = CVI_BIN_NULL_POINT;
        CVI_TRACE_SYS(CVI_DBG_ERR, "set json callback isn't registered!");
        goto ERROR_HANDLER;
    }
    ...
    if (iResult != Z_OK) {
        ...
        CVI_TRACE_SYS(CVI_DBG_ERR, "block(%d) Compress abnormally! abnormal value is:%d\n", id,
                      iResult);
        goto ERROR_HANDLER;
    }
                                 ...
                                 pfn_cvi_json_setparamtobuf pFuncsetParam)
{
    ...
    if (size != 1) {
        ...
        CVI_TRACE_SYS(CVI_DBG_WARN, "Update block(%d) para fail!\n", id);
    }
