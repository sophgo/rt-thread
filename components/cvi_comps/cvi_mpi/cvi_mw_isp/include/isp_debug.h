...
#define ISP_DEBUG(level, fmt, ...)                                                           \
    do {                                                                                     \
        if (level <= 5) {                                                                    \
            printf("#[%c] %s::%d : " fmt, ISP_DEBUG_LEVEL_SYMBOL[level], __func__, __LINE__, \
                   ##__VA_ARGS__);                                                           \
        }                                                                                    \
    } while (0)
#else
#define ISP_DEBUG(level, fmt, ...)                                                                \
    do {                                                                                          \
        if (level <= gDebugLevel) {                                                               \
            if (level <= LOG_NOTICE) {                                                            \
                CVI_TRACE(level, CVI_ID_ISP, "[%c] %s::%d : " fmt, ISP_DEBUG_LEVEL_SYMBOL[level], \
