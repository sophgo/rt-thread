...
#define CHECK_DEV_VALUE(dev)                                          \
    do {                                                              \
        if (((dev) < 0) || ((dev) >= VI_MAX_DEV_NUM)) return -ENODEV; \
    } while (0)

#define ISP_IOCTL_ERR(fd, ec1)                                                               \
    ISP_LOG_ERR("IOCTL error fd = %d value = %d ptr = %p id = %x err = %s\n", fd, ec1.value, \
                ec1.ptr, ec1.id, strerror(errno))

#define ISP_CHECK_MIN(val, min)                                             \
    do {                                                                    \
        if ((val) < (min)) {                                                \
            (val) = (min);                                                  \
...
#define ISP_CHECK_RANGE(val, min, max) \
    do {                               \
...
#define ISP_VALUE_CHECK_ERR_LOG_V(param, value) ISP_DEBUG(LOG_ERR, "Invalid value : 0x%x\n", value)
#define ISP_VALUE_CHECK_ERR_LOG_A1(param, index1, value) \
...
#define CHECK_VALID_ARRAY_1D(_struct, _param, _idx1, _lower_bound, _upper_bound)                  \
    {                                                                                             \
        for (CVI_U32 _idx = 0; _idx < _idx1; _idx++) {                                            \
            if ((_struct->_param[_idx] > _upper_bound) ||                                         \
                ((_struct->_param[_idx] + 1) < (_lower_bound + 1))) {                             \
    ...
    {                                                                                   \
        for (CVI_U32 _i = 0; _i < _idx1; _i++) {                                        \
            for (CVI_U32 _j = 0; _j < _idx2; _j++) {                                    \
                if ((_struct->_param[_i][_j] > _upper_bound) ||                         \
                    ((_struct->_param[_i][_j] + 1) < (_lower_bound + 1))) {             \
                    ISP_VALUE_CHECK_ERR_LOG_A2(STRING(_struct->_param), _i, _j,         \
                                               _struct->_param[_i][_j]);                \
                    ret = CVI_FAILURE_ILLEGAL_PARAM;                                    \
                }                                                                       \
            }                                                                           \
        }                                                                               \
            ...
            for (CVI_U32 _j = 0; _j < _idx2; _j++) {                                           \
                for (CVI_U32 _k = 0; _k < _idx3; _k++) {                                       \
                    if ((_struct->_param[_i][_j][_k] > _upper_bound) ||                        \
                        ...
                        ISP_VALUE_CHECK_ERR_LOG_A3(STRING(_struct->_param), _i, _j, _k,        \
                                                   _struct->_param[_i][_j][_k]);               \
                        ret = CVI_FAILURE_ILLEGAL_PARAM;                                       \
                    }                                                                          \
                }                                                                              \
            }                                                                                  \
...
#define CHECK_VALID_AUTO_LV_2D(_struct, _param, _idx1, _lower_bound, _upper_bound)         \
    do {                                                                                   \
        ...
        CHECK_VALID_ARRAY_2D(_struct, stAuto._param, _idx1, ISP_AUTO_LV_NUM, _lower_bound, \
                             _upper_bound);                                                \
    } while (0)
...
#define CHECK_VALID_AUTO_LV_3D(_struct, _param, _idx1, _idx2, _lower_bound, _upper_bound)         \
    do {                                                                                          \
        ...
        CHECK_VALID_ARRAY_3D(_struct, stAuto._param, _idx1, _idx2, ISP_AUTO_LV_NUM, _lower_bound, \
                             _upper_bound);                                                       \
    } while (0)
...
#define CHECK_VALID_AUTO_ISO_1D(_struct, _param, _lower_bound, _upper_bound)                  \
    do {                                                                                      \
        CHECK_VALID_CONST(_struct, stManual._param, _lower_bound, _upper_bound);              \
        CHECK_VALID_ARRAY_1D(_struct, stAuto._param, ISP_AUTO_ISO_STRENGTH_NUM, _lower_bound, \
                             _upper_bound);                                                   \
    } while (0)
...
#define CHECK_VALID_AUTO_ISO_2D(_struct, _param, _idx1, _lower_bound, _upper_bound)        \
    do {                                                                                   \
        CHECK_VALID_ARRAY_1D(_struct, stManual._param, _idx1, _lower_bound, _upper_bound); \
        CHECK_VALID_ARRAY_2D(_struct, stAuto._param, _idx1, ISP_AUTO_ISO_STRENGTH_NUM,     \
...
#define CHECK_VALID_AUTO_ISO_3D(_struct, _param, _idx1, _idx2, _lower_bound, _upper_bound)        \
    do {                                                                                          \
        CHECK_VALID_ARRAY_2D(_struct, stManual._param, _idx1, _idx2, _lower_bound, _upper_bound); \
        CHECK_VALID_ARRAY_3D(_struct, stAuto._param, _idx1, _idx2, ISP_AUTO_ISO_STRENGTH_NUM,     \
                             _lower_bound, _upper_bound);                                         \
    } while (0)

#define ISP_RELEASE_MEMORY(x) \
    do {                      \
        if (x) {              \
            free(x);          \
            x = 0;            \
...
#define ISP_MALLOC(size)                                     \
    ({                                                       \
        ...
        if (!memory) ISP_LOG_ERR("can't allocate memory\n"); \
        memory;                                              \
    })
...
#define ISP_FORLOOP_SET(target, source, length)                     \
    do {                                                            \
        for (CVI_U32 i = 0; i < length; i++) target[i] = source[i]; \
    } while (0)
