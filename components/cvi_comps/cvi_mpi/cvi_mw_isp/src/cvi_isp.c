...
#define CVI_EVENT_TIMEOUT_MS 10
static CVI_BOOL g_isp_thread_run = CVI_FALSE;
    ...
    if (strcmp(TRIGGER_AE_LOG_CMD, command) == 0) {
        ...
        for (CVI_U32 u32PipeID = 0; u32PipeID < VI_MAX_PIPE_NUM; ++u32PipeID) {
            if (aeAlgoInternalLibReg[u32PipeID].pfn_ae_dumplog != NULL) {
                aeAlgoInternalLibReg[u32PipeID].pfn_ae_setdumplogpath(command +
                                                                      ae_log_location_len);
            }
        }
        ...
        for (CVI_U32 u32PipeID = 0; u32PipeID < VI_MAX_PIPE_NUM; ++u32PipeID) {
            if (awbAlgoInternalLibReg[u32PipeID].pfn_awb_dumplog != NULL) {
                awbAlgoInternalLibReg[u32PipeID].pfn_awb_setdumplogpath(command +
                                                                        awb_log_location_len);
            }
        }
    }
...
CVI_S32 get_isp_check_event(void)
{
    G_EXT_CTRLS_PTR(VI_IOCTL_CHECK_ISP_EVENT, NULL);
    return CVI_SUCCESS;
}
CVI_S32 get_isp_de_event(struct vi_event *ev)
{
    G_EXT_CTRLS_PTR(VI_IOCTL_DQEVENT, ev);
    return CVI_SUCCESS;
}
...
CVI_S32 CVI_ISP_Run(VI_PIPE ViPipe)
{
    ...
    // struct vdev *isp_dev = get_dev_info(VDEV_TYPE_ISP, 0);
    // struct timespec event_time;
    struct cvi_isp_attr isp_attr;
    if (fd == -1) {
        ISP_LOG_EMERG("ISP%d fd state incorrect\n", fd);
        return -EBADF;
    }

    if (g_isp_thread_run) {
        ISP_LOG_ERR("ISP thread already running, Dev %d return\n", ViPipe);
        return CVI_SUCCESS;
    }
    ...
    if (initial_ipc_fifo(&ipc_fifo_fd, fifo_path) != CVI_SUCCESS)
        ISP_LOG_EMERG("Enable ISP ipc fail\n");

    if (ipc_fifo_fd) fFifo = fdopen(ipc_fifo_fd, "r");
#endif  // ISP_RECEIVE_IPC_CMD
    isp_proc_run();
    ...
    long            intervaltime = 0;
    struct timespec ts;
    G_EXT_CTRLS_PTR(VI_IOCTL_QUERY_ISP_EVENT, &isp_attr);
    ...
    while (g_isp_thread_run) {
...
#if 1                                     // TODO: Evan
        if (get_isp_check_event() < 0) {  // queue empty
            // fprintf(stderr, "VI_IOC_G_CTRL - %s NG\n", __func__);

            ret = rt_event_recv(isp_attr.event, 0x01, RT_EVENT_FLAG_OR | RT_EVENT_FLAG_CLEAR,
                                SEM_WAIT_TIMEOUT_MS, &actl_flags);
            if (ret != 0) {
                // printf("time out\n");
                continue;
            }
...
#endif
        if (get_isp_de_event(&ev) < 0) {
            continue;
        }
        clock_gettime(CLOCK_MONOTONIC, &ts);
        intervaltime = (ts.tv_sec * 1000 + ts.tv_nsec / 1000000) -
                       (ev.timestamp.tv_sec * 1000 + ev.timestamp.tv_nsec / 1000000);
        while (labs(intervaltime) >= CVI_EVENT_TIMEOUT_MS) {
            if (get_isp_de_event(&ev) < 0) {
                ret = -1;
                break;
            }
        ...
        // printf("isp run event: %d, frame_sequence: %d, timestamp: %ldms\n",
        ...
        case VI_EVENT_POST_EOF:
            isp_mgr_post_eof(0, ev.frame_sequence, ev.lft_frm);
            break;
        case VI_EVENT_POST1_EOF:
            isp_mgr_post_eof(1, ev.frame_sequence, ev.lft_frm);
            break;
        case VI_EVENT_POST2_EOF:
            isp_mgr_post_eof(2, ev.frame_sequence, ev.lft_frm);
            break;
        }
...
CVI_S32 CVI_ISP_SetPubAttr(VI_PIPE ViPipe, const ISP_PUB_ATTR_S *pstPubAttr)
{
    ...
    ISP_LOG_INFO(
        "set pub attr, b:%d,wm:%d,(%d,%d,%d,%d),fps:%f,(%d,%d),sm:%d\n", pstPubAttr->enBayer,
        pstPubAttr->enWDRMode, pstPubAttr->stWndRect.s32X, pstPubAttr->stWndRect.s32Y,
        pstPubAttr->stWndRect.u32Width, pstPubAttr->stWndRect.u32Height, pstPubAttr->f32FrameRate,
        pstPubAttr->stSnsSize.u32Width, pstPubAttr->stSnsSize.u32Height, pstPubAttr->u8SnsMode);
...
CVI_S32 CVI_ISP_SetBindAttr(VI_PIPE ViPipe, const ISP_BIND_ATTR_S *pstBindAttr)
{
    ...
    CVI_U32    i;
    ISP_CTX_S *pstIspCtx              = NULL;
    ALG_LIB_S  bindInfo[AAA_TYPE_MAX] = {pstBindAttr->stAeLib, pstBindAttr->stAwbLib,
                                         pstBindAttr->stAfLib};
...
CVI_S32 CVI_ISP_GetBindAttr(VI_PIPE ViPipe, ISP_BIND_ATTR_S *pstBindAttr)
{
    ...
    ISP_LOG_DEBUG("pstBindAttr->stAeLib %s %d\n", pstBindAttr->stAeLib.acLibName,
                  pstBindAttr->stAeLib.s32Id);
    ISP_LOG_DEBUG("pstBindAttr->stAwbLib %s %d\n", pstBindAttr->stAwbLib.acLibName,
                  pstBindAttr->stAwbLib.s32Id);
    ISP_LOG_DEBUG("pstBindAttr->stAfLib %s %d\n", pstBindAttr->stAfLib.acLibName,
                  pstBindAttr->stAfLib.s32Id);
...
#define USEC_TO_NSEC 1000
    ...
    if (u32MilliSec == 0) {
        ...
        timeoutUSec =
            (currentTime.tv_usec + u32MilliSec * MSEC_TO_USEC) - (timeoutSec * SEC_TO_USEC);
                                  ...
                                  ISP_SENSOR_REGISTER_S *pstRegister)
{
    ...
    if (pstIspCtx->bSnsReg == CVI_TRUE) return CVI_FAILURE;
...
static CVI_S32 isp_check_statistics_wb_attr_valid(const ISP_WB_STATISTICS_CFG_S *pstWBStatisticsCfg)
{
    ...
    CHECK_VALID_CONST(pstWBStatisticsCfg, u16WhiteLevel, (pstWBStatisticsCfg->u16BlackLevel) + 1,
                      0xfff);

    return ret;
}

static CVI_S32 isp_check_statistics_focus_attr_valid(
    const ISP_FOCUS_STATISTICS_CFG_S *pstFocusStatisticsCfg)
{
    CVI_S32 ret = CVI_SUCCESS;
...
//-----------------------------------------------------------------------------
CVI_S32 CVI_ISP_SetMeshShadingAttr(VI_PIPE                        ViPipe,
                                   const ISP_MESH_SHADING_ATTR_S *pstMeshShadingAttr)
{
    ISP_LOG_DEBUG("+\n");
    if ((ViPipe < 0) || (ViPipe >= VI_MAX_PIPE_NUM)) {
        ISP_LOG_ERR("ViPipe %d value error\n", ViPipe);
...
CVI_S32 CVI_ISP_SetMeshShadingGainLutAttr(
    VI_PIPE ViPipe, const ISP_MESH_SHADING_GAIN_LUT_ATTR_S *pstMeshShadingGainLutAttr)
{
    ISP_LOG_DEBUG("+\n");
    if ((ViPipe < 0) || (ViPipe >= VI_MAX_PIPE_NUM)) {
        ISP_LOG_ERR("ViPipe %d value error\n", ViPipe);
...
CVI_S32 CVI_ISP_GetMeshShadingGainLutAttr(
    VI_PIPE ViPipe, ISP_MESH_SHADING_GAIN_LUT_ATTR_S *pstMeshShadingGainLutAttr)
{
    if ((ViPipe < 0) || (ViPipe >= VI_MAX_PIPE_NUM)) {
        ISP_LOG_ERR("ViPipe %d value error\n", ViPipe);
        return -ENODEV;
    ...
    if (pTemp != NULL) {
        memcpy(pstMeshShadingGainLutAttr, pTemp, sizeof(ISP_MESH_SHADING_GAIN_LUT_ATTR_S));
    }
...
CVI_S32 CVI_ISP_SetCCMSaturationAttr(VI_PIPE                          ViPipe,
                                     const ISP_CCM_SATURATION_ATTR_S *pstCCMSaturationAttr)
{
    ISP_LOG_DEBUG("+\n");
    if ((ViPipe < 0) || (ViPipe >= VI_MAX_PIPE_NUM)) {
        ISP_LOG_ERR("ViPipe %d value error\n", ViPipe);
...
CVI_S32 CVI_ISP_GetCCMSaturationAttr(VI_PIPE                    ViPipe,
                                     ISP_CCM_SATURATION_ATTR_S *pstCCMSaturationAttr)
{
    if ((ViPipe < 0) || (ViPipe >= VI_MAX_PIPE_NUM)) {
        ISP_LOG_ERR("ViPipe %d value error\n", ViPipe);
        return -ENODEV;
...
CVI_S32 CVI_ISP_SetFSWDRAttr(VI_PIPE ViPipe, const ISP_FSWDR_ATTR_S *pstFSWDRAttr)
{
    ...
    if (pstFSWDRAttr->stManual.WDRCombineMaxWeight < pstFSWDRAttr->stManual.WDRCombineMinWeight) {
...
CVI_S32 CVI_ISP_SetClutSaturationAttr(VI_PIPE                           ViPipe,
...
CVI_S32 CVI_ISP_GetClutSaturationAttr(VI_PIPE                     ViPipe,
                                      ISP_CLUT_SATURATION_ATTR_S *pstClutSaturationAttr)
{
    if ((ViPipe < 0) || (ViPipe >= VI_MAX_PIPE_NUM)) {
        ISP_LOG_ERR("ViPipe %d value error\n", ViPipe);
        return -ENODEV;
...
CVI_S32 CVI_ISP_SetTNRNoiseModelAttr(VI_PIPE                           ViPipe,
...
CVI_S32 CVI_ISP_GetTNRNoiseModelAttr(VI_PIPE                     ViPipe,
                                     ISP_TNR_NOISE_MODEL_ATTR_S *pstTNRNoiseModelAttr)
{
    if ((ViPipe < 0) || (ViPipe >= VI_MAX_PIPE_NUM)) {
        ISP_LOG_ERR("ViPipe %d value error\n", ViPipe);
        return -ENODEV;
...
CVI_S32 CVI_ISP_SetTNRLumaMotionAttr(VI_PIPE                           ViPipe,
...
CVI_S32 CVI_ISP_GetTNRLumaMotionAttr(VI_PIPE                     ViPipe,
                                     ISP_TNR_LUMA_MOTION_ATTR_S *pstTNRLumaMotionAttr)
{
    if ((ViPipe < 0) || (ViPipe >= VI_MAX_PIPE_NUM)) {
        ISP_LOG_ERR("ViPipe %d value error\n", ViPipe);
        return -ENODEV;
...
CVI_S32 CVI_ISP_SetTNRMotionAdaptAttr(VI_PIPE                            ViPipe,
...
CVI_S32 CVI_ISP_GetTNRMotionAdaptAttr(VI_PIPE                      ViPipe,
                                      ISP_TNR_MOTION_ADAPT_ATTR_S *pstTNRMotionAdaptAttr)
{
    if ((ViPipe < 0) || (ViPipe >= VI_MAX_PIPE_NUM)) {
        ISP_LOG_ERR("ViPipe %d value error\n", ViPipe);
        return -ENODEV;
...
CVI_S32 CVI_ISP_SetYNRMotionNRAttr(VI_PIPE                         ViPipe,
                                   const ISP_YNR_MOTION_NR_ATTR_S *pstYNRMotionNRAttr)
{
    ISP_LOG_DEBUG("+\n");
    if ((ViPipe < 0) || (ViPipe >= VI_MAX_PIPE_NUM)) {
        ISP_LOG_ERR("ViPipe %d value error\n", ViPipe);
...
CVI_S32 CVI_ISP_SetCNRMotionNRAttr(VI_PIPE                         ViPipe,
                                   const ISP_CNR_MOTION_NR_ATTR_S *pstCNRMotionNRAttr)
{
    ISP_LOG_DEBUG("+\n");
    if ((ViPipe < 0) || (ViPipe >= VI_MAX_PIPE_NUM)) {
        ISP_LOG_ERR("ViPipe %d value error\n", ViPipe);
...
//  Noise Profile
//-----------------------------------------------------------------------------
CVI_S32 CVI_ISP_SetNoiseProfileAttr(VI_PIPE                             ViPipe,
                                    const ISP_CMOS_NOISE_CALIBRATION_S *pstNoiseProfileAttr)
{
    ISP_LOG_DEBUG("+\n");
    if ((ViPipe < 0) || (ViPipe >= VI_MAX_PIPE_NUM)) {
        ISP_LOG_ERR("ViPipe %d value error\n", ViPipe);
...
CVI_S32 CVI_ISP_GetNoiseProfileAttr(VI_PIPE                       ViPipe,
                                    ISP_CMOS_NOISE_CALIBRATION_S *pstNoiseProfileAttr)
{
    if ((ViPipe < 0) || (ViPipe >= VI_MAX_PIPE_NUM)) {
        ISP_LOG_ERR("ViPipe %d value error\n", ViPipe);
        return -ENODEV;
...
#define IR_DIV_0_TO_1(a)  ((0 == (a)) ? 1 : (a))
    ...
    if (minRG > maxRG) minRG = maxRG;

    if (minBG > maxBG) minBG = maxBG;
    ...
    if (normal2IrISO < ir2NormalISO) normal2IrISO = ir2NormalISO;

    if (pstIrAttr->enIrStatus == ISP_IR_STATUS_NORMAL) {
        pstIrAttr->enIrSwitch =
            (aeInfo.u32ISO >= normal2IrISO) ? ISP_IR_SWITCH_TO_IR : ISP_IR_SWITCH_NONE;
    } else {
        pstIrAttr->enIrSwitch =
            (aeInfo.u32ISO <= ir2NormalISO &&
             (RGRatio < minRG || RGRatio > maxRG || BGRatio < minBG || BGRatio > maxBG))
                ? ISP_IR_SWITCH_TO_NORMAL
                : ISP_IR_SWITCH_NONE;
    }
...
CVI_S32 CVI_ISP_DumpFrameRawInfoToBuffer(VI_PIPE ViPipe, CVI_U8 *buf, CVI_S32 buf_size)
{
    ...
    ret = isp_dbg_dumpFrameRawInfoToBuffer(ViPipe, buf, buf_size);
...
{
    return isp_freeze_set_bypass_frame_num(ViPipe, bypassNum);
}

CVI_U8 CVI_ISP_GetBypassFrm(VI_PIPE ViPipe)
{
    return isp_freeze_get_bypass_frame_num(ViPipe);
}
