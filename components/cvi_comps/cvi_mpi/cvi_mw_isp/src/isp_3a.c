...
ISP_LIB_INFO_S *pAlgoLibReg[AAA_TYPE_MAX] =
    {[AAA_TYPE_AE] = aeAlgoLibReg, [AAA_TYPE_AWB] = awbAlgoLibReg, [AAA_TYPE_AF] = afAlgoLibReg};
...
CVI_S32 isp_3aLib_find(VI_PIPE ViPipe, const ALG_LIB_S *pstAlgoLib, AAA_LIB_TYPE_E type)
{
    ...
    for (i = 0; i < MAX_REGISTER_ALG_LIB_NUM; i++) {
        if (strcmp(
                (const char *)(pstAlgoLib->acLibName),
                (const char *)(pAlgo[ViPipe * MAX_REGISTER_ALG_LIB_NUM + i].libInfo.acLibName)) ==
            0) {
            ret = i;
            break;
        }
...
CVI_S32 isp_3aLib_reg(VI_PIPE ViPipe, ALG_LIB_S *pstLib, CVI_VOID *pstRegister, AAA_LIB_TYPE_E type)
{
    ...
    for (i = 0; i < MAX_REGISTER_ALG_LIB_NUM; i++) {
        if (pAlgo[ViPipe * MAX_REGISTER_ALG_LIB_NUM + i].used == 0) {
            memcpy(&(pAlgo[ViPipe * MAX_REGISTER_ALG_LIB_NUM + i].libInfo), pstLib,
                   sizeof(ALG_LIB_S));
            ...
            if (type == AAA_TYPE_AE) {
                ...
                memcpy(&(pAlgo[ViPipe * MAX_REGISTER_ALG_LIB_NUM + i].algoFunc.awbFunc),
                       pstRegister, sizeof(ISP_AWB_EXP_FUNC_S));
    ...
    for (i = 0; i < MAX_REGISTER_ALG_LIB_NUM; i++) {
        if (pAlgo[ViPipe * MAX_REGISTER_ALG_LIB_NUM + i].used == 1) {
            if (strcmp((const CVI_CHAR *)(pstLib->acLibName),
                       (const CVI_CHAR *)(pAlgo[ViPipe * MAX_REGISTER_ALG_LIB_NUM + i]
                                              .libInfo.acLibName)) == 0) {
                memset(&(pAlgo[ViPipe * MAX_REGISTER_ALG_LIB_NUM + i].libInfo), 0,
                       sizeof(ALG_LIB_S));
...
CVI_S32 isp_3aLib_get(VI_PIPE ViPipe, ALG_LIB_S *pstAlgoLib, AAA_LIB_TYPE_E type)
{
    ...
    if (activeIdx == -1) {
        ...
        memcpy(pstAlgoLib, &(pAlgo[ViPipe * MAX_REGISTER_ALG_LIB_NUM + activeIdx].libInfo),
               sizeof(ALG_LIB_S));
    }
    return ret;
}
...
CVI_S32 isp_3aLib_init(VI_PIPE ViPipe, AAA_LIB_TYPE_E type)
{
    ...
    afInitParam.u8WDRMode = awbInitParam.u8WDRMode = aeInitParam.u8WDRMode =
    ...
    if (type == AAA_TYPE_AE) {
        if (pAlgo[ViPipe * MAX_REGISTER_ALG_LIB_NUM + activeIdx].algoFunc.aeFunc.pfn_ae_init !=
            NULL) {
        ...
        if (pAlgo[ViPipe * MAX_REGISTER_ALG_LIB_NUM + activeIdx].algoFunc.awbFunc.pfn_awb_init !=
            NULL) {
        ...
        if (pAlgo[ViPipe * MAX_REGISTER_ALG_LIB_NUM + activeIdx].algoFunc.afFunc.pfn_af_init !=
            NULL) {
...
CVI_S32 isp_3aLib_Ctrl(VI_PIPE ViPipe, AAA_LIB_TYPE_E type)
{
    ...
    if (type == AAA_TYPE_AE) {
        if (pAlgo[ViPipe * MAX_REGISTER_ALG_LIB_NUM + activeIdx].algoFunc.aeFunc.pfn_ae_ctrl !=
            NULL) {
        ...
        if (pAlgo[ViPipe * MAX_REGISTER_ALG_LIB_NUM + activeIdx].algoFunc.awbFunc.pfn_awb_ctrl !=
            NULL) {
        ...
        if (pAlgo[ViPipe * MAX_REGISTER_ALG_LIB_NUM + activeIdx].algoFunc.afFunc.pfn_af_ctrl !=
            NULL) {
...
CVI_S32 isp_3aLib_smart_info_set(VI_PIPE ViPipe, const ISP_SMART_INFO_S *pstSmartInfo,
                                 CVI_U8 TimeOut)
{
    ISP_CTX_S *pstIspCtx = NULL;
    int        index     = 0;
...
CVI_S32 isp_3aLib_run(VI_PIPE ViPipe, AAA_LIB_TYPE_E type)
{
    ...
    if (type == AAA_TYPE_AE) {
        ...
        if (pAlgo[ViPipe * MAX_REGISTER_ALG_LIB_NUM + activeIdx].algoFunc.aeFunc.pfn_ae_run !=
            NULL) {
        ...
        if (pAlgo[ViPipe * MAX_REGISTER_ALG_LIB_NUM + activeIdx].algoFunc.awbFunc.pfn_awb_run !=
            NULL) {
        ...
        if (pAlgo[ViPipe * MAX_REGISTER_ALG_LIB_NUM + activeIdx].algoFunc.afFunc.pfn_af_run !=
            NULL) {
...
CVI_S32 isp_3aLib_exit(VI_PIPE ViPipe, AAA_LIB_TYPE_E type)
{
    ...
    if (type == AAA_TYPE_AE) {
        if (pAlgo[ViPipe * MAX_REGISTER_ALG_LIB_NUM + activeIdx].algoFunc.aeFunc.pfn_ae_exit !=
            NULL)
            pAlgo[ViPipe * MAX_REGISTER_ALG_LIB_NUM + activeIdx].algoFunc.aeFunc.pfn_ae_exit(
                ViPipe);
        else {
        ...
        if (pAlgo[ViPipe * MAX_REGISTER_ALG_LIB_NUM + activeIdx].algoFunc.awbFunc.pfn_awb_exit !=
            NULL)
            pAlgo[ViPipe * MAX_REGISTER_ALG_LIB_NUM + activeIdx].algoFunc.awbFunc.pfn_awb_exit(
                ViPipe);
        else {
