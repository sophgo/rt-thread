...
const struct isp_module_ctrl bnr_mod = {.init    = isp_bnr_ctrl_init,
                                        .uninit  = isp_bnr_ctrl_uninit,
                                        .suspend = isp_bnr_ctrl_suspend,
                                        .resume  = isp_bnr_ctrl_resume,
                                        .ctrl    = isp_bnr_ctrl_ctrl};

CVI_U8 lsc_gain_lut_default[32] = {32, 32, 32, 32, 32, 32, 32, 33, 33, 34, 34, 35, 36, 37, 38, 39,
...
static CVI_S32 isp_bnr_ctrl_check_bnr_filter_attr_valid(
    const ISP_NR_FILTER_ATTR_S *pstNRFilterAttr);
...
static CVI_S32 isp_bnr_ctrl_preprocess(VI_PIPE ViPipe, ISP_ALGO_RESULT_S *algoResult)
{
    ...
    if (!bnr_attr->Enable || runtime->is_module_bypass) return ret;
...
static CVI_S32 isp_bnr_ctrl_process(VI_PIPE ViPipe)
{
    ...
    isp_algo_bnr_main((struct bnr_param_in *)&runtime->bnr_param_in,
...
static CVI_S32 isp_bnr_ctrl_postprocess(VI_PIPE ViPipe)
{
    ...
    if (is_postprocess_update == CVI_FALSE) {
        ...
        cfg_1->NS_LUMA_TH_R.bits.BNR_NS_LUMA_TH_R =
            runtime->bnr_param_out.NpLumaThr[ISP_BAYER_CHN_R];
        cfg_1->NS_SLOPE_R.bits.BNR_NS_SLOPE_R = runtime->bnr_param_out.NpSlope[ISP_BAYER_CHN_R];
        cfg_1->NS_OFFSET0_R.bits.BNR_NS_LOW_OFFSET_R =
            runtime->bnr_param_out.NpLowOffset[ISP_BAYER_CHN_R];
        cfg_1->NS_OFFSET1_R.bits.BNR_NS_HIGH_OFFSET_R =
            runtime->bnr_param_out.NpHighOffset[ISP_BAYER_CHN_R];
        cfg_1->NS_LUMA_TH_GR.bits.BNR_NS_LUMA_TH_GR =
            runtime->bnr_param_out.NpLumaThr[ISP_BAYER_CHN_GR];
        cfg_1->NS_SLOPE_GR.bits.BNR_NS_SLOPE_GR = runtime->bnr_param_out.NpSlope[ISP_BAYER_CHN_GR];
        cfg_1->NS_OFFSET0_GR.bits.BNR_NS_LOW_OFFSET_GR =
            runtime->bnr_param_out.NpLowOffset[ISP_BAYER_CHN_GR];
        cfg_1->NS_OFFSET1_GR.bits.BNR_NS_HIGH_OFFSET_GR =
            runtime->bnr_param_out.NpHighOffset[ISP_BAYER_CHN_GR];
        cfg_1->NS_LUMA_TH_GB.bits.BNR_NS_LUMA_TH_GB =
            runtime->bnr_param_out.NpLumaThr[ISP_BAYER_CHN_GB];
        cfg_1->NS_SLOPE_GB.bits.BNR_NS_SLOPE_GB = runtime->bnr_param_out.NpSlope[ISP_BAYER_CHN_GB];
        cfg_1->NS_OFFSET0_GB.bits.BNR_NS_LOW_OFFSET_GB =
            runtime->bnr_param_out.NpLowOffset[ISP_BAYER_CHN_GB];
        cfg_1->NS_OFFSET1_GB.bits.BNR_NS_HIGH_OFFSET_GB =
            runtime->bnr_param_out.NpHighOffset[ISP_BAYER_CHN_GB];
        cfg_1->NS_LUMA_TH_B.bits.BNR_NS_LUMA_TH_B =
            runtime->bnr_param_out.NpLumaThr[ISP_BAYER_CHN_B];
        cfg_1->NS_SLOPE_B.bits.BNR_NS_SLOPE_B = runtime->bnr_param_out.NpSlope[ISP_BAYER_CHN_B];
        cfg_1->NS_OFFSET0_B.bits.BNR_NS_LOW_OFFSET_B =
            runtime->bnr_param_out.NpLowOffset[ISP_BAYER_CHN_B];
        cfg_1->NS_OFFSET1_B.bits.BNR_NS_HIGH_OFFSET_B =
            runtime->bnr_param_out.NpHighOffset[ISP_BAYER_CHN_B];
        ...
        cfg_2->WEIGHT_V.bits.BNR_WEIGHT_V       = cfg_2->WEIGHT_H.bits.BNR_WEIGHT_H =
            runtime->bnr_filter_attr.VhDirFiltStr;
        cfg_2->WEIGHT_D45.bits.BNR_WEIGHT_D45 = cfg_2->WEIGHT_D135.bits.BNR_WEIGHT_D135 =
            runtime->bnr_filter_attr.AaDirFiltStr;
...
CVI_S32 isp_bnr_ctrl_get_bnr_filter_attr(VI_PIPE                      ViPipe,
                                         const ISP_NR_FILTER_ATTR_S **pstNRFilterAttr)
{
...
CVI_S32 isp_bnr_ctrl_set_bnr_filter_attr(VI_PIPE                     ViPipe,
                                         const ISP_NR_FILTER_ATTR_S *pstNRFilterAttr)
{
    if (pstNRFilterAttr == CVI_NULL) {
        return CVI_FAILURE;
