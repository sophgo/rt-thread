...
const struct isp_module_ctrl ca2_mod = {.init    = isp_ca2_ctrl_init,
...
static CVI_S32 isp_ca2_ctrl_preprocess(VI_PIPE ViPipe, ISP_ALGO_RESULT_S *algoResult)
{
    ...
    if (!ca2_attr->Enable || runtime->is_module_bypass) return ret;
...
static CVI_S32 isp_ca2_ctrl_process(VI_PIPE ViPipe)
{
    ...
    isp_algo_ca2_main((struct ca2_param_in *)&runtime->ca2_param_in,
...
static CVI_S32 isp_ca2_ctrl_postprocess(VI_PIPE ViPipe)
{
    ...
    if (is_postprocess_update == CVI_FALSE) {
        ...
        for (CVI_U32 i = 0; i < CA_LITE_NODE; i++) ca2_cfg->lut_in[i] = runtime->ca2_attr.Ca2In[i];
        memcpy(ca2_cfg->lut_out, runtime->ca2_attr.Ca2Out, sizeof(CVI_U16) * CA_LITE_NODE);
        memcpy(ca2_cfg->lut_slp, runtime->ca2_param_out.Ca2Slope,
               sizeof(CVI_U16) * (CA_LITE_NODE - 1));
    }
