...
const struct isp_module_ctrl ca_mod = {.init    = isp_ca_ctrl_init,
...
static CVI_S32 isp_ca_ctrl_preprocess(VI_PIPE ViPipe, ISP_ALGO_RESULT_S *algoResult)
{
    ...
    if (!ca_attr->Enable || runtime->is_module_bypass) return ret;
...
static CVI_S32 isp_ca_ctrl_postprocess(VI_PIPE ViPipe)
{
    ...
    if (is_postprocess_update == CVI_FALSE) {
        ...
        if (ca_attr->CaCpMode == 0) {
            memcpy(cacp_cfg->ca_y_ratio_lut, runtime->ca_attr.YRatioLut,
                   sizeof(CVI_U16) * CA_LUT_NUM);
            ...
            if (u8CurSat > 0) {
                ...
                for (; i < CA_LUT_NUM; i++) {
                    if (pstIspCtx->stAwbResult.u8AdjCASatLuma > i) {
                        u8AdjustSat = u8CurSat - (pstIspCtx->stAwbResult.u8AdjCASatLuma - i);
                    } else {
