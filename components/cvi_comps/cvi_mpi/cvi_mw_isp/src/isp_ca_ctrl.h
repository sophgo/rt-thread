/*
 * Copyright (C) Cvitek Co., Ltd. 2019-2021. All rights reserved.
 *
 * File Name: isp_ca_ctrl.h
 * Description:
 *
 */

#ifndef _ISP_CA_CTRL_H_
#define _ISP_CA_CTRL_H_

#include "cvi_comm_isp.h"
#include "isp_algo_ca.h"
#include "isp_feature_ctrl.h"

#ifdef __cplusplus
#if __cplusplus
extern "C" {
#endif
#endif /* End of #ifdef __cplusplus */

struct isp_ca_ctrl_runtime {
    struct ca_param_in  ca_param_in;
    struct ca_param_out ca_param_out;

    ISP_CA_MANUAL_ATTR_S ca_attr;

    CVI_BOOL preprocess_updated;
    CVI_BOOL process_updated;
    CVI_BOOL postprocess_updated;
    CVI_BOOL is_module_bypass;
};

CVI_S32 isp_ca_ctrl_init(VI_PIPE ViPipe);
CVI_S32 isp_ca_ctrl_uninit(VI_PIPE ViPipe);
CVI_S32 isp_ca_ctrl_suspend(VI_PIPE ViPipe);
CVI_S32 isp_ca_ctrl_resume(VI_PIPE ViPipe);
CVI_S32 isp_ca_ctrl_ctrl(VI_PIPE ViPipe, enum isp_module_cmd cmd, CVI_VOID *input);
CVI_S32 isp_ca_ctrl_get_ca_attr(VI_PIPE ViPipe, const ISP_CA_ATTR_S **pstCAAttr);
CVI_S32 isp_ca_ctrl_set_ca_attr(VI_PIPE ViPipe, const ISP_CA_ATTR_S *pstCAAttr);

extern const struct isp_module_ctrl ca_mod;

#ifdef __cplusplus
#if __cplusplus
}
#endif
#endif /* End of #ifdef __cplusplus */

#endif  // _ISP_CA_CTRL_H_
