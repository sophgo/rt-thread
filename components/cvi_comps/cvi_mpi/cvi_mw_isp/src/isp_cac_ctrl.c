...
const struct isp_module_ctrl cac_mod = {.init    = isp_cac_ctrl_init,
...
static CVI_S32 isp_cac_ctrl_preprocess(VI_PIPE ViPipe, ISP_ALGO_RESULT_S *algoResult)
{
    ...
    if (!cac_attr->Enable || runtime->is_module_bypass) return ret;
...
static CVI_S32 isp_cac_ctrl_process(VI_PIPE ViPipe)
{
    ...
    isp_algo_cac_main((struct cac_param_in *)&runtime->cac_param_in,
...
static CVI_S32 isp_cac_ctrl_postprocess(VI_PIPE ViPipe)
{
    ...
    if (is_postprocess_update == CVI_FALSE) {
        ...
        cac_2_cfg->CNR_DEPURPLE_WEIGHT_TH.bits.CNR_DEPURPLE_STR_MIN_RATIO =
            pcac_param->u8DePurpleStrRatioMin;
        cac_2_cfg->CNR_DEPURPLE_WEIGHT_TH.bits.CNR_DEPURPLE_STR_MAX_RATIO =
            pcac_param->u8DePurpleStrRatioMax;
