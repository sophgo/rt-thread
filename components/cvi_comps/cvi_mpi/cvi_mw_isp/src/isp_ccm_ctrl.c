...
const struct isp_module_ctrl ccm_mod = {.init    = isp_ccm_ctrl_init,
...
static CVI_S32 isp_ccm_ctrl_check_saturation_attr_valid(
    const ISP_SATURATION_ATTR_S *pstSaturationAttr);
static CVI_S32 isp_ccm_ctrl_check_ccm_saturation_attr_valid(
...
static CVI_S32 isp_ccm_ctrl_preprocess(VI_PIPE ViPipe, ISP_ALGO_RESULT_S *algoResult)
{
    ...
    if (!ccm_attr->Enable || runtime->is_module_bypass) return ret;
...
static CVI_S32 isp_ccm_ctrl_process(VI_PIPE ViPipe)
{
    ...
    ret = isp_algo_ccm_main((struct ccm_param_in *)&runtime->ccm_param_in,
...
static CVI_S32 isp_ccm_ctrl_postprocess(VI_PIPE ViPipe)
{
    ...
    if (is_postprocess_update == CVI_FALSE) {
        ...
        for (ISP_CHANNEL_LIST_E idx = ISP_CHANNEL_LE; idx < ISP_CHANNEL_MAX_NUM; idx++) {
            ...
            memcpy(runtime->ccm_cfg[idx].coef, runtime->ccm_attr[idx].stManual.CCM,
                   sizeof(CVI_S16) * 9);
        }
        ...
        for (CVI_U8 i = 0; i < 1; i++) {
            ...
            ccm_cfg[i]->enable =
                tmp_ccm_cfg[i]->enable && map_pipe_to_enable[i] && !runtime->is_module_bypass;
        }
    }
...
CVI_S32 isp_ccm_ctrl_get_ccm_saturation_attr(VI_PIPE                           ViPipe,
                                             const ISP_CCM_SATURATION_ATTR_S **pstCCMSaturationAttr)
{
...
CVI_S32 isp_ccm_ctrl_set_ccm_saturation_attr(VI_PIPE                          ViPipe,
                                             const ISP_CCM_SATURATION_ATTR_S *pstCCMSaturationAttr)
{
    if (pstCCMSaturationAttr == CVI_NULL) {
        return CVI_FAILURE;
...
CVI_S32 isp_ccm_ctrl_get_saturation_attr(VI_PIPE                       ViPipe,
                                         const ISP_SATURATION_ATTR_S **pstSaturationAttr)
{
...
CVI_S32 isp_ccm_ctrl_set_saturation_attr(VI_PIPE                      ViPipe,
                                         const ISP_SATURATION_ATTR_S *pstSaturationAttr)
{
    if (pstSaturationAttr == CVI_NULL) {
        return CVI_FAILURE;
