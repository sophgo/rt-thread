...
const struct isp_module_ctrl cnr_mod = {.init    = isp_cnr_ctrl_init,
...
static CVI_S32 isp_cnr_ctrl_check_cnr_motion_attr_valid(
...
static CVI_S32 isp_cnr_ctrl_preprocess(VI_PIPE ViPipe, ISP_ALGO_RESULT_S *algoResult)
{
    ...
    if (!cnr_attr->Enable || runtime->is_module_bypass) return ret;
...
static CVI_S32 isp_cnr_ctrl_process(VI_PIPE ViPipe)
{
    ...
    ret = isp_algo_cnr_main((struct cnr_param_in *)&runtime->cnr_param_in,
...
static CVI_S32 isp_cnr_ctrl_postprocess(VI_PIPE ViPipe)
{
    ...
    if (is_postprocess_update == CVI_FALSE) {
        ...
        memcpy(cnr_cfg->weight_lut_inter, runtime->cnr_param_out.weight_lut,
        ...
        memcpy(cnr_cfg->coring_motion_lut, runtime->cnr_motion_attr.MotionCnrCoringLut,
               sizeof(CVI_U8) * CNR_MOTION_LUT_NUM);
        memcpy(cnr_cfg->motion_lut, runtime->cnr_motion_attr.MotionCnrStrLut,
               sizeof(CVI_U8) * CNR_MOTION_LUT_NUM);
    }
    ...
    const ISP_CNR_MOTION_NR_ATTR_S *pstCNRMotionNRAttr)
{
    ...
    // CHECK_VALID_AUTO_ISO_2D(pstCNRMotionNRAttr, MotionCnrCoringLut, CNR_MOTION_LUT_NUM, 0x0,
    // 0xff); CHECK_VALID_AUTO_ISO_2D(pstCNRMotionNRAttr, MotionCnrStrLut, CNR_MOTION_LUT_NUM, 0x0,
    // 0xff);
...
CVI_S32 isp_cnr_ctrl_get_cnr_motion_attr(VI_PIPE                          ViPipe,
                                         const ISP_CNR_MOTION_NR_ATTR_S **pstCNRMotionNRAttr)
{
...
CVI_S32 isp_cnr_ctrl_set_cnr_motion_attr(VI_PIPE                         ViPipe,
                                         const ISP_CNR_MOTION_NR_ATTR_S *pstCNRMotionNRAttr)
{
    if (pstCNRMotionNRAttr == CVI_NULL) {
        return CVI_FAILURE;
