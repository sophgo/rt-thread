...
const struct isp_module_ctrl crosstalk_mod = {.init    = isp_crosstalk_ctrl_init,
...
static CVI_S32 isp_crosstalk_ctrl_check_crosstalk_attr_valid(
    const ISP_CROSSTALK_ATTR_S *pstCrosstalkAttr);
...
static CVI_S32 isp_crosstalk_ctrl_preprocess(VI_PIPE ViPipe, ISP_ALGO_RESULT_S *algoResult)
{
    ...
    if (!crosstalk_attr->Enable || runtime->is_module_bypass) return ret;
...
//-----------------------------------------------------------------------------
CVI_S32 isp_crosstalk_ctrl_get_crosstalk_attr(VI_PIPE                      ViPipe,
                                              const ISP_CROSSTALK_ATTR_S **pstCrosstalkAttr)
{
...
CVI_S32 isp_crosstalk_ctrl_set_crosstalk_attr(VI_PIPE                     ViPipe,
                                              const ISP_CROSSTALK_ATTR_S *pstCrosstalkAttr)
{
    if (pstCrosstalkAttr == CVI_NULL) {
        return CVI_FAILURE;
