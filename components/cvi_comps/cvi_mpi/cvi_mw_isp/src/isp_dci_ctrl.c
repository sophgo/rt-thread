...
const struct isp_module_ctrl dci_mod = {.init    = isp_dci_ctrl_init,
...
static CVI_S32 isp_dci_ctrl_preprocess(VI_PIPE ViPipe, ISP_ALGO_RESULT_S *algoResult)
{
    ...
    if (!dci_attr->Enable || runtime->is_module_bypass) return ret;
...
static CVI_S32 isp_dci_ctrl_process(VI_PIPE ViPipe)
{
    ...
    ret = isp_algo_dci_main((struct dci_param_in *)&runtime->dci_param_in,
