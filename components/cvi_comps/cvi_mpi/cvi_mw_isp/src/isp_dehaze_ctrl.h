/*
 * Copyright (C) Cvitek Co., Ltd. 2019-2021. All rights reserved.
 *
 * File Name: isp_dehaze_ctrl.h
 * Description:
 *
 */

#ifndef _ISP_DEHAZE_CTRL_H_
#define _ISP_DEHAZE_CTRL_H_

#include "cvi_comm_isp.h"
#include "isp_algo_dehaze.h"
#include "isp_feature_ctrl.h"

#ifdef __cplusplus
#if __cplusplus
extern "C" {
#endif
#endif /* End of #ifdef __cplusplus */

struct isp_dehaze_ctrl_runtime {
    struct dehaze_param_in  dehaze_param_in;
    struct dehaze_param_out dehaze_param_out;

    ISP_DEHAZE_MANUAL_ATTR_S dehaze_attr;

    CVI_BOOL preprocess_updated;
    CVI_BOOL process_updated;
    CVI_BOOL postprocess_updated;
    CVI_BOOL is_module_bypass;
};

CVI_S32 isp_dehaze_ctrl_init(VI_PIPE ViPipe);
CVI_S32 isp_dehaze_ctrl_uninit(VI_PIPE ViPipe);
CVI_S32 isp_dehaze_ctrl_suspend(VI_PIPE ViPipe);
CVI_S32 isp_dehaze_ctrl_resume(VI_PIPE ViPipe);
CVI_S32 isp_dehaze_ctrl_ctrl(VI_PIPE ViPipe, enum isp_module_cmd cmd, CVI_VOID *input);

CVI_S32 isp_dehaze_ctrl_get_dehaze_attr(VI_PIPE ViPipe, const ISP_DEHAZE_ATTR_S **pstDehazeAttr);
CVI_S32 isp_dehaze_ctrl_set_dehaze_attr(VI_PIPE ViPipe, const ISP_DEHAZE_ATTR_S *pstDehazeAttr);

extern const struct isp_module_ctrl dehaze_mod;

#ifdef __cplusplus
#if __cplusplus
}
#endif
#endif /* End of #ifdef __cplusplus */

#endif  // _ISP_DEHAZE_CTRL_H_
