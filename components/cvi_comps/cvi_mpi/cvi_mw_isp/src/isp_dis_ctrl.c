...
const struct isp_module_ctrl dis_mod = {.init    = isp_dis_ctrl_init,
...
CVI_S32 isp_dis_ctrl_init(VI_PIPE ViPipe)
{
    ...
    if (ViPipe > (DIS_SENSOR_NUM - 1)) return CVI_SUCCESS;
...
//  private functions
//-----------------------------------------------------------------------------
static void dis_debug_init(const ISP_DIS_ATTR_S *pAttr, const ISP_DIS_CONFIG_S *pConfig,
                           CVI_U32 dFrameCnt)
{
#if USE_CAM_VDO
    return;
...
#define TEST_FRM_NUM (30)
    ...
    if (sDisDbg.isAligned == 0) {
        ...
        if (testX > 30) testX = 30;
        if (testY > 30) testY = 30;
...
static CVI_S32 isp_dis_ctrl_preprocess(VI_PIPE ViPipe, ISP_ALGO_RESULT_S *algoResult)
{
...
#ifdef NEW_DIS_CTRL_ARCH
    CVI_BOOL dis_supported =
        ((disAttr->enable && !runtime->is_module_bypass) || disAttr->stillCrop) &&
        ((scene == FE_ON_BE_OFF_POST_OFF_SC || scene == FE_OFF_BE_ON_POST_OFF_SC)) &&
        (imgW <= 2880);
#else
    ...
    runtime->preprocess_updated = CVI_TRUE;
#endif
    if (runtime->preprocess_updated == CVI_FALSE) return ret;
...
static CVI_S32 isp_dis_ctrl_postprocess(VI_PIPE ViPipe)
{
    ...
    if (runtime->postprocess_updated) {
        ...
        ISP_LOG_DEBUG("Crop info [%d][%d](%d %d %d %d)\n", mdisRes.sensor_num, mdisRes.frm_num,
                      mdisRes.dis_i.start_x, mdisRes.dis_i.start_y, mdisRes.dis_i.end_x,
                      mdisRes.dis_i.end_y);
...
#define MIN_GAP (10)
    ...
    curSize =
        gmsCfg->offset_x + (gmsCfg->x_section_size + 1) * DIS_MAX_WINDOW_X_NUM + gmsCfg->x_gap * 2;
    // curSize = 770;
    if (limitW < curSize) {
    ...
    curSize =
        gmsCfg->offset_y + (gmsCfg->y_section_size + 1) * DIS_MAX_WINDOW_Y_NUM + gmsCfg->y_gap * 2;
    // curSize = 770;
    if (limitH < curSize) {
