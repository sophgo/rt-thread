...
const struct isp_module_ctrl dpc_mod = {.init    = isp_dpc_ctrl_init,
...
static CVI_S32 isp_dpc_ctrl_check_dpc_dynamic_attr_valid(
    const ISP_DP_DYNAMIC_ATTR_S *pstDPCDynamicAttr);
static CVI_S32 isp_dpc_ctrl_check_dpc_static_attr_valid(
...
static CVI_S32 isp_dpc_ctrl_preprocess(VI_PIPE ViPipe, ISP_ALGO_RESULT_S *algoResult)
{
    ...
    if (!dpc_dynamic_attr->Enable || runtime->is_module_bypass) return ret;
...
#undef AUTO
    ...
    // ParamIn
    // runtime->dpc_param_in.BrightDefectToNormalPixRatio =
    // runtime->dpc_dynamic_attr.BrightDefectToNormalPixRatio;
    // runtime->dpc_param_in.DarkDefectToNormalPixRatio =
    // runtime->dpc_dynamic_attr.DarkDefectToNormalPixRatio;
...
static CVI_S32 isp_dpc_ctrl_postprocess(VI_PIPE ViPipe)
{
    ...
    if (is_postprocess_update == CVI_FALSE) {
        ...
        runtime->dpc_cfg.enable =
            (dpc_dynamic_attr->Enable || dpc_static_attr->Enable) && !runtime->is_module_bypass;
...
#define CONVERT_BP_FORMAT(bp) ((((bp) & 0x0FFF0000) >> (16 - 12)) + ((bp) & 0x00000FFF))
            ...
            for (CVI_U32 c = 0; c < brightMax; c++, bp_cnt++)
                runtime->dpc_cfg.bp_tbl[bp_cnt] =
                    CONVERT_BP_FORMAT(dpc_static_attr->BrightTable[c]);
...
#undef CONVERT_BP_FORMAT
        ...
        struct cvi_isp_dpc_tun_cfg *cfg_dpc =
            (struct cvi_isp_dpc_tun_cfg *)&runtime->dpc_cfg.dpc_cfg;

        // TODO@ST Check formula
        cfg_dpc->DPC_3.bits.DPC_R_BRIGHT_PIXEL_RATIO =
            runtime->dpc_dynamic_attr.BrightDefectToNormalPixRatio;
        cfg_dpc->DPC_3.bits.DPC_G_BRIGHT_PIXEL_RATIO =
            runtime->dpc_dynamic_attr.BrightDefectToNormalPixRatio;
        cfg_dpc->DPC_4.bits.DPC_B_BRIGHT_PIXEL_RATIO =
            runtime->dpc_dynamic_attr.BrightDefectToNormalPixRatio;
        cfg_dpc->DPC_4.bits.DPC_R_DARK_PIXEL_RATIO =
            runtime->dpc_dynamic_attr.DarkDefectToNormalPixRatio;
        cfg_dpc->DPC_5.bits.DPC_G_DARK_PIXEL_RATIO =
            runtime->dpc_dynamic_attr.DarkDefectToNormalPixRatio;
        cfg_dpc->DPC_5.bits.DPC_B_DARK_PIXEL_RATIO =
            runtime->dpc_dynamic_attr.DarkDefectToNormalPixRatio;
...
#ifndef ARCH_RTOS_CV181X
static CVI_S32 dump_raw(VI_PIPE ViPipe, CVI_U16 *bayerBuffer, CVI_U16 width, CVI_U16 height,
                        BAYER_FORMAT_E bayerFormat)
{
    ...
    snprintf(img_name, sizeof(img_name), "./vi_%d_byrid_%s_w_%d_h_%d_bit_%d_tv_%ld_%ld.raw", ViPipe,
             order_id, width, height, 16, tv1.tv_sec, tv1.tv_usec);

    output = fopen(img_name, "wb");
    if (output) {
        ISP_LOG_DEBUG("dump raw to %s\n", img_name);
        size_t bayerBufferSize = width * height * 2;
        fwrite(bayerBuffer, bayerBufferSize, 1, output);
        fclose(output);
    } else {
...
static CVI_VOID crop_image(CVI_U32 stride, ISP_SNS_SYNC_INFO_S *snsCropInfo, CVI_U8 *src,
                           CVI_U8 *dst, CVI_FLOAT pixel_per_byte)
{
...
static CVI_S32 ISP_GetRawBuffer(VI_PIPE ViPipe, CVI_U8 *bayerBuffer, BAYER_FORMAT_E *bayerFormat)
{
    ...
    *bayerFormat = stVideoFrame.stVFrame.enBayerFormat;
    ...
    if (attr.enDumpType == VI_DUMP_TYPE_RAW) {
        ...
        memcpy(ptr, (const void *)stVideoFrame.stVFrame.u64PhyAddr[0], image_size);
...
static CVI_S32 ISP_SetDPCalibrate(VI_PIPE ViPipe, const ISP_DP_CALIB_ATTR_S *pstDPCalibAttr)
{
    ...
    if (pattr->EnableDetect) {
        ...
        bayerBuffer16 = ISP_CALLOC(1, width * height * sizeof(CVI_U16));  // 12bit data save in
                                                                          // 16bit
        ...
        do {
            ...
            if (pattr->saveFileEn) dump_raw(ViPipe, bayerBuffer16, width, height, bayerFormat);
    ...
    ERROR:
        ...
        if (bayerBuffer16 != NULL) free(bayerBuffer16);
        if (pattr != NULL) free(pattr);
        if (pBadPixelST != NULL) free(pBadPixelST);
    } else {
        if (pattr != NULL) free(pattr);
    }
    return CVI_SUCCESS;
...
//-----------------------------------------------------------------------------
CVI_S32 isp_dpc_ctrl_get_dpc_dynamic_attr(VI_PIPE                       ViPipe,
                                          const ISP_DP_DYNAMIC_ATTR_S **pstDPCDynamicAttr)
{
...
CVI_S32 isp_dpc_ctrl_set_dpc_dynamic_attr(VI_PIPE                      ViPipe,
                                          const ISP_DP_DYNAMIC_ATTR_S *pstDPCDynamicAttr)
{
    if (pstDPCDynamicAttr == CVI_NULL) {
        return CVI_FAILURE;
...
CVI_S32 isp_dpc_ctrl_get_dpc_static_attr(VI_PIPE                      ViPipe,
                                         const ISP_DP_STATIC_ATTR_S **pstDPStaticAttr)
{
...
CVI_S32 isp_dpc_ctrl_set_dpc_static_attr(VI_PIPE                     ViPipe,
                                         const ISP_DP_STATIC_ATTR_S *pstDPStaticAttr)
{
    if (pstDPStaticAttr == CVI_NULL) {
        return CVI_FAILURE;
...
CVI_S32 isp_dpc_ctrl_set_dpc_calibrate(VI_PIPE ViPipe, const ISP_DP_CALIB_ATTR_S *pstDPCalibAttr)
{
    ...
    if (pstDPCalibAttr->EnableDetect == CVI_FALSE) return ret;
