...
const struct isp_module_ctrl drc_mod = {.init    = isp_drc_ctrl_init,
...
static CVI_S32 isp_drc_ctrl_modify_rgbgamma_for_drc_tuning_mode(VI_PIPE         ViPipe,
...
CVI_S32 isp_drc_ctrl_buf_init(VI_PIPE ViPipe)
{
    ...
    if (runtime->pLocalBuffer == CVI_NULL) {
        ISP_LOG_ERR("Allocate [%d] DRC internal buffer fail, size=%d\n", ViPipe, u32BufferSize);
        return CVI_FAILURE;
    }

    u32BuffOffset = 0;

    runtime->preGlobalLut =
        ISP_PTR_CAST_PTR((ISP_PTR_CAST_U8(runtime->pLocalBuffer) + u32BuffOffset));
...
#define __DRC_CRTL_BUF_SIZE                           \
    ((LTM_G_CURVE_TOTAL_NODE_NUM * sizeof(CVI_U32)) + \
     (LTM_DARK_CURVE_NODE_NUM * sizeof(CVI_U32)) + (LTM_BRIGHT_CURVE_NODE_NUM * sizeof(CVI_U32)))
...
static CVI_S32 isp_drc_ctrl_preprocess(VI_PIPE ViPipe, ISP_ALGO_RESULT_S *algoResult)
{
    ...
    is_preprocess_update =
        ((runtime->preprocess_updated) || ((algoResult->u32FrameIdx % frame_delay) == 0) ||
...
#undef AUTO
    ...
    runtime->drc_param_in.stBritToneParam.u8SdrBritInflectPtLuma =
        runtime->drc_attr.SdrBritInflectPtLuma;
    runtime->drc_param_in.stBritToneParam.u8SdrBritContrastLow =
        runtime->drc_attr.SdrBritContrastLow;
    runtime->drc_param_in.stBritToneParam.u8SdrBritContrastHigh =
        runtime->drc_attr.SdrBritContrastHigh;
    ...
    memcpy(runtime->drc_param_in.SEHistogram,
           ae_sts->aeStat1[0].au32HistogramMemArray[ISP_CHANNEL_SE],
           sizeof(runtime->drc_param_in.SEHistogram));
    memcpy(runtime->drc_param_in.LEHistogram,
           ae_sts->aeStat1[0].au32HistogramMemArray[ISP_CHANNEL_LE],
...
static CVI_S32 isp_drc_ctrl_process(VI_PIPE ViPipe)
{
    ...
    ret = isp_algo_drc_main((struct drc_param_in *)&runtime->drc_param_in,
...
static CVI_S32 isp_drc_ctrl_postprocess(VI_PIPE ViPipe)
{
    ...
    if (is_postprocess_update == CVI_FALSE) {
        ...
        drc_cfg->dark_enh_en = drc_cfg->brit_enh_en = drc_attr->LocalToneEn;
        drc_cfg->dbg_mode                           = drc_attr->TuningMode;
        drc_cfg->dark_tone_wgt_refine_en            = drc_cfg->brit_tone_wgt_refine_en =
            drc_attr->LocalToneRefineEn;
...
#define DRC_OUT_MAX_VAL ((1 << RAW_BIT_DEPTH) - 1)
		...
		if (tunIdx != 0 && tunIdx <= 3) {
			...
			if (fdList[tunIdx] == NULL) {
				...
				snprintf(fileName, 100, "/mnt/sd/curve%d.txt", tunIdx);
			...
			for (CVI_U8 i = 0; i < DRC_TUNING_NUM; ++i) {
				if (fdList[i] != NULL) {
					printf("stop dump curve to /mnt/sd/curve%d.txt!!\n", i);
					fclose(fdList[i]);
					fdList[i] = NULL;
				}
			}
        ...
        cfg_1->REG_HA0.bits.LTM_DE_LMAP_LUT_OUT_0 =
            runtime->drc_param_out.reg_ltm_de_lmap_lut_out[0];
        cfg_1->REG_HA0.bits.LTM_DE_LMAP_LUT_OUT_1 =
            runtime->drc_param_out.reg_ltm_de_lmap_lut_out[1];
        cfg_1->REG_HA0.bits.LTM_DE_LMAP_LUT_OUT_2 =
            runtime->drc_param_out.reg_ltm_de_lmap_lut_out[2];
        cfg_1->REG_HA0.bits.LTM_DE_LMAP_LUT_OUT_3 =
            runtime->drc_param_out.reg_ltm_de_lmap_lut_out[3];
        cfg_1->REG_HA4.bits.LTM_DE_LMAP_LUT_SLOPE_0 =
            runtime->drc_param_out.reg_ltm_de_lmap_lut_slope[0];
        cfg_1->REG_HA4.bits.LTM_DE_LMAP_LUT_SLOPE_1 =
            runtime->drc_param_out.reg_ltm_de_lmap_lut_slope[1];
        cfg_1->REG_HA8.bits.LTM_DE_LMAP_LUT_SLOPE_2 =
            runtime->drc_param_out.reg_ltm_de_lmap_lut_slope[2];
        ...
        cfg_1->REG_HB0.bits.LTM_BE_LMAP_LUT_OUT_0 =
            runtime->drc_param_out.reg_ltm_be_lmap_lut_out[0];
        cfg_1->REG_HB0.bits.LTM_BE_LMAP_LUT_OUT_1 =
            runtime->drc_param_out.reg_ltm_be_lmap_lut_out[1];
        cfg_1->REG_HB0.bits.LTM_BE_LMAP_LUT_OUT_2 =
            runtime->drc_param_out.reg_ltm_be_lmap_lut_out[2];
        cfg_1->REG_HB0.bits.LTM_BE_LMAP_LUT_OUT_3 =
            runtime->drc_param_out.reg_ltm_be_lmap_lut_out[3];
        cfg_1->REG_HB4.bits.LTM_BE_LMAP_LUT_SLOPE_0 =
            runtime->drc_param_out.reg_ltm_be_lmap_lut_slope[0];
        cfg_1->REG_HB4.bits.LTM_BE_LMAP_LUT_SLOPE_1 =
            runtime->drc_param_out.reg_ltm_be_lmap_lut_slope[1];
        cfg_1->REG_HB8.bits.LTM_BE_LMAP_LUT_SLOPE_2 =
            runtime->drc_param_out.reg_ltm_be_lmap_lut_slope[2];
        ...
        cfg_1->REG_HC8.bits.LTM_EE_MOTION_LUT_SLOPE_0 =
            runtime->drc_param_out.DetailEnhanceMtSlope[0];
        cfg_1->REG_HC8.bits.LTM_EE_MOTION_LUT_SLOPE_1 =
            runtime->drc_param_out.DetailEnhanceMtSlope[1];
        cfg_1->REG_HCC.bits.LTM_EE_MOTION_LUT_SLOPE_2 =
            runtime->drc_param_out.DetailEnhanceMtSlope[2];
        ...
        cfg_2->REG_H28.bits.LTM_G_CURVE_1_QUAN_BIT =
            runtime->drc_param_out.reg_ltm_g_curve_1_quan_bit;
...
static CVI_S32 isp_drc_ctrl_ready(VI_PIPE ViPipe)
{
    ...
    if (!runtime->initialized || !runtime->mapped) ret = CVI_FAILURE;
                                                                ...
                                                                ISP_DRC_ATTR_S *pstDRCAttr)
{
    ...
    bModifyRGBGamma =
        (pstDRCAttr->Enable) && ((pstDRCAttr->TuningMode == 1) || (pstDRCAttr->TuningMode == 2));

    if (runtime->modify_rgbgamma_for_drc_tuning != bModifyRGBGamma) {
        runtime->modify_rgbgamma_for_drc_tuning = bModifyRGBGamma;

        isp_gamma_ctrl_set_rgbgamma_curve(ViPipe, (runtime->modify_rgbgamma_for_drc_tuning)
