...
// #define ISP_DGAIN_APPLY_DELAY 3        // When ae Set ISP Dgain. Need count 3 frame to apply to
// ISP.
...
CVI_S32 isp_feature_ctrl_init(VI_PIPE ViPipe)
{
    ...
    for (CVI_U32 idx = ISP_PRE_RAW_START; idx < ISP_IQ_BLOCK_MAX; idx++) {
        if (mod[idx])
            if (mod[idx]->init) mod[idx]->init(ViPipe);
    }
...
CVI_S32 isp_feature_ctrl_uninit(VI_PIPE ViPipe)
{
    ...
    for (CVI_U32 idx = ISP_PRE_RAW_START; idx < ISP_IQ_BLOCK_MAX; idx++) {
        if (mod[idx])
            if (mod[idx]->uninit) mod[idx]->uninit(ViPipe);
    }
...
CVI_S32 isp_feature_ctrl_suspend(VI_PIPE ViPipe)
{
    ...
    for (CVI_U32 idx = ISP_PRE_RAW_START; idx < ISP_IQ_BLOCK_MAX; idx++) {
        if (mod[idx])
            if (mod[idx]->suspend) mod[idx]->suspend(ViPipe);
    }
...
CVI_S32 isp_feature_ctrl_resume(VI_PIPE ViPipe)
{
    ...
    for (CVI_U32 idx = ISP_PRE_RAW_START; idx < ISP_IQ_BLOCK_MAX; idx++) {
        if (mod[idx])
            if (mod[idx]->resume) mod[idx]->resume(ViPipe);
    }
...
CVI_S32 isp_feature_ctrl_pre_fe_eof(VI_PIPE ViPipe)
{
    ...
    for (CVI_U32 idx = ISP_PRE_RAW_START; idx < ISP_IQ_BLOCK_MAX; idx++) {
        if (mod[idx])
            if (mod[idx]->ctrl) mod[idx]->ctrl(ViPipe, MOD_CMD_PREFE_EOF, NULL);
    }
...
CVI_S32 isp_feature_ctrl_pre_be_eof(VI_PIPE ViPipe)
{
    ...
    for (CVI_U32 idx = ISP_PRE_RAW_START; idx < ISP_IQ_BLOCK_MAX; idx++) {
        if (mod[idx])
            if (mod[idx]->ctrl) mod[idx]->ctrl(ViPipe, MOD_CMD_PREBE_EOF, NULL);
    }

    return ret;
}

CVI_S32 isp_feature_ctrl_post_eof(VI_PIPE ViPipe, CVI_U8 lft_frm)
{
    CVI_S32                           ret         = CVI_SUCCESS;
    struct isp_feature_ctrl_runtime **runtime_ptr = _get_feature_ctrl_runtime(ViPipe);
    ...
    case FE_OFF_BE_ON_POST_ON_SC:
        ...
        u8IspTimingDelay = 0;
        break;
    case FE_OFF_BE_ON_POST_SWITCH:
        u8IspSceneDelay  = 1;
        u8IspTimingDelay = 0;
        VI_DEV_ATTR_S stDevAttr;
        CVI_VI_GetDevAttr(ViPipe, &stDevAttr);
        // In MIPI switch mode, switch to another channel after dstFrm
        // lft_frm : the remaining number of frames to switch.
        // The following situation requires adjusting the response frame.
        // For more details, please refer to the wiki page.
        // https://wiki.sophgo.com/pages/viewpage.action?pageId=143924153
        if (lft_frm == 2 || (stDevAttr.dstFrm < 3 && lft_frm == 0)) {
            u8IspTimingDelay = 1;
            u8Switch         = 1;
        }
        break;
    ...
    if (pstAeResult->u8MeterFramePeriod + u8IspSceneDelay <
        ISP_DGAIN_APPLY_DELAY + u8IspTimingDelay) {
        aeDelayIdx = currentFrameIdx;
    } else {
        aeDelayIdx = (pstIspCtx->frameCnt + pstAeResult->u8MeterFramePeriod -
                      ISP_DGAIN_APPLY_DELAY + u8IspSceneDelay - u8IspTimingDelay) %
                     MAX_ALGO_RESULT_QUEUE_NUM;
    }
    ...
    // Fix the next index no data after response frame change in MIPI switch mode.
    if (u8Switch) {
        CVI_U8 u8SwitchIdx = (aeDelayIdx + 1) % MAX_ALGO_RESULT_QUEUE_NUM;
        runtime->algoResultSave[u8SwitchIdx].u32PostIso      = pstAeResult->u32Iso;
        runtime->algoResultSave[u8SwitchIdx].u32PostBlcIso   = pstIspCtx->stAeResult.u32BlcIso;
        runtime->algoResultSave[u8SwitchIdx].au32ExpRatio[0] = expRatio;
        runtime->algoResultSave[u8SwitchIdx].u32IspPostDgain =
            (pstAeResult->u32IspDgain == 0) ? DGAIN_UNIT : pstAeResult->u32IspDgain;
        runtime->algoResultSave[u8SwitchIdx].u32IspPostDgainSE = pstAeResult->u32IspDgainSF;
        runtime->algoResultSave[u8SwitchIdx].u32PostNpIso      = pstIspCtx->stAeResult.u32BlcIso;
        runtime->algoResultSave[u8SwitchIdx].fTnrComRatio[ISP_CHANNEL_LE] = 1.0;
        runtime->algoResultSave[u8SwitchIdx].fTnrComRatio[ISP_CHANNEL_SE] = 1.0;

        u8SwitchIdx = (ispPrerawDelayIdx + 1) % MAX_ALGO_RESULT_QUEUE_NUM;
        runtime->algoResultSave[u8SwitchIdx].u32IspPreDgain =
            (pstAeResult->u32IspDgain == 0) ? DGAIN_UNIT : pstAeResult->u32IspDgain;
        runtime->algoResultSave[u8SwitchIdx].u32IspPreDgainSE = pstAeResult->u32IspDgainSF;
        runtime->algoResultSave[u8SwitchIdx].u32PreIso        = pstAeResult->u32Iso;
        runtime->algoResultSave[u8SwitchIdx].u32PreBlcIso     = pstIspCtx->stAeResult.u32BlcIso;
        runtime->algoResultSave[u8SwitchIdx].u32PreNpIso      = pstIspCtx->stAeResult.u32BlcIso;
    }
    // AE delay frame info.
    ...
    //(pstAeResult->u32IspDgain == 0) ? DGAIN_UNIT : pstAeResult->u32IspDgain;
    runtime->algoResultSave[ispPrerawDelayIdx].u32IspPreDgainSE =
        DGAIN_UNIT;  // pstAeResult->u32IspDgainSF;
    ...
    isp_interpolate_update(ViPipe, runtime->algoResultSave[currentFrameIdx].u32PreIso,
                           runtime->algoResultSave[currentFrameIdx].u32PostIso,
                           runtime->algoResultSave[currentFrameIdx].u32PreBlcIso,
                           runtime->algoResultSave[currentFrameIdx].u32PostBlcIso, expRatio,
                           runtime->algoResultSave[currentFrameIdx].currentLV);
...
CVI_S32 isp_feature_ctrl_set_module_ctrl(VI_PIPE ViPipe, ISP_MODULE_CTRL_U *modCtrl)
{
    ...
    for (CVI_U32 idx = ISP_PRE_RAW_START; idx < ISP_IQ_BLOCK_MAX; idx++) {
        if (mod[idx])
            if (mod[idx]->ctrl) mod[idx]->ctrl(ViPipe, MOD_CMD_SET_MODCTRL, modCtrl);
    }
...
CVI_S32 isp_feature_ctrl_get_module_ctrl(VI_PIPE ViPipe, ISP_MODULE_CTRL_U *modCtrl)
{
    ...
    for (CVI_U32 idx = ISP_PRE_RAW_START; idx < ISP_IQ_BLOCK_MAX; idx++) {
        if (mod[idx])
            if (mod[idx]->ctrl) mod[idx]->ctrl(ViPipe, MOD_CMD_GET_MODCTRL, modCtrl);
    }
...
CVI_S32 isp_feature_ctrl_get_module_info(VI_PIPE ViPipe, ISP_INNER_STATE_INFO_S *innerParam)
{
    ...
    ISP_FORLOOP_SET(innerParam->drcGlobalTone, drc_info.globalToneResult,
                    LTM_GLOBAL_CURVE_NODE_NUM);
    ISP_FORLOOP_SET(innerParam->drcDarkTone, drc_info.darkToneResult, LTM_DARK_CURVE_NODE_NUM);
    ISP_FORLOOP_SET(innerParam->drcBrightTone, drc_info.brightToneResult,
                    LTM_BRIGHT_CURVE_NODE_NUM);
