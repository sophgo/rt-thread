...
#define FRAME_FREEZE_BY_3A_ALGO_CHECK_FRAMES          0
#define FRAME_FREEZE_BY_3A_ALGO_INITIAL_FREEZE_OFFSET 8
#define FRAME_FREEZE_BY_3A_ALGO_FREEZE_OFFSET         3

static CVI_U8 g_bypassFrm[VI_MAX_DEV_NUM] = {FRAME_FREEZE_BY_3A_ALGO_CHECK_FRAMES,
                                             FRAME_FREEZE_BY_3A_ALGO_CHECK_FRAMES};
...
CVI_S32 isp_freeze_init(VI_PIPE ViPipe)
{
    ...
    runtime->maxCheckFrameIndex = g_bypassFrm[ViPipe];
...
CVI_S32 isp_freeze_set_freeze(VI_PIPE ViPipe, CVI_U64 frmae_idx, CVI_BOOL stable)
{
    ...
    if (runtime->enable) {
        ...
        if (frmae_idx >= g_bypassFrm[ViPipe]) {
        ...
        for (CVI_U32 u32VIPipe = 0; u32VIPipe < VI_MAX_PIPE_NUM; ++u32VIPipe) {
            if (freeze_runtime[u32VIPipe].enable) {
                all_stable &=
                    (freeze_runtime[u32VIPipe].stable || freeze_runtime[u32VIPipe].yuvbypass);
            } else {
                all_stable &= CVI_TRUE;
            }
        }
...
CVI_S32 isp_freeze_set_fw_state(VI_PIPE ViPipe, ISP_FMW_STATE_E state)
{
    ...
    if (state >= ISP_FMW_STATE_BUTT) return CVI_FAILURE;
...
CVI_S32 isp_freeze_set_bypass_frame_num(VI_PIPE ViPipe, CVI_U8 bypassNum)
{
    g_bypassFrm[ViPipe] = bypassNum;
    return CVI_SUCCESS;
}

CVI_U8 isp_freeze_get_bypass_frame_num(VI_PIPE ViPipe)
{
    return g_bypassFrm[ViPipe];
}
