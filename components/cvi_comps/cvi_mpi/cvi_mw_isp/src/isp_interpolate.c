...
static CVI_S16 lvLut[ISP_AUTO_LV_NUM] = {-500, -400, -300, -200, -100, 0,    100,
...
static inline CVI_S32  interpolate_iso_imp(CVI_S32 v, CVI_S32 x0, CVI_S32 y0, CVI_S32 x1,
                                           CVI_S32 y1);
static inline CVI_VOID get_interpolate_info(VI_PIPE ViPipe, enum INTPLT_TYPE type, CVI_S32 *val,
                                            CVI_S32 *val1, CVI_S32 *val2, CVI_S32 *idx1,
                                            CVI_S32 *idx2);
...
CVI_U8 intrplt_u8(VI_PIPE ViPipe, enum INTPLT_TYPE type, const CVI_U8 *arr)
{
    ...
    return (CVI_U8)interpolate_iso_imp(val, val1, (CVI_S32)arr[idx1], val2, (CVI_S32)arr[idx2]);
}
...
CVI_U16 intrplt_u16(VI_PIPE ViPipe, enum INTPLT_TYPE type, const CVI_U16 *arr)
{
    ...
    return (CVI_U16)interpolate_iso_imp(val, val1, (CVI_S32)arr[idx1], val2, (CVI_S32)arr[idx2]);
}
...
CVI_U32 intrplt_u32(VI_PIPE ViPipe, enum INTPLT_TYPE type, const CVI_U32 *arr)
{
    ...
    return (CVI_U32)interpolate_iso_imp(val, val1, (CVI_S32)arr[idx1], val2, (CVI_S32)arr[idx2]);
}
...
CVI_S8 intrplt_s8(VI_PIPE ViPipe, enum INTPLT_TYPE type, const CVI_S8 *arr)
{
    ...
    return (CVI_S8)interpolate_iso_imp(val, val1, (CVI_S32)arr[idx1], val2, (CVI_S32)arr[idx2]);
}
...
CVI_S16 intrplt_s16(VI_PIPE ViPipe, enum INTPLT_TYPE type, const CVI_S16 *arr)
{
    ...
    return (CVI_S16)interpolate_iso_imp(val, val1, (CVI_S32)arr[idx1], val2, (CVI_S32)arr[idx2]);
}
...
CVI_S32 intrplt_s32(VI_PIPE ViPipe, enum INTPLT_TYPE type, const CVI_S32 *arr)
{
    ...
    return (CVI_S32)interpolate_iso_imp(val, val1, (CVI_S32)arr[idx1], val2, (CVI_S32)arr[idx2]);
}
...
CVI_S32 isp_interpolate_update(VI_PIPE ViPipe, CVI_U32 pre_iso, CVI_U32 post_iso,
                               CVI_U32 pre_blc_iso, CVI_U32 post_blc_iso, CVI_U32 exp_ratio,
                               CVI_S16 lv)
{
    struct isp_interpolate_runtime *runtime = _get_interpolate_runtime(ViPipe);
