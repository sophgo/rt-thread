...
const struct isp_module_ctrl lcac_mod = {.init    = isp_lcac_ctrl_init,
...
static CVI_S32 isp_lcac_ctrl_preprocess(VI_PIPE ViPipe, ISP_ALGO_RESULT_S *algoResult)
{
    ...
    if (!lcac_attr->Enable || runtime->is_module_bypass) return ret;
...
static CVI_S32 isp_lcac_ctrl_postprocess(VI_PIPE ViPipe)
{
    ...
    if (is_postprocess_update == CVI_FALSE) {
        ...
        lcac_tun_cfg->REG44.bits.LCAC_FCF_FILTER_KERNEL_00 =
            plcac_param->au8FCFFilterKernel[0];  // 7
        lcac_tun_cfg->REG44.bits.LCAC_FCF_FILTER_KERNEL_01 =
            plcac_param->au8FCFFilterKernel[1];  // 7
        lcac_tun_cfg->REG44.bits.LCAC_FCF_FILTER_KERNEL_02 =
            plcac_param->au8FCFFilterKernel[2];  // 7
        lcac_tun_cfg->REG44.bits.LCAC_FCF_FILTER_KERNEL_03 =
            plcac_param->au8FCFFilterKernel[3];             // 7
        ...
        lcac_tun_cfg->REG4C.bits.LCAC_FCF_FILTER_KERNEL_04 =
            plcac_param->au8FCFFilterKernel[4];             // 7
