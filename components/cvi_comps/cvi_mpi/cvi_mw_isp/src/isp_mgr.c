...
static CVI_S32 unset_sensorname_to_environment(VI_PIPE ViPipe);
...
CVI_S32 isp_mgr_uninit(VI_PIPE ViPipe)
{
    ...
    unset_sensorname_to_environment(ViPipe);
    isp_proc_exit();
    int count = 40;

    while (pstIspCtx->bIspRun && (--count > 0)) USLEEP_COMPAT(50 * 1000);
...
CVI_S32 isp_mgr_post_eof(VI_PIPE ViPipe, CVI_U32 frame_idx, CVI_U8 lft_frm)
{
    ISP_LOG_DEBUG("+\n");
    CVI_S32         ret       = CVI_SUCCESS;
    ISP_CTX_S      *pstIspCtx = NULL;
    ...
    CVI_BOOL FW_RUN =
        ((state == ISP_FMW_STATE_RUN) && (pstIspCtx->frameCnt > pstIspCtx->u8AEWaitFrame));
...
#endif
        isp_feature_ctrl_post_eof(ViPipe, lft_frm);
#ifdef ENABLE_ISP_C906L
...
#define ENV_DEBUG_MESSAGE_TO_STDOUT "ISP_ENABLE_STDOUT_DEBUG"
    ...
    if (pszEnv == NULL) return CVI_FAILURE;
...
#define NAME_SIZE 20
    ...
    CVI_CHAR sensorNameEnv[NAME_SIZE] = {0};

    CVI_S32 ret = 0;

    snprintf(sensorNameEnv, NAME_SIZE, "SENSORNAME%d", ViPipe);
    ret = unsetenv(sensorNameEnv);
    if (ret != 0) {
        ISP_LOG_WARNING("unset env value:%s fail.\n", sensorNameEnv);
    }
