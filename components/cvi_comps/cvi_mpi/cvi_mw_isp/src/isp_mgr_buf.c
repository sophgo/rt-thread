...
CVI_S32 isp_mgr_buf_init(VI_PIPE ViPipe)
{
    ...
    ISP_LOG_DEBUG("size,%ld,%ld,%ld,%ld,%ld,%ld,%ld,%ld,%ld,%ld,%ld,%ld\n",
                  sizeof(struct rtos_isp_cmd_event), sizeof(struct isp_tun_buf_ctrl_runtime),
                  sizeof(struct isp_sts_ctrl_runtime), sizeof(struct isp_mlsc_shared_buffer),
                  sizeof(struct isp_dpc_shared_buffer), sizeof(struct isp_dci_shared_buffer),
                  sizeof(struct isp_demosaic_shared_buffer), sizeof(struct isp_ldci_shared_buffer),
                  sizeof(struct isp_dis_shared_buffer), offsetof(struct isp_shared_buffer, ispCtx),
                  sizeof(ISP_CTX_S), sizeof(struct isp_shared_buffer));
    ...
    if (CVI_SYS_IonAlloc_Cached(&(runtime->paddr), &(runtime->vaddr), ion_name, runtime->len) < 0) {
        ISP_LOG_DEBUG("ISP Vipipe(%d) Allocate %s failed\n", ViPipe, ion_name);
        ret = CVI_FAILURE;
    } else {
        ISP_LOG_DEBUG("ISP Vipipe(%d) Allocate pa(%llx) va(0x%p) size(%d)\n", ViPipe,
                      runtime->paddr, runtime->vaddr, runtime->len);
    }
    ...
    g_astIspCtx[ViPipe] =
        (ISP_CTX_S *)((CVI_U8 *)runtime->vaddr + offsetof(struct isp_shared_buffer, ispCtx));
#else
    ...
    isp_mgr_buf_flush_cache(ViPipe);
    g_astIspCtx[ViPipe] =
        (ISP_CTX_S *)(runtime->paddr + offsetof(struct isp_shared_buffer, ispCtx));
#endif
...
#ifndef ARCH_RTOS_CV181X
    ISP_LOG_DEBUG("ISP Vipipe(%d) Free pa(%llx) va(0x%p)\n", ViPipe, runtime->paddr,
                  runtime->vaddr);
...
CVI_S32 isp_mgr_buf_get_addr(VI_PIPE ViPipe, ISP_IQ_BLOCK_LIST_E block, CVI_VOID **addr)
{
    ...
    case ISP_IQ_BLOCK_DEMOSAIC:
        *addr =
            (CVI_VOID *)((CVI_U8 *)runtime->vaddr + offsetof(struct isp_shared_buffer, demosaic));
        break;
    case ISP_IQ_BLOCK_FUSION:
    ...
    case ISP_IQ_BLOCK_PREYEE:
        *addr =
            (CVI_VOID *)((CVI_U8 *)runtime->vaddr + offsetof(struct isp_shared_buffer, presharpen));
        break;
    case ISP_IQ_BLOCK_YEE:
        *addr =
            (CVI_VOID *)((CVI_U8 *)runtime->vaddr + offsetof(struct isp_shared_buffer, sharpen));
        break;
    case ISP_IQ_BLOCK_BLC:
    ...
    case ISP_IQ_BLOCK_CROSSTALK:
        *addr =
            (CVI_VOID *)((CVI_U8 *)runtime->vaddr + offsetof(struct isp_shared_buffer, crosstalk));
        break;
    case ISP_IQ_BLOCK_CA2:
