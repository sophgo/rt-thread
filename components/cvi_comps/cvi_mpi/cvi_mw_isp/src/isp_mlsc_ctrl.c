...
const struct isp_module_ctrl mlsc_mod = {.init    = isp_mlsc_ctrl_init,
...
static CVI_S32 isp_mlsc_ctrl_check_mlsc_attr_valid(
...
static CVI_S32 isp_mlsc_ctrl_preprocess(VI_PIPE ViPipe, ISP_ALGO_RESULT_S *algoResult)
{
    ...
    if (!mlsc_attr->Enable || runtime->is_module_bypass) return ret;
...
#ifndef ENABLE_ISP_C906L
    ret = isp_algo_mlsc_main((struct mlsc_param_in *)&runtime->mlsc_param_in,
    ...
    if (is_postprocess_update == CVI_FALSE) {
        ...
        if (mlsc_attr->Enable && !runtime->is_module_bypass) {
            ...
            if (runtime->mlsc_mem_info.size) {
#ifndef ARCH_RTOS_CV181X
                isp_mlsc_ctrl_packing_tbl((CVI_U8 *)(runtime->mlsc_mem_info.vir_addr),
                ...
                FLUSH_CACHE_COMPAT(runtime->mlsc_mem_info.phy_addr, runtime->mlsc_mem_info.vir_addr,
                                   runtime->mlsc_mem_info.size);
#else
                isp_mlsc_ctrl_packing_tbl((CVI_U8 *)(runtime->mlsc_mem_info.phy_addr),
                                          ISP_PTR_CAST_U16(runtime->mlsc_param_out.mlsc_lut_r),
                                          ISP_PTR_CAST_U16(runtime->mlsc_param_out.mlsc_lut_g),
                                          ISP_PTR_CAST_U16(runtime->mlsc_param_out.mlsc_lut_b));
#endif
...
static CVI_S32 isp_mlsc_ctrl_buf_init(VI_PIPE ViPipe)
{
    ...
    if (runtime->mapped == CVI_TRUE) return ret;
    ...
    if (mem_info->size) {
        mem_info->vir_addr = (void *)mem_info->phy_addr;
...
#define LSC_GAIN_ROW_SIZE                                                                          \
    (((((CVI_ISP_LSC_GRID_COL + LSC_GAIN_UNIT - 1) / LSC_GAIN_UNIT) * LSC_GAIN_UNIT_SIZE) + 15) >> \
     4 << 4)
static CVI_S32 isp_mlsc_ctrl_packing_tbl(CVI_U8 *lsctable, CVI_U16 *r, CVI_U16 *g, CVI_U16 *b)
{
    if (lsctable == CVI_NULL) return CVI_FAILURE;
    if (r == CVI_NULL) return CVI_FAILURE;
    if (b == CVI_NULL) return CVI_FAILURE;
    if (g == CVI_NULL) return CVI_FAILURE;
    ...
    for (i = 0, col = 0; i < CVI_ISP_LSC_GRID_ROW; i += LSC_GAIN_UNIT, col += LSC_GAIN_UNIT_SIZE) {
        for (k = 0; k < LSC_COLOR_CHANNEL_MAX; k++) {
            ...
            for (j = 0; j < CVI_ISP_LSC_GRID_COL; j += vl) {
                ...
                vu16C1 =
                    vlse16_v_u16m8(&channel[k][j * CVI_ISP_LSC_GRID_ROW + i + 1], src_offset, vl);
...
//-----------------------------------------------------------------------------
CVI_S32 isp_mlsc_ctrl_get_mlsc_attr(VI_PIPE                         ViPipe,
                                    const ISP_MESH_SHADING_ATTR_S **pstMeshShadingAttr)
{
...
CVI_S32 isp_mlsc_ctrl_set_mlsc_attr(VI_PIPE                        ViPipe,
                                    const ISP_MESH_SHADING_ATTR_S *pstMeshShadingAttr)
{
    if (pstMeshShadingAttr == CVI_NULL) {
        return CVI_FAILURE;
...
CVI_S32 isp_mlsc_ctrl_get_mlsc_lut_attr(
...
CVI_S32 isp_mlsc_ctrl_set_mlsc_lut_attr(
    VI_PIPE ViPipe, const ISP_MESH_SHADING_GAIN_LUT_ATTR_S *pstMeshShadingGainLutAttr)
{
    if (pstMeshShadingGainLutAttr == CVI_NULL) {
        return CVI_FAILURE;
