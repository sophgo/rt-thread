...
const struct isp_module_ctrl motion_mod = {.init    = isp_motion_ctrl_init,
...
static CVI_S32 isp_motion_ctrl_preprocess(VI_PIPE ViPipe, ISP_ALGO_RESULT_S *algoResult)
{
    ...
    if (runtime->is_module_bypass) return ret;
    ...
    ptParamIn->motionThreshold  = motion_attr->MotionThreshold;
...
static CVI_S32 isp_motion_ctrl_process(VI_PIPE ViPipe)
{
    ...
    // ALGO
    isp_algo_motion_main((struct motion_param_in *)&runtime->motion_param_in,
