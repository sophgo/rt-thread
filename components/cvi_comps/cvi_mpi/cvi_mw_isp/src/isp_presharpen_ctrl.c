...
const struct isp_module_ctrl presharpen_mod = {.init    = isp_presharpen_ctrl_init,
...
static CVI_S32 isp_presharpen_ctrl_check_presharpen_attr_valid(
    const ISP_PRESHARPEN_ATTR_S *pstpresharpenAttr);
...
static CVI_S32 isp_presharpen_ctrl_preprocess(VI_PIPE ViPipe, ISP_ALGO_RESULT_S *algoResult)
{
    ...
    if (!presharpen_attr->Enable || runtime->is_module_bypass) return ret;
...
#undef AUTO
    ...
    for (CVI_U32 i = 0; i < EE_LUT_NODE; i++) {
        ...
        runtime->presharpen_param_in.MotionCorLutOut[i] =
            runtime->presharpen_attr.MotionCorLutOut[i];
        runtime->presharpen_param_in.MotionCorWgtLutIn[i] =
            runtime->presharpen_attr.MotionCorWgtLutIn[i];
        runtime->presharpen_param_in.MotionCorWgtLutOut[i] =
            runtime->presharpen_attr.MotionCorWgtLutOut[i];
        runtime->presharpen_param_in.MotionShtGainIn[i] =
            runtime->presharpen_attr.MotionShtGainIn[i];
        runtime->presharpen_param_in.MotionShtGainOut[i] =
            runtime->presharpen_attr.MotionShtGainOut[i];
...
static CVI_S32 isp_presharpen_ctrl_process(VI_PIPE ViPipe)
{
    ...
    ret = isp_algo_presharpen_main((struct presharpen_param_in *)&runtime->presharpen_param_in,
...
static CVI_S32 isp_presharpen_ctrl_postprocess(VI_PIPE ViPipe)
{
    ...
    if (is_postprocess_update == CVI_FALSE) {
        ...
        memcpy(presharpen_cfg->luma_adptctrl_lut, runtime->presharpen_attr.LumaAdpGain,
               sizeof(CVI_U8) * 33);
        memcpy(presharpen_cfg->delta_adptctrl_lut, runtime->presharpen_attr.DeltaAdpGain,
               sizeof(CVI_U8) * 33);
        memcpy(presharpen_cfg->chroma_adptctrl_lut, runtime->presharpen_attr.HueShtCtrl,
               sizeof(CVI_U8) * 33);
        ...
        cfg_1->REG_AC.bits.EE_LUMA_CORING_LUT_SLOPE_0 =
            runtime->presharpen_param_out.LumaCorLutSlope[0];
        cfg_1->REG_AC.bits.EE_LUMA_CORING_LUT_SLOPE_1 =
            runtime->presharpen_param_out.LumaCorLutSlope[1];
        cfg_1->REG_B0.bits.EE_LUMA_CORING_LUT_SLOPE_2 =
            runtime->presharpen_param_out.LumaCorLutSlope[2];
        ...
        cfg_1->REG_BC.bits.EE_MOTION_CORING_LUT_SLOPE_0 =
            runtime->presharpen_param_out.MotionCorLutSlope[0];
        cfg_1->REG_BC.bits.EE_MOTION_CORING_LUT_SLOPE_1 =
            runtime->presharpen_param_out.MotionCorLutSlope[1];
        cfg_1->REG_C0.bits.EE_MOTION_CORING_LUT_SLOPE_2 =
            runtime->presharpen_param_out.MotionCorLutSlope[2];
        ...
        cfg_1->REG_HCC.bits.EE_MCORE_GAIN_LUT_SLOPE_0 =
            runtime->presharpen_param_out.MotionCorWgtLutSlope[0];
        cfg_1->REG_HCC.bits.EE_MCORE_GAIN_LUT_SLOPE_1 =
            runtime->presharpen_param_out.MotionCorWgtLutSlope[1];
        cfg_1->REG_HD0.bits.EE_MCORE_GAIN_LUT_SLOPE_2 =
            runtime->presharpen_param_out.MotionCorWgtLutSlope[2];
        ...
        cfg_2->REG_1A4.bits.EE_CHROMA_AMP_LUT_SLOPE_0 =
            runtime->presharpen_param_out.SatShtGainSlope[0];
        cfg_2->REG_1A4.bits.EE_CHROMA_AMP_LUT_SLOPE_1 =
            runtime->presharpen_param_out.SatShtGainSlope[1];
        cfg_2->REG_1A8.bits.EE_CHROMA_AMP_LUT_SLOPE_2 =
            runtime->presharpen_param_out.SatShtGainSlope[2];
        ...
        cfg_3->REG_1D4.bits.EE_MOTION_LUT_SLOPE_0 =
            runtime->presharpen_param_out.MotionShtGainSlope[0];
        cfg_3->REG_1D4.bits.EE_MOTION_LUT_SLOPE_1 =
            runtime->presharpen_param_out.MotionShtGainSlope[1];
        cfg_3->REG_1D8.bits.EE_MOTION_LUT_SLOPE_2 =
            runtime->presharpen_param_out.MotionShtGainSlope[2];
    }
...
//-----------------------------------------------------------------------------
CVI_S32 isp_presharpen_ctrl_get_presharpen_attr(VI_PIPE                       ViPipe,
                                                const ISP_PRESHARPEN_ATTR_S **pstPreSharpenAttr)
{
...
CVI_S32 isp_presharpen_ctrl_set_presharpen_attr(VI_PIPE                      ViPipe,
                                                const ISP_PRESHARPEN_ATTR_S *pstPreSharpenAttr)
{
    if (pstPreSharpenAttr == CVI_NULL) {
        return CVI_FAILURE;
