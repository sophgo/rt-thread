...
const struct isp_module_ctrl rgbcac_mod = {.init    = isp_rgbcac_ctrl_init,
...
static CVI_S32 isp_rgbcac_ctrl_preprocess(VI_PIPE ViPipe, ISP_ALGO_RESULT_S *algoResult)
{
    ...
    if (!rgbcac_attr->Enable || runtime->is_module_bypass) return ret;
...
static CVI_S32 isp_rgbcac_ctrl_process(VI_PIPE ViPipe)
{
    ...
    ret = isp_algo_rgbcac_main((struct rgbcac_param_in *)&runtime->rgbcac_param_in,
...
static CVI_S32 isp_rgbcac_ctrl_postprocess(VI_PIPE ViPipe)
{
    ...
    if (is_postprocess_update == CVI_FALSE) {
        ...
        rgbcac_tun_cfg->RGBCAC_PURPLE_TH.bits.RGBCAC_CORRECT_STRENGTH_LE =
            runtime->rgbcac_attr.DePurpleStr0;
        rgbcac_tun_cfg->RGBCAC_PURPLE_TH.bits.RGBCAC_CORRECT_STRENGTH_SE =
            runtime->rgbcac_attr.DePurpleStr1;
        ...
        rgbcac_tun_cfg->RGBCAC_EDGE_CORING.bits.RGBCAC_EDGE_CORING =
            runtime->rgbcac_attr.EdgeCoring;
