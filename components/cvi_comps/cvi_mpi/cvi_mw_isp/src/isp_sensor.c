...
CVI_S32 isp_sensor_register(CVI_S32 ViPipe, ISP_SNS_ATTR_INFO_S *pstSnsAttrInfo,
...
CVI_S32 isp_sensor_init(CVI_S32 ViPipe)
{
    ...
    if (pSnsInfo->snsAttr.bInitByUser) {
        ISP_LOG_ERR("ViPipe %d Init ByUser\n", ViPipe);
        return CVI_SUCCESS;
    }
...
CVI_S32 isp_sensor_updateBlc(VI_PIPE ViPipe, ISP_BLACK_LEVEL_ATTR_S **ppstSnsBlackLevel)
{
    ...
    if (pSnsInfo->snsRegFunc.stSnsExp.pfn_cmos_get_isp_black_level != NULL) {
        pSnsInfo->snsRegFunc.stSnsExp.pfn_cmos_get_isp_black_level(ViPipe,
                                                                   &pSnsInfo->stSnsBlackLevel);
    } else {
        return CVI_FAILURE;
    }
...
CVI_S32 isp_snsSync_info_set(VI_PIPE ViPipe, ISP_SNS_SYNC_INFO_S *syncCfg, CVI_U32 be_frm_idx)
{
    ...
    if (!AE_GetSensorExpGainPeriod(ViPipe)) {
        snr_update.snr_cfg_node.snsr.magic_num =
            be_frm_idx + 5 - pstIspCtx->stAeResult.u8MeterFramePeriod;
    } else {
        snr_update.snr_cfg_node.snsr.magic_num = be_frm_idx + 5 - AE_GetSensorExpGainPeriod(ViPipe);
    }
    for (i = 0; i < reg_num; i++) {
        ...
        snr_update.snr_cfg_node.snsr.i2c_data[i].dly_frm_num =
            syncCfg->snsCfg.astI2cData[i].u8DelayFrmNum;
        ...
        snr_update.snr_cfg_node.snsr.i2c_data[i].reg_addr =
            syncCfg->snsCfg.astI2cData[i].u32RegAddr;
        snr_update.snr_cfg_node.snsr.i2c_data[i].addr_bytes =
            syncCfg->snsCfg.astI2cData[i].u32AddrByteNum;
        ...
        snr_update.snr_cfg_node.snsr.i2c_data[i].data_bytes =
            syncCfg->snsCfg.astI2cData[i].u32DataByteNum;

        snr_update.snr_cfg_node.snsr.i2c_data[i].drop_frame =
            syncCfg->snsCfg.astI2cData[i].bDropFrm;
        snr_update.snr_cfg_node.snsr.i2c_data[i].drop_frame_cnt =
            syncCfg->snsCfg.astI2cData[i].u8DropFrmNum;
    }
    ...
    snr_update.snr_cfg_node.cif.wdr_manu.attr.l2s_distance =
        syncCfg->cifCfg.wdr_manual.l2s_distance;
...
CVI_S32 isp_sns_info_init(VI_PIPE ViPipe)
{
    ...
    for (i = 0; i < snsSyncInfo->ispCfg.frm_num; i++) {
        ...
        cfg.snr_fmt.img_size[i].max_width =
            snsSyncInfo->ispCfg.img_size[snsExpIndex].stMaxSize.u32Width;
        cfg.snr_fmt.img_size[i].max_height =
            snsSyncInfo->ispCfg.img_size[snsExpIndex].stMaxSize.u32Height;
        ...
        cfg.snr_fmt.img_size[i].active_w =
            snsSyncInfo->ispCfg.img_size[snsExpIndex].stWndRect.u32Width;
        cfg.snr_fmt.img_size[i].active_h =
            snsSyncInfo->ispCfg.img_size[snsExpIndex].stWndRect.u32Height;
        cfg.snr_fmt.img_size[i].width =
            snsSyncInfo->ispCfg.img_size[snsExpIndex].stSnsSize.u32Width;
        cfg.snr_fmt.img_size[i].height =
            snsSyncInfo->ispCfg.img_size[snsExpIndex].stSnsSize.u32Height;
    }
