/*
 * Copyright (C) Cvitek Co., Ltd. 2019-2021. All rights reserved.
 *
 * File Name: isp_sharpen_ctrl.h
 * Description:
 *
 */

#ifndef _ISP_SHARPEN_CTRL_H_
#define _ISP_SHARPEN_CTRL_H_

#include "cvi_comm_isp.h"
#include "isp_algo_sharpen.h"
#include "isp_feature_ctrl.h"

#ifdef __cplusplus
#if __cplusplus
extern "C" {
#endif
#endif /* End of #ifdef __cplusplus */

struct isp_sharpen_ctrl_runtime {
    struct sharpen_param_in  sharpen_param_in;
    struct sharpen_param_out sharpen_param_out;

    ISP_SHARPEN_MANUAL_ATTR_S sharpen_attr;

    CVI_BOOL preprocess_updated;
    CVI_BOOL process_updated;
    CVI_BOOL postprocess_updated;
    CVI_BOOL is_module_bypass;
};

CVI_S32 isp_sharpen_ctrl_init(VI_PIPE ViPipe);
CVI_S32 isp_sharpen_ctrl_uninit(VI_PIPE ViPipe);
CVI_S32 isp_sharpen_ctrl_suspend(VI_PIPE ViPipe);
CVI_S32 isp_sharpen_ctrl_resume(VI_PIPE ViPipe);
CVI_S32 isp_sharpen_ctrl_ctrl(VI_PIPE ViPipe, enum isp_module_cmd cmd, CVI_VOID *input);

CVI_S32 isp_sharpen_ctrl_get_sharpen_attr(VI_PIPE                    ViPipe,
                                          const ISP_SHARPEN_ATTR_S **pstSharpenAttr);
CVI_S32 isp_sharpen_ctrl_set_sharpen_attr(VI_PIPE ViPipe, const ISP_SHARPEN_ATTR_S *pstSharpenAttr);

extern const struct isp_module_ctrl sharpen_mod;

#ifdef __cplusplus
#if __cplusplus
}
#endif
#endif /* End of #ifdef __cplusplus */

#endif  // _ISP_SHARPEN_CTRL_H_
