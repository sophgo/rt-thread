...
static CVI_S32 isp_sts_ctrl_unpack_ae_sts(VI_PIPE ViPipe, CVI_VOID *sts_in, CVI_VOID *ae_sts_out,
                                          CVI_VOID *awb_sts_out, ISP_CHANNEL_LIST_E chn,
                                          CVI_U32 ae_max);
...
CVI_S32 isp_sts_ctrl_pre_be_eof(VI_PIPE ViPipe, CVI_U32 frame_cnt)
{
    ...
    if (isp_sts_ctrl_ready(ViPipe) != CVI_SUCCESS) return CVI_FAILURE;
...
CVI_S32 isp_sts_ctrl_get_awb_sts(VI_PIPE ViPipe, ISP_CHANNEL_LIST_E chn,
...
static CVI_S32 isp_sts_ctrl_set_ae_sts(VI_PIPE ViPipe)
{
    ...
    struct cvi_vip_memblock *ae_sts[2] = {&sts_mem->ae_le, &sts_mem->ae_se};
    ...
    for (CVI_U32 chn = 0; chn < ISP_CHANNEL_MAX_NUM; chn++) {
        if (ae_sts[chn]->vir_addr && ae_sts_out) {
            ...
            isp_sts_ctrl_unpack_ae_sts(ViPipe, ae_sts[chn]->vir_addr, ae_sts_out, &(awb_sts[chn]),
                                       chn, 0);
            // isp_sts_ctrl_unpack_ae_face_sts(ViPipe);
            isp_sts_ctrl_unpack_ae_hist_sts(ViPipe, (ae_sts[chn]->vir_addr + hist_ofs), ae_sts_out,
                                            chn, 0);
        }
    }
                                          ...
                                          CVI_U32 ae_max)
{
    ...
    // | ae_g00 | ir_g00 | ae_g01 | ir_g01 | ae_g02 | ir_g02 | ae_g03 | ir_g03 | ae_g04 | ir_g04 |
    // ae_g05 | ir_g05 |
    //  -------- -------- -------- -------- -------- -------- -------- -------- -------- --------
    //  -------- --------
    // | ae_g10 | ir_g10 | ae_g11 | ir_g11 | ae_g12 | ir_g12 | ae_g13 | ir_g13 | ae_g14 | ir_g14 |
    // ae_g15 | ir_g15 |
    // ...
    ...
    ae_sts->aeStat2[ae_max].u16GlobalAvgGr[chn] = ae_sts->aeStat2[ae_max].u16GlobalAvgGb[chn] =
        sum_g / AE_ZONE_NUM;
    ...
    for (CVI_U32 row = 0; row < win_num_row; row++) {
        for (CVI_U32 col = 0; col < win_num_col; col++) {
            ...
            af_sts->stFEAFStat.stZoneMetrics[row][col].u64h1 =
                (*(pAfData + 4) << 16) + *(pAfData + 3);
            af_sts->stFEAFStat.stZoneMetrics[row][col].u64h0 =
                (*(pAfData + 6) << 12) + ((*(pAfData + 5) & 0xFFF0) >> 4);
...
static CVI_S32 isp_sts_ctrl_set_gms_sts(VI_PIPE ViPipe)
{
    if (ViPipe != 0) return CVI_FAILURE;
...
static CVI_S32 isp_sts_ctrl_set_motion_sts(VI_PIPE ViPipe)
{
    ...
    if (motion_sts_in && motion_sts_out) {
        ...
        memcpy(ISP_PTR_CAST_VOID(motion_sts_out->motionStsData), motion_sts_in,
               motion_sts_out->motionStsBufSize);
    }
...
static CVI_S32 isp_sts_ctrl_set_ae_cfg(VI_PIPE ViPipe)
{
    ...
    for (CVI_U8 i = 0; i < 1; i++) {
        ...
        CVI_BOOL map_pipe_to_enable =
            1;  // ae_cfg->inst == 0) ? 1 : IS_2to1_WDR_MODE(pstIspCtx->u8SnsWDRMode);
...
static CVI_S32 isp_sts_ctrl_set_af_cfg(VI_PIPE ViPipe)
{
    CVI_S32 ret = CVI_SUCCESS;
#ifndef ARCH_RTOS_CV181X
    CVI_S8 lowpassSignFactor = 1, horizon0SignFactor = 1, horizon1SignFactor = 1,
           verticalSignFactor                 = 1;
    struct isp_sts_ctrl_runtime **runtime_ptr = _get_sts_ctrl_runtime(ViPipe);
    struct isp_sts_ctrl_runtime  *runtime     = *runtime_ptr;
    ...
    if (pstIspCtx->stsCfgInfo.stFocusCfg.stConfig.stCrop.bEnable) {
        ...
        af_cfg->block_width =
            ((pstIspCtx->stsCfgInfo.stFocusCfg.stConfig.stCrop.u16W) / af_cfg->block_numx) >>
            1 << 1;
        af_cfg->block_height =
            ((pstIspCtx->stsCfgInfo.stFocusCfg.stConfig.stCrop.u16H) / af_cfg->block_numy) >>
            1 << 1;
    } else {
        af_cfg->offsetx = AF_XOFFSET_MIN;
        af_cfg->offsety = AF_YOFFSET_MIN;
        af_cfg->block_width =
            ((pstIspCtx->stSysRect.u32Width - (AF_XOFFSET_MIN << 1)) / af_cfg->block_numx) >>
            1 << 1;
        af_cfg->block_height =
            ((pstIspCtx->stSysRect.u32Height - (AF_YOFFSET_MIN << 1)) / af_cfg->block_numy) >>
            1 << 1;
    }
...
#define SIGN_FACTOR_CHECK(firstFilterCoeff, signFactor) \
    ...
    SIGN_FACTOR_CHECK(pstIspCtx->stsCfgInfo.stFocusCfg.stConfig.s8HVFltLpCoeff[0],
                      lowpassSignFactor);
    SIGN_FACTOR_CHECK(pstIspCtx->stsCfgInfo.stFocusCfg.stHParam_FIR0.s8HFltHpCoeff[0],
                      horizon0SignFactor);
    SIGN_FACTOR_CHECK(pstIspCtx->stsCfgInfo.stFocusCfg.stHParam_FIR1.s8HFltHpCoeff[0],
                      horizon1SignFactor);
    SIGN_FACTOR_CHECK(pstIspCtx->stsCfgInfo.stFocusCfg.stVParam_FIR.s8VFltHpCoeff[0],
                      verticalSignFactor);
    for (CVI_U32 i = 0; i < FIR_H_GAIN_NUM; i++) {
        af_cfg->h_low_pass_coef[i] =
            pstIspCtx->stsCfgInfo.stFocusCfg.stConfig.s8HVFltLpCoeff[i] * lowpassSignFactor;
        af_cfg->h_high_pass_coef_0[i] =
            pstIspCtx->stsCfgInfo.stFocusCfg.stHParam_FIR0.s8HFltHpCoeff[i] * horizon0SignFactor;
        af_cfg->h_high_pass_coef_1[i] =
            pstIspCtx->stsCfgInfo.stFocusCfg.stHParam_FIR1.s8HFltHpCoeff[i] * horizon1SignFactor;
    }
    for (CVI_U32 i = 0; i < FIR_V_GAIN_NUM; i++) {
        af_cfg->v_high_pass_coef[i] =
            pstIspCtx->stsCfgInfo.stFocusCfg.stVParam_FIR.s8VFltHpCoeff[i] * verticalSignFactor;
    }
    // fix coring setting.
...
#ifndef ARCH_RTOS_CV181X
    if (ViPipe != 0) return CVI_SUCCESS;
...
static CVI_S32 isp_sts_ctrl_initial_motion_sts(VI_PIPE ViPipe)
{
    ...
    if (runtime->motion_sts.motionStsData == CVI_NULL) {
        runtime->motion_sts.motionStsData =
            ISP_PTR_CAST_PTR(calloc(1, runtime->motion_sts.motionStsBufSize));
        if (runtime->motion_sts.motionStsData == CVI_NULL) {
...
#define _mmap_cache(va, pa, size)    \
    do {                             \
        if (size) va = (CVI_U8 *)pa; \
    } while (0)
    ...
    for (CVI_U32 i = 0; i < ISP_MAX_STS_BUF_NUM; i++) {
        ...
        _mmap_cache(sts_mem->hist_edge_v.vir_addr, sts_mem->hist_edge_v.phy_addr,
                    sts_mem->hist_edge_v.size);
