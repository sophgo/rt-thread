...
const struct isp_module_ctrl tnr_mod = {.init    = isp_tnr_ctrl_init,
...
static CVI_S32 isp_tnr_ctrl_check_tnr_noise_model_attr_valid(
    const ISP_TNR_NOISE_MODEL_ATTR_S *pstTNRNoiseModelAttr);
static CVI_S32 isp_tnr_ctrl_check_tnr_luma_motion_attr_valid(
    const ISP_TNR_LUMA_MOTION_ATTR_S *pstTNRLumaMotionAttr);
static CVI_S32 isp_tnr_ctrl_check_tnr_ghost_attr_valid(const ISP_TNR_GHOST_ATTR_S *pstTNRGhostAttr);
static CVI_S32 isp_tnr_ctrl_check_tnr_mt_prt_attr_valid(
    const ISP_TNR_MT_PRT_ATTR_S *pstTNRMtPrtAttr);
static CVI_S32 isp_tnr_ctrl_check_tnr_motion_adapt_attr_valid(
    const ISP_TNR_MOTION_ADAPT_ATTR_S *pstTNRMotionAdaptAttr);
static CVI_S32 isp_tnr_ctrl_set_tnr_mt_prt_attr_compatible(VI_PIPE                ViPipe,
                                                           ISP_TNR_MT_PRT_ATTR_S *pstTNRMtPrtAttr);
                                                         ...
                                                         const ISP_TNR_ATTR_S        *tnr_attr)
{
    ...
    for (CVI_U32 u32ISOIdx = 0; u32ISOIdx < ISP_AUTO_ISO_STRENGTH_NUM; ++u32ISOIdx) {
        if (pu8AutoMtUnitAttr[u32ISOIdx] > 0) {
            ...
            pu8AutoMtUnitInter[u32ISOIdx] =
                MAX(pu8AutoMtUnitAttr[u32ISOIdx], TNR_MOTION_BLOCK_MIN_UNIT);
            continue;
        }
        ...
        fSlopeGR_GB =
            ptNP->CalibrationCoef[u32ISOIdx][1][0] + ptNP->CalibrationCoef[u32ISOIdx][2][0];
        fInterceptGR_GB =
            ptNP->CalibrationCoef[u32ISOIdx][1][1] + ptNP->CalibrationCoef[u32ISOIdx][2][1];
    ...
    for (CVI_U32 u32ISOIdx = 0; u32ISOIdx < ISP_AUTO_ISO_STRENGTH_NUM; ++u32ISOIdx) {
        ISP_LOG_DEBUG("ISO:%3d ==> %2d vs %2d\n", u32ISOIdx,
                      tnr_attr->stAuto.MtDetectUnit[u32ISOIdx],
                      runtime->au8MtDetectUnit[u32ISOIdx]);
    }
    ISP_LOG_DEBUG("=======================================================\n");
#endif  //
...
static CVI_S32 isp_tnr_ctrl_preprocess(VI_PIPE ViPipe, ISP_ALGO_RESULT_S *algoResult)
{
    ...
    if (!tnr_attr->Enable || runtime->is_module_bypass) return ret;
...
#define AUTO(_attr, _param, type) \
    runtime->_attr._param = INTERPOLATE_LINEAR(ViPipe, type, _attr->stAuto._param)
        ...
        runtime->tnr_attr.MtDetectUnit =
            INTERPOLATE_LINEAR(ViPipe, INTPLT_POST_ISO, runtime->au8MtDetectUnit);
...
#undef AUTO
    ...
    for (CVI_U32 i = 0; i < 4; i++) {
        ...
        runtime->tnr_param_in.AdaptNrLumaStrIn[i] =
            runtime->tnr_motion_adapt_attr.AdaptNrLumaStrIn[i];
        runtime->tnr_param_in.AdaptNrLumaStrOut[i] =
            runtime->tnr_motion_adapt_attr.AdaptNrLumaStrOut[i];
        runtime->tnr_param_in.AdaptNrChromaStrIn[i] =
            runtime->tnr_motion_adapt_attr.AdaptNrChromaStrIn[i];
        runtime->tnr_param_in.AdaptNrChromaStrOut[i] =
            runtime->tnr_motion_adapt_attr.AdaptNrChromaStrOut[i];
    }
    ...
    for (CVI_U32 i = 0; i < ISP_BAYER_CHN_NUM; i++) {
        ...
        CVI_U32 u32RatioTNRbase = (CVI_U32)algoResult->au32WhiteBalanceGainPre[i]
                                  << TNR_BASE_1X_GAIN_BIT;
...
static CVI_S32 isp_tnr_ctrl_process(VI_PIPE ViPipe)
{
    ...
    ret = isp_algo_tnr_main((struct tnr_param_in *)&runtime->tnr_param_in,
...
static CVI_S32 isp_tnr_ctrl_postprocess(VI_PIPE ViPipe)
{
    ...
    if (is_postprocess_update == CVI_FALSE) {
        ...
        cfg_2->REG_70.bits.MMAP_0_GAIN_RATIO_R =
            runtime->GainCompensateRatio[ISP_CHANNEL_LE][ISP_BAYER_CHN_R];
        cfg_2->REG_70.bits.MMAP_0_GAIN_RATIO_G =
            runtime->GainCompensateRatio[ISP_CHANNEL_LE][ISP_BAYER_CHN_GR];
        cfg_2->REG_74.bits.MMAP_0_GAIN_RATIO_B =
            runtime->GainCompensateRatio[ISP_CHANNEL_LE][ISP_BAYER_CHN_B];
        ...
        cfg_3->REG_A0.bits.MMAP_1_GAIN_RATIO_R =
            runtime->GainCompensateRatio[ISP_CHANNEL_SE][ISP_BAYER_CHN_R];
        cfg_3->REG_A0.bits.MMAP_1_GAIN_RATIO_G =
            runtime->GainCompensateRatio[ISP_CHANNEL_SE][ISP_BAYER_CHN_GR];
        cfg_3->REG_A4.bits.MMAP_1_GAIN_RATIO_B =
            runtime->GainCompensateRatio[ISP_CHANNEL_SE][ISP_BAYER_CHN_B];
        ...
        cfg_4->REG_25.bits.REG_3DNR_MOTION_Y_LUT_IN_0 =
            runtime->tnr_motion_adapt_attr.AdaptNrLumaStrIn[0];
        cfg_4->REG_25.bits.REG_3DNR_MOTION_Y_LUT_IN_1 =
            runtime->tnr_motion_adapt_attr.AdaptNrLumaStrIn[1];
        cfg_4->REG_25.bits.REG_3DNR_MOTION_Y_LUT_IN_2 =
            runtime->tnr_motion_adapt_attr.AdaptNrLumaStrIn[2];
        cfg_4->REG_25.bits.REG_3DNR_MOTION_Y_LUT_IN_3 =
            runtime->tnr_motion_adapt_attr.AdaptNrLumaStrIn[3];
        cfg_4->REG_26.bits.REG_3DNR_MOTION_Y_LUT_OUT_0 =
            runtime->tnr_motion_adapt_attr.AdaptNrLumaStrOut[0];
        cfg_4->REG_26.bits.REG_3DNR_MOTION_Y_LUT_OUT_1 =
            runtime->tnr_motion_adapt_attr.AdaptNrLumaStrOut[1];
        cfg_4->REG_26.bits.REG_3DNR_MOTION_Y_LUT_OUT_2 =
            runtime->tnr_motion_adapt_attr.AdaptNrLumaStrOut[2];
        cfg_4->REG_26.bits.REG_3DNR_MOTION_Y_LUT_OUT_3 =
            runtime->tnr_motion_adapt_attr.AdaptNrLumaStrOut[3];
        cfg_4->REG_27.bits.REG_3DNR_MOTION_Y_LUT_SLOPE_0 =
            runtime->tnr_param_out.AdaptNrLumaSlope[0];
        cfg_4->REG_27.bits.REG_3DNR_MOTION_Y_LUT_SLOPE_1 =
            runtime->tnr_param_out.AdaptNrLumaSlope[1];
        cfg_4->REG_28.bits.REG_3DNR_MOTION_Y_LUT_SLOPE_2 =
            runtime->tnr_param_out.AdaptNrLumaSlope[2];
        cfg_4->REG_28.bits.REG_3DNR_MOTION_C_LUT_SLOPE_0 =
            runtime->tnr_param_out.AdaptNrChromaSlope[0];
        cfg_4->REG_29.bits.REG_3DNR_MOTION_C_LUT_SLOPE_1 =
            runtime->tnr_param_out.AdaptNrChromaSlope[1];
        cfg_4->REG_29.bits.REG_3DNR_MOTION_C_LUT_SLOPE_2 =
            runtime->tnr_param_out.AdaptNrChromaSlope[2];
        cfg_4->REG_30.bits.REG_3DNR_MOTION_C_LUT_IN_0 =
            runtime->tnr_motion_adapt_attr.AdaptNrChromaStrIn[0];
        cfg_4->REG_30.bits.REG_3DNR_MOTION_C_LUT_IN_1 =
            runtime->tnr_motion_adapt_attr.AdaptNrChromaStrIn[1];
        cfg_4->REG_30.bits.REG_3DNR_MOTION_C_LUT_IN_2 =
            runtime->tnr_motion_adapt_attr.AdaptNrChromaStrIn[2];
        cfg_4->REG_30.bits.REG_3DNR_MOTION_C_LUT_IN_3 =
            runtime->tnr_motion_adapt_attr.AdaptNrChromaStrIn[3];
        cfg_4->REG_31.bits.REG_3DNR_MOTION_C_LUT_OUT_0 =
            runtime->tnr_motion_adapt_attr.AdaptNrChromaStrOut[0];
        cfg_4->REG_31.bits.REG_3DNR_MOTION_C_LUT_OUT_1 =
            runtime->tnr_motion_adapt_attr.AdaptNrChromaStrOut[1];
        cfg_4->REG_31.bits.REG_3DNR_MOTION_C_LUT_OUT_2 =
            runtime->tnr_motion_adapt_attr.AdaptNrChromaStrOut[2];
        cfg_4->REG_31.bits.REG_3DNR_MOTION_C_LUT_OUT_3 =
            runtime->tnr_motion_adapt_attr.AdaptNrChromaStrOut[3];
        ...
        cfg_6->REG_C4.bits.REG_3DNR_LSC_COMP_GAIN_LUT_10 =
            runtime->tnr_param_out.TnrLscCompGain[10];
        cfg_6->REG_C4.bits.REG_3DNR_LSC_COMP_GAIN_LUT_11 =
            runtime->tnr_param_out.TnrLscCompGain[11];
        cfg_6->REG_C8.bits.REG_3DNR_LSC_COMP_GAIN_LUT_12 =
            runtime->tnr_param_out.TnrLscCompGain[12];
        cfg_6->REG_C8.bits.REG_3DNR_LSC_COMP_GAIN_LUT_13 =
            runtime->tnr_param_out.TnrLscCompGain[13];
        cfg_6->REG_CC.bits.REG_3DNR_LSC_COMP_GAIN_LUT_14 =
            runtime->tnr_param_out.TnrLscCompGain[14];
        cfg_6->REG_CC.bits.REG_3DNR_LSC_COMP_GAIN_LUT_15 =
            runtime->tnr_param_out.TnrLscCompGain[15];
        cfg_6->REG_D0.bits.REG_3DNR_LSC_COMP_GAIN_LUT_16 =
            runtime->tnr_param_out.TnrLscCompGain[16];
        ...
        cfg_6->REG_E0.bits.REG_3DNR_Y_MOTION_MAX       = cfg_6->REG_E0.bits.REG_3DNR_C_MOTION_MAX =
            tnr_mt_prt_attr->LowMtPrtAdvMax;
...
CVI_S32 isp_tnr_ctrl_set_tnr_internal_attr(VI_PIPE                     ViPipe,
                                           const ISP_TNR_INTER_ATTR_S *pstTNRInterAttr)
{
    if (pstTNRInterAttr == CVI_NULL) {
        return CVI_FAILURE;
    }
...
CVI_S32 isp_tnr_ctrl_get_tnr_noise_model_attr(
    VI_PIPE ViPipe, const ISP_TNR_NOISE_MODEL_ATTR_S **pstTNRNoiseModelAttr)
{
...
CVI_S32 isp_tnr_ctrl_set_tnr_noise_model_attr(
    VI_PIPE ViPipe, const ISP_TNR_NOISE_MODEL_ATTR_S *pstTNRNoiseModelAttr)
{
    if (pstTNRNoiseModelAttr == CVI_NULL) {
        return CVI_FAILURE;
...
CVI_S32 isp_tnr_ctrl_get_tnr_luma_motion_attr(
    VI_PIPE ViPipe, const ISP_TNR_LUMA_MOTION_ATTR_S **pstTNRLumaMotionAttr)
{
...
CVI_S32 isp_tnr_ctrl_set_tnr_luma_motion_attr(
    VI_PIPE ViPipe, const ISP_TNR_LUMA_MOTION_ATTR_S *pstTNRLumaMotionAttr)
{
    if (pstTNRLumaMotionAttr == CVI_NULL) {
        return CVI_FAILURE;
...
CVI_S32 isp_tnr_ctrl_get_tnr_ghost_attr(VI_PIPE                      ViPipe,
                                        const ISP_TNR_GHOST_ATTR_S **pstTNRGhostAttr)
{
...
CVI_S32 isp_tnr_ctrl_get_tnr_mt_prt_attr(VI_PIPE                       ViPipe,
                                         const ISP_TNR_MT_PRT_ATTR_S **pstTNRMtPrtAttr)
{
...
CVI_S32 isp_tnr_ctrl_set_tnr_mt_prt_attr(VI_PIPE                      ViPipe,
                                         const ISP_TNR_MT_PRT_ATTR_S *pstTNRMtPrtAttr)
{
    if (pstTNRMtPrtAttr == CVI_NULL) {
        return CVI_FAILURE;
...
CVI_S32 isp_tnr_ctrl_get_tnr_motion_adapt_attr(
    VI_PIPE ViPipe, const ISP_TNR_MOTION_ADAPT_ATTR_S **pstTNRMotionAdaptAttr)
{
...
CVI_S32 isp_tnr_ctrl_set_tnr_motion_adapt_attr(
    VI_PIPE ViPipe, const ISP_TNR_MOTION_ADAPT_ATTR_S *pstTNRMotionAdaptAttr)
{
    if (pstTNRMotionAdaptAttr == CVI_NULL) {
        return CVI_FAILURE;
