...
const struct isp_module_ctrl wb_mod = {.init    = isp_wb_ctrl_init,
...
static CVI_S32  isp_wb_ctrl_check_wb_colortone_attr_valid(
...
static CVI_S32 isp_wb_ctrl_postprocess(VI_PIPE ViPipe)
{
    ...
    if (is_postprocess_update == CVI_FALSE) {
        ...
        runtime->wbg_cfg.rgain_fraction =
            MIN((GAIN_EXTEND / MAX(runtime->wb_attr.u16Rgain, 1)), WBGINV_MAX);
        runtime->wbg_cfg.ggain_fraction =
            MIN((GAIN_EXTEND / MAX(runtime->wb_attr.u16Grgain, 1)), WBGINV_MAX);
        runtime->wbg_cfg.bgain_fraction =
            MIN((GAIN_EXTEND / MAX(runtime->wb_attr.u16Bgain, 1)), WBGINV_MAX);
...
//-----------------------------------------------------------------------------
CVI_S32 isp_wb_ctrl_get_wb_colortone_attr(VI_PIPE                       ViPipe,
                                          const ISP_COLOR_TONE_ATTR_S **pstColorToneAttr)
{
...
CVI_S32 isp_wb_ctrl_set_wb_colortone_attr(VI_PIPE                      ViPipe,
                                          const ISP_COLOR_TONE_ATTR_S *pstColorToneAttr)
{
    if (pstColorToneAttr == CVI_NULL) {
        return CVI_FAILURE;
