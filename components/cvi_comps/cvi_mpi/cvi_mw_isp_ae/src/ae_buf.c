...
#define AE_SNAP_LOG_BUFF_SIZE (3 * 1124)
    ...
    if (AE_LogBuf == CVI_NULL) {
        ...
        AE_PreviewLogBufSize =
            (AE_LogBufSize >= AE_SnapLogBufSize) ? AE_LogBufSize - AE_SnapLogBufSize : 0;
        AE_LogInit();
    }
}
...
#define AE_LOG_SPLIT_SIZE 0x100000  // 1M
    ...
    if (bEnable) {
        if (fp == CVI_NULL) {
            char            path[MAX_AE_LOG_PATH_LENGTH << 1];
            struct tm       tm;
            ...
            snprintf(path, sizeof(path), "%s/aelog-%d-%d-%d_%d-%d-%d.txt", aeDumpLogPath,
...
void AE_LogPrintf(const char *szFmt, ...)
{
    ...
    if (AE_LogTailIndex + bufLen >= AE_PreviewLogBufSize) {
        ...
        AE_LogTailIndex += bufLen;
        if (AE_CircleEnable) AE_LogHeadIndex = (AE_LogTailIndex + 1) % AE_PreviewLogBufSize;
    }
#endif
}
#else
...
#else
    buflen = strlen((char *)szBuffer);
    if (AE_SnapLogTailIndex + buflen >= AE_SnapLogBufSize) return;
...
CVI_S32 AE_DumpLog(void)
{
    ...
    if (AE_LogTailIndex != CVI_NULL) {
        ...
        if (strlen(aeDumpLogName)) {
            snprintf(fileName, MAX_AE_LOG_FILENAME_LENGTH, "%s/%s_AeLog.txt", aeDumpLogPath,
                     aeDumpLogName);
        } else {
            snprintf(fileName, MAX_AE_LOG_FILENAME_LENGTH, "%s/AeLog%d.txt", aeDumpLogPath,
                     fileCnt);
        }
        fp = fopen(fileName, "w");
        if (fp == CVI_NULL) {
        ...
        AE_SnapLogInit();
        for (CVI_U8 sID = 0; sID < u8SensorNum; ++sID) AE_SaveSnapLog(sID);
        AE_SaveParameterSetting();
        fwrite(AE_SnapLogBuf, 1, AE_SnapLogTailIndex, fp);
        fclose(fp);
        AE_LogInit();
        ...
        if (strlen(aeDumpLogName)) {
            snprintf(fileName, MAX_AE_LOG_FILENAME_LENGTH, "%s/%s_AeLog.bin", aeDumpLogPath,
                     aeDumpLogName);
        } else {
            snprintf(fileName, MAX_AE_LOG_FILENAME_LENGTH, "%s/AeLog%d.bin", aeDumpLogPath,
                     fileCnt);
        }
...
CVI_BOOL AE_DumpBootLog(CVI_U8 sID)
{
    ...
    snprintf(fileName, MAX_AE_LOG_PATH_LENGTH + sizeof("/AeBootLog.txt"), "%s/AeBootLog.txt",
             aeDumpLogPath);
    fp = fopen(fileName, "w");
    if (fp == CVI_NULL) {
        printf("file:%s open fail!\n", fileName);
    ...
    for (j = 0; j < AE_BOOT_MAX_FRAME; ++j) {
        for (i = 0; i < AeInfo[sID].u8AeMaxFrameNum; ++i) {
            ...
            fprintf(fp, "fid(%d,%d):%d L:%d ATL:%d Bv:%d Bs:%d LV:%d CM:%d Stb:%d\n", sID, i,
                    AeBootInfo[sID]->u8FrmID[j], AeBootInfo[sID]->u16FrmLuma[i][j],
            ...
            fprintf(fp, "\tTv:%d(%u %u) Sv:%d(%d %d %d) ISO:%u\n",
                    ...
                    AeBootInfo[sID]->stApex[i][j].s16SVEntry, bootGain.u32AGain, bootGain.u32DGain,
                    bootGain.u32ISPDGain, ISONum);
        }
        fprintf(fp, "\n");
    }
...
CVI_U8 AE_IrisParameterBufCreate(CVI_U8 sID)
{
    ...
    if (pstAeIrisAttr[sID] == CVI_NULL) {
        pstAeIrisAttr[sID] = (ISP_IRIS_ATTR_S *)AE_Malloc(sID, sizeof(ISP_IRIS_ATTR_S));
        if (pstAeIrisAttr[sID] == CVI_NULL) {
            return 11;
        }
    }
    ...
    if (pstAeIrisAttrInfo[sID] == CVI_NULL) {
        pstAeIrisAttrInfo[sID] = (ISP_IRIS_ATTR_S *)AE_Malloc(sID, sizeof(ISP_IRIS_ATTR_S));
        if (pstAeIrisAttrInfo[sID] == CVI_NULL) {
            return 12;
        }
    }
    ...
    if (pstAeDcIrisAttr[sID] == CVI_NULL) {
        pstAeDcIrisAttr[sID] = (ISP_DCIRIS_ATTR_S *)AE_Malloc(sID, sizeof(ISP_DCIRIS_ATTR_S));
        if (pstAeDcIrisAttr[sID] == CVI_NULL) {
            return 13;
        }
    }
    ...
    if (pstAeDcIrisAttrInfo[sID] == CVI_NULL) {
        pstAeDcIrisAttrInfo[sID] = (ISP_DCIRIS_ATTR_S *)AE_Malloc(sID, sizeof(ISP_DCIRIS_ATTR_S));
        if (pstAeDcIrisAttrInfo[sID] == CVI_NULL) {
            return 14;
        }
    }
...
void AE_ToolParameterBufCreate(CVI_U8 sID)
{
    ...
    if (pstAeMpiExpInfo[sID] == CVI_NULL) {
        pstAeMpiExpInfo[sID] = (ISP_EXP_INFO_S *)AE_Malloc(sID, sizeof(ISP_EXP_INFO_S));
        if (pstAeMpiExpInfo[sID] == CVI_NULL) {
            errorCode = 1;
            goto EXIT;
        }
    ...
    for (i = 0; i < AeInfo[sID].u8AeMaxFrameNum; ++i) {
        if (pstAeRouteInfo[sID][i] == CVI_NULL) {
            pstAeRouteInfo[sID][i] = (ISP_AE_ROUTE_S *)AE_Malloc(sID, sizeof(ISP_AE_ROUTE_S));
            if (pstAeRouteInfo[sID][i] == CVI_NULL) {
                errorCode = 6;
                goto EXIT;
            }
    ...
    for (i = 0; i < AeInfo[sID].u8AeMaxFrameNum; ++i) {
        if (pstAeMpiRoute[sID][i] == CVI_NULL) {
            pstAeMpiRoute[sID][i] = (ISP_AE_ROUTE_S *)AE_Malloc(sID, sizeof(ISP_AE_ROUTE_S));
            if (pstAeMpiRoute[sID][i] == CVI_NULL) {
                errorCode = 5;
                goto EXIT;
            }
...
EXIT:
    if (errorCode) ISP_LOG_ERR("%s Error:%d\n", "AeToolParam malloc fail!", errorCode);
}
