...
void AE_ShutterGainTest(CVI_U8 sID, AE_EXPOSURE *pStExp)
{
    ...
    if (AE_GetDebugMode(sID) >= SENSOR_PORTING_START_ITEM &&
        ...
        maxTime  = (tmpAeInfo->u32ExpTimeMax < 33333) ? tmpAeInfo->u32ExpTimeMax : 33333;
        ...
        if (AE_GetDebugMode(sID) == SENSOR_PORTING_START_ITEM) {
            ...
            if (tmpAeInfo->u32frmCnt % 20 < 10) {
                pStExp[AE_LE].u32SensorExpTimeLine = tmpAeInfo->u32ExpTimeMin / lineTime;
            } else {
                pStExp[AE_LE].u32SensorExpTimeLine = tmpAeInfo->u32ExpTimeMin * 2 / lineTime;
            }
            pStExp[AE_LE].stSensorExpGain.u32AGain    = AE_GAIN_BASE;
            pStExp[AE_LE].stSensorExpGain.u32DGain    = AE_GAIN_BASE;
            pStExp[AE_LE].stSensorExpGain.u32ISPDGain = AE_GAIN_BASE;
            ...
            pStExp[AE_LE].u32SensorExpTimeLine     = maxTime / lineTime;
            pStExp[AE_LE].stSensorExpGain.u32DGain = AE_GAIN_BASE;
        } else if (AE_GetDebugMode(sID) >= 75 && AE_GetDebugMode(sID) <= 82 &&
                   tmpAeInfo->bWDRMode) {
            if (AE_GetDebugMode(sID) == 75) {
                ...
                if (tmpAeInfo->u32frmCnt % 30 < 10)
                    expLine[0][AE_SE] = tmpAeInfo->u32WDRSEExpTimeMax / lineTime;
                else if (tmpAeInfo->u32frmCnt % 30 < 20)
                    expLine[0][AE_SE] = tmpAeInfo->u32WDRSEExpTimeMax / 2 / lineTime;
                else
                    expLine[0][AE_SE] = tmpAeInfo->u32WDRSEExpTimeMax / 4 / lineTime;
                ...
                expRatio =
                    tmpAeInfo->u32ExpLineMax * AE_WDR_RATIO_BASE / tmpAeInfo->u32WDRSEExpLineMax;
                ...
                for (tmpExpRatio = expRatio; tmpExpRatio <= 512; tmpExpRatio++) {
                    AE_GetWDRExpLineRange(sID, tmpExpRatio, &expLine[1][AE_LE], &expLine[1][AE_SE],
                                          1);
#if 0
                    ...
                    if (expLine[1][AE_LE] + expLine[0][AE_SE] >= tmpAeInfo->u32FrameLine) {
                        printf("R:%d Exp1 %d %d Exp2 %d %d FL:%d --- FindExp\n", tmpExpRatio,
                               expLine[0][AE_LE], expLine[0][AE_SE], expLine[1][AE_LE],
                               expLine[1][AE_SE], tmpAeInfo->u32FrameLine);
                        break;
                    }
                }
...
CVI_BOOL AE_ExposureAttr_ByPss_Test(VI_PIPE ViPipe)
{
    ...
    if (frameLuma[0] > frameLuma[1] * 11 / 10 || frameLuma[0] < frameLuma[1] * 9 / 10 ||
        time[0] != time[1])
        result = CVI_FAILURE;

    printf("BypassTest Luma0:%d Luma1:%d time0:%u time1:%u res:%d\n", frameLuma[0], frameLuma[1],
           time[0], time[1], result);
...
#define NAME_LENGTH 30
    ...
    CVI_U16             frameLuma[2]               = {0, 0};
    CVI_CHAR  itemName[AE_TEST_TOTAL][NAME_LENGTH] = {"ManualExp", "ManualISO", "ManualGain"};
    CVI_U8    sID;
    CVI_U16   diff;
    CVI_U8    diffRatio;
    CVI_U32   delayTime;
    ...
    if (frameLuma[1] < frameLuma[0] * 2 || diffRatio > 3) result = CVI_FAILURE;

    printf("%s--Luma0:%d Luma1:%d ISONum:%u %u ratio:%d Thr:%d res:%d\n", itemName[AE_TEST_ISO_NUM],
           frameLuma[0], frameLuma[1], tmpExpAttr.stManual.u32ISONum, tmpExpInfo.u32ISO, diffRatio,
    ...
    printf("%s--Luma0:%d Luma1:%d time:%u %u ratio:%d Thr:%d result:%d\n",
           itemName[AE_TEST_SHUTTER], frameLuma[0], frameLuma[1], tmpExpAttr.stManual.u32ExpTime,
    ...
    printf("%s--Luma0:%d Luma1:%d AGain:%u %u ISPDGain:%u %u Thr:%d result:%d\n",
           itemName[AE_TEST_GAIN], frameLuma[0], frameLuma[1], tmpExpAttr.stManual.u32AGain,
           tmpExpInfo.u32AGain, tmpExpAttr.stManual.u32ISPDGain, tmpExpInfo.u32ISPDGain,
           frameLuma[0] * 2, result);
...
CVI_BOOL AE_ExpsoureAttr_ExpRange_Test(VI_PIPE ViPipe)
{
    ...
    CVI_CHAR itemName[AE_TEST_TOTAL][NAME_LENGTH] = {"ExpRanage", "ISORange", "GainRange"};
    CVI_U8   sID;
    CVI_U16  frameLuma[2] = {0, 0};
    CVI_U16  diff;
    CVI_U8   diffRatio;
    ...
    if (frameLuma[1] > frameLuma[0] / 2 ||
        tmpExpInfo.u32ExpTime > tmpExpAttr.stAuto.stExpTimeRange.u32Max || diffRatio > 3)
        result = CVI_FAILURE;

    printf("%s--ExpTime:%u %u\n", itemName[AE_TEST_SHUTTER], tmpExpInfo.u32ExpTime,
           tmpExpAttr.stAuto.stExpTimeRange.u32Max);
    printf("%s--ISONum:%u %u\n", itemName[AE_TEST_ISO_NUM], tmpExpInfo.u32ISO,
           tmpExpAttr.stAuto.stISONumRange.u32Max);
    printf("Luma0:%d Luma1:%d ratio:%d result:%d\n", frameLuma[0], frameLuma[1], diffRatio, result);
    ...
    CVI_ISP_QueryExposureInfo(ViPipe, &tmpExpInfo);
    if (frameLuma[1] > frameLuma[0] / 4 || tmpExpInfo.u32ISO > 100) result = CVI_FAILURE;

    printf("%s1--svEntryMax:%d ISONum:%u AG:%u DG:%u IG:%u\n", itemName[AE_TEST_GAIN], svEntryMax,
           tmpExpInfo.u32ISO, tmpExpInfo.u32AGain, tmpExpInfo.u32DGain, tmpExpInfo.u32ISPDGain);
    printf("Luma0:%d Luma1:%d thr:%d res:%d\n", frameLuma[0], frameLuma[1], frameLuma[0] / 4,
           result);
    ...
    tmpExpAttr.stAuto.stSysGainRange.u32Max  = AE_CalTotalGain(4096, 1024, 2048);
    CVI_ISP_SetExposureAttr(ViPipe, &tmpExpAttr);
    AE_Delay1ms(delayTime);
    frameLuma[1] = AE_GetCenterG(sID, 0);

    CVI_ISP_QueryExposureInfo(ViPipe, &tmpExpInfo);
    if (tmpExpInfo.u32AGain > 4096 || tmpExpInfo.u32ISPDGain > 2048) result = CVI_FAILURE;

    printf("%s2--AGain:%u ISPDGain:%u result:%d\n", itemName[AE_TEST_GAIN], tmpExpInfo.u32AGain,
           tmpExpInfo.u32ISPDGain, result);
...
CVI_BOOL AE_ExpsoureAttr_AutoParam_Test(VI_PIPE ViPipe)
{
    ...
    frameLuma[1] = AE_GetCenterG(sID, 0);
    if (frameLuma[1] > frameLuma[0] / 2) result = CVI_FAILURE;

    printf("target %d->%d luma0:%d luma1:%d Thr:%d result:%d\n", oriTarget, newTarget, frameLuma[0],
    ...
    printf("Evbias %d->%d luma0:%d luma1:%d thr:%d result:%d\n", oriEvBias,
           tmpExpAttr.stAuto.u16EVBias, frameLuma[0], frameLuma[1], frameLuma[0] / 2, result);
    ...
    if (tmpExpInfo.u32ExpTime > 8333 && minTimeDiff > 2) result = CVI_FAILURE;
    ...
    if (tmpExpInfo.u32ExpTime > 10000 && minTimeDiff > 2) result = CVI_FAILURE;
...
#define WEIGHT_VALUE 255
    ...
    if (aeWinCfg.au8Weight[0][0] != WEIGHT_VALUE ||
        ...
        printf("NG weight:%d %d %d %d\n", aeWinCfg.au8Weight[0][0],
               aeWinCfg.au8Weight[0][column - 1], aeWinCfg.au8Weight[row - 1][0],
...
CVI_BOOL CVI_ISP_SetAERouteAttr_Test(VI_PIPE ViPipe)
{
    ...
    if (aeRoute.u32TotalNum != aeInfo.stAERoute.u32TotalNum) {
        result = CVI_FAILURE;
        printf("NG size:%u %u\n", aeRoute.u32TotalNum, aeInfo.stAERoute.u32TotalNum);
        printf("route:\n");
...
CVI_BOOL CVI_ISP_SetAERouteAttrEx_Test(VI_PIPE ViPipe)
{
    ...
    if (aeRouteEx.u32TotalNum != aeInfo.stAERouteEx.u32TotalNum) {
        result = CVI_FAILURE;
        printf("NG size:%u %u\n", aeRouteEx.u32TotalNum, aeInfo.stAERouteEx.u32TotalNum);
    } else {
...
CVI_BOOL CVI_ISP_SetWDRExposureAttr_Test(VI_PIPE ViPipe)
{
    ...
    if (aeInfo.u32WDRExpRatio < wdrAttr.au32ExpRatio[0] - AE_WDR_RATIO_BASE / 2 &&
        ...
        printf("WDR NG Ratio:%u %u(%u - %u)\n", wdrAttr.au32ExpRatio[0],
               aeInfo.u32WDRExpRatio - AE_WDR_RATIO_BASE, aeInfo.u32WDRExpRatio - AE_WDR_RATIO_BASE,
    ...
    if (aeInfo.u32WDRExpRatio < wdrAttr.u32ExpRatioMin &&
        ...
        result = CVI_FAILURE;
        printf("WDR NG Ratio:%u %u-%u\n", aeInfo.u32WDRExpRatio, wdrAttr.u32ExpRatioMin,
               wdrAttr.u32ExpRatioMax);
    }
    printf("WDR result:%d\n", result);
    return result;
}
...
{
    printf("Func : %s\n", __func__);
    if (AE_ExposureAttr_ByPss_Test(ViPipe)) return CVI_FAILURE;
    if (AE_ExposureAttr_ManualExp_Test(ViPipe)) return CVI_FAILURE;
    if (AE_ExpsoureAttr_ExpRange_Test(ViPipe)) return CVI_FAILURE;
    if (AE_ExpsoureAttr_AutoParam_Test(ViPipe)) return CVI_FAILURE;
    return CVI_SUCCESS;
}
...
void AE_SetManualGainTest(CVI_U8 sID, CVI_U32 again, CVI_U32 dgain, CVI_U32 ispDgain)
{
    ...
    printf("sensor LEexpL:%u SEexpL:%u AG:%u DG:%u IG:%u\n", sensorExp[AE_LE].u32SensorExpTimeLine,
           sensorExp[AE_SE].u32SensorExpTimeLine, sensorExp[AE_LE].stSensorExpGain.u32AGain,
           sensorExp[AE_LE].stSensorExpGain.u32DGain, sensorExp[AE_LE].stSensorExpGain.u32ISPDGain);
}
...
void AE_ShowCurrentInfo(CVI_U8 sID, const SAE_INFO *pstAeInfo)
{
    ...
    for (i = 0; i < pstAeInfo->u8AeMaxFrameNum; ++i) {
        ...
        printf("Luma:%d %d P:%d\n", pstAeInfo->u16FrameLuma[i], pstAeInfo->u8MeterGridCnt[i],
               pstAeInfo->u8MeterFramePeriod);
        ...
        printf("Bv:%d %d %d Bs:%d LvBs:%d IS:%d ME:%d\n", pstAeInfo->stApex[i].s16BVEntry,
               ...
               pstAeInfo->s16BvStepEntry[i], pstAeInfo->s16LvBvStep[i], pstAeInfo->bIsStable[i],
               pstAeInfo->bIsMaxExposure);
        ...
        printf("Init Tv:%d Sv:%d\n", pstAeInfo->stInitApex[i].s16TVEntry,
               pstAeInfo->stInitApex[i].s16SVEntry);
    }
...
void AE_ShowRoute(CVI_U8 sID, CVI_S32 param)
{
    ...
    AE_GetExposureAttr(sID, &expAttr);
    expAttr.bByPass = 1;
    if (param % 2 == 1) expAttr.bAERouteExValid = 1;
    AE_SetExposureAttr(sID, &expAttr);
    AE_Delay1ms(300);
    ...
    if (param >= 2) {
        for (i = maxBvEntry - 2 * ENTRY_PER_EV; i >= minBvEntry; --i) {
            ...
            printf("\nLE bv:%d tv:%d sv:%d\n", LeExp.s16BVEntry, LeExp.s16TVEntry,
                   LeExp.s16SVEntry);
            for (j = 2 * ENTRY_PER_EV; j <= 5 * ENTRY_PER_EV; ++j) {
                if (i + j > maxBvEntry) continue;
                tmpLeExp         = LeExp;
...
void AE_ShowExpLine(CVI_U8 sID, const SAE_INFO *pstAeInfo)
{
    ...
    printf("timeThr:%u\n", timeThr);
    for (i = pstAeInfo->s16MaxSensorTVEntry; i >= pstAeInfo->s16MinSensorTVEntry; --i) {
        ...
        if (time < timeThr) ispDgain = ispDgain * idealLine / line;
        printf("Entry:%d Time:%f iLine:%f Line:%u IG:%u SHS:%u\n", i, time, idealLine, line,
               ispDgain, SHS);
        if (i < entryThr - ENTRY_PER_EV) break;
    }
}
...
void AE_GetLimitGain(CVI_U8 sID, AE_GAIN *pstGain)
{
    ...
    if (tmpAeInfo->u8DGainAccuType == AE_ACCURACY_DB) {
        ...
        if (limitGain.u32AGain != pstGain->u32AGain) {  // only min DGain when limit AGain
            ...
            limitGain.u32ISPDGain =
                (CVI_U64)pstGain->u32AGain * pstGain->u32ISPDGain / limitGain.u32AGain;
            ...
            limitGain.u32ISPDGain =
                (CVI_U64)pstGain->u32DGain * pstGain->u32ISPDGain / limitGain.u32DGain;
        }
        AAA_LIMIT(limitGain.u32ISPDGain, ISPDGainMin, ISPDGainMax);
    }
    ...
    if (AE_GetDebugMode(sID) == 35) {
        printf("max:%u %u %u\n", tmpAeAttr.stAuto.stAGainRange.u32Max,
               tmpAeAttr.stAuto.stDGainRange.u32Max, tmpAeAttr.stAuto.stISPDGainRange.u32Max);
...
void AE_ShowISOEntry2Gain(CVI_U8 sID, CVI_BOOL mode)
{
    ...
    AE_GetExposureAttr(sID, &tmpAe);
    if (mode == 1) tmpAe.bAERouteExValid = 1;
    AE_SetExposureAttr(sID, &tmpAe);
    AE_Delay1ms(300);
    tmpAe.bByPass = 1;
    AE_Delay1ms(100);
    ...
    for (i = minISOEntry; i <= maxISOEntry; ++i) {
        ...
        if (aeGain.u32AGain != newGain.u32AGain) {
            newGain.u32ISPDGain = (newGain.u32ISPDGain * aeGain.u32AGain + newGain.u32AGain / 2) /
                                  AAA_DIV_0_TO_1(newGain.u32AGain);
        }
        if (aeGain.u32DGain != newGain.u32DGain) {
            newGain.u32ISPDGain = (newGain.u32ISPDGain * aeGain.u32DGain + newGain.u32DGain / 2) /
                                  AAA_DIV_0_TO_1(newGain.u32DGain);
        }
        ...
        diffRatio       = (aeTotalGain > sensorTotalGain)
                              ? (aeTotalGain - sensorTotalGain) * 100 / aeTotalGain
        ...
        if (diffRatio > 1)
            printf("i:%d ISO:%u AG:%u DG:%u IG:%u sAG:%u sDG:%u SIG:%u---D:%d\n", i, ISONum,
                   aeGain.u32AGain, aeGain.u32DGain, aeGain.u32ISPDGain, newGain.u32AGain,
                   newGain.u32DGain, newGain.u32ISPDGain, diffRatio);
        else
            printf("i:%d ISO:%u AG:%u DG:%u IG:%u sAG:%u sDG:%u SIG:%u\n", i, ISONum,
                   aeGain.u32AGain, aeGain.u32DGain, aeGain.u32ISPDGain, newGain.u32AGain,
                   newGain.u32DGain, newGain.u32ISPDGain);
    }
    tmpAe.bByPass         = 0;
    ...
    for (j = 0; j < AE_BOOT_MAX_FRAME; ++j) {
        for (i = 0; i < maxFrmNo; ++i) {
            ...
            printf("fid(%d, %d):%d ATL:%d Bv:%d Bs:%d Tv:%d Sv:%d CM:%d Stb:%d\n", sID, i,
                   tmpBootInfo.u8FrmID[j], tmpBootInfo.u16AdjustTargetLuma[i][j],
                   tmpBootInfo.stApex[i][j].s16BVEntry, tmpBootInfo.s16FrmBvStep[i][j],
                   tmpBootInfo.stApex[i][j].s16TVEntry, tmpBootInfo.stApex[i][j].s16SVEntry,
                   tmpBootInfo.bFrmConvergeMode[i][j], tmpBootInfo.bStable[i][j]);
        }
    }
    printf("\n");
}
...
void AE_ShowWeight(CVI_U8 sID)
{
    ...
    for (i = 0; i < AE_WEIGHT_ZONE_ROW; ++i) {
        for (j = 0; j < AE_WEIGHT_ZONE_COLUMN; ++j) printf("%d ", aeStsCfg.au8Weight[i][j]);
        printf("\n");
    }
    printf("\n");
}
...
void AE_ShowLVTarget(CVI_U8 sID)
{
    ...
    for (LvX100 = minLvX100; LvX100 <= maxLvX100; LvX100 += 25) {
        ...
        if (AE_IsWDRMode(sID))
            printf("Lv:%d target LE m:%d M:%d SE m:%d M:%d\n", LvX100, targetMin[AE_LE],
                   targetMax[AE_LE], targetMin[AE_SE], targetMax[AE_SE]);
        else
...
CVI_S32 AE_GetSensorTableGain(CVI_U8 sID, CVI_U32 *again, CVI_U32 *dgain)
{
    ...
    if (pstAeCtx->stSnsRegister.stAeExp.pfn_cmos_again_calc_table) {
        pstAeCtx->stSnsRegister.stAeExp.pfn_cmos_again_calc_table(ViPipe, again, &againDb);
    } else {
        printf("%s again calc table NULL.\n", __func__);
        return CVI_FAILURE;
    }
...
#define AE_FRAME_DELAY_TIME 300
    ...
    if (mode == 0 || mode == 6) {
        ...
        for (i = gainIdx; i <= gainIdx + ENTRY_PER_EV; i++) {
            ...
            aeExp[AE_LE].stExpGain.u32ISPDGain =
                (CVI_U64)aeExp[AE_LE].stExpGain.u32ISPDGain * aeGainTatal / sensorGainTatal;
            AE_ConfigExposureTime_Gain(sID, AE_LE, aeExp[AE_LE].fIdealExpTime,
                                       &aeExp[AE_LE].stExpGain);
        ...
        for (j = minTvEntry; j <= maxTvEntry; j += ENTRY_PER_EV) {
            for (i = j; i <= j + ENTRY_PER_EV; i++) {
                ...
                AE_GetSensorExposureSetting(sID, sensorExp);
                luma[AE_LE] =
                    (useHistogram) ? AE_GetBrightQuaG(sID, AE_LE) : AE_GetCenterG(sID, AE_LE);
                if (cnt < bufSize) {
                    AE_GetFrameID(sID, &frameID);
        ...
        for (i = timeIdx; i <= timeIdx + ENTRY_PER_EV; i++) {
            ...
            if (cnt < bufSize) {
                ...
                printf("\nfid:%u E_L:%u time:%f line_L:%f AeL:%u\n", frameID, i,
                       aeExp[AE_LE].fIdealExpTime, aeExp[AE_LE].fIdealExpTimeLine, lumaBufEv[cnt]);
        ...
        for (i = timeIdx; i <= timeIdx + ENTRY_PER_EV; i++) {
            ...
            AE_GetExposureTimeIdealLine(sID, aeExp[AE_SE].fIdealExpTime,
                                        &aeExp[AE_SE].fIdealExpTimeLine);
            ...
            AE_ConfigExposureTime_Gain(sID, AE_SE, aeExp[AE_SE].fIdealExpTime,
                                       &aeExp[AE_SE].stExpGain);
            AE_Delay1ms(AE_FRAME_DELAY_TIME);  // delay 300ms wait for AeRun update
            luma[AE_SE] = (useHistogram) ? AE_GetBrightQuaG(sID, AE_SE) : AE_GetCenterG(sID, AE_SE);
            if (cnt < bufSize) {
                ...
                printf("\nfid:%u E_S:%u time:%f line:%f AeL:%u\n", frameID, i,
                       aeExp[AE_SE].fIdealExpTime, aeExp[AE_SE].fIdealExpTimeLine, lumaBufEv[cnt]);
        ...
        for (i = timeIdx; i <= tmpAeInfo->s16MaxSensorTVEntry; i++) {
            ...
            printf("sensor expL:%u AG:%u DG:%u IG:%u\n", sensorExp[AE_LE].u32SensorExpTimeLine,
                   ...
                   sensorExp[AE_LE].stSensorExpGain.u32ISPDGain);
            cnt++;
            if (cnt % (2 * ENTRY_PER_EV) == 0) AE_Delay1ms(5000);
        }
    } else if (mode == 11) {
        AE_SetIspDgainCompensation(sID, 0);
        AAA_LIMIT(timeIdx, minTvEntry, maxTvEntry);
        ...
        for (i = gainIdx; i <= gainIdx + ENTRY_PER_EV; i++) {
            ...
            sensorGainTatal = aeGain.u32AGain * aeGain.u32DGain;
            aeExp[AE_SE].stExpGain.u32ISPDGain =
                (CVI_U64)aeExp[AE_SE].stExpGain.u32ISPDGain * aeGainTatal / sensorGainTatal;
#if 0
			aeExp[AE_SE].stExpGain.u32ISPDGain *=
				(aeExp[AE_SE].fIdealExpTimeLine) / (CVI_U32)aeExp[AE_SE].fIdealExpTimeLine;
#endif
            ...
            if (cnt < bufSize) {
                ...
                printf("\nfid:%u E_S:%u time:%f line:%f AeL:%u %u\n", frameID, i,
                       aeExp[AE_SE].fIdealExpTime, aeExp[AE_SE].fIdealExpTimeLine, luma[AE_LE],
                       luma[AE_SE]);
                printf("Sensor line_S:%u line_L:%u\n", sensorExp[AE_SE].u32SensorExpTimeLine,
                       sensorExp[AE_LE].u32SensorExpTimeLine);
                printf("AG:%u DG:%u IG:%u --- Diff(%d)\n",
    ...
    if (cnt) {
        if (mode == 2 && expGain[0].u32AGain) {
            ...
            for (i = 0; i < ENTRY_PER_EV + 1; ++i) {
                printf("%u %u %u\n", expGain[i].u32AGain, expGain[i].u32DGain,
                       expGain[i].u32ISPDGain);
            }
            printf("\n");
        }
        for (i = 0; i < ENTRY_PER_EV + 1; ++i) printf("%d\n", lumaBufEv[i]);
    } else {
        for (i = 0; i < bufSize; ++i) {
            if (lumaBuf[i]) {
                printf("%d\n", lumaBuf[i]);
                if ((i + 1) % ENTRY_PER_EV == 0) AE_Delay1ms(5);
            }
        }
    }
...
#ifndef ARCH_RTOS_CV181X  // TODO@CV181X
void GenNewRaw(void *pDstOri, void *pSrcOri, CVI_U32 sizeBk, CVI_U32 raw_mode, CVI_U32 raw_w,
               CVI_U32 raw_h, CVI_U32 nRawBlc)
{
    ...
    if (isDbglog) {
        printf("ae_wantSv %d %d %d %d\n", sAe_sim.ae_wantTv[0], sAe_sim.ae_wantTv[1],
               sAe_sim.ae_wantSv[0], sAe_sim.ae_wantSv[1]);
    }
    stride  = raw_w * 3 / 2;
    offsetW = raw_w * 1.5;
    ...
    for (i = 0; i < AE_TEST_RAW_NUM; i++) {
        ...
        aebBV[i] = tmpTv - tmpSv;
        if (isDbglog) {
            printf("Dll_%d\t%d\t%d\t%f\t%f\t%f\n", i, aeb_ev[i].tv, aeb_ev[i].sv, tmpTv, tmpSv,
                   aebBV[i]);
        }
    }
    //////////////////////////////////LE
    for (exp_sel = 0; exp_sel < genNum; exp_sel++) {
        ...
        if (isDbglog) {
            printf("\nTop_%d\t%d\t%d\t%f\t%f\t%f\n", topi, aeb_ev[topi].tv, aeb_ev[topi].sv, tmpTv,
                   tmpSv, aebBV[topi]);
            printf("Want:\t%d\t%d\t%f\t%f\t%f\n", wantTv[exp_sel], wantSv[exp_sel], tmpTv, tmpSv,
                   targetBV);
            printf("Bot_%d\t%d\t%d\t%f\t%f\t%f\n", boti, aeb_ev[boti].tv, aeb_ev[boti].sv, tmpTv,
                   tmpSv, aebBV[boti]);
