...
#define DRAK_THR        (340)  // 17* 20
    ...
    for (i = 0; i < FLICK_V_BIN; i++) {
        ...
        if (p32StabFrm[i] > stbvarMax) stbvarMax = p32StabFrm[i];
        if (p32StabFrm[i] < stbvarMin) stbvarMin = p32StabFrm[i];
    }
    if (darkcnt > 2) {
        isStable50 = 0;
    }
...
void AE_DoFlickDetect(CVI_U8 sID, CVI_S16 expinx)
{
    ...
    if (sAeDetFlicker[sID].fps == 30) {
        if (expinx != EVTT_ENTRY_1_120SEC && expinx != EVTT_ENTRY_1_60SEC &&
            expinx != EVTT_ENTRY_1_30SEC) {
            sAeDetFlicker[sID].inx = 0;
            return;
        }
    } else if (sAeDetFlicker[sID].fps == 25) {
        if (expinx != EVTT_ENTRY_1_100SEC && expinx != EVTT_ENTRY_1_50SEC &&
            expinx != EVTT_ENTRY_1_25SEC) {
            sAeDetFlicker[sID].inx = 0;
            return;
        }
    } else {
