...
CVI_S32 CVI_AE_SensorRegCallBack(VI_PIPE ViPipe, ALG_LIB_S *pstAeLib,
                                 ...
                                 AE_SENSOR_REGISTER_S *pstRegister)
{
    ...
    memcpy((CVI_VOID *)(&(pstAeCtx->stSnsAttrInfo)), (CVI_VOID *)pstSnsAttrInfo,
...
CVI_S32 CVI_ISP_SetExposureAttr(VI_PIPE ViPipe, const ISP_EXPOSURE_ATTR_S *pstExpAttr)
{
    ...
    if (pstExpAttr->stAuto.stExpTimeRange.u32Max < pstExpAttr->stAuto.stExpTimeRange.u32Min) {
        ...
        return CVI_FAILURE;
    } else if (pstExpAttr->stAuto.stISPDGainRange.u32Max <
               pstExpAttr->stAuto.stISPDGainRange.u32Min) {
        return CVI_FAILURE;
    } else if (pstExpAttr->stAuto.stSysGainRange.u32Max <
               pstExpAttr->stAuto.stSysGainRange.u32Min) {
        return CVI_FAILURE;
    }
...
CVI_S32 CVI_ISP_SetSmartExposureAttr(VI_PIPE                          ViPipe,
                                     const ISP_SMART_EXPOSURE_ATTR_S *pstSmartExpAttr)
{
    ISP_LOG_DEBUG("%s\n", "+");
...
CVI_S32 CVI_ISP_SetFastBootExposure(VI_PIPE ViPipe, CVI_U32 expLine, CVI_U32 again, CVI_U32 dgain,
                                    CVI_U32 ispdgain)
{
...
void CVI_AE_GenNewRaw(void *pDstOri, void *pSrcOri, CVI_U32 sizeBk, CVI_U32 mode, CVI_U32 w,
                      CVI_U32 h, CVI_U32 blc)
{
...
CVI_S32 CVI_ISP_GetFastConvCalibrationInfo(VI_PIPE ViPipe, CVI_S16 *pFirstFrameLuma,
                                           CVI_S16 *pStableBv)
{
    CVI_S32 res;
    CVI_U8  sID = AE_ViPipe2sID(ViPipe);
...
CVI_S32 CVI_ISP_SetResponseFrame(VI_PIPE ViPipe, CVI_U8 period)
{
    ...
    return AE_SetResponseFrame(sID, period);
}