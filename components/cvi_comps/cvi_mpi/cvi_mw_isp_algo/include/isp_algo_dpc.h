...
typedef struct {
    ...
    uint16_t  abs_thresh;  // if you want to align hisi's behavior, you should set abs_thresh =
                          // (startThresh << 2)
    uint8_t  dpType;  // 0: detect bright pixel, 1:detect dark pixel
    uint32_t table_max_size;
    uint32_t verbose;  // 0: no verbose, 1: print verbose, such as median, mean, stddev, abs_thresh,
                       // dev_thresh ...
