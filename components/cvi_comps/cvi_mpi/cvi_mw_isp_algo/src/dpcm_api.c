...
#define COMPRESSMODE \
    7  // 0 bypass, 1~3 reserved, 4 12-10-12 codec, 5 12-8-12 codec, 6 12-7-12 codec, 7 12-6-12
       // codec
...
static inline bool get_sign(CVI_S16 a)
{
    return a <= 0 ? 1 : 0;
}
static inline CVI_U16 fix_zero(CVI_U16 a)
{
    return a == 0 ? 1 : a;
}
static inline CVI_U16 deco_dpcm(CVI_U16 xp, bool s, CVI_U16 v)
{
...
static void repack_bit6_to_bit16(uint8_t *pu8In6bit, uint8_t *pu8Out16bit, uint32_t u32Width,
                                 uint32_t u32Height, uint32_t u32InStride, uint32_t u32OutStride)
{
    ...
    for (uint32_t i = 0; i < u32Height;
         ++i, pu8In6bitAddr += u32InStride, pu8Out16bitAddr += u32OutStride) {
...
static void deletePredictorData(uint16_t *inBuf, uint16_t *outBuf, uint32_t u32Width,
                                uint32_t u32Height, PREDICTOR_INFO predictorInfo,
                                uint32_t u32InStride, uint32_t u32OutStride)
{
...
static void dpcm_rx(const CVI_U16 *idata, CVI_U16 *odata, CVI_U32 width, CVI_U32 height,
                    CVI_U8 decMode)
{
...
CVI_S32 decoderRaw(RAW_INFO rawInfo, CVI_U8 *outPut)
{
    ...
    if ((rawInfo.buffer == NULL) || (rawInfo.width == 0) || (rawInfo.height == 0)) {
    ...
    if (width > TILE_MODE_WIDTH) {
        ...
        repack_bit6_to_bit16(rawInfo.buffer, pOriImg, (width + predictorInfo.size), height, stride,
                             (width + predictorInfo.size) * 2);
