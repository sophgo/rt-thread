...
static const unsigned int ISO_LUT[NUM_ISO] = {100,    200,    400,     800,    1600,   3200,
...
static int CCM_MANUAL_API(ISP_CCM_MANUAL_ATTR_S *sManualCCM, uint8_t Saturation,
                          uint16_t *uCCM_Result);
...
CVI_S32 isp_algo_ccm_main(struct ccm_param_in *ccm_param_in, struct ccm_param_out *ccm_param_out)
{
    ...
    for (ISP_CHANNEL_LIST_E idx = ISP_CHANNEL_LE; idx < ISP_CHANNEL_MAX_NUM; idx++) {
        ...
        if (ccm_Attr->enOpType == OP_TYPE_MANUAL) {
            CCM_MANUAL_API(&(ccm_Attr->stManual), saturation_attr->stManual.Saturation,
                           ISP_PTR_CAST_U16(ccm_param_out->CCM[idx]));
        } else {
            CCM_AUTO_API(&(ccm_Attr->stAuto), &(saturation_attr->stAuto),
 ...
 *  in general case, use CCM_GetSaturationMatrix() to create a saturation matrix that only dependent
 * on saturation. For each frame, the ccm might changed since it is dependent on color temperature.
 *  So you need use CCM_GetSaturationMatrix() to combine saturation matrix and CCM matrix if color
 * temperature changed.
 */
...
static void CCM_Normalize(int16_t *pCCM)
{
    ...
    for (int i = 0; i < rows; i++) {
        ...
        for (int j = 0; j < cols; j++) fsum += (float)(pCCM[i * cols + j]);
        ...
        for (int j = 0; j < (cols - 0); j++) {
            pCCM[i * cols + j] = (int16_t)(pCCM[i * cols + j] / fsum * (1 << DECIMAL_BITS));
                        ...
                        uint32_t ColorTemp, uint32_t IsoValue, uint16_t *uCCM_Result)
{
    ...
    ColorTemp = (ColorTemp < COLOR_TEMP_MIN)
                    ? COLOR_TEMP_MIN
