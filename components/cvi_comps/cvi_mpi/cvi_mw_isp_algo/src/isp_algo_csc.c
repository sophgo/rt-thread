...
static const float _BT601_RGB2Y[9]  = {0.299, 0.587, 0.114,  -0.169, -0.331,
                                       0.500, 0.500, -0.419, -0.081};
static const float _BT709_RGB2Y[9]  = {0.213, 0.715, 0.072,  -0.115, -0.385,
                                       0.500, 0.500, -0.454, -0.046};
static const float _BT2020_RGB2Y[9] = {0.262, 0.678, 0.059,  -0.139, -0.361,
                                       0.500, 0.500, -0.459, -0.041};
...
CVI_S32 isp_algo_csc_main(struct csc_param_in *csc_param_in, struct csc_param_out *csc_param_out)
{
    ...
    if (csc_param_in->eCscColorGamut == ISP_CSC_COLORGAMUT_USER) {
        memcpy(csc_param_out->s16cscCoef, csc_param_in->s16userCscCoef,
               sizeof(CVI_S16) * CSC_MATRIX_SIZE);
        ...
        for (CVI_U8 i = 0; i < 9; ++i) {
            csc_param_out->s16cscCoef[i] = ROUND(coef[i] * (1 << DECIMAL_BITS));
        }
