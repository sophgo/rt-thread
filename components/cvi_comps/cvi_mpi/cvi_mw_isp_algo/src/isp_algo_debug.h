...
#ifndef ARCH_RTOS_CV181X
#define ISP_ALGO_DEBUG(level, fmt, ...)                                                           \
    do {                                                                                          \
        if (level <= ISP_ALGO_DEBUG_LEVEL) {                                                      \
            ...
            if (ISP_ALGO_EXPORT_LOG_TO_STDOUT) {                                                  \
                printf("[%c] %s::%d : " fmt, ISP_ALGO_DEBUG_LEVEL_SYMBOL[level], __func__,        \
...
#define ISP_ALGO_DEBUG(level, fmt, ...)                                                          \
    do {                                                                                         \
        if (level <= ISP_ALGO_DEBUG_LEVEL) {                                                     \
            printf("[%c] %s::%d : " fmt, ISP_ALGO_DEBUG_LEVEL_SYMBOL[level], __func__, __LINE__, \
                   ##__VA_ARGS__);                                                               \
        }                                                                                        \
    } while (0)
#endif
