...
static CVI_VOID _get_hist_status(CVI_U16         histX[][DIS_MAX_WINDOW_X_NUM][XHIST_LENGTH],
                                 CVI_U16         histY[][DIS_MAX_WINDOW_X_NUM][YHIST_LENGTH],
                                 WINXY_DIS_HIST *curhist, REG_DIS greg_dis);
...
static CVI_U32  _pick_sort_find_med_mv(VI_PIPE       ViPipe,
                                       DIS_RGN_RAWMV rgn_curMV[][DIS_MAX_WINDOW_X_NUM],
                                       MV_STRUCT *MV_BUF, CVI_S32 curpos_wrt, REG_DIS greg_dis);
static void     _BubbleSort(CVI_S16 *arr, CVI_U8 buf_length);
static void     _hist_find_mv_LV(CVI_S32 prvMV, DIS_RGN_RAWMV *rgn_curmv, DIS_HIST *prvhist,
                                 DIS_HIST *curhist, REG_DIS *greg_dis, CVI_S32 max_jitter_range,
                                 CVI_S32 MV_STEP, CVI_U32 SAD_STEP, DIS_DIR_TYPE_E mvtype,
                                 CVI_BOOL is1st);
static void     _do_RgnMELV(DIS_RGN_RAWMV *baseMV, DIS_HIST *prvhist, DIS_HIST *curhist,
                            DIS_RGN_RAWMV *rgn_curmv, REG_DIS *greg_dis, CVI_S32 xstep, CVI_S32 ystep,
                            CVI_U32 SAD_STEP, CVI_S32 max_mvx_range, CVI_S32 max_mvy_range,
                            CVI_BOOL is1st);
...
#define MAX_LAST_Q  (5)
    ...
    if (stFlicker[ViPipe].ena == 0) return;
    ...
    for (i = 0; i < DIS_MAX_WINDOW_Y_NUM; i++) {
        ...
        for (k = 0; k < YHIST_LENGTH; k++) {
            ...
            if (inxT >= DIS_FLK_NUM_PER_Y) inxT = DIS_FLK_NUM_PER_Y - 1;
...
CVI_S32 disMain(VI_PIPE ViPipe, DIS_MAIN_INPUT_S *inPtr, DIS_MAIN_OUTPUT_S *outPtr)
{
    ...
    MEAUSURE_DIS_T(0);
    runtime->hist_ready_cnt =
        (inPtr->greg_dis.reg_sw_DIS_en == 0) ? 2 : MAX(runtime->hist_ready_cnt - 1, 0);
    runtime->dis_ready_cnt =
        (inPtr->greg_dis.reg_sw_DIS_en == 0) ? 2 : MAX(runtime->dis_ready_cnt - 1, 0);
    ...
    if (runtime->hist_ready_cnt != 0) {
        ...
        me_drop_hit =
            ((mvxdrop_bigdiff || mvxdrop_num_sml) || (mvydrop_bigdiff || mvydrop_num_sml));
        runtime->drop_end_cnt = me_drop_hit ? 5 : MAX(0, runtime->drop_end_cnt - 1);

        // step 4 : DIS Is Ready to Go:
        if (runtime->dis_ready_cnt == 0) {
            _check_freq(ViPipe, runtime->MV_BUF[runtime->curpos_wrt].x,
    ...
    fprintf(fpDump, "%01d\t%01d\t%02d\t%02d\t%01d\t%02d\t", runtime->hist_ready_cnt,
            runtime->dis_ready_cnt, runtime->curpos_exe, runtime->curpos_wrt, me_drop_hit,
            runtime->drop_end_cnt);
                                 ...
                                 WINXY_DIS_HIST *curhist, REG_DIS greg_dis)
{
    ...
    for (j = 0; j < DIS_MAX_WINDOW_Y_NUM; j++) {
        for (i = 0; i < DIS_MAX_WINDOW_X_NUM; i++) {
            for (widx = 0; widx < greg_dis.reg_XHIST_LENGTH; widx += vl) {
                vl      = vsetvl_e16m8(greg_dis.reg_XHIST_LENGTH - widx);
                vu16Val = vle16_v_u16m8(&histX[j][i][widx], vl);
                vu16Val = vsrl_vx_u16m8(vu16Val, (size_t)greg_dis.reg_HIST_NORMX, vl);
                vse16_v_u16m8(&curhist->wHist[j][i].xhist_d[widx], vu16Val, vl);
            }
            for (hidx = 0; hidx < greg_dis.reg_YHIST_LENGTH; hidx += vl) {
                vl      = vsetvl_e16m8(greg_dis.reg_YHIST_LENGTH - hidx);
                vu16Val = vle16_v_u16m8(&histY[j][i][hidx], vl);
                vu16Val = vsrl_vx_u16m8(vu16Val, (size_t)greg_dis.reg_HIST_NORMY, vl);
                vse16_v_u16m8(&curhist->wHist[j][i].yhist_d[hidx], vu16Val, vl);
            }
        }
    }

#endif  // end __riscv_vector
#else
                         ...
                         DIS_RGN_RAWMV rgn_curmv[][DIS_MAX_WINDOW_X_NUM], REG_DIS *greg_dis)
{
    ...
    for (CVI_S32 j = 0; j < DIS_MAX_WINDOW_Y_NUM; j++) {
        for (CVI_S32 i = 0; i < DIS_MAX_WINDOW_X_NUM; i++) {
            ...
            _do_RgnMELV(&rgn_init, &prvhist->wHist[j][i], &curhist->wHist[j][i], &rgn_curmv[j][i],
                        greg_dis, xstep_lv1, ystep_lv1, SAD_STEP_LV1,
                        greg_dis->reg_MAX_MVX_JITTER_RANGE, greg_dis->reg_MAX_MVY_JITTER_RANGE, 1);
            _do_RgnMELV(&rgn_curmv[j][i], &prvhist->wHist[j][i], &curhist->wHist[j][i],
                        &rgn_curmv[j][i], greg_dis, xstep_lv2, ystep_lv2, SAD_STEP_LV2,
                        (xstep_lv1 << 1), (ystep_lv1 << 1), 0);
        }
    }
}
                        ...
                        CVI_BOOL is1st)
{
    ...
    _hist_find_mv_LV(baseMV->x, rgn_curmv, prvhist, curhist, greg_dis, max_mvx_range, xstep,
                     SAD_STEP, DIS_DIR_X, is1st);
    _hist_find_mv_LV(baseMV->y, rgn_curmv, prvhist, curhist, greg_dis, max_mvy_range, ystep,
                     SAD_STEP, DIS_DIR_Y, is1st);
    if (is1st == 0) {
        CVI_S32 xcfrmSAD =
            (rgn_curmv->xavgSAD - rgn_curmv->xbestSAD) / MAX(1, greg_dis->reg_cfrmsad_norm);
        CVI_S32 ycfrmSAD =
            (rgn_curmv->yavgSAD - rgn_curmv->ybestSAD) / MAX(1, greg_dis->reg_cfrmsad_norm);

        tmpVal                = MAX(0, abs(rgn_curmv->y) - greg_dis->reg_cfrmgain_xcoring);
        CVI_S32 xcfrmMod_gain = MIN(greg_dis->reg_cfrmsad_max,
                                    tmpVal * (ycfrmSAD / MAX(1, greg_dis->reg_cfrmsad_norm)));

        tmpVal                = MAX(0, abs(rgn_curmv->x) - greg_dis->reg_cfrmgain_ycoring);
        CVI_S32 ycfrmMod_gain = MIN(greg_dis->reg_cfrmsad_max,
                                    tmpVal * (xcfrmSAD / MAX(1, greg_dis->reg_cfrmsad_norm)));

        // by SAD to check confidence of mv results:
        CVI_S32 xcfrmSAD_mod =
            MAX(0, (rgn_curmv->xbestSAD / MAX(1, greg_dis->reg_cfrmsad_norm)) -
                       ((xcfrmMod_gain * MAX(0, xcfrmSAD - greg_dis->reg_cfrmsad_xcoring)) / 16) -
                       (8 * xcfrmSAD >> 4));
        CVI_S32 ycfrmSAD_mod =
            MAX(0, (rgn_curmv->ybestSAD / MAX(1, greg_dis->reg_cfrmsad_norm)) -
...
{  // per 12.5 us
    ...
    size_t          block_steps = block_length / SAD_STEP;
    size_t          vl;
    vint16m1_t      v16m1Zero = vmv_v_x_i16m1((int16_t)0, vlmax);
    vint16m1_t      vi16m1Sum;
    const ptrdiff_t hist_offset = SAD_STEP * sizeof(int16_t);
#endif
...
#else
        CVI_U16   *preHistLastPtr = preHistPtr;
        vint16m8_t v16PreLastHistVal;

        tmpSAD += abs((preHistPtr[0] - curHistPtr[0] + SADdiff));
        curHistPtr += SAD_STEP;
        preHistPtr += SAD_STEP;

        for (size_t idx = 1; idx < block_steps; idx += vl) {
            vl                        = vsetvl_e16m8(block_steps - idx);
            vint16m8_t vi16CurHistVal = vlse16_v_i16m8((int16_t *)curHistPtr, hist_offset, vl);
            vint16m8_t vi16PreHistVal = vlse16_v_i16m8((int16_t *)preHistPtr, hist_offset, vl);
            vint16m8_t vi16Val        = vsub_vv_i16m8(vi16PreHistVal, vi16CurHistVal, vl);

            vi16Val = vadd_vx_i16m8(vi16Val, SADdiff, vl);
            vi16Val = vabs_v_i16m8(vi16Val, vl);  // self-defined abs

            vi16m1Sum = vredsum_vs_i16m8_i16m1(vi16m1Sum, vi16Val, v16m1Zero, vl);
            tmpSAD += vmv_x_s_i16m1_i16(vi16m1Sum);

            if (isDoCplx) {
                v16PreLastHistVal = vlse16_v_i16m8((int16_t *)preHistLastPtr, hist_offset, vl);

                vi16Val = vsub_vv_i16m8(v16PreLastHistVal, vi16PreHistVal, vl);
                vi16Val = vabs_v_i16m8(vi16Val, vl);

                vi16m1Sum = vredsum_vs_i16m8_i16m1(vi16m1Sum, vi16Val, v16m1Zero, vl);
                f2_cplx += vmv_x_s_i16m1_i16(vi16m1Sum);

                preHistLastPtr += vl * SAD_STEP;
            }

            curHistPtr += vl * SAD_STEP;
            preHistPtr += vl * SAD_STEP;
        }
...
static CVI_S16 _decide_final_mv(CVI_S16 *mv_candidate, CVI_U8 buf_length, CVI_U8 *me_drop_status,
                                REG_DIS greg_dis)
{
