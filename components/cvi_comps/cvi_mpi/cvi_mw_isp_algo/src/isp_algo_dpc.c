...
{
    for (uint32_t i = 0; i < data_size; i++) printf("%u,\t", data[i]);
    printf("\n");
}
...
static inline void print_pixel(uint16_t *image, uint16_t width, uint16_t height, uint16_t stride,
...
static inline void merge(uint16_t *data, uint16_t *data1, uint16_t size1, uint16_t *data2,
                         uint16_t size2)
{
...
static inline bool dpc_detect(uint16_t *p, uint16_t stride, uint16_t abs_thresh, uint8_t position,
                              uint8_t dpType)
{
    // return true;
...
static inline void dpc_table_add(uint32_t *table, uint32_t *count, uint32_t table_max_size,
                                 uint16_t r, uint16_t c)
{
...
int32_t Bayer_12bit_2_16bit(uint8_t *bayerBuffer, uint16_t *outBuffer, uint16_t width,
        ...
        // 2 5 8
            if (r <= upper) {
                ...
                if (c <= left) {
                    ...
                    if (result) dpc_table_add(table, &count, table_max_size, r, c - 2);
#if VERIFY_TRAVERSAL
                ...
                if (result) dpc_table_add(table, &count, table_max_size, r, c);
#if VERIFY_TRAVERSAL
...
static void test5(void)
{
    ...
    char     filename[] =
        "imx334/coutMin0_startThresh4/"
        "HisiRAW_3840x2160_12bits_RGGB_Linear_20210317184551.raw_hiiso";
    char           type[] = "RGGB";
