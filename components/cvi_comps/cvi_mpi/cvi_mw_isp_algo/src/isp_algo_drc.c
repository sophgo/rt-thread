...
static void hdr_sample_flow_drche(CVI_BOOL fswdr_en, float exposure_ratio,
                                  struct drc_param_in  *drc_in_param,
                                  struct drc_param_out *drc_out_param);

static void set_dr_information(int ExposureRatio, int *reg_ltm_b_curve_quan_bit,
                               int *reg_ltm_g_curve_1_quan_bit);

static int DRC_7_6(uint32_t *global_tone, uint32_t *dark_tone, uint32_t *brit_tone,
                   uint32_t *le_histogram, uint32_t *global_tone_se_step, TCM_Param *cm_param,
                   TGT_Param *gt_param, TDTA_Param *dta_param, TBT_Param *bt_param);

static void linear_drc_dta_proc(uint32_t *global_tone, uint32_t *dark_tone, uint32_t *brit_tone,
                                uint32_t *le_histogram, TCM_Param *cm_param, TGT_Param *gt_param,
                                TDTA_Param *dta_param, TBT_Param *bt_param);

/* -------------------- debug 182x simulation mode start -------------------- */
static int generate_flt_distance_coeff(int *dist_wgt);
static int generate_flt_distance_normalization_coeff(int rng, int iLmapThr, int *dist_wgt,
                                                     int *shift, int *gain);
/* --------------------- debug 182x simulation mode end --------------------- */

// static int generate_flt_distance_normalization_coeff(int rng, int iLmapThr, int *dist_wgt, int
// *shift, int *gain);
static int DRC_Blending_API(uint32_t *pMapHist, uint32_t *pMapHist_prev, uint32_t *pMapHist_output,
                            int prebinNum, int bins_num, int weight_prev);
...
CVI_S32 isp_algo_drc_main(struct drc_param_in *drc_param_in, struct drc_param_out *drc_param_out)
{
    ...
    hdr_sample_flow_drche(drc_param_in->fswdr_en, drc_param_in->exposure_ratio, drc_param_in,
                          drc_param_out);
    ...
    for (CVI_U32 i = 0; i < 3; i++) {
        ...
        drc_param_out->DetailEnhanceMtSlope[i] =
            get_lut_slp(in0, in1, out0, out1, DETAIL_ENHANCE_SLOPE_BIT);
    }
    return ret;
}
...
static void drc_tone_curve_user_define_global_parser(float                 exposure_ratio,
                                                     struct drc_param_in  *drc_in_param,
                                                     struct drc_param_out *drc_out_param)
{
    CVI_U32 i, idx;
    ...
    float   slope =
        (float)(DRC_USER_DEFINE_CURVE_MAX_INDEX - MAX_HIST_BINS) / (wdr_bin_num - MAX_HIST_BINS);
    ...
    for (i = 0; i < max_node; i++) {
        ...
        stepOffset += stepValue;
        drc_out_param->reg_ltm_deflt_lut[LTM_G_CURVE_0_NODE_NUM + i] =
            drc_in_param->au16CurveUserDefine[idx];
    }
...
static void drc_tone_curve_user_define_dark_parser(struct drc_param_in  *drc_in_param,
                                                   struct drc_param_out *drc_out_param)
{
    CVI_U32 i;
...
static void drc_tone_curve_user_define_bright_parser(float                 exposure_ratio,
                                                     struct drc_param_in  *drc_in_param,
                                                     struct drc_param_out *drc_out_param)
{
...
static void linear_drc_tone_curve_user_define_global_parser(struct drc_param_in  *drc_in_param,
                                                            struct drc_param_out *drc_out_param)
{
    CVI_U32 i, iStep, idx;
...
static void linear_drc_tone_curve_user_define_dark_parser(struct drc_param_in  *drc_in_param,
                                                          struct drc_param_out *drc_out_param)
{
    CVI_U32 i, iStep, idx;
...
static void linear_drc_tone_curve_user_define_bright_parser(struct drc_param_in  *drc_in_param,
                                                            struct drc_param_out *drc_out_param)
{
    CVI_U32 i, iStep, idx;
...
static void set_lmap_lut(CVI_U8 darkMapStr, CVI_U8 britMapStr, struct drc_param_out *drc_out_param)
{
    ...
    for (CVI_U8 i = 0; i < 3; ++i) {
        leLmapSlope[i] = (int)((CVI_FLOAT)(leLmapOut[i + 1] - leLmapOut[i]) /
                               (leLmapIn[i + 1] - leLmapIn[i]) * 16);
        seLmapSlope[i] = (int)((CVI_FLOAT)(seLmapOut[i + 1] - seLmapOut[i]) /
                               (seLmapIn[i + 1] - seLmapIn[i]) * 16);
    }
...
{
    // set DR information
    set_dr_information(drc_in_param->ToneParserMaxRatio, &(drc_out_param->reg_ltm_b_curve_quan_bit),
                       &(drc_out_param->reg_ltm_g_curve_1_quan_bit));
    ...
    if (drc_in_param->ToneCurveSelect == 0) {
        ...
        if (fswdr_en == 1) {
            ...
            int wdr_bin_num =
                (float)(min_weight / 256.0f) * MAX_HIST_BINS +
            ...
            DRC_7_6(global_tone, dark_tone, brit_tone, le_histogram, &global_tone_se_step,
            ...
            DRC_Blending_API(global_tone, ISP_PTR_CAST_U32(drc_in_param->pu32PreGlobalLut),
                             global_tone, drc_in_param->preWdrBinNum, LTM_G_CURVE_TOTAL_NODE_NUM,
                             drc_in_param->average_frame);
            DRC_Blending_API(brit_tone, ISP_PTR_CAST_U32(drc_in_param->pu32PreBritLut), brit_tone,
                             drc_in_param->preWdrBinNum, LTM_BRIGHT_CURVE_NODE_NUM,
                             drc_in_param->average_frame);
            DRC_Blending_API(dark_tone, ISP_PTR_CAST_U32(drc_in_param->pu32PreDarkLut), dark_tone,
                             drc_in_param->preWdrBinNum, LTM_DARK_CURVE_NODE_NUM,
                             drc_in_param->average_frame);

            /* ---------------------- HDR mode luma map adjustment ---------------------- */
            set_lmap_lut(drc_in_param->DarkMapStr, drc_in_param->BritMapStr, drc_out_param);

            /* -------------------- debug 182x simulation mode start -------------------- */
            if (drc_in_param->dbg_182x_sim_enable) {
                int temp_ltm_de_lmap_thr = (int)((float)MAX(cm_param.avg_low, 8) *
                                                 drc_in_param->stDarkToneParam.fDarkMapStr);
            ...
            /* -------------------------- linearDRC main proc -------------------------- */
            linear_drc_dta_proc(global_tone, dark_tone, brit_tone, le_histogram, &cm_param,
                                &gt_param, &dta_param, &bt_param);
            ...
            set_lmap_lut(drc_in_param->SdrDarkMapStr, drc_in_param->SdrBritMapStr, drc_out_param);
        ...
        if (drc_in_param->dbg_182x_sim_enable) {
            generate_flt_distance_normalization_coeff(
                5, ltm_de_lmap_thr, drc_out_param->reg_ltm_de_dist_wgt,
                &(drc_out_param->reg_ltm_de_strth_dshft), &(drc_out_param->reg_ltm_de_strth_gain));

            generate_flt_distance_normalization_coeff(
                5, ltm_be_lmap_thr, drc_out_param->reg_ltm_be_dist_wgt,
                &(drc_out_param->reg_ltm_be_strth_dshft), &(drc_out_param->reg_ltm_be_strth_gain));
...
static void get_dark_tone_adapt_info(TDTA_Param *dta_param, const uint32_t *le_histogram,
                                     TCM_Param *cm_param)
{
                                                     ...
                                                     TCM_Param *cm_param, TDTA_Param *dta_param)
{
    ...
    // default
    T = (float)(Tgain * cm_param->P1) / (Tgain * cm_param->P1 + cm_param->P2);
    ...
    ISP_ALGO_DEBUG(
        LOG_DEBUG,
        "[dark_offset] = %d, [avg_low] = %d, [avg] = %d, [new_avg] = %d, [T_absolute] = %d\n",
    ...
    if ((avg_low_dark_offset <= 0) || (new_avg_dark_offset <= 0) ||
        (absolute_gtone_dark_offset <= 0) || (gtone_newavg_gtone_dark_offset <= 0)) {
        return CVI_FALSE;
    }
                                ...
                                uint32_t *global_tone, uint32_t *se_step)
{
    ...
    for (int i = 0; i < LTM_G_CURVE_0_NODE_NUM; i++) {
        ...
        if (i <= Linear_start)
            ...
            u32linear_tone =
                DRC_OUT_MIN_VAL + drc_out_val_range * (i - Linear_start) / linear_val_range;
        else
    ...
    for (int i = 0; i < LTM_G_CURVE_1_NODE_NUM; i++, iRefIdx += step) {
        if (iRefIdx < nbins) {
            ...
            mapped_n = gt_param->global_tone_strength_UI * u32wdr_tone;
            mapped_n =
                (mapped_n + (256 - gt_param->global_tone_strength_UI) * u32linear_tone) / 256;
            // mapped_n = (mapped_n * gt_param->contrast_gain_UI + 64) >> 7;
            u32wdr_tone = MINMAX(mapped_n, DRC_OUT_MIN_VAL, DRC_OUT_MAX_VAL);
...
static void gen_drc_dark_tone_adaptive(const uint32_t *global_tone, const TDTA_Param *dta_param,
                                       uint32_t *dark_tone)
{
...
static void gen_drc_bright_tone(const uint32_t *global_tone, const TCM_Param *cm_param,
                                TBT_Param *bt_param, uint32_t *brit_tone)
{
    ...
    if (PCHIP_InterP1_PCHIP_V2_Preprocess(&stPCHIP, &stPChip_Private) != CVI_SUCCESS) {
        ...
        ISP_ALGO_LOG_WARNING("nbins : %4d, pchip (%4d, %4d), (%4d, %4d), (%4d, %4d), (%4d, %4d)\n",
                             iBTnbins, cp1_node, cp1_val, le_node, le_val, cp2_node, cp2_val,
        ...
        if (PCHIP_InterP1_PCHIP_V2_Preprocess(&stPCHIP, &stPChip_Private) != CVI_SUCCESS) {
            ...
            ISP_ALGO_LOG_WARNING(
                "nbins : %4d, pchip (%4d, %4d), (%4d, %4d), (%4d, %4d), (%4d, %4d)\n", iBTnbins,
...
static void gen_linear_drc_global_tone(const TGT_Param *gt_param, const TCM_Param *cm_param,
                                       uint32_t *global_tone)
{
...
static void gen_linear_drc_bright_tone(const uint32_t *global_tone, const TCM_Param *cm_param,
                                       const TBT_Param *bt_param, const TDTA_Param *dta_param,
                                       uint32_t *brit_tone)
{
#define PCHIP_SDR_BRIGHT_TONE_OUT_MEM_SIZE (1028)  // ((AE_HIST_BIN_NUM + 1) * S32) = (256 + 1) * 4
    ...
    if (PCHIP_InterP1_PCHIP(&stPCHIP) != CVI_SUCCESS) {
        ISP_ALGO_LOG_WARNING("DRC PChip fail (P1)!!\n");
        ISP_ALGO_LOG_WARNING("nbins : %4d, pchip (%4d, %4d), (%4d, %4d), (%4d, %4d), (%4d, %4d)\n",
                             nbins, cp1_node, cp1_val, dark_end, le_val, cp2_node, cp2_val,
        ...
        if (PCHIP_InterP1_PCHIP(&stPCHIP) != CVI_SUCCESS) {
            ...
            ISP_ALGO_LOG_WARNING(
                "nbins : %4d, pchip (%4d, %4d), (%4d, %4d), (%4d, %4d), (%4d, %4d)\n", nbins,
                cp1_node, cp1_val, dark_end, le_val, cp2_node, cp2_val, AE_HIST_BIN_NUM,
                DRC_OUT_MAX_VAL);
        }
    }
                                ...
                                TDTA_Param *dta_param, TBT_Param *bt_param)
{
    ...
    // protect out of LE range case (input > DRC_TONE_CURVE_MAX_VALUE => output =
    // DRC_TONE_CURVE_MAX_VALUE)
    linear_drc_tone_protect(global_tone, MAX_HIST_BINS, LTM_G_CURVE_TOTAL_NODE_NUM);
                            ...
                            int prebinNum, int bins_num, int weight_prev)
{
    ...
    if (bins_num == 0) {
        ...
        for (int n = 0; n < bins_num; n++) {
            ...
            pMapHist_prev[n] = pMapHist_output[n] =
                (pMapHist[n] * wt_cur + pMapHist_prev[n] * pre_weight) / (pre_weight + wt_cur) +
                add_on;
