...
static void hdr_update_combine_min_weight(float exposure_ratio, struct fswdr_param_in *wdr_in_param,
                                          CVI_U32   *outWDRCombineMinWeight,
                                          CVI_FLOAT *outWDRMinWeightSNRIIR);

CVI_S32 isp_algo_fswdr_main(struct fswdr_param_in  *fswdr_param_in,
                            struct fswdr_param_out *fswdr_param_out)
{
    ...
    ExposureRatio =
        (ExposureRatio > WDR_EXPOSURE_RATIO_MAX) ? WDR_EXPOSURE_RATIO_MAX : ExposureRatio;
                                  ...
                                  &fswdr_param_out->WDRMinWeightSNRIIR);
    {
        ...
        fswdr_param_out->WDRCombineSlope =
            get_lut_slp(in0, in1, out0, out1, WDR_FUSION_LUT_SLP_F_BITS);
    }

    if (fswdr_param_in->WDRFsCalPxlNum > 0) {
        fswdr_param_out->WDRSeFixOffsetR =
            fswdr_param_in->WDRPxlDiffSumR / fswdr_param_in->WDRFsCalPxlNum;
        fswdr_param_out->WDRSeFixOffsetG =
            fswdr_param_in->WDRPxlDiffSumG / fswdr_param_in->WDRFsCalPxlNum;
        fswdr_param_out->WDRSeFixOffsetB =
            fswdr_param_in->WDRPxlDiffSumB / fswdr_param_in->WDRFsCalPxlNum;
    } else {
...

    {
        ...
        fswdr_param_out->DarkToneRefinedSlope =
            get_lut_slp(in0, in1, out0, out1, WDR_FUSION_LUT_SLP_F_BITS);
    }

    {
        ...
        fswdr_param_out->BrightToneRefinedSlope =
            get_lut_slp(in0, in1, out0, out1, WDR_FUSION_LUT_SLP_F_BITS);
    }
...

    {
        ...
        fswdr_param_out->WDRMotionCombineSlope =
            get_lut_slp(in0, in1, out0, out1, WDR_FUSION_LUT_SLP_F_BITS);
    }
...
static const CVI_U32 ISO_TABLE[16] = {100,    200,    400,     800,    1600,   3200,
...
#define SNR_MIN_WEIGHT_UPDATE_LOG_FREQ 66
    ...
    if (SNRAwareLog) {
        printf("SNRAware : en %d, sl %d, lthr %f, hthr %f, level %d\n", SNRAwareEn,
               SNRAwareSmoothLevel, SNRAwareLowThr, SNRAwareHighThr, SNRAwareToleranceLevel);
        printf("ISO: %d, DGain: %d, RGain: %d, BGain: %d, IIR: %f\n", wdr_in_param->iso,
               wdr_in_param->digital_gain, wdr_in_param->wb_r_gain, wdr_in_param->wb_b_gain,
    ...
    CVI_FLOAT(*cali_noise_model)
    [NOISE_PROFILE_CHANNEL_NUM][NOISE_PROFILE_LEVEL_NUM] = np->CalibrationCoef;
    ...
    if (total_weight == 0) {
        ...
        slp_r = ((low_weight * cali_noise_model[low_iso][NS_MODEL_R][SLP_IDX]) +
                 (high_weight * cali_noise_model[high_iso][NS_MODEL_R][SLP_IDX])) /
                total_weight;
        off_r = ((low_weight * cali_noise_model[low_iso][NS_MODEL_R][OFF_IDX]) +
                 (high_weight * cali_noise_model[high_iso][NS_MODEL_R][OFF_IDX])) /
                total_weight;
        ...
        slp_b = ((low_weight * cali_noise_model[low_iso][NS_MODEL_B][SLP_IDX]) +
                 (high_weight * cali_noise_model[high_iso][NS_MODEL_B][SLP_IDX])) /
                total_weight;
        off_b = ((low_weight * cali_noise_model[low_iso][NS_MODEL_B][OFF_IDX]) +
                 (high_weight * cali_noise_model[high_iso][NS_MODEL_B][OFF_IDX])) /
                total_weight;
    }
    ...
    if (SNRAwareLog) {
        ...
        printf("ns_r/g/b/v, exposureR, NoiseVal*exposureR: %f, %f, %f, %f, %f, %f\n", ns_r, ns_g,
               ns_b, ns_v, exposure_ratio, ns_val);
    }
    ...
    if (ns_val < SNRAwareLowThr)
        ...
        float x_diff =
            ((SNRAwareHighThr - SNRAwareLowThr) == 0) ? 1 : SNRAwareHighThr - SNRAwareLowThr;
        float y_diff = (wdr_in_param->WDRCombineMinWeight > SNRAwareToleranceLevel)
                           ? 0
                           : (SNRAwareToleranceLevel - wdr_in_param->WDRCombineMinWeight);
    ...
    if (SNRAwareLog) {
        printf(
            "SNR LowThrd, HighThrd, ToleranceLevel, MinWeight, MinWeightSNR : %f, %f, %d, %d, %d\n",
    ...
    if (SNRAwareSmoothLevel == 0) SNRAwareSmoothLevel = 1;

    float iir_alpha = 1.0 / (float)SNRAwareSmoothLevel;

    fswdr_min_weight_snr_cur =
        iir_alpha * fswdr_min_weight_snr + (1.0 - iir_alpha) * wdr_in_param->WDRMinWeightSNRIIR;
    ...
    if (SNRAwareLog) {
        printf("SNREn, iir, PrvWeight, CurWeight: %d, %d, %f, %d\n\n\n", SNRAwareEn,
               SNRAwareSmoothLevel, wdr_in_param->WDRMinWeightSNRIIR, fswdr_min_weight_snr);
    }
