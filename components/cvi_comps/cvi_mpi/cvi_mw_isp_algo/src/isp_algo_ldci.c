...
static void    create_1D_Gaussian_kernel(int Mean, int Sigma, int Wgt, int filter_size, int *kernel,
                                         float *tempbuf);
static int     ldci_tone_curve_gen_sw(int *tone_curve_lut, int curve_lut_line_count,
                                      int curve_lut_line_size, float *x1, float *x2,
                                      int iDarkContrastLow, int iDarkContrastHigh,
                                      int iBrightContrastLow, int iBrightCOntrastHigh,
                                      int iDarkContrastLow2, int iDarkContrastHigh2,
                                      int iBrightContrastLow2, int iBrightCOntrastHigh2);
static CVI_S32 isp_algo_ldci_update_idx_map_filter(struct ldci_param_in  *ldci_param_in,
                                                   struct ldci_param_out *ldci_param_out);
static CVI_S32 isp_algo_ldci_update_var_map_filter(struct ldci_param_in  *ldci_param_in,
                                                   struct ldci_param_out *ldci_param_out);
static CVI_S32 isp_algo_ldci_update_luma_based_weight_lut(struct ldci_param_in  *ldci_param_in,
                                                          struct ldci_param_out *ldci_param_out);
static CVI_S32 isp_algo_ldci_update_tone_curve(struct ldci_param_in  *ldci_param_in,
                                               struct ldci_param_out *ldci_param_out);
...
CVI_S32 isp_algo_ldci_get_init_coef(int W, int H, struct ldci_init_coef *pldci_init_coef)
{
    ...
    BlockSizeX    = (W % LDCI_BLOCKS_X == 0) ? (W / LDCI_BLOCKS_X)
                                             : (W / LDCI_BLOCKS_X) + 1;  // Width of one block
    BlockSizeY    = (H % LDCI_BLOCKS_Y == 0) ? (H / LDCI_BLOCKS_Y)
                                             : (H / LDCI_BLOCKS_Y) + 1;  // Height of one block
...
CVI_S32 isp_algo_ldci_main(struct ldci_param_in  *ldci_param_in,
                           struct ldci_param_out *ldci_param_out)
{
    // ISP_DEBUG(LOG_INFO, "%s\n", "+");
    CVI_S32 ret = CVI_SUCCESS;
                                                          ...
                                                          struct ldci_param_out *ldci_param_out)
{
    ...
    create_1D_Gaussian_kernel(Mean, Sigma, Wgt, LDCI_LUMA_WEIGHT_LUT_SIZE, reg_ldci_luma_wgt_lut,
                              tempbuf);
                                               ...
                                               struct ldci_param_out *ldci_param_out)
{
    ...
    ldci_tone_curve_gen_sw(reg_ldci_tone_curve_lut_all, LDCI_TONE_CURVE_LUT_NUM,
                           LDCI_TONE_CURVE_LUT_SIZE, x1, x2, DarkContrastLow, DarkContrastHigh,
                           BrightContrastLow, BrightCOntrastHigh, DarkContrastLow2,
                           DarkContrastHigh2, BrightContrastLow2, BrightCOntrastHigh2);
...
static void ContrastSet_Gen(float ContrastStrStart, float ContrastStrEnd, const int ContrastSize,
                            float *ContrastSet)
{
...
static int ldci_tone_curve_s(int *LUT, float x1, float x2, float y1, float y2)
{
    ...
    int       node_x[4] = {(int)(fx[0] * idx_max + 0.5), (int)(fx[1] * idx_max + 0.5),
                           (int)(fx[2] * idx_max + 0.5), (int)(fx[3] * idx_max + 0.5)};
    int       node_y[4] = {(int)(fy[0] * 1023 + 0.5), (int)(fy[1] * 1023 + 0.5),
                           (int)(fy[2] * 1023 + 0.5), (int)(fy[3] * 1023 + 0.5)};
