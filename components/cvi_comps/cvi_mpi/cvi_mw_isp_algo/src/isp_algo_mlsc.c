...
static int lsc_chroma_gain_table_fusion(int current_color_temp, unsigned short *color_temp,
                                        int LSC_table_num, unsigned short **r_gain_in,
                                        unsigned short **g_gain_in, unsigned short **b_gain_in,
                                        int NumKnotX, int NumKnotY, unsigned short *r_gain_out,
                                        unsigned short *g_gain_out, unsigned short *b_gain_out,
                                        unsigned int *idx1, unsigned int *idx2);
static int lsc_luma_chroma_gain_to_reg_table(int NumKnotX, int NumKnotY, uint16_t strength,
                                             float *mlsc_compensate_gain, unsigned short *r_gain,
                                             unsigned short *g_gain, unsigned short *b_gain);

CVI_S32 isp_algo_mlsc_main(struct mlsc_param_in  *mlsc_param_in,
                           struct mlsc_param_out *mlsc_param_out)
{
    ...
    lsc_chroma_gain_table_fusion(
        mlsc_param_in->color_temperature, mlsc_param_in->color_temp_tbl,
        mlsc_param_in->color_temp_size, (unsigned short **)mlsc_param_in->mlsc_lut_r,
        (unsigned short **)mlsc_param_in->mlsc_lut_g, (unsigned short **)mlsc_param_in->mlsc_lut_b,
        mlsc_param_in->grid_col, mlsc_param_in->grid_row,
        ISP_PTR_CAST_U16(mlsc_param_out->mlsc_lut_r), ISP_PTR_CAST_U16(mlsc_param_out->mlsc_lut_g),
        ISP_PTR_CAST_U16(mlsc_param_out->mlsc_lut_b), &(mlsc_param_out->temp_lower_idx),
        &(mlsc_param_out->temp_upper_idx));

    lsc_luma_chroma_gain_to_reg_table(
        mlsc_param_in->grid_col, mlsc_param_in->grid_row, mlsc_param_in->strength,
        &(mlsc_param_out->mlsc_compensate_gain), ISP_PTR_CAST_U16(mlsc_param_out->mlsc_lut_r),
        ISP_PTR_CAST_U16(mlsc_param_out->mlsc_lut_g), ISP_PTR_CAST_U16(mlsc_param_out->mlsc_lut_b));
...
#ifdef __riscv_vector
    const size_t NumKnotTotalSize = NumKnotX * NumKnotY * sizeof(uint16_t);
#endif
    // 1. single table case
    if (LSC_table_num == 1) {
...
#else
        rvv_memcpy(r_gain_out, reorder_r_gain_in[0], NumKnotTotalSize);
        rvv_memcpy(g_gain_out, reorder_g_gain_in[0], NumKnotTotalSize);
        rvv_memcpy(b_gain_out, reorder_b_gain_in[0], NumKnotTotalSize);
...
#else
            rvv_memcpy(r_gain_out, reorder_r_gain_in[LSC_table_num - 1], NumKnotTotalSize);
            rvv_memcpy(g_gain_out, reorder_g_gain_in[LSC_table_num - 1], NumKnotTotalSize);
            rvv_memcpy(b_gain_out, reorder_b_gain_in[LSC_table_num - 1], NumKnotTotalSize);
...
#else
            size_t       vl;
            int          NumKnotTotal = NumKnotX * NumKnotY;
            vuint16m4_t  vu16Gain0;
            vuint16m4_t  vu16Gain1;
            vfloat32m8_t vf32Val;

            // hint: maybe can be optimized more
            // convert float32 to float16, and set vl to e16m8 to get double performance
            for (idx = 0; idx < NumKnotTotal; idx += vl) {
                vl = vsetvl_e32m8(NumKnotTotal - idx);

                vu16Gain0 = vle16_v_u16m4(&(reorder_r_gain_in[table_0][idx]), vl);
                vu16Gain1 = vle16_v_u16m4(&(reorder_r_gain_in[table_1][idx]), vl);
                vf32Val   = vfmul_vf_f32m8(vfwcvt_f_xu_v_f32m8(vu16Gain0, vl), ratio_0, vl);
                vf32Val = vfmacc_vf_f32m8(vf32Val, ratio_1, vfwcvt_f_xu_v_f32m8(vu16Gain1, vl), vl);
                vu16Gain0 = vfncvt_xu_f_w_u16m4(vf32Val, vl);
                vse16_v_u16m4(&(r_gain_out[idx]), vu16Gain0, vl);

                vu16Gain0 = vle16_v_u16m4(&(reorder_g_gain_in[table_0][idx]), vl);
                vu16Gain1 = vle16_v_u16m4(&(reorder_g_gain_in[table_1][idx]), vl);
                vf32Val   = vfmul_vf_f32m8(vfwcvt_f_xu_v_f32m8(vu16Gain0, vl), ratio_0, vl);
                vf32Val = vfmacc_vf_f32m8(vf32Val, ratio_1, vfwcvt_f_xu_v_f32m8(vu16Gain1, vl), vl);
                vu16Gain0 = vfncvt_xu_f_w_u16m4(vf32Val, vl);
                vse16_v_u16m4(&(g_gain_out[idx]), vu16Gain0, vl);

                vu16Gain0 = vle16_v_u16m4(&(reorder_b_gain_in[table_0][idx]), vl);
                vu16Gain1 = vle16_v_u16m4(&(reorder_b_gain_in[table_1][idx]), vl);
                vf32Val   = vfmul_vf_f32m8(vfwcvt_f_xu_v_f32m8(vu16Gain0, vl), ratio_0, vl);
                vf32Val = vfmacc_vf_f32m8(vf32Val, ratio_1, vfwcvt_f_xu_v_f32m8(vu16Gain1, vl), vl);
                vu16Gain0 = vfncvt_xu_f_w_u16m4(vf32Val, vl);
                vse16_v_u16m4(&(b_gain_out[idx]), vu16Gain0, vl);
            }
                                             ...
                                             unsigned short *g_gain, unsigned short *b_gain)
{
    ...
    size_t   vl;
    int      NumKnotTotal = NumKnotX * NumKnotY;
    uint32_t u32_per      = _percent_0_ggain + (1 << 11);
    uint32_t u32_rnd      = 1 << (LSC_GAIN_BASE - 1);

    for (idx = 0; idx < NumKnotTotal; idx += vl) {
        vl = vsetvl_e32m8(NumKnotTotal - idx);

        vuint32m8_t vu32Val;
        vuint16m4_t vu16Rgain = vle16_v_u16m4(&(r_gain[idx]), vl);
        vuint16m4_t vu16Ggain = vle16_v_u16m4(&(g_gain[idx]), vl);
        vuint16m4_t vu16Bgain = vle16_v_u16m4(&(b_gain[idx]), vl);

        vu32Val   = vwmulu_vx_u32m8(vu16Ggain, strength, vl);
        vu32Val   = vadd_vx_u32m8(vu32Val, u32_per, vl);
        vu16Ggain = vnsrl_wx_u16m4(vu32Val, (uint16_t)(12), vl);

        vu32Val   = vwmulu_vv_u32m8(vu16Rgain, vu16Ggain, vl);
        vu32Val   = vadd_vx_u32m8(vu32Val, u32_rnd, vl);
        vu16Rgain = vnsrl_wx_u16m4(vu32Val, (uint16_t)LSC_GAIN_BASE, vl);
        vu16Rgain = vminu_vx_u16m4(vu16Rgain, (uint16_t)LSC_GAIN_MAX, vl);

        vu32Val   = vwmulu_vv_u32m8(vu16Bgain, vu16Ggain, vl);
        vu32Val   = vadd_vx_u32m8(vu32Val, u32_rnd, vl);
        vu16Bgain = vnsrl_wx_u16m4(vu32Val, (uint16_t)LSC_GAIN_BASE, vl);
        vu16Bgain = vminu_vx_u16m4(vu16Bgain, (uint16_t)LSC_GAIN_MAX, vl);

        vse16_v_u16m4(&(r_gain[idx]), vu16Rgain, vl);
        vse16_v_u16m4(&(g_gain[idx]), vu16Ggain, vl);
        vse16_v_u16m4(&(b_gain[idx]), vu16Bgain, vl);

        vuint16m1_t vu16Min = vmv_s_x_u16m1(vu16Min, min_gain, vl);

        vu16Min = vredminu_vs_u16m4_u16m1(vu16Min, vu16Rgain, vu16Min, vl);
        vu16Min = vredminu_vs_u16m4_u16m1(vu16Min, vu16Bgain, vu16Min, vl);

        min_gain = vmv_x_s_u16m1_u16(vu16Min);
    }

#else  // end __riscv_vector
    for (int j = 0; j < NumKnotY; j++) {
        int i = 0;
#ifdef __ARM_NEON__
        ...
        for (; i < NumKnotX; i++) {
            ...
            if (r_gain[idx] < min_gain) min_gain = r_gain[idx];
            b_gain[idx] =
                MIN(LSC_GAIN_MAX,
                    (b_gain[idx] * g_gain[idx] + (1 << (LSC_GAIN_BASE - 1))) >> LSC_GAIN_BASE);
            if (b_gain[idx] < min_gain) min_gain = b_gain[idx];
        }
    }
    ...
    *mlsc_compensate_gain = comp_gain;
    ...
    if (comp_gain > 1.0) {
        ...
        for (idx = 0; idx < NumKnotTotal; idx += vl) {
            ...
            vf32Val   = vfwcvt_f_xu_v_f32m8(vu16Rgain, vl);
            vf32Val   = vfmul_vf_f32m8(vf32Val, comp_gain, vl);
            vu16Rgain = vfncvt_xu_f_w_u16m4(vf32Val, vl);
            vu16Rgain = vminu_vx_u16m4(vu16Rgain, (unsigned short)LSC_GAIN_MAX, vl);

            vf32Val   = vfwcvt_f_xu_v_f32m8(vu16Ggain, vl);
            vf32Val   = vfmul_vf_f32m8(vf32Val, comp_gain, vl);
            vu16Ggain = vfncvt_xu_f_w_u16m4(vf32Val, vl);
            vu16Ggain = vminu_vx_u16m4(vu16Ggain, (unsigned short)LSC_GAIN_MAX, vl);

            vf32Val   = vfwcvt_f_xu_v_f32m8(vu16Bgain, vl);
            vf32Val   = vfmul_vf_f32m8(vf32Val, comp_gain, vl);
            vu16Bgain = vfncvt_xu_f_w_u16m4(vf32Val, vl);
            vu16Bgain = vminu_vx_u16m4(vu16Bgain, (unsigned short)LSC_GAIN_MAX, vl);
