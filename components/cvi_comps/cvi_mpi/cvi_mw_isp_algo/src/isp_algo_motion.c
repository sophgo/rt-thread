...
static CVI_S32 motion_level_calculate(struct motion_param_in  *param_in,
                                      struct motion_param_out *param_out)
{
    ...
    if (param_in->motionStsBufSize >= (total_grid * sizeof(CVI_U16))) {
#ifdef __riscv_vector
        CVI_U8 *motionMapPtr = ISP_PTR_CAST_U8(param_in->motionMapAddr);
        size_t  vl;
        size_t  grid_idx;

        for (grid_idx = 0; grid_idx < total_grid; grid_idx += vl) {
            vl                  = vsetvl_e8m8(total_grid - grid_idx);
            size_t     _u8_idx  = grid_idx << 1;
            vuint8m8_t vu8m8Val = vlse8_v_u8m8(&motionMapPtr[_u8_idx], (ptrdiff_t)0x2, vl);
            vbool1_t   vb1Mask  = vmsgtu_vx_u8m8_b1(vu8m8Val, u8MotionTh, vl);

            motion_cnt = motion_cnt + vcpop_m_b1(vb1Mask, vl);
        }
        ...
        for (y = 0; y < grid_h_num; y = y + 1) {
            for (x = 0; x < grid_w_num; x = x + 1) {
                CVI_U16 *motionValue = (CVI_U16 *)&(ISP_PTR_CAST_U8(param_in->motionMapAddr)[idx]);
...
CVI_S32 isp_algo_motion_main(struct motion_param_in  *motion_param_in,
