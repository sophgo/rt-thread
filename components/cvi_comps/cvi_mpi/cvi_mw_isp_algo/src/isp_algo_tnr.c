...
static CVI_S32 get_manr_noise_profile_reg(CVI_U32 cur_iso_val, CVI_S32 blk_size_w_bits,
                                          CVI_S32 reg_rgbmap_input_sel, CVI_S32 blk_size_h_bits,
                                          CVI_S32 digital_gain, CVI_S32 wb_r_gain,
                                          CVI_S32 wb_b_gain, CVI_FLOAT cali_noise_model[16][4][2],
                                          CVI_S32 noise_level_low, CVI_S32 noise_level_high,
                                          CVI_S32 color_id, CVI_S32 *slope, CVI_S32 *luma_th,
                                          CVI_S32 *low_offset, CVI_S32 *high_offset);
static CVI_S32 get_manr_flick_profile_reg(int flick_slp, int *slope, int *luma_th, int *low_offset,
                                          int *high_offset);
...
CVI_S32 isp_algo_tnr_main(struct tnr_param_in *tnr_param_in, struct tnr_param_out *tnr_param_out)
{
    ...
    for (CVI_U32 i = 0; i < 3; i++) {
        ...
        tnr_param_out->LowMtPrtSlopeY[i] =
            get_lut_slp(in0, in1, out0, out1, MANR_ALPHA_LUT_SLP_FBIT);
    }
    ...
    for (CVI_U32 i = 0; i < 3; i++) {
        ...
        tnr_param_out->LowMtPrtSlopeU[i] =
            get_lut_slp(in0, in1, out0, out1, MANR_ALPHA_LUT_SLP_FBIT);
    }
    ...
    for (CVI_U32 i = 0; i < 3; i++) {
        ...
        tnr_param_out->LowMtPrtSlopeV[i] =
            get_lut_slp(in0, in1, out0, out1, MANR_ALPHA_LUT_SLP_FBIT);
    }
    ...
    for (CVI_U32 i = 0; i < 3; i++) {
        ...
        tnr_param_out->LowMtPrtAdvSlope[i] =
            get_lut_slp(in0, in1, out0, out1, MANR_ALPHA_LUT_SLP_FBIT);
    }
    ...
    for (CVI_U32 i = 0; i < 3; i++) {
        ...
        tnr_param_out->LowMtPrtAdvDebugSlope[i] =
            get_lut_slp(in0, in1, out0, out1, MANR_ALPHA_LUT_SLP_FBIT);
    }
    ...
    for (CVI_U32 i = 0; i < 3; i++) {
        ...
        tnr_param_out->AdaptNrLumaSlope[i] =
            get_lut_slp(in0, in1, out0, out1, MANR_ALPHA_LUT_SLP_FBIT);
    }
    ...
    for (CVI_U32 i = 0; i < 3; i++) {
        ...
        tnr_param_out->AdaptNrChromaSlope[i] =
            get_lut_slp(in0, in1, out0, out1, MANR_ALPHA_LUT_SLP_FBIT);
    }

    for (CVI_U32 path = 0; path < TNR_PATH_NUM; path++) {
        for (CVI_U32 chan = 0; chan < TNR_CHAN_NUM; chan++) {
            get_manr_noise_profile_reg(
                tnr_param_in->iso, tnr_param_in->rgbmap_w_bit, tnr_param_in->gbmap_input_sel,
                tnr_param_in->rgbmap_h_bit, tnr_param_in->ispdgain, tnr_param_in->wb_rgain,
                tnr_param_in->wb_bgain, tnr_param_in->np.CalibrationCoef,
    ...
    for (CVI_U32 path = 0; path < TNR_PATH_NUM; path++) {
        for (CVI_U32 chan = 0; chan < TNR_CHAN_NUM; chan++) {
            get_manr_flick_profile_reg(
                tnr_param_in->BrightnessNoiseLevelLE[path], &(tnr_param_out->slope[path][chan]),
                &(tnr_param_out->luma[path][chan]), &(tnr_param_out->low[path][chan]),
                &(tnr_param_out->high[path][chan]));
        }
    }

    // TODO@CV181X sw function
    tnr_param_out->SharpenCompGain = 256;
    for (CVI_U32 i = 0; i < 17; i++) tnr_param_out->LumaCompGain[i] = 512;

    tnr_param_out->TnrLscCenterX = 960;
    tnr_param_out->TnrLscCenterY = 540;
    tnr_param_out->TnrLscNorm    = 30;
    tnr_param_out->TnrLscDYGain  = 128;
    for (CVI_U32 i = 0; i < 17; i++) tnr_param_out->TnrLscCompGain[i] = 512;

    tnr_param_out->MtLscCenterX = 960;
    tnr_param_out->MtLscCenterY = 540;
    tnr_param_out->MtLscNorm    = 30;
    tnr_param_out->MtLscDYGain  = 128;
    for (CVI_U32 i = 0; i < 17; i++) tnr_param_out->MtLscCompGain[i] = 512;
...
static CVI_FLOAT isp_algo_tnr_get_threshold(struct tnr_motion_block_size_threshold *ptStruct)
{
    ...
    ISP_ALGO_LOG_DEBUG("Block : %3u x %3u, Size : %5u, B/R : %5u (%8.5f), G : %5u (%8.5f)\n",
                       ptStruct->u32BlockDim, ptStruct->u32BlockDim, u32BlockSize, u32BlockSizeBR,
                       fLengthBR, u32BlockSizeG, fLengthG);
...
// -----------------------------------------------------------------------------
CVI_S32 isp_algo_tnr_generateblocksize(struct tnr_motion_block_size_in  *ptIn,
                                       struct tnr_motion_block_size_out *ptOut)
{
    ...
    for (CVI_U32 u32BlockValueIdx = 0; u32BlockValueIdx < gu32BlockValueTblSize;
         ++u32BlockValueIdx) {
...
// -----------------------------------------------------------------------------
static int get_manr_flick_profile_reg(int flick_slp, int *slope, int *luma_th, int *low_offset,
                                      int *high_offset)
{
    int rtn = 0;
                                          ...
                                          CVI_S32 *low_offset, CVI_S32 *high_offset)
{
    ...
    if (total_weight == 0) {  // take low iso data
        ...
        if (color_id == COLOR_R) {
            ...
            param_slp  = (low_weight *
                             (noise_model[idx_low_0 + SLP_IDX] + noise_model[idx_low_1 + SLP_IDX]) +
                         high_weight * (noise_model[idx_high_0 + SLP_IDX] +
                                        noise_model[idx_high_1 + SLP_IDX])) /
                        (2 * total_weight);
            param_off = (low_weight *
                             (noise_model[idx_low_0 + OFF_IDX] + noise_model[idx_low_1 + OFF_IDX]) +
                         high_weight * (noise_model[idx_high_0 + OFF_IDX] +
                                        noise_model[idx_high_1 + OFF_IDX])) /
                        (2 * total_weight);
        } else {
            idx_low_0  = low_iso * 4 * 2 + NS_MODEL_B * 2;
    ...
    param_off =
        ((luma_low_bound * param_slp + param_off) * digital_gain) / (1 << DIGITAL_GAIN_F_BITS);
