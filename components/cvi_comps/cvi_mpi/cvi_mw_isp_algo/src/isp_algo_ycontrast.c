...
CVI_S32 isp_algo_ycontrast_main(struct ycontrast_param_in  *param_in,
                                struct ycontrast_param_out *param_out)
{
    // ISP_DEBUG(LOG_INFO, "%s\n", "+");
    CVI_S32 ret = CVI_SUCCESS;

    ycontrast_curve_gen(param_in->ContrastLow, param_in->ContrastHigh, param_in->CenterLuma,
                               ...
                               CVI_U8 *pau8YCurve_YLut, CVI_U16 *pu16YCurve_YLastPoint)
{
    ...
    CVI_U32 u32HighTarget =
        round((CVI_FLOAT)((Y_CURVE_MAX_VERTICAL - u32CenterLuma256) * u8ContrastHigh) / 100.0f +
              u32CenterLuma256);
    ...
    if (u32CenterLuma64 == 0) {
        as32XIn[0]        = 0;
        as32YIn[0]        = u32CenterLuma256;
        as32XIn[1]        = u32HighNode;
        as32YIn[1]        = u32HighTarget;
        as32XIn[2]        = Y_CURVE_MAX_HORIZONTAL;
        as32YIn[2]        = Y_CURVE_MAX_VERTICAL;
        stPCHIP.u32SizeIn = 3;
    } else if (u32CenterLuma64 == 1) {
        as32XIn[0]        = 0;
        as32YIn[0]        = u32LowTarget;
        as32XIn[1]        = u32CenterLuma64;
        as32YIn[1]        = u32CenterLuma256;
        as32XIn[2]        = u32HighNode;
        as32YIn[2]        = u32HighTarget;
        as32XIn[3]        = Y_CURVE_MAX_HORIZONTAL;
        as32YIn[3]        = Y_CURVE_MAX_VERTICAL;
        stPCHIP.u32SizeIn = 4;
    } else if (u32CenterLuma64 == (Y_CURVE_MAX_HORIZONTAL - 1)) {
        as32XIn[0]        = 0;
        as32YIn[0]        = 0;
        as32XIn[1]        = u32LowNode;
        as32YIn[1]        = u32LowTarget;
        as32XIn[2]        = u32CenterLuma64;
        as32YIn[2]        = u32CenterLuma256;
        as32XIn[3]        = u32HighNode;
        as32YIn[3]        = u32HighTarget;
        stPCHIP.u32SizeIn = 4;
        ...
        as32XIn[3]        = u32HighNode;
        as32YIn[3]        = u32HighTarget;
        as32XIn[4]        = Y_CURVE_MAX_HORIZONTAL;
        as32YIn[4]        = Y_CURVE_MAX_VERTICAL;
        stPCHIP.u32SizeIn = 5;
    }
    ...
    for (CVI_U32 i = 0; i < YCURVE_LUT_ENTRY_NUM; ++i) {
        pau8YCurve_YLut[i] =
            (as32YOut[i] >= Y_CURVE_MAX_LUT) ? Y_CURVE_MAX_LUT : (CVI_U8)as32YOut[i];
    }
