...
int IIR_U8_Once(TIIR_U8_Ctrl *ptIIRCoef)
{
    ...
    if (u16IIRWeight <= 1) {
        for (u16Idx = 0; u16Idx < ptIIRCoef->u16LutSize; u16Idx += vl) {
            vl                   = vsetvl_e32m8(ptIIRCoef->u16LutSize - u16Idx);
            vuint8m2_t  vu8m2In  = vle8_v_u8m2(&pu8In[u16Idx], vl);
            vuint16m4_t vu16m4In = vwcvtu_x_x_v_u16m4(vu8m2In, vl);
            vuint32m8_t vu32m8In = vwcvtu_x_x_v_u32m8(vu16m4In, vl);

            vu32m8In = vsll_vx_u32m8(vu32m8In, (size_t)IIR_U8_DECIMAL_BITS, vl);
            vse32_v_u32m8(&pu32History[u16Idx], vu32m8In, vl);
            rvv_memcpy(&pu8Out[u16Idx], &pu8In[u16Idx], vl);
        }
    } else {
        uint16_t u16IIRDenominator = u16IIRWeight + 1;
        ...
        for (u16Idx = 0; u16Idx < ptIIRCoef->u16LutSize; u16Idx += vl) {
            ...
            vuint16m4_t vu16m4In      = vwcvtu_x_x_v_u16m4(vu8m2In, vl);
            vuint32m8_t vu32m8In      = vwcvtu_x_x_v_u32m8(vu16m4In, vl);
            vuint32m8_t vu32m8History = vle32_v_u32m8(&pu32History[u16Idx], vl);

            vu32m8In      = vsll_vx_u32m8(vu32m8In, (size_t)IIR_U8_DECIMAL_BITS, vl);
            vu32m8In      = vmacc_vx_u32m8(vu32m8In, (size_t)u16IIRWeight, vu32m8History, vl);
            vu32m8History = vdivu_vx_u32m8(vu32m8In, (size_t)u16IIRDenominator, vl);
            vse32_v_u32m8(&pu32History[u16Idx], vu32m8History, vl);

            vu32m8In = vadd_vx_u32m8(vu32m8History, (size_t)IIR_U8_ROUNDING_HALF, vl);
            vu32m8In = vsrl_vx_u32m8(vu32m8In, (size_t)IIR_U8_DECIMAL_BITS, vl);
            vu16m4In = vncvt_x_x_w_u16m4(vu32m8In, vl);
            vu8m2In  = vncvt_x_x_w_u8m2(vu16m4In, vl);
            vse8_v_u8m2(&pu8Out[u16Idx], vu8m2In, vl);
        }
    }
#endif  // __riscv_vector
#else
...
int IIR_U10_Once(TIIR_U10_Ctrl *ptIIRCoef)
{
    ...
    if (u16IIRWeight <= 1) {
        for (u16Idx = 0; u16Idx < ptIIRCoef->u16LutSize; u16Idx += vl) {
            vl                   = vsetvl_e32m8(ptIIRCoef->u16LutSize - u16Idx);
            vuint16m4_t vu16m4In = vle16_v_u16m4(&pu16In[u16Idx], vl);
            vuint32m8_t vu32m8In = vwcvtu_x_x_v_u32m8(vu16m4In, vl);

            vu32m8In = vsll_vx_u32m8(vu32m8In, (size_t)IIR_U10_DECIMAL_BITS, vl);
            vse32_v_u32m8(&pu32History[u16Idx], vu32m8In, vl);
            rvv_memcpy(&pu16Out[u16Idx], &pu16In[u16Idx], sizeof(pu16Out[u16Idx]) * vl);
        }
    } else {
        uint16_t u16IIRDenominator = u16IIRWeight + 1;
        ...
        for (u16Idx = 0; u16Idx < ptIIRCoef->u16LutSize; u16Idx += vl) {
            ...
            vu32m8In = vadd_vx_u32m8(vu32m8History, (size_t)IIR_U10_ROUNDING_HALF, vl);
            vu32m8In = vsrl_vx_u32m8(vu32m8In, (size_t)IIR_U10_DECIMAL_BITS, vl);
            vu16m4In = vncvt_x_x_w_u16m4(vu32m8In, vl);
            vse16_v_u16m4(&pu16Out[u16Idx], vu16m4In, vl);
        }
    }
#endif  // __riscv_vector
#else
