    ...
    for (CVI_U32 u32Idx = 0, u32IdxP1 = 1; u32Idx < u32Size - 1; ++u32Idx, ++u32IdxP1) {
        ...
        if (ps32YIn[u32Idx] > ps32YIn[u32IdxP1]) {
            ISP_ALGO_LOG_WARNING("[warn] y_in should be increasing y[%u]=%d, y[%u]=%d\n", u32Idx,
                                 ps32YIn[u32Idx], u32IdxP1, ps32YIn[u32IdxP1]);
            return CVI_FAILURE;
        }
...
//			float tmp = (float)OutIdx * delta[0];
    ...
    if (IS_POSITIVE(slopes[0]) != IS_POSITIVE(delta[0])) {
        slopes[0] = 0.0f;
    } else if ((IS_POSITIVE(delta[0]) != IS_POSITIVE(delta[1])) &&
    ...
    slopes[u32SizeM1] = ((h[u32SizeM2] + h[u32SizeM2] + h[u32SizeM3]) * delta[u32SizeM2] -
    ...
    } else if ((IS_POSITIVE(delta[u32SizeM2]) != IS_POSITIVE(delta[u32SizeM3])) &&
...
// ----------------------------------------------------------------------------
static inline CVI_U32 PCHIP_InterP1_GetReferenceInIndex(CVI_U32 u32Offset, const CVI_S32 *ps32XIn,
                                                        CVI_U32 u32XInSize)
{
    CVI_S32 s32InIdx;
    CVI_S32 s32Temp;
...
CVI_S32 PCHIP_InterP1_PCHIP_V2_Preprocess(TPCHIP_Info *ptInfo, TPCHIP_PrivateInfo *ptPrivateInfo)
{
    ...
    if (IS_POSITIVE(pSlopes[0]) != IS_POSITIVE(pDelta[0])) {
        pSlopes[0] = 0.0f;
    } else if ((IS_POSITIVE(pDelta[0]) != IS_POSITIVE(pDelta[1])) &&
    ...
    // Slopes at end points.
    pSlopes[u32SizeM1] = ((pH[u32SizeM2] + pH[u32SizeM2] + pH[u32SizeM3]) * pDelta[u32SizeM2] -
    ...
    } else if ((IS_POSITIVE(pDelta[u32SizeM2]) != IS_POSITIVE(pDelta[u32SizeM3])) &&
...
// ----------------------------------------------------------------------------
CVI_S32 PCHIP_InterP1_PCHIP_V2_Process(CVI_U32 u32XOutIndex, TPCHIP_Info *ptInfo,
                                       TPCHIP_PrivateInfo *ptPrivateInfo, CVI_S32 *ps32YOutValue)
{
    CVI_S32 *ps32XIn = ptInfo->ps32XIn;
    CVI_S32 *ps32YIn = ptInfo->ps32YIn;
    float   *pH      = ptPrivateInfo->h;
