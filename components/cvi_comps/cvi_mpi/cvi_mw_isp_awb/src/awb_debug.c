...
static void AWB_ShowCTCurve(CVI_U8 sID, const sWBInfo *pstWBInfo)
{
    ...
    for (i = pstWBInfo->stAWBBound.u16RLowBound; i <= pstWBInfo->stAWBBound.u16RHighBound;
         i += AWB_STEP)
...
static void AWB_ShowCurInfo(CVI_U8 sID, const sWBInfo *pstWBInfo)
{
    ...
    printf("CR:%d, %d %d, %d %d, %d %d, %d\n", tmpCurve.u16LowTempTopRange,
           tmpCurve.u16LowTempBotRange, tmpCurve.u16MidTempTopRange, tmpCurve.u16MidTempBotRange,
           tmpCurve.u16CurveWhiteTopRange, tmpCurve.u16CurveWhiteBotRange,
           tmpCurve.u16HighTempBotRange, tmpCurve.u16HighTempBotRange);
    ...
    printf("rawG:%d TC:%d EC:%d GC:%d GR:%d\n", pSampleInfo->u16CentRawG,
           pSampleInfo->u16TotalSampleCnt, pSampleInfo->u16EffectiveSampleCnt,
           pSampleInfo->u16GrayCnt, pSampleInfo->u8GrayRatio);
...
static void AWB_ShowBootInfo(CVI_U8 sID, const sWBInfo *pstWBInfo)
{
    ...
    for (i = 0; i < AWB_BOOT_MAX_FRAME; ++i)
        printf("%d %d Fid:%u R:%d B:%d K:%d CS:%d\n", sID, i, pstWBInfo->u32FrameID,
               pstWBInfo->stBootGain[i].u16RGain, pstWBInfo->stBootGain[i].u16BGain,
               pstWBInfo->u16BootColorTemp[i], pstWBInfo->bBootConvergeStatus[i]);
}
...
static void AWB_ShowCalibration(CVI_U8 sID)
{
    ...
    for (i = 0; i < AWB_CALIB_PTS_NUM; ++i)
        printf("%d R:%d B:%d CT:%d\n", i, tmpWBCalib.u16AvgRgain[i], tmpWBCalib.u16AvgBgain[i],
               tmpWBCalib.u16ColorTemperature[i]);
}
...
static CVI_BOOL CVI_ISP_SetWBAttr_Test(VI_PIPE ViPipe)
{
    ...
    if (u32RunCnt[1] == u32RunCnt[0]) return CVI_FAILURE;
    return CVI_SUCCESS;
}
...
static CVI_BOOL CVI_ISP_SetWBAttrEx_Test(VI_PIPE ViPipe)
{
    ...
    if (u16GrayCnt[1]) return CVI_FAILURE;
...
static CVI_BOOL CVI_ISP_QueryWBInfo_Test(VI_PIPE ViPipe)
{
    ...
    printf("Rgain %d\n", tmpWbInfo.u16Rgain);
    CVI_ISP_SetWBAttr(ViPipe, &oldWbAttr);
    if (tmpWbInfo.u16Rgain != tmpWbAttr.stManual.u16Rgain) return CVI_FAILURE;
    return CVI_SUCCESS;
}
...
static CVI_BOOL CVI_ISP_CalGainByTemp_Test(VI_PIPE ViPipe)
{
    ...
    if (tmpGain[0][ISP_BAYER_CHN_R] < tmpGain[1][ISP_BAYER_CHN_R]) return CVI_FAILURE;
    if (tmpGain[0][ISP_BAYER_CHN_B] > tmpGain[1][ISP_BAYER_CHN_B]) return CVI_FAILURE;
