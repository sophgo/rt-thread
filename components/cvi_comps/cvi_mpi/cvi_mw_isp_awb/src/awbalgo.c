...
static ISP_WB_ATTR_S     stAwbAttrInfo[AWB_SENSOR_NUM];
static ISP_AWB_ATTR_EX_S stAwbAttrInfoEx[AWB_SENSOR_NUM];

CVI_U8 isNeedUpdateAttr[AWB_SENSOR_NUM], isNeedUpdateAttrEx[AWB_SENSOR_NUM];
CVI_U8 isNeedUpdateCalib[AWB_SENSOR_NUM];

static sWBInfo                       WBInfo[AWB_SENSOR_NUM];
static sWBSampleInfo                 stSampleInfo[AWB_SENSOR_NUM];
static sWBBoundary                   stBoundaryInfo[AWB_SENSOR_NUM];
static sWBCurveInfo                  stCurveInfo[AWB_SENSOR_NUM];
static ISP_AWB_Calibration_Gain_S    stWbDefCalibration[AWB_SENSOR_NUM];
static ISP_AWB_Calibration_Gain_S_EX stWbCalibrationEx[AWB_SENSOR_NUM];
...
static CVI_S32 AWB_BuildTempCurve(CVI_FLOAT *BufferX, CVI_FLOAT *BufferY, CVI_U8 Amount,
                                  CVI_FLOAT *ParaK);
...
static void AWB_BoundaryInit(CVI_U8 sID, sWBCurveInfo *pstWBCurveInfo, sWBBoundary *pstWBBoundary)
{
    ...
    if (pstRegion->u16Region1 < pstRegion->u16Region2 &&
        ...
        pstWBCurveInfo->u16Region1_R = AWB_GetGainByColorTemperature(sID, pstRegion->u16Region1);
        pstWBCurveInfo->u16Region2_R = AWB_GetGainByColorTemperature(sID, pstRegion->u16Region2);
        pstWBCurveInfo->u16Region3_R = AWB_GetGainByColorTemperature(sID, pstRegion->u16Region3);
    } else {
        pstWBCurveInfo->u16Region1_R = AWB_GetGainByColorTemperature(sID, CT_SHIFT_LIMIT_REGION1);
        pstWBCurveInfo->u16Region2_R = AWB_GetGainByColorTemperature(sID, CT_SHIFT_LIMIT_REGION2);
        pstWBCurveInfo->u16Region3_R = AWB_GetGainByColorTemperature(sID, CT_SHIFT_LIMIT_REGION3);
    }
...
static void AWB_CalculateCurveOld(CVI_U8 sID, CVI_FLOAT *pCurveCoei)
{
    ...
    if (WBInfo[sID].stGoldenWB.u16RGain) {
        WBInfo[sID].u16FixedWBOffsetR =
            (WBInfo[sID].stDefaultWB.u16RGain * 100 + WBInfo[sID].stGoldenWB.u16RGain / 2) /
            WBInfo[sID].stGoldenWB.u16RGain;
    } else {
        WBInfo[sID].u16FixedWBOffsetR = 100;
        ISP_LOG_ERR("%s\n", "GoldenR is 0");
    }
    if (WBInfo[sID].stGoldenWB.u16BGain) {
        WBInfo[sID].u16FixedWBOffsetB =
            (WBInfo[sID].stDefaultWB.u16BGain * 100 + WBInfo[sID].stGoldenWB.u16BGain / 2) /
            WBInfo[sID].stGoldenWB.u16BGain;
    } else {
        WBInfo[sID].u16FixedWBOffsetB = 100;
        ISP_LOG_ERR("%s\n", "GoldenB is 0");
    }

    if (WBInfo[sID].u16FixedWBOffsetR == 0) WBInfo[sID].u16FixedWBOffsetR = 100;
    ...
    for (i = WBInfo[sID].stAWBBound.u16RLowBound; i <= WBInfo[sID].stAWBBound.u16RHighBound;
         i += AWB_STEP) {
        ...
        if (inx < AWB_CURVE_SIZE) WBInfo[sID].u16CTCurve[inx] = y;
...
static void AWB_CalculateCurveNew(CVI_U8 sID, ISP_WB_ATTR_S *pwbAttr)
{
    ...
    for (i = 0; i < AWB_CURVE_PARA_NUM; i++) {
        ...
        DBGMSG("CN %d %f\n", pwbAttr->stAuto.as32CurvePara[i], WBInfo[sID].fCurveCoei[i]);
    }
    ...
    WBInfo[sID].stAWBBound.u16BLowBound  = AWB_BOUND_LOW_B * WBInfo[sID].u16FixedWBOffsetB / 100;
    WBInfo[sID].stAWBBound.u16BHighBound = AWB_BOUND_HIGH_B * WBInfo[sID].u16FixedWBOffsetB / 100;
    WBInfo[sID].stAWBBound.u16RHighBound =
        (WBInfo[sID].stAWBBound.u16RHighBound / AWB_STEP) * AWB_STEP;
    WBInfo[sID].stAWBBound.u16RLowBound =
        (WBInfo[sID].stAWBBound.u16RLowBound / AWB_STEP) * AWB_STEP;

    for (i = WBInfo[sID].stAWBBound.u16RHighBound; i >= WBInfo[sID].stAWBBound.u16RLowBound;
         i -= AWB_STEP) {
        j  = i;
        fr = GoldenR / (CVI_FLOAT)(j);
        fb = WBInfo[sID].fCurveCoei[0] * fr * fr + WBInfo[sID].fCurveCoei[1] * fr +
             WBInfo[sID].fCurveCoei[2];
...
void AWB_BuildCurve(CVI_U8 sID, const ISP_AWB_Calibration_Gain_S *psWBGain, ISP_WB_ATTR_S *pwbAttr)
{
    ...
    ISP_LOG_INFO("Golden %d %d %d\n", pwbAttr->stAuto.au16StaticWB[ISP_BAYER_CHN_R],
                 pwbAttr->stAuto.au16StaticWB[ISP_BAYER_CHN_GR],
                 pwbAttr->stAuto.au16StaticWB[ISP_BAYER_CHN_B]);
    ...
    WBInfo[sID].bQuadraticPolynomial =
        AWB_CalculateQuadraticCoeff(WBInfo[sID].fCurveCoei, RGain, BGain);
    ...
    for (i = 0; i < AWB_CALIB_PTS_NUM; ++i) {
        bufferX[i] =
            WBInfo[sID].stCalibWBGain[i].u16RGain / (float)WBInfo[sID].stCalibWBGain[i].u16BGain;
...
static void AWB_GetShfitRatio(CVI_U8 sID, CVI_FLOAT *fshiftRatio)
{
    ...
    if (pstShiftlv->u16HighLvThr[ISP_AWB_COLORTEMP_LOW] >
            pstShiftlv->u16LowLvThr[ISP_AWB_COLORTEMP_LOW] &&
        pstShiftlv->u16HighLvThr[ISP_AWB_COLORTEMP_LOW] >
            pstShiftlv->u16LowLvThr[ISP_AWB_COLORTEMP_HIGH] &&
        pstShiftlv->u16HighLvThr[ISP_AWB_COLORTEMP_HIGH] >
            pstShiftlv->u16LowLvThr[ISP_AWB_COLORTEMP_LOW] &&
        pstShiftlv->u16HighLvThr[ISP_AWB_COLORTEMP_HIGH] >
            pstShiftlv->u16LowLvThr[ISP_AWB_COLORTEMP_HIGH] &&
    ...
    if (pstShiftlv->u8LowLvMode || pstShiftlv->u8HighLvMode) {
        // low colortemp
        if (pstShiftlv->u8LowLvMode && s16Lv <= pstShiftlv->u16LowLvThr[ISP_AWB_COLORTEMP_LOW]) {
            ...
            if (pstShiftlv->u16LowLvCT[ISP_AWB_COLORTEMP_LOW] & MIDTEMPTOP)
                fshiftRatio[3] = pstShiftlv->u16LowLvRatio[ISP_AWB_COLORTEMP_LOW] / (float)100;
                   ...
                   s16Lv >= pstShiftlv->u16HighLvThr[ISP_AWB_COLORTEMP_LOW] && bParamVaild) {
        ...
        // high colortemp
        if (pstShiftlv->u8LowLvMode && s16Lv <= pstShiftlv->u16LowLvThr[ISP_AWB_COLORTEMP_HIGH] &&
            ...
            if (pstShiftlv->u16LowLvCT[ISP_AWB_COLORTEMP_HIGH] & HIGHTEMPTOP)
                fshiftRatio[7] = 100 / (float)pstShiftlv->u16LowLvRatio[ISP_AWB_COLORTEMP_HIGH];
                   ...
                   s16Lv >= pstShiftlv->u16HighLvThr[ISP_AWB_COLORTEMP_HIGH] && bParamVaild) {
...
static void AWB_AdjCurveBoundaryByISO(CVI_U8 sID, sWBBoundary *pstWBBoundary)
{
    ...
    if (stAwbAttrInfo[sID].stAuto.stCbCrTrack.bEnable) {
        ...
        if (u16Shift) u16Shift = AWB_GAIN_BASE * AWB_GAIN_BASE / u16Shift;   // G/R
...
static void AWB_AdjCurveBoundaryByColorTemperature(CVI_U8 sID, sWBBoundary *pstWBBoundary,
                                                   CVI_U16 lowCT, CVI_U16 highCT)
{
...
static void AWB_GetWhiteBGainRange(CVI_U8 sID, CVI_U16 RGain, CVI_U16 *BGain, CVI_U16 *curveTop,
                                   CVI_U16 *curveBot, CVI_U16 *cWideTop, CVI_U16 *cWieBot,
                                   sWBCurveInfo *wbCurveInfo)
{
    ...
    if (RGain < WBInfo[sID].stAWBBound.u16RLowBound ||
        RGain > WBInfo[sID].stAWBBound.u16RHighBound) {
        ...
        if (RGain < RGainTh1 + SMOOTH_GAIN_RANGE) {
            ...
            if ((RGainDiff1 + RGainDiff2)) {
                tmpTopRange = (RGainDiff1 * wbCurveInfo->u16MidTempTopRange +
                               RGainDiff2 * wbCurveInfo->u16LowTempTopRange) /
                              (RGainDiff1 + RGainDiff2);
                tmpBotRange = (RGainDiff1 * wbCurveInfo->u16MidTempBotRange +
                               RGainDiff2 * wbCurveInfo->u16LowTempBotRange) /
                              (RGainDiff1 + RGainDiff2);
            }
        } else {
        ...
        if (RGain < RGainTh3 + SMOOTH_GAIN_RANGE) {
            ...
            if ((RGainDiff1 + RGainDiff2)) {
                tmpTopRange = (RGainDiff1 * wbCurveInfo->u16HighTempTopRange +
                               RGainDiff2 * wbCurveInfo->u16CurveWhiteTopRange) /
                              (RGainDiff1 + RGainDiff2);
                tmpBotRange = (RGainDiff1 * wbCurveInfo->u16HighTempBotRange +
                               RGainDiff2 * wbCurveInfo->u16CurveWhiteBotRange) /
                              (RGainDiff1 + RGainDiff2);
            }
        } else {
        ...
        if (RGain < RGainTh2 + SMOOTH_GAIN_RANGE) {
            ...
            if ((RGainDiff1 + RGainDiff2)) {
                tmpTopRange = (RGainDiff1 * wbCurveInfo->u16CurveWhiteTopRange +
                               RGainDiff2 * wbCurveInfo->u16MidTempTopRange) /
                              (RGainDiff1 + RGainDiff2);
                tmpBotRange = (RGainDiff1 * wbCurveInfo->u16CurveWhiteBotRange +
                               RGainDiff2 * wbCurveInfo->u16MidTempBotRange) /
                              (RGainDiff1 + RGainDiff2);
            }
        } else {
...
static AWB_POINT_TYPE AWB_ExtraLightWB(CVI_U8 sID, CVI_U16 RGain, CVI_U16 BGain, CVI_BOOL *pInCurve)
{
    ...
    if (stAwbAttrInfo[sID].stAuto.enAlgType != AWB_ALG_ADVANCE) return AWB_POINT_EXTRA_NONE;

    if (stAwbAttrInfoEx[sID].bExtraLightEn == 0) return AWB_POINT_EXTRA_NONE;

    for (j = 0; j < AWB_LS_NUM; j++) {
        if (((*pInCurve) == 0) &&
            stAwbAttrInfoEx[sID].stLightInfo[j].u8LightStatus == AWB_EXTRA_LS_ADD) {
            ...
            if (RGain > sExLight.ext_ls_r_min[sID][j] && RGain < sExLight.ext_ls_r_max[sID][j]) {
                if (BGain > sExLight.ext_ls_b_min[sID][j] &&
                    BGain < sExLight.ext_ls_b_max[sID][j]) {
                    *pInCurve = 1;
                    if (((expQuant > 50) && (expQuant < 100)) || (expQuant > 150)) {
...
static void AWB_ExtraLightWBInit(CVI_U8 sID)
{
    ...
    if (stAwbAttrInfo[sID].stAuto.enAlgType != AWB_ALG_ADVANCE) return;
    if (stAwbAttrInfoEx[sID].bExtraLightEn == 0) return;
        ...
        if (stAwbAttrInfoEx[sID].stLightInfo[j].u8LightStatus != AWB_EXTRA_LS_DONT_CARE) {
            ...
            if (stAwbAttrInfoEx[sID].stLightInfo[j].u16WhiteRgain) {
                sExLight.ext_ls_r_max[sID][j] =
                    (AWB_GAIN_BASE * AWB_GAIN_SMALL_BASE) /
    ...
    for (j = 0; j < AWB_LS_NUM; j++) {
        printf("Extra %d st:%d,Rm:%d RM:%d Bm:%d BM:%d\n", j,
               stAwbAttrInfoEx[sID].stLightInfo[j].u8LightStatus, sExLight.ext_ls_r_min[sID][j],
               sExLight.ext_ls_r_max[sID][j], sExLight.ext_ls_b_min[sID][j],
               sExLight.ext_ls_b_max[sID][j]);
        printf("%d %d %d %d\n", stAwbAttrInfoEx[sID].stLightInfo[j].u16WhiteRgain,
               stAwbAttrInfoEx[sID].stLightInfo[j].u16WhiteBgain,
...
static void AWB_AccWB_Data(CVI_U8 sID, sWBSampleInfo *pInfo, sPOINT_WB_INFO *pPtInfo,
                           CVI_U16 posWgt)
{
...
#define MOVE_DUSK_TEMP_THRES (300)
    ...
    if (stAwbAttrInfo[sID].stAuto.stLumaHist.bEnable) {
        ...
        if (histInx > (HISTO_WEIGHT_SIZE - 1)) histInx = (HISTO_WEIGHT_SIZE - 1);
    ...
    if (binInx < AWB_BIN_ZONE_NUM) {
        ...
        pInfo->u64CT_BinPixelSum[binInx][AWB_R] += (RGain * u32binWgt);
        pInfo->u64CT_BinPixelSum[binInx][AWB_G] += (GGain * u32binWgt);
        pInfo->u64CT_BinPixelSum[binInx][AWB_B] += (BGain * u32binWgt);
        ...
        if (stAwbAttrInfoEx[sID].stInOrOut.bEnable) {
            ...
            pInfo->u64OutDoorBinPixelSum[binInx][AWB_R] += (RGain * u32binWgt);
            pInfo->u64OutDoorBinPixelSum[binInx][AWB_G] += (GGain * u32binWgt);
            pInfo->u64OutDoorBinPixelSum[binInx][AWB_B] += (BGain * u32binWgt);
            pInfo->u64OutDoorBinGrayCnt[binInx] += u32binWgt;
        }
    }
}

static void AWB_GetGrayPointZoneInfo(CVI_U8 radius, const sWBSampleInfo *pInfo,
                                     const CVI_U64 *pAccSum, stZone_Info *pZone_Info)
{
    ...
    DBGMSG("MaxI:%d %ld %ld %d-%d Rgain:%ld Bgain:%ld\n", maxSumInx * 32, maxSum, real_MaxZoneCnt,
           maxSumInx, endR, pZone_Info->u64MaxZoneRgain, pZone_Info->u64MaxZoneBgain);
    ...
    DBGMSG("subMaxI:%d %ld %ld %d-%d Rgain:%ld Bgain:%ld\n", subMaxSumInx * 32, subMaxSum,
           real_SubZoneCnt, subMaxSumInx, endR, pZone_Info->u64SubZoneRgain,
           pZone_Info->u64SubZoneBgain);
}
...
#define DOUBLECOLORTMP_RATIO (0.2)
    ...
    DBGMSG("diff R:%ld diff B: %ld \n", AWB_ABS((CVI_S64)(FstZoneRgain - SecZoneRgain)),
           AWB_ABS((CVI_S64)(FstZoneBgain - SecZoneBgain)));
    if (bIsDoubleColTmp) {
        DBGMSG("mz_rgain =%d mz_rgain1 =%d \n", FstZoneRgain * baseNum - radius * AWB_BIN_STEP,
               FstZoneRgain * baseNum + radius * AWB_BIN_STEP);
        DBGMSG("mz_bgain =%d mz_bgain1 =%d \n", FstZoneBgain * baseNum - radius * AWB_BIN_STEP,
               FstZoneBgain * baseNum + radius * AWB_BIN_STEP);
        ...
        for (y = 0; y < WBInfo[sID].u16WBRowSize; ++y) {
            for (x = 0; x < WBInfo[sID].u16WBColumnSize; ++x) {
                ...
                if ((pstPointInfo[idx].u16RGain >
                         (FstZoneRgain * baseNum - radius * AWB_BIN_STEP) &&
                     pstPointInfo[idx].u16RGain <
                         (FstZoneRgain * baseNum + radius * AWB_BIN_STEP)) &&
                    (pstPointInfo[idx].u16BGain >
                         (FstZoneBgain * baseNum - radius * AWB_BIN_STEP) &&
                     pstPointInfo[idx].u16BGain <
                         (FstZoneBgain * baseNum + radius * AWB_BIN_STEP))) {
                    fstBlockCnt++;
...
#define SKIN_FALLING_RATIO      (2)
    ...
    DBGMSG("%ld %d %d\n", AWB_ABS((CVI_S64)(MaxZoneRgain - SubZoneRgain)),
           u16SkinRGain_Diff - SKIN_SHIFT, u16SkinRGain_Diff + SKIN_SHIFT);
    DBGMSG("%ld %d %d\n", AWB_ABS((CVI_S64)(MaxZoneBgain - SubZoneBgain)),
           u16SkinBGain_Diff - SKIN_SHIFT, u16SkinBGain_Diff + SKIN_SHIFT);
    if (bIsDoubleColTmp) {
...
#define SAME_CT_ZONE_SIZE (16)
    ...
    if (sum1st) {
        ...
        if (ratio2nd >= MIN_2ND_RATIO) {
            ratio2nd =
                (ratio2nd - MIN_2ND_RATIO) * MAX_MIX_12_RATIO / (MAX_2ND_RATIO - MIN_2ND_RATIO);
            DBGMSG("r:%d\n", ratio2nd);
...
static CVI_S32 _Interpulate(CVI_S32 s32Mid, CVI_S32 s32Left, CVI_S32 s32LValue, CVI_S32 s32Right,
                            CVI_S32 s32RValue)
{
    CVI_S32 s32Value = 0;
    CVI_S32 k        = 0;
    ...
    if (k == 0) {
        ...
        s32Value =
            (((s32Right - s32Mid) * s32LValue + (s32Mid - s32Left) * s32RValue + (k >> 1)) / k);
    }
...
static void AWB_CalcOutdoorBinAWB(CVI_U8 sID, sWBSampleInfo *pInfo, sWBGain *pGain)
{
    ...
    if (!Is_Awb_AdvMode)  // AWB_ALG_LOWCOST
        return;
    if (stAwbAttrInfoEx[sID].stInOrOut.bEnable == 0) return;
    PRINT_FUNC();
...
static void AWB_BinOutdoorWeightInit(CVI_U8 sID)
{
    ...
    if (u16LowStop >= u16LowStart) err = 1;
    if (u16LowStart >= u16HighStart) err = 1;
    if (u16HighStart >= u16HighStop) err = 1;
    if (err) {
...
static void AWB_MixOutdoorAWB(CVI_U8 sID, sWBGain *pBinGain, sWBGain *pOutDoorGain)
{
    ...
    if (!Is_Awb_AdvMode)  // AWB_ALG_LOWCOST
        return;
    if (stAwbAttrInfoEx[sID].stInOrOut.bEnable == 0) return;
    if (pBinGain->u16RGain == 0) return;
    if (pOutDoorGain->u16RGain == 0) return;
    PRINT_FUNC();
    ...
    rgain = (outdoorRatio[sID] * pOutDoorGain->u16RGain +
             (256 - outdoorRatio[sID]) * pBinGain->u16RGain) /
            256;
    bgain = (outdoorRatio[sID] * pOutDoorGain->u16BGain +
             (256 - outdoorRatio[sID]) * pBinGain->u16BGain) /
            256;
...
static void AWB_CalcMixColorTemp(CVI_U8 sID, sWBGain *pBinGain, sWBCurveInfo *pCurInfo,
                                 sWBSampleInfo *pInfo)
{
...
#define H_L_TMP_LC_THRES   (600)
    ...
    if (!Is_Awb_AdvMode)  // AWB_ALG_LOWCOST
        return;
    if (!ttlCnt) return;
    if (WBInfo[sID].stEnvInfo.s16LvX100 > H_L_TMP_LC_THRES) return;
    PRINT_FUNC();
    ...
    if (highCnt) {
        ...
        AWB_GetWhiteBGainRange(sID, highRgain, &highBgain, &wbCurveTopRange, &wbCurveBotRange,
                               &wbWideTopRange, &wbWideBotRange, pCurInfo);
        if (lowCnt) {
...
static void AWB_HistWeightInit(CVI_U8 sID)
{
    ...
    if (!stAwbAttrInfo[sID].stAuto.stLumaHist.bEnable) return;
    ...
    if (stAwbAttrInfo[sID].stAuto.stLumaHist.enOpType == OP_TYPE_MANUAL) {
        ...
        if (stCurHist.au8HistThresh[0] != 0) err++;
        if (stCurHist.au8HistThresh[AWB_LUM_HIST_NUM - 1] != 0xFF) err++;
        if (err) stAwbAttrInfo[sID].stAuto.stLumaHist.enOpType = OP_TYPE_AUTO;
    }
    ...
    for (i = 0; i < 256; i++) {
        ...
        if ((stCurHist.au8HistThresh[curInx] - stCurHist.au8HistThresh[curInx - 1])) {
            if (preWt < curWt) {
                u16HistWt[sID][i] =
                    preWt +
                    (i - stCurHist.au8HistThresh[curInx - 1]) * (curWt - preWt) /
                        (stCurHist.au8HistThresh[curInx] - stCurHist.au8HistThresh[curInx - 1]);
            } else {
                u16HistWt[sID][i] =
                    preWt -
                    (i - stCurHist.au8HistThresh[curInx - 1]) * (preWt - curWt) /
                        (stCurHist.au8HistThresh[curInx] - stCurHist.au8HistThresh[curInx - 1]);
            }
        }
        // DBGMSG("%d\t%d\n", i, u16HistWt[sID][i]);
...
#define SECONED_CT_AVAILABLE_CNT 25
    ...
    CVI_U16   tmpRValue[WB_WDR_FRAME_MAX], tmpGValue[WB_WDR_FRAME_MAX], tmpBValue[WB_WDR_FRAME_MAX];
    ...
    DBGMSG("seconed color temperature cnt =%d rGain = %ld BGain =%ld  diffIdx = %d\n", secCTCnt,
           RGainValSum / secCTCnt, BGainValSum / secCTCnt, diffIdx);
...
#define CT_VAR_THRES (20)
    ...
    CVI_U16 CTRatio =
        AWB_MIN(real_SubZoneCnt, real_MaxZoneCnt) * 100 / AWB_MAX(real_SubZoneCnt, real_MaxZoneCnt);
...
#define STRICT_EXTRA_ENABLE_RATIO (0.4)
    ...
    CVI_U16 tmpPixelR[WB_WDR_FRAME_MAX], tmpPixelG[WB_WDR_FRAME_MAX], tmpPixelB[WB_WDR_FRAME_MAX];
    ...
    if (pstTmpSampeInfo == NULL) return;
    G_Min = 35;
    G_Max = 800;
    ...
    if (WBInfo[sID].stEnvInfo.s16LvX100 < ADJ_MIN_G_LV_THRES) {
        ...
        if (min_sub > 15) min_sub = 15;
        G_Min -= min_sub;

        if (G_Min < 20) G_Min = 20;
        ...
        for (y = 0; y < WBInfo[sID].u16WBRowSize; ++y) {
            for (x = 0; x < WBInfo[sID].u16WBColumnSize; ++x) {
                ...
                if (pixelG < G_Max) {
                    if (pixelG > 5) G5_1023++;
                    if (pixelG > 10) G10_1023++;
                    if (pixelG > 15) G15_1023++;
                    if (pixelG > 20) G20_1023++;
                }
            }
        }
        DBGMSG("G5_1023:%d G10_1023:%d G15_1023:%d G20_1023:%d\n", G5_1023, G10_1023, G15_1023,
               G20_1023);
    ...
    memset(pstPointInfo, 0,
           WBInfo[sID].u16WBRowSize * WBInfo[sID].u16WBColumnSize * sizeof(sPOINT_WB_INFO));
    ...
    if (u8AwbDbgBinFlag[sID] == AWB_DBG_BIN_FLOW_UPDATE) {
        if (pAwbDbg) {
            ...
            ii = 0;
            for (RGain = stTmpBoundary.u16RLowBound; RGain <= stTmpBoundary.u16RHighBound;
                 RGain += 16) {
                AWB_GetWhiteBGainRange(sID, RGain, &wbCurveBGain, &wbCurveTopRange,
                                       &wbCurveBotRange, &wbWideTopRange, &wbWideBotRange,
                                       &stTmpCurveInfo);
        ...
        for (x = 0; x < WBInfo[sID].u16WBColumnSize; ++x) {
            ...
            if (pixelR > 0 && pixelB > 0 && (pixelG > G_Min && pixelG < G_Max)) {
                ...
                if (RGain >= stTmpBoundary.u16RLowBound && RGain <= stTmpBoundary.u16RHighBound) {
                    ...
                    if ((sPtInfo.enPointType == AWB_POINT_EXTRA_STRICT) &&
                        (bEnableStrictExtra[sID] == 0) && (sPtInfo.fStrictExtraRatio == 0.0)) {
                        pixelInCurve = 0;
                    }
            ...
            if (pixelInCurve) {
                ...
                if (pixelInCurve) {
                    ...
                    if (useSE_LEdata == WB_WDR_SE) AWB_AccWB_DataSE(sID, pstTmpSampeInfo, &sPtInfo);
                }
            }
            if (pxlInWide) {
    ...
    if (stSampleInfo[sID].u16TotalSampleCnt) {
        u32grayR100 =
            stSampleInfo[sID].u16GrayCnt * 100 * 100 / stSampleInfo[sID].u16TotalSampleCnt;
        u32wild_grayR100 =
            stSampleInfo[sID].u32WildCnt * 100 * 100 / stSampleInfo[sID].u16TotalSampleCnt;
    } else {
        u32grayR100      = 0;
        u32wild_grayR100 = 0;
    }
    ...
    if (stSampleInfo[sID].u32GrayWeightCnt) {  // Low Cost AWB
        lowCostAwbGain.u16RGain =
            (stSampleInfo[sID].u64TotalPixelSum[1][AWB_R] / stSampleInfo[sID].u32GrayWeightCnt);
        lowCostAwbGain.u16GGain =
            (stSampleInfo[sID].u64TotalPixelSum[1][AWB_G] / stSampleInfo[sID].u32GrayWeightCnt);
        lowCostAwbGain.u16BGain =
            (stSampleInfo[sID].u64TotalPixelSum[1][AWB_B] / stSampleInfo[sID].u32GrayWeightCnt);
    } else {
    ...
    if (!stSampleInfo[sID].bUsePreBalanceGain) {  // 3%
        ...
        if (finalGain.u16GGain) {  // mix Wild WB
            ...
            finalGain.u16RGain =
                ((100 - mixWild) * finalGain.u16RGain + mixWild * wildGain.u16RGain) / 100;
            finalGain.u16BGain =
                ((100 - mixWild) * finalGain.u16BGain + mixWild * wildGain.u16BGain) / 100;
            DBGMSG("%d %d\n", finalGain.u16RGain, finalGain.u16BGain);
            WBInfo[sID].stBalanceWB = finalGain;
            WBInfo[sID].bStable     = CVI_TRUE;
        } else {  // use wild
    ...
    if (stSampleInfo[sID].u16EffectiveSampleCnt) {
        WBInfo[sID].stGrayWorldWB.u16RGain =
            stSampleInfo[sID].u64TotalPixelSum[0][AWB_R] / stSampleInfo[sID].u16EffectiveSampleCnt;
        WBInfo[sID].stGrayWorldWB.u16GGain = AWB_GAIN_BASE;
        WBInfo[sID].stGrayWorldWB.u16BGain =
            stSampleInfo[sID].u64TotalPixelSum[0][AWB_B] / stSampleInfo[sID].u16EffectiveSampleCnt;
    }

    WBInfo[sID].stFinalWB =
        (WBInfo[sID].bUseGrayWorld) ? WBInfo[sID].stGrayWorldWB : WBInfo[sID].stBalanceWB;
...
#define AWB_MIX_DIFF      (128)
    ...
    if (stWbGain.u16RGain > AWB_MAX_GAIN) stWbGain.u16RGain = AWB_MAX_GAIN;

    if (stWbGain.u16BGain > AWB_MAX_GAIN) stWbGain.u16BGain = AWB_MAX_GAIN;
...
static void AWB_LowColorTmMixDaylight(CVI_U8 sID, sWBGain *wbGain)
{
    ...
    if (stAwbAttrInfo[sID].stAuto.bNaturalCastEn && curDiff) {
        tmpWBGain.u16RGain =
            (tmpWBGain.u16RGain * RRatio + WBInfo[sID].stDefaultWB.u16RGain * (100 - RRatio)) / 100;
        tmpWBGain.u16BGain =
            (tmpWBGain.u16BGain * BRatio + WBInfo[sID].stDefaultWB.u16BGain * (100 - BRatio)) / 100;
        *wbGain = tmpWBGain;
    }
}
...
static void AWB_ParamInit(CVI_U8 sID, const ISP_AWB_PARAM_S *pstAwbParam)
{
    ...
    // param init
    WBInfo[sID].stBalanceWB = WBInfo[sID].stCurrentWB = WBInfo[sID].stGrayWorldWB =
        WBInfo[sID].stDefaultWB;
...
static CVI_U16 AWB_GetGainByColorTemperature(CVI_U8 sID, CVI_U16 u16CT)
{
    ...
    if (u16CT < minCT) {  // extrapolation
        ...
        fct = u16CT;
        RGain =
            fct * fct * fCTtoRgCurveP[sID][0] + fct * fCTtoRgCurveP[sID][1] + fCTtoRgCurveP[sID][2];
    }
...
CVI_U16 AWB_GetRBGainByColorTemperature(CVI_U8 sID, CVI_U16 u16CT, CVI_S16 s16Shift,
                                        CVI_U16 *pu16AWBGain)
{
    CVI_U16   minRGain, maxRGain, midRGain;
    CVI_U16   minCT, maxCT, midCT;
...
void AWB_SetAttr(CVI_U8 sID, const ISP_WB_ATTR_S *pstWBAttr)
{
    ...
    *pstAwbMpiAttr[sID] = *pstWBAttr;
    ...
    if (pstAwbMpiAttr[sID]->u8DebugMode == 77) AWB_ShowInfoList();
...
static void AwbAttr_Init(CVI_U8 sID)
{
    ...
    pstAwbMpiAttr[sID]->stAuto.u16ShiftLimit[2] = 240;   // high top
...
#ifndef AAA_PC_PLATFORM
static void AWB_SetWBGainToResult(CVI_U8 sID, ISP_AWB_RESULT_S *pstAwbResult,
                                  const sWBGain *pstWBGain)
{
...
#define THRESHOLD (30)
    ...
    if (AWB_GetDebugMode() == 3)
        printf("sID:%d R:%d G:%d B:%d CT:%d\n", sID, wbGain[sID].u16RGain, wbGain[sID].u16GGain,
               wbGain[sID].u16BGain, colorTemp[sID]);
}
...
CVI_S32 awbRun(VI_PIPE ViPipe, const ISP_AWB_INFO_S *pstAwbInfo, ISP_AWB_RESULT_S *pstAwbResult,
               CVI_S32 s32Rsv)
{
    CVI_U8                sID;
    CVI_U32               periodT[AWB_SENSOR_NUM], processT[AWB_SENSOR_NUM];
    static struct timeval firstAwbTime[AWB_SENSOR_NUM], curAwbTime[AWB_SENSOR_NUM],
        preAwbTime[AWB_SENSOR_NUM], proAwbTime[AWB_SENSOR_NUM];
    ...
    MEAUSURE_AWB_T(1);
    if (stAwbAttrInfo[sID].stAuto.bEnable) AWB_RunAlgo(sID);
    MEAUSURE_AWB_T(2);
    ...
    if (AWB_GetDebugMode() == 1) {
        ...
        printf("AWB CTv:%d %d PTv:%d %d\n", (CVI_S32)curAwbTime[sID].tv_sec,
               (CVI_S32)curAwbTime[sID].tv_usec, (CVI_S32)preAwbTime[sID].tv_sec,
               (CVI_S32)preAwbTime[sID].tv_usec);
        printf("PeriodT:%u ProT:%u\n", periodT[sID], processT[sID]);
        printf("WB Gain:%d %d\n", WBInfo[sID].stCurrentWB.u16RGain,
               WBInfo[sID].stCurrentWB.u16BGain);
        debugFrmCnt[sID]++;
...
static CVI_S32 AWB_DumpLog2Buff(CVI_U8 sID)
{
    ...
    for (i = 0; i < AWB_SENSOR_NUM; i++) {
        if (maxInterval < stAwbAttrInfo[i].u8AWBRunInterval)
...
CVI_S32 AWB_DumpLog(CVI_U8 sID)
{
    ...
    if (AWB_LogTailIndex && pAWB_LogBuf != CVI_NULL) {
        ...
        if (strlen(awbDumpLogName)) {
            snprintf(fileName, MAX_LOG_FILENAME_LENGTH, "%s/%s_AwbLog.txt", awbDumpLogPath,
                     awbDumpLogName);
        } else {
            snprintf(fileName, MAX_LOG_FILENAME_LENGTH, "%s/AwbLog%d.txt", awbDumpLogPath,
                     fileCnt++);
        }
        fp = fopen(fileName, "w");
        if (fp == CVI_NULL) {
...
static void AWB_SaveLog(CVI_U8 sID)
{
    ...
    for (k = 0; k < WBInfo[sID].u8AwbMaxFrameNum; ++k) {
        for (i = 0; i < AWB_CHANNEL_NUM; ++i) {
            for (j = 0; j < WBInfo[sID].u16WBRowSize * WBInfo[sID].u16WBColumnSize; ++j) {
                ...
                if ((j + 1) % WBInfo[sID].u16WBColumnSize == 0) AWB_LogPrintf("\n");
            }
            AWB_LogPrintf("\n\n");
        }
    }
    ...
    AWB_LogPrintf("AWB %d %d\n", WBInfo[sID].stBalanceWB.u16RGain,
                  WBInfo[sID].stBalanceWB.u16BGain);
    ...
    AWB_LogPrintf("LV:%d I:%d\n\n", WBInfo[sID].stEnvInfo.s16LvX100,
                  WBInfo[sID].stEnvInfo.u32ISONum);
    AWB_LogPrintf("CAL\n");
    for (i = 0; i < AWB_CALIB_PTS_NUM; ++i) {
        AWB_LogPrintf("%d %d %d\n", WBInfo[sID].stCalibWBGain[i].u16RGain,
                      WBInfo[sID].stCalibWBGain[i].u16BGain, WBInfo[sID].u16CalibColorTemp[i]);
    }
    AWB_LogPrintf("RHL:%d %d\n", stBoundaryInfo[sID].u16RLowBound,
                  stBoundaryInfo[sID].u16RHighBound);
    AWB_LogPrintf("BHL:%d %d\n", stBoundaryInfo[sID].u16BLowBound,
                  stBoundaryInfo[sID].u16BHighBound);
    AWB_LogPrintf("isGW:%d\n", WBInfo[sID].bUseGrayWorld);
    AWB_LogPrintf("Sft\n");
    AWB_LogPrintf("%d %d\n", stCurveInfo[sID].u16LowTempTopRange,
                  stCurveInfo[sID].u16LowTempBotRange);
    AWB_LogPrintf("%d %d\n", stCurveInfo[sID].u16MidTempTopRange,
                  stCurveInfo[sID].u16MidTempBotRange);
    AWB_LogPrintf("%d %d\n", stCurveInfo[sID].u16CurveWhiteTopRange,
                  stCurveInfo[sID].u16CurveWhiteBotRange);
    AWB_LogPrintf("%d %d\n", stCurveInfo[sID].u16HighTempTopRange,
                  stCurveInfo[sID].u16HighTempBotRange);
    ...
    AWB_LogPrintf("aEn:%d spd:%d\n", stAwbAttrInfo[sID].stAuto.bEnable,
                  stAwbAttrInfo[sID].stAuto.u16Speed);
...
static CVI_S32 GetParaBuffer(CVI_FLOAT *Para, const CVI_FLOAT *X, const CVI_FLOAT *Y,
                             CVI_S32 Amount, CVI_S32 SizeSrc)
{
    CVI_S32 i, j;
...
static CVI_S32 CalCurvePara(const CVI_FLOAT *BufferX, const CVI_FLOAT *BufferY, CVI_S32 Amount,
                            CVI_S32 SizeSrc, CVI_FLOAT *ParaResK)
{
    CVI_FLOAT *ParaK = (CVI_FLOAT *)AWB_Malloc(SizeSrc * (SizeSrc + 1) * sizeof(CVI_FLOAT));

    if (ParaK == NULL) return CVI_FAILURE;
...
#define DETECTION_MODE 110
    ...
    if ((AWB_GetDebugMode() == DETECTION_MODE) && !WBInfo[sID].bStable &&
        (lCurTime - s_lLastTime > 5 * 60)) {
