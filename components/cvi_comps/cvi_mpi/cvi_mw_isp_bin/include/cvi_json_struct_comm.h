...
#define JSON_A(r_w_flag, type, value, length) \
    JSON_A_(r_w_flag, obj, type, #value, data->value, (length))
...
#define JSON_END(r_w_flag)                                                    \
    {                                                                         \
        if (r_w_flag == R_FLAG) {                                             \
            ...
            if (strstr(key, "ISP_MESH_SHADING_GAIN_LUT_ATTR_S") ||            \
                strstr(key, "ISP_CLUT_ATTR_S")) {                             \
...
#define PARSE_JSON_STR_TO_INT_VALUE(key, json_str, data) \
    do {                                                 \
        unsigned int tmpVal;                             \
...
#define PARSE_JSON_STR_TO_INT_ARRAY(key, json_str, data, size)             \
    do {                                                                   \
        unsigned int tmpVal;                                               \
...
void JSON_CHECK_RANGE_OF_U64(char *name, unsigned long long *value, unsigned long long min,
