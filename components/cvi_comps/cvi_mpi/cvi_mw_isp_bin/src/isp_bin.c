...
#define CVI_TRACE_ISP_BIN(level, fmt, ...)                                             \
    CVI_TRACE(level, CVI_ID_ISP, "%s:%d:%s(): " fmt, __FILENAME__, __LINE__, __func__, \
              ##__VA_ARGS__)
#if CONFIG_PQBIN_USE_JSON
#define CVI_SET_PARAM_TO_JSON(func, key, param, case)                                              \
    do {                                                                                           \
        ...
        if (json_object) {                                                                         \
            ...
            if (json_buf[idx] == NULL) {                                                           \
                ...
                CVI_TRACE_ISP_BIN(CVI_DBG_WARN, "%s\n", "Allocate memory fail");                   \
            ...
            CVI_TRACE_ISP_BIN(CVI_DBG_WARN, "(id:%d %s isp_raw_parameter )Get New Object fail.\n", \
                              id, key);                                                            \
            ret = CVI_BIN_JSONHANLE_ERROR;                                                         \
        }                                                                                          \
    } while (0);

#define CVI_GET_PARAM_FROM_JSON(func, key, param, case)              \
    do {                                                             \
...
CVI_U8 en_pqbin_single_mode = 0;  // 0:disable 1:enable
...
CVI_S32 isp_bin_dis_single_mode(void)
{
    ...
    en_pqbin_single_mode = 0;

    return ret;
}
CVI_S32 isp_bin_is_single_mode(void)
{
    CVI_S32 ret = en_pqbin_single_mode;
    return ret;
}
...
#endif
#ifndef ISP_BIN_MD5
#define ISP_BIN_MD5 "721fce427a703820ccc00b6a5cdf4685"
#endif
    size_t len = 0;
...
static CVI_S32 isp_bin_getBinParamImp(VI_PIPE ViPipe, CVI_U8 *addr, CVI_U32 binSize)
{
    ...
    if (isp_bin_checkMd5(ViPipe, addr, binSize) != CVI_SUCCESS) {
        CVI_TRACE_ISP_BIN(CVI_DBG_WARN, "section[CVI_BIN_ID_ISP%d] Md5 not matched\n", ViPipe);
        return CVI_FAILURE;
    }
    for (CVI_U32 idx = CVI_BIN_ID_MIN; idx < CVI_BIN_ID_ISP0 + ViPipe; idx++) {
    ...
    CVI_TRACE_ISP_BIN(CVI_DBG_DEBUG, "apply bin setting to %d\n", ViPipe);
...
static CVI_S32 isp_bin_setBinParamImp(VI_PIPE ViPipe, FILE *fp)
{
    ...
    if (pstParam == NULL) {
        CVI_TRACE_ISP_BIN(CVI_DBG_ERR, "%s\n", "Allocate memory fail");
...
static CVI_S32 isp_bin_checkMd5(VI_PIPE ViPipe, CVI_U8 *addr, CVI_U32 binSize)
{
    ...
    if (strcmp(binMd5, curMd5)) {
        CVI_TRACE_ISP_BIN(CVI_DBG_ERR, "%s%s%s",
        ...
        CVI_TRACE_ISP_BIN(CVI_DBG_ERR, "please update to version (branch:%s commitId:%s)\n",
                          binGerrit, binCommitId);
        ret = CVI_FAILURE;
    } else {
        isp_bin_getBinSizeImp(ViPipe, &size);
        if (size != binSize) {
            CVI_TRACE_ISP_BIN(CVI_DBG_ERR, "vi(%d), Bin Size not matched(%u vs %u),", ViPipe, size,
                              binSize);
            CVI_TRACE_ISP_BIN(
                CVI_DBG_ERR, "%s%s",
...
static CVI_S32 isp_bin_checkBinVersion(CVI_U8 *addr, CVI_U32 binSize)
{
    ...
    pIspParmDesc = (CVI_CHAR *)calloc(1, 2048);
    ...
    // printf("\n");
    if (pIspParmDesc) {
        CVI_CHAR tmpChar[256] = {0};

        sprintf(pIspParmDesc, "cvi_bin_isp message\n");
        // printf("%-15s%-15s%-15s%-15s\n", "gerritId:", binGerrit, "commitId:", binCommitId);
        snprintf(tmpChar, sizeof(tmpChar), "%-15s%-15s\n", "md5:", binMd5);
        strcat(pIspParmDesc, tmpChar);
        snprintf(tmpChar, sizeof(tmpChar), "%-15s%-15d\n", "sensorNum", sensorInfo.num);
        strcat(pIspParmDesc, tmpChar);
        for (CVI_U8 i = 0; i < sensorInfo.num; ++i) {
            snprintf(tmpChar, sizeof(tmpChar), "%s%-5d%-15s\n", "sensorName", i,
                     sensorInfo.name[i]);
            strcat(pIspParmDesc, tmpChar);
        }
        strcat(pIspParmDesc, "\n");
    ...
    //	"createTime:", pExtraInfo->Time, "pqbin version:", pqbinVersion,
    ...
    if (pIspParmDesc) {
        ...
        strcat(pIspParmDesc, "PQBIN message\n");
        snprintf(tmpChar, sizeof(tmpChar), "%-15s%-15s\n", "md5:", binMd5);
        strcat(pIspParmDesc, tmpChar);
        snprintf(tmpChar, sizeof(tmpChar), "%-15s%-15s\n", "sensorNum", sensorNum);
        strcat(pIspParmDesc, tmpChar);
        ...
        strcat(pIspParmDesc, "\n");
        ptmp = pIspParmDesc + strlen(pIspParmDesc);
        sprintf(ptmp, "%-15s%-15s%-15s%-15s\n", "author:", pExtraInfo->Author,
                "desc:", pExtraInfo->Desc);
        snprintf(tmpChar, sizeof(tmpChar), "%-15s%-30s%-15s%-15s\n%-15s%-30s%-6s%-5s\n",
                 "createTime:", pExtraInfo->Time, "pqbin version:", pqbinVersion,
                 "tool Version:", toolVersion, "mode:", pqbinMode);
        strcat(pIspParmDesc, tmpChar);
        setenv("ISPPQPARAM", pIspParmDesc, 1);
        free(pIspParmDesc);
        pIspParmDesc = NULL;
    }
    // check sensor
...
static CVI_S32 isp_set_paramstruct(VI_PIPE ViPipe, ISP_Parameter_Structures *pstParaBuf)
{
    if (((ViPipe) < 0) || ((ViPipe) >= SUPPORT_VI_MAX_PIPE_NUM)) {
        CVI_TRACE_ISP_BIN(CVI_DBG_ERR, "ViPipe %d value error\n", ViPipe);
        return CVI_FAILURE;
    }
...
CVI_S32 isp_json_getParamFromJsonbuffer(const char *buffer, enum CVI_BIN_SECTION_ID id)
{
    ...
    for (i = 0; i < JSON_3A_PARAM_NUM; i++) {
        CVI_GET_PARAM_FROM_JSON(ISP_3A_PARAMETER_BUFFER_JSON, "isp_3a_parameter", pisp_3a_parameter,
                                i + 1);
    }
...
CVI_S32 isp_json_setParamToJsonbuffer(CVI_S8 **buffer, enum CVI_BIN_SECTION_ID id, CVI_S32 *len)
{
    ...
    for (i = 0; i < JSON_3A_PARAM_NUM; i++) {
        CVI_SET_PARAM_TO_JSON(ISP_3A_PARAMETER_BUFFER_JSON, "isp_3a_parameter", pisp_3a_parameter,
                              i + 1);
    }
