...
#define REG_DUMP_BUFFER_SIZE  (240 * 1024)
#define AE_SNAP_LOG_BUFF_SIZE (3 * 1124)  // from ae_buf.c
#define FILE_NAME_MAX_LENGTH  256

typedef struct _RAW_DUMP_INFO {
    CVI_CHAR acfileName[FILE_NAME_MAX_LENGTH];
    CVI_U32  u32TotalFrameCnt;
    RECT_S   stRoiRect;
    bool     bDump2Remote;
    CVI_U8  *pu8ExpInfoBuffer;
    CVI_U32  u32ExpInfoSize;
    CVI_U8  *pu8AELogBuffer;
    CVI_U32  u32AELogBufferSize;
    CVI_U8  *pu8AWBLogBuffer;
    CVI_U8  *pu8RawInfo;
    CVI_U8  *pu8RegDump;
    CVI_U8  *pu8AWBbin;
    CVI_U32  u32AWBbinSize;
} RAW_DUMP_INFO_S;
...
#define FIFO_INIT(head, _capacity)                                 \
    do {                                                           \
...
#define FIFO_EXIT(head)                       \
    do {                                      \
        (head)->front = (head)->tail = -1;    \
        (head)->capacity             = 0;     \
        if ((head)->fifo) free((head)->fifo); \
    } while (0)

#define FIFO_EMPTY(head) ((head)->front == -1)

#define FIFO_FULL(head)                       \
    (((head)->front == ((head)->tail + 1)) || \
     (((head)->front == 0) && ((head)->tail == ((head)->capacity - 1))))

#define FIFO_CAPACITY(head) ((head)->capacity)

#define FIFO_SIZE(head) \
    (FIFO_EMPTY(head)   \
         ? 0            \
         : ((((head)->tail + (head)->capacity - (head)->front) % (head)->capacity) + 1))

#define FIFO_PUSH(head, elm)                                                              \
    do {                                                                                  \
        if (FIFO_EMPTY(head))                                                             \
            (head)->front = (head)->tail = 0;                                             \
        else                                                                              \
            (head)->tail = ((head)->tail == (head)->capacity - 1) ? 0 : (head)->tail + 1; \
        (head)->fifo[(head)->tail] = elm;                                                 \
    } while (0)

#define FIFO_POP(head, pelm)                                                                 \
    do {                                                                                     \
...
#define FIFO_FOREACH(var, head, idx)                                                          \
    for (idx = (head)->front, var = (head)->fifo[idx]; idx < (head)->front + FIFO_SIZE(head); \
