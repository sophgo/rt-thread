...
typedef struct {
    ...
    //	SAMPLE_VI_CONFIG_S stViConfig;TODO Evan
    //	SAMPLE_INI_CFG_S stIniCfg;TODO Evan
...
static void  raw_replay_use_TDMA(CVI_BOOL bUseCurrentParam, ISP_MWB_ATTR_S *pstMwbAttr,
                                 ISP_EXP_INFO_S *pstExpInfo);
static void  raw_replay_use_vb(CVI_BOOL bUseCurrentParam, ISP_MWB_ATTR_S *pstMwbAttr,
...
static CVI_U8  get_rgbmap_block_wBit(CVI_U32 iso);
...
CVI_S32 set_raw_replay_data(const CVI_VOID *header, const CVI_VOID *data, CVI_U32 totalFrame,
                            CVI_U32 curFrame, CVI_U32 rawFrameSize)
{
    ...
    if (curFrame == 0) {
        if (pRawReplayCtx == NULL) {
            ...
            if (pRawReplayCtx->bRawReplayThreadEnabled) {
                ...
                free_rgbmapbuf();
#endif
                s32Ret = CVI_SYS_Munmap(pRawReplayCtx->pu8VirAddr, pRawReplayCtx->u32BlkSize);
        ...
        LOGOUT("totalFrame: %d, rawFrameSize: %d, %d,%d, roiFrameSize: %d, %d,%d,%d,%d,%d OP:%d\n",
               totalFrame, rawFrameSize, pRawInfo->width, pRawInfo->height, pRawInfo->roiFrameSize,
               pRawInfo->roiFrameNum, pRawInfo->stRoiRect.s32X, pRawInfo->stRoiRect.s32Y,
               pRawInfo->stRoiRect.u32Width, pRawInfo->stRoiRect.u32Height, pRawInfo->op_mode);
        ...
        if (pRawReplayCtx->pRawHeader == NULL) {
            pRawReplayCtx->pRawHeader =
                (RAW_REPLAY_INFO *)calloc(totalFrame, sizeof(RAW_REPLAY_INFO));
        }
        ...
        if (pRawReplayCtx->PoolID == 0) {
            ...
            pRawReplayCtx->pu8VirAddr =
                (CVI_U8 *)CVI_SYS_MmapCache(pRawReplayCtx->u64PhyAddr, cfg.u32BlkSize);

            LOGOUT("create vb pool cnt: %d, blksize: %d phyAddr: %llu\n", 1, cfg.u32BlkSize,
                   pRawReplayCtx->u64PhyAddr);
        }

        if (pRawReplayCtx->u32RoiTotalFrame != 0) {
            pRawReplayCtx->u64RoiPhyAddr[0] =
                pRawReplayCtx->u64PhyAddr + pRawReplayCtx->u32RawFrameSize;
            pRawReplayCtx->pu8RoiVirAddr[0] =
                pRawReplayCtx->pu8VirAddr + pRawReplayCtx->u32RawFrameSize;
#if defined(__CV181X__) || defined(__CV180X__)
            if (pRawInfo->enWDR) {
                pRawReplayCtx->u64RoiPhyAddr[0] =
                    pRawReplayCtx->u64PhyAddr + pRawReplayCtx->u32RawFrameSize / 2;
                pRawReplayCtx->pu8RoiVirAddr[0] =
                    pRawReplayCtx->pu8VirAddr + pRawReplayCtx->u32RawFrameSize / 2;
        ...
        if (pRawReplayCtx->u32RoiTotalFrame != 0 &&
            ...
            pRawReplayCtx->stTDMAParam[0].paddr_dst =
                pRawReplayCtx->u64PhyAddr +
            ...
            if (pRawInfo->enWDR) {
                ...
                pRawReplayCtx->stTDMAParam[0].w_bytes =
                    pRawReplayCtx->stTDMAParam[0].stride_bytes_src;
                pRawReplayCtx->stTDMAParam[0].paddr_dst =
                    pRawReplayCtx->u64PhyAddr +
                    pRawReplayCtx->stTDMAParam[0].stride_bytes_dst * pRawInfo->stRoiRect.s32Y +
                ...
                pRawReplayCtx->stTDMAParam[1].paddr_dst =
                    pRawReplayCtx->u64PhyAddr + pRawReplayCtx->u32BlkSize / 2 +
                    pRawReplayCtx->stTDMAParam[1].stride_bytes_dst * pRawInfo->stRoiRect.s32Y +
                    pRawInfo->stRoiRect.s32X / 2 * 3;
            }
#else
            if (pRawInfo->enWDR) {
            ...
            LOGOUT("stride_src: %d, stride_dst: %d, h: %d, w_bytes: %d\n",
                   pRawReplayCtx->stTDMAParam[0].stride_bytes_src,
                   pRawReplayCtx->stTDMAParam[0].stride_bytes_dst, pRawReplayCtx->stTDMAParam[0].h,
                   pRawReplayCtx->stTDMAParam[0].w_bytes);
        } else {
            pRawReplayCtx->bUseTDMA = CVI_FALSE;
        }
    ...
    if (curFrame < pRawReplayCtx->u32TotalFrame) {
        memcpy((uint8_t *)pRawReplayCtx->pRawHeader + curFrame * sizeof(RAW_REPLAY_INFO), header,
        ...
        if (pRawReplayCtx->u32RoiTotalFrame == 0) {
            if (op_mode != RAW_OP_MODE_AE_SIM || curFrame) {
                ...
                if (pRawReplayCtx->pRawHeader[0].enWDR) {
                    CVI_U8 *tmpAddrLe =
                        pRawReplayCtx->pu8VirAddr + curFrame * pRawReplayCtx->u32RawFrameSize / 2;
                    CVI_U8 *tmpAddrSe = pRawReplayCtx->pu8VirAddr + pRawReplayCtx->u32BlkSize / 2 +
                                        curFrame * pRawReplayCtx->u32RawFrameSize / 2;
                    int lineSize =
                        pRawReplayCtx->u32RawFrameSize / pRawReplayCtx->pRawHeader[0].height / 2;
...
#else
                memcpy(pRawReplayCtx->pu8VirAddr + curFrame * pRawReplayCtx->u32RawFrameSize, data,
                       pRawReplayCtx->u32RawFrameSize);
#endif
            }
        } else {
            if (curFrame == 0) {
                ...
                if (pRawReplayCtx->pRawHeader[0].enWDR) {
                    ...
                    CVI_U8 *tmpAddrSe = pRawReplayCtx->pu8RoiVirAddr[1] +
                                        (curFrame - 1) * pRawReplayCtx->u32RoiRawFrameSize / 2;
                    int lineSize =
                        pRawReplayCtx->u32RoiRawFrameSize / pRawReplayCtx->stRoiRect.u32Height / 2;
...
CVI_S32 start_raw_replay(VI_PIPE ViPipe)
{
    ...
    if (!pRawReplayCtx->bRawReplayThreadEnabled) {
        ...
        RETURN_FAILURE_IF(
            pthread_create(&pRawReplayCtx->rawReplayTid, NULL, raw_replay_thread, NULL) != 0);
...
static void update_ae_config(const ISP_EXP_INFO_S *pstExpInfo, ISP_OP_TYPE_E eType)
{
    ...
    if (eType == OP_TYPE_MANUAL) CVI_ISP_AESetRawReplayExposure(pRawReplayCtx->ViPipe, pstExpInfo);
}
...
static CVI_S32 restart_sensor(void)
{
    ...
    // Initial VI & ISP.
    // s32Ret = SAMPLE_PLAT_VI_INIT(&pRawReplayCtx->stViConfig); TODO Evan
    RETURN_FAILURE_IF(s32Ret != CVI_SUCCESS);
                                ...
                                ISP_EXP_INFO_S *pstExpInfo)
{
    ...
    while (pRawReplayCtx->bRawReplayThreadEnabled && !pRawReplayCtx->bRawReplayThreadSuspend) {
        ...
        if (pRawReplayCtx->pRawHeader[0].enWDR) {
            tempPhyAddrLe =
                pRawReplayCtx->u64RoiPhyAddr[0] + index * pRawReplayCtx->u32RoiRawFrameSize / 2;
            pRawReplayCtx->stTDMAParam[0].paddr_src = tempPhyAddrLe;
            // s32Ret = CVI_SYS_TDMACopy2D(&pRawReplayCtx->stTDMAParam[0]);//TODO Evan
            ERROR_IF(s32Ret != CVI_SUCCESS);
            tempPhyAddrSe =
                pRawReplayCtx->u64RoiPhyAddr[1] + index * pRawReplayCtx->u32RoiRawFrameSize / 2;
            pRawReplayCtx->stTDMAParam[1].paddr_src = tempPhyAddrSe;
            // s32Ret = CVI_SYS_TDMACopy2D(&pRawReplayCtx->stTDMAParam[1]);//TODO Evan
            ERROR_IF(s32Ret != CVI_SUCCESS);
        } else {
            tempPhyAddrLe =
                pRawReplayCtx->u64RoiPhyAddr[0] + index * pRawReplayCtx->u32RoiRawFrameSize;
            pRawReplayCtx->stTDMAParam[0].paddr_src = tempPhyAddrLe;
            // s32Ret = CVI_SYS_TDMACopy2D(&pRawReplayCtx->stTDMAParam[0]);//TODO Evan
            ERROR_IF(s32Ret != CVI_SUCCESS);
        }
    ...
    while (pRawReplayCtx->bRawReplayThreadEnabled && !pRawReplayCtx->bRawReplayThreadSuspend) {
        if (op_mode == RAW_OP_MODE_AE_SIM) {
            ...
            if (!pRawReplayCtx->bSuspend) {
                CVI_AE_GenNewRaw((void *)pRawReplayCtx->pu8VirAddr, (void *)tempVirAddr,
                                 pRawReplayCtx->u32RawFrameSize, pRawReplayCtx->pRawHeader[0].enWDR,
                                 pRawReplayCtx->pRawHeader[0].width,
            ...
            for (index = 1; index < (CVI_S32)pRawReplayCtx->u32TotalFrame; index++) {
                ...
                pstVideoFrame->stVFrame.u64PhyAddr[0] =
                    tempPhyAddrLe + index * pRawReplayCtx->u32RawFrameSize;
#if defined(__CV181X__) || defined(__CV180X__)
                if (mode) {
                    pstVideoFrame->stVFrame.u64PhyAddr[0] =
                        tempPhyAddrLe + index * pRawReplayCtx->u32RawFrameSize / 2;
                    pstVideoFrame->stVFrame.u64PhyAddr[1] =
                        tempPhyAddrSe + index * pRawReplayCtx->u32RawFrameSize / 2;
                }
...
static CVI_U8 get_rgbmap_block_wBit(CVI_U32 iso)
{
    CVI_U8         upper_idx = 0, lower_idx = 0;
    CVI_U8         wBit = 0;
    ...
    if (stTNRAttr.enOpType == OP_TYPE_MANUAL) {
        ...
        for (upper_idx = 0; upper_idx < ISP_AUTO_ISO_STRENGTH_NUM - 1; upper_idx++) {
            if (iso <= iso_lut[upper_idx]) break;
        }
        lower_idx = (upper_idx - 1 > 0) ? (upper_idx - 1) : 0;

        CVI_U8 wBit_l = stTNRAttr.stAuto.MtDetectUnit[lower_idx];
        CVI_U8 wBit_u = stTNRAttr.stAuto.MtDetectUnit[upper_idx];
        CVI_U8 iso_l  = iso_lut[lower_idx];
        CVI_U8 iso_u  = iso_lut[lower_idx];
        ...
        if ((iso <= iso_l) || (lower_idx == upper_idx)) {
            ...
            wBit = (wBit_l - wBit_l) * (iso - iso_l) / (iso_u - iso_l) + wBit_l;
        }
    }
...
static void get_rgbmap_info(RAW_REPLAY_CTX_S *pCtx, RAW_REPLAY_INFO *pRawInfo)
{
    ...
    // LOGOUT("Raw Index:%d, ISO:%d, rgbMap info wBit: %d, hBit: %d, gridRow: %d, gridCol: %d, size:
...
static void generate_rgbmap_by_frame(RAW_REPLAY_CTX_S *pCtx, int index)
{
    ...
    if (roiMode && index != 0) {
        if (wdrMode) {
            ...
            CVI_U8 *dst =
                fullRawBuf + pCtx->stRoiRect.s32Y * fullLineSize + pCtx->stRoiRect.s32X / 2 * 3;
        ...
        if (wdrMode) {
            ...
            CVI_U8 *tmpAddrSe =
                pCtx->pu8VirAddr + pCtx->u32BlkSize / 2 + index * pCtx->u32RawFrameSize / 2;
...
{
    int width =
        pCtx->pRawHeader[0].enWDR ? pCtx->pRawHeader[0].width / 2 : pCtx->pRawHeader[0].width;
        ...
        for (int bx = 0; bx < gridCol; bx++) {
            ...
            for (int y = 0; y < blkH; y++) {
                for (int x = 0; x < blkW; x++) {
                    ...
                    switch (eColorID) {
                    case RR:
                        iSumR += get_raw_pixel_data(raw, width, iPosX, iPosY);
                        break;
                    case BB:
                        iSumB += get_raw_pixel_data(raw, width, iPosX, iPosY);
                        break;
                    case GB:
                    case GR:
                        iSumG += get_raw_pixel_data(raw, width, iPosX, iPosY);
                        break;
                    }
                }
            }
