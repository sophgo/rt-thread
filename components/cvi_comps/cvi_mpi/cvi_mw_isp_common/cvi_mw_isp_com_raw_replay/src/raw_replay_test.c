...
CVI_S32 rawReplayAutoTestInit(const char *pathPrefix)
{
    ...
    char cmd[_BUFF_LEN + 29] = {0};

    FILE *fp = NULL;

    snprintf(path, sizeof(path), "%s/sensor_cfg.ini", pathPrefix);

    snprintf(cmd, sizeof(cmd), "cp %s /mnt/data/sensor_cfg.ini", path);
...
CVI_S32 rawReplayAutoTestSaveYuv(const char *pathPrefix)
{
    ...
    case PIXEL_FORMAT_YUV_PLANAR_420:
        ...
        if ((ret == CVI_SUCCESS) && (stVICrop.bEnable)) {
            ...
            au32PlanarSize[0] =
                stVideoFrame.stVFrame.u32Stride[0] * stVideoFrame.stVFrame.u32Height;
            au32PlanarSize[1] =
                stVideoFrame.stVFrame.u32Stride[1] * stVideoFrame.stVFrame.u32Height / 2;
            au32PlanarSize[2] =
                stVideoFrame.stVFrame.u32Stride[2] * stVideoFrame.stVFrame.u32Height / 2;
        }
        break;
    ...
    if (ret == CVI_SUCCESS) {
        ...
        for (CVI_U32 u32PlanarIdx = 0; u32PlanarIdx < u32PlanarCount; u32PlanarIdx++) {
            ...
            fwrite(stVideoFrame.stVFrame.pu8VirAddr[u32PlanarIdx], 1, au32PlanarSize[u32PlanarIdx],
                   fp);
        }
...
CVI_S32 rawReplayAutoTestStart(const char *pathPrefix)
{
    ...
    ret = set_raw_replay_data(pAutoTestCtx->pHeader, pAutoTestCtx->pData, 1, 0,
                              pAutoTestCtx->intRawSize);
...
CVI_S32 rawReplayAutoTestMd5(const char *pathPrefix)
{
    ...
    do {
        snprintf(path, sizeof(path), "%s/output.yuv", pathPrefix);

        snprintf(cmd, sizeof(cmd), "md5sum %s > /tmp/temp.md5", path);
        system(cmd);
