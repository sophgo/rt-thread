...
void isp_daemon2_init(unsigned int port)
{
    ...
    if (CVI_ISPD2_ES_CreateService(&gtObject, port) != CVI_SUCCESS) {
        int i = 0;
        for (i = 0; i < 2; i++) {
            usleep(500 * 1000);
            if (CVI_ISPD2_ES_CreateService(&gtObject, port) == CVI_SUCCESS) break;
        }
        if (i >= 2) {
            ISP_DAEMON2_DEBUG(LOG_ERR, "isp_daemon2_init failed");
            return;
        }
    }
    CVI_ISPD2_ConfigMessageHandler(&(gtObject.tHandlerInfo));
    CVI_ISPD2_Dump_Init(&gtObject);
