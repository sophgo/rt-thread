...
#if CONFIG_PQTOOL_UART_SUPPORT
#define write(s, dataptr, len) write_by_uart(s, dataptr, len)
#endif
    ...
    if (ptFrameData->pstRAWHeader != NULL) {
        ...
        if (ptFrameInfo->pau8FrameAddr[0]) {
            for (CVI_U32 u32Idx = 0; u32Idx < MAX_WDR_FUSION_FRAMES; ++u32Idx) {
                if ((ptFrameInfo->pau8FrameAddr[u32Idx] != NULL) &&
        ...
        if (ptFrameData->astVpssVideoFrame[0].stVFrame.u64PhyAddr[0] != 0) {
            s32Ret =
                CVI_VPSS_ReleaseChnFrame(VpssGrp, VpssChn, &(ptFrameData->astVpssVideoFrame[0]));
        }
#ifdef USE_TWO_YUV_FRAME
        if (ptFrameData->astVpssVideoFrame[1].stVFrame.u64PhyAddr[0] != 0) {
            s32Ret =
                CVI_VPSS_ReleaseChnFrame(VpssGrp, VpssChn, &(ptFrameData->astVpssVideoFrame[1]));
        }
        ...
        memset(ptFrameData->astVpssVideoFrame, 0,
               sizeof(VIDEO_FRAME_INFO_S) * MAX_WDR_FUSION_FRAMES);
    }
...
// -----------------------------------------------------------------------------
static CVI_S32 CVI_ISPD2_GetYUVFrameInfo(const TGetFrameInfo *pstFrameInfo,
                                         TFrameHeader        *pstFrameHeader)
{
    ...
    if (eImageSrc == IMAGE_FROM_VI) {
        s32Ret = CVI_VI_GetChnFrame(0, pstFrameInfo->ViChn, &stVideoFrame, MAX_GET_FRAME_TIMEOUT);
    } else if (eImageSrc == IMAGE_FROM_VPSS) {
        s32Ret = CVI_VPSS_GetChnFrame(pstFrameInfo->VpssGrp, pstFrameInfo->VpssChn, &stVideoFrame,
                                      MAX_GET_FRAME_TIMEOUT);
    }
    if (s32Ret != CVI_SUCCESS) {
    ...
    ISP_DAEMON2_DEBUG_EX(LOG_DEBUG, "W:%d,H:%d,stride:%d,%d,%d,len:%d,%d,%d\n",
                         stVideoFrame.stVFrame.u32Width, stVideoFrame.stVFrame.u32Height,
                         pstFrameHeader->stride[0], pstFrameHeader->stride[1],
                         pstFrameHeader->stride[2], stVideoFrame.stVFrame.u32Length[0],
                         stVideoFrame.stVFrame.u32Length[1], stVideoFrame.stVFrame.u32Length[2]);

    pstFrameHeader->size = stVideoFrame.stVFrame.u32Length[0] + stVideoFrame.stVFrame.u32Length[1] +
    ...
    if (eImageSrc == IMAGE_FROM_VI) {
        ...
        s32Ret =
            CVI_VPSS_ReleaseChnFrame(pstFrameInfo->VpssGrp, pstFrameInfo->VpssChn, &stVideoFrame);
    }
    if (s32Ret != CVI_SUCCESS) {
...
// -----------------------------------------------------------------------------
static CVI_S32 CVI_ISPD2_InitYUVFrameBuffer(TFrameData         *ptFrameData,
                                            const TFrameHeader *ptFrameHeader,
                                            CVI_U32             u32TotalFrames)
{
    ...
    if (ptFrameData->bTightlyMode) {
        ptFrameData->pu8ImageBuffer =
            (CVI_U8 *)calloc(u32TotalFrames, ptFrameData->u32YUVFrameSize);
...
// -----------------------------------------------------------------------------
static CVI_S32 CVI_ISPD2_GetRAWFrameInfo(const TGetFrameInfo *pstFrameInfo,
                                         TFrameHeader        *pstFrameHeader)
{
    ...
    u32BlkSize     = VI_GetRawBufferSize(
        u32FrameWidth, u32FrameHeight, PIXEL_FORMAT_RGB_BAYER_12BPP, stChnAttr.enCompressMode,
        DEFAULT_ALIGN, (u32FrameWidth > RAW_FRAME_TILE_MODE_WIDTH) ? CVI_TRUE : CVI_FALSE);
    u8WDRFusionFrame = CVI_ISPD2_GetWDRFusionFrame(stDevAttr.stWDRAttr.enWDRMode);
    u8WDRFusionFrame =
        (u8WDRFusionFrame < MAX_WDR_FUSION_FRAMES) ? u8WDRFusionFrame : MAX_WDR_FUSION_FRAMES;
...
// -----------------------------------------------------------------------------
static CVI_S32 CVI_ISPD2_InitRAWFrameBuffer(TFrameData         *ptFrameData,
                                            const TFrameHeader *ptFrameHeader,
                                            CVI_U32 u32TotalFrames, CVI_U32 u32CropWidth,
                                            CVI_U32 u32CropHeight)
{
    ...
    if (u32Temp) {
        ...
        ptFrameData->pu8AELogBuffer = (CVI_U8 *)calloc(1, sizeof(CVI_U8));
    }
    ...
    for (CVI_U32 u32FrameIdx = 0; u32FrameIdx < u32TotalFrames; ++u32FrameIdx, pstRAWHeader++) {
        ...
        pstRAWHeader->compress    = (ptFrameHeader->compress == COMPRESS_MODE_NONE) ? 0 : 1;
...
static CVI_S32 CVI_ISPD2_UpdateRAWHeader(TFrameData *ptFrameData)
{
    ...
    ISP_INNER_STATE_INFO_S *pstInnerStateInfo =
        (ISP_INNER_STATE_INFO_S *)malloc(sizeof(*pstInnerStateInfo));

    if (pstInnerStateInfo == NULL) {
        ISP_DAEMON2_DEBUG(LOG_ERR, "not enough memory");
        return CVI_FAILURE;
    }
    ...
    for (CVI_U32 u32FrameIdx = 0; u32FrameIdx < ptFrameData->u32TotalFrames;
         ++u32FrameIdx, pstRAWHeader++) {
        ...
        for (CVI_U32 u32CCMIdx = 0; u32CCMIdx < 9; ++u32CCMIdx) {
            pstRAWHeader->ccm[u32CCMIdx] = pstInnerStateInfo->ccm[u32CCMIdx];
        }

        pstRAWHeader->blcOffset[0] = pstInnerStateInfo->blcOffsetR;
        pstRAWHeader->blcOffset[1] = pstInnerStateInfo->blcOffsetGr;
        pstRAWHeader->blcOffset[2] = pstInnerStateInfo->blcOffsetGb;
        pstRAWHeader->blcOffset[3] = pstInnerStateInfo->blcOffsetB;
        pstRAWHeader->blcGain[0]   = pstInnerStateInfo->blcGainR;
        pstRAWHeader->blcGain[1]   = pstInnerStateInfo->blcGainGr;
        pstRAWHeader->blcGain[2]   = pstInnerStateInfo->blcGainGb;
        pstRAWHeader->blcGain[3]   = pstInnerStateInfo->blcGainB;
    }
...
// -----------------------------------------------------------------------------
static CVI_S32 CVI_ISPD2_GetMultiplyYUV(TFrameData *ptFrameData, CVI_U32 u32TotalFrames)
{
    TGetFrameInfo      *ptFrameInfo  = &(ptFrameData->stFrameInfo);
    ...
    ISP_DAEMON2_DEBUG_EX(LOG_DEBUG,
                         "GetMultiplyYUV Source (%d, 0:VI, 1:VPSS), FrameCounts : %u\n"
    ...
    for (CVI_U32 u32FrameIdx = 0; u32FrameIdx < u32TotalFrames; ++u32FrameIdx, ptYUVHeader++) {
        ...
        u32PlanarCount    = 3;
        au32PlanarSize[0] = ptVideoFrame->stVFrame.u32Length[0];
        au32PlanarSize[1] = ptVideoFrame->stVFrame.u32Length[1];
        au32PlanarSize[2] = ptVideoFrame->stVFrame.u32Length[2];
        ...
        if (ptFrameData->bTightlyMode) {
            ...
            for (CVI_U32 u32PlanarIdx = 0; u32PlanarIdx < u32PlanarCount; ++u32PlanarIdx) {
                if (au32PlanarSize[u32PlanarIdx] > 0) {
                    memcpy(ptFrameData->pu8ImageBuffer + u32BufOffset,
                           (void *)ptVideoFrame->stVFrame.u64PhyAddr[0] + u32PlaneOffset,
            ...
            ptFrameInfo->pau8FrameAddr[0] = (CVI_U8 *)ptVideoFrame->stVFrame.u64PhyAddr[0];
            ptFrameInfo->u32FrameSize[0]  = ptYUVHeader->size;
...
static CVI_S32 CVI_ISPD2_GetMultiplyRAW_Kernel(TFrameData *ptFrameData, CVI_U32 u32TotalFrames)
{
    ...
    ISP_DAEMON2_DEBUG_EX(LOG_DEBUG, "GetMultiplyRAW Source: VIPipe: %d, TotalFrames: %u", ViPipe,
                         u32TotalFrames);
    ...
    for (CVI_U32 u32FrameIdx = 0; u32FrameIdx < u32TotalFrames; ++u32FrameIdx, ptRAWHeader++) {
        ...
        for (CVI_U32 u32Idx = 0; u32Idx < ptRAWHeader->fusionFrame; ++u32Idx) {
            ...
            if (ptFrameData->bTightlyMode) {
                memcpy((void *)(ptFrameData->pu8ImageBuffer + u32BufOffset),
                       (const void *)ptVideoFrame[u32Idx].stVFrame.u64PhyAddr[0], u32FrameSize);
                // CVI_SYS_Munmap((void *)ptVideoFrame[u32Idx].stVFrame.pu8VirAddr[0],
                //				u32FrameSize);
                u32BufOffset += u32FrameSize;
            } else {
                ptFrameInfo->pau8FrameAddr[u32Idx] =
                    (CVI_U8 *)ptVideoFrame[u32Idx].stVFrame.u64PhyAddr[0];
        ...
        usleep(33 * 1000);
        ISP_DAEMON2_DEBUG_EX(LOG_DEBUG, "Frame#: %u / %u, fusion frame = %u", u32FrameIdx,
                             u32TotalFrames, ptRAWHeader->fusionFrame);
        for (CVI_U32 u32Idx = 0; u32Idx < ptRAWHeader->fusionFrame; ++u32Idx) {
            ISP_DAEMON2_DEBUG_EX(LOG_DEBUG, "Frame size (%u) : %u", u32Idx,
                                 au32ExposureSize[u32Idx]);
        }
    }
...
// -----------------------------------------------------------------------------
static CVI_S32 CVI_ISPD2_GetMultiplyRAW_ByCustomVBPool(TFrameData *ptFrameData,
                                                       CVI_U32     u32TotalFrames)
{
    TRAWHeader *pstRAWHeader   = ptFrameData->pstRAWHeader;
    ...
    stVBPoolCfg.u32BlkSize = VI_GetRawBufferSize(
        pstRAWHeader[0].cropWidth, pstRAWHeader[0].cropHeight, PIXEL_FORMAT_RGB_BAYER_12BPP,
        (pstRAWHeader[0].compress != COMPRESS_MODE_NONE) ? COMPRESS_MODE_TILE : COMPRESS_MODE_NONE,
        DEFAULT_ALIGN,
...
// -----------------------------------------------------------------------------
static CVI_S32 CVI_ISPD2_GetMultiplyRAW_ByUsingVPSSBuffer(TFrameData *ptFrameData,
                                                          CVI_U32     u32TotalFrames)
{
    TRAWHeader *pstRAWHeader   = ptFrameData->pstRAWHeader;
    CVI_U32     u32FusionFrame = pstRAWHeader[0].fusionFrame;
    ...
    ISP_DAEMON2_DEBUG_EX(
        LOG_DEBUG, "VPSS Chn Frame : Length : %u, %u, %u", ptVideoFrame[0].stVFrame.u32Length[0],
    ...
    if (pstRAWHeader[0].fusionFrame > 1) {
        ...
        do {
            s32Ret =
                CVI_VPSS_GetChnFrame(VpssGrp, VpssChn, &(ptVideoFrame[1]), MAX_GET_FRAME_TIMEOUT);
            if (s32Ret != CVI_SUCCESS) {
        ...
        if (s32Ret != CVI_SUCCESS) {
            if (ptVPSSVideoFrame[0].stVFrame.u64PhyAddr[0] != 0) {
                if (CVI_VPSS_ReleaseChnFrame(VpssGrp, VpssChn, &(ptVPSSVideoFrame[0])) !=
                    CVI_SUCCESS) {
    ...
    if (u32FusionFrame > 1) {
        if (ptVideoFrame[0].stVFrame.u32Length[0] >= (pstRAWHeader[0].size * u32FusionFrame)) {
            ...
            for (CVI_U32 u32Idx = 1; u32Idx < u32PerFrameSize; ++u32Idx) {
                ...
                memcpy(&ptVPSSVideoFrame[u32Idx], &ptVideoFrame[u32Idx],
...
static CVI_S32 CVI_ISPD2_GetMultiplyRAW(TFrameData *ptFrameData, CVI_U32 u32TotalFrames)
{
    ...
    ISP_DAEMON2_DEBUG(LOG_INFO,
                      "GetMultiplyRAW (custom VB pool) fail. Try use VPSS frame buffer mode");
...
// -----------------------------------------------------------------------------
static CVI_S32 CVI_ISPD2_BinaryData_WriteDataToDestination(int iFd, CVI_VOID *pvData,
                                                           CVI_U32 u32DataLen,
                                                           CVI_U32 u32PartialLen)
{
...
static CVI_S32 CVI_ISPD2_BinaryData_WriteNONEDataToDestination(int iFd)
{
    ...
    if (CVI_ISPD2_BinaryData_WriteDataToDestination(
            iFd, (CVI_VOID *)szNoneStrings, strlen(szNoneStrings),
...
static CVI_S32 CVI_ISPD2_BinaryData_ExportSingleYUVFrame(int iFd, TISPDeviceInfo *ptDevInfo)
{
    ...
    CVI_U32 u32CurFrame, u32TotalFrames;
    ...
    ISP_DAEMON2_DEBUG_EX(LOG_INFO,
                         "YUV Image (tightly:%u)\n"
                         "header: %u\nimage: %u\ninfo: %ux%u, (%u, %u, %u)",
                         ptFrameData->bTightlyMode, (CVI_U32)sizeof(TYUVHeader), pstYUVHeader->size,
                         pstYUVHeader->width, pstYUVHeader->height, pstYUVHeader->stride[0],
                         pstYUVHeader->stride[1], pstYUVHeader->stride[2]);

    if (ptFrameData->bTightlyMode) {
        if (ptFrameData->pu8ImageBuffer != NULL) {
            // header
            if (CVI_ISPD2_BinaryData_WriteDataToDestination(
                    iFd, (CVI_VOID *)pstYUVHeader, sizeof(TYUVHeader),
                    MAX_TCP_DATA_SIZE_PER_TRANSMIT) != CVI_SUCCESS) {
                ISP_DAEMON2_DEBUG(LOG_ERR, "Write data to socket fail");
                return CVI_FAILURE;
            }
            // data
            if (CVI_ISPD2_BinaryData_WriteDataToDestination(
                    iFd, (CVI_VOID *)ptFrameData->pu8ImageBuffer, pstYUVHeader->size,
                    MAX_TCP_DATA_SIZE_PER_TRANSMIT) != CVI_SUCCESS) {
                ISP_DAEMON2_DEBUG(LOG_ERR, "Write data to socket fail");
                return CVI_FAILURE;
            }
        ...
        if (ptFrameData->stFrameInfo.pau8FrameAddr[0] != NULL) {
            ...
            // data
            if (CVI_ISPD2_BinaryData_WriteDataToDestination(
                    iFd, (CVI_VOID *)ptFrameData->stFrameInfo.pau8FrameAddr[0], pstYUVHeader->size,
                    MAX_TCP_DATA_SIZE_PER_TRANSMIT) != CVI_SUCCESS) {
                ISP_DAEMON2_DEBUG(LOG_ERR, "Write data to socket fail");
                return CVI_FAILURE;
            }
...
static CVI_S32 CVI_ISPD2_BinaryData_ExportMultipleYUVFrame(int iFd, TISPDeviceInfo *ptDevInfo)
{
    ...
    ISP_DAEMON2_DEBUG_EX(LOG_INFO,
                         "YUV Multiple Image (tightly:%u) (%u/%u, %u)\n"
                         "header: %u\nimage: %u\ninfo: %ux%u, (%u, %u, %u)",
                         ptFrameData->bTightlyMode, u32CurFrame, u32TotalFrames, u32MemOffset,
                         (CVI_U32)sizeof(TYUVHeader), pstYUVHeader->size, pstYUVHeader->width,
                         pstYUVHeader->height, pstYUVHeader->stride[0], pstYUVHeader->stride[1],
                         pstYUVHeader->stride[2]);
    ...
    if (ptFrameData->bTightlyMode) {
        if (ptFrameData->pu8ImageBuffer != NULL) {
            ...
            // data
            if (CVI_ISPD2_BinaryData_WriteDataToDestination(
                    iFd, (CVI_VOID *)(ptFrameData->pu8ImageBuffer + u32MemOffset),
                    pstYUVHeader->size, MAX_TCP_DATA_SIZE_PER_TRANSMIT) != CVI_SUCCESS) {
                ISP_DAEMON2_DEBUG(LOG_ERR, "Write data to socket fail");
                return CVI_FAILURE;
            }
...
// -----------------------------------------------------------------------------
static CVI_S32 CVI_ISPD2_WriteBufferToFile(const char *pszFilename, const CVI_VOID *pvBuffer,
...
static CVI_S32 CVI_ISPD2_BinaryData_ExportMultipleRAWFrame(int iFd, TISPDeviceInfo *ptDevInfo)
{
    ...
    ISP_DAEMON2_DEBUG_EX(LOG_INFO,
                         "RAW Multiple Image (tightly:%u) (%u/%u, %u)\n"
                         "header:%u\nimage:%u\ninfo: %ux%u, Crop: (%u, %u), %ux%u",
                         ptFrameData->bTightlyMode, u32CurFrame, u32TotalFrames, u32MemOffset,
                         (CVI_U32)sizeof(TRAWHeader), pstRAWHeader->size, pstRAWHeader->width,
                         pstRAWHeader->height, pstRAWHeader->cropX, pstRAWHeader->cropY,
    ...
    if (ptFrameData->bTightlyMode) {
        if (ptFrameData->pu8ImageBuffer != NULL) {
            // header
            if (CVI_ISPD2_BinaryData_WriteDataToDestination(
                    iFd, (CVI_VOID *)pstRAWHeader, sizeof(TRAWHeader),
                    MAX_TCP_DATA_SIZE_PER_TRANSMIT) != CVI_SUCCESS) {
                ISP_DAEMON2_DEBUG(LOG_ERR, "Write data to socket fail");
                return CVI_FAILURE;
            }
            ...
            if (CVI_ISPD2_BinaryData_WriteDataToDestination(
                    iFd, (CVI_VOID *)(ptFrameData->pu8ImageBuffer + u32MemOffset),
                    pstRAWHeader->size, MAX_TCP_DATA_SIZE_PER_TRANSMIT) != CVI_SUCCESS) {
                ISP_DAEMON2_DEBUG(LOG_ERR, "Write data to socket fail");
                return CVI_FAILURE;
            }

#ifdef DEBUG_DUMP_BINARY_FRAME_BUFFER
            CVI_ISPD2_WriteBufferToFile_EX(
                u32CurFrame, u32TotalFrames,
                (const CVI_VOID *)(ptFrameData->pu8ImageBuffer + u32MemOffset), pstRAWHeader->size);
#endif  // DEBUG_DUMP_BINARY_FRAME_BUFFER
        ...
        CVI_U32 u32NonTightlyIndex = (u32CurFrame < pstRAWHeader->fusionFrame) ? u32CurFrame : 0;
        ...
        if (ptFrameData->stFrameInfo.pau8FrameAddr[u32NonTightlyIndex] != NULL) {
            ...
            // data
            if (CVI_ISPD2_BinaryData_WriteDataToDestination(
                    iFd, (CVI_VOID *)ptFrameData->stFrameInfo.pau8FrameAddr[u32NonTightlyIndex],
                    pstRAWHeader->size, MAX_TCP_DATA_SIZE_PER_TRANSMIT) != CVI_SUCCESS) {
                ISP_DAEMON2_DEBUG(LOG_ERR, "Write data to socket fail");
                return CVI_FAILURE;
            }
...
static CVI_S32 CVI_ISPD2_BinaryData_ExportAEBinData(int iFd, TISPDeviceInfo *ptDevInfo)
{
    ...
    if ((ptBinData->pu8Buffer) && (ptBinData->u32Size > 0)) {
        if (CVI_ISPD2_BinaryData_WriteDataToDestination(
                iFd, (CVI_VOID *)ptBinData->pu8Buffer, ptBinData->u32Size,
                MAX_TCP_DATA_SIZE_PER_TRANSMIT) != CVI_SUCCESS) {
            ISP_DAEMON2_DEBUG(LOG_ERR, "Write data to socket fail");
            return CVI_FAILURE;
        }
...
static CVI_S32 CVI_ISPD2_BinaryData_ExportTuningBinData(int iFd, TISPDeviceInfo *ptDevInfo)
{
    ...
    if ((ptBinaryData->pu8Buffer) && (ptBinaryData->u32Size > 0)) {
        if (CVI_ISPD2_BinaryData_WriteDataToDestination(
                iFd, (CVI_VOID *)ptBinaryData->pu8Buffer, ptBinaryData->u32Size,
                MAX_TCP_DATA_SIZE_PER_TRANSMIT) != CVI_SUCCESS) {
            ISP_DAEMON2_DEBUG(LOG_ERR, "Write data to socket fail");
            return CVI_FAILURE;
        }
                                           ...
                                           JSONObject         *pJsonResponse)
{
    ...
    TVPSSAdjGroupInfo astVPSSAdjGrp_System;
    ...
    if ((stVIVPSSMode.aenMode[pstFrameInfo->ViPipe] == VI_OFFLINE_VPSS_ONLINE) ||
        ...
        CVI_VPSS_GetGrpProcAmp(pstFrameInfo->VpssGrp, PROC_AMP_BRIGHTNESS,
                               &(astVPSSAdjGrp_System.s32Brightness));
        CVI_VPSS_GetGrpProcAmp(pstFrameInfo->VpssGrp, PROC_AMP_CONTRAST,
                               &(astVPSSAdjGrp_System.s32Contrast));
        CVI_VPSS_GetGrpProcAmp(pstFrameInfo->VpssGrp, PROC_AMP_SATURATION,
                               &(astVPSSAdjGrp_System.s32Saturation));
        CVI_VPSS_GetGrpProcAmp(pstFrameInfo->VpssGrp, PROC_AMP_HUE, &(astVPSSAdjGrp_System.s32Hue));

        CVI_VPSS_SetGrpProcAmp(pstFrameInfo->VpssGrp, PROC_AMP_BRIGHTNESS, s32DefaultBrightness);
        CVI_VPSS_SetGrpProcAmp(pstFrameInfo->VpssGrp, PROC_AMP_CONTRAST, s32DefaultContrast);
        CVI_VPSS_SetGrpProcAmp(pstFrameInfo->VpssGrp, PROC_AMP_SATURATION, s32DefaultSaturation);
        CVI_VPSS_SetGrpProcAmp(pstFrameInfo->VpssGrp, PROC_AMP_HUE, s32DefaultHue);
    }
    ...
    ISP_DAEMON2_DEBUG_EX(LOG_INFO, "Single YUV Frame, W/H : %u x %u, Stride : %u, Size : %u",
                         stFrameHeader.width, stFrameHeader.height, stFrameHeader.stride[0],
                         stFrameHeader.size);
    ...
    s32GetYUVRet = CVI_ISPD2_GetMultiplyYUV(ptFrameData, 1);

    // restore VPSS adjustment
    if (bSetVPSSDefault) {
        CVI_VPSS_SetGrpProcAmp(pstFrameInfo->VpssGrp, PROC_AMP_BRIGHTNESS,
                               astVPSSAdjGrp_System.s32Brightness);
        CVI_VPSS_SetGrpProcAmp(pstFrameInfo->VpssGrp, PROC_AMP_CONTRAST,
                               astVPSSAdjGrp_System.s32Contrast);
        CVI_VPSS_SetGrpProcAmp(pstFrameInfo->VpssGrp, PROC_AMP_SATURATION,
                               astVPSSAdjGrp_System.s32Saturation);
        CVI_VPSS_SetGrpProcAmp(pstFrameInfo->VpssGrp, PROC_AMP_HUE, astVPSSAdjGrp_System.s32Hue);
    }
                                              ...
                                              JSONObject         *pJsonResponse)
{
    ...
    if (ptContentIn->pParams) {
        ...
        GET_INT_FROM_JSON(ptContentIn->pParams, "/imageSrcIndex", s32ImageSrcVipe,
                          s32ImageSrcVipeState);
        ...
        if (s32ImageSrcVipeState == CVI_SUCCESS) {
            if (s32ImageSrcVipe < s32DevNum) {
                ...
                snprintf((char *)buffer, 50, "Num of current board sensor is %d!!!", s32DevNum);
                ISP_DAEMON2_DEBUG(LOG_DEBUG, "Num of current board sensor is lower than 2!");
    ...
    if ((!bTightlyMode) && (u32TotalFrames > 1)) {
        ...
        CVI_ISPD2_Utils_ComposeMessage(
            ptContentOut, JSONRPC_CODE_INVALID_PARAMS,
            "YUV multiple frame function only support 1 frame in non-tightly mode!)");
    ...
    if (eImageSrc == IMAGE_FROM_VI) {
        ...
        if ((stVIVPSSMode.aenMode[ViPipe] == VI_OFFLINE_VPSS_ONLINE) ||
            ...
            // backup and apply default VPSS Adjustment
            CVI_VPSS_GetGrpProcAmp(VpssGrp, PROC_AMP_BRIGHTNESS,
                                   &(astVPSSAdjGrp_System.s32Brightness));
            CVI_VPSS_GetGrpProcAmp(VpssGrp, PROC_AMP_CONTRAST, &(astVPSSAdjGrp_System.s32Contrast));
            CVI_VPSS_GetGrpProcAmp(VpssGrp, PROC_AMP_SATURATION,
                                   &(astVPSSAdjGrp_System.s32Saturation));
            CVI_VPSS_GetGrpProcAmp(VpssGrp, PROC_AMP_HUE, &(astVPSSAdjGrp_System.s32Hue));

            CVI_VPSS_SetGrpProcAmp(VpssGrp, PROC_AMP_BRIGHTNESS, s32DefaultBrightness);
            CVI_VPSS_SetGrpProcAmp(VpssGrp, PROC_AMP_CONTRAST, s32DefaultContrast);
            CVI_VPSS_SetGrpProcAmp(VpssGrp, PROC_AMP_SATURATION, s32DefaultSaturation);
            CVI_VPSS_SetGrpProcAmp(VpssGrp, PROC_AMP_HUE, s32DefaultHue);
        }
    } else if (eImageSrc == IMAGE_FROM_VPSS) {
        CVI_ISPD2_Utils_GetCurrentVPSSInfo(ptDevInfo, &VpssGrp, &VpssChn);
    } else {
    ...
    ISP_DAEMON2_DEBUG_EX(
        LOG_INFO, "Multiple YUV Frames (%u), W/H : %u x %u, Stride : %u, Size : %u", u32TotalFrames,
    ...
    if (CVI_ISPD2_InitYUVFrameBuffer(ptFrameData, &stFrameHeader, u32TotalFrames) != CVI_SUCCESS) {
        ...
        ISP_DAEMON2_DEBUG(LOG_DEBUG, "Init YUV frame buffer fail");
        CVI_ISPD2_Utils_ComposeMessage(
            ptContentOut, JSONRPC_CODE_INTERNAL_API_ERROR,
            "Init YUV frame buffer fail (Failed to allocate memory. Please decrease the number of "
            "frame or uncheck the \"Capture raw tightly\" checkbox.)");

        return CVI_FAILURE;
    }

    s32Ret = CVI_ISPD2_GetMultiplyYUV(ptFrameData, u32TotalFrames);

    // restore VPSS adjustment
    if (bSetVPSSDefault) {
        CVI_VPSS_SetGrpProcAmp(VpssGrp, PROC_AMP_BRIGHTNESS, astVPSSAdjGrp_System.s32Brightness);
        CVI_VPSS_SetGrpProcAmp(VpssGrp, PROC_AMP_CONTRAST, astVPSSAdjGrp_System.s32Contrast);
        CVI_VPSS_SetGrpProcAmp(VpssGrp, PROC_AMP_SATURATION, astVPSSAdjGrp_System.s32Saturation);
        CVI_VPSS_SetGrpProcAmp(VpssGrp, PROC_AMP_HUE, astVPSSAdjGrp_System.s32Hue);
    }
...
static CVI_S32 CVI_ISPD2_UpdateRegDump(TFrameData *ptFrameData)
{
    ...
    if (CVI_VI_DumpHwRegisterToFile(ViPipe, ptFrameData->pu8RegDump, &stDumpRegTable) !=
        CVI_SUCCESS) {
...
static CVI_S32 CVI_ISPD2_UpdateRAWInfo(TFrameData *ptFrameData)
{
    ...
    if (CVI_ISP_DumpFrameRawInfoToBuffer(ViPipe, ptFrameData->pu8RawInfo, RAW_INFO_BUFFER_SIZE) !=
        CVI_SUCCESS) {
        ISP_DAEMON2_DEBUG(LOG_ERR, "CVI_ISP_DumpFrameRawInfoToBuffer fail");
        return CVI_FAILURE;
    }
...
static CVI_S32 CVI_ISPD2_UpdateAWBLogData(TFrameData *ptFrameData)
{
    ...
    if (CVI_ISP_GetAWBSnapLogBuf(ViPipe, ptFrameData->pu8AWBLogBuffer, AWB_SNAP_LOG_BUFF_SIZE) !=
        CVI_SUCCESS) {
                                              ...
                                              JSONObject         *pJsonResponse)
{
    ...
    if ((!bTightlyMode) && (u32TotalFrames > 1)) {
        ISP_DAEMON2_DEBUG(LOG_WARNING,
                          "RAW multiple frame function only support 1 frame in non-tightly mode");
        CVI_ISPD2_Utils_ComposeMessage(
            ptContentOut, JSONRPC_CODE_INVALID_PARAMS,
            "YUV multiple frame function only support 1 frame in non-tightly mode!)");
    ...
    ISP_DAEMON2_DEBUG_EX(LOG_INFO,
                         "Multiple RawFrames (%u), W/H : %u x %u, Crop W/H : %u x %u, Size : %u\n"
                         "Fusion frame = %u, BayerID : %u, Compress : %u",
                         u32TotalFrames, stFrameHeader.width, stFrameHeader.height, u32CropWidth,
                         u32CropHeight, stFrameHeader.size, stFrameHeader.fusionFrame,
                         stFrameHeader.bayerID, stFrameHeader.compress);

    // initialize raw header and buffer
    ptFrameData->u32RawFrameSize = stFrameHeader.size;
    if (CVI_ISPD2_InitRAWFrameBuffer(ptFrameData, &stFrameHeader, u32TotalFrames, u32CropWidth,
                                     u32CropHeight) != CVI_SUCCESS) {
        CVI_ISPD2_ReleaseFrameData(ptDevInfo);

        ISP_DAEMON2_DEBUG(LOG_DEBUG, "Init RAW frame buffer fail");
        CVI_ISPD2_Utils_ComposeMessage(
            ptContentOut, JSONRPC_CODE_INTERNAL_API_ERROR,
            "Init RAW frame buffer fail (Failed to allocate memory. Please decrease the number of "
            "frame or uncheck the \"Capture raw tightly\" checkbox.)");
                                      ...
                                      TJSONRpcContentOut *ptContentOut, JSONObject *pJsonResponse)
{
    ...
    if (ptFrameData->pstYUVHeader != NULL) {
        ...
        if (u32CurFrame < u32TotalFrames) {
            ...
            ISP_DAEMON2_DEBUG_EX(LOG_INFO, "Get YUV frame info: (Tightly:%u) (%u/%u, %u), %ux%u",
        ...
        if (u32CurFrame < u32TotalFrames) {
            ...
            ISP_DAEMON2_DEBUG_EX(
                ...
                bTightlyMode, u32CurFrame, u32TotalFrames, u32MemOffset, ptRAWHeader->width,
                ptRAWHeader->height, ptRAWHeader->cropX, ptRAWHeader->cropY, ptRAWHeader->cropWidth,
                ptRAWHeader->cropHeight);
            ...
            if (bTightlyMode) {
                ...
                CVI_U32 u32NonTightlyIndex =
                    (u32CurFrame < ptRAWHeader->fusionFrame) ? u32CurFrame : 0;
                                      ...
                                      TJSONRpcContentOut *ptContentOut, JSONObject *pJsonResponse)
{
    ...
    if (ptFrameData->pstYUVHeader != NULL) {
        ...
        if (u32CurFrame < u32TotalFrames) {
            ...
            ISP_DAEMON2_DEBUG_EX(LOG_INFO, "Get YUV frame data: (Tightly:%u) (%u/%u, %u), %ux%u",
                                 bTightlyMode, u32CurFrame, u32TotalFrames, u32MemOffset,
                                 ptYUVHeader->width, ptYUVHeader->height);
                                       ...
                                       TJSONRpcContentOut *ptContentOut, JSONObject *pJsonResponse)
{
    ...
    case EBINARYDATA_TUNING_BIN_DATA:
        ...
        if ((ptBinData->eDataType == EBINARYDATA_TUNING_BIN_DATA) && (ptBinData->pu8Buffer) &&
            (ptBinData->u32Size > 0)) {
    ...
    case EBINARYDATA_TOOL_DEFINITION_DATA:
        ...
        if ((ptBinData->eDataType == EBINARYDATA_TOOL_DEFINITION_DATA) && (ptBinData->pu8Buffer) &&
            (ptBinData->u32Size > 0)) {
            SET_STRING_TO_JSON("Content type", "Tool definition data", pDataOut);
            SET_INT_TO_JSON("Data size", ptBinData->u32Size, pDataOut);
        } else {
                                           ...
                                           JSONObject         *pJsonResponse)
{
    ...
    ISP_DAEMON2_DEBUG_EX(LOG_INFO,
                         "Multiple RAW Frames (%u), W/H : %u x %u, Crop W/H : %u x %u, Size : %u\n"
    ...
    ISP_DAEMON2_DEBUG_EX(LOG_DEBUG, "Bracketing exposure leEV = %d, seEV = %d", s16LeEV, s16SeEV);
