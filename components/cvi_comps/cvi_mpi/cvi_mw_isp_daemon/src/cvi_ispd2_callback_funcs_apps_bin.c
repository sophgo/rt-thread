                                       ...
                                       TJSONRpcContentOut *ptContentOut, JSONObject *pJsonResponse)
{
    ...
    if (s32RetTmp == CVI_SUCCESS) {
        ...
        if (CVI_ISPD2_WriteSingleISPRegToBinBuffer(id, ptBinaryOutData->pu8Buffer, u32BinBufSize,
        ...
        if (CVI_ISPD2_WriteISPRegToBinBuffer(ptBinaryOutData->pu8Buffer, u32BinBufSize, &stBinExtra,
                                             &u32BinLen) != CVI_SUCCESS) {
...
static CVI_S32 CVI_ISPD2_WriteISPRegToBinFp(FILE *fp, CVI_BIN_EXTRA_S *ptBinExtra)
{
    ...
    if (CVI_ISPD2_WriteISPRegToBinBuffer(pu8Buffer, u32BinBufSize, ptBinExtra, &u32BinLen) !=
        CVI_SUCCESS) {
        ISP_DAEMON2_DEBUG(LOG_DEBUG, "Dump tuning bin fail");
        SAFE_FREE(pu8Buffer);
        return CVI_FAILURE;
...
// -----------------------------------------------------------------------------
CVI_S32 CVI_ISPD2_CBFunc_ImportBinFile(TBinaryData *ptBinaryData, TJSONRpcContentOut *ptContentOut,
