...
// -----------------------------------------------------------------------------
typedef enum { IMAGE_FROM_VI, IMAGE_FROM_VPSS } EImageSource;
...
typedef struct {
    ...
    CVI_S32 (*set_raw_replay_data)
    (const CVI_VOID *header, const CVI_VOID *data, CVI_U32 totalFrame, CVI_U32 curFrame,
     CVI_U32 rawFrameSize);
