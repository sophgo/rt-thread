...
static CVI_S32 CVI_ISPD2_InitRawRepalyHandle(TRawReplayHandle *pHandle)
{
    ...
    do {
        ...
        pHandle->start_raw_replay = (CVI_S32 (*)(VI_PIPE)) dlsym(pHandle->pSoHandle,
    "start_raw_replay"); error = dlerror(); if (error != NULL) { break;
        }

        pHandle->stop_raw_replay = (CVI_S32 (*)(CVI_VOID)) dlsym(pHandle->pSoHandle,
    "stop_raw_replay"); error = dlerror(); if (error != NULL) { break;
        }
                                               ...
                                               JSONObject *pJsonRes)
{
    ...
    if (pRawReplayHandle->pSoHandle == NULL) {
        ...
        if (s32Ret != CVI_SUCCESS) {
            CVI_ISPD2_Utils_ComposeAPIErrorMessageEX(
                ptOut, "load libraw_replay.so failed, please put it to board.");
            return CVI_FAILURE;
        }
    }
    ...
    if (pRawReplayHandle->header == NULL) {
        ...
        if (pRawReplayHandle == NULL) {
            CVI_ISPD2_Utils_ComposeAPIErrorMessageEX(ptOut, "calloc raw header buffer failed.");
            return CVI_FAILURE;
        }
    }
    ...
    GET_INT_FROM_JSON(ptIn->pParams, "/raw_stRoiRect.u32Width", pHeader->stRoiRect.u32Width,
                      s32Ret);
    GET_INT_FROM_JSON(ptIn->pParams, "/raw_stRoiRect.u32Height", pHeader->stRoiRect.u32Height,
                      s32Ret);
    ...
    // ISP_DAEMON2_DEBUG_EX(LOG_DEBUG, "raw replay params: %s", ptIn->pParams);

    ISP_DAEMON2_DEBUG_EX(LOG_DEBUG,
                         "raw replay, totalFrame: %d, curFrame: %d, enWDR: %d, op_mode: %d",
                         pHeader->numFrame, pHeader->curFrame, pHeader->enWDR, pHeader->op_mode);

    ISP_DAEMON2_DEBUG_EX(LOG_DEBUG, "raw repaly, frame info: %dX%d, %d, roi frame number: %d, %d",
                         pHeader->width, pHeader->height, pHeader->size, pHeader->roiFrameNum,
                         pHeader->roiFrameSize);

    if (pRawReplayHandle->data == NULL || pHeader->curFrame == 0) {
        ...
        if (pRawReplayHandle->data == NULL) {
            CVI_ISPD2_Utils_ComposeAPIErrorMessageEX(ptOut, "calloc raw data buffer failed.");
            return CVI_FAILURE;
        }
    }
                                           ...
                                           JSONObject *pJsonRes)
{
    ...
    if ((ptBinaryData->eDataType != EBINARYDATA_RAW_DATA) || (ptBinaryData->pu8Buffer == NULL) ||
    ...
    if (pRawReplayHandle->set_raw_replay_data(pRawReplayHandle->header, pRawReplayHandle->data,
                                              pHeader->numFrame, pHeader->curFrame,
                                        ...
                                        JSONObject *pJsonRes)
{
    ...
    if (pRawReplayHandle->pSoHandle == NULL) {
        CVI_ISPD2_Utils_ComposeAPIErrorMessageEX(ptOut, "init failed.");
        return CVI_FAILURE;
    }
