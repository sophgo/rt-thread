                                        ...
                                        TJSONRpcContentOut *ptContentOut, JSONObject *pJsonResponse)
{
    ...
    snprintf(szVersion, 64, "%d.%d.%d.%d", ISP_DAEMON2_VERSION_MAJOR, ISP_DAEMON2_VERSION_MINOR,
...
// -----------------------------------------------------------------------------
static CVI_S32 CVI_ISPD2_CBFunc_VIVPSSBindCtrl(const TISPDeviceInfo *ptDevInfo, CVI_S32 ViPipe,
                                               CVI_S32 ViChn)
{
    ...
    if (CVI_SYS_UnBind(&stSrcChn, &stBindDest.astMmfChn[0]) != CVI_SUCCESS) {
        ISP_DAEMON2_DEBUG_EX(LOG_ERR, "CVI_SYS_UnBind VI(%d, %d)-VPSS(%d, %d) fail",
                             stSrcChn.s32DevId, stSrcChn.s32ChnId, stBindDest.astMmfChn[0].s32DevId,
                             stBindDest.astMmfChn[0].s32ChnId);
        return CVI_FAILURE;
    }
                                             ...
                                             JSONObject         *pJsonResponse)
{
    ...
    SET_INT_ARRAY_TO_JSON("GlobalToneCurve", LTM_GLOBAL_CURVE_NODE_NUM,
                          stInnerStateInfo.drcGlobalTone, pDataOut);
    SET_INT_ARRAY_TO_JSON("DarkToneCurve", LTM_DARK_CURVE_NODE_NUM, stInnerStateInfo.drcDarkTone,
                          pDataOut);
    SET_INT_ARRAY_TO_JSON("BrightToneCurve", LTM_BRIGHT_CURVE_NODE_NUM,
                          stInnerStateInfo.drcBrightTone, pDataOut);
                                             ...
                                             JSONObject         *pJsonResponse)
{
    ...
    if (CVI_ISP_GetAWBDbgBinBuf(ptDevInfo->s32ViPipe, pu8AWBDbgBin, s32AWBDbgBinSize) !=
        CVI_SUCCESS) {
    ...
    // ISP_AE_STATISTICS_S
    SET_INT_ARRAY_TO_JSON("FE0 Histogram", MAX_HIST_BINS, stAEStatistic.au32FEHist1024Value[0][0],
                          pDataOut);
    SET_INT_ARRAY_TO_JSON("FE1 Histogram", MAX_HIST_BINS, stAEStatistic.au32FEHist1024Value[1][0],
                          pDataOut);
    ...
    for (CVI_U32 u32RowIdx = 0; u32RowIdx < (CVI_U32)AE_ZONE_ROW / 2; ++u32RowIdx) {
        ...
        for (CVI_U32 u32ColIdx = 0; u32ColIdx < (CVI_U32)AE_ZONE_COLUMN / 2; ++u32ColIdx) {
            ISPD2_json_object_array_add(
                pFE0_ZoneR_ValueSubArray,
                ISPD2_json_object_new_int(
                    (stAEStatistic.au16FEZoneAvg[0][0][u32RowIdx * 2][u32ColIdx * 2][3] +
                     ...
                     stAEStatistic.au16FEZoneAvg[0][0][u32RowIdx * 2 + 1][u32ColIdx * 2 + 1][3]) /
                    4));
            ISPD2_json_object_array_add(
                pFE0_ZoneGr_ValueSubArray,
                ISPD2_json_object_new_int(
                    (stAEStatistic.au16FEZoneAvg[0][0][u32RowIdx * 2][u32ColIdx * 2][2] +
                     ...
                     stAEStatistic.au16FEZoneAvg[0][0][u32RowIdx * 2 + 1][u32ColIdx * 2 + 1][2]) /
                    4));
            ISPD2_json_object_array_add(
                pFE0_ZoneGb_ValueSubArray,
                ISPD2_json_object_new_int(
                    (stAEStatistic.au16FEZoneAvg[0][0][u32RowIdx * 2][u32ColIdx * 2][1] +
                     ...
                     stAEStatistic.au16FEZoneAvg[0][0][u32RowIdx * 2 + 1][u32ColIdx * 2 + 1][1]) /
                    4));
            ISPD2_json_object_array_add(
                pFE0_ZoneB_ValueSubArray,
                ISPD2_json_object_new_int(
                    (stAEStatistic.au16FEZoneAvg[0][0][u32RowIdx * 2][u32ColIdx * 2][0] +
                     ...
                     stAEStatistic.au16FEZoneAvg[0][0][u32RowIdx * 2 + 1][u32ColIdx * 2 + 1][0]) /
                    4));

            ISPD2_json_object_array_add(
                pFE1_ZoneR_ValueSubArray,
                ISPD2_json_object_new_int(
                    (stAEStatistic.au16FEZoneAvg[1][0][u32RowIdx * 2][u32ColIdx * 2][3] +
                     ...
                     stAEStatistic.au16FEZoneAvg[1][0][u32RowIdx * 2 + 1][u32ColIdx * 2 + 1][3]) /
                    4));
            ISPD2_json_object_array_add(
                pFE1_ZoneGr_ValueSubArray,
                ISPD2_json_object_new_int(
                    (stAEStatistic.au16FEZoneAvg[1][0][u32RowIdx * 2][u32ColIdx * 2][2] +
                     ...
                     stAEStatistic.au16FEZoneAvg[1][0][u32RowIdx * 2 + 1][u32ColIdx * 2 + 1][2]) /
                    4));
            ISPD2_json_object_array_add(
                pFE1_ZoneGb_ValueSubArray,
                ISPD2_json_object_new_int(
                    (stAEStatistic.au16FEZoneAvg[1][0][u32RowIdx * 2][u32ColIdx * 2][1] +
                     ...
                     stAEStatistic.au16FEZoneAvg[1][0][u32RowIdx * 2 + 1][u32ColIdx * 2 + 1][1]) /
                    4));
            ISPD2_json_object_array_add(
                pFE1_ZoneB_ValueSubArray,
                ISPD2_json_object_new_int(
                    (stAEStatistic.au16FEZoneAvg[1][0][u32RowIdx * 2][u32ColIdx * 2][0] +
                     ...
                     stAEStatistic.au16FEZoneAvg[1][0][u32RowIdx * 2 + 1][u32ColIdx * 2 + 1][0]) /
                    4));
        }
    ...
    SET_INT_ARRAY_TO_JSON("WB Zone CountAll", AWB_ZONE_NUM, stWBStatistic.au16ZoneCountAll,
                          pDataOut);
    ...
    SET_INT_TO_JSON("LightInfo[0].WhiteRgain",
                    ptAWBDbg->dbgInfoAttrEx[0].stLightInfo[0].u16WhiteRgain, pDataOut);
    SET_INT_TO_JSON("LightInfo[0].WhiteBgain",
                    ptAWBDbg->dbgInfoAttrEx[0].stLightInfo[0].u16WhiteBgain, pDataOut);
    SET_INT_TO_JSON("LightInfo[0].LightStatus",
                    ptAWBDbg->dbgInfoAttrEx[0].stLightInfo[0].u8LightStatus, pDataOut);
    SET_INT_TO_JSON("LightInfo[0].Radius", ptAWBDbg->dbgInfoAttrEx[0].stLightInfo[0].u8Radius,
                    pDataOut);
    SET_INT_TO_JSON("LightInfo[1].WhiteRgain",
                    ptAWBDbg->dbgInfoAttrEx[0].stLightInfo[1].u16WhiteRgain, pDataOut);
    SET_INT_TO_JSON("LightInfo[1].WhiteBgain",
                    ptAWBDbg->dbgInfoAttrEx[0].stLightInfo[1].u16WhiteBgain, pDataOut);
    SET_INT_TO_JSON("LightInfo[1].LightStatus",
                    ptAWBDbg->dbgInfoAttrEx[0].stLightInfo[1].u8LightStatus, pDataOut);
    SET_INT_TO_JSON("LightInfo[1].Radius", ptAWBDbg->dbgInfoAttrEx[0].stLightInfo[1].u8Radius,
                    pDataOut);
    SET_INT_TO_JSON("LightInfo[2].WhiteRgain",
                    ptAWBDbg->dbgInfoAttrEx[0].stLightInfo[2].u16WhiteRgain, pDataOut);
    SET_INT_TO_JSON("LightInfo[2].WhiteBgain",
                    ptAWBDbg->dbgInfoAttrEx[0].stLightInfo[2].u16WhiteBgain, pDataOut);
    SET_INT_TO_JSON("LightInfo[2].LightStatus",
                    ptAWBDbg->dbgInfoAttrEx[0].stLightInfo[2].u8LightStatus, pDataOut);
    SET_INT_TO_JSON("LightInfo[2].Radius", ptAWBDbg->dbgInfoAttrEx[0].stLightInfo[2].u8Radius,
                    pDataOut);
    SET_INT_TO_JSON("LightInfo[3].WhiteRgain",
                    ptAWBDbg->dbgInfoAttrEx[0].stLightInfo[3].u16WhiteRgain, pDataOut);
    SET_INT_TO_JSON("LightInfo[3].WhiteBgain",
                    ptAWBDbg->dbgInfoAttrEx[0].stLightInfo[3].u16WhiteBgain, pDataOut);
    SET_INT_TO_JSON("LightInfo[3].LightStatus",
                    ptAWBDbg->dbgInfoAttrEx[0].stLightInfo[3].u8LightStatus, pDataOut);
    SET_INT_TO_JSON("LightInfo[3].Radius", ptAWBDbg->dbgInfoAttrEx[0].stLightInfo[3].u8Radius,
                    pDataOut);
    ...
    ISPD2_json_object_array_add(pAWB_CropInfo_ValueArray,
                                ISPD2_json_object_new_int(ptAWBDbg->u16WinWnum));
    ISPD2_json_object_array_add(pAWB_CropInfo_ValueArray,
                                ISPD2_json_object_new_int(ptAWBDbg->u16WinHnum));
    ISPD2_json_object_array_add(pAWB_CropInfo_ValueArray,
                                ISPD2_json_object_new_int(ptAWBDbg->u16WinOffX));
    ISPD2_json_object_array_add(pAWB_CropInfo_ValueArray,
                                ISPD2_json_object_new_int(ptAWBDbg->u16WinOffY));
    ISPD2_json_object_array_add(pAWB_CropInfo_ValueArray,
                                ISPD2_json_object_new_int(ptAWBDbg->u16WinWsize));
    ISPD2_json_object_array_add(pAWB_CropInfo_ValueArray,
                                ISPD2_json_object_new_int(ptAWBDbg->u16WinHsize));
