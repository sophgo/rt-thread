...
#define CVI_ISP_SET_API(set_api_name, get_api_name, struct_name)                                   \
    {                                                                                              \
        ...
        TISPDeviceInfo *ptDevInfo             = ptContentIn->ptDeviceInfo;                         \
                                                                                                   \
        snprintf(content_buffer, CONTENT_BUFFER_SIZE, "%s-%s", GET_RESPONSE_KEY_NAME,              \
                 #struct_name);                                                                    \
...
#define CVI_ISP_GET_API_MALLOC(get_api_name, struct_name)                                 \
    {                                                                                     \
        struct_name *pstAttr = (struct_name *)malloc(sizeof(struct_name));                \
                                                                                          \
        TISPDeviceInfo *ptDevInfo = ptContentIn->ptDeviceInfo;                            \
        if (get_api_name(ptDevInfo->s32ViPipe, pstAttr) != CVI_SUCCESS) {                 \
            CVI_ISPD2_Utils_ComposeAPIErrorMessage(ptContentOut);                         \
            free(pstAttr);                                                                \
            return CVI_FAILURE;                                                           \
        }                                                                                 \
        struct_name##_JSON(JSONBIN_WRITE, pJsonResponse, GET_RESPONSE_KEY_NAME, pstAttr); \
        free(pstAttr);                                                                    \
                                                                                          \
        /* const char *pszParams = ISPD2_json_object_to_json_string_ext(pJsonResponse,    \
...
// -----------------------------------------------------------------------------
#define CVI_ISP_GET_API_EX_MALLOC(module_name, struct_name) \
    {CVI_ISP_GET_API_MALLOC(CVI_ISP_Get##module_name##Attr, struct_name)}

// -----------------------------------------------------------------------------
static CVI_S32 CVI_ISPD2_SetFrameInfotoEncVui(CVI_FLOAT f32FrameRate)
{
    VENC_CHN_ATTR_S stChnAttr;
    return CVI_SUCCESS;
    for (int vencChn = 0; vencChn < VENC_MAX_CHN_NUM; vencChn++) {
        if (CVI_VENC_GetChnAttr(vencChn, &stChnAttr) == CVI_FAILURE) {
            break;
        }
        if (stChnAttr.stVencAttr.enType == PT_H264) {
            VENC_H264_VUI_S stH264Vui;

            if (CVI_VENC_GetH264Vui(vencChn, &stH264Vui) == CVI_SUCCESS) {
                stH264Vui.stVuiTimeInfo.timing_info_present_flag = 1;
                stH264Vui.stVuiTimeInfo.time_scale               = f32FrameRate * 2;
                stH264Vui.stVuiTimeInfo.num_units_in_tick        = 1;
                CVI_VENC_SetH264Vui(vencChn, &stH264Vui);
            }
        } else if (stChnAttr.stVencAttr.enType == PT_H265) {
            VENC_H265_VUI_S stH265Vui;

            if (CVI_VENC_GetH265Vui(vencChn, &stH265Vui) == CVI_SUCCESS) {
                stH265Vui.stVuiTimeInfo.timing_info_present_flag = 1;
                stH265Vui.stVuiTimeInfo.time_scale               = f32FrameRate;
                stH265Vui.stVuiTimeInfo.num_units_in_tick        = 1;
                CVI_VENC_SetH265Vui(vencChn, &stH265Vui);
            }
        }
    }
                                        ...
                                        TJSONRpcContentOut *ptContentOut, JSONObject *pJsonResponse)
{
    ...
    stPubAttr.f32FrameRate = (CVI_FLOAT)(dlFrameRate);
    CVI_ISPD2_SetFrameInfotoEncVui(stPubAttr.f32FrameRate);

    if (CVI_ISP_SetPubAttr(ptDevInfo->s32ViPipe, &stPubAttr) != CVI_SUCCESS) {
        CVI_ISPD2_Utils_ComposeAPIErrorMessage(ptContentOut);
        return CVI_FAILURE;
    }
