...
// -----------------------------------------------------------------------------
CVI_S32 CVI_ISPD2_CBFunc_VPSS_LDCBinData(CVI_S32 VpssGrp, CVI_S32 VpssChn,
                                         TBinaryData        *ptBinaryData,
                                         TJSONRpcContentOut *ptContentOut,
                                         JSONObject         *pJsonResponse)
{
    if ((ptBinaryData->eDataType != EBINARYDATA_VPSS_LDC_BIN_DATA) ||
        (ptBinaryData->pu8Buffer == NULL) || (ptBinaryData->u32Size == 0)) {
    ...
    if (ret == CVI_SUCCESS) {
        ...
        meshDumpAttr.enModId          = (MOD_ID_E)6;
        meshDumpAttr.vpssMeshAttr.grp = VpssGrp;
        meshDumpAttr.vpssMeshAttr.chn = VpssChn;
        ret = CVI_GDC_LoadMeshWithBuf(&meshDumpAttr, &stLDCAttr, ptBinaryData->pu8Buffer,
                                      ptBinaryData->u32Size);
        if (ret != CVI_SUCCESS) {
            ISP_DAEMON2_DEBUG_EX(LOG_ALERT, "(VPSS) CVI_GDC_LoadMeshWithBuf fail");
            CVI_ISPD2_Utils_ComposeAPIErrorMessageEX(ptContentOut,
                                                     "(VPSS) CVI_GDC_LoadMeshWithBuf fail");
        }
    } else {
                                            ...
                                            JSONObject         *pJsonResponse)
{
    ...
    if (CVI_VPSS_GetChnLDCAttr(ptDevInfo->s32VpssGrp, ptDevInfo->s32VpssChn, &stAttr) !=
        CVI_SUCCESS) {
        CVI_ISPD2_Utils_ComposeAPIErrorMessage(ptContentOut);
        return CVI_FAILURE;
    }
    ...
    GET_INT_FROM_JSON(pstJsonObject, "/s32DistortionRatio", stAttr.stAttr.s32DistortionRatio,
                      s32Ret);

    if (CVI_VPSS_SetChnLDCAttr(ptDevInfo->s32VpssGrp, ptDevInfo->s32VpssChn, &stAttr) !=
        CVI_SUCCESS) {
        CVI_ISPD2_Utils_ComposeAPIErrorMessage(ptContentOut);
        return CVI_FAILURE;
    }
