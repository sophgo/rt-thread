...
#undef TIME_INTERVAL_PACKET_RECV
#define TIME_INTERVAL_PACKET_RECV (30)
#endif
...
#define CONNECTINFO_RESET_BINARY_MODE_STATUS(connectInfo)                                 \
    {                                                                                     \
        if (connectInfo->ptDaemonInfo->tDeviceInfo.bKeepBinaryInDataInfoOnce) {           \
            ...
            CVI_ISPD2_ResetBinaryInStructure(                                             \
                &(connectInfo->ptDaemonInfo->tDeviceInfo.tBinaryInData));                 \
        }                                                                                 \
    }

// -----------------------------------------------------------------------------
static void CVI_ISPD2_ES_CB_AllocBuffer(cvi_uv_handle_t *pHandle, size_t lSuggestedLength,
                                        cvi_uv_buf_t *pBuf)
{
    lSuggestedLength = MIN(MAX_CVI_UV_ALLOCATE_BUFFER_SIZE, lSuggestedLength);
    pBuf->base       = (char *)malloc(lSuggestedLength);
    pBuf->len        = (pBuf->base != NULL) ? lSuggestedLength : 0;

    // ISP_DAEMON2_DEBUG_EX(LOG_DEBUG, "UV allocate buffer suggested length : %d",
    // lSuggestedLength);
                                             ...
                                             TISPDaemon2ConnectInfo *ptConnectObj)
{
    ...
    bBinaryMode = bBinaryMode && ((ptBinaryInData->eDataState == EBINARYSTATE_INITIAL) ||
    ...
    if (!bBinaryMode) {
        ...
        eDataState = CVI_ISPD2_Message_CommandChecker(
            pstPacketInfo, pszBuf, pstPacketInfo->u32CurPacketReadOffset, u32ContentTailPos);
        if (eDataState == PACKET_RECEIVE_FINISH) {
            ...
            ptConnectObj->u32RecvBufferOffset =
                u32ContentTotalSize - pstPacketInfo->u32CurPacketSize;
            memcpy(pszBuf, pszBuf + pstPacketInfo->u32CurPacketSize,
                   ptConnectObj->u32RecvBufferOffset);
            CONNECTINFO_RESET_BINARY_MODE_STATUS(ptConnectObj);
        } else {
        ...
        if (ptBinaryInData->pu8Buffer == CVI_NULL) {
            ...
            if (ptBinaryInData->u32Size == (ptBinaryInData->u32RecvSize + u32ContentTotalSize)) {
                ptBinaryInData->eDataState = EBINARYSTATE_DONE;
                memcpy(ptBinaryInData->pu8Buffer + ptBinaryInData->u32RecvSize, pszBuf,
                       ptBinaryInData->u32Size - ptBinaryInData->u32RecvSize);
                ...
                CONNECTINFO_RESET_RECV_BUFFER_STATUS(ptConnectObj);
                CONNECTINFO_RESET_BINARY_MODE_STATUS(ptConnectObj);
            } else if (ptBinaryInData->u32Size >
                       (ptBinaryInData->u32RecvSize + u32ContentTotalSize)) {
                ptBinaryInData->eDataState = EBINARYSTATE_RECV;
                memcpy(ptBinaryInData->pu8Buffer + ptBinaryInData->u32RecvSize, pszBuf,
                       u32ContentTotalSize);
                                              ...
                                              TISPDaemon2ConnectInfo *ptConnectObj)
{
    ...
    if ((ptConnectObj->u32RecvBufferOffset > 0) &&
        (tvCurrentTime.tv_sec >=
         (ptConnectObj->tvLastRecvTime.tv_sec + TIME_INTERVAL_PACKET_RECV))) {
    ...
    if (u32AvailableRecvBufSize < u32BufferLen) {
        ISP_DAEMON2_DEBUG(LOG_WARNING,
                          "No avaiable memory for socket command (Clean current buffer content)");
        CONNECTINFO_RESET_RECV_BUFFER_STATUS(ptConnectObj);
        CONNECTINFO_RESET_BINARY_MODE_STATUS(ptConnectObj);
        return;
    }
...
// -----------------------------------------------------------------------------
static void CVI_ISPD2_ES_CB_SocketRead(cvi_uv_stream_t *pUVClientHandle, ssize_t lReadLength,
                                       const cvi_uv_buf_t *pBuf)
{
...
static void CVI_ISPD2_ES_CB_NewConnection(cvi_uv_stream_t *pUVServerHandle, int iStatus)
{
    ...
    if (cvi_uv_accept(pUVServer, (cvi_uv_stream_t *)&(pUVClientEx->UVTcp)) == 0) {
        ...
        cvi_uv_read_start((cvi_uv_stream_t *)pUVClientEx, CVI_ISPD2_ES_CB_AllocBuffer,
                          CVI_ISPD2_ES_CB_SocketRead);
    } else {
...
CVI_S32 CVI_ISPD2_ES_CreateService(TISPDaemon2Info *ptObject, uint16_t u16ServicePort)
{
    ...
    iListenRes = cvi_uv_listen((cvi_uv_stream_t *)(&(ptObject->UVServerEx)), DEFAULT_BACKLOG,
                               CVI_ISPD2_ES_CB_NewConnection);
    if (iListenRes != 0) {
...
static void *CVI_ISPD2_ES_ServiceThread(void *hHandle)
{
    ...
    while (ptObject->bServiceThreadRunning) {
        cvi_uv_run(ptObject->pUVLoop, CVI_UV_RUN_NOWAIT);
        usleep(TIME_INTERVAL_CVI_UV_RUN * 1000);
    }
...
CVI_S32 CVI_ISPD2_ES_RunService(TISPDaemon2Info *ptObject)
{
    ...
    pthread_create(&(ptObject->thDaemonThreadId), &attr, CVI_ISPD2_ES_ServiceThread,
                   (void *)ptObject);
    // pthread_setname_np(ptObject->thDaemonThreadId, "ISPD2Service");
    return CVI_SUCCESS;
}
...
{
    void *pBuf = malloc(MAX_CVI_UART_ALLOCATE_BUFFER_SIZE);
    int   len  = (pBuf != NULL) ? MAX_CVI_UART_ALLOCATE_BUFFER_SIZE : 0;

    memset(pBuf, 0, len);
    TISPDaemon2Info *ptObject = ptConnectObj->ptDaemonInfo;

    int recv_len = receive_by_uart(pBuf, len, 200);

    if (strcmp(pBuf, "q") == 0) {  // forced stop by typing 'q' using uart tool
        ptObject->bServiceThreadRunning = CVI_FALSE;
    }
...
static void *CVI_ISPD2_ES_ServiceThread_Uart(void *hHandle)
{
    ...
    ptObject->isConnectionByUart = CVI_FALSE;
    uart_deinit_pqtool();
    console_restart(0, 115200, 512);
    usleep(200 * 1000);
    printf("exit from pqtool of uart.\n");
    printf("you can type your cmd now.\n");
...
CVI_S32 CVI_ISPD2_ES_RunService_Uart(TISPDaemon2Info *ptObject)
{
    ...
    pthread_create(&(ptObject->thDaemonThreadId), &attr, CVI_ISPD2_ES_ServiceThread_Uart,
                   (void *)ptObject);
    // pthread_setname_np(&(ptObject->thDaemonThreadId), "ISPD2ServiceUart");
