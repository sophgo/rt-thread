...
CVI_S32 CVI_ISPD2_Message_RegisterHandler(TISPD2HandlerInfo *ptHandlerInfo,
                                          JSONRpcCBFunction pcbFunction, const char *pszName,
                                          void *pvData)
{
    if (ptHandlerInfo->ptHandler == NULL) {
        ptHandlerInfo->ptHandler =
            (TISPD2Handler *)calloc(DEFAULT_RPC_HANDLER_COUNT, sizeof(TISPD2Handler));
        if (ptHandlerInfo->ptHandler == NULL) {
            return CVI_FAILURE;
        }
    ...
    if (ptHandlerInfo->u32Count == ptHandlerInfo->u32MaxCount) {
        ...
        pTemp =
            (TISPD2Handler *)realloc(ptHandlerInfo->ptHandler, u32NewCount * sizeof(TISPD2Handler));
        if (pTemp == NULL) {
            return CVI_FAILURE;
        }
...
static CVI_S32 CVI_ISPD2_Message_ComposeResponse(JSONObject *pJsonResponse,
                                                 JSONObject *pJsonContent, CVI_S32 s32StatusCode,
                                                 CVI_S32 s32Id)
{
    ISPD2_json_object_object_add(pJsonResponse, "jsonrpc",
                                 ISPD2_json_object_new_string(JSONRPC_VERSION));
...
static CVI_S32 CVI_ISPD2_Message_ComposeErrorResponse(JSONObject *pJsonResponse,
                                                      CVI_S32 s32ErrorCode, const char *pszMessage,
                                                      CVI_S32 s32Id)
{
    ISPD2_json_object_object_add(pJsonResponse, "jsonrpc",
                                 ISPD2_json_object_new_string(JSONRPC_VERSION));
    ...
    if (pszMessage) {
        ISPD2_json_object_object_add(pJsonError, "message",
                                     ISPD2_json_object_new_string(pszMessage));
    }
...
// -----------------------------------------------------------------------------
static CVI_S32 CVI_ISPD2_Message_CheckValidAndGetInfo(JSONObject *pJsonRoot, char *pszErrorMessage,
                                                      CVI_U32   u32MaxMessageLen,
                                                      CVI_BOOL *pbNotifyMode, CVI_U32 *pu32Id)
{
...
// -----------------------------------------------------------------------------
static CVI_S32 CVI_ISPD2_Message_HandleSingleRequest(
    JSONObject *pJsonRoot, JSONObject *pJsonResponse, const TISPD2HandlerInfo *ptHandlerInfo,
    TISPDeviceInfo *ptDeviceInfo, CVI_BOOL *pbSkipJSONRPCResponse, TJSONRpcBinaryOut *ptBinaryData)
{
    ...
    *pbSkipJSONRPCResponse = CVI_FALSE;
    ...
    s32CheckCode =
        CVI_ISPD2_Message_CheckValidAndGetInfo(pJsonRoot, szErrorMessage, 64, &bNotifyMode, &u32Id);
    if (s32CheckCode != JSONRPC_CODE_OK) {
        CVI_ISPD2_Message_ComposeErrorResponse(pJsonResponse, s32CheckCode, szErrorMessage, u32Id);
        return CVI_SUCCESS;
    }

    iRes = ISPD2_json_pointer_get(pJsonRoot, "/method", &pJsonMethodPtr);
    if (iRes != 0) {
        CVI_ISPD2_Message_ComposeErrorResponse(pJsonResponse, JSONRPC_CODE_PARSE_ERROR,
                                               "Parse Error (Method)", u32Id);
        return CVI_SUCCESS;
    }
    ...
    for (CVI_U32 u32Idx = 0; u32Idx < ptHandlerInfo->u32Count; ++u32Idx, ptHandlerAddr++) {
        if (u32MethodLen != ptHandlerAddr->u32NameLen) continue;
    ...
    if (!bMethodMatch) {
        CVI_ISPD2_Message_ComposeErrorResponse(pJsonResponse, JSONRPC_CODE_METHOD_NOT_FOUND,
                                               "Method Not Found", u32Id);
...
static void CVI_ISPD2_Message_SendJsonMessage(int iFd, JSONObject *pJsonObj)
{
    ...
    size_t      u32ContentLength = 0;  // test Evan :compare size_t and CVI_U32
    CVI_U32     u32WriteLen      = 0;
    ssize_t     lWriteLen        = 0;

    pszJsonContent = ISPD2_json_object_to_json_string_length(pJsonObj, JSON_C_TO_STRING_SPACED,
                                                             (size_t *)&u32ContentLength);
    while (u32WriteLen < u32ContentLength) {
        CVI_U32 u32WriteDataLen =
            MIN(u32ContentLength - u32WriteLen, MESSAGE_PARTICAL_WRITE_LENGTH);
...
static void CVI_ISPD2_Message_HandleJsonObject(JSONObject *pJsonRoot, int iFd,
                                               const TISPD2HandlerInfo *ptHandlerInfo,
    ...
    if (iIsJsonArray) {
        ...
        for (CVI_U32 u32Idx = 0; u32Idx < ISPD2_json_object_array_length(pJsonRoot); ++u32Idx) {
            ...
            ISP_DAEMON2_DEBUG_EX(
                LOG_DEBUG, "\t[%u]=%s", u32Idx,
                ISPD2_json_object_to_json_string_ext(pObj, JSON_C_TO_STRING_SPACED));

            CVI_ISPD2_Message_HandleSingleRequest(pObj, pJsonSubResponse, ptHandlerInfo,
                                                  ptDeviceInfo, &bSkipJSONRPCResponseTemp,
                                                  &tBinaryOut);
        ...
        CVI_ISPD2_Message_HandleSingleRequest(pJsonRoot, pJsonResponse, ptHandlerInfo, ptDeviceInfo,
                                              &bSkipJSONRPCResponse, &tBinaryOut);
    ...
    do {
        ...
        if (pJsonTok->err != ISPD2_json_tokener_success) {
            ISP_DAEMON2_DEBUG_EX(LOG_DEBUG, "JSON parsing error: %s (offset : %d)",
                                 ISPD2_json_tokener_error_desc(pJsonTok->err),
                                 pJsonTok->char_offset);

            pJsonResponse = ISPD2_json_object_new_object();
            CVI_ISPD2_Message_ComposeErrorResponse(pJsonResponse, JSONRPC_CODE_PARSE_ERROR,
                                                   "Parse Error (Content)", -1);
            CVI_ISPD2_Message_SendJsonMessage(iFd, pJsonResponse);
            ISPD2_json_object_put(pJsonResponse);
            pJsonResponse = NULL;
            break;
                                       ...
                                       TISPDeviceInfo          *ptDeviceInfo)
{
    ...
    case EBINARYDATA_VI_LDC_BIN_DATA:
        ...
        CVI_ISPD2_CBFunc_VI_LDCBinData(ViPipe, ViChn, ptBinaryData, &tContentOut,
                                       pJsonDataResponse);
        break;
    case EBINARYDATA_VPSS_LDC_BIN_DATA:
        VpssGrp = ptDeviceInfo->s32VpssGrp;
        VpssChn = ptDeviceInfo->s32VpssChn;
        CVI_ISPD2_CBFunc_VPSS_LDCBinData(VpssGrp, VpssChn, ptBinaryData, &tContentOut,
                                         pJsonDataResponse);
        break;
    default:
...
// -----------------------------------------------------------------------------
EPacketState CVI_ISPD2_Message_CommandChecker(TPACKETINFO *pstPacketInfo, const char *pszBuf,
                                              CVI_U32 u32CheckStart, CVI_U32 u32CheckTail)
{
