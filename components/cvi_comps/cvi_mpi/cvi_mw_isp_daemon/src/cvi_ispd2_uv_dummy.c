...
int cvi_uv_accept(cvi_uv_stream_t *server, cvi_uv_stream_t *client)
{
    ...
    pUVClientEx->UVTcp.sockFd =
        accept(pUVServerEx->UVTcp.sockFd, (struct sockaddr *)&connAddr, &addr_len);
        ...
        if (ret > 0) {
            if (FD_ISSET(socket_id, &readFd)) {
                ...
                if (ret > 0) {
                    cb->pBuf.len = ret;
                    if (cb->read_cb) {
                        cb->read_cb(cb->handle, cb->pBuf.len,
                                    &(cb->pBuf));  // CVI_ISPD2_ES_CB_SocketRead
                    }
...
int cvi_uv_read_start(cvi_uv_stream_t *stream, cvi_uv_alloc_cb alloc_cb, cvi_uv_read_cb read_cb)
{
    ...
    ret = pthread_create(&id, &attr, cvi_uv_read, (void *)cb);

    if (ret != 0) {
        ISP_DAEMON2_DEBUG(LOG_DEBUG, "create error\n");
        return -1;
    }
    // pthread_setname_np(id, "cvi_uv_read");
    pthread_detach(id);
