...
#define STRFY(s) #s
#define ADDR_ALIGN_SIZE 64
#define MEMORY_BARRIER() __asm__ volatile ("" ::: "memory")
#define IVE_DEFAULT_ALIGN 32
...
{
	int alloc_size;
	alloc_size = size + ADDR_ALIGN_SIZE + sizeof(void*);
	void *ptr = malloc(alloc_size);
	memset((CVI_U8 *)ptr, 0x0, alloc_size);
	if (ptr) {
		void *aligned_ptr = (void*)(((size_t)ptr + ADDR_ALIGN_SIZE - 1) & ~(ADDR_ALIGN_SIZE - 1));
		if (aligned_ptr == ptr)
			aligned_ptr = ptr + ADDR_ALIGN_SIZE;
		*phy_addr = (CVI_U64)aligned_ptr;
		((void**)aligned_ptr)[-1] = ptr;
		CVI_SYS_IonFlushCache((CVI_U64)aligned_ptr, NULL, size);
		// printf("malloc aligned size=%ld, size=%d, aligned addr=%p, addr=%p\n",size, alloc_size, aligned_ptr, ptr);
		return CVI_SUCCESS;

	} else {
		printf("malloc aligned aos memrory fail\n");
		return CVI_FAILURE;
	}
...
{
	if (phy_addr) {
		void *original_ptr = ((void**)phy_addr)[-1];
		// printf("free aligned addr=%p, addr=%p\n",phy_addr, original_ptr);
		free(original_ptr);
		return CVI_SUCCESS;
	} else {
		return CVI_FAILURE;
			 ...
			 unsigned char *dst, int imw, int imh, int stride)
{
	...
	unsigned char *visited =
		(unsigned char *)malloc(imh * stride * sizeof(unsigned char));
	unsigned char *pdst =
		(unsigned char *)malloc(imh * stride * sizeof(unsigned char));
...
CVI_S32 CVI_IVE_CompareIveData(IVE_DATA_S *pstData1, IVE_DATA_S *pstData2)
{
	...
	size = pstData1->u32Stride * pstData1->u32Height;
	...
	printf("compare A at %p, B at %p\n",
		   (char *)pstData1->u64VirAddr,
		   (char *)pstData2->u64VirAddr);

	printf("data1 stride=%d, data2 stride=%d\n", pstData1->u32Stride, pstData2->u32Stride);
	CVI_SYS_IonInvalidateCache(pstData1->u64VirAddr, NULL, size);
	for (y = 0; y < pstData1->u32Height; y++) {
		n = memcmp((char *)pstData1->u64VirAddr + y * pstData1->u32Stride,
		(char *)pstData2->u64VirAddr + y * pstData2->u32Stride,
		pstData1->u32Width);
		if (n != 0) {
			printf("Not same content, line num=%d\n", y);
			return CVI_FAILURE;
		}
	}
...
CVI_IVE_CompareIveMem(IVE_MEM_INFO_S *pstMem1, IVE_MEM_INFO_S *pstMem2)
{
	int n, size_len;

	if ((char *)pstMem1->u64VirAddr == NULL || (char *)pstMem2->u64VirAddr == NULL) {
		printf("invalid address\n");
		return CVI_ERR_IVE_INVALID_DEVID;
	}

	size_len = pstMem1->u32Size;
	CVI_SYS_IonInvalidateCache(pstMem1->u64PhyAddr, NULL, size_len);
	printf("compare A at %p, B at %p\n",
		   (char *)pstMem1->u64VirAddr,
		   (char *)pstMem2->u64VirAddr);
	n = memcmp((char *)pstMem1->u64VirAddr, (char *)pstMem2->u64VirAddr, pstMem2->u32Size);
	if (n != 0) {
...
CVI_S32 CVI_IVE_CompareIveImage(IVE_IMAGE_S *pstImage1, IVE_IMAGE_S *pstImage2)
{
	...
	u32Stride1 = pstImage1->u32Stride[0];
	u32Stride2 = pstImage2->u32Stride[0];//WidthAlign(u32Width, IVE_DEFAULT_ALIGN);
	size_len = u32Stride1 * u32Height;
	s32Succ = CVI_SUCCESS;
	CVI_SYS_IonInvalidateCache(pstImage1->u64PhyAddr[0], NULL, size_len);
	switch (enType) {
	case IVE_IMAGE_TYPE_U8C1:
	case IVE_IMAGE_TYPE_S8C1: {
		pData1 = (CVI_U8 *)pstImage1->u64VirAddr[0];
		pData2 = (CVI_U8 *)pstImage2->u64VirAddr[0];
		printf("compare A at %p, B at %p\n", pData1, pData2);
		for (y = 0; y < u32Height;
			 y++, pData1 += u32Stride1, pData2 += u32Stride2) {
	...
	case IVE_IMAGE_TYPE_YUV420SP: {
		...
		pData1 = (CVI_U8 *)pstImage1->u64VirAddr[1];
		pData2 = (CVI_U8 *)pstImage2->u64VirAddr[1];
	...
	case IVE_IMAGE_TYPE_U8C3_PLANAR: {
		for (int i = 0; i < 3; i++) {
			char YUVStr[4] = { 'Y', 'U', 'V' };
			pData1 = (CVI_U8 *)pstImage1->u64VirAddr[i];
			pData2 = (CVI_U8 *)pstImage2->u64VirAddr[i];
				...
				IVE_SAD_MODE_E mode, CVI_BOOL isDMAhalf)
{
	...
	u32Stride1 = pstImage1->u32Stride[0];
	u32Stride2 = pstImage2->u32Stride[0];
	s32Succ = CVI_SUCCESS;
	CVI_SYS_IonInvalidateCache(pstImage1->u64PhyAddr[0], NULL, u32Stride1 * pstImage1->u32Height);
	switch (enType) {
	case IVE_IMAGE_TYPE_S16C1:
	case IVE_IMAGE_TYPE_U16C1:
		u32ByteSize = 2;
	...
	printf("SAD Width %u Height %u Stride %u %u\n", u32SADWidth,
		   u32SADHeight, u32SADStride, u32Stride1);
	...
	case IVE_IMAGE_TYPE_U16C1: {
		...
		for (y = 0; y < u32SADHeight;
			 y++, pData1 += u32SADStride, pData2 += u32Stride2) { //dst=96 ref=384
...
CVI_S32 CVI_IVE_VideoFrameInfo2Image(VIDEO_FRAME_INFO_S *pstVFISrc, IVE_IMAGE_S *pstIIDst)
{
	...
	for (CVI_U32 i = 0; i < u32Channel; i++) {
		pstIIDst->u64VirAddr[i] = (CVI_U64)pstVFSrc->pu8VirAddr[i];
		...
		//, CVI_BOOL invertPackage)
{
	...
	for (CVI_U32 i = 0; i < u32Channel; i++) {
		pstVFSrc->pu8VirAddr[i] = (CVI_U8 *)pstIISrc->u64VirAddr[i];
			 ...
			 CVI_U16 u32Height)
{
	...
	int readCnt =
		aos_read(fd, (char *)pstData->u64VirAddr, uSize);

	if (readCnt == 0) {
		printf("Image %s read failed.\n", filename);
		return ERR_IVE_READ_FILE;
	}
	CVI_SYS_IonFlushCache(pstData->u64VirAddr, NULL, readCnt);
	aos_close(fd);
	return s32Succ;
}
				  ...
				  char *pBuffer, CVI_U16 u32Width, CVI_U16 u32Height)
{
	...
	for (size_t j = 0; j < (size_t)u32Height; j++) {
		memcpy((char *)(pstData->u64VirAddr +
			...
			const char *filename, CVI_U32 uSize)
{
	...
	size_t readCnt =
		aos_read(fd, (char *)pstMem->u64VirAddr, uSize);
	if (readCnt == 0) {
		printf("Image %s read failed.\n", filename);
		return ERR_IVE_READ_FILE;
	}

	CVI_SYS_IonFlushCache(pstMem->u64VirAddr, NULL, readCnt);
	aos_close(fd);
	return s32Succ;
}
				 ...
				 char *pBuffer, CVI_U32 uSize)
{
	...
	memcpy((char *)pstMem->u64VirAddr, (void *)pBuffer, uSize);
	return s32Succ;
}
				   ...
				   CVI_U16 u32Width, CVI_U16 u32Height)
{
	...
	if (enType == IVE_IMAGE_TYPE_U8C3_PLANAR) {
		...
		for (j = 0; j < u32Height; j++) {
			memcpy((char *)(pstImg->u64VirAddr[0] +
						   (j * pstImg->u32Stride[0])),
				   ptr, u32Width);
			ptr += u32Width;
		}
		for (j = 0; j < pstImg->u32Height; j++) {
			memcpy((char *)(pstImg->u64VirAddr[1] +
						   (j * pstImg->u32Stride[1])),
				   ptr, u32Width);
			ptr += u32Width;
		}
		for (j = 0; j < pstImg->u32Height; j++) {
			memcpy((char *)(pstImg->u64VirAddr[2] +
						   (j * pstImg->u32Stride[2])),
				   ptr, u32Width);
			ptr += u32Width;
		}
		CVI_SYS_IonFlushCache(pstImg->u64VirAddr[0], NULL, pstImg->u32Stride[0] * u32Height * 3);
		...
		for (i = 0; i < u32Height; i++) {
			...
			for (j = 0; j < u32Width; j++) {
				...
				((char *)pstImg->u64VirAddr[0])[img_idx] = ptr[buf_idx];
				((char *)pstImg->u64VirAddr[0])[img_idx + 1] = ptr[buf_idx + 1];
				((char *)pstImg->u64VirAddr[0])[img_idx + 2] = ptr[buf_idx + 2];
			}
		}
		CVI_SYS_IonFlushCache(pstImg->u64VirAddr[0], NULL, pstImg->u32Stride[0] * u32Height);
		...
		for (j = 0; j < pstImg->u32Height / 2; j++) {
			memcpy((char *)(pstImg->u64VirAddr[1] +
						   (j * pstImg->u32Stride[1])),
				   ptr, u32Width);
			ptr += u32Width;
		}
		CVI_SYS_IonFlushCache(pstImg->u64VirAddr[0], NULL, pstImg->u32Stride[0] * u32Height * 3/2);
		...
		CVI_SYS_IonFlushCache(pstImg->u64VirAddr[0], NULL, pstImg->u32Stride[0] * u32Height * 2);
			  ...
			  CVI_U16 u32Width, CVI_U16 u32Height)
{
	...
	if (desiredNChannels > 0) {
		...
		size_t readCnt = aos_read(fd, buffer, buf_size);
		...
		CVI_SYS_IonFlushCache((CVI_U64)buffer, NULL, readCnt);
		// fclose(fp);
...
IVE_IMAGE_S CVI_IVE_ReadImage(IVE_HANDLE pIveHandle, const char *filename, IVE_IMAGE_TYPE_E enType)
{
	...
	//UNUSED(filename);
	//UNUSED(enType);
	//printf("not support ReadImage direct\n");
...
CVI_S32 CVI_IVE_ResetImage(IVE_IMAGE_S *pstImage, CVI_U8 val)
{
	...
	case IVE_IMAGE_TYPE_U8C1:
	case IVE_IMAGE_TYPE_S8C1: {
		pData = (CVI_U8 *)pstImage->u64VirAddr[0];
	...
	case IVE_IMAGE_TYPE_YUV420SP: {
		...
		pData = (CVI_U8 *)pstImage->u64VirAddr[1];
	...
	case IVE_IMAGE_TYPE_U8C3_PLANAR: {
		for (int i = 0; i < 3; i++) {
			pData = (CVI_U8 *)pstImage->u64VirAddr[i];
			for (y = 0; y < pstImage->u32Height;
				  ...
				  CVI_U32 u32ByteSize)
{
	...
	int s32Ret = CVI_Aligned_Aos_Malloc((CVI_U64 *)&pstMemInfo->u64PhyAddr,
										u32ByteSize);

	pstMemInfo->u64VirAddr = pstMemInfo->u64PhyAddr;
				   ...
				   CVI_U16 u32Width, CVI_U16 u32Height)
{
	...
	/*for alios ion addr align*/
	len = pstDataInfo->u32Stride * u32Height;
	int s32Ret = CVI_Aligned_Aos_Malloc((CVI_U64 *)&pstDataInfo->u64PhyAddr, len);

	pstDataInfo->u64VirAddr = pstDataInfo->u64PhyAddr;
				...
				uint16_t u32Height, CVI_BOOL bCached)
{
	...
	s32Ret = CVI_Aligned_Aos_Malloc(&pstImg->u64PhyAddr[0], u32Len);

	if (s32Ret) {
		printf("CVI_Aligned_Aos_Malloc failed with %#x\n", s32Ret);
		return CVI_FAILURE;
	}
	/*for alios ion addr align*/
	// if (pstImg->u64PhyAddr[0] % 16 != 0) {
	// 	pstImg->bPhyAddrAlign = true;
	// 	pstImg->u64PhyAddr[0] += 8;
	// } else {
	// 	pstImg->bPhyAddrAlign = false;
	// }


	for (i = 1; i < u32Channel; i++) {
	...
	for (i = u32Channel; i < 3; i++) {
		//pstImg->u64VirAddr[i] = 0;
		pstImg->u64PhyAddr[i] = -1;
	}

	/*for alios  viraddr = phyaddr*/
	for (i = 0; i < 3; i++) {
		pstImg->u64VirAddr[i] = pstImg->u64PhyAddr[i];
	}
...
CVI_S32 _CVI_IVE_BufFlush_Request(IVE_HANDLE pIveHandle, IVE_IMAGE_S *pstImg, CVI_BOOL bFlush)
{
	...
	if (bFlush)
		return CVI_SYS_IonFlushCache(pstImg->u64PhyAddr[0], NULL, u32Len);
	return CVI_SYS_IonInvalidateCache(pstImg->u64PhyAddr[0], NULL, u32Len);
}
...
CVI_S32 _CVI_IVE_Write(const char *filename, CVI_U64 u64VirAddr, CVI_U32 len)
{
	int fd;
	int readCnt = 0;
	CVI_U32 size_len = len;

	fd = aos_open(filename, O_WRONLY | O_CREAT);
	if (fd < 0) {
		printf("Can't open %s\n", filename);
		return ERR_IVE_OPEN_FILE;
	}

	readCnt = aos_write(fd, (void *)u64VirAddr, size_len);
	if (readCnt == 0) {
...
{
	UNUSED(pIveHandle);
	// if (pstMem->bPhyAddrAlign)
	// 	return CVI_SYS_IonFree(pstMem->u64PhyAddr - 8, NULL);

	return CVI_Aligned_Aos_Free((void *)pstMem->u64PhyAddr);
}

CVI_S32 CVI_SYS_FreeI(IVE_HANDLE pIveHandle, IVE_IMAGE_S *pstImg)
{
	UNUSED(pIveHandle);
	// if (pstImg->bPhyAddrAlign)
	// 	return CVI_SYS_IonFree(pstImg->u64PhyAddr[0] - 8, NULL);

	return CVI_Aligned_Aos_Free((void *)pstImg->u64PhyAddr[0]);
}

CVI_S32 CVI_SYS_FreeD(IVE_HANDLE pIveHandle, IVE_DATA_S *pstData)
{
	UNUSED(pIveHandle);
	// if (pstData->bPhyAddrAlign)
	// 	return CVI_SYS_IonFree(pstData->u64PhyAddr - 8, NULL);

	return CVI_Aligned_Aos_Free((void *)pstData->u64PhyAddr);
}
				 ...
				 IVE_DST_IMAGE_S *pstFrmDiffFg)
{
	...
	for (i = 0; i < size; i++) {
		((char *)pstBgDiffFg->u64VirAddr[0])[i] = ((char *)pstDiffFg->u64VirAddr[0])[i * 2];
		((char *)pstFrmDiffFg->u64VirAddr[0])[i] = ((char *)pstDiffFg->u64VirAddr[0])[i * 2 + 1];
	}
	return CVI_SUCCESS;
}
				 ...
				 IVE_DST_IMAGE_S *pstChStaLift)
{
	...
	for (i = 0; i < size; i++) {
		((char *)pstChgStaImg->u64VirAddr[0])[i] = ((char *)pstChgSta->u64VirAddr[0])[i * 4];//0
		((char *)pstChgStaFg->u64VirAddr[0])[i] = ((char *)pstChgSta->u64VirAddr[0])[i * 4 + 1];//1
		((char *)pstChStaLift->u64VirAddr[0])[i * 2] = ((char *)pstChgSta->u64VirAddr[0])[i * 4 + 2];//2
		((char *)pstChStaLift->u64VirAddr[0])[i * 2 + 1] = ((char *)pstChgSta->u64VirAddr[0])[i * 4 + 3];//3
	}
	return CVI_SUCCESS;
}
...
CVI_S32 CVI_IVE_DUMP(IVE_HANDLE pIveHandle)
{
	...
	cvi_ive_ioctl(CVI_IVE_IOC_DUMP, NULL);
	return CVI_SUCCESS;
}
...
CVI_S32 CVI_IVE_RESET(IVE_HANDLE pIveHandle, int s)
{
	...
	cvi_ive_ioctl(CVI_IVE_IOC_RESET, &s);
	return CVI_SUCCESS;
}
			  ...
			  CVI_BOOL bBlock)
{
	...
	cvi_ive_ioctl(CVI_IVE_IOC_QUERY, &ive_arg);
	return CVI_SUCCESS;
}
			...
			CVI_BOOL bInstant)
{
	...
	cvi_ive_ioctl(CVI_IVE_IOC_DMA, &ive_arg);
	return 0;
}
			...
			CVI_BOOL bInstant)
{
	...
	cvi_ive_ioctl(CVI_IVE_IOC_And, &ive_arg);
	return 0;
}
		   ...
		   CVI_BOOL bInstant)
{
	...
	cvi_ive_ioctl(CVI_IVE_IOC_Or, &ive_arg);
	return 0;
}
			...
			CVI_BOOL bInstant)
{
	...
	cvi_ive_ioctl(CVI_IVE_IOC_Xor, &ive_arg);
	return 0;
}
			...
			IVE_ADD_CTRL_S *pstCtrl, CVI_BOOL bInstant)
{
	...
	if (cvi_ive_ioctl(CVI_IVE_IOC_Add, &ive_arg) < 0) {
			...
			IVE_SUB_CTRL_S *pstCtrl, CVI_BOOL bInstant)
{
	...
	cvi_ive_ioctl(CVI_IVE_IOC_Sub, &ive_arg);
	return 0;
}
			  ...
			  CVI_BOOL bInstant)
{
	...
	cvi_ive_ioctl(CVI_IVE_IOC_Erode, &ive_arg);
	return 0;
}
			   ...
			   CVI_BOOL bInstant)
{
	...
	cvi_ive_ioctl(CVI_IVE_IOC_Dilate, &ive_arg);
	return 0;
}
			   ...
			   CVI_BOOL bInstant)
{
	...
	cvi_ive_ioctl(CVI_IVE_IOC_Thresh, &ive_arg);
	return 0;
}
				 ...
				 CVI_BOOL bInstant)
{
	...
	cvi_ive_ioctl(CVI_IVE_IOC_MatchBgModel, &ive_arg);
	return 0;
}
				  ...
				  CVI_BOOL bInstant)
{
	...
	cvi_ive_ioctl(CVI_IVE_IOC_UpdateBgModel, &ive_arg);
	return 0;
}
			...
			CVI_BOOL bInstant)
{
	...
	cvi_ive_ioctl(CVI_IVE_IOC_GMM, &ive_arg);
	return 0;
}
			 ...
			 CVI_BOOL bInstant)
{
	...
	cvi_ive_ioctl(CVI_IVE_IOC_GMM2, &ive_arg);
	return 0;
}
			...
			CVI_BOOL bInstant)
{
	...
	cvi_ive_ioctl(CVI_IVE_IOC_Bernsen, &ive_arg);
	return 0;
}
			   ...
			   CVI_BOOL bInstant)
{
	...
	cvi_ive_ioctl(CVI_IVE_IOC_Filter, &ive_arg);
	return 0;
}
			  ...
			  IVE_SOBEL_CTRL_S *pstCtrl, CVI_BOOL bInstant)
{
	...
	cvi_ive_ioctl(CVI_IVE_IOC_Sobel, &ive_arg);
	return 0;
}
			  ...
			  IVE_MAG_AND_ANG_CTRL_S *pstCtrl, CVI_BOOL bInstant)
{
	...
	cvi_ive_ioctl(CVI_IVE_IOC_MagAndAng, &ive_arg);
	return 0;
}
			...
			CVI_BOOL bInstant)
{
	...
	cvi_ive_ioctl(CVI_IVE_IOC_CSC, &ive_arg);
	return 0;
}
				 ...
				 CVI_BOOL bInstant)
{
	...
	cvi_ive_ioctl(CVI_IVE_IOC_FilterAndCSC, &ive_arg);
	return 0;
}
			 ...
			 IVE_DST_MEM_INFO_S *pstDst, CVI_BOOL bInstant)
{
	...
	cvi_ive_ioctl(CVI_IVE_IOC_Hist, &ive_arg);
	return 0;
}
			...
			IVE_MAP_CTRL_S *pstCtrl, CVI_BOOL bInstant)
{
	...
	cvi_ive_ioctl(CVI_IVE_IOC_Map, &ive_arg);
	return 0;
}
			...
			CVI_BOOL bInstant)
{
	...
	cvi_ive_ioctl(CVI_IVE_IOC_NCC, &ive_arg);
	return 0;
}
			  ...
			  CVI_BOOL bInstant)
{
	...
	cvi_ive_ioctl(CVI_IVE_IOC_Integ, &ive_arg);
	return 0;
}
			...
			CVI_BOOL bInstant)
{
	...
	ive_arg.stCtrl = *pstCtrl;
	ive_arg.bInstant = bInstant;
	cvi_ive_ioctl(CVI_IVE_IOC_LBP, &ive_arg);
	return 0;
}
			   ...
			   IVE_THRESH_S16_CTRL_S *pstCtrl, CVI_BOOL bInstant)
{
	...
	cvi_ive_ioctl(CVI_IVE_IOC_Thresh_S16, &ive_arg);
	return 0;
}
			   ...
			   IVE_THRESH_U16_CTRL_S *pstCtrl, CVI_BOOL bInstant)
{
	...
	cvi_ive_ioctl(CVI_IVE_IOC_Thresh_U16, &ive_arg);
	return 0;
}
				...
				CVI_BOOL bInstant)
{
	...
	cvi_ive_ioctl(CVI_IVE_IOC_16BitTo8Bit, &ive_arg);
	return 0;
}
				  ...
				  CVI_BOOL bInstant)
{
	...
	cvi_ive_ioctl(CVI_IVE_IOC_OrdStatFilter, &ive_arg);
	return 0;
}
...
{
	UNUSED(pstStack);
	do_hysteresis_wo_ang((unsigned char *)pstEdge->u64VirAddr[0], (unsigned char *)pstEdge->u64VirAddr[0],
				 ...
				 CVI_BOOL bInstant)
{
	...
	cvi_ive_ioctl(CVI_IVE_IOC_CannyHysEdge, &ive_arg);
	return 0;
}
			 ...
			 IVE_NORM_GRAD_CTRL_S *pstCtrl, CVI_BOOL bInstant)
{
	...
	cvi_ive_ioctl(CVI_IVE_IOC_NormGrad, &ive_arg);
	return 0;
}
			   ...
			   CVI_BOOL bInstant)
{
	...
	cvi_ive_ioctl(CVI_IVE_IOC_GradFg, &ive_arg);
	return 0;
}
			...
			CVI_BOOL bInstant)
{
	...
	cvi_ive_ioctl(CVI_IVE_IOC_SAD, &ive_arg);
	return 0;
}
			   ...
			   CVI_BOOL bInstant)
{
	...
	cvi_ive_ioctl(CVI_IVE_IOC_Resize, &ive_arg);
	return 0;
}
				...
				CVI_BOOL bInstant)
{
	...
	cvi_ive_ioctl(CVI_IVE_IOC_imgInToOdma, &ive_arg);
	return 0;
}
					 ...
					 CVI_BOOL bInstant)
{
	...
	cvi_ive_ioctl(CVI_IVE_IOC_rgbPToYuvToErodeToDilate, &ive_arg);
	return 0;
}
				  ...
				  CVI_BOOL bInstant)
{
	...
	cvi_ive_ioctl(CVI_IVE_IOC_STCandiCorner, &ive_arg);
	return 0;
}
				...
				CVI_BOOL bInstant)
{
	...
	cvi_ive_ioctl(CVI_IVE_IOC_MD, &ive_arg);
	return 0;
}
...
CVI_S32 CVI_IVE_CMDQ(IVE_HANDLE pIveHandle)
{
	...
	cvi_ive_ioctl(CVI_IVE_IOC_CMDQ, NULL);
	return 0;
}
