...
CVI_S32 venc_create_enc_ctx(venc_enc_ctx *pEncCtx, CVI_VOID *pchnctx)
{
    ...
    case PT_H264:
        ...
        {
            pEncCtx->ext.vid.setInitCfgRc = pEncCtx->ext.vid.setInitCfgCbr;
            pEncCtx->base.bVariFpsEn      = prcatt->stH264Cbr.bVariFpsEn;
        }
        else if (pEncCtx->base.rcMode == RC_MODE_VBR)
        {
            pEncCtx->ext.vid.setInitCfgRc = pEncCtx->ext.vid.setInitCfgVbr;
            pEncCtx->base.bVariFpsEn      = prcatt->stH264Vbr.bVariFpsEn;
        }
        else if (pEncCtx->base.rcMode == RC_MODE_AVBR)
        {
            pEncCtx->ext.vid.setInitCfgRc = pEncCtx->ext.vid.setInitCfgAVbr;
            pEncCtx->base.bVariFpsEn      = prcatt->stH264AVbr.bVariFpsEn;
        }
        else if (pEncCtx->base.rcMode == RC_MODE_FIXQP)
        {
            pEncCtx->ext.vid.setInitCfgRc = pEncCtx->ext.vid.setInitCfgFixQp;
            pEncCtx->base.bVariFpsEn      = prcatt->stH264FixQp.bVariFpsEn;
        }
        else if (pEncCtx->base.rcMode == RC_MODE_UBR)
        {
            pEncCtx->ext.vid.setInitCfgRc = pEncCtx->ext.vid.setInitCfgUbr;
    ...
    case PT_H265:
        ...
        {
            pEncCtx->ext.vid.setInitCfgRc = pEncCtx->ext.vid.setInitCfgAVbr;
            pEncCtx->base.bVariFpsEn      = prcatt->stH265AVbr.bVariFpsEn;
        }
        else if (pEncCtx->base.rcMode == RC_MODE_FIXQP)
        {
            pEncCtx->ext.vid.setInitCfgRc = pEncCtx->ext.vid.setInitCfgFixQp;
            pEncCtx->base.bVariFpsEn      = prcatt->stH265FixQp.bVariFpsEn;
        }
        else if (pEncCtx->base.rcMode == RC_MODE_QPMAP)
        {
            pEncCtx->ext.vid.setInitCfgRc = pEncCtx->ext.vid.setInitCfgQpMap;
            pEncCtx->base.bVariFpsEn      = prcatt->stH265QpMap.bVariFpsEn;
        }
        else if (pEncCtx->base.rcMode == RC_MODE_UBR)
        {
            pEncCtx->ext.vid.setInitCfgRc = pEncCtx->ext.vid.setInitCfgUbr;
...
static CVI_S32 jpege_open(CVI_VOID *handle, CVI_VOID *pchnctx)
{
    ...
    config.u.enc.singleEsBuffer   = pHandle->ModParam.stJpegeModParam.bSingleEsBuf;
    ...
    else if (prcatt->enRcMode == VENC_RC_MODE_MJPEGCBR)
    {
        ...
        {
            config.u.enc.framerate = ((frameRateDiv - 1) << 16) + frameRateRes;
        }
    }
    else
    {
                                 ...
                                 CVI_S32                   s32MIlliSec)
{
    ...
    status = CVIJpgSendFrameData(pHandle, (void *)&srcInfo, JPEG_MEM_EXTERNAL,
                                 s32MIlliSec);
                           ...
                           const VIDEO_FRAME_INFO_S *pstFrame)
{
    ...
    case PIXEL_FORMAT_YUV_PLANAR_422:
        ...
        psi->vbCb.phys_addr =
            pstVFrame->u64PhyAddr[1] + psi->strideC * pvecb->y + pvecb->x / 2;
        psi->vbCr.phys_addr =
            pstVFrame->u64PhyAddr[2] + psi->strideC * pvecb->y + pvecb->x / 2;
        break;
    case PIXEL_FORMAT_NV12:
    case PIXEL_FORMAT_NV21:
        psi->format           = CVI_FORMAT_420;
        psi->packedFormat     = CVI_PACKED_FORMAT_NONE;
        psi->chromaInterleave = (pstVFrame->enPixelFormat == PIXEL_FORMAT_NV12)
                                    ? CVI_CBCR_INTERLEAVE
                                    : CVI_CRCB_INTERLEAVE;
        psi->vbCb.phys_addr =
            pstVFrame->u64PhyAddr[1] + psi->strideC * pvecb->y / 2 + pvecb->x;
    ...
    default:
        ...
        psi->chromaInterleave = CVI_CBCR_SEPARATED;
        psi->vbCb.phys_addr =
            pstVFrame->u64PhyAddr[1] + psi->strideC * pvecb->y / 2 + pvecb->x / 2;
        psi->vbCr.phys_addr =
            pstVFrame->u64PhyAddr[2] + psi->strideC * pvecb->y / 2 + pvecb->x / 2;
        break;
    }

    psi->vbY.virt_addr  = (void *)pstVFrame->pu8VirAddr[0] + (psi->vbY.phys_addr - pstVFrame->u64PhyAddr[0]);
    psi->vbCb.virt_addr = (void *)pstVFrame->pu8VirAddr[1] + (psi->vbCb.phys_addr - pstVFrame->u64PhyAddr[1]);
    psi->vbCr.virt_addr = (void *)pstVFrame->pu8VirAddr[2] + (psi->vbCr.phys_addr - pstVFrame->u64PhyAddr[2]);

    CVI_VENC_SRC("x = %d, y = %d, stride = %d\n", pvecb->x, pvecb->y,
                 psi->strideY);
    CVI_VENC_SRC("orig pa Y = %#" PRIx64 ", Cb = %#" PRIx64 ", Cr = %#" PRIx64
                 "\n",
                 pstVFrame->u64PhyAddr[0], pstVFrame->u64PhyAddr[1],
                 pstVFrame->u64PhyAddr[2]);
    CVI_VENC_SRC("phyAddrY = 0x%llX, phyAddrCb = 0x%llX, phyAddrCr = 0x%llX\n",
                                ...
                                CVI_S32 s32MilliSec)
{
    ...
    status = CVIJpgGetFrameData(pHandle, (CVI_U8 *)&cviBuf, sizeof(CVIBUF),
...
static CVI_S32 vidEnc_open(CVI_VOID *handle, CVI_VOID *pchnctx)
{
    ...
    pInitEncCfg->codec      = (pVencAttr->enType == PT_H265) ? CODEC_H265 : CODEC_H264;
    ...
    CVI_VENC_TRACE("codec = %d, width = %d, height = %d, rcMode = %d\n",
                   pInitEncCfg->codec, pInitEncCfg->width, pInitEncCfg->height,
                   pInitEncCfg->rcMode);
    ...
    if (pInitEncCfg->codec == CODEC_H264)
    {
        ...
        pInitEncCfg->singleLumaBuf = pVencAttr->stAttrH264e.bSingleLumaBuf;
        pInitEncCfg->addrRemapEn   = pVencAttr->stAttrH264e.addrRemapEn;
        pInitEncCfg->bSingleEsBuf  = pHandle->ModParam.stH264eModParam.bSingleEsBuf;
    }
    else if (pInitEncCfg->codec == CODEC_H265)
    {
        pInitEncCfg->userDataMaxLength =
            pHandle->ModParam.stH265eModParam.u32UserDataMaxLen;
        pInitEncCfg->bSingleEsBuf = pHandle->ModParam.stH265eModParam.bSingleEsBuf;
        pInitEncCfg->addrRemapEn  = pVencAttr->stAttrH265e.addrRemapEn;
        if (pHandle->ModParam.stH265eModParam.enRefreshType == H265E_REFRESH_IDR)
    ...
    if (pInitEncCfg->bSingleEsBuf)
    {
        if (pInitEncCfg->codec == CODEC_H264)
            pInitEncCfg->bitstreamBufferSize =
                pHandle->ModParam.stH264eModParam.u32SingleEsBufSize;
        else if (pInitEncCfg->codec == CODEC_H265)
            pInitEncCfg->bitstreamBufferSize =
                pHandle->ModParam.stH265eModParam.u32SingleEsBufSize;
    }
    else
    {
    ...
    CVI_VENC_TRACE("es buf size = 0x%X, singleLumaBuf = %d\n",
                   pInitEncCfg->bitstreamBufferSize, pInitEncCfg->singleLumaBuf);
                                    ...
                                    CVI_VOID         *pchnctx)
{
    ...
    CVI_VENC_CFG("statTime = %d, gop = %d, bitrate = %d, fps = %d\n",
                 pInitEncCfg->statTime, pInitEncCfg->gop, pInitEncCfg->bitrate,
                 pInitEncCfg->framerate);
                                    ...
                                    CVI_VOID         *pchnctx)
{
    ...
    CVI_VENC_CFG(
        ...
        pInitEncCfg->statTime, pInitEncCfg->gop, pInitEncCfg->maxbitrate,
        pInitEncCfg->framerate, pInitEncCfg->s32ChangePos);
                                     ...
                                     CVI_VOID         *pchnctx)
{
    ...
    CVI_VENC_CFG(
        ...
        pInitEncCfg->statTime, pInitEncCfg->gop, pInitEncCfg->maxbitrate,
        pInitEncCfg->framerate, pInitEncCfg->s32ChangePos);
    CVI_VENC_CFG("StillPercent = %d, StillQP = %d, MotionSensitivity = %d\n",
    ...
    CVI_VENC_CFG("FrmLostOpen = %d, FrmGap = %d, PureStillThr = %d\n",
                 pInitEncCfg->s32AvbrFrmLostOpen, pInitEncCfg->s32AvbrFrmGap,
...
static CVI_S32 h264e_mapNaluType(VENC_PACK_S *ppack, CVI_S32 cviNalType)
{
    ...
    int naluType = ppack->pu8Addr[4] & 0x1f;
...
static CVI_S32 h265e_mapNaluType(VENC_PACK_S *ppack, CVI_S32 cviNalType)
{
    ...
    int naluType = (ppack->pu8Addr[4] & 0x7f) >> 1;
    ...
    if (naluType == H265_NALU_TYPE_W_RADL || naluType == H265_NALU_TYPE_N_LP)
                                  ...
                                  CVI_S32                   s32MilliSec)
{
    ...
    status = cviVEncEncOnePic(pEncCtx->ext.vid.pHandle, pPicCfg, s32MilliSec);

    if (status == ENC_TIMEOUT) return CVI_ERR_VENC_BUSY;
                                 ...
                                 CVI_S32 s32MilliSec)
{
    ...
    status = cviVEncGetStream(pEncCtx->ext.vid.pHandle, pStreamInfo, s32MilliSec);
    ...
    pstStream->u32PackCount = psp->totalPacks;
    for (idx = 0; idx < pstStream->u32PackCount; idx++)
    {
        ...
        ppack->u64PTS     = pEncCtx->base.u64PTS;
        status            = pEncCtx->ext.vid.mapNaluType(ppack, psp->pack[idx].cviNalType);
