...
struct cvi_gdc_mesh vi_mesh[VI_MAX_CHN_NUM];
...
static CVI_S32 _vi_chn_enable_mirror_flip(VI_CHN ViChn)
{
    ...
    if (CHECK_VI_CTX_NULL_PTR(gViCtx) != CVI_SUCCESS) return CVI_ERR_VI_FAILED_NOTCONFIG;
    ...
    if (s_pfnDevMirrorFlip[ViDev]) s_pfnDevMirrorFlip[ViDev](ViDev, eChnMirrorFlip);
                                   ...
                                   ROTATION_E enRotation)
{
    ...
    struct cvi_gdc_mesh *pmesh = &vi_mesh[ViChn];
    CVI_S32              s32Ret;
    ...
    if (paddr_old && paddr_old != DEFAULT_MESH_PADDR) CVI_SYS_IonFree(paddr_old, vaddr_old);
...
CVI_S32 CVI_VI_GetDevState(VI_PIPE ViPipe)
{
    ...
    if (CHECK_VI_CTX_NULL_PTR(gViCtx) != CVI_SUCCESS) return CVI_FAILURE;

    return gViCtx->isDevEnable[ViPipe] == CVI_TRUE ? CVI_SUCCESS : CVI_FAILURE;
}
...
CVI_S32 CVI_VI_StopPipe(VI_PIPE ViPipe)
{
    ...
    s32Ret = vi_sdk_stop_pipe(fd, ViPipe);
    if (s32Ret != CVI_SUCCESS) {
        CVI_TRACE_VI(CVI_DBG_ERR, "vi_sdk_start_pipe ioctl failed\n");
        return CVI_ERR_VI_FAILED_NOT_ENABLED;
    }
...
CVI_S32 CVI_VI_GetPipeAttr(VI_PIPE ViPipe, VI_PIPE_ATTR_S *pstPipeAttr)
{
    ...
    if (gViCtx->pipeAttr[ViPipe].u32MaxW == 0 && gViCtx->pipeAttr[ViPipe].u32MaxH == 0) {
...
CVI_S32 CVI_VI_StartSmoothRawDump(const VI_SMOOTH_RAW_DUMP_INFO_S *pstDumpInfo)
{
    ...
    for (CVI_U8 i = 0; i < frm_num; i++) {
        ...
        (raw_blk + i)->compress_mode = pstDumpInfo->enDumpRaw;
...
CVI_S32 CVI_VI_GetSmoothRawDump(VI_PIPE ViPipe, VIDEO_FRAME_INFO_S *pstVideoFrame,
                                CVI_S32 s32MilliSec)
{
    CVI_S32 fd     = -1;
    CVI_S32 s32Ret = CVI_SUCCESS;
...
CVI_S32 CVI_VI_SendPipeYUV(VI_PIPE ViPipe, const VIDEO_FRAME_INFO_S *pstVideoFrame,
                           CVI_S32 s32MilliSec)
{
    CHECK_VI_PIPEID_VALID(ViPipe);
    CHECK_VI_NULL_PTR(pstVideoFrame);
...
CVI_S32 CVI_VI_SetPipeCrop(VI_PIPE ViPipe, const CROP_INFO_S *pstCropInfo)
{
    ...
    if ((pstCropInfo->stRect.s32X + pstCropInfo->stRect.u32Width) >
        ...
        CVI_TRACE_VI(CVI_DBG_ERR,
                     "crop_x(%d)+w(%d) or y(%d)+h(%d) is bigger than chn_w(%d)_h(%d)\n",
                     pstCropInfo->stRect.s32X, pstCropInfo->stRect.u32Width,
                     pstCropInfo->stRect.s32Y, pstCropInfo->stRect.u32Height,
                     gViCtx->pipeAttr[ViPipe].u32MaxW, gViCtx->pipeAttr[ViPipe].u32MaxH);
        return CVI_ERR_VI_INVALID_PARA;
    }
...
CVI_S32 CVI_VI_EnableChn(VI_PIPE ViPipe, VI_CHN ViChn)
{
    ...
    if (ViChn >= VI_EXT_CHN_START) {
        ...
        if (ViChn < VI_MAX_CHN_NUM) _vi_chn_enable_mirror_flip(ViChn);

        if (gViCtx->total_dev_num != ViChn + 1) return CVI_SUCCESS;
...
CVI_S32 CVI_VI_SetChnCrop(VI_PIPE ViPipe, VI_CHN ViChn, const VI_CROP_INFO_S *pstCropInfo)
{
    ...
    if ((pstCropInfo->stCropRect.s32X + pstCropInfo->stCropRect.u32Width) >
        ...
        CVI_TRACE_VI(
            CVI_DBG_ERR, "crop_x(%d)+w(%d) or y(%d)+h(%d) is bigger than chn_w(%d)_h(%d)\n",
            pstCropInfo->stCropRect.s32X, pstCropInfo->stCropRect.u32Width,
            pstCropInfo->stCropRect.s32Y, pstCropInfo->stCropRect.u32Height,
            gViCtx->chnAttr[ViChn].stSize.u32Width, gViCtx->chnAttr[ViChn].stSize.u32Height);
        return CVI_ERR_VI_INVALID_PARA;
    }
...
CVI_S32 CVI_VI_GetChnFrame(VI_PIPE ViPipe, VI_CHN ViChn, VIDEO_FRAME_INFO_S *pstFrameInfo,
                           CVI_S32 s32MilliSec)
{
    CVI_S32 fd     = -1;
    CVI_S32 s32Ret = CVI_SUCCESS;
...
{
    CHECK_VI_NULL_PTR(pstModParam);
    if (CHECK_VI_CTX_NULL_PTR(gViCtx) != CVI_SUCCESS) return CVI_ERR_VI_FAILED_NOTCONFIG;

    gViCtx->modParam = *pstModParam;
...
CVI_S32 CVI_VI_GetModParam(VI_MOD_PARAM_S *pstModParam)
{
    ...
    *pstModParam = gViCtx->modParam;

    return CVI_SUCCESS;
}

CVI_S32 CVI_VI_SetChnEarlyInterrupt(VI_PIPE ViPipe, VI_CHN ViChn,
                                    const VI_EARLY_INTERRUPT_S *pstEarlyInterrupt)
{
    ...
    gViCtx->enEalyInt[ViChn] = *pstEarlyInterrupt;

    return CVI_SUCCESS;
}

CVI_S32 CVI_VI_GetChnEarlyInterrupt(VI_PIPE ViPipe, VI_CHN ViChn,
                                    VI_EARLY_INTERRUPT_S *pstEarlyInterrupt)
{
    ...
    *pstEarlyInterrupt = gViCtx->enEalyInt[ViChn];
...
// 	CHECK_VI_PIPEID_VALID(ViPipe);
// 	CHECK_VI_NULL_PTR(pstSyncTask);

// 	s32Ret = vi_sdk_reg_sync_task(-1, ViPipe, pstSyncTask);
// 	if (s32Ret != CVI_SUCCESS) {
// 		CVI_TRACE_VI(CVI_DBG_ERR, "vi_sdk_reg_sync_task ioctl failed\n");
// 		return CVI_FAILURE;
// 	}
...
// 	s32Ret = vi_sdk_unreg_sync_task(-1, ViPipe, pstSyncTask);
// 	if (s32Ret != CVI_SUCCESS) {
// 		CVI_TRACE_VI(CVI_DBG_ERR, "vi_sdk_unreg_sync_task ioctl failed\n");
// 		return CVI_FAILURE;
// 	}
...
CVI_S32 CVI_VI_DisableFrameValid(VI_DEV ViDev)
{
    ...
    s32Ret = vi_sdk_disable_frame_valid(-1, ViDev);
    if (s32Ret != CVI_SUCCESS) {
        CVI_TRACE_VI(CVI_DBG_ERR, "vi_sdk_disable_framevalid ioctl failed\n");
    }
...
CVI_S32 CVI_VI_EnableFrameValid(VI_DEV ViDev)
{
    ...
    s32Ret = vi_sdk_enable_frame_valid(-1, ViDev);
    if (s32Ret != CVI_SUCCESS) {
        CVI_TRACE_VI(CVI_DBG_ERR, "vi_sdk_enable_framevalid ioctl failed\n");
    }
