...
#define VDEV_CLOSED_CHK(_dev_type, _dev_id)                                              \
    ...
    if (IS_VDEV_CLOSED(d->state)) {                                                      \
        CVI_TRACE_VI(CVI_DBG_ERR, "vi dev(%d) state(%d) incorrect.", _dev_id, d->state); \
...
#define SET_SW_MODE_ENABLE(addr_ofs, val_ofs, on_off) SET_REGISTER_COMMON(addr_ofs, val_ofs, on_off)
...
static void _dump_gamma_table(void *fp, struct ip_info *ip_info_list, struct gamma_tbl *tbl,
                              CVI_U32 *offset)
{
    CVI_U32   paddr;
    CVI_U32   size;
    ...
    GET_REGISTER_COMMON(tbl->force_clk_enable.addr_ofs, tbl->force_clk_enable.val_ofs,
                        tbl->force_clk_enable.data);
...
#ifdef __CV180X__
    if (tbl->ip_info_id == IP_INFO_ID_RGBGAMMA || tbl->ip_info_id == IP_INFO_ID_YGAMMA ||
    ...
    SET_REGISTER_COMMON(tbl->force_clk_enable.addr_ofs, tbl->force_clk_enable.val_ofs,
                        tbl->force_clk_enable.data);
