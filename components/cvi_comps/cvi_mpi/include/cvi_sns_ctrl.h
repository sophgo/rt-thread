...
typedef struct _ISP_SNS_STATE_S {
    ...
    ISP_SNS_SYNC_INFO_S
    astSyncInfo[2]; /* [0]: Sensor reg info of cur-frame; [1]: Sensor reg info of pre-frame ; */
...
typedef struct _ISP_INIT_ATTR_S {
    ...
    CVI_BOOL bInitByUser;       // Initialize by user flag.
...
typedef struct _ISP_SNS_OBJ_S {
    CVI_S32 (*pfnRegisterCallback)(VI_PIPE ViPipe, ALG_LIB_S *,
                                   ALG_LIB_S *);  // Register callback function.
    CVI_S32 (*pfnUnRegisterCallback)(VI_PIPE ViPipe, ALG_LIB_S *,
                                     ALG_LIB_S *);  // Unregister callback function.
    CVI_S32 (*pfnSetBusInfo)(VI_PIPE           ViPipe,
                             ISP_SNS_COMMBUS_U unSNSBusInfo);  // Set communication bus information.
    CVI_VOID (*pfnStandby)(VI_PIPE ViPipe);                    // Put the sensor into standby mode.
    CVI_VOID (*pfnRestart)(VI_PIPE ViPipe);                    // Restart the sensor.
    CVI_VOID (*pfnMirrorFlip)(VI_PIPE                   ViPipe,
                              ISP_SNS_MIRRORFLIP_TYPE_E eSnsMirrorFlip);  // Set mirror/flip mode.
    CVI_S32 (*pfnWriteReg)(VI_PIPE ViPipe, CVI_S32 s32Addr,
                           CVI_S32 s32Data);                 // Write to a sensor register.
    CVI_S32 (*pfnReadReg)(VI_PIPE ViPipe, CVI_S32 s32Addr);  // Read from a sensor register.
    CVI_S32 (*pfnSetInit)(VI_PIPE ViPipe,
                          ISP_INIT_ATTR_S *);         // Initialize the sensor with attributes.
...
typedef struct _SNS_AHD_OBJ_S {
    ...
    CVI_S32 (*pfnSetAhdBusInfo)(VI_PIPE ViPipe,
                                CVI_S32 astI2cDev);  // Set AHD communication bus information.
    CVI_S32 (*pfnDetectAhdStatus)(VI_PIPE ViPipe, CVI_S32 ahdOldType,
                                  CVI_S32 *ahdType);  // Detect AHD status.
} SNS_AHD_OBJ_S;
