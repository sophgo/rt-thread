 ...
 */
#ifndef CONTAINER_OF_DEFINED
#define CONTAINER_OF_DEFINED
...
#define dlist_entry(addr, type, member) \
    ((type *)((long)addr - offsetof(type, member)))
 ...
 */
#define DLIST_INIT(list)  {&(list), &(list)}
...
#define dlist_for_each_entry_safe(queue, n, node, type, member) \
    for (node = container_of((queue)->next, type, member),  \
         ...
         &node->member != (queue);                              \
         node = container_of(n, type, member), n = n ? n->next : NULL)
...
#define slist_for_each_entry_safe(queue, tmp, node, type, member) \
    for (node = (queue)->next? container_of((queue)->next, type, member) : NULL, \
         ...
         node;                                           \
         node = tmp ? container_of(tmp, type, member) : NULL, tmp = tmp ? tmp->next : NULL)
