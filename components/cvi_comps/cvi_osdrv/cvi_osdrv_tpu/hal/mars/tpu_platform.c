...
static int poll_cmdbuf_done(struct TPU_PLATFORM_CFG *pCfg, struct CMD_ID_NODE *id_node)
{
	...
	uintptr_t iomem_tiuBase = pCfg->iomem_tiuBase;
	// uint64_t tick = krhino_sys_tick_get();
	// uint64_t expire = tick + MS2TICK(TIMEOUT_MS);
	...
	if (id_node->tdma_cmd_id > 0) {
		...
		if ((regVal2 >> 16) < id_node->tdma_cmd_id) {
			tpu_printf(TPU_DBG, "got tdma int but tdma id not the last one, maybe not finished\r\n");
			return -1;
		}
	}
	...
	if (id_node->bd_cmd_id > 0) {
		...
		while (1) {
			...
			// if (expire < tick) {
			// 	tpu_printf(TPU_DBG, "poll tiu timeout, BD_CTRL_BASE_ADDR=0x%x\r\n", regVal);
			// 	return -1;
			// }
		}
	}
...
static void platform_run_dmabuf_setArrayBase(struct TPU_PLATFORM_CFG *pCfg, struct dma_hdr_t *header)
{
	...
	tpu_printf(TPU_DBG,
		   "base0L=0x%x, base1L=0x%x, base2L=0x%x, base3L=0x%x,\r\n base0H=0x%x, base1H=0x%x, base2H=0x%x, base3H=0x%x\r\n",
	...
	tpu_printf(TPU_DBG,
		   "base4L=0x%x, base5L=0x%x, base6L=0x%x, base7L=0x%x,\r\n base4H=0x%x, base5H=0x%x, base6H=0x%x, base7H=0x%x\r\n",
...
static int reinit_sem(sem_t *tdma_done)
{
	int value = 0;
	int ret = -1;
	ret = sem_getvalue(tdma_done, &value);
	if (ret == 0) {
...
int platform_run_dmabuf(struct cvi_tpu_device *ndev, void *dmabuf_v, uint64_t dmabuf_p)
{
	...
	tpu_printf(TPU_DBG, "dmabuf_v=0x%p, dmabuf_p=0x%llx\r\n", dmabuf_v, dmabuf_p);
	...
	if (tpu_reg_backup.tdma_ctrl & (0x1 << TDMA_CTRL_ENABLE_BIT)) {
		...
		if (!tpu_sync_backup) {
			...
			while (1) {
				...
				// tick = krhino_sys_tick_get();
				// if (expire < tick) {
				// 	tpu_printf(TPU_ERR, "polling for tdma INT failed before suspend\r\n");
				// 	break;
				// }
			}
...
int platform_run_pio(struct cvi_tpu_device *ndev, struct tpu_tdma_pio_info *info)
{
	...
	// ret = sem_timedwait(&ndev->tdma_done, 10);
