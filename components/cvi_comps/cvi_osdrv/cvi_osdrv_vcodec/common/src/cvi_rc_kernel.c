...
int rc_mask = CVI_RC_MASK_CURR;

static void cviGetVcomMask(void);
// ----------------------------------------------------------------

float getFloat(RC_Float val)
{
    float *fl;
...
{
    CVI_RC_FLOAT("g_lambdaQpScale = %f, lambda = %f, g_lambdaQpBias = %f\n",
                 getFloat(g_lambdaQpScale), getFloat(lambda),
                 getFloat(g_lambdaQpBias));
    return CVI_FLOAT_TO_INT(CVI_FLOAT_ADD(
        CVI_FLOAT_MUL(g_lambdaQpScale, CVI_FLOAT_LOG(lambda)), g_lambdaQpBias));
}

static RC_Float QpToLambda(RC_Float qp)
{
    return CVI_FLOAT_EXP(
        CVI_FLOAT_DIV(CVI_FLOAT_SUB(qp, g_lambdaQpBias), g_lambdaQpScale));
}

static RC_Float intraLambdaToBpp(RC_Float lambda, RC_Float alpha, RC_Float beta,
                                 RC_Float tc)
{
    RC_Float bpp_div_tc =
        CVI_FLOAT_POW(CVI_FLOAT_DIV(CVI_FLOAT_MUL(FLOAT_VAL_256, lambda), alpha),
...
{
    return CVI_FLOAT_POW(CVI_FLOAT_DIV(lambda, alpha),
                         CVI_FLOAT_DIV(FLOAT_VAL_1, beta));
}
...
void updateAlphaBetaIntra(RC_Float *alpha, RC_Float *beta, RC_Float tc,
                          int targetBits, int encodedBits)
{
    RC_Float lnbpp      = CVI_FLOAT_LOG(tc);
    RC_Float diffLambda = CVI_FLOAT_MUL(
        *beta, CVI_FLOAT_SUB(CVI_FLOAT_LOG(INT_TO_CVI_FLOAT(encodedBits)),
                             CVI_FLOAT_LOG(INT_TO_CVI_FLOAT(targetBits))));

    diffLambda = CVI_FLOAT_CLIP(FLOAT_VAL_minus_p125, FLOAT_VAL_p125,
...
void rcGopBitAlloc(stRcKernelInfo *info, int picIdx)
{
    ...
    int smoothWinSize =
        MAX(info->statFrameNum - picIdxInIPeriod + 1, info->framerate);
    int picTargetBit =
        MAX(info->minPicBit, info->picAvgBit + (info->bitError / smoothWinSize));
    int gopSize;
    ...
    gopSize            = ((picIdx / info->rcGopSize) + ((picIdx % info->rcGopSize) != 0)) * info->rcGopSize + 1 - picIdx;
...
{
    int gopRemainAvgBit =
        MAX(MAX(0, info->gopBitLeft) / MAX(1, info->gopPicLeft), info->minPicBit);
    int targetBits = (g_rcGopRemainBitWeight * gopRemainAvgBit + ((100 - g_rcGopRemainBitWeight) * info->gopPicAvgBit)) / 100;
    return targetBits;
}

RC_Float calculateLambdaIntra(RC_Float alpha, RC_Float beta,
                              RC_Float picTextCplx, RC_Float bitsPerPixel)
{
    return CVI_FLOAT_MUL(
        FRAC_INT_TO_CVI_FLOAT(
            (CVI_FLOAT_TO_FRAC_INT(alpha, FIX_POINT_FRAC_BIT) >> 8),
            FIX_POINT_FRAC_BIT),
        CVI_FLOAT_POW(CVI_FLOAT_DIV(picTextCplx, bitsPerPixel), beta));
}

RC_Float estPicLambda(stRcKernelInfo *info, int targetBits, int isIPic,
                      int picIdx)
{
    int      picLv = isIPic == 0;
    ...
    unsigned long long targetBits64 =
        ((unsigned long long)targetBits << FIX_POINT_FRAC_BIT);
    int      bpp32;
    RC_Float bpp;
...
#else
    RC_Float bpp = FRAC_INT_TO_CVI_FLOAT(
        ((long long int)targetBits << FIX_POINT_FRAC_BIT) / info->numOfPixel,
        FIX_POINT_FRAC_BIT);
#endif
    ...
    CVI_RC_TRACE("targetBits = %d\n", targetBits);

    CVI_RC_FLOAT("picLv = %d, alpha = %f, beta = %f, bpp = %f, estLambda = %f\n",
                 picLv, getFloat(alpha), getFloat(beta), getFloat(bpp),
                 getFloat(estLambda));

    lastLevelLambda = info->lastLevelLambda[picLv];
    lastPicLambda   = info->lastPicLambda;
    CVI_RC_FLOAT("lastLevelLambda = %f, lastPicLambda = %f\n",
                 getFloat(lastLevelLambda), getFloat(lastPicLambda));

    isFirstPPic = (info->isCurPicI == 0) && (info->isLastPicI == 1) && (picIdx > info->intraPeriod);
    levelClip   = (info->isCurPicI == 1) ? (picIdx > info->intraPeriod)
                                         : (info->isLastPicI == 0);
    ...
    if (CVI_FLOAT_GT(lastLevelLambda, FLOAT_VAL_0) && levelClip == 1)
    {
        ...
        lastLevelLambda =
            CVI_FLOAT_CLIP(FLOAT_VAL_p1, FLOAT_VAL_10000, lastLevelLambda);
        ...
        CVI_RC_FLOAT("lastLevelLambda = %f, estLambda = %f, low = %f, high = %f\n",
                     getFloat(lastLevelLambda), getFloat(estLambda), getFloat(low),
                     getFloat(high));
    }
    ...
    if (CVI_FLOAT_GT(lastPicLambda, FLOAT_VAL_0) && isFirstPPic == 1)
    {
        ...
        estLambda = CVI_FLOAT_CLIP(low, high, estLambda);
        CVI_RC_FLOAT("lastPicLambda = %f, estLambda = %f, low = %f, high = %f\n",
                     getFloat(lastPicLambda), getFloat(estLambda), getFloat(low),
                     getFloat(high));
    }
    ...
    CVI_RC_FLOAT("estLambda = %f, minLambda = %f, maxLambda = %f\n",
...
int estPicQP(stRcKernelInfo *info, RC_Float lambda, int isIPic, int picIdx)
{
    ...
    int levelClip   = (info->isCurPicI == 1) ? (picIdx > info->intraPeriod)
                                             : (info->isLastPicI == 0);
    int maxQp, minQp;

    CVI_RC_FLOAT("qp = %d, lambda = %f\n", qp, getFloat(lambda));
    CVI_RC_FLOAT("g_lambdaQpScale = %f, g_lambdaQpBias = %f\n",
                 getFloat(g_lambdaQpScale), getFloat(g_lambdaQpBias));
    CVI_RC_TRACE("maxIQp = %d, minIQp = %d, maxQp = %d, minQp = %d\n",
                 info->maxIQp, info->minIQp, info->maxQp, info->minQp);

    if (lastLevelQP > 0 && levelClip == 1)
    {
        qp = CLIP(lastLevelQP - info->levelPicQpClip,
                  lastLevelQP + info->levelPicQpClip, qp);
    }
    if (lastPicQP > 0 && isFirstPPic == 1)
    {
        qp = CLIP(lastPicQP - info->lastPicQpClip, lastPicQP + g_rcPicQpNormalClip,
                  qp);
    }
    else if (lastPicQP > 0 && isIPic == 0)
    {
        qp = CLIP(lastPicQP - info->lastPicQpClip, lastPicQP + info->lastPicQpClip,
                  qp);
    }
    else if (lastPicQP > 0)
    {
        qp = CLIP(lastPicQP - info->picQpNormalClip,
                  lastPicQP + info->picQpNormalClip, qp);
    }
...
void cviRcKernel_setMinMaxQp(stRcKernelInfo *info, int minQp, int maxQp,
                             int isIntra)
{
    if (isIntra == 1)
    {
        ...
        CVI_RC_FLOAT("minQp = %d, maxQp = %d, maxILambda = %f, minILambda = %f\n",
                     minQp, maxQp, getFloat(info->maxILambda),
                     getFloat(info->minILambda));
    }
    else
    {
        info->maxQp     = maxQp;
...
void cviRcKernel_setBitrateAndFrameRate(stRcKernelInfo *info, int targetBitrate,
                                        RC_Float frameRate)
{
    int      maxIPicBitByIPRatio, maxIPicBitByMinPBudget;
    RC_Float fPicAvgBit, targetBpp;

    CVI_RC_FLOAT("targetBitrate = %d, frameRate = %f\n", targetBitrate,
                 getFloat(frameRate));
    ...
    maxIPicBitByIPRatio = CVI_FLOAT_TO_INT(CVI_FLOAT_MUL(
        CVI_FLOAT_DIV(INT_TO_CVI_FLOAT(info->maxIprop * info->intraPeriod),
                      INT_TO_CVI_FLOAT(info->maxIprop + info->intraPeriod - 1)),
        fPicAvgBit));
    maxIPicBitByMinPBudget =
        CVI_FLOAT_TO_INT(
            CVI_FLOAT_MUL(INT_TO_CVI_FLOAT(info->intraPeriod), fPicAvgBit))
        - (info->minPicBit * (info->intraPeriod - 1));
    info->minIPicBit = CVI_FLOAT_TO_INT(CVI_FLOAT_MUL(
        CVI_FLOAT_DIV(INT_TO_CVI_FLOAT(info->minIprop * info->intraPeriod),
                      INT_TO_CVI_FLOAT(info->minIprop + info->intraPeriod - 1)),
        fPicAvgBit));
    info->maxIPicBit = MIN(maxIPicBitByIPRatio, maxIPicBitByMinPBudget);

    CVI_RC_CVRC("targetBitrate = %d, framerate = %d, statFrameNum = %d\n",
                info->targetBitrate, info->framerate, info->statFrameNum);
    CVI_RC_CVRC(
        "picAvgBit= %d, neiIBitMargin= %d, minIPicBit = %d, maxIPicBit = %d\n",
...
void cviRcKernel_setLevelPicQpClip(stRcKernelInfo *info, int levelPicQpClip)
{
    ...
    CVI_RC_TRACE("levelPicQpClip = %d\n", info->levelPicQpClip);

    info->lastLevelLambdaScaleLow = CVI_FLOAT_POW(
        FLOAT_VAL_2,
        CVI_FLOAT_DIV(INT_TO_CVI_FLOAT(-levelPicQpClip), FLOAT_VAL_3));
    info->lastLevelLambdaScaleHigh = CVI_FLOAT_POW(
        FLOAT_VAL_2,
        CVI_FLOAT_DIV(INT_TO_CVI_FLOAT(levelPicQpClip), FLOAT_VAL_3));
}
...
void cviRcKernel_setLastPicQpClip(stRcKernelInfo *info, int lastPicQpClip)
{
    ...
    CVI_RC_TRACE("lastPicQpClip = %d\n", info->lastPicQpClip);

    info->lastPicLambdaScaleLow = CVI_FLOAT_POW(
        FLOAT_VAL_2,
        CVI_FLOAT_DIV(INT_TO_CVI_FLOAT(-lastPicQpClip), FLOAT_VAL_3));
    info->lastPicLambdaScaleHigh = CVI_FLOAT_POW(
        FLOAT_VAL_2, CVI_FLOAT_DIV(INT_TO_CVI_FLOAT(lastPicQpClip), FLOAT_VAL_3));
}

void cviRcKernel_setpPicQpNormalClip(stRcKernelInfo *info,
                                     int             picQpNormalClip)
{
    info->picQpNormalClip = picQpNormalClip;
    CVI_RC_TRACE("picQpNormalClip = %d\n", info->picQpNormalClip);

    info->lastNormalLambdaScaleLow = CVI_FLOAT_POW(
        FLOAT_VAL_2,
        CVI_FLOAT_DIV(INT_TO_CVI_FLOAT(-picQpNormalClip), FLOAT_VAL_3));
    info->lastNormalLambdaScaleHigh = CVI_FLOAT_POW(
        FLOAT_VAL_2,
        CVI_FLOAT_DIV(INT_TO_CVI_FLOAT(picQpNormalClip), FLOAT_VAL_3));
}

void cviRcKernel_setRCModelParam(stRcKernelInfo *info, RC_Float alpha,
                                 RC_Float beta, int model_idx)
{
...
void cviRcKernel_setRCModelUpdateStep(stRcKernelInfo *info, RC_Float alphaStep,
                                      RC_Float betaStep)
{
...
void cviRcKernel_init(stRcKernelInfo *info, stRcKernelCfg *cfg)
{
    ...
    info->minPicBit       = CVI_FLOAT_TO_INT(
        CVI_FLOAT_MUL(INT_TO_CVI_FLOAT(info->numOfPixel), g_rcMinPPicBpp));
    ...
    rcIterNum = 6;
    baseRcGopSize =
        MAX(MIN(info->intraPeriod, info->statFrameNum) / rcIterNum, 1);
    info->rcGopSize = 1;
...
static void cviGetVcomMask(void)
{
    rc_mask |= CVI_RC_MASK_ERR;
}

void cviRcKernel_estimatePic(stRcKernelInfo *info, stRcKernelPicOut *out,
                             int isIPic, int picIdx)
{
    ...
    CVI_RC_TRACE(
        "isIPic = %d, intraPeriod = %d, isLastPicI = %d, rcGopSize = %d\n",
    ...
    {
        CVI_RC_FLOAT("picTargetBit = %d, lambda = %f, qp = %d\n", picTargetBit,
                     getFloat(lambda), qp);
    }
...
void cviRcKernel_updatePic(stRcKernelInfo *info, stRcKernelPicIn *stats,
                           int isIPic)
{
    int      picLv;
    RC_Float alpha, beta;
    ...
    if (rc_mask & CVI_RC_MASK_DBG)
    {
        CVI_RC_FLOAT("isIPic = %d, encodedQp = %f, lastPicLambda = %f\n", isIPic,
                     getFloat(stats->encodedQp), getFloat(info->lastPicLambda));
    }

    picLv = isIPic == 0;
    // update bitrate status
    info->bitError    = CLIP(-MAX_BIT_ERROR, MAX_BIT_ERROR,
                             info->bitError + info->picAvgBit - stats->encodedBit);
    info->gopBitLeft -= stats->encodedBit;
    info->gopPicLeft -= 1;

    CVI_RC_FLOAT("isIPic = %d, rcMdlUpdatType = %d, skipRatio = %f\n", isIPic,
                 info->rcMdlUpdatType, getFloat(stats->skipRatio));
    ...
    if (isIPic == 1)
    {
        updateAlphaBetaIntra(&alpha, &beta, info->picTextCplx, info->picTargetBit,
                             stats->encodedBit);
    }
    else if (info->rcMdlUpdatType == 0)
    {
        ...
        targetBits64 =
            ((unsigned long long)stats->encodedBit << FIX_POINT_FRAC_BIT);
        do_div(targetBits64, info->numOfPixel);
        bpp32        = (int)targetBits64;
        picActualBpp = FRAC_INT_TO_CVI_FLOAT(bpp32, FIX_POINT_FRAC_BIT);
#else
        picActualBpp = FRAC_INT_TO_CVI_FLOAT(
            ((long long int)stats->encodedBit << FIX_POINT_FRAC_BIT) / info->numOfPixel,
            FIX_POINT_FRAC_BIT);
#endif
        // LMS algorithm
        ...
        calLambda =
            CVI_FLOAT_CLIP(CVI_FLOAT_DIV(inputLambda, FLOAT_VAL_10),
                           CVI_FLOAT_MUL(inputLambda, FLOAT_VAL_10), calLambda);

        if (tmp != calLambda)
        {
            CVI_RC_FLOAT("clip calLambda = %f\n", getFloat(calLambda));
        }

        logLambdaDiff =
            CVI_FLOAT_SUB(CVI_FLOAT_LOG(inputLambda), CVI_FLOAT_LOG(calLambda));
        alpha_incr =
            CVI_FLOAT_MUL(CVI_FLOAT_MUL(info->alphaStep, logLambdaDiff), alpha);
        ...
        beta_incr =
            CVI_FLOAT_MUL(CVI_FLOAT_MUL(info->betaStep, logLambdaDiff), lnbpp);
        beta = CVI_FLOAT_ADD(beta, beta_incr);

        CVI_RC_FLOAT(
            "picLv = %d, logLambdaDiff = %f, alpha_incr = %f, beta_incr = %f\n",
            picLv, getFloat(logLambdaDiff), getFloat(alpha_incr),
            getFloat(beta_incr));
        ...
        if (tmp != beta)
        {
            CVI_RC_FLOAT("clip beta = %f\n", getFloat(beta));
        }
    }
    else
    { // RD function analytics
#ifdef UNUSED_CODE
        RC_Float picActualBpp = FRAC_INT_TO_CVI_FLOAT(
            ((long long int)stats->encodedBit << FIX_POINT_FRAC_BIT) / info->numOfPixel,
            FIX_POINT_FRAC_BIT);
        RC_Float updatedK = CVI_FLOAT_DIV(
            CVI_FLOAT_MUL(picActualBpp, info->lastPicLambda), stats->mse);
        RC_Float updatedC =
            CVI_FLOAT_DIV(stats->mse, CVI_FLOAT_POW(picActualBpp, -updatedK));
        RC_Float updateScale =
            CVI_FLOAT_CLIP(FLOAT_VAL_p1, FLOAT_VAL_p9,
                           CVI_FLOAT_SUB(FLOAT_VAL_1, stats->skipRatio));
        RC_Float newAlpha = CVI_FLOAT_MUL(updatedC, updatedK);
        RC_Float newBeta =
            CVI_FLOAT_SUB(FLOAT_VAL_0, CVI_FLOAT_ADD(updatedK, FLOAT_VAL_1));
        RC_Float oneMinusUpdateScale = CVI_FLOAT_SUB(FLOAT_VAL_1, updateScale);
        RC_Float updateAlpha =
            CVI_FLOAT_ADD(CVI_FLOAT_MUL(updateScale, newAlpha),
                          CVI_FLOAT_MUL(oneMinusUpdateScale, alpha));
        RC_Float updateBeta =
            CVI_FLOAT_ADD(CVI_FLOAT_MUL(updateScale, newBeta),
