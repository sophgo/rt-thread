...
{
    Uint32 sub_pic_hrd_params_present_flag;         // derived from syntax elements
                                                    // found in the hrd_parameters( )
    Uint32 au_cpb_removal_delay_length_minus1;      // derived from syntax
                                                    // elements found in the
                                                    // hrd_parameters( )
    Uint32 initial_cpb_removal_delay_length_minus1; // derived from syntax
                                                    // elements found in the
                                                    // hrd_parameters( )
    Uint32 CpbCnt;                                  // CpbCnt are derived from syntax elements found in the
                                                    // sub_layer_hrd_parameters( ) syntax structure
    Uint32 NalHrdBpPresentFlag;                     // NalHrdBpPresentFlag and
                                                    // VclHrdBpPresentFlag are found in or
                                                    // derived from syntax elements found in the
                                                    // hrd_parameters( )
    Uint32 VclHrdBpPresentFlag;                     // NalHrdBpPresentFlag and
                                                    // VclHrdBpPresentFlag are found in or
                                                    // derived from syntax elements found in the
                                                    // hrd_parameters( )
    ...
    Uint32 cpb_delay_offset; // The syntax element has a length in bits
                             // given by au_cpb_removal_delay_length_minus1 +
                             // 1
...
typedef struct sei_pic_timing_tag
{
    ...
    Uint32 CpbDpbDelaysPresentFlag; // if nal_hrd_parameters_present_flag or
                                    // vcl_hrd_parameters_present_flag  is
                                    // 1,
...
typedef enum
{
    AVC_NUT_NON_IDR = 1,  // coded slice of an non-IDR picture
    AVC_NUT_IDR     = 5,  // coded slice of an IDR picture
    AVC_NUT_SEI     = 6,  // supplemental enhancement information
    AVC_NUT_SPS     = 7,  // sequence parameter set
    AVC_NUT_PPS     = 8,  // picture parameter set
    AVC_NUT_AUD     = 9,  // access unit delimiter
    AVC_NUT_EOSEQ   = 10, // end of sequence
    AVC_NUT_EOS     = 11, // end of stream
} AVC_NAL_UNIT_TYPE;
...
typedef struct
{
    ...
    Uint32 field_seq_flag;                // this syntax shall be 1 when pic_struct in
                                          // sei_pic_timing is 1 or 2
    Uint32 frame_field_info_present_flag; // this syntax shall be 1 when
                                          // pic_struct in sei_pic_timing is
                                          // not 0
