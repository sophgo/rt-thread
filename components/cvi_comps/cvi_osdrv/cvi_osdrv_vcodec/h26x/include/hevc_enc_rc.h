...
typedef struct
{
    ...
    int used_for_ref; // flag to indicate that this frame is reference frame or
                      // not
} rps_t;
...
void rc_seq_init(
    ...
    int gop_size, gop_entry_t gop[MAX_GOP_SIZE], int pic_width, int pic_height,
    int intra_period, int rc_mode, int intra_qp, int rc_initial_qp,
    int is_para_change, int weight_factor, int gamma,
#ifdef CLIP_PIC_DELTA_QP
...
void rc_pic_init(
    // IN
    rc_pic_t *rc, int is_intra, int enc_order_in_gop, int min_qp, int max_qp,
// OUT
#ifdef CLIP_PIC_DELTA_QP
    int *max_delta_qp_minus, int *max_delta_qp_plus,
#endif

    int curr_long_term, int *pic_qp, int *pic_target_bit, int *hrd_buf_level,
    int *hrd_buf_size);
