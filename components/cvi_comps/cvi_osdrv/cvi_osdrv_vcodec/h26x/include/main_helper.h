...
Int32 LoadFirmware_EX(Uint32 coreIdx, Uint8 **retFirmware,
                      Uint32 *retSizeInWord);
...
void DisplayEncodedInformation(EncHandle handle, CodStd codec,
                               EncOutputInfo *encodedInfo, int srcEndFlag,
                               int srcFrameIdx);
...
typedef struct YuvFeederImpl
{
    void *context;
    BOOL(*Create)
    (struct YuvFeederImpl *impl, const char *path, Uint32 packed, Uint32 fbStride,
     Uint32 fbHeight);
    BOOL(*Feed)
    (struct YuvFeederImpl *impl, Int32 coreIdx, FrameBuffer *fb, size_t picWidth,
...
typedef struct RenderDevice
{
    ...
    void (*Render)(struct RenderDevice *device, DecOutputInfo *fbInfo, Uint8 *yuv,
...
typedef struct TestEncConfig_struct
{
    ...
    int     roiLevel[MAX_ROI_NUMBER]; /**< An importance level for the given ROI
                                     ...
                                     is with a lower QP.  */
    int     roi_cfg_type;             // only for HEVC, 0: original bounding box, 1: ctu map
    ...
    Uint8 *userDataBuf[NUM_OF_USER_DATA_BUF];
    Uint32 userDataLen[NUM_OF_USER_DATA_BUF];
...
void GenRegionToMap(VpuRect *region, /**< The size of the ROI region for H.265
                                        (start X/Y in CTU, end X/Y in CTU)  */
...
void setRoiMapFromMap(int coreIdx, TestEncConfig *encConfig,
                      EncOpenParam *pEncOP, PhysicalAddress addrRoiMap,
...
void cviSetWaveRoiBySdk(TestEncConfig *pEncConfig, EncParam *param,
                        EncOpenParam *pEncOP, Int32 base_qp);
int  openRoiMapFile(TestEncConfig *pEncConfig);
...
int setCtuQpMap(int coreIdx, TestEncConfig *pEncConfig, EncOpenParam *pEncOP,
