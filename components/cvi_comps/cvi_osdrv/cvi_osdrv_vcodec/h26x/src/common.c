...
static char *cmd2Txt(Uint32 cmd);
#endif
...
void Wave4BitIssueCommand(CodecInst *instance, Uint32 cmd)
{
    ...
    if ((instance) && (cmd != INIT_VPU) && (cmd != GET_FW_VERSION) && (cmd != SLEEP_VPU))
    {
        if (instance->codecMode < HEVC_ENC)
        {
            pVbWork = &instance->CodecInfo->decInfo.vbWork;
        }
        else
        {
            pVbWork = &instance->CodecInfo->encInfo.vbWork;
        }
    ...
    if (cmd != INIT_VPU)
    {
        CVI_VC_MCU("W4_VPU_HOST_INT_REQ\n");
        VpuWriteReg(coreIdx, W4_VPU_HOST_INT_REQ, 1);
    }
}

typedef struct _CMD_TXT_
{
    Uint32 cmd;
    char  *txt;
} cmd_txt;
...
{
    Uint32         idx       = 0;
    static cmd_txt allCmds[] = {
        {    0x0,                 "UNKNOWN"},
        { 0x0001,                "INIT_VPU"},
        { 0x0002, "DEC_PIC_HDR / SET_PARAM"},
        { 0x0004,                "FINI_SEQ"},
        { 0x0008,       "DEC_PIC / ENC_PIC"},
        { 0x0010,            "SET_FRAMEBUF"},
        { 0x0020,           "FLUSH_DECODER"},
        { 0x0100,          "GET_FW_VERSION"},
        { 0x0200,           "QUERY_DECODER"},
        { 0x0400,               "SLEEP_VPU"},
        { 0x0800,              "WAKEUP_VPU"},
        { 0x4000,         "CREATE_INSTANCE"},
        {0x10000,               "RESET_VPU"},
    };

    for (idx = 0; idx < sizeof(allCmds) / sizeof(cmd_txt); idx++)
    {
        if (allCmds[idx].cmd == cmd)
            return allCmds[idx].txt;
    }

    return allCmds[0].txt;
}
#endif

static RetCode SetupWave4Properties(Uint32 coreIdx)
{
    ...
    {
    case WAVE420L_CODE:
        pAttr->productId = PRODUCT_ID_420L;
        break;
    default:
        pAttr->productId = PRODUCT_ID_NONE;
        break;
    }
    ...
    pAttr->supportEndianMask =
        (Uint32)((1 << VDI_LITTLE_ENDIAN) | (1 << VDI_BIG_ENDIAN) | (1 << VDI_32BIT_LITTLE_ENDIAN) | (1 << VDI_32BIT_BIG_ENDIAN) | (0xffff << 16));
...
RetCode Wave4VpuGetVersion(Uint32 coreIdx, Uint32 *versionInfo,
                           Uint32 *revision)
{
    Uint32          regVal;
...
RetCode Wave4VpuInit(Uint32 coreIdx, void *firmware, Uint32 size)
{
    ...
    memset((void *)&hdr, 0x00, sizeof(CodecInstHeader));
    ...
    CVI_VC_PERF("Wave4 LoadFirmware = " VCODEC_UINT64_FORMAT_DEC " us\n", end_time - start_time);
    ...
    vdi_flush_ion_cache(codeBase, (void *)codeBase, codeSize);
    ...
    if (vdi_wait_vpu_busy(coreIdx, __VPU_BUSY_TIMEOUT, W4_VPU_BUSY_STATUS) == -1)
    {
        CVI_VC_ERR("VPU init(W4_VPU_REMAP_CORE_START) timeout\n");
#ifdef ENABLE_CNM_DEBUG_MSG
    ...
    if (regVal == 0)
    {
        ...
        CVI_VC_ERR("VPU init(W4_RET_SUCCESS) failed(%d) REASON CODE(%08x)\n", regVal, reasonCode);
        return RETCODE_FAILURE;
    }
...
RetCode Wave4VpuReInit(Uint32 coreIdx, void *firmware, Uint32 size)
{
    ...
    oldCodeBase = vdi_remap_memory_address(coreIdx, VpuReadReg(coreIdx, W4_VPU_REMAP_PADDR));
    ...
    if (oldCodeBase != codeBase)
    {
        ...
        vdi_fio_write_register(coreIdx, W4_GDI_VCORE0_BUS_CTRL, 0x100); // WAVE410 GDI added new feature:
        // disable_request

        // Step2 : Waiting for completion of bus transaction
        if (vdi_wait_bus_busy(coreIdx, __VPU_BUSY_TIMEOUT, W4_GDI_VCORE0_BUS_STATUS) == -1)
        {
            vdi_fio_write_register(coreIdx, W4_GDI_VCORE0_BUS_CTRL,
                                   0x00);
#ifdef ENABLE_CNM_DEBUG_MSG
	...
	*/
        ...
        if (vdi_wait_vpu_busy(coreIdx, __VPU_BUSY_TIMEOUT, W4_VPU_BUSY_STATUS) == -1)
        {
            CVI_VC_ERR("INIT_VPU RESPONSE_TIMEOUT\n");
            return RETCODE_VPU_RESPONSE_TIMEOUT;
        }
...
Int32 Wave4VpuWaitInterrupt(CodecInst *handle, Int32 timeout)
{
    ...
    reason = vdi_wait_interrupt(coreIdx, timeout, (Uint64 *)&handle->u64EndTime);
    if (reason > 0)
    {
...
RetCode Wave4VpuSleepWake_EX(Uint32 coreIdx, int iSleepWake, Uint32 size)
{
    CodecInstHeader hdr;
    Uint32          regVal;
    vpu_buffer_t    vb;
    ...
    if (iSleepWake == 1)
    {
        ...
        if (regVal == 0)
        {
            ...
            CVI_VC_ERR("SLEEP_VPU fail\n");
            return RETCODE_FAILURE;
        }

        regVal = vdi_fio_read_register(coreIdx, W4_GDI_VCORE0_BUS_CTRL);
        ...
        if (vdi_wait_bus_busy(coreIdx, __VPU_BUSY_TIMEOUT, W4_GDI_VCORE0_BUS_STATUS) == -1)
        {
            ...
            vdi_log(coreIdx, RESET_VPU, 2);
#endif
            CVI_VC_ERR("wait_bus_busy timeout\n");
            return RETCODE_VPU_RESPONSE_TIMEOUT;
        }

        regVal = W4_RST_BLOCK_ACLK_ALL | W4_RST_BLOCK_BCLK_ALL | W4_RST_BLOCK_CCLK_ALL;
        VpuWriteReg(coreIdx, W4_VPU_RESET_REQ, regVal); // Reset All blocks
        ...
        {
            /* VPU has a bus transaction controller */
            vdi_fio_write_register(coreIdx, W4_GDI_VCORE0_BUS_CTRL, 0x0);
        }
    ...
    else
    {
        ...
        if (codeSize < size * 2)
        {
            CVI_VC_ERR("INSUFFICIENT_RESOURCE\n");
            return RETCODE_INSUFFICIENT_RESOURCE;
        }
        ...
        regVal    = 0x80000000 | (0 << 16) | (W4_REMAP_CODE_INDEX << 12) | (1 << 11) | remapSize;
        VpuWriteReg(coreIdx, W4_VPU_REMAP_CTRL, regVal);
        VpuWriteReg(coreIdx, W4_VPU_REMAP_VADDR, 0x00000000); /* DO NOT
									 CHANGE!
								       */
        VpuWriteReg(coreIdx, W4_VPU_REMAP_PADDR, codeBase);
        VpuWriteReg(coreIdx, W4_ADDR_CODE_BASE, codeBase);
        ...
        VpuWriteReg(coreIdx, W4_VPU_BUSY_STATUS, 1);
        VpuWriteReg(coreIdx, W4_RET_SUCCESS, 0);
        VpuWriteReg(coreIdx, W4_CORE_INDEX, 0);
...
RetCode Wave4VpuSleepWake(Uint32 coreIdx, int iSleepWake, Uint32 size)
{
    CodecInstHeader hdr;
    Uint32          regVal;
    vpu_buffer_t    vb;
...
RetCode Wave4VpuReset(Uint32 coreIdx, SWResetMode resetMode)
{
    ...
    {
        ret = Wave4VpuSleepWake(coreIdx, TRUE, 0);
        if (ret != RETCODE_SUCCESS)
        {
            CVI_VC_ERR("Wave4VpuSleepWake error %d\n", ret);
            return ret;
        }
    }
    ...
    if (val)
    {
        ...
        if (vdi_wait_vpu_busy(coreIdx, __VPU_BUSY_TIMEOUT, W4_VPU_RESET_STATUS) == -1)
        {
            ...
            vdi_log(coreIdx, RESET_VPU, 2);
#endif
            CVI_VC_ERR("wait_vpu_busy timeout\n");
            return RETCODE_VPU_RESPONSE_TIMEOUT;
        }
        VpuWriteReg(coreIdx, W4_VPU_RESET_REQ, 0);
    }
    ...
    {
        ret = Wave4VpuSleepWake(coreIdx, FALSE, 0);
    }
...
RetCode Wave4VpuBuildUpDecParam(CodecInst *instance, DecOpenParam *param)
{
    ...
    else
    {
        ...
        if (VDI_ALLOCATE_MEMORY(instance->coreIdx, &pDecInfo->vbWork, 0, NULL) < 0)
        {
    ...
    vdi_clear_memory(instance->coreIdx, pDecInfo->vbWork.phys_addr, pDecInfo->vbWork.size);
...
#endif
        VDI_FREE_MEMORY(instance->coreIdx, &pDecInfo->vbWork);
        return RETCODE_VPU_RESPONSE_TIMEOUT;
    }
...
RetCode Wave4VpuDecRegisterFramebuffer(CodecInst *inst, FrameBuffer *fbArr, TiledMapType mapType, Uint32 count)
{
    ...
    if (mapType == COMPRESSED_FRAME_MAP)
    {
        ...
        if (inst->codecMode == HEVC_DEC || inst->codecMode == C7_HEVC_DEC || inst->codecMode == C7_VP9_DEC || inst->codecMode == C7_AVS2_DEC)
        {
            ...
            for (k = 0; k < mvCount; k++)
            {
                if (pDecInfo->vbMV[k].size == 0)
                {
                    ...
                    if (VDI_ALLOCATE_MEMORY(inst->coreIdx, &vbBuffer, 0, NULL) < 0)
                        return RETCODE_INSUFFICIENT_RESOURCE;
                    pDecInfo->vbMV[k] = vbBuffer;
                }
            }
        }
        fbcYTblSize = WAVE4_FBC_LUMA_TABLE_SIZE(
        ...
        {
            lumaStride =
                VPU_ALIGN16(pDecInfo->initialInfo.picWidth) * (pDecInfo->initialInfo.lumaBitdepth > 8 ? 5 : 4);
            lumaStride = VPU_ALIGN32(lumaStride);
            chromaStride =
                VPU_ALIGN16(pDecInfo->initialInfo.picWidth / 2) * (pDecInfo->initialInfo.chromaBitdepth > 8 ? 5 : 4);
    ...
    regVal = ((mapType == LINEAR_FRAME_MAP) << 28) | (axiID << 24) | (1 << 23) | /* PIXEL ORDER in 128bit. first pixel in low address */
             (yuvFormat << 20) | (colorFormat << 19) | (outputFormat << 16) | (stride);
    ...
    if (pDecInfo->interResChange == 1)
    {
        ...
        VpuWriteReg(coreIdx, W4_SFB_OPTION,
                    (pDecInfo->openParam.fbc_mode << 20) | (endian << 16) | (1 << 4) | (1 << 3) | 3);
        if (mapType == COMPRESSED_FRAME_MAP)
        {
        ...
        if (mapType == COMPRESSED_FRAME_MAP)
        {
            VpuWriteReg(coreIdx, W4_ADDR_FBC_Y_OFFSET0 + (i << 4), pDecInfo->vbFbcYTbl[idx].phys_addr);
            /* Luma FBC offset table */
            VpuWriteReg(coreIdx, W4_ADDR_FBC_C_OFFSET0 + (i << 4), pDecInfo->vbFbcCTbl[idx].phys_addr);
    ...
    if (ConfigSecAXIWave(
            coreIdx, inst->codecMode, &pDecInfo->secAxiInfo,
            pDecInfo->initialInfo.picWidth,
            pDecInfo->initialInfo.picHeight,
            sequenceInfo->profile, sequenceInfo->level)
        == 0)
    {
        return RETCODE_INSUFFICIENT_RESOURCE;
    }
    ...
    if (framebufferIndexes != NULL)
    {
        for (j = 0, i = W4_RET_DEC_DISPLAY_INDEX_0; i <= W4_RET_DEC_DISPLAY_INDEX_15 && j < size; i += 4, j++)
        {
            ...
            APIDPRINT("FLUSH LINEAR(%d) TILED(%d)\n",
                      framebufferIndexes[j].linearIndex,
                      framebufferIndexes[j].tiledIndex);
        }
    }
...
RetCode Wave4VpuBuildUpEncParam(CodecInst *instance, EncOpenParam *param)
{
    ...
    CVI_VC_MEM("vbWork, phys_addr = 0x" VCODEC_UINT64_FORMAT_HEX ", size = 0x%X\n", pEncInfo->vbWork.phys_addr, pEncInfo->vbWork.size);
    if (VDI_ALLOCATE_MEMORY(instance->coreIdx, &pEncInfo->vbWork, 0, NULL) < 0)
    {
    ...
    vdi_clear_memory(instance->coreIdx, pEncInfo->vbWork.phys_addr, pEncInfo->vbWork.size);
    ...
    if (vdi_wait_vpu_busy(instance->coreIdx, __VPU_BUSY_TIMEOUT, W4_VPU_BUSY_STATUS) == -1)
    {
        ...
        if (instance->loggingEnable)
            vdi_log(instance->coreIdx, CREATE_INSTANCE, 2);
#endif
        VDI_FREE_MEMORY(instance->coreIdx, &pEncInfo->vbWork);
        CVI_VC_ERR("CREATE_INSTANCE timeout\n");
#ifdef ENABLE_CNM_DEBUG_MSG
        vdi_print_vpu_status(instance->coreIdx);
#endif
        return RETCODE_VPU_RESPONSE_TIMEOUT;
    }
    ...
    {
        VDI_FREE_MEMORY(instance->coreIdx, &pEncInfo->vbWork);
        CVI_VC_ERR("CREATE_INSTANCE fail\n");
        ret = RETCODE_FAILURE;
    }
