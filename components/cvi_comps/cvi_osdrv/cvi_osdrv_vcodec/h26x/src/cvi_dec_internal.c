...
#define SECONDARY_AXI_H264 0 //0xf
...
extern void DisplayDecodedInformation(DecHandle handle, CodStd codec, Uint32 frameNo,
...
CVI_VDEC_CALLBACK pVdecDrvCbFunc = NULL;

cviVideoDecoder *cviAllocVideoDecoder(void)
{
	cviVideoDecoder *pvdec;

	pvdec = (cviVideoDecoder *) malloc(sizeof(cviVideoDecoder));
	if (!pvdec) {
...
int cviInitDecMcuEnv(cviDecConfig *pdeccfg)
{
	...
	if (pdeccfg->sizeInWord == 0 && pdeccfg->pusBitCode == NULL) {
		if (LoadFirmwareH(productId, (Uint8 **)&pdeccfg->pusBitCode, &pdeccfg->sizeInWord) < 0) {
...
// #endif
	...
	ret = VPU_InitWithBitcode(pdeccfg->coreIdx, (const Uint16 *)pdeccfg->pusBitCode, pdeccfg->sizeInWord);
	...
	pvdec->bsBufferCount = (pdeccfg->bitstreamMode == BS_MODE_PIC_END) ? 2 : 1;

#ifndef CVI_H26X_USE_ION_MEM
	for (index = 0; index < pvdec->bsBufferCount; index++) {
		pvdec->vbStream[index].size = pdeccfg->bsSize;
		CVI_VC_MEM("vbStream[%d].size = 0x%X\n", index, pvdec->vbStream[index].size);
		if (VDI_ALLOCATE_MEMORY(pdeccfg->coreIdx, &pvdec->vbStream[index], 0, NULL) < 0) {
	...
	if (VDI_ALLOCATE_MEMORY(pdeccfg->coreIdx, &pvdec->vbStream[0], BS_STREAM_CACHED, NULL) < 0) {
		VLOG(ERR, "fail to allocate bitstream buffer\n");
		return CVI_ERR_DEC_INIT;
	}
	pvdec->vbStream[0].size = pvdec->vbStream[1].size = pdeccfg->bsSize;
	pvdec->vbStream[1].phys_addr = pvdec->vbStream[0].phys_addr + pdeccfg->bsSize;
	pvdec->vbStream[1].virt_addr = pvdec->vbStream[0].virt_addr + pdeccfg->bsSize;
#endif

	CVI_VC_TRACE("cviApiMode = %d, bitstreamMode = %d\n", pvdec->cviApiMode, pdeccfg->bitstreamMode);
	...
	pdop->bitstreamBufferSize = pvdec->vbStream[0].size * pvdec->bsBufferCount;
	...
	if (VDI_ALLOCATE_MEMORY(pdeccfg->coreIdx, &pvdec->vbUserData, 0, NULL) < 0) {
		VLOG(ERR, "%s:%d Failed to allocate memory\n", __func__, __LINE__);
		return CVI_ERR_DECODE_END;
	}
	//pBase = (Uint8*)malloc(USERDATA_BUFFER_SIZE);
	VPU_DecGiveCommand(pvdec->decHandle, SET_ADDR_REP_USERDATA, (void *)&pvdec->vbUserData.phys_addr);
	VPU_DecGiveCommand(pvdec->decHandle, SET_SIZE_REP_USERDATA, (void *)&pvdec->vbUserData.size);
	VPU_DecGiveCommand(pvdec->decHandle, ENABLE_REP_USERDATA, (void *)&pdeccfg->enableUserData);

	if (pdeccfg->thumbnailMode == TRUE) {
		VPU_DecGiveCommand(pvdec->decHandle, ENABLE_DEC_THUMBNAIL_MODE, NULL);
	}

	pvdec->renderer = SimpleRenderer_Create(pvdec->decHandle, pdeccfg->renderType, pdeccfg->outputPath);
...
// #endif
	...
	end_time = cviGetCurrentTime();
	CVI_VC_PERF("VPU_InitWithBitcode = %lu us\n", end_time - start_time);
	...
	if (VDI_ALLOCATE_MEMORY(pdeccfg->coreIdx, pvbs, BS_STREAM_CACHED, NULL) < 0) {
	...
	VLOG(INFO, "[wtlMode            ]: %d\n", pdop->wtlMode);
	VLOG(INFO, "[bitstreamBuffer    ]: 0x%08lx\n", pdop->bitstreamBuffer);
	...
	if (pdop->bitstreamFormat == STD_AVC) {
		...
		if (ret != RETCODE_SUCCESS) {
			CVI_VC_ERR("VPU_DecGiveCommand[SET_LOW_DELAY_CONFIG] failed Error code is 0x%x\n", ret);
			return CVI_ERR_DECODE_END;
		}
	}
...
int cviInitSeqSetting(cviVideoDecoder *pvdec)
{
	...
	VPU_DecSetOutputFormat(pvdec->decHandle, pdeccfg->cbcrInterleave, pdeccfg->nv21);

	if (pdeccfg->bitFormat == STD_HEVC) {
		pvdec->bsQueueIndex = (pvdec->bsQueueIndex + 1) % (pvdec->bsBufferCount);
		VPU_DecSetRdPtr(pvdec->decHandle, pvdec->vbStream[pvdec->bsQueueIndex].phys_addr, TRUE);
	} else {
...
static int cviBitstreamFeeder_Act(cviVideoDecoder *pvdec)
{
	...
	return BitstreamFeeder_Act(pvdec->decHandle, pvdec->feeder, pdeccfg->streamEndian, bsBuf);
}
				 ...
				 vpu_buffer_t *retFbAddrs, Uint32 *retStride)
{
	...
	if (productId == PRODUCT_ID_4102 || productId == PRODUCT_ID_420 ||
		...
		format = (seqInfo.lumaBitdepth > 8 || seqInfo.chromaBitdepth > 8) ?
	...
	*retStride = VPU_ALIGN32(seqInfo.picWidth);
	framebufStride = CalcStride(seqInfo.picWidth, seqInfo.picHeight, format,
				    pdeccfg->cbcrInterleave, pdeccfg->mapType, FALSE, TRUE);
	...
	memset((void *)&fbAllocInfo, 0x00, sizeof(fbAllocInfo));
	if (pvdec->eVdecVBSource == E_CVI_VB_SRC_COMMON) {
		memset((void *)retFbArray, 0x00,
	...
	if (pvdec->eVdecVBSource == E_CVI_VB_SRC_COMMON) {
		for (index = 0; index < tiledFbCount; index++) {
			...
			if (VDI_ALLOCATE_MEMORY(coreIndex, pvb, 0, NULL) < 0) {
		...
		result = pVdecDrvCbFunc(pvdec->chnNum, CVI_H26X_DEC_CB_AllocFB, (void *)&hostAllocFb);
		if (result == 0) {
			CVI_VC_ERR("fail to allocate frame buffer\n");
			return FALSE;
		}
	...
	if (tiledFbCount != 0) {
		ret = VPU_DecAllocateFrameBuffer(decHandle, fbAllocInfo, retFbArray);
		if (ret != RETCODE_SUCCESS) {
			CVI_VC_ERR("failed to VPU_DecAllocateFrameBuffer(), ret(%d)\n", ret);
			ReleaseVideoMemory(coreIndex, retFbAddrs, totalFbCount);
			return FALSE;
		}
	}
	...
	if (pdeccfg->enableWTL == TRUE || linearFbCount != 0) {
		...
		if (pvdec->eVdecVBSource == E_CVI_VB_SRC_COMMON) {
			for (index = tiledFbCount; index < totalFbCount; index++) {
				pvb = &retFbAddrs[index];
				pvb->size = framebufSize;
				CVI_VC_MEM("tiledFbCount, FB[%d], pvb.size = 0x%X\n", index, pvb->size);
				if (VDI_ALLOCATE_MEMORY(coreIndex, pvb, 0, NULL) < 0) {
					CVI_VC_ERR("fail to allocate frame buffer\n");
					ReleaseVideoMemory(coreIndex, retFbAddrs, totalFbCount);
					return FALSE;
...
int _cviH265_FreeFrmBuf(cviVideoDecoder *pvdec)
{
	if (pvdec->eVdecVBSource == E_CVI_VB_SRC_COMMON) {
		...
		pVdecDrvCbFunc(pvdec->chnNum, CVI_H26X_DEC_CB_FreeFB, (void *)NULL);
	}

	return CVI_INIT_SEQ_OK;
}

static void _ReleasePreviousSequenceResources(
    DecHandle           handle,
    ...
    DecGetFramebufInfo * prevSeqFbInfo)
{
	...
	for (i = 0; i < MAX_REG_FRAME; i++) {
		if (prevSeqFbInfo->vbFbcYTbl[i].size > 0)
			VDI_FREE_MEMORY(coreIndex, &prevSeqFbInfo->vbFbcYTbl[i]);
		if (prevSeqFbInfo->vbFbcCTbl[i].size > 0)
			VDI_FREE_MEMORY(coreIndex, &prevSeqFbInfo->vbFbcCTbl[i]);
...
int _cviH265_Free_ReAlloc_FrmBuf(cviVideoDecoder *pvdec)
{
	...
	memset(p, 0x00, sizeof(SequenceMemInfo));
	...
	compressedFbCount = pSeqInfo->minFrameBufferCount + EXTRA_FRAME_BUFFER_NUM; /* max_dec_pic_buffering + @, @ >= 1 */
	linearFbCount = pSeqInfo->frameBufDelay + (1 + EXTRA_FRAME_BUFFER_NUM * 2);
	 ...
	 */
	memset((void *)pvdec->fbMem, 0x00, sizeof(vpu_buffer_t)*MAX_REG_FRAME);


	if (_AllocateH265DecFrameBuffer(pvdec, pvdec->decHandle, pdeccfg,
					compressedFbCount, linearFbCount, pvdec->Frame, pvdec->fbMem, &framebufStride) == FALSE) {
	...
	if (ret != RETCODE_SUCCESS) {
		CVI_VC_ERR("[SEQ_CHANGE] VPU_DecRegisterFrameBufferEx failed Error code is 0x%x\n", ret);
		return CVI_ERR_DECODE_END;
	}
...
int _cviH265_AllocFrmBuf(cviVideoDecoder *pvdec)
{
    ...
    */
	psau->u.wave4.useIpEnable = (pdeccfg->secondaryAXI & 0x01) ? TRUE : FALSE;
	psau->u.wave4.useLfRowEnable = (pdeccfg->secondaryAXI & 0x02) ? TRUE : FALSE;
	psau->u.wave4.useBitEnable = (pdeccfg->secondaryAXI & 0x04) ? TRUE : FALSE;
	VPU_DecGiveCommand(pvdec->decHandle, SET_SEC_AXI, psau);

	compressedFbCount = pSeqInfo->minFrameBufferCount + EXTRA_FRAME_BUFFER_NUM; // max_dec_pic_buffering
	if (pdeccfg->enableWTL == TRUE) {
		linearFbCount = compressedFbCount;
		VPU_DecGiveCommand(pvdec->decHandle, DEC_SET_WTL_FRAME_FORMAT, &pdeccfg->wtlFormat);
	} else {
		linearFbCount = 0;
	}

	CVI_VC_INFO("min FB Buf Count = %d, compressedFbCount = %d, linearFbCount = %d\n",
		pSeqInfo->minFrameBufferCount, compressedFbCount, linearFbCount);
    ...
    ********************************************************************************/
	...
	val = (pdeccfg->bitFormat == STD_HEVC) ? SEQ_CHANGE_ENABLE_ALL_HEVC : SEQ_CHANGE_ENABLE_ALL_VP9;
	VPU_DecGiveCommand(pvdec->decHandle, DEC_SET_SEQ_CHANGE_MASK, (void *)&val);
...
int _cviH264_AllocFrmBuf(cviVideoDecoder *pvdec)
{
	...
	if (pvdec->eVdecVBSource == E_CVI_VB_SRC_COMMON) {
		for (index = 0; index < pvdec->fbCount; index++) {
			...
			if (VDI_ALLOCATE_MEMORY(pdeccfg->coreIdx, pvb, 0, NULL) < 0) {
				CVI_VC_ERR("fail to allocate frame buffer\n");
				return CVI_ERR_DECODE_END;
			}
	...
	ret = VPU_DecAllocateFrameBuffer(pvdec->decHandle, pvdec->fbAllocInfo, pvdec->Frame);
	if (ret != RETCODE_SUCCESS) {
	...
	if (pdeccfg->enableWTL == TRUE) {
		...
		for (index = pvdec->fbCount; index < pvdec->fbCount * 2; index++) {
			pvb = &pvdec->fbMem[index];
			pvb->size = framebufSize;
		...
		pFbAllocInfo->num = pvdec->fbCount;
		pFbAllocInfo->type = FB_TYPE_CODEC;
		ret = VPU_DecAllocateFrameBuffer(pvdec->decHandle, pvdec->fbAllocInfo, &pvdec->Frame[pvdec->fbCount]);
		if (ret != RETCODE_SUCCESS) {
			VLOG(ERR, "%s:%d failed to VPU_DecAllocateFrameBuffer() ret:%d\n", __func__, __LINE__, ret);
			return CVI_ERR_DECODE_END;
		}
	...
	framebufStride =
		CalcStride(pSeqInfo->picWidth, pSeqInfo->picHeight, FORMAT_420,
			   pdeccfg->cbcrInterleave,
			   pdeccfg->enableWTL == TRUE ? LINEAR_FRAME_MAP : pdeccfg->mapType,
			   FALSE, TRUE);
	...
	if (pvdec->enablePPU == TRUE) {
		...
		sizePPUFb = VPU_GetFrameBufSize(pdeccfg->coreIdx, stridePpu,
						rotateHeight, LINEAR_FRAME_MAP, FORMAT_420,
						pdeccfg->cbcrInterleave, NULL);
		...
		ret = VPU_DecAllocateFrameBuffer(pvdec->decHandle, pvdec->fbAllocInfo, pvdec->PPUFrame);
		if (ret != RETCODE_SUCCESS) {
			VLOG(ERR, "%s:%d failed to VPU_DecAllocateFrameBuffer() ret:%d\n", __func__, __LINE__, ret);
			return CVI_ERR_DECODE_END;
		}
		...
		for (index = 0; index < pvdec->ppuFbCount; index++) {
			Queue_Enqueue(pvdec->ppuQ, (void *)&pvdec->PPUFrame[index]);
		}
	}
...
static int cviInitSeq(cviVideoDecoder *pvdec)
{
	...
	if (ret != RETCODE_SUCCESS) {
		seqInitEscape = 0;
		CVI_VC_INFO("can not to set seqInitEscape in current bitstream mode Option\n");
	}
	...
	if (seqInitEscape) {
		...
		while (pvdec->seqInited == FALSE) {
			...
			interruptFlag = VPU_WaitInterrupt(pdeccfg->coreIdx, VPU_WAIT_TIME_OUT);
			if (interruptFlag == -1) {
				if (timeoutCount * VPU_WAIT_TIME_OUT >
				    VPU_DEC_TIMEOUT) {
					VLOG(ERR,
					     "\n VPU interrupt wait timeout\n");
					VPU_SWReset(pdeccfg->coreIdx, SW_RESET_SAFETY, pvdec->decHandle);
					return CVI_ERR_DECODE_END;
				}
				timeoutCount++;
				interruptFlag = 0;
		...
		if (ret != RETCODE_SUCCESS) {
			CVI_VC_ERR("SEQ_INIT(ERROR REASON: %d(0x%x)\n", pSeqInfo->seqInitErrReason, pSeqInfo->seqInitErrReason);
			pvdec->seqInited = FALSE;
	...
	DisplayDecodedInformation(pvdec->decHandle, (CodStd)pdeccfg->bitFormat, 0, NULL);
...
int _cviH265_DecodeOneFrame(cviVideoDecoder *pvdec)
{
	...
	********************************************************************************/
	...
	if (pvdec->cviApiMode == API_MODE_DRIVER) {
		pDisplayInfo = (DecOutputInfo *)SimpleRenderer_GetFreeFrameInfo(pvdec->renderer);
		...
		if (pDisplayInfo != NULL) {
			...
			if (ptr && *ptr != pDisplayInfo->sequenceNo) {
				...
				index = (*ptr) % MAX_SEQUENCE_MEM_COUNT;
				p = &pvdec->seqMemInfo[index];
				_ReleasePreviousSequenceResources(pvdec->decHandle, p->allocFbMem, &p->fbInfo);
				memset(p, 0x00, sizeof(SequenceMemInfo));
				Queue_Dequeue(pvdec->sequenceQ);
			}
			if (pDisplayInfo->sequenceNo == pSeqInfo->sequenceNo) {
				VPU_DecClrDispFlag(pvdec->decHandle, pDisplayInfo->indexFrameDisplay);
			}
		}
	}

	if ((pvdec->cviApiMode == API_MODE_SDK) && (pvdec->waitFrmBufFree == TRUE)) {
		Int32 count;

		count = pVdecDrvCbFunc(pvdec->chnNum, CVI_H26X_DEC_CB_GET_DISPQ_COUNT, NULL);
...
int _cviH264_DecodeOneFrame(cviVideoDecoder *pvdec)
{
	...
	if (pvdec->cviApiMode == API_MODE_DRIVER) {
		do {
			...
			if (pvdec->enablePPU == TRUE) {
				Queue_Enqueue(pvdec->ppuQ, (void *)&pDisplayInfo->dispFrame);
			} else {
				CVI_VC_DISP("myIndex = %d\n", pDisplayInfo->dispFrame.myIndex);
				VPU_DecClrDispFlag(pvdec->decHandle, pDisplayInfo->dispFrame.myIndex);
			}
		} while (1);
	}

	if (pvdec->needStream == TRUE) {
		VPU_DecSetRdPtr(pvdec->decHandle, pvdec->vbStream[0].phys_addr, TRUE);
	...
	if (pvdec->assertedFieldDoneInt == TRUE) {
		...
		if (pvdec->enablePPU == TRUE) {
			pvdec->ppuFb = (FrameBuffer *)Queue_Dequeue(pvdec->ppuQ);
			if (pvdec->ppuFb == NULL) {
				MSleep(0);
				pvdec->needStream = FALSE;
			...
			VPU_DecGiveCommand(pvdec->decHandle, SET_ROTATOR_OUTPUT, (void *)pvdec->ppuFb);
			if (pdeccfg->coda9.rotate > 0) {
				VPU_DecGiveCommand(pvdec->decHandle, ENABLE_ROTATION, NULL);
			}
			if (pdeccfg->coda9.mirror > 0) {
				VPU_DecGiveCommand(pvdec->decHandle, ENABLE_MIRRORING, NULL);
			}
			if (pdeccfg->coda9.enableDering == TRUE) {
				VPU_DecGiveCommand(pvdec->decHandle, ENABLE_DERING, NULL);
			}
		}

		VPU_DecGiveCommand(pvdec->decHandle, DEC_SET_TARGET_TEMPORAL_ID, (void *)&pdeccfg->tid);
...
int _cviH265_WaitInterrupt(cviVideoDecoder *pvdec)
{
	...
	while (repeat == TRUE) {
		...
		if (interruptFlag & (1 << INT_WAVE_BIT_BUF_EMPTY)) {
			...
			Uint32 avail;
			VPU_DecGetBitstreamBuffer(pvdec->decHandle, NULL, NULL, &avail);
			if (avail < FEEDING_SIZE) {
				VPU_DecUpdateBitstreamBuffer(pvdec->decHandle, 0); // Set end-of-stream
		...
		if (pdeccfg->bitstreamMode == BS_MODE_INTERRUPT && pvdec->seqChangeRequest == FALSE) {
			if (pvdec->cviApiMode == API_MODE_SDK) {
				PhysicalAddress rdPtr, wrPtr;
				Uint32 room;

				CVI_VC_BS("SDK, BS_MODE_INTERRUPT\n");
				VPU_DecGetBitstreamBuffer(pvdec->decHandle, &rdPtr, &wrPtr, &room);
				CVI_VC_BS("rdPtr = 0x%lX, wrPtr = 0x%lX, room = 0x%X\n", rdPtr, wrPtr, room);
			} else {
				pvdec->nWritten = BitstreamFeeder_Act(pvdec->decHandle, pvdec->feeder, pdeccfg->streamEndian, NULL);
				if (pvdec->nWritten < 0) {
					return CVI_ERR_DECODE_END;
				}
			}
...
int _cviH265_GetDecodedData(cviVideoDecoder *pvdec)
{
	...
	if (ret != RETCODE_SUCCESS) {
		CVI_VC_ERR("VPU_DecGetOutputInfo failed Error code is 0x%x , instIdx=%d\n", ret, pdeccfg->instIdx);
	...
	if ((pdoi->decodingSuccess & 0x01) == 0) {
		if (pdoi->indexFramePrescan == -2) {
			...
			CVI_VC_ERR(
				...
				pdoi->errorReasonExt);
			if (pdoi->errorReason == WAVE4_SYSERR_WATCHDOG_TIMEOUT) {
				VPU_SWReset(pdeccfg->coreIdx, SW_RESET_SAFETY, pvdec->decHandle);
	...
	if (pvdec->sequenceChangeFlag) {
		...
		dpbChanged = (pvdec->sequenceChangeFlag & SEQ_CHANGE_ENABLE_DPB_COUNT) ? TRUE : FALSE;
		sizeChanged = (pvdec->sequenceChangeFlag & SEQ_CHANGE_ENABLE_SIZE) ? TRUE : FALSE;
		bitDepthChanged = (pvdec->sequenceChangeFlag & SEQ_CHANGE_ENABLE_BITDEPTH) ? TRUE : FALSE;

		if (dpbChanged || sizeChanged || bitDepthChanged) {
			DecOutputInfo *pDecOutInfo = (DecOutputInfo *)malloc(sizeof(DecOutputInfo) * MAX_GDI_IDX);
			// BOOL remainingFbs[MAX_REG_FRAME];
			Uint32 retNum;
			Uint32 curSeqNo = pdoi->sequenceNo;
			Uint32 seqMemIndex = curSeqNo % MAX_SEQUENCE_MEM_COUNT;
			SequenceMemInfo *pSeqMem = &pvdec->seqMemInfo[seqMemIndex];
			DecGetFramebufInfo *prevSeqFbInfo = (DecGetFramebufInfo *)malloc(sizeof(DecGetFramebufInfo));
			// Uint32 framebufStride;
			...
			if (pvdec->cviApiMode == API_MODE_SDK) {
				Queue_Enqueue(pvdec->sequenceQ, (void *)&curSeqNo);
				// Get previous memory related to framebuffer
				VPU_DecGiveCommand(pvdec->decHandle, DEC_GET_FRAMEBUF_INFO, (void *)prevSeqFbInfo);
				memcpy(&pSeqMem->fbInfo, prevSeqFbInfo, sizeof(DecGetFramebufInfo));

				// Flush all remaining framebuffers of previous sequence.
				VPU_DecFrameBufferFlush(pvdec->decHandle, pDecOutInfo, &retNum);
	...
	do {
		...
		memcpy((void *)pdoi, pDisplayInfo, sizeof(DecOutputInfo));
		DisplayDecodedInformation(pvdec->decHandle, (CodStd)pdeccfg->bitFormat, pvdec->decodedIdx, pdoi);
	...
	if (pDisplayInfo != NULL) {
		...
		if (pvdec->dispIdx > 0 && pvdec->dispIdx == (Uint32)pdeccfg->forceOutNum) {
			return CVI_DECODE_BREAK;
		}
	...
	if (pdeccfg->bitstreamMode == BS_MODE_PIC_END) {
		...
		if (pvdec->decHandle->codecMode == C7_HEVC_DEC) {
			...
			if (pdoi->indexFramePrescan == -1 || pvdec->seqChangeRequest != 0) {
				pvdec->needStream = FALSE;
				VPU_DecSetRdPtr(pvdec->decHandle, pvdec->vbStream[pvdec->bsQueueIndex].phys_addr, FALSE);
			}
		}
	}
	...
	if (pdoi->indexFrameDisplay == DISPLAY_IDX_FLAG_SEQ_END) {
		if (Queue_Peek(pvdec->displayQ) != NULL) {
			VLOG(ERR, "Queue_Peek=0x%llx\n", (unsigned long long)Queue_Peek(pvdec->displayQ));
			pvdec->success = FALSE;
		}
	...
	if ((pdoi->indexFrameDecoded == DECODED_IDX_FLAG_NO_FB) && (pdoi->indexFrameDisplay == DISPLAY_IDX_FLAG_NO_FB)) {
		return CVI_DECODE_NO_FB;
	}
...
int _cviH264_GetDecodedData(cviVideoDecoder *pvdec)
{
	...
	if (pdoi->sequenceChanged == TRUE) {
		...
		ret = VPU_DecSetRdPtr(pvdec->decHandle, pvdec->seqChangedRdPtr, TRUE);
		if (ret != RETCODE_SUCCESS) {
			CVI_VC_ERR("Failed to VPU_DecSetRdPtr(%d), ret(%d)\n", pvdec->seqChangedRdPtr, ret);
			return CVI_DECODE_BREAK;
		}
		pvdec->seqChangedStreamEndFlag = pdoi->streamEndFlag;
		VPU_DecUpdateBitstreamBuffer(pvdec->decHandle, 1); // let f/w to know
		// stream end condition
		// in bitstream buffer.
		// force to know that
		// bitstream buffer
		// will be empty.
		VPU_DecUpdateBitstreamBuffer(pvdec->decHandle, STREAM_END_SET_FLAG); // set to stream end
	...
	if (pvdec->enablePPU == TRUE) {
		if (pvdec->prevFbIndex >= 0) {
			VPU_DecClrDispFlag(pvdec->decHandle, pvdec->prevFbIndex);
		}
		...
		if (pvdec->waitPostProcessing != TRUE) {
			...
			pvdec->needStream = (pdeccfg->bitstreamMode == BS_MODE_PIC_END);
	...
	if (pdoi->indexFrameDisplay >= 0 || pvdec->enablePPU == TRUE) {
		...
		SimpleRenderer_Act(pvdec->renderer, pdoi, NULL, width, height);
		pvdec->dispIdx++;
	}
		...
		if (pvdec->seqChangeRequest == TRUE) {
			...
			if (pvdec->cviApiMode == API_MODE_SDK) {
				...
				if (pvdec->seqChangedStreamEndFlag == 1) {
					VPU_DecUpdateBitstreamBuffer(pvdec->decHandle, STREAM_END_SET_FLAG);
				} else {
					VPU_DecUpdateBitstreamBuffer(pvdec->decHandle, STREAM_END_CLEAR_FLAG);
				}

				ret = CVI_SEQ_CHG_WAIT_BUF_FREE;
			} else {
				VPU_DecSetRdPtr(pvdec->decHandle, pvdec->seqChangedRdPtr, TRUE);
				...
				if (pvdec->seqChangedWrPtr >= pvdec->seqChangedRdPtr) {
					VPU_DecUpdateBitstreamBuffer(pvdec->decHandle, pvdec->seqChangedWrPtr - pvdec->seqChangedRdPtr);
				} else {
					VPU_DecUpdateBitstreamBuffer(
						pvdec->decHandle,
						(pvdec->vbStream[0].phys_addr + pvdec->vbStream[0].size) - pvdec->seqChangedRdPtr +
						(pvdec->seqChangedWrPtr - pvdec->vbStream[0].phys_addr));
				}
...
void _cviH265_CloseDecoder(cviVideoDecoder *pvdec)
{
	...
	if (pvdec->decHandle) {
		for (index = 0; index < MAX_SEQUENCE_MEM_COUNT; index++) {
			_ReleasePreviousSequenceResources(pvdec->decHandle,
...
int parseArgs(int argc, char **argv, cviDecConfig *pdeccfg)
{
	...
	struct OptionExt options_help[] = {
		...
		{ "enable-cbcrinterleave", 0, NULL, 0, "--enable-cbcrinterleave" },
	...
	while ((opt = getopt_long(argc, argv, optString, options, &index)) != -1) {
		...
		case 0:
			if (strcmp(options[index].name, "output") == 0) {
				memcpy(pdeccfg->outputPath, optarg, strlen(optarg));
			} else if (strcmp(options[index].name, "input") == 0) {
				memcpy(pdeccfg->inputPath, optarg, strlen(optarg));
			} else if (strcmp(options[index].name, "codec") == 0) {
				pdeccfg->bitFormat = atoi(optarg);
			} else if (strcmp(options[index].name, "render") == 0) {
				pdeccfg->renderType = (RenderDeviceType)atoi(optarg);
				if (pdeccfg->renderType < RENDER_DEVICE_NULL || pdeccfg->renderType >= RENDER_DEVICE_MAX) {
					CVI_VC_ERR("unknown render device type(%d)\n", pdeccfg->renderType);
					Help(argv[0]);
					return -1;
				}
			} else if (strcmp(options[index].name, "maptype") == 0) {
				pdeccfg->mapType = (TiledMapType)atoi(optarg);
			} else if (strcmp(options[index].name, "disable-wtl") == 0) {
				pdeccfg->enableWTL = FALSE;
			} else if (strcmp(options[index].name, "coreIdx") == 0) {
				pdeccfg->coreIdx = atoi(optarg);
			} else if (strcmp(options[index].name, "loop-count") == 0) {
				...
				pdeccfg->streamEndian = (EndianMode)atoi(optarg);
			} else if (strcmp(options[index].name,
					  "frame-endian") == 0) {
				pdeccfg->frameEndian = (EndianMode)atoi(optarg);
			} else if (strcmp(options[index].name, "enable-nv21") == 0) {
				pdeccfg->nv21 = TRUE;
				pdeccfg->cbcrInterleave = TRUE;
			} else if (strcmp(options[index].name, "secondary-axi") == 0) {
				pdeccfg->secondaryAXI = strtoul(optarg, stop, !strncmp("0x", optarg, 2) ? 16 : 10);
			} else if (strcmp(options[index].name, "bsmode") == 0) {
				pdeccfg->bitstreamMode = atoi(optarg);
			} else if (strcmp(options[index].name, "enable-tiled2linear") == 0) {
				pdeccfg->coda9.enableTiled2Linear = TRUE;
				...
				val = atoi(optarg);
				if ((val % 90) != 0) {
					CVI_VC_ERR("Invalid rotation value: %d\n", val);
					Help(argv[0]);
					return -1;
				}
				...
				val = atoi(optarg);
				if (val < 0 || val > 3) {
					CVI_VC_ERR("Invalid mirror option: %d\n", val);
					Help(argv[0]);
					return -1;
				}
				pdeccfg->coda9.mirror = val;
			} else if (strcmp(options[index].name, "enable-dering") == 0) {
				pdeccfg->coda9.enableDering = TRUE;
			} else if (strcmp(options[index].name, "enable-deblock") == 0) {
				pdeccfg->coda9.enableDeblock = TRUE;
			} else if (strcmp(options[index].name, "mp4class") == 0) {
				pdeccfg->coda9.mp4class = atoi(optarg);
			} else if (strcmp(options[index].name, "ref-yuv") == 0) {
				memcpy(pdeccfg->refYuvPath, optarg, strlen(optarg));
			} else if (strcmp(options[index].name, "enable-mvc") == 0) {
				pdeccfg->coda9.enableMvc = TRUE;
			} else if (strcmp(options[index].name, "low-delay") == 0) {
...
void checkDecConfig(cviDecConfig *pdeccfg)
{
	...
	CVI_VC_WARN("------ WRONG PARAMETER COMBINATION ------\n");
	...
	case LINEAR_FIELD_MAP:
		if (pdeccfg->coda9.enableTiled2Linear == TRUE || pdeccfg->enableWTL == TRUE) {
	...
	case TILED_FIELD_MB_RASTER_MAP:
		if (pdeccfg->cbcrInterleave == FALSE) {
			CVI_VC_WARN("CBCR-interleave must be enable when maptype is TILED_FRAME/FIELD_MB_RASTER_MAP.\n");
	...
	if (pdeccfg->coda9.enableTiled2Linear == TRUE) {
		CVI_VC_WARN("In case of Tiledmap, disabled BWB for better performance.\n");
		pdeccfg->coda9.enableBWB = FALSE;
	}

	if (pdeccfg->coda9.lowDelay.lowDelayEn == TRUE && pdeccfg->bitFormat != STD_AVC) {
		CVI_VC_WARN("The low-delay decoding option is valid when a codec is H.264\n");
...
int cviAttachFrmBuf(cviVideoDecoder *pvdec, cviBufInfo *pFrmBufArray, int nFrmNum)
{
	int ret = 0;
	int i = 0;
	...
	for (i = 0; i < nFrmNum; i++) {
		...
		CVI_VC_TRACE("vbAddr=0x%lx, vbsize=0x%x, vbVirt=%p\n",
			     (Uint64)pvdec->vbFrame[i].phys_addr, pvdec->vbFrame[i].size, (void *)pvdec->vbFrame[i].virt_addr);
	}
