...
static void set_pic_qp_by_delta(stRcInfo *pRcInfo, EncOpenParam *pEncOP, int delta)
{
    pRcInfo->picIMaxQp = (pEncOP->userQpMaxI > 0) ? CLIP3(0, 51, pEncOP->userQpMaxI - delta) : 51;
    pRcInfo->picIMinQp = (pEncOP->userQpMinI > 0) ? CLIP3(0, 51, pEncOP->userQpMinI + delta) : 0;
    pRcInfo->picPMaxQp = (pEncOP->userQpMaxP > 0) ? CLIP3(0, 51, pEncOP->userQpMaxP - delta) : 51;
    pRcInfo->picPMinQp = (pEncOP->userQpMinP > 0) ? CLIP3(0, 51, pEncOP->userQpMinP + delta) : 0;
}
...
int _cviEncRc_getBitrate(stRcInfo *pRcInfo)
{
    ...
    return (pRcInfo->rcMode == RC_MODE_CBR || pRcInfo->rcMode == RC_MODE_UBR) ? pRcInfo->targetBitrate : pRcInfo->maximumBitrate;
}
...
static void _cviEncRc_setBitrateParam(stRcInfo *pRcInfo, EncOpenParam *pEncOP)
{
    ...
    if (pRcInfo->rcMode == RC_MODE_VBR)
    {
        ...
        pRcInfo->targetBitrate      = (pEncOP->bitRate * pEncOP->changePos) / 100;
    ...
    else if (pRcInfo->rcMode == RC_MODE_AVBR)
    {
        ...
        int bitrateByMotLv = (pEncOP->bitRate * motBitRatio) / 100;
        ...
        pRcInfo->targetBitrate         = (bitrateByMotLv * pEncOP->changePos) / 100;
        pRcInfo->maximumBitrate        = pEncOP->bitRate;
        CVI_VC_INFO("new targetBitrate = %d\n", pRcInfo->targetBitrate);
    }
    else
    {
    ...
    if (pRcInfo->cviRcEn)
    {
        ...
        pRcInfo->picAvgBit = (int)(pRcInfo->targetBitrate * 1000 * frameRateDiv / frameRateRes);
    }
    else
    {
        ...
        CVI_VC_RC("cviRcEn = %d\n", pRcInfo->cviRcEn);
#else
        fFramerate = (float)frameRateRes / (float)frameRateDiv;
        pRcInfo->picAvgBit =
            (int)(pRcInfo->targetBitrate * 1000 / fFramerate + 0.5);
#endif
    }
    ...
    if ((pRcInfo->codec == STD_AVC || pRcInfo->rcMode == RC_MODE_UBR) && pEncOP->maxIprop > 0 && pEncOP->gopSize > 1)
    {
        if (pRcInfo->cviRcEn)
        {
            pRcInfo->maxIPicBit =
                (int)(pEncOP->maxIprop * pEncOP->gopSize * pRcInfo->targetBitrate * 1000 * frameRateDiv / frameRateRes / (pEncOP->maxIprop + pEncOP->gopSize - 1));
        }
        else
        {
#if 1 //def USE_KERNEL_MODE
...
#else
            pRcInfo->maxIPicBit =
                (int)(pEncOP->maxIprop * ((float)pEncOP->gopSize * pRcInfo->targetBitrate * 1000 / (fFramerate * (pEncOP->maxIprop + pEncOP->gopSize - 1))));
#endif
        }
    ...
    if (pRcInfo->cviRcEn)
    {
        RC_Float frameRate = CVI_FLOAT_DIV(INT_TO_CVI_FLOAT(frameRateRes), INT_TO_CVI_FLOAT(frameRateDiv));

        if (dbg_mask & CVI_MASK_CVRC)
        {
            CVI_RC_FLOAT("bitRate = %d, frameRate = %f\n", pEncOP->bitRate, getFloat(frameRate));
        }
        cviRcKernel_setBitrateAndFrameRate(pRcKerInfo, pEncOP->bitRate * 1000, frameRate);
    }
}
...
BOOL cviEnc_Avbr_PicCtrl(stRcInfo *pRcInfo, EncOpenParam *pEncOP, int frameIdx)
{
    ...
    pRcInfo->avbrChangeValidCnt = (pRcInfo->avbrChangeValidCnt > 0) ? (pRcInfo->avbrChangeValidCnt - 1) : 0;
    ...
    if (pRcInfo->avbrChangeValidCnt == 0)
    {
        ...
        int diffMotLv           = (pRcInfo->lastPeriodMotionLv > pRcInfo->periodMotionLv) ? (pRcInfo->lastPeriodMotionLv - pRcInfo->periodMotionLv) >> 4 : (pRcInfo->periodMotionLv - pRcInfo->lastPeriodMotionLv) >> 4;
        changeBrEn              = (diffMotLv >= 1) || (pRcInfo->periodMotionLv == 0 && pRcInfo->lastPeriodMotionLv > 0 && pRcInfo->lastPeriodMotionLv <= 16);
        ...
        if (changeBrEn)
        {
            ...
            pRcInfo->avbrChangeValidCnt = MAX(20, pRcInfo->framerate); //AVBR_MAX_BITRATE_WIN;
        }
...
BOOL cviEncRc_Avbr_CheckFrameSkip(stRcInfo *pRcInfo, EncOpenParam *pEncOP, int isIFrame)
{
    BOOL isFrameSkip = FALSE;
    BOOL isStillPic  = TRUE;
    int  idx;

    if ((!pRcInfo->rcEnable) || (pRcInfo->rcMode != RC_MODE_AVBR) || pRcInfo->avbrContiSkipNum == 0)
    {
        return FALSE;
    }
    ...
    if (isStillPic && pRcInfo->frameSkipCnt < pRcInfo->avbrContiSkipNum && !isIFrame)
    {
        isFrameSkip = TRUE;
    ...
    CVI_VC_INFO("FrameSkipByStillPic: %d %d %d\n", isFrameSkip, pEncOP->picMotionLevel, pRcInfo->periodMotionLv);
    return isFrameSkip;
}
...
void cviEncRc_Open(stRcInfo *pRcInfo, EncOpenParam *pEncOP)
{
    ...
    // frame skipping
    pRcInfo->contiSkipNum    = (!pEncOP->frmLostOpen) ? 0 : (pEncOP->encFrmGaps == 0) ? 65535
                                                                                      : pEncOP->encFrmGaps;
    ...
    pRcInfo->avbrContiSkipNum   = (!pEncOP->avbrFrmLostOpen) ? 0 : (pEncOP->avbrFrmGaps == 0) ? 65535
                                                                                              : pEncOP->avbrFrmGaps;
	...
	pRcInfo->bitrateBuffer =
		pRcInfo->targetBitrate * 0.4 * 1000; // initial buffer level
#else
    pRcInfo->bitrateBuffer =
...
#if 0 //ndef USE_KERNEL_MODE
	cviRcMdl_Init(&pRcInfo->IntraRqMdl,
...
void cviEncRc_UpdatePicInfo(stRcInfo *pRcInfo, EncOutputInfo *pEncOutInfo)
{
    ...
    pRcInfo->bitrateBuffer = MAX(pRcInfo->bitrateBuffer + encPicBit - pRcInfo->picAvgBit, 0);
    pRcInfo->isLastPicI    = (pEncOutInfo->picType == PIC_TYPE_I) || (pEncOutInfo->picType == PIC_TYPE_IDR);
...
static void cviEncRc_RcKernelUpdatePic(stRcInfo *pRcInfo, EncOutputInfo *pEncOutInfo)
{
    ...
    if (pRcInfo->codec == STD_AVC)
    {
        int mbs = pRcInfo->numOfPixel / 256;
    ...
    CVI_VC_RC("isLastPicI = %d, encodedBit = %d\n",
              pRcInfo->isLastPicI, pRcPicIn->encodedBit);
...
{
    pRcInfo->contiSkipNum    = (!frmLostOpen) ? 0 : (encFrmGaps == 0) ? 65535
                                                                      : encFrmGaps;
...
int cviEncRc_ChkFrameSkipByBitrate(stRcInfo *pRcInfo, int isIFrame)
{
    ...
    if (pRcInfo->bitrateBuffer > pRcInfo->frameSkipBufThr && pRcInfo->frameSkipCnt < pRcInfo->contiSkipNum && !isIFrame)
    {
        isFrameDrop = 1;
        pRcInfo->frameSkipCnt++;
    }
    ...
    CVI_VC_INFO("FrameDrop: %d  %d / %d \n", isFrameDrop, pRcInfo->bitrateBuffer, pRcInfo->frameSkipBufThr);
    return isFrameDrop;
}
...
#if 0 //ndef USE_KERNEL_MODE
void cviRcMdl_Init(stRQModel *pRqMdl, int picPelNum)
{
	pRqMdl->alpha = 28;
	pRqMdl->beta = -0.3;
	pRqMdl->alphaStep = 0.2;
	pRqMdl->betaStep = 0.1;
	pRqMdl->picPelNum = picPelNum;
}

static float calculateQ(float alpha, float beta, float bpp)
{
	return alpha * pow(bpp, beta);
}

int cviRcMdl_EstimateQ(stRQModel *pRqMdl, int targetBit)
{
	float bpp = targetBit / (float)pRqMdl->picPelNum;
	int estQ = (int)(calculateQ(pRqMdl->alpha, pRqMdl->beta, bpp) + 0.5);
	estQ = CLIP3(pRqMdl->lastQ - 6, pRqMdl->lastQ + 6, estQ);
	return CLIP3(0, 51, estQ);
}

void cviRcMdl_UpdateModel(stRQModel *pRqMdl, int encBit, int q)
{
	float actualBpp = encBit / (float)pRqMdl->picPelNum;
	// update model parameters
	float refQ = calculateQ(pRqMdl->alpha, pRqMdl->beta, actualBpp);
	float alpha = pRqMdl->alpha;
	float beta = pRqMdl->beta;
	float lnQDiff, lnbpp;
	float inputQ;

	pRqMdl->lastQ = q;
	inputQ = q;
	refQ = CLIP3(inputQ - 8, inputQ + 8, refQ);
	lnQDiff = log(inputQ) - log(refQ);
	alpha += pRqMdl->alphaStep * lnQDiff * alpha;
	lnbpp = log(actualBpp);
	lnbpp = CLIP3(-5.0, 0.1, lnbpp);
	beta += pRqMdl->betaStep * lnQDiff * lnbpp;

	alpha = CLIP3(0.25, 100, alpha);
	beta = CLIP3(-3, -0.1, beta);
	pRqMdl->alpha = alpha;
	pRqMdl->beta = beta;
}
#endif
