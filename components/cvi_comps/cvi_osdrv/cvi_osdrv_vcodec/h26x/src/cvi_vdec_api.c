...
int cviVDecOpen(cviInitDecConfig *pInitDecCfg, void *pHandle)
{
	...
	Uint32 core_idx = pInitDecCfg->codec == CODEC_H265 ? CORE_H265 : CORE_H264;
...
int cviVDecDecOnePic(void *pHandle, cviDecOnePicCfg *pdopc, int timeout_ms)
{
	...
	if (pvdec->cviApiMode == API_MODE_SDK) {
		...
		CVI_VC_BS("virt_addr = %p, size = %d, bStreamOfEnd = %d\n",
			  psdkp->bsBuf.virt_addr, psdkp->bsBuf.size, pvdec->bStreamOfEnd);
	}
...
int cviVDecGetFrame(void *pHandle, cviDispFrameCfg *pdfc)
{
	...
	pdfc->addrY = cviVDecGetVirtAddr(pHandle, fb->bufY);
	if (!pdfc->addrY) {
		CVI_VC_ERR("vdi_get_vir_addr\n");
		return CVI_ERR_VIR_ADDR;
	}
	pdfc->addrCb = cviVDecGetVirtAddr(pHandle, fb->bufCb);
	if (!pdfc->addrCb) {
		CVI_VC_ERR("vdi_get_vir_addr\n");
		return CVI_ERR_VIR_ADDR;
	}
	...
	if (IS_VALID_PA(fb->bufCr)) {
		...
		pdfc->addrCr = cviVDecGetVirtAddr(pHandle, fb->bufCr);
		if (!pdfc->addrCr) {
			CVI_VC_ERR("vdi_get_vir_addr\n");
			return CVI_ERR_VIR_ADDR;
		}
...
void *cviVDecGetVirtAddr(void *pHandle, PhysicalAddress phyAddr)
{
	...
	vpu_buffer_t tmpVb = { 0 };

	if (pHandle == NULL) {
		CVI_VC_ERR("pHandle == NULL\n");
		return NULL;
	}
	...
	if (pvdec->eVdecVBSource == E_CVI_VB_SRC_COMMON) {
		pRetAddr = vdi_get_vir_addr(VPU_HANDLE_CORE_INDEX(pvdec->decHandle), phyAddr);
	} else {
		for (Uint32 i = 0; i < pvdec->nVbFrameNum; i++) {
			if (phyAddr >= pvdec->vbFrame[i].phys_addr &&
			    phyAddr < (pvdec->vbFrame[i].phys_addr + pvdec->vbFrame[i].size)) {
				tmpVb = pvdec->vbFrame[i];
				break;
			}
		}

		uOffset = (unsigned long)phyAddr - (unsigned long)tmpVb.phys_addr;
		pRetAddr = (void *)((char *)tmpVb.virt_addr + uOffset);
	}
