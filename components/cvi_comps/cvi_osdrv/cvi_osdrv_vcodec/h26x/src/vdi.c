...
#define VPU_CODA9_REG_BASE				(0x0B010000)
#define VPU_WAVE4_REG_BASE				(0x0B020000)

#define SYS_CLK_EN_2 (*(volatile unsigned int *)(0x03002008))
#define SYS_CLK_EN_3 (*(volatile unsigned int *)(0x0300200C))
#define SYS_CLK_EN_4 (*(volatile unsigned int *)(0x03002010))

#define SYS_CLK_SEL_0 (*(volatile unsigned int *)(0x03002020))

#define SYS_DIV_CLK_AXI_VIDEO_CODEC_0 (*(volatile unsigned int *)(0x030020E4))
#define SYS_DIV_CLK_AXI_VIDEO_CODEC_1 (*(volatile unsigned int *)(0x030020E8))

#define SYS_DIV_CLK_VC_SRC0_0 (*(volatile unsigned int *)(0x030020EC))
#define SYS_DIV_CLK_VC_SRC0_1 (*(volatile unsigned int *)(0x030020F0))
#define SYS_DIV_CLK_VC_SRC1_0 (*(volatile unsigned int *)(0x03002108))
#define SYS_DIV_CLK_VC_SRC1_1 (*(volatile unsigned int *)(0x0300210C))
#define SYS_DIV_CLK_VC_SRC2 (*(volatile unsigned int *)(0x0300211C))
#define VPU_SRAM_CONFIG (*(volatile unsigned int *)(0x0B030024))
...
#define VDI_SRAM_SIZE_CODA9_1835 0x1D000 // FHD MAX size, 0x17D00  4K MAX size 0x34600
...
typedef struct vpudrv_buffer_t {
    Uint32 size;
    Uint64 phys_addr;
    Uint64 base;      /* kernel logical address in use kernel */
    Uint8 *virt_addr; /* virtual user space address */
#ifdef __arm__
    Uint32 padding; /* padding for keeping same size of this structure */
#endif
} vpudrv_buffer_t;
...
#ifdef USE_POSIX
    pthread_t mutex_owner_tid;
#endif
} vdi_info_t;

#ifdef USE_POSIX
pthread_mutex_t vcodecLock[MAX_NUM_VPU_CORE] = {PTHREAD_MUTEX_INITIALIZER, PTHREAD_MUTEX_INITIALIZER};
#else
SemaphoreHandle_t vcodecLock[MAX_NUM_VPU_CORE] = {NULL, NULL};
#endif
...
static BOOL vdi_is_share_single_es_buf(unsigned long core_idx,
                                       unsigned int single_es_buf_size, vdi_info_t **ppVdiSwitch);
#endif
...
static int vdi_get_product_version(unsigned long core_idx)
{
    ...
    vdi->chip_version = CHIP_ID_1835;
#elif defined(__CV181X__) || defined(__CV180X__)
    vdi->chip_version = CHIP_ID_1821A;
#else
    vdi->chip_version = CHIP_ID_1822;
#endif

    CVI_VC_INFO("vdi->chip_version 0x%x\n", vdi->chip_version);
...
int vdi_init(unsigned long core_idx, BOOL bCountTaskNum)
{
    ...
    if (vdi_get_product_version(core_idx)) {
        CVI_VC_INFO("[VDI] fail to vdi_get_product_version \n");
        goto ERR_VDI_INIT;
    }

	if (!vdi_get_instance_pool(core_idx)) {
		CVI_VC_INFO("[VDI] fail to create shared info for saving context \n");
		goto ERR_VDI_INIT;
	}
    if (vdi->pvip->instance_pool_inited == FALSE) {
        ...
        pthread_mutex_init((MUTEX_HANDLE *)vdi->core_mutex, &mutexattr);
        pthread_mutex_init((MUTEX_HANDLE *)vdi->vpu_disp_mutex, &mutexattr);
#else
        vdi->vpu_mutex = xSemaphoreCreateMutex();
        vdi->core_mutex = xSemaphoreCreateMutex();
        vdi->vpu_disp_mutex = xSemaphoreCreateMutex();
#endif
...
static int cviVdiCfgReg(vdi_info_t *vdi, int core_idx)
{
    ...
    if (core_idx == CORE_H264) {
        vdi->vdb_register.phys_addr = VPU_CODA9_REG_BASE;
        vdi->vdb_register.virt_addr = (Uint8 *)VPU_CODA9_REG_BASE;
    } else if (core_idx == CORE_H265) {
        vdi->vdb_register.phys_addr = VPU_WAVE4_REG_BASE;
        vdi->vdb_register.virt_addr = (Uint8 *)VPU_WAVE4_REG_BASE;
    }
    vdi->vdb_register.size = VPU_REG_SIZE;

    CVI_VC_INFO("mmap vdi->vdb_register.virt_addr %p, vdi->vdb_register.size %d, vdi->vdb_register.phys_addr 0x" VCODEC_UINT64_FORMAT_HEX "\n",
                vdi->vdb_register.virt_addr, vdi->vdb_register.size, vdi->vdb_register.phys_addr);
...
int vdi_release(unsigned long core_idx)
{
    ...
    memset(&vdi->vdb_register, 0x00, sizeof(vpudrv_buffer_t));
    vdb.size = 0;
    ...
    CVI_VC_INFO("physaddr=0x" VCODEC_UINT64_FORMAT_HEX ", virtaddr=%p, size=0x%x, index=%d\n",
    ...
    if (vdi->SingleCore == 1) {
        if (free_common_memory(core_idx) < 0) {
            CVI_VC_ERR("[VDI] fail to free common buffer from driver\n");
        }
    }
    ...
    ProductVpuSetId(core_idx, PRODUCT_ID_NONE);

    MUTEX_DESTROY(*(MUTEX_T *)vdi->vpu_mutex);
    MUTEX_DESTROY(*(MUTEX_T *)vdi->core_mutex);
    MUTEX_DESTROY(*(MUTEX_T *)vdi->vpu_disp_mutex);
...
int allocate_common_memory(unsigned long core_idx)
{
    ...
    if (vdi->vpu_common_memory.base == 0) {
        vdb.base = (Uint64)malloc(vdb.size + 16 * 1024);
        vdb.phys_addr = ALIGN(vdb.base, 16 * 1024);
        vdb.virt_addr = (Uint8 *)vdb.phys_addr;
    } else {
    ...
    if (vdi->SingleCore == 1) {
        vdi->pvip->vpu_common_buffer.size = vdi_get_size_common(core_idx, vdi->SingleCore);
        ...
        vdi->pvip->vpu_common_buffer.phys_addr = (vdb.phys_addr + (core_idx * SIZE_COMMON));
        vdi->pvip->vpu_common_buffer.base = (vdb.base + (core_idx * SIZE_COMMON));
        vdi->pvip->vpu_common_buffer.virt_addr = (vdb.virt_addr + (core_idx * SIZE_COMMON));
    }

    memcpy(&vdi->vpu_common_memory, &vdi->pvip->vpu_common_buffer, sizeof(vpudrv_buffer_t));
    ...
    CVI_VC_TRACE("physaddr=0x" VCODEC_UINT64_FORMAT_HEX ", size=0x%x, virtaddr=%p\n",
...
int free_common_memory(unsigned long core_idx)
{
    ...
    memcpy(&vdb, &vdi->pvip->vpu_common_buffer, sizeof(vpudrv_buffer_t));
    ...
    CVI_VC_MEM("release common mem physaddr=0x" VCODEC_UINT64_FORMAT_HEX " size=0x%x\n", vdb.phys_addr, vdb.size);
    free((void *)vdb.base);
    ...
    memset(p_vdb, 0x0, sizeof(vpudrv_buffer_t));
    memset(&vdi->pvip->vpu_common_buffer, 0x0, sizeof(vpu_buffer_t));
    memset(&vdi->vpu_common_memory, 0x0, sizeof(vpu_buffer_t));
...
#endif
        vdb.phys_addr = (Uint64)calloc(1, vdb.size);
        vdb.virt_addr = (Uint8 *)vdb.phys_addr;

        CVI_VC_MEM("sizeof(vpu_instance_pool_t) = %zd\n", sizeof(vpu_instance_pool_t));
        // CVI_VC_MEM("vmalloc, sizeof(MUTEX_HANDLE) * VDI_NUM_LOCK_HANDLES = %zd\n", sizeof(MUTEX_HANDLE) * VDI_NUM_LOCK_HANDLES);
        CVI_VC_MEM("mmap vdb.virt_addr %p, vdb.size %d, vdb.phys_addr 0x" VCODEC_UINT64_FORMAT_HEX ", lr %p\n",
                   vdb.virt_addr, vdb.size, vdb.phys_addr, __builtin_return_address(0));
...
#ifdef USE_POSIX
        vdi->vpu_mutex =
            (void *)((unsigned long)vdi->pvip +
                     sizeof(vpu_instance_pool_t));
        // change the
        // pointer of vpu_mutex to at end pointer of vpu_instance_pool_t
        // to assign at allocated position.
        vdi->vpu_disp_mutex = (void *)((unsigned long)vdi->pvip +
                                       sizeof(vpu_instance_pool_t) +
                                       sizeof(MUTEX_HANDLE));
        vdi->core_mutex = (void *)((unsigned long)vdi->pvip +
                                   sizeof(vpu_instance_pool_t) +
                                   sizeof(MUTEX_HANDLE) * 2);
#endif
        CVI_VC_INFO("[VDI] instance pool physaddr=0x" VCODEC_UINT64_FORMAT_HEX ", virtaddr=%p, base=0x" VCODEC_UINT64_FORMAT_HEX ", size=%d\n",
...
int vdi_close_instance(unsigned long core_idx, unsigned long inst_idx)
{
    ...
    CVI_VC_INFO("inst_idx %ld, after close vdi->pvip->vpu_instance_num %d\n", inst_idx, vdi->pvip->vpu_instance_num);
...
static void restore_mutex_in_dead(MUTEX_HANDLE *mutex)
{
    ...
    if (mutex_value == (int)0xdead10cc) {
        // destroy by device driver
        pthread_mutexattr_t mutexattr;
        pthread_mutexattr_init(&mutexattr);
...
int vdi_check_lock_by_me(unsigned long core_idx)
{
	...
	if (vdi->mutex_owner_tid) {
		if (pthread_equal(vdi->mutex_owner_tid, pidt)) {
			return 0;
		} else {
			CVI_VC_LOCK("mtx_oid 0x%lx, my_oid 0x%lx\n",
				vdi->mutex_owner_tid, pidt);
		}
	}
#else
...
int vdi_lock(unsigned long core_idx)
{
    ...
    if (!vdi->vpu_mutex)
        CVI_VC_ERR("vpu_mutex %p\n", vdi->vpu_mutex);
    int ret = pthread_mutex_lock((MUTEX_HANDLE *)vdi->vpu_mutex);
...
#if 0
	vdi->mutex_owner_tid = pthread_self();
#endif
#endif
...
#ifdef USE_POSIX
    vdi->mutex_owner_tid = 0;

    int ret = pthread_mutex_unlock((MUTEX_HANDLE *)vdi->vpu_mutex);
...
void vdi_vcodec_initlock(unsigned long core_idx)
{
#ifdef ARCH_CV183X
#else
    core_idx = 0;
...
#ifndef USE_POSIX
    vcodecLock[core_idx] = xSemaphoreCreateMutex();
#endif
}

void vdi_vcodec_deinitlock(unsigned long core_idx)
{
#ifdef ARCH_CV183X
#else
    core_idx = 0;
...
#ifndef USE_POSIX
    vSemaphoreDelete(vcodecLock[core_idx]);
#endif
}
...
#ifdef USE_POSIX
	return pthread_mutex_trylock(&vcodecLock[core_idx]);
#endif
}
...
int vdi_vcodec_timelock(unsigned long core_idx, int timeout_ms)
{
    ...
    time_interval = (unsigned long)timeout_ms;
	...
	ret = pthread_mutex_timedlock(&vcodecLock[core_idx], &tout);
	if (ret != 0) {
		CVI_VC_ERR("lock error %d, timeout:%d\n", ret, timeout_ms);
		return ret;
	}
#endif
...
#ifdef USE_POSIX
	pthread_mutex_lock(&vcodecLock[core_idx]);
#endif
}
...
void vdi_vcodec_unlock(unsigned long core_idx)
{
	...
	pthread_mutex_unlock(&vcodecLock[core_idx]);
...
static vdi_info_t *vdi_get_vdi_info(unsigned long core_idx)
{
    ...
    if (!vdi || vdi->vpu_fd == -1 || vdi->vpu_fd == 0x00) {
        CVI_VC_WARN("lock failed, vdi %p, ret %p\n", vdi, __builtin_return_address(0));
        if (vdi)
            CVI_VC_WARN("vpu_fd %d\n", vdi->vpu_fd);
        return NULL;
    }
...
unsigned int vdi_read_register(unsigned long core_idx, unsigned long addr)
{
    ...
    reg_addr = (unsigned int *)(addr + (unsigned long)vdi->vdb_register.virt_addr);
    value = *(volatile unsigned int *)reg_addr;
...
int vdi_clear_memory(unsigned long core_idx, PhysicalAddress addr, int len)
{
    ...
    memset((void *)((unsigned long)vdb.virt_addr + offset), 0x00, len);
                     ...
                     unsigned char *data, int len, int endian)
{
    ...
    memcpy((void *)((unsigned long)vdb.virt_addr + offset), data, len);

    vdi_flush_ion_cache(addr, vdb.virt_addr + offset, len);
    return len;
}
                    ...
                    unsigned char *data, int len, int endian)
{
	...
	CVI_VC_TRACE(
			"i = %d, addr = 0x"VCODEC_UINT64_FORMAT_HEX", phys_addr = 0x"VCODEC_UINT64_FORMAT_HEX", virt_addr = %p, data = %p\n",
			i, addr, vdb.phys_addr, (void *)vdb.virt_addr, data);

    offset = addr - (unsigned long)vdb.phys_addr;

    memcpy(data, (const void *)((unsigned long)vdb.virt_addr + offset), len);
    swap_endian(core_idx, data, len, endian);
...
void *vdi_get_vir_addr(unsigned long core_idx, PhysicalAddress addr)
{
    ...
    CVI_VC_TRACE("i = %d, addr = 0x" VCODEC_UINT64_FORMAT_HEX ", phys_addr = 0x" VCODEC_UINT64_FORMAT_HEX ", virt_addr = %p\n",
                 i, addr, vdb.phys_addr, (void *)vdb.virt_addr);

    offset = addr - (unsigned long)vdb.phys_addr;

    srcAddr = ((void *)vdb.virt_addr + offset);
...
static void vdi_share_single_es_buf(unsigned long core_idx, vdi_info_t *vdi, BOOL *pbReUsedEsBuf)
{
    vdi_info_t *vdiSwitch = NULL;

    if (vdi_is_share_single_es_buf(core_idx, vdi->single_es_buf_size, &vdiSwitch)) {
        int i;

        memcpy(&vdi->vdb_bitstream, &vdiSwitch->vdb_bitstream, sizeof(vpudrv_buffer_t));

        for (i = 0; i < MAX_VPU_BUFFER_POOL; i++) {
            if (vdi->vpu_buffer_pool[i].inuse == 0) {
                vdi->vpu_buffer_pool_count++;
                vdi->vpu_buffer_pool[i].inuse = 1;
                memcpy(&vdi->vpu_buffer_pool[i].vdb, &vdi->vdb_bitstream, sizeof(vpudrv_buffer_t));
                break;
            }
        }
        if (i >= MAX_VPU_BUFFER_POOL)
...
int vdi_allocate_single_es_buf_memory(unsigned long core_idx, vpu_buffer_t *vb) // saw not lock
{
    vdi_info_t *vdi;
    ...
    if (vdi->task_num > 1 || bReUsedEsBuf) {
        ...
        if (VDI_ALLOCATE_MEMORY(core_idx, vb, BS_STREAM_CACHED, NULL) < 0) {
            CVI_VC_ERR("[VDI] fail to vdi_allocate_single_es_buf_memory size=%d\n",
                       vb->size);
            return -1;
        }
    ...
    CVI_VC_MEM("physaddr=" VCODEC_UINT64_FORMAT_HEX ", virtaddr=%p~%p, size=0x%x\n",
...
void vdi_free_single_es_buf_memory(unsigned long core_idx, vpu_buffer_t *vb)
{
    ...
    if (vdi->task_num == 1) {
        ...
        if (vdi_is_share_single_es_buf(core_idx, vdi->single_es_buf_size, &vdiSwitch) == FALSE)
#endif
        {
            CVI_VC_MEM("relese physaddr=" VCODEC_UINT64_FORMAT_HEX ", virtaddr=%p~%p, size=0x%x\n",
                       vb->phys_addr, (void *)vb->virt_addr,
                       (void *)(vb->virt_addr + vb->size), vb->size);
            VDI_FREE_MEMORY(core_idx, vb);
        }
        memset(&vdi->vdb_bitstream, 0x00, sizeof(vpudrv_buffer_t));
    }
}
...
int vdi_attach_dma_memory(unsigned long core_idx, vpu_buffer_t *vb)
{
    ...
    CVI_VC_MEM("physaddr=0x" VCODEC_UINT64_FORMAT_HEX ", virtaddr=%p, size=0x%x, index=%d\n",
...
int vdi_invalidate_ion_cache(uint64_t u64PhyAddr, void *pVirAddr, uint32_t u32Len)
{
...
int vdi_allocate_dma_memory(unsigned long core_idx, vpu_buffer_t *vb) // saw not lock
{
    vdi_info_t *vdi;
    int i;
    vpudrv_buffer_t vdb;
    ...
    vdb.size = vb->size;
    vdb.base = (unsigned long)malloc(vdb.size + 64);
    if (vdb.base == -1) {
    ...
    vdb.phys_addr = ALIGN(vdb.base, 64);
    vdb.virt_addr = (Uint8 *)vdb.phys_addr;

    vb->phys_addr = vdb.phys_addr;
    vb->base = vdb.base;
    vb->virt_addr = (Uint8 *)vb->phys_addr;

    for (i = 0; i < MAX_VPU_BUFFER_POOL; i++) {
        if (vdi->vpu_buffer_pool[i].inuse == 0) {
            memcpy(&vdi->vpu_buffer_pool[i].vdb, &vdb, sizeof(vpudrv_buffer_t));
            vdi->vpu_buffer_pool[i].inuse = 1;
            vdi->vpu_buffer_pool_count++;
            break;
        }
    }
    memset(vdb.virt_addr, 0x0, vdb.size);
    vdi_invalidate_ion_cache(vdb.phys_addr, vdb.virt_addr, vdb.size);
    if (i >= MAX_VPU_BUFFER_POOL)
        CVI_VC_ERR("i >= MAX_VPU_BUFFER_POOL\n");
...
void vdi_free_dma_memory(unsigned long core_idx, vpu_buffer_t *vb)
{
    ...
    if (!p_vdb->size) {
        CVI_VC_ERR("[VDI] invalid buffer to free address = 0x%p\n",
                   p_vdb->virt_addr);
        goto FREE_DMA_ERROR;
    }

    free((void *)p_vdb->base);
    if (p_vdb) {
        memset(p_vdb, 0x0, sizeof(vpudrv_buffer_t));
    }

    memset(vb, 0x0, sizeof(vpu_buffer_t));
...
static void vdi_clk_enable(unsigned long core_idx)
{
    SYS_CLK_EN_2 |= (1 << 9); // clk_prepare_enable(vdev->clk_vc_src0);

    SYS_CLK_EN_4 |= 1 << 0;   // clk_prepare_enable(vdev->clk_cfg_reg_vc);
    SYS_CLK_EN_2 |= (1 << 8); // clk_prepare_enable(vdev->clk_axi_video_codec);

    // sram share config
    if (core_idx == CORE_H264) {
        // devmem 0xb030024 32 1
        VPU_SRAM_CONFIG = 0x1; // writel(0x1, pReg->virt_addr + 0x24);
    } else if (core_idx == CORE_H265) {
        // devmem 0xb030024 32 2
        VPU_SRAM_CONFIG = 0x2; // writel(0x2, pReg->virt_addr + 0x24);
    }

    // Enable both H264/H265 clk first
    // then to disable unnecessary one
    SYS_CLK_EN_2 |= 1 << 10; // clk_prepare_enable(vdev->clk_h264c);
    SYS_CLK_EN_2 |= 1 << 14; // clk_prepare_enable(vdev->clk_apb_h264c);
    SYS_CLK_EN_2 |= 1 << 11; // clk_prepare_enable(vdev->clk_h265c);
    SYS_CLK_EN_2 |= 1 << 15; // clk_prepare_enable(vdev->clk_apb_h265c);

    if (core_idx == CORE_H264) {
        SYS_CLK_EN_2 &= ~(1 << 11); // clk_disable_unprepare(vdev->clk_h265c);
        SYS_CLK_EN_2 &= ~(1 << 15); // clk_disable_unprepare(vdev->clk_apb_h265c);
    } else if (core_idx == CORE_H265) {
        SYS_CLK_EN_2 &= ~(1 << 10); // clk_disable_unprepare(vdev->clk_h264c);
        SYS_CLK_EN_2 &= ~(1 << 14); // clk_disable_unprepare(vdev->clk_apb_h264c);
    }
}

static void vdi_clk_disable(unsigned long core_idx)
{
    if (core_idx == CORE_H264) {
        //		SYS_CLK_EN_2 &= ~(1<<10);//clk_disable_unprepare(vdev->clk_h264c);
        //		SYS_CLK_EN_2 &= ~(1<<14);//clk_disable_unprepare(vdev->clk_apb_h264c);
    ...
    if (core_idx == CORE_H265) {
        //		SYS_CLK_EN_2 &= ~(1<<11);//clk_disable_unprepare(vdev->clk_h265c);
        //		SYS_CLK_EN_2 &= ~(1<<15);//clk_disable_unprepare(vdev->clk_apb_h265c);
    }

    //	SYS_CLK_EN_2 &= ~(1<<9);//clk_disable_unprepare(vdev->clk_vc_src0);
    //	SYS_CLK_EN_4 &= ~(1<<0);//clk_disable_unprepare(vdev->clk_cfg_reg_vc);
    //	SYS_CLK_EN_2 &= ~(1<<8);//clk_disable_unprepare(vdev->clk_axi_video_codec);
}
...
int vdi_set_clock_gate(unsigned long core_idx, int enable)
{
    ...
    if (enable)
        vdi_clk_enable(core_idx);
    else
        vdi_clk_disable(core_idx);
                      ...
                      unsigned int addr_bit_busy_flag)
{
	...
	code = vdi_read_register(core_idx, VPU_PRODUCT_CODE_REGISTER); /* read
                                      product
                                      code
                                    */
...
int vdi_wait_interrupt(unsigned long coreIdx, int timeout, uint64_t *pu64TimeStamp)
{
    int intr_reason = 0;
    vdi_info_t *vdi;
    ...
    Uint32 intrReasonReg;
    Int32 startTime, endTime;

    if (PRODUCT_CODE_W_SERIES(vdi->product_code)) {
        intrStatusReg = W4_VPU_VPU_INT_STS;
        intrReasonReg = W4_VPU_VINT_REASON;
    } else if (PRODUCT_CODE_NOT_W_SERIES(vdi->product_code)) {
        intrStatusReg = BIT_INT_STS;
        intrReasonReg = BIT_INT_REASON;
    } else {
        CVI_VC_ERR("Unknown product id : %08x\n", vdi->product_code);
        return -1;
    }
    ...
    while (TRUE) {
        if (vdi_read_register(coreIdx, intrStatusReg)) {
            intr_reason =
                vdi_read_register(coreIdx, intrReasonReg);
            if (intr_reason)
                break;
        }
        clock_gettime(CLOCK_MONOTONIC, &ts);
	...
	// alios not use hw irq, so get current time to TimeStamp
	if (pu64TimeStamp) {
		// clock_gettime(CLOCK_MONOTONIC, &ts);
		// *pu64TimeStamp =  ts.tv_sec * 1000000 + ts.tv_nsec/1000;   // in us
		*pu64TimeStamp = rt_tick_get();
	}
...
void vdi_print_vpu_status(unsigned long coreIdx)
{
    ...
    if (PRODUCT_CODE_W_SERIES(product_code)) {
        ...
        printf("------                            VCPU STATUS                             -----\n");
        printf("-------------------------------------------------------------------------------\n");
        rd = vdi_remap_memory_address(coreIdx, VpuReadReg(coreIdx, W4_BS_RD_PTR));
        wr = vdi_remap_memory_address(coreIdx, VpuReadReg(coreIdx, W4_BS_WR_PTR));
        printf("RD_PTR: 0x%lX WR_PTR: 0x%lX BS_OPT: 0x%08x BS_PARAM: 0x%08x\n",
        ...
        printf("[DEBUG-BPUHEVC] CABAC_FSM> SAO: 0x%02x, CU: 0x%02x, PU: 0x%02x, TU: 0x%02x, EOS: "
               "0x%02x\n",
               ((cabac_fsm >> 25) & 0x3f), ((cabac_fsm >> 19) & 0x3f),
               ((cabac_fsm >> 13) & 0x3f), ((cabac_fsm >> 6) & 0x7f),
               (cabac_fsm & 0x3f));
        printf("[DEBUG-BPUHEVC] CU_INFO value = 0x%04x \n\t\t(l2cb: 0x%1x, cux: %1d, cuy; %1d, "
               "pred: %1d, pcm: "
               "%1d, wr_done: %1d, par_done: %1d, nbw_done: %1d, dec_run: %1d)\n",
        ...
        printf("       TQ            IP              MC             LF      GDI_EMPTY          "
               "ROOM \n");
        printf("-----------------------------------------------------------------------------------"
               "------------"
               "-------------\n");
        printf("| %d %04d %04d | %d %04d %04d |  %d %04d %04d | %d %04d %04d | 0x%08x | CU(%d) "
               "TU(%d) TC(%d) "
               "LF(%d) IP(%d)\n",
        ...
        printf("[DCIDEBUG] QUEUE COUNT: CU(%5d) TU(%5d) ",
               (reg_val >> 16) & 0xffff, reg_val & 0xffff);
        reg_val = read_vce_register(0, 0, 0x1A0);
        printf("TC(%5d) IP(%5d) ", (reg_val >> 16) & 0xffff,
               reg_val & 0xffff);
        ...
        printf("cur_main_state %2d, r_lf_pic_deblock_disable %1d, r_lf_pic_sao_disable %1d\n",
               ...
               (reg_val >> 14) & 0x1);
        printf("para_load_done %1d, i_rdma_ack_wait %1d, i_sao_intl_col_done %1d, "
               "i_sao_outbuf_full %1d\n",
               (reg_val >> 13) & 0x1, (reg_val >> 12) & 0x1,
               (reg_val >> 11) & 0x1, (reg_val >> 10) & 0x1);
        printf("lf_sub_done %1d, i_wdma_ack_wait %1d, lf_all_sub_done %1d, cur_ycbcr %1d, "
               "sub8x8_done %2d\n",
...
#define VDI_LOG_ROT_DST_IDX (0x400 + 0x110)
            ...
            CVI_VC_INFO("MC_MB_XY_DONE=(y:%d, x:%d)\n",
...
PhysicalAddress vdi_remap_memory_address(unsigned long coreIdx, PhysicalAddress address)
{
    vdi_info_t *vdi;
...
int vdi_set_single_es_buf(unsigned long core_idx, BOOL use_single_es_buf, unsigned int *single_es_buf_size)
{
    vdi_info_t *vdi;
    ...
    if (vdi->task_num <= 0) {
        ...
        if (vdi->singleEsBuf != use_single_es_buf) {
            CVI_VC_ERR("use_single_es_buf flag must be the same for all tasks\n");
            return -1;
        }
    }
...
int vdi_get_chip_version(void)
{
    ...
    for (int i = 0; i < MAX_NUM_VPU_CORE; i++) {
...
int vdi_show_vdi_info(unsigned long core_idx)
{
    ...
    vdi = vdi_get_vdi_info(core_idx);
    if (!vdi) {
        printf("vdi_get_vdi_info\n");
        return -1;
    }

    printf("chip_version:0x%x\n", vdi->chip_version);
    printf("core_idx:%ld\n", vdi->core_idx);
    printf("product_code:0x%x\n", vdi->product_code);
    printf("vpu_fd:%d\n", vdi->vpu_fd);
    printf("task_num:%d\n", vdi->task_num);
    printf("clock_state:%d\n", vdi->clock_state);
    printf("chip_version:0x%x\n", vdi->chip_version);
    printf("vdb_register_size:%d\n", vdi->vdb_register.size);
    printf("vpu_common_memory_size:%d\n", vdi->vpu_common_memory.size);
    printf("vpu_buffer_pool_count:%d\n", vdi->vpu_buffer_pool_count);
    printf("vdb_bitstream_size:%d\n", vdi->vdb_bitstream.size);
    printf("SingleCore:%d\n", vdi->SingleCore);
    printf("singleEsBuf:%d\n", vdi->singleEsBuf);
    printf("single_es_buf_size:%d\n", vdi->single_es_buf_size);
