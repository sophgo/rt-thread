...
#endif
extern void rcLibUpdateRc(CodecInst *pCodecInst, EncOutputInfo *info,
                          int avg_qp, int real_pic_bit);
...
static RetCode PrepareDecodingPicture(CodecInst *instance, Uint32 cmd)
{
    ...
    if (pDecInfo->streamEndflag == 1) bsOption = 3;
    ...
    regVal = (pDecInfo->secAxiInfo.u.wave4.useBitEnable << 0) | (pDecInfo->secAxiInfo.u.wave4.useIpEnable << 9) | (pDecInfo->secAxiInfo.u.wave4.useLfRowEnable << 15);
    ...
    if (cmd == DEC_PIC)
    {
        ...
        VpuWriteReg(coreIdx, W4_CMD_DEC_REPORT_PARAM,
                    (pDecInfo->cuDataEnable << 31 | (VDI_128BIT_LE_WORD_BYTE_SWAP & VDI_128BIT_ENDIAN_MASK)));
    }
...
static void GetSequenceInformation(CodecInst *instance, DecInitialInfo *info)
{
    ...
    pDecInfo->frameDisplayFlag =
        VpuReadReg(instance->coreIdx, pDecInfo->frameDisplayFlagRegAddr);
    ...
    {
        info->aspectRateInfo =
            VpuReadReg(instance->coreIdx, W4_RET_DEC_ASPECT_RATIO);
...
RetCode Wave4VpuDecInitSeq(CodecInst *instance)
{
    ...
    if (instance == NULL) return RETCODE_INVALID_PARAM;
    ...
    if (pDecInfo->thumbnailMode) cmdOption = INIT_SEQ_W_THUMBNAIL;
...
RetCode Wave4VpuDecGetResult(CodecInst *instance, DecOutputInfo *result)
{
    ...
    if (instance->loggingEnable) vdi_log(instance->coreIdx, DEC_PIC, 0);
#endif
    ...
    if (result->decodingSuccess == FALSE)
    {
        ...
        {
            result->errorReasonExt =
                VpuReadReg(instance->coreIdx, W4_RET_DEC_ERR_INFO);
        }
    ...
    if (result->sequenceChanged == 0)
    {
        ...
        if (instance->codecMode == C7_VP9_DEC)
        {
            result->indexInterFrameDecoded = VpuReadReg(
                instance->coreIdx, W4_RET_DEC_INTER_RES_CHANGE_REALLOCATED_DPB_IDX);
        }
    }
    else
    {
        ...
        memcpy((void *)&pDecInfo->newSeqInfo, (void *)&pDecInfo->initialInfo,
...
RetCode Wave4VpuDecGetSeqInfo(CodecInst *pCodecInst, DecInitialInfo *info)
{
...
#ifdef ENABLE_CNM_DEBUG_MSG
    if (pCodecInst->loggingEnable) vdi_log(pCodecInst->coreIdx, DEC_PIC_HDR, 0);
#endif
    ...
    if (regVal == FALSE)
    {
        ...
        {
            info->seqInitErrReason =
                VpuReadReg(pCodecInst->coreIdx, W4_RET_DEC_ERR_INFO);
        }
                                       ...
                                       TiledMapType mapType, Uint32 count)
{
    ...
    Int32    q = 0, j = 0, i = 0, remain = 0, idx = 0, bufHeight = 0, bufWidth = 0;
    ...
    if (mapType == COMPRESSED_FRAME_MAP)
    {
        ...
        vbBuffer.size =
            ((mvColSize * count + 4095) & ~4095) + 4096; /* 4096 is a margin */
        vbBuffer.phys_addr = 0;

        CVI_VC_MEM("mvColSize = 0x%X\n", vbBuffer.size);
        if (VDI_ALLOCATE_MEMORY(coreIdx, &vbBuffer, 0, NULL) < 0)
    ...
    VpuWriteReg(coreIdx, W4_ADDR_SUB_SAMPLED_FB_BASE,
                pEncInfo->vbSubSamBuf.phys_addr);
    // set sub-sampled buffer base addr
    VpuWriteReg(coreIdx, W4_SUB_SAMPLED_ONE_FB_SIZE, subSampledSize);
    // set sub-sampled buffer size for one frame

    endian =
        vdi_convert_endian(coreIdx, fbArr[0].endian) & VDI_128BIT_ENDIAN_MASK;
    ...
    {
        lumaStride =
            VPU_ALIGN32(bufWidth) * (pOpenParam->EncStdParam.hevcParam.internalBitDepth > 8 ? 5 : 4);
        lumaStride = VPU_ALIGN32(lumaStride);
        chromaStride =
            VPU_ALIGN16((bufWidth >> 1)) * (pOpenParam->EncStdParam.hevcParam.internalBitDepth > 8 ? 5 : 4);
        chromaStride = VPU_ALIGN32(chromaStride);
    }
    else
    {
        lumaStride =
            VPU_ALIGN16(pOpenParam->picWidth) * (pOpenParam->EncStdParam.hevcParam.internalBitDepth > 8 ? 5 : 4);
        lumaStride = VPU_ALIGN32(lumaStride);

        chromaStride =
            VPU_ALIGN16((pOpenParam->picWidth >> 1)) * (pOpenParam->EncStdParam.hevcParam.internalBitDepth > 8 ? 5 : 4);
        chromaStride = VPU_ALIGN32(chromaStride);
    }

    VpuWriteReg(coreIdx, W4_FBC_STRIDE, lumaStride << 16 | chromaStride);

    regVal = (nv21 << 29) | ((mapType == LINEAR_FRAME_MAP) << 28) | (axiID << 24) | (yuvFormat << 20) | (cbcrInterleave << 16) | (stride * bpp);

    VpuWriteReg(coreIdx, W4_COMMON_PIC_INFO, regVal);

    if (pEncInfo->addrRemapEn) cviInitAddrRemapFb(instance);
    ...
    for (j = 0; j < q; j++)
    {
        ...
        for (i = 0; i < 8 && i < remain; i++)
        {
            ...
            {
                VpuWriteReg(
                    coreIdx, W4_ADDR_FBC_Y_OFFSET0 + (i << 4),
                    pEncInfo->vbFbcYTbl.phys_addr + idx * fbcYTblSize); /* Luma
                                                                   FBC
                                                                   offset
                                                                   table
                                                                 */
                VpuWriteReg(
                    coreIdx, W4_ADDR_FBC_C_OFFSET0 + (i << 4),
                    pEncInfo->vbFbcCTbl.phys_addr + idx * fbcCTblSize); /* Chroma
                                                                       FBC
                                                                       offset
                                                                       table
                                                                     */
    ...
    if (ConfigSecAXIWave(coreIdx, instance->codecMode, &pEncInfo->secAxiInfo,
                         pOpenParam->picWidth, pOpenParam->picHeight,
                         pOpenParam->EncStdParam.hevcParam.profile,
                         pOpenParam->EncStdParam.hevcParam.level)
        == 0)
    {
        return RETCODE_INSUFFICIENT_RESOURCE;
    }
...
RetCode Wave4VpuEncSetup(CodecInst *instance)
{
   ...
   */
    ...
    if (((rotMirMode != 0) && !((pEncInfo->rotationAngle == 180) && (pEncInfo->mirrorDirection == MIRDIR_HOR_VER))) && ((pOpenParam->picWidth != width32) || (pOpenParam->picHeight != height32))) // if rot/mir enable && pic
    ...
    VpuWriteReg(coreIdx, W4_BS_PARAM,
                (pEncInfo->lineBufIntEn << 6) | (pEncInfo->sliceIntEnable << 5) | (pEncInfo->ringBufferEnable << 4) | bsEndian);

    /* Secondary AXI */
    vdi_get_sram_memory(coreIdx, &vb);
    VpuWriteReg(coreIdx, W4_ADDR_SEC_AXI, vb.phys_addr);
    VpuWriteReg(coreIdx, W4_SEC_AXI_SIZE, pEncInfo->secAxiInfo.bufSize);
    VpuWriteReg(coreIdx, W4_USE_SEC_AXI,
                (pEncInfo->secAxiInfo.u.wave4.useEncImdEnable << 9) | (pEncInfo->secAxiInfo.u.wave4.useEncRdoEnable << 11) | (pEncInfo->secAxiInfo.u.wave4.useEncLfEnable << 15));
    ...
    VpuWriteReg(coreIdx, W4_CMD_ENC_SEQ_SRC_SIZE, srcHeight << 16 | srcWidth);
    { // WAVE420L
        VpuWriteReg(
            coreIdx, W4_CMD_ENC_SEQ_PIC_PARAM,
            (pHevc->profile << 0) | (pHevc->level << 3) | (pHevc->tier << 12) | (pHevc->internalBitDepth << 14) | (pHevc->chromaFormatIdc << 18) | (pHevc->losslessEnable << 20) | (pHevc->constIntraPredFlag << 21) | ((pHevc->chromaCbQpOffset & 0x1f) << 22) | ((pHevc->chromaCrQpOffset & 0x1f) << 27));
        ...
        VpuWriteReg(coreIdx, W4_CMD_ENC_RC_PARAM,
                    (pOpenParam->rcEnable << 0) | (pHevc->cuLevelRCEnable << 1) | (pHevc->hvsQPEnable << 2) | (pHevc->hvsQpScaleEnable << 3) | (pHevc->hvsQpScale << 4) | (pHevc->bitAllocMode << 7) | (pHevc->initBufLevelx8 << 9) | (pHevc->ctuOptParam.roiEnable << 13) | (pHevc->initialRcQp << 14) | (pOpenParam->initialDelay << 20));
    }

    VpuWriteReg(coreIdx, W4_CMD_ENC_SEQ_INTRA_PARAM,
                (pHevc->decodingRefreshType << 0) | (pHevc->intraQP << 3) | (pHevc->forcedIdrHeaderEnable << 9) | (pHevc->intraPeriod << 16));
    ...
    VpuWriteReg(coreIdx, W4_CMD_ENC_SEQ_INDEPENDENT_SLICE,
                pHevc->independSliceModeArg << 16 | pHevc->independSliceMode);
    ...
    regVal  = (pHevc->useRecommendEncParam) | (pHevc->scalingListEnable << 3) | (pHevc->cuSizeMode << 4) | (pHevc->tmvpEnable << 7) | (pHevc->wppEnable << 8) | (pHevc->maxNumMerge << 9) | (pHevc->dynamicMerge8x8Enable << 12) | (pHevc->dynamicMerge16x16Enable << 13) | (pHevc->dynamicMerge32x32Enable << 14) | (pHevc->disableDeblk << 15) | (pHevc->lfCrossSliceBoundaryEnable << 16) | ((pHevc->betaOffsetDiv2 & 0xF) << 17) | ((pHevc->tcOffsetDiv2 & 0xF) << 21) | (pHevc->skipIntraTrans << 25) | (pHevc->saoEnable << 26) | (pHevc->intraInInterSliceEnable << 27) | (pHevc->intraNxNEnable << 28);
    ...
    VpuWriteReg(coreIdx, W4_CMD_ENC_RC_MIN_MAX_QP,
                (pOpenParam->userQpMinP << 0) | (pOpenParam->userQpMaxP << 6) | (pHevc->maxDeltaQp << 12) | ((pHevc->intraQpOffset & 0xFFFF) << 18));

    VpuWriteReg(coreIdx, W4_CMD_ENC_RC_BIT_RATIO_LAYER_0_3,
                (pHevc->fixedBitRatio[0] << 0) | (pHevc->fixedBitRatio[1] << 8) | (pHevc->fixedBitRatio[2] << 16) | (pHevc->fixedBitRatio[3] << 24));

    VpuWriteReg(coreIdx, W4_CMD_ENC_RC_BIT_RATIO_LAYER_4_7,
                (pHevc->fixedBitRatio[4] << 0) | (pHevc->fixedBitRatio[5] << 8) | (pHevc->fixedBitRatio[6] << 16) | (pHevc->fixedBitRatio[7] << 24));

    VpuWriteReg(coreIdx, W4_CMD_ENC_NR_PARAM,
                (pHevc->nrYEnable << 0) | (pHevc->nrCbEnable << 1) | (pHevc->nrCrEnable << 2) | (pHevc->nrNoiseEstEnable << 3) | (pHevc->nrNoiseSigmaY << 4) | (pHevc->nrNoiseSigmaCb << 12) | (pHevc->nrNoiseSigmaCr << 20));

    VpuWriteReg(
        coreIdx, W4_CMD_ENC_NR_WEIGHT,
        (pHevc->nrIntraWeightY << 0) | (pHevc->nrIntraWeightCb << 5) | (pHevc->nrIntraWeightCr << 10) | (pHevc->nrInterWeightY << 15) | (pHevc->nrInterWeightCb << 20) | (pHevc->nrInterWeightCr << 25));

    VpuWriteReg(coreIdx, W4_CMD_ENC_RC_TARGET_RATE, rcInfo->targetBitrate * 1000);
...
#ifdef ENABLE_CNM_DEBUG_MSG
    if (instance->loggingEnable) vdi_log(coreIdx, SET_PARAM, 0);
#endif
   ...
   */
    if (pHevc->gopPresetIdx == PRESET_IDX_CUSTOM_GOP)
    {
        ...
        VpuWriteReg(coreIdx, W4_CMD_ENC_SET_CUSTOM_GOP_ENABLE,
        ...
        VpuWriteReg(coreIdx, W4_CMD_ENC_CUSTOM_GOP_PARAM,
                    pHevc->gopParam.customGopSize << 0 | pHevc->gopParam.useDeriveLambdaWeight << 4);

        for (i = 0; i < pHevc->gopParam.customGopSize; i++)
        {
            VpuWriteReg(coreIdx, W4_CMD_ENC_CUSTOM_GOP_PIC_PARAM_0 + (i * 4),
                        (pHevc->gopParam.picParam[i].picType << 0) | (pHevc->gopParam.picParam[i].pocOffset << 2) | (pHevc->gopParam.picParam[i].picQp << 6) | ((pHevc->gopParam.picParam[i].refPocL0 & 0x1F) << 14) | ((pHevc->gopParam.picParam[i].refPocL1 & 0x1F) << 19) | (pHevc->gopParam.picParam[i].temporalId << 24));
            VpuWriteReg(coreIdx, W4_CMD_ENC_CUSTOM_GOP_PIC_LAMBDA_0 + (i * 4),
    ...
    if (pHevc->vuiParam.vuiParamFlags || pEncInfo->openParam.encodeVuiRbsp || pEncInfo->openParam.encodeHrdRbspInVPS || pEncInfo->openParam.encodeHrdRbspInVUI)
    {
        ...
        if (pHevc->vuiParam.vuiParamFlags)
        {
            ...
            VpuWriteReg(coreIdx, W4_CMD_ENC_VUI_OVERSCAN_APPROPRIATE,
        ...
        if (pEncInfo->openParam.encodeVuiRbsp || pEncInfo->openParam.encodeHrdRbspInVPS || pEncInfo->openParam.encodeHrdRbspInVUI)
        {
            ...
            VpuWriteReg(coreIdx, W4_CMD_ENC_VUI_HRD_RBSP_PARAM_FLAG,
                        (pEncInfo->openParam.encodeHrdRbspInVPS << 2) | (pEncInfo->openParam.encodeHrdRbspInVUI << 1) | (pEncInfo->openParam.encodeVuiRbsp));
                                        ...
                                        EncParam       *option)
{
    ...
    {
        if (pEncInfo->bEsBufQueueEn == 0 || ((pEncInfo->streamBufEndAddr - paBitStreamWrPtr) < H265_MIN_BITSTREAM_SIZE))
        {
...
RetCode Wave4VpuEncode(CodecInst *instance, EncParam *option)
{
    ...
    srcFrameFormat =
        packedFormat << 2 | pOpenParam->cbcrInterleave << 1 | pOpenParam->nv21;

    switch (pOpenParam->packedFormat)
    { // additional packed format
    ...
    if (option->codeOption.implicitHeaderEncode == 1)
    {
        VpuWriteReg(coreIdx, W4_CMD_ENC_CODE_OPTION,
                    CODEOPT_ENC_HEADER_IMPLICIT | CODEOPT_ENC_VCL | // implicitly encode a
                        // header(headers) for
                        ...
                        // for GiveCommand)
                        (option->codeOption.encodeAUD << 5) | (option->codeOption.encodeEOS << 6) | (option->codeOption.encodeEOB << 7));
    }
    else
    {
        VpuWriteReg(coreIdx, W4_CMD_ENC_CODE_OPTION,
                    (option->codeOption.implicitHeaderEncode << 0) | (option->codeOption.encodeVCL << 1) | (option->codeOption.encodeVPS << 2) | (option->codeOption.encodeSPS << 3) | (option->codeOption.encodePPS << 4) | (option->codeOption.encodeAUD << 5) | (option->codeOption.encodeEOS << 6) | (option->codeOption.encodeEOB << 7) | (option->codeOption.encodeVUI << 9));
    }

    VpuWriteReg(coreIdx, W4_CMD_ENC_PIC_PARAM,
                (option->skipPicture << 0) | (option->forcePicQpEnable << 1) | (option->forcePicQpI << 2) | (option->forcePicQpP << 8) | (option->forcePicQpB << 14) | (option->forcePicTypeEnable << 20) | (option->forcePicType << 21) | (option->forcePicQpSrcOrderEnable << 29) | (option->forcePicTypeSrcOrderEnable << 30));
    ...
    CVI_VC_SRC("bufY = 0x" VCODEC_UINT64_FORMAT_HEX
               ", bufCb = 0x" VCODEC_UINT64_FORMAT_HEX
               ", bufCr = 0x" VCODEC_UINT64_FORMAT_HEX "\n",
    ...
    {
        srcStrideC = VPU_ALIGN16((pSrcFrame->stride >> 1)) * (1 << pSrcFrame->cbcrInterleave);
        if (pSrcFrame->cbcrInterleave == 1) srcStrideC = pSrcFrame->stride;
    }
    else
    {
        srcStrideC = (pSrcFrame->cbcrInterleave == 1) ? pSrcFrame->stride
                                                      : (pSrcFrame->stride >> 1);
    }
    ...
    VpuWriteReg(coreIdx, W4_CMD_ENC_SRC_FORMAT,
                (srcFrameFormat << 0) | (srcPixelFormat << 3) | (bsEndian << 6));
    ...
    VpuWriteReg(coreIdx, W4_CMD_ENC_PREFIX_SEI_INFO,
                pEncInfo->prefixSeiDataSize << 16 | pEncInfo->prefixSeiDataEncOrder << 1 | pEncInfo->prefixSeiNalEnable);
    ...
    VpuWriteReg(coreIdx, W4_CMD_ENC_SUFFIX_SEI_INFO,
                pEncInfo->suffixSeiDataSize << 16 | pEncInfo->suffixSeiDataEncOrder << 1 | pEncInfo->suffixSeiNalEnable);
    ...
    VpuWriteReg(coreIdx, W4_CMD_ENC_CTU_OPT_PARAM,
                ((option->ctuOptParam.roiEnable) << 0) | (option->ctuOptParam.roiDeltaQp << 1) | (option->ctuOptParam.ctuModeEnable << 8) | (option->ctuOptParam.ctuQpEnable << 9) | (option->ctuOptParam.mapEndian << 12) | (option->ctuOptParam.mapStride << 16));

    VpuWriteReg(coreIdx, W4_CMD_ENC_SRC_TIMESTAMP_LOW, 0);
    VpuWriteReg(coreIdx, W4_CMD_ENC_SRC_TIMESTAMP_HIGH, 0);
    VpuWriteReg(
        coreIdx, W4_CMD_ENC_LONGTERM_PIC,
        (option->useCurSrcAsLongtermPic << 0) | (option->useLongtermRef << 1));
...
static void cviDumpVpuReg(Int32 coreIdx)
{
    ...
    for (idx = 0x100; idx <= 0x200; idx += 0x10)
    {
        printf("0x%04X  %08X  %08X  %08X  %08X\n", idx, VpuReadReg(coreIdx, idx),
               VpuReadReg(coreIdx, idx + 0x4), VpuReadReg(coreIdx, idx + 0x8),
...
RetCode Wave4VpuEncGetResult(CodecInst *instance, EncOutputInfo *result)
{
...
#ifdef ENABLE_CNM_DEBUG_MSG
    if (instance->loggingEnable) vdi_log(coreIdx, ENC_PIC, 0);
#endif
    ...
    if (encodingSuccess == FALSE)
    {
        ...
        {
            CVI_VC_ERR("errorReason == WAVE4_SYSERR_WRITEPROTECTION\n");
#ifdef ENABLE_CNM_DEBUG_MSG
            vdi_print_vpu_status(coreIdx);
#endif
            return RETCODE_MEMORY_ACCESS_VIOLATION;
        }
        if (errorReason == WAVE4_SYSERR_CP0_DATA_ACCESS_VIOLATION || errorReason == WAVE4_SYSERR_CP0_INST_ADDR_ALIGN || errorReason == WAVE4_SYSERR_CP0_DATA_ADDR_ALIGN || errorReason == WAVE4_SYSERR_CP0_PRIVILEDGE_VIOLATION || errorReason == WAVE4_SYSERR_CP0_WRITE_PROT)
        {
            CVI_VC_ERR("errorReason == WAVE4_SYSERR_CP0_EXCEPTION\n");
#ifdef ENABLE_CNM_DEBUG_MSG
            vdi_print_vpu_status(coreIdx);
#endif
        ...
        {
            CVI_VC_ERR("errorReason == WAVE4_SYSERR_STREAM_BUF_FULL\n");
#ifdef ENABLE_CNM_DEBUG_MSG
            vdi_print_vpu_status(coreIdx);
#endif
        ...
        {
            CVI_VC_ERR("errorReason == WAVE4_SYSERR_ACCESS_VIOLATION_HW\n");
#ifdef ENABLE_CNM_DEBUG_MSG
            vdi_print_vpu_status(coreIdx);
#endif
    ...
    CVI_VC_BS(
        "bitstreamSize = 0x%X, bitstreamBuffer = 0x" VCODEC_UINT64_FORMAT_HEX
        "\n"
        "rdPtr = 0x" VCODEC_UINT64_FORMAT_HEX
        ", wrPtr = 0x" VCODEC_UINT64_FORMAT_HEX "\n",
...
static void printResult(EncInfo *pEncInfo, EncOutputInfo *result)
{
    ...
    CVI_VC_TRACE("rdPtr = 0x" VCODEC_UINT64_FORMAT_HEX "\n", result->rdPtr);
    CVI_VC_TRACE("wrPtr = 0x" VCODEC_UINT64_FORMAT_HEX "\n", result->wrPtr);
    CVI_VC_TRACE("bitstreamSize = 0x%X\n", result->bitstreamSize);
    CVI_VC_FLOW("rdPtr = 0x" VCODEC_UINT64_FORMAT_HEX
                ", wrPtr = 0x" VCODEC_UINT64_FORMAT_HEX
                ", bitstreamSize = 0x%x\n",
                result->rdPtr, result->wrPtr, result->bitstreamSize);
}

RetCode Wave4VpuEncGetHeader(EncHandle       instance,
                             EncHeaderParam *encHeaderParam)
{
    EncInfo        *pEncInfo;
    EncOpenParam   *encOP;
    ...
    CVI_VC_BS("buf = 0x" VCODEC_UINT64_FORMAT_HEX ", size = 0x%zx\n",
              encHeaderParam->buf, encHeaderParam->size);
...
RetCode Wave4VpuEncParaChange(EncHandle instance, EncChangeParam *param)
{
    ...
    if (param->changeParaMode == OPT_COMMON)
    {
        ...
        if (param->enable_option & ENC_PIC_PARAM_CHANGE)
        {
            ...
            {
                regVal =
                    (param->losslessEnable << 21) | (param->constIntraPredFlag << 22);
                VpuWriteReg(coreIdx, W4_CMD_ENC_SEQ_PIC_PARAM, regVal);
            }
            else
            { // WAVE420L
                VpuWriteReg(coreIdx, W4_CMD_ENC_SEQ_PIC_PARAM,
                            (param->losslessEnable << 20) | (param->constIntraPredFlag << 21) | ((param->chromaCbQpOffset & 0x1f) << 22) | ((param->chromaCrQpOffset & 0x1f) << 27));
            }
        }
        ...
        {
            VpuWriteReg(coreIdx, W4_CMD_ENC_SEQ_INTRA_PARAM,
                        (param->decodingRefreshType << 0) | (param->intraQP << 3) | (param->intraPeriod << 16));
        }

        if (param->enable_option & ENC_CONF_WIN_TOP_BOT_CHANGE)
        {
            VpuWriteReg(coreIdx, W4_CMD_ENC_SEQ_CONF_WIN_TOP_BOT,
                        param->confWinBot << 16 | param->confWinTop);
        }

        if (param->enable_option & ENC_CONF_WIN_LEFT_RIGHT_CHANGE)
        {
            VpuWriteReg(coreIdx, W4_CMD_ENC_SEQ_CONF_WIN_LEFT_RIGHT,
                        param->confWinRight << 16 | param->confWinLeft);
        }
        ...
        if (param->enable_option & ENC_RC_INTRA_MIN_MAX_QP_CHANGE)
        {
            if (instance->productId == PRODUCT_ID_420)
            {
                VpuWriteReg(coreIdx, W4_CMD_ENC_RC_INTRA_MIN_MAX_QP,
                            param->intraMaxQp << 6 | param->intraMinQp);
            }
        }
        ...
        {
            VpuWriteReg(coreIdx, W4_CMD_ENC_SEQ_DEPENDENT_SLICE,
                        param->dependSliceModeArg << 16 | param->dependSliceMode);
        }

        if (param->enable_option & ENC_INTRA_REFRESH_CHANGE)
        {
            VpuWriteReg(coreIdx, W4_CMD_ENC_SEQ_INTRA_REFRESH,
                        param->intraRefreshArg << 16 | param->intraRefreshMode);
        }
        ...
        if (param->enable_option & ENC_PARAM_CHANGE)
        {
            VpuWriteReg(
                ...
                (param->useRecommendEncParam) | (param->scalingListEnable << 3) | (0x7 << 4) | // cuSizeMode should be
                    // always 0x7 (should use
                    // all cu size (cu8x8,
                    // cu16x16, cu32x32))
                    (param->tmvpEnable << 7) | (param->wppEnable << 8) | (param->maxNumMerge << 9) | (param->dynamicMerge8x8Enable << 12) | (param->dynamicMerge16x16Enable << 13) | (param->dynamicMerge32x32Enable << 14) | (param->disableDeblk << 15) | (param->lfCrossSliceBoundaryEnable << 16) | ((param->betaOffsetDiv2 & 0xF) << 17) | ((param->tcOffsetDiv2 & 0xF) << 21) | (param->skipIntraTrans << 25) | (param->saoEnable << 26) | (param->intraInInterSliceEnable << 27) | (param->intraNxNEnable << 28));
        }
        if (param->enable_option & ENC_RC_PARAM_CHANGE)
        {
            ...
            {
                VpuWriteReg(coreIdx, W4_CMD_ENC_RC_PARAM,
                            (param->rcEnable << 0) | (param->cuLevelRCEnable << 1) | (param->hvsQPEnable << 2) | (param->hvsQpScaleEnable << 3) | (param->hvsQpScale << 4) | (param->bitAllocMode << 8) | (param->initBufLevelx8 << 10) |
                                /* [15] reserved */
            ...
            else
            {
                VpuWriteReg(
                    ...
                    (param->rcEnable << 0) | (param->cuLevelRCEnable << 1) | (param->hvsQPEnable << 2) | (param->hvsQpScaleEnable << 3) | (param->hvsQpScale << 4) | (param->bitAllocMode << 7) | (param->initBufLevelx8 << 9) | (param->initialRcQp << 14) | (param->initialDelay << 20));
            }
        }
        ...
        {
            VpuWriteReg(coreIdx, W4_CMD_ENC_RC_MIN_MAX_QP,
                        (param->minQp << 0) | (param->maxQp << 6) | (param->maxDeltaQp << 12) | (param->intraQpOffset << 18));
        }

        if (param->enable_option & ENC_RC_TARGET_RATE_LAYER_0_3_CHANGE)
        {
            VpuWriteReg(coreIdx, W4_CMD_ENC_RC_BIT_RATIO_LAYER_0_3,
                        (param->fixedBitRatio[0] << 0) | (param->fixedBitRatio[1] << 8) | (param->fixedBitRatio[2] << 16) | (param->fixedBitRatio[3] << 24));
        }

        if (param->enable_option & ENC_RC_TARGET_RATE_LAYER_4_7_CHANGE)
        {
            VpuWriteReg(coreIdx, W4_CMD_ENC_RC_BIT_RATIO_LAYER_4_7,
                        (param->fixedBitRatio[4] << 0) | (param->fixedBitRatio[5] << 8) | (param->fixedBitRatio[6] << 16) | (param->fixedBitRatio[7] << 24));
        }
        if (param->enable_option & ENC_NR_PARAM_CHANGE)
        {
            VpuWriteReg(
                coreIdx, W4_CMD_ENC_NR_PARAM,
                (param->nrYEnable << 0) | (param->nrCbEnable << 1) | (param->nrCrEnable << 2) | (param->nrNoiseEstEnable << 3) | (param->nrNoiseSigmaY << 4) | (param->nrNoiseSigmaCb << 12) | (param->nrNoiseSigmaCr << 20));
        }
        ...
        if (param->enable_option & ENC_NR_WEIGHT_CHANGE)
        {
            VpuWriteReg(
                coreIdx, W4_CMD_ENC_NR_WEIGHT,
                (param->nrIntraWeightY << 0) | (param->nrIntraWeightCb << 5) | (param->nrIntraWeightCr << 10) | (param->nrInterWeightY << 15) | (param->nrInterWeightCb << 20) | (param->nrInterWeightCr << 25));
        }
        ...
        {
            VpuWriteReg(coreIdx, W4_CMD_ENC_RC_TARGET_RATE, param->bitRate * 1000);
        }
       ...
       */
            ...
            if (pEncInfo->rotationEnable == FALSE)
            { // previous
                // rotator is
                // disabled,
                // change to
            ...
            { // previous rotator angle is not matched with
...
RetCode CheckEncCommonParamValid(EncOpenParam *pop)
{
    ...
    else if (param->gopPresetIdx == 2 || param->gopPresetIdx == 3 || param->gopPresetIdx == 6 || param->gopPresetIdx == 7) // low-delay case (IPPP, IBBB)
        low_delay = 1;
    ...
    {
        VLOG(ERR,
             "CFG FAIL : Not support decoding refresh type[%d] for closed gop "
             "structure\n",
    ...
    if (param->gopPresetIdx == PRESET_IDX_CUSTOM_GOP)
    {
        for (i = 0; i < param->gopParam.customGopSize; i++)
        {
            if (param->gopParam.picParam[i].temporalId >= MAX_NUM_TEMPORAL_LAYER)
            {
                ...
                VLOG(ERR,
                     "RECOMMEND CONFIG PARAMETER : Adjust temporal ID under "
                     "MAX_NUM_TEMPORAL_LAYER(7) in GOP structure\n");
                ret = RETCODE_FAILURE;
            }
            ...
            if (param->gopParam.picParam[i].temporalId < 0)
            {
                VLOG(ERR, "CFG FAIL : Must be %d-th temporal_id >= 0\n",
                     param->gopParam.picParam[i].temporalId);
                VLOG(ERR,
                     "RECOMMEND CONFIG PARAMETER : Adjust temporal layer above '0' in "
                     "GOP structure\n");
                ret = RETCODE_FAILURE;
            }
        }
    ...
    if (param->useRecommendEncParam == 0)
    {
        ...
        {
            VLOG(ERR,
                 "CFG FAIL : If intraInInterSliceEnable is '0', Intra refresh mode "
                 "must be '0'\n");
        ...
        // RDO
        {
            ...
            {
                VLOG(ERR,
                     "CFG FAIL : Picture width and height must be aligned with 8 "
                     "pixels when enable CU8x8 of cuSizeMode\n");
            ...
            {
                VLOG(ERR,
                     "CFG FAIL : Picture width and height must be aligned with 16 "
                     "pixels when enable CU16x16 of cuSizeMode\n");
            ...
            {
                VLOG(ERR,
                     "CFG FAIL : Picture width and height must be aligned with 32 "
                     "pixels when enable CU32x32 of cuSizeMode\n");
        ...
        {
            VLOG(ERR,
                 "CFG FAIL : If WaveFrontSynchro(WPP) '1', the option of multi-slice "
                 "must be '0'\n");
            VLOG(ERR,
                 "RECOMMEND CONFIG PARAMETER : independSliceMode = 0, "
                 "dependSliceMode = 0\n");
            ret = RETCODE_FAILURE;
        }
    }
    ...
    // Slice
    {
        ...
        {
            VLOG(ERR,
                 "CFG FAIL : If independSliceMode is '0', dependSliceMode must be "
                 "'0'\n");
            VLOG(ERR,
                 "RECOMMEND CONFIG PARAMETER : independSliceMode = 1, "
                 "independSliceModeArg = TotalCtuNum\n");
            ret = RETCODE_FAILURE;
        }
        else if ((param->independSliceMode == 1) && (param->dependSliceMode == 1))
        {
            ...
            {
                VLOG(ERR,
                     "CFG FAIL : If independSliceMode & dependSliceMode is both '1' "
                     "(multi-slice with ctu count), must be independSliceModeArg >= "
                     "dependSliceModeArg\n");
        ...
        if (param->independSliceMode != 0)
        {
            ...
            {
                VLOG(ERR,
                     "CFG FAIL : If independSliceMode is not 0, must be "
                     "independSliceModeArg <= 0xFFFF\n");
                ret = RETCODE_FAILURE;
            }
        }
        ...
        if (param->dependSliceMode != 0)
        {
            ...
            {
                VLOG(ERR,
                     "CFG FAIL : If dependSliceMode is not 0, must be "
                     "dependSliceModeArg <= 0xFFFF\n");
                ret = RETCODE_FAILURE;
            }
        }
    }
    ...
    {
        VLOG(ERR,
             "CFG FAIL : conf_win_top : %d value is not available. The value "
             "should be equal to multiple of 2.\n",
             param->confWinTop);
        ret = RETCODE_FAILURE;
    }
    ...
    {
        VLOG(ERR,
             "CFG FAIL : conf_win_bot : %d value is not available. The value "
             "should be equal to multiple of 2.\n",
             param->confWinBot);
        ret = RETCODE_FAILURE;
    }
    ...
    {
        VLOG(ERR,
             "CFG FAIL : conf_win_left : %d value is not available. The value "
             "should be equal to multiple of 2.\n",
             param->confWinLeft);
        ret = RETCODE_FAILURE;
    }
    ...
    {
        VLOG(ERR,
             "CFG FAIL : conf_win_right : %d value is not available. The value "
             "should be equal to multiple of 2.\n",
             param->confWinRight);
        ret = RETCODE_FAILURE;
    }
...
RetCode CheckEncRcParamValid(EncOpenParam *pop)
{
    ...
    if (pop->rcEnable == 1)
    {
        ...
        if (((pop->bitRate * 1000) > param->transRate) && (param->transRate != 0))
        {
