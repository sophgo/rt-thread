...
#define CVI_JPG_DBG_FLOAT(msg, ...)                                             \
    do {                                                                        \
        if (jpeg_mask & CVI_MASK_FLOAT)                                         \
            printf("[FLOAT] %s = %d, " msg, __func__, __LINE__, ##__VA_ARGS__); \
    } while (0)
...
typedef struct jpu_buffer_t
{
    ...
    uint64_t phys_addr;
    uint64_t base;
    uint8_t *virt_addr; /* virtual user space address */
#ifdef __arm__
    uint32_t padding;   /* padding for keeping same size of this structure */
#endif
} jpu_buffer_t;
