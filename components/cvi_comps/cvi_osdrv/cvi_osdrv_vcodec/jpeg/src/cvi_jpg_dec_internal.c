...
int cviJpgDecOpen(CVIJpgHandle *pHandle, CVIDecConfigParam *pConfig)
{
    ...
    {
        JLOG(ERR,
             "Invalid operation mode : packed mode and ROI mode can not be "
             "worked\n");
        ret = JPG_RET_INVALID_PARAM;
        goto ERR_DEC_INIT;
    }

    if ((pConfig->iHorScaleMode || pConfig->iVerScaleMode) && pConfig->roiEnable)
    {
        JLOG(ERR,
             "Invalid operation mode : Scaler mode and ROI mode can not be "
             "worked\n");
        ret = JPG_RET_INVALID_PARAM;
        goto ERR_DEC_INIT;
    }
    ...
    {
        JLOG(ERR,
             "Invalid operation mode : Rotator mode and ROI mode can not be "
             "worked\n");
        ret = JPG_RET_INVALID_PARAM;
        goto ERR_DEC_INIT;
    }

    if (pConfig->dec_buf.packedFormat < 0 || pConfig->dec_buf.packedFormat > 6)
    {
        JLOG(ERR, "Invalid operation mode : packedFormat must between 0~5\n");
        ret = JPG_RET_INVALID_PARAM;
        goto ERR_DEC_INIT;
    }
   ...
   */
    if (PACKED_FORMAT_444_RGB == (PackedOutputFormat)pConfig->dec_buf.packedFormat)
    {
        decOP.packedFormat = PACKED_FORMAT_444;
    }
    else
    {
        decOP.packedFormat = (PackedOutputFormat)pConfig->dec_buf.packedFormat;
    }
    /* UV Interleave Mode */
    if (decOP.packedFormat)
        decOP.chromaInterleave = (CbCrInterLeave)CBCR_SEPARATED;
    else
        decOP.chromaInterleave = (CbCrInterLeave)pConfig->dec_buf.chromaInterleave;
    ...
    {
        JLOG(ERR,
             "The length 0f the input stream is greater than the length of the JPU "
             "input buffer!, vbStream addr = 0x%lX\n",
             vbStream.phys_addr);
        goto ERR_DEC_INIT;
    }
    else
    {
        vbStream.size =
            (pConfig->iDataLen & 0xffffc000) + 0x4000; // STREAM_BUF_SIZE;
    }

    CVI_JPG_DBG_FLOW("vbStream.size = 0x%x\n", vbStream.size);

    if (JDI_ALLOCATE_MEMORY(&vbStream, 0, BS_STREAM_CACHED) < 0)
    {
    ...
    {
        JLOG(ERR,
             "JPU_DecGiveCommand[SET_JPG_USE_PARTIAL_MODE] failed Error code is "
             "0x%x\n",
             ret);
        goto ERR_DEC_INIT;
    }
    ...
    {
        JLOG(ERR,
             "JPU_DecGiveCommand[SET_JPG_PARTIAL_FRAME_NUM] failed Error code is "
             "0x%x\n",
             ret);
        goto ERR_DEC_INIT;
    }
...
int cviJpgDecClose(CVIJpgHandle handle)
{
    ...
    {
        JPU_ReleaseFlock();
        printf("cviJpgDecClose   CheckJpgInstValidity failed Error code is 0x%x \n",
               ret);
        return ret;
    }
...
int cviJpgDecSendFrameData(CVIJpgHandle handle, void *data, int length)
{
    ...
    bufInfo.size  = length;
    bufInfo.point = 0;
    ret           = WriteJpgBsBufHelper(handle, &bufInfo, pDecInfo->streamBufStartAddr,
                                        pDecInfo->streamBufEndAddr, 0, 0, &streameos,
                                        pDecInfo->streamEndian);
    if (ret != JPG_RET_SUCCESS)
    {
        printf("WriteBsBufHelper failed Error code is 0x%x\n", ret);
    ...
    if (1 != pDecInfo->initialInfoObtained)
    {
        ...
        if (ret != JPG_RET_SUCCESS)
        {
            printf("JPU_DecGetInitialInfo failed Error code is 0x%x\n", ret);
            return ret;
        }
    }
    if (pDecInfo->usePartial)
    {
        ...
        if (partMaxIdx < pDecInfo->bufNum) pDecInfo->bufNum = partMaxIdx;
    }
    ...
    if (framebufWidth < 128 || framebufHeight < 128)
    {
        if (pDecInfo->iHorScaleMode || pDecInfo->iVerScaleMode)
            JLOG(WARN,
                 "Invalid operation mode : Not supported resolution with Scaler, "
                 "width=%d, height=%d\n",
    ...
    if (pDecInfo->packedFormat >= PACKED_FORMAT_422_YUYV && pDecInfo->packedFormat <= PACKED_FORMAT_422_VYUY)
    {
    ...
    if (pDecInfo->usePartial)
    {
        if (pDecInfo->bufNum > 4) pDecInfo->bufNum = 4;

        regFrameBufCount *= pDecInfo->bufNum;
        ret =
            JPU_DecGiveCommand(handle, SET_JPG_PARTIAL_LINE_NUM, &(partialHeight));
        if (ret != JPG_RET_SUCCESS)
        {
            JLOG(ERR,
                 "JPU_DecGiveCommand[SET_JPG_PARTIAL_LINE_NUM] failed Error code is "
                 "0x%x\n",
                 ret);
            return ret;
        }
    ...
    if (pDecInfo->dst_type != JPEG_MEM_EXTERNAL)
    {
        ...
        {
            pDecInfo->pFrame[i]         = GetFrameBuffer(pJpgInst->instIndex, i);
            pDecInfo->frameBuf[i].bufY  = pDecInfo->pFrame[i]->vbY.phys_addr;
            pDecInfo->frameBuf[i].bufCb = pDecInfo->pFrame[i]->vbCb.phys_addr;
            if (pDecInfo->chromaInterleave == CBCR_SEPARATED)
                pDecInfo->frameBuf[i].bufCr = pDecInfo->pFrame[i]->vbCr.phys_addr;
        }

        if (pDecInfo->rotationEnable == 1 && pDecInfo->mirrorEnable == 1)
        {
            JPU_DecGiveCommand(handle, SET_JPG_ROTATOR_OUTPUT,
        ...
        ret = JPU_DecRegisterFrameBuffer(handle, pDecInfo->frameBuf,
                                         regFrameBufCount, framebufStride);
        if (ret != JPG_RET_SUCCESS)
        {
            printf("JPU_DecRegisterFrameBuffer failed Error code is 0x%x\n", ret);
            goto SEND_DEC_DATA_ERR;
        }
    }
    else
    {
        pDecInfo->dst_info.strideY = framebufStride;
    ...
    if (ret != JPG_RET_SUCCESS && ret != JPG_RET_EOS)
    {
        ...
        {
            printf("JPU_DecStartOneFrame failed Error code is 0x%x\n", ret);
            goto SEND_DEC_DATA_ERR;
        }
    }
    ...
    while (1)
    {
        ...
        if (-1 == int_reason)
        {
            ...
            {
                printf(
                    "info : jpu dec interruption timeout happened and is reset "
                    "automatically\n");
            }
#endif
        ...
        if (int_reason & (1 << INT_JPU_DONE) || int_reason & (1 << INT_JPU_ERROR))
        {
        ...
        if (int_reason & (1 << INT_JPU_BIT_BUF_STOP))
        {
            ...
            if (ret != JPG_RET_SUCCESS)
            {
                printf("JPU_DecCompleteStop failed Error code is 0x%x\n", ret);
                goto SEND_DEC_DATA_ERR;
            }
...
int cviJpgDecGetFrameData(CVIJpgHandle jpgHandle, void *data)
{
    ...
    if (ret != JPG_RET_SUCCESS)
    {
        printf("JPU_DecGetOutputInfo failed Error code is 0x%x\n", ret);
        goto GET_DEC_DATA_ERR;
    }

    if (outputInfo.decodingSuccess == 0)
        printf("JPU_DecGetOutputInfo decode fail\n");

#ifdef CVIDEBUG_V
    JLOG(TRACE,
         "#%d || consumedByte %d || frameStart=0x%x || ecsStart=0x%x || "
         "rdPtr=0x%x || wrPtr=0x%x || pos=%d\n",
    ...
    cviFrameBuf->chromaInterleave = (CVICbCrInterLeave)pDecInfo->chromaInterleave;
    if (pDecInfo->dst_type == JPEG_MEM_EXTERNAL)
    {
        cviFrameBuf->vbY.phys_addr = pDecInfo->dst_info.bufY;
        cviFrameBuf->strideY       = pDecInfo->dst_info.strideY;
        ...
        case CVI_FORMAT_422:
            ...
            cviFrameBuf->strideC        = pDecInfo->dst_info.strideY >> 1;
            break;
        case CVI_FORMAT_444:
            cviFrameBuf->vbCb.phys_addr = pDecInfo->dst_info.bufCb;
            cviFrameBuf->vbCr.phys_addr = pDecInfo->dst_info.bufCr;
            cviFrameBuf->strideC        = pDecInfo->dst_info.strideY;
            break;
        case CVI_FORMAT_420:
        default:
    ...
    else
    {
        ...
        if (pDecInfo->rotationAngle == 90 || pDecInfo->rotationAngle == 270)
        {
            cviFrameBuf->width  = outputInfo.decPicHeight; // pDecInfo->picHeight;
            cviFrameBuf->height = outputInfo.decPicWidth;  // pDecInfo->picWidth;
        }
        else
        {
            cviFrameBuf->width  = pDecInfo->picWidth;
        ...
        if (pDecInfo->chromaInterleave) cviFrameBuf->strideC *= 2;
    }
