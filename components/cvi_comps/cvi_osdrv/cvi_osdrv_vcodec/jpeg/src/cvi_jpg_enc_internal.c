...
extern int jdi_invalidate_ion_cache(uint64_t u64PhyAddr, void *pVirAddr,
                                    uint32_t u32Len);
extern int jdi_flush_ion_cache(uint64_t u64PhyAddr, void *pVirAddr,
                               uint32_t u32Len);
...
void _cviJpeSetRcInfo(stRcInfo *pRcInfo, stRcCfg *pRcCfg)
{
    ...
    pRcInfo->picAvgBit =
        (int)((((float)pRcInfo->targetBitrate / 8) * 1000) / fFps);
...
static void cviJpeRc_RcKernelInit(stRcInfo *pRcInfo, stRcCfg *pRcCfg)
{
    ...
    CVI_JPG_DBG_RC("pRcInfo->maxQs = %d set %d\n", pRcInfo->maxQs,
                   cviJpgGetEnv("maxIQp"));
    pRcInfo->maxQs =
        (pRcInfo->maxQs >= 0) ? cvi_jpeg_quality_scaling(pRcInfo->maxQs) : 1;
    // pRcInfo->minQs = cviJpgGetEnv("minIQp");
    CVI_JPG_DBG_RC("pRcInfo->minQs = %d set %d\n", pRcInfo->minQs,
                   cviJpgGetEnv("minIQp"));
    pRcInfo->minQs =
        (pRcInfo->minQs >= 0) ? cvi_jpeg_quality_scaling(pRcInfo->minQs) : 100;
    ...
    pRcKerCfg->framerate       = CVI_FLOAT_DIV(INT_TO_CVI_FLOAT(frameRateRes),
    ...
    CVI_JPG_DBG_CVRC(
        "targetBitrate = %d, codec = %d, framerate = %d, intraPeriod = %d\n",
        pRcKerCfg->targetBitrate, pRcKerCfg->codec, pRcKerCfg->framerate,
        pRcKerCfg->intraPeriod);
    CVI_JPG_DBG_CVRC(
        "statTime = %d, ipQpDelta = %d, numOfPixel = %d, maxIprop = %d, minIprop "
        "= %d\n",
        pRcKerCfg->statTime, pRcKerCfg->ipQpDelta, pRcKerCfg->numOfPixel,
        pRcKerCfg->maxIprop, pRcKerCfg->minIprop);
    CVI_JPG_DBG_CVRC("maxQp = %d, minQp = %d, maxIQp = %d, minIQp = %d\n",
                     pRcKerCfg->maxQp, pRcKerCfg->minQp, pRcKerCfg->maxIQp,
                     pRcKerCfg->minIQp);
    CVI_JPG_DBG_CVRC("firstFrmstartQp = %d, rcMdlUpdatType = %d\n",
                     pRcKerCfg->firstFrmstartQp, pRcKerCfg->rcMdlUpdatType);
    CVI_JPG_DBG_CVRC("minQs = %d, maxQs = %d\n", pRcInfo->minQs, pRcInfo->maxQs);
    ...
    cviRcKernel_setRCModelParam(pRcKerInfo, INT_TO_CVI_FLOAT(CVI_RC_DEF_ALPHA),
                                INT_TO_CVI_FLOAT(CVI_RC_DEF_BETA), 0);

    printf("  fn, targetBit, qp, encodedBit\n");
    printf("-------------------------------\n");
}
...
#else
    if (pRcInfo->cviRcEn) return cviJpeRc_RcKernelEstimatePic(pRcInfo);

    // #! hard code here
    pRcInfo->picTargetBit = (int)CLIP(1, INT_MAX,
...
static int cviJpeRc_RcKernelEstimatePic(stRcInfo *pRcInfo)
{
    ...
    CVI_JPG_DBG_FLOAT("qp = %d, lambda = %f, targetBit = %d\n", pRcPicOut->qp,
                      getFloat(pRcPicOut->lambda), pRcPicOut->targetBit);
...
void cviJpeRc_UpdatePic(stRcInfo *pRcInfo, int encByte)
{
    ...
    if (pRcInfo->cviRcEn) cviJpeRc_RcKernelUpdatePic(pRcInfo, encByte);
}
...
static void cviJpeRc_RcKernelUpdatePic(stRcInfo *pRcInfo, int encByte)
{
    ...
    {
        printf("%4d, %9d, %2d, %11d\n", pRcInfo->picIdx, pRcPicOut->targetBit,
               pRcPicOut->qp, pRcPicIn->encodedBit);
    }
}
 ...
 */
{
   ...
   * cvi_jpeg_add_quant_table to make all the table entries 1 (hence, minimum
 ...
 */
{
    ...
    if (scale_factor > 5000) scale_factor = 5000;
                       ...
                       unsigned char *qMatTab1)
{
    ...
    cvi_jpeg_add_quant_table(qMatTab0, std_luminance_quant_tbl, scale_factor,
                             force_baseline);
    cvi_jpeg_add_quant_table(qMatTab1, std_chrominance_quant_tbl, scale_factor,
                             force_baseline);
}
                             ...
                             int packed)
{
    ...
    if (picWidth == stride)
    {
        JpuWriteMem(addr, (Uint8 *)(srcFrameBuffer->vbY.virt_addr), lumaSize,
                    endian);

        if (format == FORMAT_400) return size;
    ...
    else
    {
        for (y = 0; y < nY; ++y)
        {
            JpuWriteMem(addr + stride * y,
                        (Uint8 *)(srcFrameBuffer->vbY.virt_addr + y * picWidth),
                        picWidth, endian);
        }
...
int cviJpeEncSetRcParam(CVIJpgHandle pHandle, struct CVIJpegEncRcParam *rc)
{
    ...
    if (pEncInfo->openParam.bitrate != 0)
    {
        ...
        pRcCfg->targetBitrate = pEncInfo->openParam.bitrate * rc->chgPos / 100;
        /*jpg maxqfactor got hight quality so need inverse*/
        pRcInfo->minQs = rc->maxQfactor;
        pRcInfo->maxQs = rc->minQfactor;
...
int cviJpgEncOpen(CVIJpgHandle *pHandle, CVIEncConfigParam *pConfig)
{
    ...
    if ((pConfig->rotAngle != 0) || (pConfig->mirDir != 0))
    {
        ...
        JPU_EncGiveCommand(handle, SET_JPG_ROTATION_ANGLE, &pConfig->rotAngle);
        JPU_EncGiveCommand(handle, SET_JPG_MIRROR_DIRECTION, &pConfig->mirDir);
    }
    ...
    if (pEncOP->packedFormat >= PACKED_FORMAT_422_YUYV && pEncOP->packedFormat <= PACKED_FORMAT_422_VYUY)
    {
    ...
    if (pConfig->src_type != JPEG_MEM_EXTERNAL)
    {
        ...
        if (!AllocateFrameBuffer(pJpgInst->instIndex, srcFrameFormat,
                                 framebufStride, framebufHeight, needFrameBufCount,
                                 pEncOP->packedFormat))
        {
        ...
        for (i = 0; i < needFrameBufCount; ++i)
        {
            ...
            pEncInfo->frameBuf[i].strideY = pEncInfo->pFrame[i]->strideY;
            pEncInfo->frameBuf[i].strideC = pEncInfo->pFrame[i]->strideC;
...
static int cviJpgGetEnv(char *envVar)
{
    ...
    if (strcmp(envVar, "maxIQp") == 0) return maxIQp;
    if (strcmp(envVar, "minIQp") == 0) return minIQp;
    if (strcmp(envVar, "cviRcEn") == 0) return cviRcEn;
...
int cviJpgEncClose(CVIJpgHandle handle)
{
    ...
    /* free frame buffer */
    if (pEncInfo->pFrame[0]) FreeFrameBuffer(pJpgInst->instIndex);
    pEncInfo->pFrame[0]           = 0;
    pEncInfo->frameBuf[0].bufY    = 0;
    pEncInfo->frameBuf[0].bufCb   = 0;
    pEncInfo->frameBuf[0].bufCr   = 0;
    pEncInfo->frameBuf[0].strideY = 0;
    pEncInfo->frameBuf[0].strideC = 0;
...
{
#ifndef REENCODE_JPEG_SUPERFRAME
    BYTE *p = malloc(pEncInfo->preStreamLen + pEncInfo->streamBufSize);
    ...
    jdi_invalidate_ion_cache(pEncInfo->streamBufStartAddr, pEncInfo->pBitStream,
...
int cviJpgEncSendFrameData(CVIJpgHandle handle, void *data, int srcType)
{
    ...
    {
        LoadYuvImageBurstFormat2((BYTE *)data, pEncInfo->frameBuf[srcFrameIdx].bufY,
                                 ...
                                 pEncInfo->chromaInterleave, pEncInfo->sourceFormat,
                                 pEncInfo->frameEndian, pEncInfo->packedFormat);
    ...
    else if (JPEG_MEM_EXTERNAL == srcType)
    {
        ...
        externalBuf.strideY        = pSrcInfo->strideY;
        externalBuf.strideC        = pSrcInfo->strideC;
        ...
        if (pEncInfo->sourceFormat == FORMAT_420 || pEncInfo->sourceFormat == FORMAT_224)
            pEncInfo->alignedHeight = ((pEncInfo->picHeight + 15) >> 4) << 4;
        else
    ...
    if ((pEncInfo->openParam.bitrate != pRcInfo->targetBitrate) || (pEncInfo->openParam.framerate != pRcInfo->fps))
    {
        stRcCfg *pRcCfg = &pEncInfo->openParam.RcCfg;
    ...
    if (pEncInfo->openParam.bitrate != 0 || pEncInfo->openParam.quality != 0)
    {
        ...
        {
            pEncInfo->curScalar = cviJpeRc_EstimatePicQs(&pEncInfo->openParam.RcInfo);
            cvi_jpgGetQMatrix(pEncInfo->curScalar,
                              pEncInfo->openParam.qMatTab[DC_TABLE_INDEX0],
                              pEncInfo->openParam.qMatTab[AC_TABLE_INDEX0]);
        }
        else
        {
            cvi_jpgGetQMatrix(cvi_jpeg_quality_scaling(pEncInfo->openParam.quality),
                              pEncInfo->openParam.qMatTab[DC_TABLE_INDEX0],
                              pEncInfo->openParam.qMatTab[AC_TABLE_INDEX0]);
        }
...
static int cviJpegEncWaitInterrupt(CVIJpgHandle jpgHandle, JpgRet *pRet)
{
    ...
    while (1)
    {
        ...
        if (-1 == int_reason)
        {
            ...
            {
                JLOG(ERR,
                     "info : jpu enc interruption timeout happened and is reset "
                     "automatically\n");
            }
#endif
...
{
#ifndef REENCODE_JPEG_SUPERFRAME
    pEncInfo->pFinalStream = malloc(pEncInfo->preStreamLen + *size);
...
static int cviJpgEncGetOneFrameData(CVIJpgHandle jpgHandle, void *data)
{
    ...
    if (pEncInfo->rgbPacked)
    {
        ...
        if ((0x01 == pStream[0x256]) && (0x02 == pStream[0x259]) && (0x03 == pStream[0x25C]) && (0x01 == pStream[0x264]) && (0x02 == pStream[0x266]) && (0x03 == pStream[0x268]))
        {
            pStream[0x256] = 0x52;
            pStream[0x259] = 0x47;
            pStream[0x25C] = 0x42;
...
int cviJpgEncGetInputDataBuf(CVIJpgHandle jpgHandle, void *data)
{
    ...
    if (NULL == pEncInfo->pFrame) return JPG_RET_WRONG_CALL_SEQUENCE;
    if (NULL == data) return JPG_RET_INVALID_PARAM;
    ...
    if (pEncInfo->chromaInterleave) cviFrameBuf->strideC *= 2;
...
int cviJpgEncEncodeUserData(CVIJpgHandle jpgHandle, void *data)
{
    ...
    if (pEncInfo->userDataLen + USER_DATA_MARKER_CODE_SIZE + USER_DATA_LENGTH_CODE_SIZE + pSrc->len > USER_DATA_BUF_SIZE)
    {
    ...
    *(pDst++) = (USER_DATA_LENGTH_CODE_SIZE + pSrc->len) & 0xFF;
    for (i = 0; i < pSrc->len; i++) *(pDst++) = pSrc->userData[i];

    pEncInfo->userDataLen +=
        (USER_DATA_MARKER_CODE_SIZE + USER_DATA_LENGTH_CODE_SIZE + pSrc->len);
    return JPG_RET_SUCCESS;
}
static void *pfnJpgWaitEncodeDone(void *param)
{
    ...
    while (1)
    {
        ...
        if (ret == JPG_RET_SUCCESS)
        {
            ...
            if (pEncInfo->bSbmEn)
            {
                SEM_POST(tSbmWaitQueue[pJpgInst->s32ChnNum]);
            }
        }
        else
        {
...
int cviJpgEncStart(CVIJpgHandle jpgHandle, void *data)
{
    ...
    if (pEncInfo->bIsoSendFrmEn && !pEncInfo->tPthreadId)
    {
        ...
        ret = pthread_create(&pEncInfo->tPthreadId, NULL, pfnJpgWaitEncodeDone,
                             (void *)jpgHandle);
        if (ret)
        {
