...
void JPU_RequireFlock(void)
{
    JpgEnterFlock();
}
// TODO this should be removed
void JPU_ReleaseFlock(void)
{
    JpgLeaveFlock();
}
...
JpgRet JPU_DecGetInitialInfo(JpgDecHandle handle, JpgDecInitialInfo *info)
{
    ...
    if (pDecInfo->roiEnable)
    {
        ...
        if ((pDecInfo->roiOffsetX > pDecInfo->alignedWidth) || (pDecInfo->roiOffsetY > pDecInfo->alignedHeight) || (pDecInfo->roiOffsetX + pDecInfo->roiWidth > pDecInfo->alignedWidth) || (pDecInfo->roiOffsetY + pDecInfo->roiHeight > pDecInfo->alignedHeight))
            return JPG_RET_INVALID_PARAM;
...
JpgRet JPU_DecStartOneFrame(JpgDecHandle handle, JpgDecParam *param)
{
    ...
    if (pDecInfo->frameBufPool == 0 && pDecInfo->dst_type != 3)
    {
    ...
    if (pDecInfo->errInfo.bError)
    {
        ...
        JpuWriteReg(
            MJPEG_PIC_CTRL_REG,
            pDecInfo->huffAcIdx << 10 | pDecInfo->huffDcIdx << 7 | pDecInfo->userHuffTab << 6 | (JPU_CHECK_WRITE_RESPONSE_BVALID_SIGNAL << 2) | 0);
        JpuWriteReg(MJPEG_MCU_INFO_REG,
                    pDecInfo->mcuBlockNum << 16 | pDecInfo->compNum << 12 | pDecInfo->compInfo[0] << 8 | pDecInfo->compInfo[1] << 4 | pDecInfo->compInfo[2]);
    ...
    JpuWriteReg(MJPEG_PIC_CTRL_REG,
                pDecInfo->huffAcIdx << 10 | pDecInfo->huffDcIdx << 7 | pDecInfo->userHuffTab << 6 | (JPU_CHECK_WRITE_RESPONSE_BVALID_SIGNAL << 2) | pDecInfo->usePartial);
    ...
    if (pDecInfo->packedFormat == PACKED_FORMAT_NONE)
        JpuWriteReg(MJPEG_DPB_CONFIG_REG,
                    (pDecInfo->frameEndian << 6) | (0 << 5) | (0 << 4) | ((pDecInfo->chromaInterleave == 0) ? 0 : (pDecInfo->chromaInterleave == 1) ? 2
                                                                                                                                                    : 3));
    else if (pDecInfo->packedFormat == PACKED_FORMAT_444)
        JpuWriteReg(MJPEG_DPB_CONFIG_REG,
                    (pDecInfo->frameEndian << 6) | (1 << 5) | (0 << 4) | (0 << 2) | ((pDecInfo->chromaInterleave == 0) ? 0 : (pDecInfo->chromaInterleave == 1) ? 2
                                                                                                                                                               : 3));
    else
        JpuWriteReg(MJPEG_DPB_CONFIG_REG,
                    (pDecInfo->frameEndian << 6) | (0 << 5) | (1 << 4) | ((pDecInfo->packedFormat - 1) << 2) | ((pDecInfo->chromaInterleave == 0) ? 0 : (pDecInfo->chromaInterleave == 1) ? 2
                                                                                                                                                                                          : 3));
    ...
    {
        pDecInfo->format =
            (pDecInfo->format == FORMAT_422) ? FORMAT_224 : (pDecInfo->format == FORMAT_224) ? FORMAT_422
                                                                                             : pDecInfo->format;
    }
    ...
    else
    {
        ...
        {
            JpuWriteReg(MJPEG_DPB_BASE00_REG, pDecInfo->dst_info.bufY);
            JpuWriteReg(MJPEG_DPB_BASE01_REG, pDecInfo->dst_info.bufCb);
            JpuWriteReg(MJPEG_DPB_BASE02_REG, pDecInfo->dst_info.bufCr);
        }
        else
        {
    ...
    if (pDecInfo->roiEnable)
    {
        ...
        JpuWriteReg(MJPEG_CLP_SIZE_REG,
                    (pDecInfo->roiMcuWidth * pDecInfo->mcuWidth) << 16 | (pDecInfo->roiMcuHeight * pDecInfo->mcuHeight)); // pixel Unit
    }
    else
    {
...
#ifdef MJPEG_ERROR_CONCEAL
    info->consumedByte =
        pDecInfo->gbuStartPtr + ((JpuReadReg(MJPEG_GBU_TT_CNT_REG)) >> 3);
#else
...
JpgRet JPU_EncOpen(JpgEncHandle *pHandle, JpgEncOpenParam *pop)
{
    ...
    *pHandle = pJpgInst;
    ...
    if (pop->packedFormat == PACKED_FORMAT_444 && pEncInfo->sourceFormat != FORMAT_444)
    {
        return JPG_RET_INVALID_PARAM;
    }
...
JpgRet JPU_EncStartOneFrame(JpgEncHandle handle, JpgEncParam *param)
{
    ...
    JpuWriteReg(MJPEG_PIC_CTRL_REG,
                0x18 | pEncInfo->usePartial | (JPU_CHECK_WRITE_RESPONSE_BVALID_SIGNAL << 2));
    JpuWriteReg(MJPEG_SCL_INFO_REG, 0);
    if (pEncInfo->packedFormat == PACKED_FORMAT_NONE)
        JpuWriteReg(MJPEG_DPB_CONFIG_REG,
                    (pEncInfo->frameEndian << 6) | (0 << 5) | (0 << 4) | (0 << 2) | ((pEncInfo->chromaInterleave == 0) ? 0 : (pEncInfo->chromaInterleave == 1) ? 2
                                                                                                                                                               : 3));
    else if (pEncInfo->packedFormat == PACKED_FORMAT_444)
        JpuWriteReg(MJPEG_DPB_CONFIG_REG,
                    (pEncInfo->frameEndian << 6) | (1 << 5) | (0 << 4) | (0 << 2) | ((pEncInfo->chromaInterleave == 0) ? 0 : (pEncInfo->chromaInterleave == 1) ? 2
                                                                                                                                                               : 3));
    else
        JpuWriteReg(MJPEG_DPB_CONFIG_REG,
                    (pEncInfo->frameEndian << 6) | (0 << 5) | (1 << 4) | ((pEncInfo->packedFormat - 1) << 2) | ((pEncInfo->chromaInterleave == 0) ? 0 : (pEncInfo->chromaInterleave == 1) ? 2
                                                                                                                                                                                          : 3));
    ...
    JpuWriteReg(MJPEG_MCU_INFO_REG,
                pEncInfo->mcuBlockNum << 16 | pEncInfo->compNum << 12 | pEncInfo->compInfo[0] << 8 | pEncInfo->compInfo[1] << 4 | pEncInfo->compInfo[2]);
...
#endif /* MJPEG_INTERFACE_API */
    info->bitstreamSize =
        jdi_get_memory_addr_high(JpuReadReg(MJPEG_BBC_WR_PTR_REG)) - pEncInfo->streamBufStartAddr;
