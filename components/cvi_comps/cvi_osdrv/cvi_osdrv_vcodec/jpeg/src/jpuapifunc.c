...
static int jpuproc_fd;

static pthread_mutex_t jpgLock = PTHREAD_MUTEX_INITIALIZER;
    ...
    if (enTableMode == JPG_TBL_NORMAL)
    {
        ...
        for (i = 0; i < 64; i += 4)
        {
            JpuWriteReg(MJPEG_GBU_PBIT_32_REG,
                        pEncInfo->pQMatTab[0][i] << 24 | pEncInfo->pQMatTab[0][i + 1] << 16 | pEncInfo->pQMatTab[0][i + 2] << 8 | pEncInfo->pQMatTab[0][i + 3]);
        }
        ...
        if (pEncInfo->sourceFormat != FORMAT_400)
        {
            ...
            for (i = 0; i < 64; i += 4)
            {
                JpuWriteReg(
                    ...
                    pEncInfo->pQMatTab[1][i] << 24 | pEncInfo->pQMatTab[1][i + 1] << 16 | pEncInfo->pQMatTab[1][i + 2] << 8 | pEncInfo->pQMatTab[1][i + 3]);
            }
        }
    }
    else
    { // if (enTableMode == JPG_TBL_MERGE)
        if (pEncInfo->sourceFormat != FORMAT_400)
...
static void JpgEncEncodeDHT(JpgEncInfo *pEncInfo, JpgTableMode enTableMode)
{
    ...
    if (enTableMode == JPG_TBL_NORMAL)
    {
        ...
        {
            JpuWriteReg(MJPEG_GBU_PBIT_32_REG,
                        pEncInfo->pHuffBits[0][i] << 24 | pEncInfo->pHuffBits[0][i + 1] << 16 | pEncInfo->pHuffBits[0][i + 2] << 8 | pEncInfo->pHuffBits[0][i + 3]);
        }

        for (i = 0; i < 12; i += 4)
        {
            JpuWriteReg(MJPEG_GBU_PBIT_32_REG,
                        pEncInfo->pHuffVal[0][i] << 24 | pEncInfo->pHuffVal[0][i + 1] << 16 | pEncInfo->pHuffVal[0][i + 2] << 8 | pEncInfo->pHuffVal[0][i + 3]);
        }
        ...
        {
            JpuWriteReg(MJPEG_GBU_PBIT_32_REG,
                        pEncInfo->pHuffBits[1][i] << 24 | pEncInfo->pHuffBits[1][i + 1] << 16 | pEncInfo->pHuffBits[1][i + 2] << 8 | pEncInfo->pHuffBits[1][i + 3]);
        }

        for (i = 0; i < 160; i += 4)
        {
            JpuWriteReg(MJPEG_GBU_PBIT_32_REG,
                        pEncInfo->pHuffVal[1][i] << 24 | pEncInfo->pHuffVal[1][i + 1] << 16 | pEncInfo->pHuffVal[1][i + 2] << 8 | pEncInfo->pHuffVal[1][i + 3]);
        }

        JpuWriteReg(MJPEG_GBU_PBIT_16_REG,
                    pEncInfo->pHuffVal[1][160] << 8 | pEncInfo->pHuffVal[1][161]);
        ...
        if (pEncInfo->sourceFormat != FORMAT_400)
        {
            ...
            for (i = 0; i < 16; i += 4)
            {
                JpuWriteReg(
                    MJPEG_GBU_PBIT_32_REG,
                    pEncInfo->pHuffBits[2][i] << 24 | pEncInfo->pHuffBits[2][i + 1] << 16 | pEncInfo->pHuffBits[2][i + 2] << 8 | pEncInfo->pHuffBits[2][i + 3]);
            }
            ...
            for (i = 0; i < 12; i += 4)
            {
                JpuWriteReg(
                    MJPEG_GBU_PBIT_32_REG,
                    pEncInfo->pHuffVal[2][i] << 24 | pEncInfo->pHuffVal[2][i + 1] << 16 | pEncInfo->pHuffVal[2][i + 2] << 8 | pEncInfo->pHuffVal[2][i + 3]);
            }
            ...
            for (i = 0; i < 16; i += 4)
            {
                JpuWriteReg(
                    MJPEG_GBU_PBIT_32_REG,
                    pEncInfo->pHuffBits[3][i] << 24 | pEncInfo->pHuffBits[3][i + 1] << 16 | pEncInfo->pHuffBits[3][i + 2] << 8 | pEncInfo->pHuffBits[3][i + 3]);
            }
            ...
            for (i = 0; i < 160; i += 4)
            {
                JpuWriteReg(
                    MJPEG_GBU_PBIT_32_REG,
                    pEncInfo->pHuffVal[3][i] << 24 | pEncInfo->pHuffVal[3][i + 1] << 16 | pEncInfo->pHuffVal[3][i + 2] << 8 | pEncInfo->pHuffVal[3][i + 3]);
            }

            JpuWriteReg(MJPEG_GBU_PBIT_16_REG,
                        pEncInfo->pHuffVal[3][160] << 8 | pEncInfo->pHuffVal[3][161]);
        }
    }
    else
    { // if (enTableMode == JPG_TBL_MERGE)
...
static void JpgEncEncodeSOF0(JpgEncInfo *pEncInfo)
{
    ...
    for (i = 0; i < pEncInfo->compNum; i++)
    {
        JpuWriteReg(MJPEG_GBU_PBIT_08_REG, (i + 1));
        JpuWriteReg(MJPEG_GBU_PBIT_08_REG,
                    ((pCInfoTab[i][1] << 4) & 0xF0) + (pCInfoTab[i][2] & 0x0F));
...
{
    JpuWriteReg(MJPEG_GBU_PBIT_32_REG, 0xFFE00010); // APP0 marker, length 16
...
static void JpgEncEncodeApp15UserData(JpgEncInfo *pEncInfo)
{
    ...
    if (pEncInfo->userDataLen > 0)
    {
        for (i = 0; i < (int)pEncInfo->userDataLen; ++i)
            JpuWriteReg(MJPEG_GBU_PBIT_08_REG, pEncInfo->userData[i]);
...
JpgRet InitJpgInstancePool(void)
{
    ...
    {
        CVI_JPG_DBG_ERR("JpgInst size = %d, MAX_INST_HANDLE_SIZE = %d\n",
...
JpgRet CheckJpgDecOpenParam(JpgDecOpenParam *pop)
{
    ...
    if (pop->chromaInterleave != CBCR_SEPARATED && pop->chromaInterleave != CBCR_INTERLEAVE && pop->chromaInterleave != CRCB_INTERLEAVE)
    {
        return JPG_RET_INVALID_PARAM;
    }
...
int JpgDecHuffTabSetUp(JpgDecInfo *jpg)
{
    ...
    for (j = 0; j < 16; j++)
    {
        ...
        v        = (v << 15) | (v << 14) | (v << 13) | (v << 12) | (v << 11) | (v << 10) | (v << 9) | (v << 8) | (v << 7) | (v << 6) | (v << 5) | (v << 4) | (v << 3) | (v << 2) | (v << 1) | (v);
    ...
    for (j = 0; j < 16; j++)
    {
        ...
        v        = (HuffData & 0x80) >> 7;
        v        = (v << 23) | (v << 22) | (v << 21) | (v << 20) | (v << 19) | (v << 18) | (v << 17) | (v << 16) | (v << 15) | (v << 14) | (v << 13) | (v << 12) | (v << 11) | (v << 10) | (v << 9) | (v << 8) | (v << 7) | (v << 6) | (v << 5) | (v << 4) | (v << 3) | (v << 2) | (v << 1) | (v);
        JpuWriteReg(MJPEG_HUFF_DATA_REG,
...
int JpegDecodeConcealError(JpgDecInfo *jpg)
{
    ...
    for (;;)
    {
        ...
        if (iRstIdx >= 0 && iRstIdx <= 7)
        {
            ...
            jpg->gbuStartPtr = ((ecsPtr - jpg->frameOffset) >> 8) << 8;
...
JpgRet CheckJpgEncOpenParam(JpgEncOpenParam *pop)
{
    ...
    if (pop->bitstreamBuffer % 8)
    {
        CVI_JPG_DBG_ERR("bitstreamBuffer %% 8, bitstreamBuffer = 0x%llx\n", pop->bitstreamBuffer);
        return JPG_RET_INVALID_PARAM;
    }

    if (pop->bitstreamBufferSize % 1024 || pop->bitstreamBufferSize < 1024 || pop->bitstreamBufferSize > MAX_BS_SIZE)
    {
...
int JpgEncGenHuffTab(JpgEncInfo *pEncInfo, int tabNum)
{
    ...
    int *huffsize = calloc(sizeof(int), 256);
    int *huffcode = calloc(sizeof(int), 256);
    int  code;
...
int JpgEncLoadQMatTab(JpgEncInfo *pEncInfo)
{
    ...
    for (comp = 0; comp < 3; comp++)
    {
        ...
        if (quantID >= 4)
            return 0;
        t = (comp == 0) ? Q_COMPONENT0 : (comp == 1) ? Q_COMPONENT1
                                                     : Q_COMPONENT2;
...
JpgRet JpgInitLock(void)
{
#ifndef USE_POSIX
    if (jpgLock == NULL)
        jpgLock = xSemaphoreCreateMutex();
...
JpgRet JpgDeinitLock(void)
{
#ifndef USE_POSIX
    if (jpgLock != NULL)
        vSemaphoreDelete(jpgLock);
...
{
    if (pthread_mutex_trylock(&jpgLock))
        return JPG_RET_FAILURE;
    return JPG_RET_SUCCESS;
}
...
JpgRet JpgEnterTimeLock(int timeout_ms)
{
    ...
    if (pthread_mutex_timedlock(&jpgLock, &tout))
        return JPG_RET_FAILURE;
...
// TODO this should be a process lock
JpgRet JpgEnterFlock(void)
{
    return JPG_RET_SUCCESS;
}

JpgRet JpgLeaveFlock(void)
{
    return JPG_RET_SUCCESS;
}
