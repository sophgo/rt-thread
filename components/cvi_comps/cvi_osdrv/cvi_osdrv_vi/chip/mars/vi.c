 ...
 * 0: disable
 */
static int csi_patgen_en[ISP_PRERAW_VIRT_MAX] = {0, 0};
...
// static struct rt_mempool vi_heap;
size_t    vi_block_size;
rt_smem_t vi_heap;
void     *vi_heap_memory;
...
void vi_wake_up(struct vi_thread_attr *event)
{
    ...
    rt_event_send(&event->event, 0x01);
}
...
static CVI_VOID vi_gdc_callback(CVI_VOID *pParam, VB_BLK blk)
{
    ...
    if (blk != VB_INVALID_HANDLE) vb_done_handler(_pParam->chn, CHN_TYPE_OUT, blk);
    free(pParam);
}
...
{
    uintptr_t ba;
    uint32_t  dmaid = ISP_BLK_ID_DMA_CTL10;
    uint32_t  seglen, stride, max_size;
    uint8_t   i     = 0;
    void     *vaddr = NULL;

    // TODO for mipi switch maybe need support different grip size
    if (ctx->is_rgbmap_sbm_on || ctx->isp_pipe_cfg[raw_num].is_mux) return;
    ...
    if (raw_num == ISP_PRERAW_A) {
        ...
        if (chn_num == ISP_FE_CH0) dmaid = ISP_BLK_ID_DMA_CTL20;
    }
    ...
    vi_pr(VI_DBG, "raw_num=%d, chn_num=%d, seglen=%d, stride=%d, max_size=%d\n", raw_num, chn_num,
          seglen, stride, max_size);

    if ((seglen != stride) && (max_size > 0)) {
        for (i = 0; i < RGBMAP_BUF_IDX; i++) {
            if (chn_num == ISP_FE_CH0)
                vaddr = (void *)isp_bufpool[raw_num].rgbmap_le[i];
            else if (chn_num == ISP_FE_CH1)
                vaddr = (void *)isp_bufpool[raw_num].rgbmap_se[i];
...
void _vi_yuv_dma_setup(struct isp_ctx *ctx, const enum cvi_isp_raw raw_num)
{
    ...
    u8 total_chn = (raw_num == ISP_PRERAW_A) ? ctx->rawb_chnstr_num : ctx->total_chn_num;
    u8 chn_str   = (raw_num == ISP_PRERAW_A) ? 0 : ctx->rawb_chnstr_num;

    for (; chn_str < total_chn; chn_str++) {
        // ToDo yuv sensor flow
        u32 dma = (chn_str == ctx->rawb_chnstr_num) ? ISP_BLK_ID_DMA_CTL12 : ISP_BLK_ID_DMA_CTL13;
        ...
        for (i = 0; i < OFFLINE_YUV_BUF_NUM; i++) {
            ...
            if (i == 0) ispblk_dma_setaddr(ctx, dma, b->addr);
...
static void _isp_preraw_be_dma_dump(struct isp_ctx *ictx, enum cvi_isp_raw raw_max)
{
    ...
    for (; raw < ISP_PRERAW_VIRT_MAX; raw++) {
        if (!ictx->isp_pipe_enable[raw]) continue;
...
static void _isp_rgbtop_dma_dump(struct isp_ctx *ictx, enum cvi_isp_raw raw_max)
{
    ...
    for (; raw < ISP_PRERAW_VIRT_MAX; raw++) {
        ...
        vi_pr(VI_INFO,
              "tdnr(0x%" PRIx64 ", 0x%" PRIx64 "), tdnr_rtile(0x%" PRIx64 ", 0x%" PRIx64 ")\n",
              isp_bufpool[raw].tdnr[0], isp_bufpool[raw].tdnr[1], isp_bufpool[raw].tdnr_rtile[0],
...
void _isp_preraw_fe_dma_setup(struct isp_ctx *ictx, enum cvi_isp_raw raw_num)
{
    ...
    if (_is_be_post_online(ictx) && raw_num < ISP_PRERAW_MAX) {  // fe->dram->be->post
        for (i = 0; i < OFFLINE_RAW_BUF_NUM; i++) {
            ...
            if (ictx->isp_pipe_cfg[raw_num].is_mux) {
                ...
                // disable frm ctrl use self size
                if (!ictx->isp_pipe_cfg[raw_num].switch_info.is_frm_ctrl) {
                    bufaddr = _mempool_get_addr();
                    bufsize = (i == 0) ? cur_size : vir_size;
                    _mempool_pop(bufsize);
                } else {
                    bufaddr = _mempool_get_addr();
                    bufsize = MAX(cur_size, vir_size);
                    _mempool_pop(bufsize);
                }
            } else
        ...
        ispblk_dma_config(ictx, raw_le, raw_num, isp_bufpool[hw_raw].bayer_le[0]);
        ...
        if (ictx->isp_pipe_cfg[raw_num].is_hdr_on) {
            ...
            for (i = 0; i < OFFLINE_RAW_BUF_NUM; i++) {
                if (ictx->isp_pipe_cfg[raw_num].is_mux) {
                    u32 cur_size  = ispblk_dma_buf_get_size(ictx, raw_le, raw_num);
                    u32 next_size = ispblk_dma_buf_get_size(ictx, raw_le, raw_num + ISP_PRERAW_MAX);

                    // disable frm ctrl use self size
                    if (!ictx->isp_pipe_cfg[raw_num].switch_info.is_frm_ctrl) {
                        bufsize = (i == 0) ? cur_size : next_size;
                        _mempool_pop(bufsize);
                    } else {
                        bufaddr = _mempool_get_addr();
                        bufsize = MAX(cur_size, next_size);
                        _mempool_pop(bufsize);
                    }
                } else
            ...
            ispblk_dma_config(ictx, raw_se, raw_num, isp_bufpool[hw_raw].bayer_se[0]);
        }
    }
    ...
    if (ictx->is_3dnr_on) {
        ...
        ispblk_rgbmap_dma_config(ictx, hw_raw, rgbmap_le);
        ispblk_dma_setaddr(ictx, rgbmap_le, isp_bufpool[hw_raw].rgbmap_le[0]);
        ...
        if (ictx->isp_pipe_cfg[raw_num].is_hdr_on) {
            ...
            ispblk_rgbmap_dma_config(ictx, hw_raw, rgbmap_se);
            ispblk_dma_setaddr(ictx, rgbmap_se, isp_bufpool[hw_raw].rgbmap_se[0]);
...
void _isp_preraw_be_dma_setup(struct isp_ctx *ictx, enum cvi_isp_raw raw_max)
{
    ...
    if (cfg_dma == false) goto EXIT;

    if (ictx->is_offline_be && !ictx->isp_pipe_cfg[ISP_PRERAW_A].is_offline_preraw) {
        // apply pre_fe_le buffer
        ispblk_dma_config(ictx, ISP_BLK_ID_DMA_CTL4, raw_num,
                          isp_bufpool[ISP_PRERAW_A].bayer_le[0]);

        if (ictx->is_hdr_on) {
            // apply pre_fe_se buffer
            ispblk_dma_config(ictx, ISP_BLK_ID_DMA_CTL5, raw_num,
                              isp_bufpool[ISP_PRERAW_A].bayer_se[0]);
        }
    }
    ...
    if (_is_fe_be_online(ictx)) {  // fe->be->dram->post
        if (ictx->is_slice_buf_on) {
            ...
            ispblk_dma_config(ictx, ISP_BLK_ID_DMA_CTL22, raw_num, bufaddr);
            bufsize = ispblk_dma_buf_get_size(ictx, ISP_BLK_ID_DMA_CTL22, raw_num);
            ...
            if (ictx->isp_pipe_cfg[ISP_PRERAW_A].is_hdr_on) {
                ...
                // ring buffer constraint. reg_base is 256 byte-align
                bufaddr = VI_256_ALIGN(bufaddr_tmp);
                ispblk_dma_config(ictx, ISP_BLK_ID_DMA_CTL23, raw_num, bufaddr);
                bufsize = ispblk_dma_buf_get_size(ictx, ISP_BLK_ID_DMA_CTL23, raw_num);
                _mempool_pop(bufsize + (u32)(bufaddr - bufaddr_tmp));
    ...
    for (; raw_num < ISP_PRERAW_VIRT_MAX; raw_num++) {
        // Be out dma
        if (!ictx->isp_pipe_enable[raw_num]) continue;
        if (ictx->isp_pipe_cfg[raw_num].is_yuv_bypass_path) continue;
...
void _isp_rawtop_dma_setup(struct isp_ctx *ictx, enum cvi_isp_raw raw_max)
{
    ...
    if (_is_fe_be_online(ictx)) {  // fe->be->dram->post
        ...
        ispblk_dma_config(ictx, ISP_BLK_ID_DMA_CTL28, raw_num,
                          isp_bufpool[ISP_PRERAW_A].prebe_le[0]);

        if (ictx->is_hdr_on) {
            // apply pre_be se buffer from DMA_CTL23
            ispblk_dma_config(ictx, ISP_BLK_ID_DMA_CTL29, raw_num,
                              isp_bufpool[ISP_PRERAW_A].prebe_se[0]);
        }
    }
...
void _isp_rgbtop_dma_setup(struct isp_ctx *ictx, enum cvi_isp_raw raw_max)
{
    ...
    for (raw_num = ISP_PRERAW_A; raw_num < ISP_PRERAW_VIRT_MAX; raw_num++) {
        ...
        // manr
        if (ictx->is_3dnr_on && ictx->isp_pipe_cfg[raw_num].is_3dnr_enable) {
            ...
            isp_bufpool[raw_num].sts_mem[0].mmap.size = isp_bufpool[raw_num].sts_mem[1].mmap.size =
                bufsize;
            ...
            if (_is_all_online(ictx)) {
                ...
                if (_is_fe_be_online(ictx) && ictx->is_rgbmap_sbm_on)
                    ispblk_dma_setaddr(ictx, ISP_BLK_ID_DMA_CTL32,
            ...
            if (ictx->isp_pipe_cfg[raw_num].is_hdr_on) {
                ...
                if (_is_fe_be_online(ictx) && ictx->is_rgbmap_sbm_on)
                    ispblk_dma_setaddr(ictx, ISP_BLK_ID_DMA_CTL33,
...
void _isp_yuvtop_dma_setup(struct isp_ctx *ictx, enum cvi_isp_raw raw_max)
{
    ...
    if (cfg_dma) {
        if (ictx->isp_pipe_cfg[ISP_PRERAW_A].is_offline_scaler ||
            (ictx->is_dual_sensor && ictx->isp_pipe_cfg[ISP_PRERAW_B].is_offline_scaler)) {
...
void _vi_dma_setup(struct isp_ctx *ictx, enum cvi_isp_raw raw_max)
{
    ...
    if (!ictx->isp_pipe_cfg[ISP_PRERAW_A].is_offline_preraw) {
        for (raw_num = ISP_PRERAW_A; raw_num < ISP_PRERAW_VIRT_MAX; raw_num++) {
            if (!ictx->isp_pipe_enable[raw_num]) continue;
...
{
    ispblk_dma_set_sw_mode(ctx, ISP_BLK_ID_DMA_CTL4, true);
    ispblk_dma_set_sw_mode(ctx, ISP_BLK_ID_DMA_CTL5, true);
    // ispblk_dma_set_sw_mode(ctx, ISP_BLK_ID_DMA_CTL10, false);
    // ispblk_dma_set_sw_mode(ctx, ISP_BLK_ID_DMA_CTL11, false);
    ispblk_rgbmap_dma_mode(ctx, ISP_BLK_ID_DMA_CTL10);
    // ispblk_rgbmap_dma_mode(ctx, ISP_BLK_ID_DMA_CTL11);
    // ispblk_dma_set_sw_mode(ctx, ISP_BLK_ID_DMA_CTL16, false);
    // ispblk_dma_set_sw_mode(ctx, ISP_BLK_ID_DMA_CTL17, false);
    ispblk_rgbmap_dma_mode(ctx, ISP_BLK_ID_DMA_CTL16);
    ispblk_rgbmap_dma_mode(ctx, ISP_BLK_ID_DMA_CTL17);
    // ispblk_dma_set_sw_mode(ctx, ISP_BLK_ID_DMA_CTL20, false);
    ispblk_rgbmap_dma_mode(ctx, ISP_BLK_ID_DMA_CTL20);
    ispblk_dma_set_sw_mode(ctx, ISP_BLK_ID_DMA_CTL22, false);
    ispblk_dma_set_sw_mode(ctx, ISP_BLK_ID_DMA_CTL23, false);
    ispblk_dma_set_sw_mode(ctx, ISP_BLK_ID_DMA_CTL30, false);
    // ispblk_dma_set_sw_mode(ctx, ISP_BLK_ID_DMA_CTL31, false);
    ispblk_dma_set_sw_mode(ctx, ISP_BLK_ID_DMA_CTL6, true);
    ispblk_dma_set_sw_mode(ctx, ISP_BLK_ID_DMA_CTL7, true);
#if defined(__CV180X__)
    ispblk_dma_set_sw_mode(ctx, ISP_BLK_ID_DMA_CTL8, true);
    ispblk_dma_set_sw_mode(ctx, ISP_BLK_ID_DMA_CTL9, true);
#else
    ispblk_dma_set_sw_mode(ctx, ISP_BLK_ID_DMA_CTL8, true);
    ispblk_dma_set_sw_mode(ctx, ISP_BLK_ID_DMA_CTL9, true);
    ispblk_dma_set_sw_mode(ctx, ISP_BLK_ID_DMA_CTL12, true);
    ispblk_dma_set_sw_mode(ctx, ISP_BLK_ID_DMA_CTL13, true);
#endif
    ...
    ispblk_dma_set_sw_mode(ctx, ISP_BLK_ID_DMA_CTL28, true);
    ispblk_dma_set_sw_mode(ctx, ISP_BLK_ID_DMA_CTL29, true);
    ispblk_dma_set_sw_mode(ctx, ISP_BLK_ID_DMA_CTL38, false);  // hist_v
    ispblk_dma_set_sw_mode(ctx, ISP_BLK_ID_DMA_CTL46, true);
    ispblk_dma_set_sw_mode(ctx, ISP_BLK_ID_DMA_CTL47, true);
    // ispblk_dma_set_sw_mode(ctx, ISP_BLK_ID_DMA_CTL32, false);
    // ispblk_dma_set_sw_mode(ctx, ISP_BLK_ID_DMA_CTL33, false);
    // ispblk_dma_set_sw_mode(ctx, ISP_BLK_ID_DMA_CTL34, false);
    // ispblk_dma_set_sw_mode(ctx, ISP_BLK_ID_DMA_CTL35, false);
    ...
    ispblk_dma_set_sw_mode(ctx, ISP_BLK_ID_DMA_CTL36, false);
    ispblk_dma_set_sw_mode(ctx, ISP_BLK_ID_DMA_CTL37, false);
    ispblk_dma_set_sw_mode(ctx, ISP_BLK_ID_DMA_CTL41, false);
    ispblk_dma_set_sw_mode(ctx, ISP_BLK_ID_DMA_CTL42, false);
    if (ctx->is_fbc_on) {
        ispblk_dma_set_sw_mode(ctx, ISP_BLK_ID_DMA_CTL43, true);
        ispblk_dma_set_sw_mode(ctx, ISP_BLK_ID_DMA_CTL44, true);
    } else {
        ispblk_dma_set_sw_mode(ctx, ISP_BLK_ID_DMA_CTL43, false);
        ispblk_dma_set_sw_mode(ctx, ISP_BLK_ID_DMA_CTL44, false);
    }
    ispblk_dma_set_sw_mode(ctx, ISP_BLK_ID_DMA_CTL45, true);   // dci
    ispblk_dma_set_sw_mode(ctx, ISP_BLK_ID_DMA_CTL39, false);  // LTM
    ispblk_dma_set_sw_mode(ctx, ISP_BLK_ID_DMA_CTL40, false);  // LTM
...
void _vi_pre_fe_get_dma_size(struct isp_ctx *ictx, enum cvi_isp_raw raw_num)
{
    ...
    if (!(_is_fe_be_online(ictx) && ictx->is_rgbmap_sbm_on)) {
        if (!_is_all_online(ictx)) {
            for (i = 1; i < RGBMAP_BUF_IDX; i++) _mempool_pop(bufsize);
        }
    }
...
void _vi_pre_be_get_dma_size(struct isp_ctx *ictx, enum cvi_isp_raw raw_max)
{
    ...
    for (; raw < ISP_PRERAW_VIRT_MAX; raw++) {
        ...
        if (ictx->isp_pipe_cfg[raw].is_yuv_bypass_path) continue;
...
void _vi_rgbtop_get_dma_size(struct isp_ctx *ictx, enum cvi_isp_raw raw_max)
{
    ...
    for (raw = ISP_PRERAW_A; raw < ISP_PRERAW_VIRT_MAX; raw++) {
        ...
        // manr
        if (ictx->is_3dnr_on && ictx->isp_pipe_cfg[raw].is_3dnr_enable) {
            uint64_t bufaddr     = 0;
            uint64_t tmp_bufaddr = 0;
...
void _vi_yuvtop_get_dma_size(struct isp_ctx *ictx, enum cvi_isp_raw raw_max)
{
    ...
    for (raw = ISP_PRERAW_A; raw < ISP_PRERAW_VIRT_MAX; raw++) {
        ...
        vi_pr(VI_INFO, "ldci bufsize: total(%d), used(%d), wasted_for_alignment(%d)\n",
              bufsize + (uint32_t)(bufaddr - tmp_bufaddr), bufsize,
...
static u32 _is_drop_next_frame(struct cvi_vi_dev *vdev, const enum cvi_isp_raw raw_num,
                               const enum cvi_isp_pre_chn_num chn_num)
{
    ...
    if (ctx->isp_pipe_cfg[raw_num].is_drop_next_frame) {
        // for tuning_dis and not mipi switch, shoudn't trigger preraw;
        if ((ctx->is_dual_sensor) && (!ctx->isp_pipe_cfg[raw_num].is_yuv_bypass_path)) {
        ...
        if (!ctx->isp_pipe_cfg[raw_num].is_frame_valid) return 1;
...
static void _set_drop_frm_info(const struct cvi_vi_dev *vdev, const enum cvi_isp_raw raw_num,
                               struct isp_i2c_data *i2c_data)
{
    ...
    vi_pr(VI_DBG, "raw_%d, drop_ref_frm_num=%d, drop frame=%d\n", raw_num,
          ctx->isp_pipe_cfg[raw_num].drop_ref_frm_num, i2c_data->drop_frame_cnt);
}

static void _clear_drop_frm_info(const struct cvi_vi_dev *vdev, const enum cvi_isp_raw raw_num)
{
    struct isp_ctx *ctx = (struct isp_ctx *)(&vdev->ctx);
...
static void _isp_crop_update_chk(struct cvi_vi_dev *vdev, const enum cvi_isp_raw raw_num,
                                 struct _isp_crop_node *node)
{
    ...
    if (node->n.dly_frm_num == 0) {
        ...
        vdev->ctx.isp_pipe_cfg[raw_num].csibdg_width  = node->n.wdr.img_size[0].width;
        vdev->ctx.isp_pipe_cfg[raw_num].csibdg_height = node->n.wdr.img_size[0].height;

        vi_pr(VI_DBG, "Preraw_%d, %d crop_x:y:w:h=%d:%d:%d:%d\n", raw_num, i,
              vdev->ctx.isp_pipe_cfg[raw_num].crop.x, vdev->ctx.isp_pipe_cfg[raw_num].crop.y,
              vdev->ctx.isp_pipe_cfg[raw_num].crop.w, vdev->ctx.isp_pipe_cfg[raw_num].crop.h);
...
static void _isp_crop_update(struct cvi_vi_dev *vdev, const enum cvi_isp_raw raw_num,
                             struct _isp_crop_node **_crop_n, const u16 _crop_num)
{
    struct _isp_crop_node *node;
    u16                    i = 0;
...
static void _snr_i2c_update(struct cvi_vi_dev *vdev, const enum cvi_isp_raw raw_num,
                            struct _isp_snr_i2c_node **_i2c_n, const u16 _i2c_num)
{
    struct isp_ctx           *ctx = (struct isp_ctx *)(&vdev->ctx);
    ...
    for (j = 0; j < _i2c_num; j++) {
        ...
        vi_pr(VI_DBG,
              "raw_num=%d, i2c_num=%d, j=%d, magic_num=%d, fe_frm_num=%d, drop=%d,dump=%d\n",
              raw_num, _i2c_num, j, node->n.magic_num, fe_frm_num, vdev->drop_frame_number[raw_num],
        ...
        if (((ctx->isp_pipe_cfg[raw_num].is_hdr_on && ctx->is_synthetic_hdr_on) &&
            ...
            if (node->n.magic_num != fe_frm_num) {
                vi_pr(VI_WARN, "raw_num_%d exception handle, send delayed i2c data.\n", raw_num);
            }
            ...
            for (i = 0; i < node->n.regs_num; i++) {
                ...
                vi_pr(VI_DBG, "i2c_data[%d]:addr=0x%x write:0x%x update:%d dly_frm:%d\n", i,
                      i2c_data->reg_addr, i2c_data->data, i2c_data->update, i2c_data->dly_frm_num);

                if (i2c_data->update && (i2c_data->dly_frm_num == 0)) {
                    vip_sys_cmm_cb_i2c(cmd, (void *)i2c_data);
                    i2c_data->update = 0;
                    if (burst_i2c_en) dev_mask |= BIT(i2c_data->i2c_dev);

                    if (i2c_data->drop_frame) _set_drop_frm_info(vdev, raw_num, i2c_data);
            ...
            if ((j + 1) < _i2c_num) {
                ...
                for (i = 0; i < next_node->n.regs_num; i++) {
                    ...
                    if (i2c_data->update && next_i2c_data->update == 0) {
                        ...
                        vi_pr(VI_WARN, "raw_%d exception handle, i2c node merge, addr:0x%x\n",
                              raw_num, i2c_data->reg_addr);
                    }
                }
...
static void _isp_snr_cfg_deq_and_fire(struct cvi_vi_dev *vdev, const enum cvi_isp_raw raw_num)
{
    ...
    {
        i2c_n[i2c_num] = clist_entry(pos, struct _isp_snr_i2c_node, list);
        i2c_num++;
    }
    ...
    {
        if (crop_num < i2c_num) {
            crop_n[crop_num] = clist_entry(pos, struct _isp_crop_node, list);
            crop_num++;
        }
    }
    ...
    if (i2c_num > 0) _snr_i2c_update(vdev, raw_num, i2c_n, i2c_num);
    if (crop_num > 0) _isp_crop_update(vdev, raw_num, crop_n, crop_num);
}

static inline void _vi_clear_mmap_fbc_ring_base(struct cvi_vi_dev     *vdev,
...
static void _usr_pic_timer_handler(unsigned long data)
{
	...
	if (rt_atomic_load(&vdev->pre_be_state[ISP_BE_CH0]) == ISP_PRE_BE_IDLE &&
		(rt_atomic_load(&vdev->isp_streamoff) == 0) && ctx->is_ctrl_inited) {
		...
		if (_is_fe_be_online(ctx) && ctx->isp_pipe_cfg[ISP_PRERAW_A].is_hdr_on) {
			...
			if (!b) {
				vi_pr(VI_WARN, "pre_be chn_num_%d outbuf is empty\n", ISP_FE_CH1);
				return;
			}
		...
		if (rt_atomic_load(&vdev->isp_raw_dump_en[raw_num]) == 1 && _is_fe_be_online(ctx)) {//raw_dump flow
			_isp_fe_be_raw_dump_cfg(vdev, raw_num, 0);
			rt_atomic_store(&vdev->isp_raw_dump_en[raw_num], 3);
		}
		...
		//	tasklet_hi_schedule(&vdev->vi_th[E_VI_TH_POSTRAW]);
	}
...
void vi_event_queue(struct cvi_vi_dev *vdev, const u32 type, const u32 frm_num, u8 lft_frm)
{
    unsigned long      flags;
    ...
    ev_k = rt_smem_alloc(vi_heap, vi_block_size);
    if (ev_k == NULL) {
        vi_pr(VI_ERR, "vi_event_queue rt_mp_alloc size(%lu) fail\n", sizeof(*ev_k));
        return;
    }
    ...
    ev_k->ev.lft_frm        = lft_frm;

    if (!list_empty(&event_q.list) && (event_q.num_rdy >= (VI_MAX_LIST_NUM - 1))) {
        rt_smem_free(ev_k);
...
void cvi_isp_dqbuf_list(struct cvi_vi_dev *vdev, const u32 frm_num, const u8 raw_num,
                        const u8 chn_id)
{
    unsigned long        flags;
    ...
    n = rt_smem_alloc(vi_heap, vi_block_size);
    if (n == NULL) {
        vi_pr(VI_ERR, "cvi_isp_dqbuf_list rt_mp_alloc size(%lu) fail\n", sizeof(*n));
        return;
    }
    n->raw_num   = raw_num;
    n->chn_id    = chn_id;
...
static void vi_init(void)
{
    ...
    for (i = 0; i < VI_MAX_CHN_NUM; ++i)
        for (j = 0; j < VI_MAX_EXTCHN_BIND_PER_CHN; ++j) gViCtx->chn_bind[i][j] = VI_INVALID_CHN;
}
...
static void _isp_yuv_bypass_buf_enq(struct cvi_vi_dev *vdev, const enum cvi_isp_raw raw_num,
                                    const u8 buf_chn)
{
    struct isp_ctx     *ictx = &vdev->ctx;
    struct cvi_isp_buf *b    = NULL;
    ...
    if (raw_num == ISP_PRERAW_A) {
        ...
        case 1:
            ...
            break;
        default:
            vi_pr(VI_ERR, "RAW_B Wrong chn_num(%d), rawb_chnstr_num(%d)\n", buf_chn,
...
static void _isp_yuv_bypass_trigger(struct cvi_vi_dev *vdev, const enum cvi_isp_raw raw_num,
                                    const u8 hw_chn_num)
{
    struct isp_ctx *ctx = &vdev->ctx;
    u8              buf_chn;

    if (rt_atomic_load(&vdev->isp_streamoff) == 0) {
        if (atomic_cmpxchg(&vdev->pre_fe_state[raw_num][hw_chn_num], ISP_PRERAW_IDLE,
                           ISP_PRERAW_RUNNING) == ISP_PRERAW_RUNNING) {
...
void _vi_pre_fe_ctrl_setup(enum cvi_isp_raw raw_num, struct cvi_vi_dev *vdev)
{
    ...
    if (ictx->isp_pipe_cfg[raw_num].is_yuv_bypass_path) {  // YUV sensor
        ...
        if (ictx->isp_pipe_cfg[raw_num].is_offline_scaler) {  // vi vpss offline mode
            u8 total_chn = (raw_num == ISP_PRERAW_A) ? ictx->rawb_chnstr_num : ictx->total_chn_num;
            u8 chn_str   = (raw_num == ISP_PRERAW_A) ? 0 : ictx->rawb_chnstr_num;

            for (; chn_str < total_chn; chn_str++) _isp_yuv_bypass_buf_enq(vdev, raw_num, chn_str);
        }
    } else {  // RGB sensor
        if (raw_num == ISP_PRERAW_A) {
...
void _vi_ctrl_init(enum cvi_isp_raw raw_num, struct cvi_vi_dev *vdev)
{
    ...
    if (ictx->is_ctrl_inited) return;

    vdev->snr_info[raw_num].snr_fmt.frm_num              = 1;
    vdev->snr_info[raw_num].snr_fmt.img_size[0].active_w = 1920;
    if (vdev->snr_info[raw_num].snr_fmt.img_size[0].active_w != 0) {  // MW config snr_info flow

        ictx->isp_pipe_cfg[raw_num].csibdg_width =
            vdev->snr_info[raw_num].snr_fmt.img_size[0].width;
        ictx->isp_pipe_cfg[raw_num].csibdg_height =
            vdev->snr_info[raw_num].snr_fmt.img_size[0].height;
        ...
        // test:set csi_bdg size
        ictx->isp_pipe_cfg[raw_num].csibdg_width  = 1920;
        ictx->isp_pipe_cfg[raw_num].csibdg_height = 1080;
        ictx->isp_pipe_cfg[raw_num].max_width     = 1920;
        ictx->isp_pipe_cfg[raw_num].max_height    = 1080;

        ictx->isp_pipe_cfg[raw_num].crop.w = 1920;
        ictx->isp_pipe_cfg[raw_num].crop.h = 1080;
        ictx->isp_pipe_cfg[raw_num].crop.x = 0;
        ictx->isp_pipe_cfg[raw_num].crop.y = 0;
    ...
    if (ictx->isp_pipe_cfg[raw_num].is_patgen_en) {
        ...
        vi_pr(VI_INFO, "csi_bdg=%d:%d, post_crop=%d:%d:%d:%d\n", vdev->usr_fmt.width,
              vdev->usr_fmt.height, vdev->usr_crop.width, vdev->usr_crop.height,
    ...
    for (raw = ISP_PRERAW_A; raw < ISP_PRERAW_VIRT_MAX; raw++) {
        if (!ictx->isp_pipe_enable[raw]) continue;
        if (!ictx->isp_pipe_cfg[raw].is_ctrl_inited) break;
    }

    if (raw == ISP_PRERAW_VIRT_MAX) ictx->is_ctrl_inited = true;
}
...
void _vi_scene_ctrl(struct cvi_vi_dev *vdev, enum cvi_isp_raw *raw_max)
{
    ...
    if (gViCtx->total_dev_num >= 2) {  // Multi sensor scenario
        ...
        for (raw_num = ISP_PRERAW_A; raw_num < ISP_PRERAW_VIRT_MAX; raw_num++) {
            ...
            if (ctx->isp_pipe_cfg[raw_num].is_3dnr_enable) count++;
        }

        if (count == 1) ctx->is_fbc_on = true;
        ...
        *raw_max             = ISP_PRERAW_B;
        ...
        if (ctx->is_synthetic_hdr_on || ctx->isp_pipe_cfg[raw_num].enPathMode == VI_PATH_FE_DRAM) {
            ctx->is_offline_be      = true;
            ctx->is_offline_postraw = false;
            ctx->is_slice_buf_on    = false;
            RGBMAP_BUF_IDX          = 3;
        } else if (ctx->isp_pipe_cfg[raw_num].enPathMode == VI_PATH_FE_BE_DRAM) {
            ctx->is_slice_buf_on  = false;
            ctx->is_rgbmap_sbm_on = false;
        }

        if (rgbmap_sbm_en && ctx->is_slice_buf_on) ctx->is_rgbmap_sbm_on = true;
    }
#endif
    ...
    for (raw_num = ISP_PRERAW_A; raw_num < ISP_PRERAW_VIRT_MAX; raw_num++) {
        ...
        ctx->chnstr_num[raw_num] = ctx->total_chn_num;
    ...
    vi_pr(VI_INFO, "Total_chn_num=%d, rawb_chnstr_num=%d enPath[%d]\n", ctx->total_chn_num,
          ctx->rawb_chnstr_num, ctx->isp_pipe_cfg[ISP_PRERAW_A].enPathMode);
}
...
static void _vi_suspend(struct cvi_vi_dev *vdev)
{
    ...
    if (pviProcCtx->vi_stt == VI_SUSPEND) {
        ...
        if (vdev->ctx.is_dual_sensor) isp_streaming(&vdev->ctx, false, ISP_PRERAW_B);
...
static void _set_init_state(struct cvi_vi_dev *vdev, const enum cvi_isp_raw raw_num,
                            const u8 chn_num)
{
    struct isp_ctx *ctx = &vdev->ctx;
    ...
    if (ctx->is_dual_sensor) {
        if (!ctx->isp_pipe_cfg[raw_num].is_yuv_bypass_path) {
            if (_is_fe_be_online(ctx) && ctx->is_slice_buf_on) {
                rt_atomic_store(&vdev->pre_fe_state[raw_num][chn_num], ISP_PRERAW_RUNNING);
            } else if (_is_fe_be_online(ctx) && !ctx->is_slice_buf_on)
                rt_atomic_store(&vdev->pre_be_state[chn_num], ISP_PRE_BE_RUNNING);
            else if (_is_be_post_online(ctx))
                rt_atomic_store(&vdev->pre_fe_state[raw_num][chn_num], ISP_PRERAW_RUNNING);
            else if (_is_all_online(ctx))
                rt_atomic_store(&vdev->postraw_state, ISP_POSTRAW_RUNNING);
        } else {
...
int vi_start_streaming(struct cvi_vi_dev *vdev)
{
    ...
    if (ctx->is_dual_sensor) {  // two sensor mode
        dev_num = ISP_PRERAW_B;
        _vi_call_cb(E_MODULE_CIF, CIF_CB_RESET_LVDS, &dev_num);
    }
    ...
    for (raw_num = ISP_PRERAW_A; raw_num < ISP_PRERAW_MAX; raw_num++) {
        ...
        if (ctx->isp_pipe_cfg[raw_num].is_patgen_en) {
            ...
            vi_pr(VI_WARN, "patgen enable, w_h(%d:%d), color mode(%d)\n", vdev->usr_fmt.width,
                  vdev->usr_fmt.height, vdev->usr_fmt.code);
        }
        ...
        if (!ctx->is_dual_sensor) {  // only single sensor maybe break
        ...
        if (!vdev->ctx.isp_pipe_cfg[raw_num].is_offline_preraw) {
            if (!vdev->ctx.isp_pipe_cfg[raw_num].is_yuv_bypass_path) {  // RGB sensor
                ...
                if (vdev->ctx.isp_pipe_cfg[raw_num].is_hdr_on && !vdev->ctx.is_synthetic_hdr_on) {
            ...
            // for muxdev, init switch gpio
            if (ctx->isp_pipe_cfg[raw_num].is_mux &&
                ctx->isp_pipe_cfg[raw_num].switch_info.gpio_idx != 255) {
                // // rc = csi_gpio_init(&ctx->isp_pipe_cfg[raw_num].switch_info.gpio,
                // // 	ctx->isp_pipe_cfg[raw_num].switch_info.gpio_idx);
                // // if (rc) {
                // // 	vi_pr(VI_ERR, "request for switch_gpio idx_%d failed:%d\n",
                // // 		ctx->isp_pipe_cfg[raw_num].switch_info.gpio_idx, rc);
                // // 	return 0;
                // // }
                // // rc = csi_gpio_dir(&ctx->isp_pipe_cfg[raw_num].switch_info.gpio,
                // // 	1 << ctx->isp_pipe_cfg[raw_num].switch_info.gpio_pin,
                // // 	GPIO_DIRECTION_OUTPUT);

                // // csi_gpio_write(&ctx->isp_pipe_cfg[raw_num].switch_info.gpio,
                // // 	1 << ctx->isp_pipe_cfg[raw_num].switch_info.gpio_pin,
                // // 	ctx->isp_pipe_cfg[raw_num].switch_info.gpio_pol);

                // cvi_pin_mode(ctx->isp_pipe_cfg[raw_num].switch_info.gpio_pin, 0);

                // cvi_pin_write(ctx->isp_pipe_cfg[raw_num].switch_info.gpio_pin,
                // ctx->isp_pipe_cfg[raw_num].switch_info.gpio_pol);

                vi_pr(VI_DBG, "raw_num_%d is_mux\n", raw_num);
            }
        }
    }
    ...
    vi_pr(VI_INFO, "ISP scene path, be_off=%d, post_off=%d, slice_buff_on=%d\n", ctx->is_offline_be,
          ctx->is_offline_postraw, ctx->is_slice_buf_on);
    ...
    if (_is_all_online(ctx)) {
        ...
        if (ctx->isp_pipe_cfg[raw_num].is_offline_scaler) {  // offline mode
            ...
            if (_vi_call_cb(E_MODULE_VPSS, VPSS_CB_VI_ONLINE_TRIGGER, &post_para) != 0) {
                ...
                rt_atomic_store(&vdev->ol_sc_frm_done, 0);
        ...
        if (ctx->isp_pipe_cfg[raw_num].is_offline_scaler) {  // offline mode
            ...
            rt_atomic_store(&vdev->pre_be_state[ISP_BE_CH0], ISP_PRE_BE_RUNNING);
        } else {  // online mode
            struct sc_cfg_cb post_para = {0};
    ...
    rt_atomic_store(&vdev->isp_streamoff, 0);
...
int vi_stop_streaming(struct cvi_vi_dev *vdev)
{
    ...
    rt_atomic_store(&vdev->isp_streamoff, 1);
    ...
    while (--count > 0) {
        if (rt_atomic_load(&vdev->postraw_state) == ISP_POSTRAW_IDLE &&
            rt_atomic_load(&vdev->pre_fe_state[ISP_PRERAW_A][ISP_FE_CH0]) == ISP_PRERAW_IDLE &&
            rt_atomic_load(&vdev->pre_fe_state[ISP_PRERAW_A][ISP_FE_CH1]) == ISP_PRERAW_IDLE &&
            rt_atomic_load(&vdev->pre_fe_state[ISP_PRERAW_B][ISP_FE_CH0]) == ISP_PRERAW_IDLE &&
            rt_atomic_load(&vdev->pre_fe_state[ISP_PRERAW_B][ISP_FE_CH1]) == ISP_PRERAW_IDLE &&
            rt_atomic_load(&vdev->pre_be_state[ISP_BE_CH0]) == ISP_PRERAW_IDLE &&
            rt_atomic_load(&vdev->pre_be_state[ISP_BE_CH1]) == ISP_PRERAW_IDLE)
            break;
    ...
    if (count == 0) {
        vi_pr(
            VI_ERR,
            "isp status fe_0(ch0:%d, ch1:%d) fe_1(ch0:%d, ch1:%d) be(ch0:%d, ch1:%d) postraw(%d)\n",
            rt_atomic_load(&vdev->pre_fe_state[ISP_PRERAW_A][ISP_FE_CH0]),
            rt_atomic_load(&vdev->pre_fe_state[ISP_PRERAW_A][ISP_FE_CH1]),
            rt_atomic_load(&vdev->pre_fe_state[ISP_PRERAW_B][ISP_FE_CH0]),
            rt_atomic_load(&vdev->pre_fe_state[ISP_PRERAW_B][ISP_FE_CH1]),
            rt_atomic_load(&vdev->pre_be_state[ISP_BE_CH0]),
            rt_atomic_load(&vdev->pre_be_state[ISP_BE_CH1]), rt_atomic_load(&vdev->postraw_state));
    }
    ...
    for (i = 0; i < ISP_PRERAW_VIRT_MAX; i++) {
        while ((isp_b = isp_buf_remove(&pre_out_queue[i])) != NULL) free(isp_b);
        while ((isp_b = isp_buf_remove(&pre_out_se_queue[i])) != NULL) free(isp_b);
    }

    for (i = 0; i < ISP_PRERAW_VIRT_MAX; i++) {
        while ((isp_b = isp_buf_remove(&raw_dump_b_dq[i])) != NULL) free(isp_b);
        while ((isp_b = isp_buf_remove(&raw_dump_b_se_dq[i])) != NULL) free(isp_b);
        while ((isp_b = isp_buf_remove(&raw_dump_b_q[i])) != NULL) free(isp_b);
        while ((isp_b = isp_buf_remove(&raw_dump_b_se_q[i])) != NULL) free(isp_b);
        ...
        while ((isp_b = isp_buf_remove(&pre_be_in_se_q[i])) != NULL) free(isp_b);
        while ((isp_b = isp_buf_remove(&pre_be_in_se_syn_q[i])) != NULL) free(isp_b);
    }

    while ((isp_b = isp_buf_remove(&pre_be_in_q)) != NULL) free(isp_b);
    while ((isp_b = isp_buf_remove(&pre_be_in_syn_q)) != NULL) free(isp_b);
    while ((isp_b = isp_buf_remove(&pre_be_out_q)) != NULL) free(isp_b);
    while ((isp_b = isp_buf_remove(&pre_be_out_se_q)) != NULL) free(isp_b);

    while ((isp_b = isp_buf_remove(&post_in_queue)) != NULL) free(isp_b);
    while ((isp_b = isp_buf_remove(&post_in_se_queue)) != NULL) free(isp_b);
    ...
    for (raw_num = ISP_PRERAW_A; raw_num < gViCtx->total_dev_num; raw_num++) {
        ...
        // 	vdev->ctx.isp_pipe_cfg[raw_num].switch_info.gpio_idx != 255) {
        // 	csi_gpio_uninit(&vdev->ctx.isp_pipe_cfg[raw_num].switch_info.gpio);
        // }
    }
...
static int _pre_be_outbuf_enque(struct cvi_vi_dev *vdev, const enum cvi_isp_raw raw_num,
                                const u8 hw_chn_num)
{
    struct isp_ctx *ctx = &vdev->ctx;
    ...
    if (!ctx->isp_pipe_cfg[raw_num].is_yuv_bypass_path) {  // RGB sensor
        ...
        enum ISP_BLK_ID_T pre_be_dma =
            (hw_chn_num == ISP_BE_CH0) ? ISP_BLK_ID_DMA_CTL22 : ISP_BLK_ID_DMA_CTL23;
        struct isp_buffer *b = NULL;
        ...
        // ToDo yuv sensor flow
        u32 dma = (buf_chn == ctx->rawb_chnstr_num) ? ISP_BLK_ID_DMA_CTL12 : ISP_BLK_ID_DMA_CTL13;
...
static int _pre_fe_outbuf_enque(struct cvi_vi_dev *vdev, const enum cvi_isp_raw raw_num,
                                const enum cvi_isp_pre_chn_num fe_chn_num)
{
    ...
    if (!ctx->isp_pipe_cfg[raw_num].is_yuv_bypass_path) {  // RGB sensor
        struct isp_queue *fe_out_q =
            (fe_chn_num == ISP_FE_CH0) ? &pre_out_queue[hw_raw] : &pre_out_se_queue[hw_raw];
        ...
        if (hw_raw == ISP_PRERAW_A) {
            if (ctx->isp_pipe_cfg[raw_num].is_hdr_on && ctx->is_synthetic_hdr_on)
                ...
                pre_fe_dma = (fe_chn_num == ISP_FE_CH0) ? ISP_BLK_ID_DMA_CTL6 : ISP_BLK_ID_DMA_CTL7;
        } else if (hw_raw == ISP_PRERAW_B) {
            if (ctx->isp_pipe_cfg[raw_num].is_hdr_on && ctx->is_synthetic_hdr_on)
                pre_fe_dma = ISP_BLK_ID_DMA_CTL12;
            else
                pre_fe_dma =
                    (fe_chn_num == ISP_FE_CH0) ? ISP_BLK_ID_DMA_CTL12 : ISP_BLK_ID_DMA_CTL13;
        } else {
            // if (ctx->isp_pipe_cfg[raw_num].is_hdr_on && ctx->is_synthetic_hdr_on)
            // 	pre_fe_dma = ISP_BLK_ID_DMA_CTL18;
            // else
            pre_fe_dma = (fe_chn_num == ISP_FE_CH0) ? ISP_BLK_ID_DMA_CTL18 : ISP_BLK_ID_DMA_CTL19;
        }

        if (rt_atomic_load(&vdev->isp_raw_dump_en[raw_num]) == 1) {  // raw_dump flow
            if (ctx->isp_pipe_cfg[raw_num].is_hdr_on) {
            ...
            if (trigger) {
                ...
                if (ctx->isp_pipe_cfg[raw_num].enDumpRaw != COMPRESS_MODE_DEFAULT)
                    ispblk_csibdg_update_dpcm(
                        ctx, raw_num, ctx->isp_pipe_cfg[raw_num].enDumpRaw == COMPRESS_MODE_TILE);
                rt_atomic_store(&vdev->isp_raw_dump_en[raw_num], 2);
            }
        } else if ((ctx->isp_pipe_cfg[raw_num].is_hdr_on && ctx->is_synthetic_hdr_on) &&
                   (rt_atomic_load(&vdev->isp_raw_dump_en[raw_num]) == 3)) {
            struct isp_buffer *b_se        = NULL;
            struct isp_queue  *fe_out_q_se = &raw_dump_b_se_q[raw_num];
        ...
        b = isp_next_buf(fe_out_q);
        if (!b) {
            vi_pr(VI_WARN, "pre_fe_%d buf_chn_num_%d outbuf is empty\n", raw_num, buf_chn);
            return 0;
        }
...
{
    struct isp_ctx    *ctx     = &vdev->ctx;
    struct isp_queue  *in_q    = ctx->is_hdr_on ? &pre_be_in_syn_q : &pre_be_in_q;
    struct isp_queue  *in_se_q = NULL;
    struct isp_buffer *b = NULL, *b_se = NULL;
    enum cvi_isp_raw   hw_raw, raw_num;
    bool               is_drop = false;

    for (raw_num = 0; raw_num < ISP_PRERAW_VIRT_MAX; raw_num++) {
        if (!ctx->isp_pipe_cfg[raw_num].is_drop_next_frame) {
            is_drop = true;
            break;
        }
    ...
    if (((tuning_dis[0] <= 0) && !is_drop) || _is_fe_be_online(ctx)) return;

retry:
    while ((b = isp_next_buf(in_q)) != NULL) {
        if (!((tuning_dis[0] > 0) && ((tuning_dis[0] - 1) != b->raw_num)) &&
            !ctx->isp_pipe_cfg[b->raw_num].is_drop_next_frame)
            break;
        ...
        vi_pr(VI_WARN, "rm raw_%d from be_in_q return preraw_%d\n", b->raw_num, hw_raw);

        if (ctx->isp_pipe_cfg[b->raw_num].is_hdr_on) {
            in_se_q = ctx->isp_pipe_cfg[b->raw_num].is_hdr_on ? &pre_be_in_se_syn_q[b->raw_num]
                                                              : &pre_be_in_se_q[b->raw_num];
            b_se    = isp_buf_remove(in_se_q);
            if (b_se == NULL) {
                vi_pr(VI_WARN, "rm raw_%d no se buffer\n", b->raw_num);
            } else {
                isp_buf_queue(&pre_out_se_queue[hw_raw], b_se);
                vi_pr(VI_WARN, "rm raw_%d from be_in_q_se return preraw_%d\n", b->raw_num, hw_raw);
            }
        }
    ...
    if (in_q == &pre_be_in_syn_q) {
        in_q = &pre_be_in_q;
        goto retry;
    }
}

static int _postraw_inbuf_enq_check(struct cvi_vi_dev *vdev, enum cvi_isp_raw *raw_num,
                                    enum cvi_isp_chn_num *chn_num)
{
    ...
    if (_is_fe_be_online(ctx)) {  // fe->be->dram->post
        ...
        in_q = ctx->is_hdr_on ? &pre_be_in_syn_q : &pre_be_in_q;
    }
...
retry:
    ...
    if (b == NULL) {
        if (_is_fe_be_online(ctx))  // fe->be->dram->post
            ...
            if (ctx->is_hdr_on && in_q == &pre_be_in_syn_q) {
...
static void _postraw_outbuf_enque(struct cvi_vi_dev *vdev, const enum cvi_isp_raw raw_num)
{
    ...
    if (b == NULL) {
        vi_pr(VI_DBG, "out_buffer empty\n");
        return;
    }

    vb2_buf = &b->buf;

    vi_pr(VI_DBG, "update isp-buf: 0x%llx-0x%llx\n", vb2_buf->planes[0].addr,
          vb2_buf->planes[1].addr);
...
/*workaround reduce fe_frm_num*/
static int buf_num_check(struct cvi_vi_dev *vdev, const enum cvi_isp_raw raw_num,
                         const enum cvi_isp_pre_chn_num chn_num)
{
    struct isp_ctx *ctx = &vdev->ctx;

    if (!ctx->isp_pipe_cfg[raw_num].is_hdr_on) return 1;
    ...
    if (_is_be_post_online(ctx)) {  // fe->dram->be->post
        if (abs((vdev->pre_fe_frm_num[raw_num][ISP_FE_CH0] -
            ...
            vi_pr(VI_ERR, "drop frm fe%d_chn0_frm_num = %d, fe%d_chn1_frm_num = %d\n", raw_num,
                  vdev->pre_fe_frm_num[raw_num][ISP_FE_CH0], raw_num,
                  vdev->pre_fe_frm_num[raw_num][ISP_FE_CH1]);
            return 0;
        }
        ...
        if (abs((vdev->pre_be_frm_num[raw_num][ISP_FE_CH0] -
            ...
            vi_pr(VI_ERR, "drop frm be%d_chn0_frm_num = %d, be%d_chn1_frm_num = %d\n", raw_num,
                  vdev->pre_fe_frm_num[raw_num][ISP_FE_CH0], raw_num,
                  vdev->pre_fe_frm_num[raw_num][ISP_FE_CH1]);
            return 0;
        }
 ...
 */
void _pre_hw_enque(struct cvi_vi_dev *vdev, const enum cvi_isp_raw raw_num, const u8 chn_num)
{
    struct isp_ctx  *ctx    = &vdev->ctx;
    enum cvi_isp_raw hw_raw = raw_num;
    ...
    if (rt_atomic_load(&vdev->isp_err_handle_flag) == 1) return;
    ...
    if (rt_atomic_load(&vdev->isp_streamoff) == 0) {
        ...
        if (_is_fe_be_online(ctx) && !ctx->is_slice_buf_on) {      // fe->be->dram->post
            if (!ctx->isp_pipe_cfg[raw_num].is_yuv_bypass_path) {  // RGB sensor
                if (atomic_cmpxchg(&vdev->pre_be_state[chn_num], ISP_PRE_BE_IDLE,
                                   ISP_PRE_BE_RUNNING) == ISP_PRE_BE_RUNNING) {
                    vi_pr(VI_DBG, "Pre_be chn_num_%d is running\n", chn_num);
                    return;
                }
            } else {  // YUV sensor
                if (atomic_cmpxchg(&vdev->pre_fe_state[raw_num][chn_num], ISP_PRERAW_IDLE,
                                   ISP_PRERAW_RUNNING) == ISP_PRERAW_RUNNING) {
            ...
            if (_pre_be_outbuf_enque(vdev, raw_num, chn_num)) {
                ...
                if (!ctx->isp_pipe_cfg[raw_num].is_yuv_bypass_path)  // RGB sensor
                    rt_atomic_store(&vdev->pre_be_state[chn_num], ISP_PRE_BE_IDLE);
                else  // YUV sensor
                    rt_atomic_store(&vdev->pre_fe_state[raw_num][chn_num], ISP_PRERAW_IDLE);
            }
        } else if (_is_be_post_online(ctx)) {  // fe->dram->be->post
            if (ctx->isp_pipe_cfg[ISP_PRERAW_A].is_offline_preraw) {
                if (rt_atomic_load(&vdev->isp_streamon) == 0) {
                ...
                if (ctx->isp_pipe_cfg[raw_num].is_offline_scaler) {  // Scaler onffline mode
                    if (_postraw_outbuf_empty(vdev, raw_num)) {
                        rt_atomic_store(&vdev->postraw_state, ISP_POSTRAW_IDLE);
                        return;
                    }
                ...
                if (atomic_cmpxchg(&vdev->pre_fe_state[hw_raw][chn_num], ISP_PRERAW_IDLE,
                                   ISP_PRERAW_RUNNING) == ISP_PRERAW_RUNNING) {
                    vi_pr(VI_DBG, "Pre_fe_%d chn_num_%d is running\n", hw_raw, chn_num);
                    return;
                }
                ...
                if (_pre_fe_outbuf_enque(vdev, raw_num, chn_num))
                    isp_pre_trig(ctx, hw_raw, chn_num);
                else
                    rt_atomic_store(&vdev->pre_fe_state[hw_raw][chn_num], ISP_PRERAW_IDLE);
            }
        } else if (_is_all_online(ctx)) {
            if (atomic_cmpxchg(&vdev->postraw_state, ISP_POSTRAW_IDLE, ISP_POSTRAW_RUNNING) !=
                ISP_POSTRAW_IDLE) {
...
static inline void _post_rgbmap_update(struct isp_ctx *ctx, const enum cvi_isp_raw raw_num,
                                       const u32 frm_num)
{
...
static inline void _post_ldci_update(struct isp_ctx *ctx, const enum cvi_isp_raw raw_num)
{
    ...
    ispblk_dma_setaddr(ctx, ISP_BLK_ID_DMA_CTL48, ldci_dma);
}
...
static inline void _post_dma_update(struct cvi_vi_dev *vdev, const enum cvi_isp_raw raw_num)
{
    ...
    if (ctx->is_3dnr_on && ctx->isp_pipe_cfg[raw_num].is_3dnr_enable) {
        if (ctx->is_fbc_on) {
            // 3dnr y
...
static u32 _is_first_frm_after_drop(struct cvi_vi_dev *vdev, const enum cvi_isp_raw raw_num)
{
    struct isp_ctx *ctx                      = &vdev->ctx;
...
static inline void _post_ctrl_update(struct cvi_vi_dev *vdev, const enum cvi_isp_raw raw_num)
{
    ...
    if (ctx->isp_pipe_cfg[raw_num].is_3dnr_enable) {
        // Enable 3DNR dma
...
static uint8_t _pre_be_sts_in_use_chk(struct cvi_vi_dev *vdev, const enum cvi_isp_raw raw_num,
                                      const u8 chn_num)
{
    unsigned long flags;
    static u8     be_in_use;
...
static inline void _swap_pre_be_sts_buf(struct cvi_vi_dev *vdev, const enum cvi_isp_raw raw_num,
                                        const u8 chn_num)
{
    struct isp_ctx *ctx = &vdev->ctx;
    struct _membuf *pool;
		...
		if (en && !__clk_is_enabled(vdev->isp_clk[5])) {
			if (clk_enable(vdev->isp_clk[5])) {
				...
				if (_is_fe_be_online(ctx))
					...
					rt_atomic_store(&vdev->pre_be_state[ISP_BE_CH0], ISP_PRE_BE_IDLE);
					rt_atomic_store(&vdev->postraw_state, ISP_POSTRAW_IDLE);
				}
 ...
 */
static void _post_hw_enque(struct cvi_vi_dev *vdev)
{
    struct isp_ctx      *ctx     = &vdev->ctx;
    enum cvi_isp_raw     raw_num = ISP_PRERAW_A;
    enum cvi_isp_chn_num chn_num = ISP_CHN0;

    if (rt_atomic_load(&vdev->isp_streamoff) == 1) return;
    ...
    if (_is_fe_be_online(ctx) && !ctx->is_slice_buf_on) {  // fe->be->dram->post
        ...
        if (_isp_clk_dynamic_en(vdev, true) < 0) return;
    ...
    YUV_POSTRAW:
        ...
        if (rt_atomic_load(&vdev->postraw_state) == ISP_POSTRAW_RUNNING) {
            vi_pr(VI_DBG, "Postraw is running\n");
            return;
        }
        ...
        if (!ctx->isp_pipe_cfg[raw_num].is_offline_preraw) _pre_hw_enque(vdev, raw_num, ISP_FE_CH0);
    } else if (_is_fe_be_online(ctx) && ctx->is_slice_buf_on) {
        ...
        if (atomic_cmpxchg(&ctx->is_post_done, 1, 0) == 1) {     // Change is_post_done flag to 0
            if (ctx->isp_pipe_cfg[raw_num].is_offline_scaler) {  // Scaler offline mode
                if (!_postraw_outbuf_empty(vdev, raw_num)) _postraw_outbuf_enq(vdev, raw_num);
            }
            ...
            if (atomic_cmpxchg(&vdev->pre_be_state[ISP_BE_CH0], ISP_PRE_BE_IDLE,
                               ISP_PRE_BE_RUNNING) != ISP_PRE_BE_IDLE) {
                vi_pr(VI_WARN, "BE is running\n");
                return;
            }
            ...
            if (!ctx->isp_pipe_cfg[raw_num].is_offline_scaler) {  // Scaler online mode
                ...
                vi_fill_mlv_info(NULL, raw_num, &post_para.m_lv_i, false);
                if (_vi_call_cb(E_MODULE_VPSS, VPSS_CB_VI_ONLINE_TRIGGER, &post_para) != 0) {
                    vi_pr(VI_WARN, "snr_num_%d, SC is not ready\n", raw_num);
                    rt_atomic_store(&vdev->pre_be_state[ISP_BE_CH0], ISP_PRE_BE_IDLE);
                    return;
                }
            ...
            // slice buffer update be at be_done
            pre_be_tuning_update(ctx, raw_num);

            if (!ctx->isp_pipe_cfg[raw_num].is_offline_preraw) {
                _pre_hw_enque(vdev, raw_num, ISP_FE_CH0);
                if (ctx->isp_pipe_cfg[raw_num].is_hdr_on) _pre_hw_enque(vdev, raw_num, ISP_FE_CH1);
            }
        }
    }
}

static void _pre_fe_rgbmap_update(struct cvi_vi_dev *vdev, const enum cvi_isp_raw raw_num,
                                  const enum cvi_isp_pre_chn_num chn_num)
{
    struct isp_ctx  *ctx        = &vdev->ctx;
...
void vi_destory_thread(struct cvi_vi_dev *vdev, enum E_VI_TH th_id)
{
    ...
    if (vdev->vi_th[th_id].w_thread != 0) {
        int ret = 0;

        rt_atomic_store(&vdev->vi_th[th_id].exit_flag, 1);
        vi_wake_up(&vdev->vi_th[th_id]);
        if (!ret) {
            while (rt_atomic_load(&vdev->vi_th[th_id].thread_exit) == 0) {
                vi_pr(VI_WARN, "wait for %s exit\n", vdev->vi_th[th_id].th_name);
                usleep(10 * 1000);
            }
        }
        pthread_join(vdev->vi_th[th_id].w_thread, NULL);
        vdev->vi_th[th_id].w_thread = 0;

        rt_event_detach(&vdev->vi_th[th_id].event);
    }
}
...
int vi_create_thread(struct cvi_vi_dev *vdev, enum E_VI_TH th_id)
{
    ...
    if (vdev->vi_th[th_id].w_thread == 0) {
        ...
        case E_VI_TH_POSTRAW:
            memcpy(vdev->vi_th[th_id].th_name, "_isp_post_thread", sizeof("_isp_post_thread"));
            vdev->vi_th[th_id].th_handler = _isp_post_thread;
            break;
        default:
        ...
        snprintf(vdev->vi_th[th_id].event_name, sizeof(vdev->vi_th[th_id].event_name), "%s%s",
                 vdev->vi_th[th_id].th_name, "_event");
        rt_event_init(&vdev->vi_th[th_id].event, vdev->vi_th[th_id].event_name, 0);

        rc = pthread_create(&vdev->vi_th[th_id].w_thread, &attr, vdev->vi_th[th_id].th_handler,
                            (void *)vdev);
        ...
        rt_atomic_store(&vdev->vi_th[th_id].thread_exit, 0);
        rt_atomic_store(&vdev->vi_th[th_id].exit_flag, 0);
    }
...
// TODO rawreplay timer
    ...
    if (pviProcCtx->vi_stt != VI_SUSPEND)
        memset(vdev->snr_info, 0, sizeof(struct cvi_isp_snr_info) * ISP_PRERAW_VIRT_MAX);
    ...
    for (i = 0; i < ISP_PRERAW_VIRT_MAX; i++) {
        ...
        ctx->isp_pipe_cfg[i].is_frame_valid    = true;

        for (j = 0; j < ISP_FE_CHN_MAX; j++) {
            vdev->pre_fe_sof_cnt[i][j] = 0;
            vdev->pre_fe_frm_num[i][j] = 0;

            rt_atomic_store(&vdev->pre_fe_state[i][j], ISP_PRERAW_IDLE);
        }

        spin_lock_init(&snr_node_lock[i]);

        rt_atomic_store(&vdev->isp_raw_dump_en[i], 0);
        rt_atomic_store(&vdev->isp_smooth_raw_dump_en[i], 0);
    }
    ...
    rt_atomic_store(&vdev->pre_be_state[ISP_BE_CH0], ISP_PRERAW_IDLE);
    rt_atomic_store(&vdev->pre_be_state[ISP_BE_CH1], ISP_PRERAW_IDLE);
    rt_atomic_store(&vdev->postraw_state, ISP_POSTRAW_IDLE);
    rt_atomic_store(&vdev->isp_streamoff, 0);
    rt_atomic_store(&vdev->isp_err_handle_flag, 0);
    rt_atomic_store(&vdev->ol_sc_frm_done, 1);
    rt_atomic_store(&vdev->isp_dbg_flag, 0);
    rt_atomic_store(&vdev->ctx.is_post_done, 0);
    ...
    rt_event_init(&vdev->isp_event_wait_q.event, "isp_event_wait_q_event", 0);
    rt_event_init(&vdev->isp_dbg_wait_q.event, "isp_dbg_wait_q_event", 0);
// TODO need mw work
#if 0
...
static void _vi_init_param(struct cvi_vi_dev *vdev)
{
    ...
    rt_atomic_store(&dev_open_cnt, 0);
    ...
    for (i = 0; i < ISP_PRERAW_VIRT_MAX; i++) {
        vdev->isp_int_flag[i] = false;
        snprintf(vdev->isp_int_wait_q[i].event_name, sizeof(vdev->isp_int_wait_q[i].event_name),
                 "%s%s%d", "vdev->isp_int_wait_q", "_event", i);
        snprintf(vdev->dis_wait_q[i].event_name, sizeof(vdev->dis_wait_q[i].event_name), "%s%s%d",
                 "vdev->dis_wait_q", "_event", i);
        rt_event_init(&vdev->isp_int_wait_q[i].event, vdev->isp_int_wait_q[i].event_name, 0);
        rt_event_init(&vdev->dis_wait_q[i].event, vdev->dis_wait_q[i].event_name, 0);
    }

    // ToDo sync_task_ext
    for (i = 0; i < ISP_PRERAW_VIRT_MAX; i++) sync_task_init(i);

    rt_atomic_store(&vdev->isp_streamon, 0);
}
...
static int _vi_mempool_setup(void)
{
    ...
    size_t size_buf     = sizeof(struct cvi_isp_buf);
    size_t size_dqbuf   = sizeof(struct _isp_dqbuf_n);
    size_t size_event   = sizeof(struct vi_event_k);
    size_t size_buffer  = sizeof(struct isp_buffer);
    size_t size_snr_i2c = sizeof(struct _isp_snr_i2c_node);
    size_t size_raw_num = sizeof(struct _isp_raw_num_n);

    vi_block_size = size_buf;
    if (size_dqbuf > vi_block_size) vi_block_size = size_dqbuf;
    if (size_event > vi_block_size) vi_block_size = size_event;
    if (size_buffer > vi_block_size) vi_block_size = size_buffer;
    if (size_snr_i2c > vi_block_size) vi_block_size = size_snr_i2c;
    if (size_raw_num > vi_block_size) vi_block_size = size_raw_num;

    if (vi_block_size % 32 != 0) {
        vi_block_size += (32 - (vi_block_size % 32));
    }

    _vi_mempool_reset();
    ret = vi_tuning_buf_setup();

    vi_heap_memory = rt_malloc(vi_block_size * 100);

    vi_heap = rt_smem_init("vi_heap", vi_heap_memory, vi_block_size * 100);
...
void _vi_sdk_release(struct cvi_vi_dev *vdev)
{
    ...
    for (i = 0; i < VI_MAX_PIPE_NUM; i++) gViCtx->isPipeCreated[i] = false;

    for (i = 0; i < VI_MAX_DEV_NUM; i++) gViCtx->isDevEnable[i] = false;

    rt_event_detach(&vdev->isp_event_wait_q.event);
    rt_event_detach(&vdev->isp_dbg_wait_q.event);
}
...
static long _vi_s_ctrl(struct cvi_vi_dev *vdev, struct vi_ext_control *p)
{
    ...
    case VI_IOCTL_STS_PUT: {
        ...
        if (raw_num >= ISP_PRERAW_VIRT_MAX) break;
    ...
    case VI_IOCTL_USR_PIC_CFG: {
        ...
        if ((cfg.crop.width < 32) || (cfg.crop.width > 4096) || (cfg.crop.left > cfg.crop.width) ||
            (cfg.crop.top > cfg.crop.height)) {
            vi_pr(VI_ERR, "USR_PIC_CFG:(Invalid Param) w(%d) h(%d) x(%d) y(%d)", cfg.crop.width,
                  cfg.crop.height, cfg.crop.left, cfg.crop.top);
        } else {
    ...
    case VI_IOCTL_USR_PIC_ONOFF: {
        ...
        ctx->isp_pipe_cfg[ISP_PRERAW_A].is_offline_preraw = (vdev->isp_source == CVI_ISP_SOURCE_FE);
    ...
    case VI_IOCTL_USR_PIC_PUT: {
        if (ctx->isp_pipe_cfg[ISP_PRERAW_A].is_offline_preraw) {
            ...
            if (vdev->usr_pic_delay) usr_pic_timer_init(vdev);
...
// TODO rawreplay
    ...
    case VI_IOCTL_SET_SNR_CFG_NODE: {
        ...
        if (vdev->ctx.isp_pipe_cfg[ISP_PRERAW_A].is_offline_preraw) break;
        ...
        vi_pr(VI_DBG, "raw_num=%d, magic_num=%d, regs_num=%d, i2c_update=%d, isp_update=%d\n",
              raw_num, snr_update->snr_cfg_node.snsr.magic_num,
              snr_update->snr_cfg_node.snsr.regs_num, snr_update->snr_cfg_node.snsr.need_update,
    ...
    case VI_IOCTL_SET_SNR_INFO: {
        ...
        if (snr_info.raw_num == ISP_PRERAW_B) viproc_en[1] = 1;

        vi_pr(VI_INFO, "raw_num=%d, color_mode=%d, frm_num=%d, snr_w:h=%d:%d, active_w:h=%d:%d\n",
              snr_info.raw_num, vdev->snr_info[snr_info.raw_num].color_mode,
    ...
    case VI_IOCTL_SET_PROC_CONTENT: {
        ...
        if (proc_cfg.buffer_size == 0) break;
// TODO VI_IOCTL_SET_PROC_CONTENT
    ...
    case VI_IOCTL_ENQ_BUF: {
        ...
        qbuf = calloc(sizeof(struct cvi_isp_buf), 1);
        if (qbuf == NULL) {
        ...
        if (_is_all_online(ctx) && cvi_isp_rdy_buf_empty(vdev, ISP_PRERAW_A) &&
    ...
    case VI_IOCTL_SET_DMA_BUF_INFO: {
        ...
        if ((info.size == 0) || (info.paddr == 0)) break;
    ...
    case VI_IOCTL_START_STREAMING: {
        ...
        rt_atomic_store(&vdev->isp_streamon, 1);
...
static long _vi_g_ctrl(struct cvi_vi_dev *vdev, struct vi_ext_control *p)
{
...
#endif
        memcpy(p->ptr, isp_bufpool[raw_num].sts_mem, sizeof(struct cvi_isp_sts_mem) * 2);
    ...
    case VI_IOCTL_GET_SCENE_INFO: {
        ...
        if (ctx->isp_pipe_cfg[ISP_PRERAW_A].is_mux) {
            info = FE_OFF_BE_ON_POST_SWITCH;
        }
        p->value = info;
    ...
    case VI_IOCTL_GET_BUF_SIZE: {
        ...
        for (raw_num = ISP_PRERAW_A; raw_num < ISP_PRERAW_VIRT_MAX; raw_num++) {
            if (!vdev->ctx.isp_pipe_enable[raw_num]) continue;
    ...
    case VI_IOCTL_CHECK_ISP_EVENT: {
        if (!list_empty(&event_q.list)) rc = 0;
        break;
    }
...
int vi_open(void)
{
    ...
    if (!rt_atomic_load(&dev_open_cnt)) {
// TODO clk relate
#if 0
...
unsigned int vi_poll(struct file *file, struct poll_table_struct *wait)
{
	...
	if (req_events & POLLIN) {
		if (rt_atomic_load(&vdev->isp_dbg_flag)) {
...
int vi_cb(void *dev, enum ENUM_MODULES_ID caller, u32 cmd, void *arg)
{
    ...
    case VI_CB_SET_VIVPSSMODE: {
        ...
        if (vi_online) {
            ...
            vi_pr(VI_DBG,
                  "Caller_Mod(%d) set vi_online:%d, is_offline_postraw=%d, is_offline_be=%d\n",
...
// TODO dbg_thread
#if 0
	rt_atomic_store(&vdev->isp_dbg_flag, 1);
...
void *_vi_event_handler_thread(void *arg)
{
    ...
    MMF_CHN_S    src_chn = {.enModId = CVI_ID_VI, .s32DevId = 0, .s32ChnId = 0};
    MMF_CHN_S    dst_chn = {.enModId = CVI_ID_VI, .s32DevId = 0, .s32ChnId = 0};
#ifdef VI_PROFILE
    ...
    while (1) {
#ifdef VI_PROFILE
        _vi_update_chnRealFrameRate(&gViCtx->chnStatus[dst_chn.s32ChnId]);
        clock_gettime(CLOCK_MONOTONIC, &time[0]);
#endif
        ret = rt_event_recv(&vdev->vi_th[th_id].event, 0x01, RT_EVENT_FLAG_OR | RT_EVENT_FLAG_CLEAR,
                            1000, &actl_flags);
        if (vdev->vi_th[th_id].flag == 0) continue;

        flag                    = vdev->vi_th[th_id].flag;
        vdev->vi_th[th_id].flag = 0;

        if (rt_atomic_load(&vdev->vi_th[th_id].exit_flag) == 1) {
            vi_pr(VI_DBG, "%s exit\n", vdev->vi_th[th_id].th_name);
            rt_atomic_store(&vdev->vi_th[th_id].thread_exit, 1);
            pthread_exit(NULL);
        }
        ...
        if (ret != 0) {
            ...
            src_chn.s32ChnId = b.chnId;
            dst_chn.s32ChnId = (vdev->ctx.isp_pipe_cfg[b.raw_num].switch_info.is_comm_chn &&
                                b.raw_num > ISP_PRERAW_MAX - 1)
                                   ? vdev->ctx.chnstr_num[b.raw_num]
                                   : src_chn.s32ChnId;

            ret2 = vb_dqbuf(src_chn, CHN_TYPE_OUT, &blk);
            if (ret2 != CVI_SUCCESS) {
            ...
            ((struct vb_s *)blk)->buf.dev_num = dst_chn.s32ChnId;
            ...
            gViCtx->chnStatus[dst_chn.s32ChnId].u32IntCnt++;
            gViCtx->chnStatus[dst_chn.s32ChnId].u32FrameNum++;
            gViCtx->chnStatus[dst_chn.s32ChnId].u32RecvPic = b.sequence;

            vi_pr(VI_DBG, "dqbuf chn_id=%d, frm_num=%d\n", dst_chn.s32ChnId, b.sequence);

            if (gViCtx->bypass_frm[dst_chn.s32ChnId] >= b.sequence) {
                // Release buffer if bypass_frm is not zero
                vb_release_block(blk);
                goto QBUF;
            }

            if (!gViCtx->pipeAttr[dst_chn.s32ChnId].bYuvBypassPath) {
                vi_fill_mlv_info((struct vb_s *)blk, 0, NULL, true);
                vi_fill_dis_info(vdev, (struct vb_s *)blk);
            }
            ...
            struct cvi_gdc_mesh *pmesh = &g_vi_mesh[dst_chn.s32ChnId];
            struct vb_s         *vb    = (struct vb_s *)blk;

            if (pthread_mutex_trylock(&pmesh->lock) == 0) {
                if (gViCtx->stLDCAttr[dst_chn.s32ChnId].bEnable) {
                    struct _vi_gdc_cb_param cb_param = {.chn = dst_chn, .usage = GDC_USAGE_LDC};

                    if (_mesh_gdc_do_op_cb(
                            GDC_USAGE_LDC, &gViCtx->stLDCAttr[dst_chn.s32ChnId].stAttr, vb,
                            gViCtx->chnAttr[dst_chn.s32ChnId].enPixelFormat, pmesh->paddr,
                            CVI_FALSE, &cb_param, sizeof(cb_param), CVI_ID_VI,
                            gViCtx->enRotation[dst_chn.s32ChnId]) != CVI_SUCCESS) {
                        pthread_mutex_unlock(&pmesh->lock);
                        vi_pr(VI_ERR, "gdc LDC failed.\n");
                    }
                    goto QBUF;
                } else if (gViCtx->enRotation[dst_chn.s32ChnId] != ROTATION_0) {
                    struct _vi_gdc_cb_param cb_param = {.chn   = dst_chn,
                                                        .usage = GDC_USAGE_ROTATION};
                    if (_mesh_gdc_do_op_cb(GDC_USAGE_ROTATION, NULL, vb,
                                           gViCtx->chnAttr[dst_chn.s32ChnId].enPixelFormat,
                                           pmesh->paddr, CVI_FALSE, &cb_param, sizeof(cb_param),
                                           CVI_ID_VI,
                                           gViCtx->enRotation[dst_chn.s32ChnId]) != CVI_SUCCESS) {
                        pthread_mutex_unlock(&pmesh->lock);
            ...
            vb_done_handler(dst_chn, CHN_TYPE_OUT, blk);
        QBUF:
            // get another vb for next frame
            if (vi_sdk_qbuf(src_chn) != CVI_SUCCESS)
                vb_acquire_block(vi_sdk_qbuf, src_chn, gViCtx->blk_size[src_chn.s32ChnId],
                                 gViCtx->chnAttr[dst_chn.s32ChnId].u32BindVbPool);
        }
#ifdef VI_PROFILE
        ...
        if (++count == 100) {
            vi_pr(VI_DBG, "VI duration(ms): average(%d), max(%d) min(%d)\n", sum / count / 1000,
                  duration_max / 1000, duration_min / 1000);
            count = 0;
            sum = duration_max = 0;
...
static void _vi_err_retrig_preraw(struct cvi_vi_dev *vdev, const enum cvi_isp_raw err_raw_num)
{
    ...
    for (wait_fe = ISP_PRERAW_A; wait_fe < ISP_PRERAW_MAX; wait_fe++) {
        if (!ctx->isp_pipe_enable[wait_fe]) continue;
        ...
        if (ctx->isp_pipe_cfg[wait_fe].is_yuv_bypass_path &&
            ...
            for (fe_chn = ISP_FE_CH0; fe_chn < fe_max; fe_chn++) {
                u8 buf_chn =
                    (wait_fe == ISP_PRERAW_A) ? fe_chn : vdev->ctx.rawb_chnstr_num + fe_chn;

                // if yuv sensor err need push, that's right qbuf num.
                if (wait_fe == err_raw_num) vdev->qbuf_num[buf_chn]++;
        ...
        } else {  // rgb sensor or yuv sensor online2sc
            spin_lock_irqsave(&stream_lock, flags);
...
static void _vi_record_debug_info(struct isp_ctx *ctx)
{
    ...
    if (gOverflowInfo != NULL) return;

    gOverflowInfo = rt_smem_alloc(vi_heap, sizeof(struct cvi_overflow_info));
    if (gOverflowInfo == NULL) {
        vi_pr(VI_ERR, "pre_raw_num_q rt_mp_alloc size(%lu) fail\n", sizeof(*gOverflowInfo));
        return;
    }
...
static void _vi_show_debug_info(void)
{
    ...
    if (vc_info->enable) {
        ...
        vi_pr(VI_ERR, "vc reg_00=0x%x, reg_08=0x%x, reg_88=0x%x, reg_90=0x%x, reg_94=0x%x\n",
              vc_info->reg_00, vc_info->reg_08, vc_info->reg_88, vc_info->reg_90, vc_info->reg_94);
    }

    if (vpss_info->enable) {
        vpss_info->enable = false;
        vi_pr(VI_ERR, "sc_top         valid     ready\n");
        vi_pr(VI_ERR, "isp2ip_y in       %d        %d\n", vpss_info->sc_top.isp2ip_y_in[0],
              vpss_info->sc_top.isp2ip_y_in[1]);
        vi_pr(VI_ERR, "isp2ip_u in       %d        %d\n", vpss_info->sc_top.isp2ip_u_in[0],
              vpss_info->sc_top.isp2ip_u_in[1]);
        vi_pr(VI_ERR, "isp2ip_v in       %d        %d\n", vpss_info->sc_top.isp2ip_v_in[0],
              vpss_info->sc_top.isp2ip_v_in[1]);
        vi_pr(VI_ERR, "img_d out         %d        %d\n", vpss_info->sc_top.img_d_out[0],
              vpss_info->sc_top.img_d_out[1]);
        vi_pr(VI_ERR, "img_v out         %d        %d\n", vpss_info->sc_top.img_d_out[0],
              vpss_info->sc_top.img_d_out[1]);
        vi_pr(VI_ERR, "bld_sa(img_v)     %d        %d\n", vpss_info->sc_top.bld_sa[0],
              vpss_info->sc_top.bld_sa[1]);
        vi_pr(VI_ERR, "bld_sb(img_d)     %d        %d\n", vpss_info->sc_top.bld_sb[0],
              vpss_info->sc_top.bld_sb[1]);
        vi_pr(VI_ERR, "bld_m             %d        %d\n", vpss_info->sc_top.bld_m[0],
              vpss_info->sc_top.bld_m[1]);
        vi_pr(VI_ERR, "pri_sp            %d        %d\n", vpss_info->sc_top.pri_sp[0],
              vpss_info->sc_top.pri_sp[1]);
        vi_pr(VI_ERR, "pri_m             %d        %d\n", vpss_info->sc_top.pri_m[0],
              vpss_info->sc_top.pri_m[1]);
        vi_pr(VI_ERR, "sc_d              %d        %d\n", vpss_info->sc_top.sc_d[0],
              vpss_info->sc_top.sc_d[1]);
        vi_pr(VI_ERR, "sc_v1             %d        %d\n", vpss_info->sc_top.sc_v1[0],
              vpss_info->sc_top.sc_v1[1]);
        vi_pr(VI_ERR, "sc_v2             %d        %d\n", vpss_info->sc_top.sc_v2[0],
              vpss_info->sc_top.sc_v2[1]);
        vi_pr(VI_ERR, "sc_v3             %d        %d\n", vpss_info->sc_top.sc_v3[0],
              vpss_info->sc_top.sc_v3[1]);
        vi_pr(VI_ERR, "sc_d_out          %d        %d\n", vpss_info->sc_top.sc_d_out[0],
              vpss_info->sc_top.sc_d_out[1]);

        vi_pr(VI_ERR, "sc(%d) odma\n", vpss_info->sc);
        vi_pr(VI_ERR, "    sc_odma_axi_cmd_cs [0]=%d, [1]=%d, [2]=%d, [3]=%d\n",
        ...
        vi_pr(VI_ERR, "    sc_odma_v_buf_empty=%d, sc_odma_v_buf_full=%d\n",
              vpss_info->odma.sc_odma_v_buf_empty, vpss_info->odma.sc_odma_v_buf_full);
        vi_pr(VI_ERR, "    sc_odma_u_buf_empty=%d, sc_odma_u_buf_full=%d\n",
              vpss_info->odma.sc_odma_u_buf_empty, vpss_info->odma.sc_odma_u_buf_full);
        vi_pr(VI_ERR, "    sc_odma_y_buf_empty=%d, sc_odma_y_buf_full=%d\n",
              vpss_info->odma.sc_odma_y_buf_empty, vpss_info->odma.sc_odma_y_buf_full);
        vi_pr(VI_ERR, "    sc_odma_axi_v_active=%d, sc_odma_axi_u_active=%d\n",
              vpss_info->odma.sc_odma_axi_v_active, vpss_info->odma.sc_odma_axi_u_active);
        vi_pr(VI_ERR, "    sc_odma_axi_y_active=%d, sc_odma_axi_active=%d\n",
              vpss_info->odma.sc_odma_axi_y_active, vpss_info->odma.sc_odma_axi_active);
        vi_pr(VI_ERR, "    reg_v_sb_empty=%d, reg_v_sb_full=%d\n", vpss_info->odma.reg_v_sb_empty,
              vpss_info->odma.reg_v_sb_full);
        vi_pr(VI_ERR, "    reg_u_sb_empty=%d, reg_u_sb_full=%d\n", vpss_info->odma.reg_u_sb_empty,
              vpss_info->odma.reg_u_sb_full);
        vi_pr(VI_ERR, "    reg_y_sb_empty=%d, reg_y_sb_full=%d\n", vpss_info->odma.reg_y_sb_empty,
              vpss_info->odma.reg_y_sb_full);
        vi_pr(VI_ERR, "    reg_sb_full=%d\n", vpss_info->odma.reg_sb_full);
        vi_pr(VI_ERR, "    sb_mode=%d, sb_size=%d, sb_nb=%d, sb_full_nb=%d, sb_sw_wptr=%d\n",
              vpss_info->sb_ctrl.sb_mode, vpss_info->sb_ctrl.sb_size, vpss_info->sb_ctrl.sb_nb,
              vpss_info->sb_ctrl.sb_full_nb, vpss_info->sb_ctrl.sb_sw_wptr);
        vi_pr(VI_ERR, "    u_sb_wptr_ro=%d, u_sb_full=%d, u_sb_empty=%d, u_sb_dptr_ro=%d\n",
              vpss_info->sb_stat.u_sb_wptr_ro, vpss_info->sb_stat.u_sb_full,
              vpss_info->sb_stat.u_sb_empty, vpss_info->sb_stat.u_sb_dptr_ro);
        vi_pr(VI_ERR, "    v_sb_wptr_ro=%d, v_sb_full=%d, v_sb_empty=%d, v_sb_dptr_ro=%d\n",
              vpss_info->sb_stat.v_sb_wptr_ro, vpss_info->sb_stat.v_sb_full,
              vpss_info->sb_stat.v_sb_empty, vpss_info->sb_stat.v_sb_dptr_ro);
        vi_pr(VI_ERR,
              "    y_sb_wptr_ro=%d, y_sb_full=%d, y_sb_empty=%d, y_sb_dptr_ro=%d, sb_full=%d\n",
              vpss_info->sb_stat.y_sb_wptr_ro, vpss_info->sb_stat.y_sb_full,
              vpss_info->sb_stat.y_sb_empty, vpss_info->sb_stat.y_sb_dptr_ro,
              vpss_info->sb_stat.sb_full);
        vi_pr(VI_ERR, "    latched_line_cnt=%d\n", vpss_info->latched_line_cnt);
    }

    if (vi_info->enable) {
        vi_info->enable = false;
        vi_pr(VI_ERR, "ISP_TOP, blk_idle(h38)=0x%x\n", vi_info->isp_top.blk_idle);
        for (i = 0; i <= 6; i++) {
            vi_pr(VI_ERR, "dbus_sel=%d, r_0=0x%x, r_4=0x%x, r_8=0x%x, r_c=0x%x\n", i,
                  vi_info->isp_top.dbus_sel[i].r_0, vi_info->isp_top.dbus_sel[i].r_4,
                  vi_info->isp_top.dbus_sel[i].r_8, vi_info->isp_top.dbus_sel[i].r_c);
        }
        vi_pr(VI_ERR, "PRE_RAW_FE0, preraw_info(h34)=0x%x, fe_idle_info(h50)=0x%x\n",
              vi_info->preraw_fe.preraw_info, vi_info->preraw_fe.fe_idle_info);
        vi_pr(VI_ERR,
              "PRE_RAW_BE, preraw_be_info(h14)=0x%x, be_dma_idle_info(h18)=0x%x, "
              "ip_idle_info(h1c)=0x%x\n",
              vi_info->preraw_be.preraw_be_info, vi_info->preraw_be.be_dma_idle_info,
              vi_info->preraw_be.ip_idle_info);
        vi_pr(VI_ERR, "PRE_RAW_BE, stvalid_status(h28)=0x%x, stready_status(h2c)=0x%x\n",
              vi_info->preraw_be.stvalid_status, vi_info->preraw_be.stready_status);
        vi_pr(VI_ERR,
              "RAW_TOP, stvalid_status(h40)=0x%x, stready_status(h44)=0x%x, dma_idle(h60)=0x%x\n",
              vi_info->rawtop.stvalid_status, vi_info->rawtop.stready_status,
              vi_info->rawtop.dma_idle);
        vi_pr(VI_ERR, "RGB_TOP, ip_stvalid_status(h50)=0x%x, ip_stready_status(h54)=0x%x\n",
              vi_info->rgbtop.ip_stvalid_status, vi_info->rgbtop.ip_stready_status);
        vi_pr(VI_ERR, "RGB_TOP, dmi_stvalid_status(h58)=0x%x, dmi_stready_status(h5c)=0x%x\n",
              vi_info->rgbtop.dmi_stvalid_status, vi_info->rgbtop.dmi_stready_status);
        vi_pr(VI_ERR, "RGB_TOP xcnt_rpt=0x%x, ycnt_rpt=0x%x\n", vi_info->rgbtop.xcnt_rpt,
              vi_info->rgbtop.ycnt_rpt);
        vi_pr(VI_ERR,
              "YUV_TOP debug_state(h18)=0x%x, stvalid_status(h6c)=0x%x, stready_status(h70)=0x%x\n",
              vi_info->yuvtop.debug_state, vi_info->yuvtop.stvalid_status,
              vi_info->yuvtop.stready_status);
        vi_pr(VI_ERR, "YUV_TOP xcnt_rpt=0x%x, ycnt_rpt=0x%x\n", vi_info->yuvtop.xcnt_rpt,
              vi_info->yuvtop.ycnt_rpt);
        vi_pr(VI_ERR, "rdma28, dbg_sel(h000)=0x%x, status(h014)=0x%x\n", vi_info->rdma28[0].dbg_sel,
              vi_info->rdma28[0].status);
        vi_pr(VI_ERR, "rdma28, dbg_sel(h000)=0x%x, status(h014)=0x%x\n", vi_info->rdma28[1].dbg_sel,
              vi_info->rdma28[1].status);
    }

    rt_smem_free(gOverflowInfo);
    gOverflowInfo = NULL;
}
...
{
    struct isp_ctx *ctx     = &vdev->ctx;
    u8              wait_fe = ISP_PRERAW_A, count = 20;

    // Stop pre/postraw trigger go
    //  rt_atomic_store(&vdev->isp_err_handle_flag, 1);

    // step 1 : set frm vld = 0
    isp_frm_err_handler(ctx, err_raw_num, 1);

    if (ctx->is_dual_sensor) wait_fe = (err_raw_num == ISP_PRERAW_A) ? ISP_PRERAW_B : ISP_PRERAW_A;

    // step 2 : wait to make sure post and the other fe is done.
    while (--count > 0) {
        if (_is_be_post_online(ctx)) {
            if (rt_atomic_load(&vdev->postraw_state) == ISP_POSTRAW_IDLE &&
                rt_atomic_load(&vdev->pre_fe_state[wait_fe][ISP_FE_CH0]) == ISP_PRERAW_IDLE &&
                rt_atomic_load(&vdev->pre_fe_state[wait_fe][ISP_FE_CH1]) == ISP_PRERAW_IDLE &&
                rt_atomic_load(&vdev->pre_be_state[ISP_BE_CH0]) == ISP_PRE_BE_IDLE &&
                rt_atomic_load(&vdev->pre_be_state[ISP_BE_CH1]) == ISP_PRE_BE_IDLE)
                break;
            vi_pr(VI_WARN, "wait fe_%d, be and post idle count(%d) for be_post online\n", wait_fe,
                  count);
        } else if (_is_fe_be_online(ctx) && !ctx->is_slice_buf_on) {
            if (ctx->is_dual_sensor) {
                if (err_raw_num == ISP_PRERAW_A) {
                    ...
                    if (rt_atomic_load(&vdev->postraw_state) == ISP_POSTRAW_IDLE &&
                        rt_atomic_load(&vdev->pre_fe_state[wait_fe][ISP_FE_CH0]) ==
                            ISP_PRERAW_IDLE &&
                        rt_atomic_load(&vdev->pre_fe_state[wait_fe][ISP_FE_CH1]) == ISP_PRERAW_IDLE)
                        break;
                ...
                if (rt_atomic_load(&vdev->postraw_state) == ISP_POSTRAW_IDLE) break;
                vi_pr(VI_WARN, "wait post idle(%d) count(%d) for fe_be online single\n",
                      rt_atomic_load(&vdev->postraw_state), count);
            }
        } else {
            break;
        }
    ...
    if (count == 0) {
        if (ctx->is_dual_sensor) {
            vi_pr(VI_ERR, "isp status fe_%d(ch0:%d, ch1:%d) be(ch0:%d, ch1:%d) postraw(%d)\n",
                  wait_fe, rt_atomic_load(&vdev->pre_fe_state[wait_fe][ISP_FE_CH0]),
                  rt_atomic_load(&vdev->pre_fe_state[wait_fe][ISP_FE_CH1]),
                  rt_atomic_load(&vdev->pre_be_state[ISP_BE_CH0]),
                  rt_atomic_load(&vdev->pre_be_state[ISP_BE_CH1]),
                  rt_atomic_load(&vdev->postraw_state));
        } else {
    ...
    if (isp_frm_err_handler(ctx, err_raw_num, 3) < 0) return;
    ...
    if (isp_frm_err_handler(ctx, err_raw_num, 6) < 0) return;

    // step 7 : reset sw state to idle
    if (_is_be_post_online(ctx)) {
        rt_atomic_store(&vdev->pre_fe_state[err_raw_num][ISP_FE_CH0], ISP_PRERAW_IDLE);
        rt_atomic_store(&vdev->pre_fe_state[err_raw_num][ISP_FE_CH1], ISP_PRERAW_IDLE);
    } else if (_is_fe_be_online(ctx) && !ctx->is_slice_buf_on) {
        if (ctx->is_dual_sensor) {
            if (err_raw_num == ISP_PRERAW_A) {
                rt_atomic_store(&vdev->pre_be_state[ISP_BE_CH0], ISP_PRE_BE_IDLE);
                rt_atomic_store(&vdev->pre_be_state[ISP_BE_CH1], ISP_PRE_BE_IDLE);
            } else {
                rt_atomic_store(&vdev->pre_fe_state[err_raw_num][ISP_FE_CH0], ISP_PRERAW_IDLE);
                rt_atomic_store(&vdev->pre_fe_state[err_raw_num][ISP_FE_CH1], ISP_PRERAW_IDLE);
            }
    ...
    if (ctx->is_dual_sensor) vdev->ctx.isp_pipe_cfg[wait_fe].first_frm_cnt = 0;
...
void *_vi_err_handler_thread(void *arg)
{
    ...
    while (1) {
        if (rt_event_recv(&vdev->vi_th[th_id].event, 0x01, RT_EVENT_FLAG_OR | RT_EVENT_FLAG_CLEAR,
                          RT_WAITING_FOREVER, &actl_flags) != 0) {
            continue;
        }
        if (vdev->vi_th[th_id].flag == 0) continue;
...
{
    if (rt_atomic_load(&vdev->isp_err_handle_flag) == 1) {
        vi_pr(VI_WARN, "err_handler running\n");
        return;
    }

    if (err_raw == ISP_PRERAW_A)
        vdev->vi_th[E_VI_TH_ERR_HANDLER].flag = 1;
    else if (err_raw == ISP_PRERAW_B)
        vdev->vi_th[E_VI_TH_ERR_HANDLER].flag = 2;
...
u32 isp_err_chk(struct cvi_vi_dev *vdev, struct isp_ctx *ctx,
...
void *_vi_preraw_thread(void *arg)
{
    ...
    while (1) {
        ...
        {
            n[enq_num] = clist_entry(pos, struct _isp_raw_num_n, list);
            list_del_init(&n[enq_num]->list);
            enq_num++;
        ...
        for (i = 0; i < enq_num; i++) {
            ...
            if (ctx->isp_pipe_cfg[ISP_PRERAW_A].is_offline_preraw) {
                ...
                if (_is_fe_be_online(ctx) || _is_all_online(ctx)) {
                    if (!ctx->is_slice_buf_on) pre_be_tuning_update(&vdev->ctx, raw_num);
            ...
            if (!ctx->isp_pipe_cfg[raw_num].is_frame_valid) {
                ctx->isp_pipe_cfg[raw_num].is_drop_next_frame = true;
            }
            ...
            if (ctx->isp_pipe_cfg[raw_num].is_drop_next_frame) {
                ...
                if (_is_all_online(ctx) && !ctx->isp_pipe_cfg[raw_num].is_offline_scaler) continue;
...
static void _isp_yuv_online_handler(struct cvi_vi_dev *vdev, const enum cvi_isp_raw raw_num,
                                    const u8 hw_chn_num)
{
    struct isp_ctx    *ctx = &vdev->ctx;
    struct isp_buffer *b   = NULL;
    ...
    rt_atomic_store(&vdev->pre_fe_state[raw_num][hw_chn_num], ISP_PRERAW_IDLE);
    ...
    if (!ctx->isp_pipe_cfg[raw_num].is_offline_preraw) _pre_hw_enque(vdev, raw_num, hw_chn_num);
}

static void _isp_yuv_bypass_handler(struct cvi_vi_dev *vdev, const enum cvi_isp_raw raw_num,
                                    const u8 hw_chn_num)
{
    u8 buf_chn = (raw_num == ISP_PRERAW_A) ? hw_chn_num : vdev->ctx.rawb_chnstr_num + hw_chn_num;
    ...
    vdev->vi_th[E_VI_TH_EVENT_HANDLER].flag = (raw_num == ISP_PRERAW_A) ? 1 : 2;
...
static void _isp_sof_handler(struct cvi_vi_dev *vdev, const enum cvi_isp_raw raw_num)
{
    ...
    if (!(_is_fe_be_online(ctx) && ctx->is_slice_buf_on) ||
        ctx->isp_pipe_cfg[raw_num].is_drop_next_frame) {
        _vi_wake_up_preraw_th(vdev, raw_num);
    }

    if (rt_atomic_load(&vdev->isp_raw_dump_en[raw_num]) == 2)  // raw_dump flow
        rt_atomic_store(&vdev->isp_raw_dump_en[raw_num], 3);

    tasklet_hi_schedule(&vdev->vi_th[E_VI_TH_POSTRAW]);

    vi_event_queue(vdev, type, vdev->pre_fe_sof_cnt[raw_num][ISP_FE_CH0], UINT8_MAX);
#if 0
	if (ctx->isp_pipe_cfg[raw_num].is_offline_scaler) {
		spin_lock_irqsave(&dq_lock, flags);
		if (!list_empty(&dqbuf_q.list)) {
			n = list_first_entry(&dqbuf_q.list, struct _isp_dqbuf_n, list);
			vdev->vi_th[E_VI_TH_EVENT_HANDLER].flag = (n->chn_id == ISP_PRERAW_A) ? 1 : 2;
			vi_wake_up(&vdev->vi_th[E_VI_TH_EVENT_HANDLER]);
		}
		spin_unlock_irqrestore(&dq_lock, flags);
	}
...
static inline void _isp_pre_fe_done_handler(struct cvi_vi_dev *vdev, const enum cvi_isp_raw raw_num,
                                            const enum cvi_isp_pre_chn_num chn_num)
{
    struct isp_ctx           *ctx       = &vdev->ctx;
    u32                       trigger   = false;
    ...
    struct isp_sync_task_data sync_data = {0};

    if (ctx->isp_pipe_cfg[raw_num].is_mux) {
        if (ctx->isp_pipe_cfg[raw_num].is_mux_switch !=
            ctx->isp_pipe_cfg[raw_num].switch_info.gpio_pol)
            cur_raw = raw_num + ISP_PRERAW_MAX;
        if (++ctx->isp_pipe_cfg[cur_raw].switch_info.cnt_frm <
            ctx->isp_pipe_cfg[cur_raw].switch_info.dst_frm)
            next_raw = cur_raw;
        else {  // switch next raw
            next_raw = cur_raw == raw_num ? raw_num + ISP_PRERAW_MAX : raw_num;
            ctx->isp_pipe_cfg[cur_raw].switch_info.cnt_frm = 0;
        }

        vi_pr(VI_DBG, "cur_raw[%d] next_raw[%d] cnt_frm_%d, dst_frm_%d\n", cur_raw, next_raw,
              ctx->isp_pipe_cfg[cur_raw].switch_info.cnt_frm,
              ctx->isp_pipe_cfg[cur_raw].switch_info.dst_frm);

        sync_data.value = next_raw;
    }

    ++vdev->pre_fe_frm_num[cur_raw][chn_num];
    if (unlikely(vdev->isp_err_times[raw_num])) vdev->isp_err_times[raw_num] = 0;

    if (ctx->isp_pipe_cfg[raw_num].is_yuv_bypass_path) {
        // reset error times when fe_done
        if (vdev->isp_err_times[raw_num]) vdev->isp_err_times[raw_num] = 0;
        if (ctx->isp_pipe_cfg[raw_num].is_offline_scaler) {  // offline mode
            vi_pr(VI_DBG, "pre_fe_%d chn_num_%d yuv offline done\n", raw_num, chn_num);
            _isp_yuv_bypass_handler(vdev, raw_num, chn_num);
        } else {  // YUV sensor online mode
            vi_pr(VI_DBG, "pre_fe_%d chn_num_%d yuv online done\n", raw_num, chn_num);
    ...
    if (_is_fe_be_online(ctx) || _is_all_online(ctx)) {  // fe->be->dram->post or on the fly mode
        if (rt_atomic_load(&vdev->isp_raw_dump_en[raw_num]) == 3) {  // raw_dump flow
            struct isp_buffer *b = NULL;
            struct isp_queue  *fe_out_q =
                (chn_num == ISP_FE_CH0) ? &raw_dump_b_q[raw_num] : &raw_dump_b_se_q[raw_num];
            struct isp_queue *raw_d_q =
                (chn_num == ISP_FE_CH0) ? &raw_dump_b_dq[raw_num] : &raw_dump_b_se_dq[raw_num];
            u32 x, y, w, h, dmaid;

            if (ctx->isp_pipe_cfg[raw_num].rawdump_crop.w &&
                ctx->isp_pipe_cfg[raw_num].rawdump_crop.h) {
                x = (chn_num == ISP_FE_CH0) ? ctx->isp_pipe_cfg[raw_num].rawdump_crop.x
                                            : ctx->isp_pipe_cfg[raw_num].rawdump_crop_se.x;
                y = (chn_num == ISP_FE_CH0) ? ctx->isp_pipe_cfg[raw_num].rawdump_crop.y
                                            : ctx->isp_pipe_cfg[raw_num].rawdump_crop_se.y;
                w = (chn_num == ISP_FE_CH0) ? ctx->isp_pipe_cfg[raw_num].rawdump_crop.w
                                            : ctx->isp_pipe_cfg[raw_num].rawdump_crop_se.w;
                h = (chn_num == ISP_FE_CH0) ? ctx->isp_pipe_cfg[raw_num].rawdump_crop.h
                                            : ctx->isp_pipe_cfg[raw_num].rawdump_crop_se.h;
            } else {
                x = 0;
                y = 0;
                w = (chn_num == ISP_FE_CH0) ? ctx->isp_pipe_cfg[raw_num].crop.w
                                            : ctx->isp_pipe_cfg[raw_num].crop_se.w;
                h = (chn_num == ISP_FE_CH0) ? ctx->isp_pipe_cfg[raw_num].crop.h
            ...
            if (chn_num == ISP_FE_CH0) ++vdev->dump_frame_number[raw_num];
            ...
            b->byr_size                 = ispblk_dma_get_size(ctx, dmaid, raw_num, w, h);
            b->frm_num                  = vdev->pre_fe_frm_num[raw_num][chn_num];
            b->is_unuse = ctx->isp_pipe_cfg[raw_num].is_pipe_on;  // means don't get start pipe raw
        ...
        struct isp_grid_s_info m_info;
        struct isp_queue      *fe_out_q =
            (chn_num == ISP_FE_CH0) ? &pre_out_queue[raw_num] : &pre_out_se_queue[raw_num];
        struct isp_queue *be_in_q = NULL;
        struct isp_queue *raw_d_q =
            (chn_num == ISP_FE_CH0) ? &raw_dump_b_dq[cur_raw] : &raw_dump_b_se_dq[cur_raw];
        enum ISP_BLK_ID_T rgbmap_id =
            (chn_num == ISP_FE_CH0) ? ISP_BLK_ID_RGBMAP0 : ISP_BLK_ID_RGBMAP1;

        if (rt_atomic_load(&vdev->isp_raw_dump_en[cur_raw]) == 3)  // raw dump enable
        ...
        if (ctx->isp_pipe_cfg[cur_raw].is_hdr_on)
            be_in_q = (chn_num == ISP_FE_CH0) ? &pre_be_in_syn_q : &pre_be_in_se_syn_q[cur_raw];
        else
            be_in_q = (chn_num == ISP_FE_CH0) ? &pre_be_in_q : &pre_be_in_se_q[cur_raw];

        if (rt_atomic_load(&vdev->isp_raw_dump_en[cur_raw]) == 3) {  // raw dump enable
            u32 w = (chn_num == ISP_FE_CH0) ? ctx->isp_pipe_cfg[cur_raw].crop.w
                                            : ctx->isp_pipe_cfg[cur_raw].crop_se.w;
            u32 h = (chn_num == ISP_FE_CH0) ? ctx->isp_pipe_cfg[cur_raw].crop.h
            ...
            if (chn_num == ISP_FE_CH0) ++vdev->dump_frame_number[cur_raw];
            ...
            if (ctx->isp_pipe_cfg[cur_raw].is_pipe_on && buf_num_check(vdev, cur_raw, chn_num)) {
                m_info            = ispblk_rgbmap_info(ctx, raw_num);
                ...
                b->is_yuv_frm = 0;
                b->chn_num    = 0;
                b->lft_frm    = (ctx->isp_pipe_cfg[raw_num].is_mux && next_raw == cur_raw)
                                    ? (ctx->isp_pipe_cfg[cur_raw].switch_info.dst_frm -
                                    ctx->isp_pipe_cfg[cur_raw].switch_info.cnt_frm)
                                    : 0;

                // for mux dev we change raw_num
                if (ctx->isp_pipe_cfg[raw_num].is_mux) b->raw_num = cur_raw;

                if (!ctx->isp_pipe_cfg[cur_raw].is_3dnr_enable &&
                    (vdev->pre_fe_frm_num[cur_raw][chn_num] < 3)) {
                    /*workaround multi sensor enable fbc need drop buffer which 3dnr disable*/
                ...
                if (!buf_num_check(vdev, cur_raw, chn_num))
                    --vdev->pre_fe_frm_num[cur_raw][chn_num];
                else
                    ++vdev->dump_frame_number[cur_raw];
            ...
            ispblk_rgbmap_config_dma(ctx, rgbmap_id, !ctx->isp_pipe_cfg[next_raw].is_pipe_on,
                                     next_raw);
        }
        ...
        if (trigger) {
            ...
            if (rt_atomic_load(&vdev->isp_raw_dump_en[cur_raw]) == 3) {  // raw dump flow
                _isp_raw_dump_chk(vdev, cur_raw, b->frm_num);
            } else {
                tasklet_hi_schedule(&vdev->vi_th[E_VI_TH_POSTRAW]);
            }

            sync_data.vi_pipe    = cur_raw;
            sync_data.sync_event = VI_SYNC_EVENT_FE_DONE;
            isp_sync_task_process(cur_raw, &sync_data);
        }

        if (ctx->isp_pipe_cfg[raw_num].is_mux && next_raw != cur_raw) {
            ctx->isp_pipe_cfg[raw_num].is_mux_switch ^= 1;
            ispblk_csibdg_config(ctx, next_raw);
            ispblk_csibdg_crop_update(ctx, next_raw, true);
            ispblk_preraw_fe_config(ctx, next_raw);

            if (ctx->isp_pipe_cfg[raw_num].switch_info.gpio_idx != 255) {
                // csi_gpio_write(&ctx->isp_pipe_cfg[raw_num].switch_info.gpio,
                // 		1 << ctx->isp_pipe_cfg[raw_num].switch_info.gpio_pin,
                // 		ctx->isp_pipe_cfg[raw_num].is_mux_switch);
...
static inline void _isp_pre_be_done_handler(struct cvi_vi_dev *vdev, const u8 chn_num)
{
    struct isp_ctx           *ctx     = &vdev->ctx;
    enum cvi_isp_raw          raw_num = ISP_PRERAW_A;
    u32                       type;
    u32                       trigger   = false;
    struct isp_sync_task_data sync_data = {0};
    u8                        lft_frm   = UINT8_MAX;

    if (_is_fe_be_online(ctx) && !ctx->is_slice_buf_on) {  // fe->be->dram->post
        struct isp_buffer     *b = NULL;
        struct isp_grid_s_info m_info;
        struct isp_queue *be_out_q  = (chn_num == ISP_FE_CH0) ? &pre_be_out_q : &pre_be_out_se_q;
        struct isp_queue *post_in_q = (chn_num == ISP_FE_CH0) ? &post_in_queue : &post_in_se_queue;
        enum ISP_BLK_ID_T rgbmap_id =
            (chn_num == ISP_FE_CH0) ? ISP_BLK_ID_RGBMAP0 : ISP_BLK_ID_RGBMAP1;
        ...
        // only pipe on and Aviod le\se drop frame on hdr mode
        if (ctx->isp_pipe_cfg[raw_num].is_pipe_on && buf_num_check(vdev, raw_num, chn_num)) {
            m_info            = ispblk_rgbmap_info(ctx, raw_num);
            b->rgbmap_i.w_bit = m_info.w_bit;
            b->rgbmap_i.h_bit = m_info.h_bit;
            ...
            if (!buf_num_check(vdev, raw_num, chn_num))
                --vdev->pre_be_frm_num[raw_num][chn_num];
            else
                ++vdev->dump_frame_number[raw_num];
        ...
        ispblk_rgbmap_config_dma(ctx, rgbmap_id, !ctx->isp_pipe_cfg[raw_num].is_pipe_on, raw_num);
        ...
        if (!ctx->isp_pipe_cfg[raw_num].is_offline_preraw) _pre_hw_enque(vdev, raw_num, chn_num);
        ...
        if (trigger) {
            vi_event_queue(vdev, type, vdev->pre_be_frm_num[raw_num][ISP_BE_CH0], lft_frm);

            tasklet_hi_schedule(&vdev->vi_th[E_VI_TH_POSTRAW]);

            sync_data.vi_pipe    = raw_num;
            sync_data.sync_event = VI_SYNC_EVENT_BE_DONE;
            isp_sync_task_process(raw_num, &sync_data);
        }
    } else if (_is_be_post_online(ctx)) {  // fe->dram->be->post
        struct isp_buffer *b         = NULL;
        struct isp_queue  *be_in_q   = NULL;
        ...
        if (ctx->isp_pipe_cfg[vdev->ctx.cam_id].is_hdr_on)
            be_in_q =
                (chn_num == ISP_BE_CH0) ? &pre_be_in_syn_q : &pre_be_in_se_syn_q[vdev->ctx.cam_id];
        else
            be_in_q = (chn_num == ISP_BE_CH0) ? &pre_be_in_q : &pre_be_in_se_q[vdev->ctx.cam_id];
        ...
        vi_pr(VI_WARN, "pre_be_%d frm_done chn_num=%d frm_num=%d\n", raw_num, chn_num,
              vdev->pre_be_frm_num[raw_num][chn_num]);

        if (!ctx->isp_pipe_cfg[ISP_PRERAW_A].is_offline_preraw) {
            pre_out_q =
                (chn_num == ISP_BE_CH0) ? &pre_out_queue[hw_raw] : &pre_out_se_queue[hw_raw];
...
static void _isp_postraw_done_handler(struct cvi_vi_dev *vdev)
{
    ...
    if (_is_fe_be_online(ctx)) raw_num = ctx->cam_id;

    if (_isp_clk_dynamic_en(vdev, false) < 0) return;
    ...
    if (_is_fe_be_online(ctx) && !ctx->is_slice_buf_on) {  // fe->be->dram->post
        ...
        if (ispb->raw_num >= ISP_PRERAW_VIRT_MAX) {
            vi_pr(VI_ERR, "buf raw_num_%d is wrong\n", ispb->raw_num);
            return;
        }
        ...
        if (_is_fe_be_online(ctx) && ctx->is_slice_buf_on)
            rt_atomic_store(&vdev->ctx.is_post_done, 1);
    }
    ...
    if (!ctx->isp_pipe_cfg[raw_num].is_yuv_bypass_path) {  // ISP team no need yuv post done
        ...
        vi_event_queue(vdev, type, vdev->postraw_frame_number[raw_num], lft_frm);
    }
...
{
    struct isp_ctx                          *ctx = &vdev->ctx;
    union REG_ISP_CSI_BDG_INTERRUPT_STATUS_0 cbdg_0_sts, cbdg_0_sts_b;
    union REG_ISP_CSI_BDG_INTERRUPT_STATUS_1 cbdg_1_sts, cbdg_1_sts_b;
    ...
    if (!rt_atomic_load(&vdev->isp_streamon)) return;

    vi_perf_record_dump();

    cbdg_0_sts.raw = cbdg_0_sts_b.raw = cbdg_1_sts.raw = cbdg_1_sts_b.raw = 0;
    isp_csi_intr_status(ctx, ISP_PRERAW_A, &cbdg_0_sts, &cbdg_1_sts);
    if (ctx->is_dual_sensor) isp_csi_intr_status(ctx, ISP_PRERAW_B, &cbdg_0_sts_b, &cbdg_1_sts_b);
    ...
    for (raw_num = ISP_PRERAW_A; raw_num < ISP_PRERAW_MAX; raw_num++) {
        ...
        for (i = 0; i < ISP_FE_CHN_MAX; i++)
            ctx->isp_pipe_cfg[raw_num].dg_info.bdg_chn_debug[i] =
                ispblk_csibdg_chn_dbg(ctx, raw_num, i);
    }

    if (isp_err_chk(vdev, ctx, cbdg_0_sts, cbdg_1_sts, cbdg_0_sts_b, cbdg_1_sts_b) == -1) return;
        ...
        if (!vdev->ctx.isp_pipe_cfg[ISP_PRERAW_A].is_yuv_bypass_path) {  // RGB sensor
            if (!vdev->ctx.isp_pipe_cfg[ISP_PRERAW_A].is_offline_preraw) {
                if (vdev->ctx.isp_pipe_cfg[ISP_PRERAW_A].is_mux &&
                    (vdev->ctx.isp_pipe_cfg[ISP_PRERAW_A].is_mux_switch !=
                     vdev->ctx.isp_pipe_cfg[ISP_PRERAW_A].switch_info.gpio_pol))
                    _isp_sof_handler(vdev, ISP_PRERAW_A + ISP_PRERAW_MAX);
                else
                    _isp_sof_handler(vdev, ISP_PRERAW_A);
            }
        } else if (!vdev->ctx.isp_pipe_cfg[ISP_PRERAW_A]
                        .is_offline_scaler) {  // YUV sensor online mode
    ...
    if (top_sts_2.bits.FRAME_START_FE1 & 0x1) {
        ...
        } else if (!vdev->ctx.isp_pipe_cfg[ISP_PRERAW_B]
                        .is_offline_scaler) {  // YUV sensor online mode
            // ISP team no need sof event by yuv sensor
            //_post_hw_enque(vdev);
            tasklet_hi_schedule(&vdev->vi_th[E_VI_TH_POSTRAW]);
    ...
    if (!ctx->is_synthetic_hdr_on) {
        ...
        if (top_sts_1.bits.PQ_DONE_FE0 & 0x1) {
            if (ctx->isp_pipe_cfg[ISP_PRERAW_A].is_hdr_on &&
                ...
                ISP_WR_BITS(ctx->phys_regs[ISP_BLK_ID_PRE_RAW_FE0], REG_PRE_RAW_FE_T,
                            PRE_RAW_FRAME_VLD, FE_PQ_VLD_CH1, 1);
            }
        }
        ...
        if (top_sts_1.bits.PQ_DONE_FE1 & 0x1) {
            if (ctx->isp_pipe_cfg[ISP_PRERAW_B].is_hdr_on &&
                ...
                ISP_WR_BITS(ctx->phys_regs[ISP_BLK_ID_PRE_RAW_FE1], REG_PRE_RAW_FE_T,
                            PRE_RAW_FRAME_VLD, FE_PQ_VLD_CH1, 1);
            }
        }
    }
...
int vi_destroy_instance(struct cvi_vi_dev *pdev)
{
    ...
    for (i = 0; i < E_VI_TH_MAX; i++) vi_destory_thread(vdev, i);
    ...
    for (i = 0; i < ISP_PRERAW_VIRT_MAX; i++) {
        vdev->isp_int_flag[i] = false;
        rt_event_detach(&vdev->isp_int_wait_q[i].event);
        rt_event_detach(&vdev->dis_wait_q[i].event);
    }
