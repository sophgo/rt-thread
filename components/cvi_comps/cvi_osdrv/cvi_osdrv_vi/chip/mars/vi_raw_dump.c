...
extern void vi_wake_up(struct vi_thread_attr *event);
...
void _isp_fe_be_raw_dump_cfg(struct cvi_vi_dev *vdev, const enum cvi_isp_raw raw_num,
                             const u8 chn_num)
{
    ...
    if (trigger) {
        ...
        if (ctx->isp_pipe_cfg[raw_num].rawdump_crop.w &&
            ...
            vi_pr(VI_DBG, "rawdump_crop w(%d), h(%d)\n", ctx->isp_pipe_cfg[raw_num].rawdump_crop.w,
                  ctx->isp_pipe_cfg[raw_num].rawdump_crop.h);
            vi_pr(VI_DBG, "b->crop_le x(%d), y(%d), w(%d), h(%d)\n", b->crop_le.x, b->crop_le.y,
                  b->crop_le.w, b->crop_le.h);
        ...
        if (ctx->isp_pipe_cfg[raw_num].enDumpRaw != COMPRESS_MODE_DEFAULT)
            ispblk_csibdg_update_dpcm(ctx, raw_num,
                                      ctx->isp_pipe_cfg[raw_num].enDumpRaw == COMPRESS_MODE_TILE);
...
int isp_raw_dump(struct cvi_vi_dev *vdev, struct cvi_vip_isp_raw_blk *dump)
{
    ...
    COMPRESS_MODE_E    mode       = COMPRESS_MODE_NONE;
    ...
    if (ctx->isp_pipe_cfg[raw_num].is_hdr_on) {
        ...
        vi_pr(VI_DBG, "b->crop_se x(%d), y(%d), w(%d), h(%d)\n", b->crop_se.x, b->crop_se.y,
              b->crop_se.w, b->crop_se.h);
        isp_buf_queue(&raw_dump_b_se_q[b->raw_num], b);
    }

    rt_atomic_store(&vdev->isp_raw_dump_en[raw_num], 1);

    if (ctx->isp_pipe_cfg[raw_num].enDumpRaw == COMPRESS_MODE_DEFAULT)
        mode = ctx->is_dpcm_on ? COMPRESS_MODE_TILE : COMPRESS_MODE_NONE;
    else if (ctx->isp_pipe_cfg[raw_num].enDumpRaw == COMPRESS_MODE_TILE)
        mode = COMPRESS_MODE_TILE;

    ret = rt_event_recv(&vdev->isp_int_wait_q[raw_num].event, 0x01,
                        RT_EVENT_FLAG_OR | RT_EVENT_FLAG_CLEAR, dump->time_out, &actl_flags);
    if (ret || vdev->isp_int_flag[raw_num] == 0) {
...
int isp_start_smooth_raw_dump(struct cvi_vi_dev                   *vdev,
                              struct cvi_vip_isp_smooth_raw_param *pstSmoothRawParam)
{
    struct isp_ctx    *ctx = &vdev->ctx;
    struct isp_buffer *b = NULL, *b_se = NULL;
    int                ret = 0;
    ...
    for (j = 0; j < frm_num; j++) {
        b = calloc(sizeof(*b), 1);
        if (b == NULL) {
            vi_pr(VI_ERR, "roi_le_%d malloc size(%lu) fail\n", j, sizeof(*b));
            ret = -1;
            goto err;
        }
        ...
        b->crop_le = ctx->isp_pipe_cfg[raw_num].rawdump_crop;
        vi_pr(VI_DBG,
              "raw_num=%d enque raw_dump le 0x%" PRIx64 ", crop x(%d), y(%d), w(%d), h(%d)\n",
              raw_num, b->addr, b->crop_le.x, b->crop_le.y, b->crop_le.w, b->crop_le.h);
        isp_buf_queue(&raw_dump_b_q[raw_num], b);

        if (ctx->isp_pipe_cfg[raw_num].is_hdr_on) {
            b_se = calloc(sizeof(*b_se), 1);
            if (b_se == NULL) {
                vi_pr(VI_ERR, "roi_se_%d malloc size(%lu) fail\n", j, sizeof(*b_se));
                ret = -1;
                goto err;
            }
            j++;
            ...
            vi_pr(VI_DBG,
                  "raw_num=%d enque raw_dump se 0x%" PRIx64 ", crop x(%d), y(%d), w(%d), h(%d)\n",
                  raw_num, b_se->addr, b_se->crop_se.x, b_se->crop_se.y, b_se->crop_se.w,
                  b_se->crop_se.h);
            isp_buf_queue(&raw_dump_b_se_q[raw_num], b_se);
        }
    }

    ctx->isp_pipe_cfg[raw_num].enDumpRaw = pstSmoothRawParam->raw_blk->compress_mode;

    vi_pr(VI_DBG, "raw_num_%d smooth_dumpraw compress mode %d\n", raw_num,
          ctx->isp_pipe_cfg[raw_num].enDumpRaw);

    rt_atomic_store(&vdev->isp_smooth_raw_dump_en[raw_num], 1);
    rt_atomic_store(&vdev->isp_raw_dump_en[raw_num], 1);

err:
    if (ret == -1) {
        while ((b = isp_buf_remove(&raw_dump_b_q[raw_num])) != NULL) free(b);
        while ((b_se = isp_buf_remove(&raw_dump_b_se_q[raw_num])) != NULL) free(b_se);
    }

    return ret;
}

int isp_stop_smooth_raw_dump(struct cvi_vi_dev                   *vdev,
                             struct cvi_vip_isp_smooth_raw_param *pstSmoothRawParam)
{
    u8 raw_num;

    raw_num = pstSmoothRawParam->raw_num;
    rt_atomic_store(&vdev->isp_smooth_raw_dump_en[raw_num], 2);
...
int isp_put_smooth_raw_dump(struct cvi_vi_dev *vdev, struct cvi_vip_isp_raw_blk *dump)
{
    ...
    b = calloc(sizeof(*b), 1);
    if (b == NULL) {
        vi_pr(VI_ERR, "le malloc size(%lu) fail\n", sizeof(*b));
        free(b);
        return CVI_FAILURE;
    }
    b->addr    = dump[0].raw_dump.phy_addr;
    b->raw_num = raw_num;
    b->crop_le = ctx->isp_pipe_cfg[raw_num].rawdump_crop;
    vi_pr(VI_DBG, "raw_num=%d enque raw_dump le 0x%" PRIx64 ", crop x(%d), y(%d), w(%d), h(%d)\n",
          raw_num, b->addr, b->crop_le.x, b->crop_le.y, b->crop_le.w, b->crop_le.h);
    isp_buf_queue(&raw_dump_b_q[b->raw_num], b);
    ...
    if (ctx->isp_pipe_cfg[raw_num].is_hdr_on) {
        b_se = calloc(sizeof(*b_se), 1);
        if (b_se == NULL) {
            vi_pr(VI_ERR, "se malloc size(%lu) fail\n", sizeof(*b_se));
            free(b_se);
            return CVI_FAILURE;
        }
...
void _isp_raw_dump_chk(struct cvi_vi_dev *vdev, const enum cvi_isp_raw raw_num,
                       const uint32_t frm_num)
{
    switch (rt_atomic_load(&vdev->isp_smooth_raw_dump_en[raw_num])) {
    default:
    case 0: {
        vi_pr(VI_DBG, "wake up wait_q\n");

        vdev->isp_int_flag[raw_num] = 1;
        vi_wake_up(&vdev->isp_int_wait_q[raw_num]);

        rt_atomic_store(&vdev->isp_raw_dump_en[raw_num], 0);
        ispblk_csibdg_update_dpcm(&vdev->ctx, raw_num, vdev->ctx.is_dpcm_on);
        vdev->ctx.isp_pipe_cfg[raw_num].enDumpRaw = COMPRESS_MODE_DEFAULT;
        return;
    }
    case 1: {
    ...
    case 2: {
        ...
        while ((b = isp_buf_remove(&raw_dump_b_dq[raw_num])) != NULL) free(b);
        while ((b = isp_buf_remove(&raw_dump_b_se_dq[raw_num])) != NULL) free(b);
        while ((b = isp_buf_remove(&raw_dump_b_q[raw_num])) != NULL) free(b);
        while ((b = isp_buf_remove(&raw_dump_b_se_q[raw_num])) != NULL) free(b);

        rt_atomic_store(&vdev->isp_raw_dump_en[raw_num], 0);
        rt_atomic_store(&vdev->isp_smooth_raw_dump_en[raw_num], 0);
        ispblk_csibdg_update_dpcm(&vdev->ctx, raw_num, vdev->ctx.is_dpcm_on);
        vdev->ctx.isp_pipe_cfg[raw_num].enDumpRaw = COMPRESS_MODE_DEFAULT;
        return;
    }
