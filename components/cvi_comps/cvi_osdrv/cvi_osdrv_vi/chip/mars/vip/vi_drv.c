...
void isp_post_trig(struct isp_ctx *ctx, enum cvi_isp_raw raw_num)
{
	...
	if (_is_fe_be_online(ctx) && !ctx->is_slice_buf_on) { //fe->be->dram->post
		...
		ISP_WR_REG(isptopb, REG_ISP_TOP_T, SW_CTRL_1, sw_ctrl_1.raw);
		ISP_WR_REG(isptopb, REG_ISP_TOP_T, SW_CTRL_0, sw_ctrl_0.raw);
		vi_pr(VI_DBG, "dram-> post trig raw_num(%d), is_hdr_on(%d)\n",
...
{
	struct isp_grid_s_info dummy;
	memset(&dummy, 0 ,sizeof(struct isp_grid_s_info));
	return dummy;
}
...
int ispblk_dma_buf_get_size(struct isp_ctx *ctx, int dmaid, u8 raw_num)
{
	...
	case ISP_BLK_ID_DMA_CTL9: //fe1_csibdg_le
	...
	{
		/* csibdg */
		if (dmaid == ISP_BLK_ID_DMA_CTL12 || dmaid == ISP_BLK_ID_DMA_CTL13)
...
int ispblk_dma_config(struct isp_ctx *ctx, int dmaid, enum cvi_isp_raw raw_num, uint64_t buf_addr)
{
	...
	case ISP_BLK_ID_DMA_CTL19: //fe2_csibdg_se
		...
		w = ctx->isp_pipe_cfg[raw_num].crop.w;
		num = ctx->isp_pipe_cfg[raw_num].crop.h;
		if ((ctx->isp_pipe_cfg[raw_num].enDumpRaw == COMPRESS_MODE_DEFAULT && ctx->is_dpcm_on) ||
	...
	vi_pr(VI_DBG, "dmaid=%d, len=%d, stride=%d, num=%d, addr=0x%x\n", dmaid, len, stride, num, buf_addr);
...
{
	/*only mux sensor, use common chn buffer return hw_chn num*/
	return ((ctx->isp_pipe_cfg[raw_num].is_mux && ctx->isp_pipe_cfg[raw_num].switch_info.is_comm_chn)
		   && (raw_num > ISP_PRERAW_MAX - 1))
		   ? ctx->chnstr_num[raw_num - ISP_PRERAW_MAX]
		   : ctx->chnstr_num[raw_num];
}
...
enum cvi_isp_raw find_raw_num_by_chn(struct isp_ctx *ctx, int chn_num)
{
	...
	for (raw_num = ISP_PRERAW_A; raw_num < ISP_PRERAW_VIRT_MAX; raw_num++) {
		if (ctx->chnstr_num[raw_num] >= chn_num) break;
	}
...
void isp_first_frm_reset(struct isp_ctx *ctx, uint8_t reset)
{
	...
	//only disable DMA when TNR reset
	if (reset & 0x4)
	...
	ISP_WR_BITS(isptopb, REG_ISP_TOP_T, FIRST_FRAME, FIRST_FRAME_SW, reset ? reset : 0x0);
	//do not disable DMA when TNR not need reset
	if (!(reset & 0x4))
		ISP_WR_BITS(tnr, REG_ISP_444_422_T, REG_8, FORCE_DMA_DISABLE, 0);
}

#define F_D (15)
static inline void _ispblk_lsc_cfg_update(struct isp_ctx *ctx)
{
...
static inline void _ispblk_ldci_cfg_update(struct isp_ctx *ctx, const enum cvi_isp_raw raw_num)
{
	...
	if ((width % 16 == 0) && (height % 12 == 0))
		ISP_WR_BITS(ba, REG_ISP_LDCI_T, LDCI_ENABLE, LDCI_IMAGE_SIZE_DIV_BY_16X12, 1);
	else
		ISP_WR_BITS(ba, REG_ISP_LDCI_T, LDCI_ENABLE, LDCI_IMAGE_SIZE_DIV_BY_16X12, 0);
	...
	ISP_WR_REG(ba, REG_ISP_LDCI_T, LDCI_BLK_SIZE_X, blk_size_x.raw);
	...
	ISP_WR_REG(ba, REG_ISP_LDCI_T, LDCI_BLK_SIZE_X1, blk_size_x1.raw);

	subblk_size_x.raw = 0;
	subblk_size_x.bits.LDCI_SUBBLK_SIZE_X = SubBlockSizeX;
	subblk_size_x.bits.LDCI_SUBBLK_SIZE_Y = SubBlockSizeY;
	ISP_WR_REG(ba, REG_ISP_LDCI_T, LDCI_SUBBLK_SIZE_X, subblk_size_x.raw);
	...
	ISP_WR_REG(ba, REG_ISP_LDCI_T, LDCI_SUBBLK_SIZE_X1, subblk_size_x1.raw);
	...
	ISP_WR_REG(ba, REG_ISP_LDCI_T, LDCI_INTERP_NORM_LR, interp_norm_lr.raw);
	...
	ISP_WR_REG(ba, REG_ISP_LDCI_T, LDCI_INTERP_NORM_LR1, interp_norm_lr1.raw);
	...
	ISP_WR_REG(ba, REG_ISP_LDCI_T, LDCI_SUB_INTERP_NORM_LR, sub_interp_norm_lr.raw);
	...
	ISP_WR_REG(ba, REG_ISP_LDCI_T, LDCI_SUB_INTERP_NORM_LR1, sub_interp_norm_lr1.raw);

	mean_norm_x.raw = 0;
	mean_norm_x.bits.LDCI_MEAN_NORM_X = (1 << 14) / MAX(BlockSizeX, 1);
	mean_norm_x.bits.LDCI_MEAN_NORM_Y = (1 << 13) / MAX(line_mean_num, 1);
	ISP_WR_REG(ba, REG_ISP_LDCI_T, LDCI_MEAN_NORM_X, mean_norm_x.raw);

	var_norm_y.raw = 0;
	var_norm_y.bits.LDCI_VAR_NORM_Y = (1 << 13) / MAX(line_var_num, 1);
	ISP_WR_REG(ba, REG_ISP_LDCI_T, LDCI_VAR_NORM_Y, var_norm_y.raw);
}
...
void _ispblk_rgbtop_cfg_update(struct isp_ctx *ctx, const enum cvi_isp_raw raw_num)
{
	...
	if (ctx->isp_pipe_cfg[raw_num].is_yuv_bypass_path || !ctx->isp_pipe_cfg[raw_num].is_3dnr_enable) { //YUV sensor
		//Disable manr
...
int ispblk_dma_get_size(struct isp_ctx *ctx, int dmaid, const enum cvi_isp_raw raw_num, uint32_t _w, uint32_t _h)
{
	uint32_t len = 0, num = 0, w;
