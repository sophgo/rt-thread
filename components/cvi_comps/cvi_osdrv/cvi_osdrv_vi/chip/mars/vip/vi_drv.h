...
struct _isp_cfg {
	...
	struct isp_switch_info	switch_info;
	...
	enum _VI_PATH_MODE_E	enPathMode;
	enum _COMPRESS_MODE_E	enDumpRaw;
	...
	uint32_t		is_mux			: 1;
	uint32_t		is_mux_switch		: 1;
	uint32_t		is_pipe_enable		: 1;
	uint32_t		is_ctrl_inited		: 1;
	uint32_t		is_3dnr_enable		: 1;
	uint32_t		is_pipe_on		: 1;
	uint32_t		is_frame_valid		: 1;
};
...
struct isp_ctx {
	...
	uint8_t			chnstr_num[ISP_PRERAW_VIRT_MAX];
	uint8_t			total_chn_num;
	rt_atomic_t		is_post_done;
...
/**
 * get rgbmap info from hw register
 *
 * @param [in] ctx pointer to the isp context
 * @param [in] raw_num the number of isp device
 *
 * @return rgbmap info
 */
...
/**
 * get lmap info from hw register
 *
 * @param [in] ctx pointer to the isp context
 * @param [in] raw_num the number of isp device
 *
...
void ispblk_csibdg_update_dpcm(struct isp_ctx *ctx, enum cvi_isp_raw raw_num, bool en);
...
void ispblk_rgbmap_config_dma(struct isp_ctx *ctx, int map_id, bool en, enum cvi_isp_raw raw_num);
