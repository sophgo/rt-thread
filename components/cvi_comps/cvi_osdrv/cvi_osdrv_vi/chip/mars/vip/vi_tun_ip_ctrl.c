	...
	enum cvi_isp_raw raw_num)
{
	...
	int ret = ispblk_clut_tun_cfg(ctx, clut_cfg, raw_num);
...
int vi_tuning_buf_setup(void)
{
	...
	for (i = 0; i < ISP_PRERAW_VIRT_MAX; i++) {
		...
		if (vi_tuning_ptr[i] == NULL) {
			vi_tuning_ptr[i] = calloc(size, 1);
			if (vi_tuning_ptr[i] == NULL) {
				vi_pr(VI_ERR, "tuning_buf ptr[%d] kmalloc size(%d) fail\n", i, size);
				return -ENOMEM;
			}

			phyAddr = (u64)(vi_tuning_ptr[i]);
		}

		if (post_paddr[i] == 0) {
			post_paddr[i] = phyAddr;
			tuning_buf_addr.post_addr[i] = post_paddr[i];
			tuning_buf_addr.post_vir[i] = (void *)(post_paddr[i]);
		}

		if (be_paddr[i] == 0) {
			be_paddr[i] = phyAddr + VI_ALIGN(sizeof(struct cvi_vip_isp_post_cfg));
			tuning_buf_addr.be_addr[i] = be_paddr[i];
			tuning_buf_addr.be_vir[i] = (void *)(be_paddr[i]);
		}
		...
		if (fe_paddr[i] == 0) {
			...
			tuning_buf_addr.fe_vir[i] = (void *)(fe_paddr[i]);
		}

		vi_pr(VI_INFO, "tuning fe_addr[%d]=0x%"PRIx64", be_addr[%d]=0x%"PRIx64", post_addr[%d]=0x%"PRIx64"\n",
...
void vi_tuning_buf_clear(void)
{
	...
	for (i = 0; i < ISP_PRERAW_VIRT_MAX; i++) {
		...
		if (tuning_buf_addr.post_vir[i] != NULL) {
			...
			tun_idx = post_cfg->tun_idx;
			vi_pr(VI_INFO, "Clear post tuning 0x%0x tun_update(%d), tun_idx(%d)\n",
					tuning_buf_addr.post_vir[i], post_cfg->tun_update[tun_idx], tun_idx);
		}
		...
		if (tuning_buf_addr.be_vir[i] != NULL) {
			...
			vi_pr(VI_INFO, "Clear be tuning 0x%0x tun_update(%d), tun_idx(%d)\n",
					tuning_buf_addr.be_vir[i], be_cfg->tun_update[tun_idx], tun_idx);
		}
		...
		if (tuning_buf_addr.fe_vir[i] != NULL) {
			...
			vi_pr(VI_INFO, "Clear fe tuning 0x%0x tun_update(%d), tun_idx(%d)\n",
					tuning_buf_addr.fe_vir[i], fe_cfg->tun_update[tun_idx], tun_idx);
		}
	}
}
	...
	enum cvi_isp_raw raw_num)
{
	...
	if (!ctx->isp_pipe_cfg[raw_num].is_3dnr_enable)
		return;
	{
		struct cvi_vip_isp_ldci_config *ldci_cfg_cur;
		struct cvi_vip_isp_mono_config *mono_cfg_cur;
		static u8 ldci_mode_sts[ISP_PRERAW_VIRT_MAX];
		static u8 mono_mode_sts[ISP_PRERAW_VIRT_MAX];
		uint8_t reset_ip = 0;
		...
		//[2]: TNR
		//[3]: MMAP
		ldci_cfg_cur = &post_tun->ldci_cfg;
		ispblk_ldci_tun_cfg(ctx, ldci_cfg_cur, raw_num);
		if (ldci_cfg_cur->enable != ldci_mode_sts[raw_num])
			reset_ip |= 1 << 1;
		ldci_mode_sts[raw_num] = ldci_cfg_cur->enable;

		mono_cfg_cur = &post_tun->mono_cfg;
		ispblk_mono_tun_cfg(ctx, mono_cfg_cur, raw_num);
		if (mono_cfg_cur->force_mono_enable != mono_mode_sts[raw_num])
			reset_ip |= 1 << 2;
		mono_mode_sts[raw_num] = mono_cfg_cur->force_mono_enable;

		isp_first_frm_reset(ctx, reset_ip);
	}
}
	...
	const enum cvi_isp_raw raw_num)
{
	...
	if (!ctx->is_3dnr_on || !cfg->update || !ctx->isp_pipe_cfg[raw_num].is_3dnr_enable)
		return;
#if 0
	...
	cfg->tnr_2_cfg.REG_70.bits.MMAP_0_GAIN_RATIO_R = 4096;
	cfg->tnr_2_cfg.REG_70.bits.MMAP_0_GAIN_RATIO_G = 4096;
	cfg->tnr_2_cfg.REG_74.bits.MMAP_0_GAIN_RATIO_B = 4096;
