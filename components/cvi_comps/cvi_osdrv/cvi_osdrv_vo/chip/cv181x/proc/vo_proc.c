...
static void _intfType_to_String(CVI_U32 intfType, char *str, int len)
{
	...
	case VO_INTF_HDMI:
		...
		break;
	case VO_INTF_I80_SW:
		strncpy(str, "SW_I80", len);
		break;
	case VO_INTF_I80_HW:
		strncpy(str, "HW_I80", len);
		break;
	default:
...
int proc_vo(CVI_S32 argc, char **argv)
{
	...
	if (!pvoCtx) {
		CVI_TRACE_VO(CVI_VO_ERR, "vo shm = NULL\n");
		return -1;
	}
	...
	for (i = 0; i < VO_MAX_LAYER_NUM; ++i) {
		...
		printf("%8s%2d%10s%20s%10d%10d%10d%10d%10d%10d%10d\n",
				"#",
				i,
	...
	for (i = 0; i < VO_MAX_LAYER_NUM; ++i) {
		...
		printf("%8s%2d%10d%10d%10d%10d%10d%10d%10d\n",
				"#",
				i,
				0,
				cnt,
	...
	for (i = 0; i < VO_MAX_LAYER_NUM; ++i) {
		for (j = 0; j < VO_MAX_CHN_NUM; ++j) {
			...
			printf("%8s%2d%8s%2d%10s%10d%10d%10d%10d%10d%10s\n",
				"#",
				i,
				"#",
				j,
	...
	for (i = 0; i < VO_MAX_LAYER_NUM; ++i) {
		for (j = 0; j < VO_MAX_CHN_NUM; ++j) {
			...
			printf("%8s%2d%8s%2d%10s%10s%10d%10d%20d%20lld%20lld\n",
				"#",
				i,
				"#",
				j,
...
MSH_CMD_EXPORT(proc_vo, vo info);