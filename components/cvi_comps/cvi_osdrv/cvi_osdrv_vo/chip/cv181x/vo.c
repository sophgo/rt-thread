...
static aos_event_t vo_waitsync_event;
static CVI_U8 vo_waitsync_flag = AOS_EVENT_OR_CLEAR;
void (*vo_flush_event_cb)(void);
...
int vo_set_interface(struct cvi_vo_dev *vdev, struct cvi_disp_intf_cfg *cfg)
{
	...
	} else if (cfg->intf_type == CVI_VIP_DISP_INTF_I80_SW) {
		union sclr_bt_enc enc;
		union sclr_bt_sync_code sync;

		sclr_disp_set_intf(SCLR_VO_INTF_I80_SW);
		enc.raw = 0;
		enc.b.fmt_sel = 2;
		enc.b.clk_inv = 1;
		sync.raw = 0;
	...
	} else if (cfg->intf_type == CVI_VIP_DISP_INTF_I80_HW) {
		CVI_TRACE_VO(CVI_VO_INFO, "HW I80 use I80_INIT to control.\n");
		rc = 0;
		return rc;
...
static void _vo_hw_enque(struct cvi_vo_dev *vdev)
{
	...
	if (vdev->disp_interface == CVI_VIP_DISP_INTF_I80_SW) {
		sclr_disp_reg_force_up();
		sclr_i80_run();
	}
}
...
CVI_U32 _MAKECOLOR(CVI_U8 r, CVI_U8 g, CVI_U8 b, enum SW_I80_FORMAT fmt)
{
	CVI_U8 r1, g1, b1;
...
void vo_hwi80_intf_init(struct cvi_vo_dev *vdev, struct cvi_hw_mcu_intf_cfg *cfg)
{
	sclr_disp_set_mcu_disable(cfg->mode);
	sclr_disp_mux_sel(SCLR_VO_SEL_HW_MCU);
	sclr_disp_set_intf(SCLR_VO_INTF_I80_HW);

	if (cfg->mode == MCU_MODE_RGB565) {
		dphy_dsi_set_pll(cfg->pixelclock * 4, 4, 24);
		vip_sys_clk_setting(0x10080);
	} else if (cfg->mode == MCU_MODE_RGB888) {
		dphy_dsi_set_pll(cfg->pixelclock * 6, 4, 24);
		vip_sys_clk_setting(0x100c0);
	}

	_disp_sel_remux(cfg->pins.d_pins, cfg->pins.pin_num);

	hw_mcu_cmd_send(cfg->instrs.instr_cmd, cfg->instrs.instr_num);
	sclr_disp_set_mcu_en(cfg->mode);
}
...
static void *_vo_disp_thread(void *arg)
{
	...
	while (1) {
		...
		while (clock_gettime(CLOCK_REALTIME, &cur_time) == -1)
			continue;
		cur_time.tv_nsec += SEM_WAIT_TIMEOUT_MS * 1000000;
		cur_time.tv_sec += cur_time.tv_nsec / 1000000000;
		cur_time.tv_nsec %= 1000000000;

		ret = sem_timedwait(&jobs->sem, &cur_time);
		if (ret != 0) {
		...
		if (FIFO_SIZE(&jobs->workq) == 1 && (gVoCtx->stPubAttr.enIntfType != VO_INTF_I80_SW) &&
		...
		while (clock_gettime(CLOCK_REALTIME, &cur_time) == -1)
			continue;
		cur_time.tv_nsec += SEM_WAIT_TIMEOUT_MS * 1000 * 1000;
		cur_time.tv_sec += cur_time.tv_nsec / 1000000000;
		cur_time.tv_nsec %= 1000000000;

		ret = sem_timedwait(&vdev->vo_th[th_id].sem, &cur_time);
		if (ret != 0) {
			CVI_TRACE_VO(CVI_VO_WARN, "interrupt time, loop\n");
			continue;
		}
...
static long _vo_s_ctrl(struct cvi_vo_dev *vdev, struct vo_ext_control *p)
{
	...
	case VO_IOCTL_SET_FLUSH_EVENT_CB: {
		vo_flush_event_cb = (void (*)(void)) p->ptr;
		rc = 0;
		break;
	}
	case VO_IOCTL_GET_SHARE_MEM:
		...
		break;
#ifdef __CV180X__
	case VO_IOCTL_GET_WAIT_VSYNC: {
		unsigned int actl_flags;
		vo_waitsync_flag = AOS_EVENT_OR;
		int ret = aos_event_get(&vo_waitsync_event, 0x01, AOS_EVENT_OR_CLEAR,
		 &actl_flags, AOS_WAIT_FOREVER);
		memcpy(p->ptr, &ret, sizeof(ret));
		rc = 0;
	}
#endif
	default:
...
static void _vo_sw_init(struct cvi_vo_dev *vdev)
{
	...
	vdev->num_rdy = 0;
	atomic_set(&vdev->disp_streamon, 0);
	aos_event_new(&vo_waitsync_event, 0);
}
...
int vo_close(void)
{
	...
	aos_event_free(&vo_waitsync_event);
...
{
	aos_event_set(&vo_waitsync_event, 0x01, vo_waitsync_flag);
	if (vo_flush_event_cb != NULL) {
		vo_flush_event_cb();
	}
	...
	if (intr_status.b.disp_frame_end) {
		...
		if ((vdev->num_rdy > 1) || (vdev->disp_interface == CVI_VIP_DISP_INTF_I80_SW)) {
			vo_buf_remove((struct cvi_vo_dev *)vdev);
