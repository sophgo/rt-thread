...
int dphy_dsi_set_lane(u8 lane_num, enum lane_id lane, bool pn_swap, bool clk_phase_shift)
{
    ...
    _reg_write_mask(reg_base + REG_DSI_PHY_LANE_PN_SWAP, (u32)BIT(lane_num), pn_swap << lane_num);
...
void dphy_init(enum sclr_vo_intf intf)
{
    ...
    if ((intf == SCLR_VO_INTF_BT656) || (intf == SCLR_VO_INTF_BT1120) || (intf == SCLR_VO_INTF_I80_SW)
        || (intf == SCLR_VO_INTF_I80_SW))
...
{
    u8   gain             = 1 << (u8)ilog2(MAX(1, 25000000UL / VCORx10000));
    ...
    u64  modifies         = (u64)(factor * loop_gain) << 26;
...
#if 0
	printf("clkkHz(%d) VCORx10000(%d) gain(%d)\n", clkkHz, VCORx10000, gain);
	printf("VCOCx1000(%d) dig_dig(%d) loop_gain(%d)\n", VCOCx1000, dig_dig, loop_gain);
	printf("regs: disp_div_sel(%d), divout_sel(%d), set(%#x)\n", reg_disp_div_sel, reg_divout_sel, *reg_set);
	printf("vip_sy : bt_div(%d)\n", bt_div);
#endif
}
