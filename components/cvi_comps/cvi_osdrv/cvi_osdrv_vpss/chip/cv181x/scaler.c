...
extern void csi_dcache_clean_range(CVI_U64 *addr, CVI_S64 dsize);
...
static spinlock_t __sc_top_lock;
/****************************************************************************
 * Initial info
 ****************************************************************************/
...
void sclr_top_set_cfg(struct sclr_top_cfg *cfg)
{
    ...
    // raw_spin_lock_irqsave(&__sc_top_lock, flags);  //for linux
...
{
    u32 val = ((cfg->sb_wr_ctrl0 << SC_TOP_REG_SB_WR_CTRL0_OFFSET) & SC_TOP_REG_SB_WR_CTRL0_MASK) | ((cfg->sb_wr_ctrl1 << SC_TOP_REG_SB_WR_CTRL1_OFFSET) & SC_TOP_REG_SB_WR_CTRL1_MASK) | ((cfg->sb_rd_ctrl << SC_TOP_REG_SB_RD_CTRL_OFFSET) & SC_TOP_REG_SB_RD_CTRL_MASK);
...
void sclr_update_coef(u8 inst, enum sclr_scaling_coef coef, void *param)
{
    ...
    if (coef == SCL_COEF_BICUBIC)
    {
        for (i = 0; i < 128; ++i)
        {
            _reg_write(reg_base + REG_SCL_COEF1(inst),
                       (scl_coef[i][1] << 16) | (scl_coef[i][0] & 0x0fff));
            _reg_write(reg_base + REG_SCL_COEF2(inst),
                       (scl_coef[i][3] << 16) | (scl_coef[i][2] & 0x0fff));
    ...
    else if (coef == SCL_COEF_NEAREST)
    {
        ...
        {
            _reg_write(reg_base + REG_SCL_COEF1(inst),
                       (nearest_coef[1] << 16) | (nearest_coef[0] & 0x0fff));
            _reg_write(reg_base + REG_SCL_COEF2(inst),
                       (nearest_coef[3] << 16) | (nearest_coef[2] & 0x0fff));
            _reg_write(reg_base + REG_SCL_COEF0(inst), (0x5 << 8) | i);
        }
    }
    else if (coef == SCL_COEF_BILINEAR)
    {
        ...
        for (i = 0; i < 128; ++i)
        {
            ...
            _reg_write(reg_base + REG_SCL_COEF1(inst),
                       (bilinear_coef[1] << 16) | (bilinear_coef[0] & 0x0fff));
            _reg_write(reg_base + REG_SCL_COEF2(inst),
                       (bilinear_coef[3] << 16) | (bilinear_coef[2] & 0x0fff));
            _reg_write(reg_base + REG_SCL_COEF0(inst), (0x5 << 8) | i);
        }
    }
    else if (coef == SCL_COEF_Z2)
    {
        ...
        {
            _reg_write(reg_base + REG_SCL_COEF1(inst),
                       (scl_coef_z2[i][1] << 16) | (scl_coef_z2[i][0] & 0x0fff));
            _reg_write(reg_base + REG_SCL_COEF2(inst),
                       (scl_coef_z2[i][3] << 16) | (scl_coef_z2[i][2] & 0x0fff));
            _reg_write(reg_base + REG_SCL_COEF0(inst), (0x5 << 8) | i);
        }
    }
    else if (coef == SCL_COEF_Z3)
    {
        ...
        {
            _reg_write(reg_base + REG_SCL_COEF1(inst),
                       (scl_coef_z3[i][1] << 16) | (scl_coef_z3[i][0] & 0x0fff));
            _reg_write(reg_base + REG_SCL_COEF2(inst),
                       (scl_coef_z3[i][3] << 16) | (scl_coef_z3[i][2] & 0x0fff));
            _reg_write(reg_base + REG_SCL_COEF0(inst), (0x5 << 8) | i);
        }
    }
    else if (coef == SCL_COEF_DOWNSCALE_SMOOTH)
    {
        ...
        {
            _reg_write(reg_base + REG_SCL_COEF1(inst),
                       (scl_coef_lp[i][1] << 16) | (scl_coef_lp[i][0] & 0x0fff));
            _reg_write(reg_base + REG_SCL_COEF2(inst),
                       (scl_coef_lp[i][3] << 16) | (scl_coef_lp[i][2] & 0x0fff));
            _reg_write(reg_base + REG_SCL_COEF0(inst), (0x5 << 8) | i);
        }
    }
    else if (coef == SCL_COEF_USER)
    {
        ...
        {
            _reg_write(reg_base + REG_SCL_COEF1(inst),
                       (coef[i][1] << 16) | (coef[i][0] & 0x0fff));
            _reg_write(reg_base + REG_SCL_COEF2(inst),
                       (coef[i][3] << 16) | (coef[i][2] & 0x0fff));
            _reg_write(reg_base + REG_SCL_COEF0(inst), (0x5 << 8) | i);
        }
    }
...
void sclr_init(u8 inst)
{
    ...
    {
        CVI_TRACE_VPSS(CVI_DBG_ERR, "[cvi-vip][sc] %s: no enough sclr-instance\n ", __func__);
                  ...
                  bool cir_bypass, bool odma_bypass)
{
    ...
    _reg_write_mask(reg_base + REG_SCL_CFG(inst), 0xf7, g_sc_cfg[inst].raw);
}
...
struct sclr_status sclr_get_status(u8 inst)
{
    ...
    struct sclr_status *status = (struct sclr_status *)&tmp;
...
void sclr_img_set_cfg(u8 inst, struct sclr_img_cfg *cfg)
{
    ...
    _reg_write(reg_base + REG_SCL_IMG_CFG(inst),
               (cfg->csc_en << 12) | (cfg->burst << 8) | (cfg->fmt << 4) | cfg->src);
...
void sclr_img_set_sb(u8 inst, struct sclr_img_in_sb_cfg *cfg)
{
    ...
    val = ((cfg->sb_mode << IMG_IN_REG_SB_MODE_OFFSET) & IMG_IN_REG_SB_MODE_MASK) | ((cfg->sb_size << IMG_IN_REG_SB_SIZE_OFFSET) & IMG_IN_REG_SB_SIZE_MASK) | ((cfg->sb_nb << IMG_IN_REG_SB_NB_OFFSET) & IMG_IN_REG_SB_NB_MASK) | ((cfg->sb_sw_rptr << IMG_IN_REG_SB_SW_RPTR_OFFSET) & IMG_IN_REG_SB_SW_RPTR_MASK) | ((cfg->sb_set_str << IMG_IN_REG_SB_SET_STR_OFFSET) & IMG_IN_REG_SB_SET_STR_MASK) | ((cfg->sb_sw_clr << IMG_IN_REG_SB_SW_CLR_OFFSET) & IMG_IN_REG_SB_SW_CLR_MASK);
...
void sclr_cir_set_cfg(u8 inst, struct sclr_cir_cfg *cfg)
{
    ...
    else
    {
        ...
        _reg_write(reg_base + REG_SCL_CIR_COLOR(inst),
                   (cfg->color_b << 16) | (cfg->color_g << 8) | cfg->color_r);
...
void sclr_border_set_cfg(u8 inst, struct sclr_border_cfg *cfg)
{
    if (cfg->cfg.b.enable)
    {
        if ((g_sc_cfg[inst].sc.dst.w + cfg->start.x) > g_odma_cfg[inst].mem.width)
        {
            CVI_TRACE_VPSS(CVI_DBG_ERR, "[cvi-vip][sc] %s: sc_width(%d) + offset(%d)\n", __func__,
                           g_sc_cfg[inst].sc.dst.w, cfg->start.x);
            CVI_TRACE_VPSS(CVI_DBG_ERR, " > odma_width(%d)\n", g_odma_cfg[inst].mem.width);
            cfg->start.x = g_odma_cfg[inst].mem.width - g_sc_cfg[inst].sc.dst.w;
        }

        if ((g_sc_cfg[inst].sc.dst.h + cfg->start.y) > g_odma_cfg[inst].mem.height)
        {
            CVI_TRACE_VPSS(CVI_DBG_ERR, "[cvi-vip][sc] %s: sc_height(%d) + offset(%d)\n", __func__,
                           g_sc_cfg[inst].sc.dst.h, cfg->start.y);
            CVI_TRACE_VPSS(CVI_DBG_ERR, " > odma_height(%d)\n", g_odma_cfg[inst].mem.height);
...
struct sclr_oenc_cfg *sclr_oenc_get_cfg(void)
{
    ...
    if (oenc_trig.go_intr.b.done)
        CVI_TRACE_VPSS(CVI_DBG_DEBUG, "[cvi-vip][sc] SCLR OSD Compression done!!\n");

    CVI_TRACE_VPSS(CVI_DBG_DEBUG, "[cvi-vip][sc] SCLR OSD Compression INTR vector:");
    if (oenc_trig.go_intr.b.intr_vec & BIT(0))
        CVI_TRACE_VPSS(CVI_DBG_ERR, "Successful!!\n");
    if (oenc_trig.go_intr.b.intr_vec & BIT(2))
        CVI_TRACE_VPSS(CVI_DBG_ERR, "Fail, Bitstream size great than limiter!!\n");
    if (oenc_trig.go_intr.b.intr_vec & BIT(3))
        CVI_TRACE_VPSS(CVI_DBG_ERR, "Fail, Watch Dog time-out!!\n");
    if (oenc_trig.go_intr.b.intr_vec & BIT(4))
        CVI_TRACE_VPSS(CVI_DBG_ERR, "Fail, Out of Dram Write protection region!!\n");
...
void sclr_gop_set_cfg(u8 inst, u8 layer, struct sclr_gop_cfg *cfg, bool update)
{
    if (inst < SCL_MAX_INST)
    {
        ...
        {
            CVI_TRACE_VPSS(CVI_DBG_ERR, "Invalid layer(%d), sc gop only has 2 layer(0 & 1)\n", layer);
            return;
        }
...
int sclr_gop_setup_256LUT(u8 inst, u8 layer, u16 length, u16 *data)
{
	...
	iounmap(vip_clk_reg);
	CVI_TRACE_VPSS(CVI_DBG_ERR, "hw vip clk:%#x\n", vip_pll);
#endif

    CVI_TRACE_VPSS(CVI_DBG_DEBUG, "[cvi-vip][sc]:  inst(%d) layer(%d) length(%d)\n", inst, layer, length);
    gop_cfg = *sclr_gop_get_cfg(inst, layer);
    if (layer == 0)
    {
        CVI_TRACE_VPSS(CVI_DBG_DEBUG, "before update LUT, gop_cfg ctrl:%#x fmt:%#x\n",
    ...
    if (length > 256)
    {
        CVI_TRACE_VPSS(CVI_DBG_ERR, "LUT length(%d) error, should less or equal to 256!\n", length);
        return -1;
    }

    if (inst < SCL_MAX_INST)
    {
        CVI_TRACE_VPSS(CVI_DBG_DEBUG, "[cvi-vip][sc] update 256LUT in gop%d of sc_%d. Length is %d.\n",
                       layer, inst, length);
        if (layer == 0)
        {
            ...
            for (i = 0; i < length; ++i)
            {
                ...
                _reg_write(reg_base + REG_SCL_GOP0_256LUT1(inst), ~BIT(16));
                CVI_TRACE_VPSS(CVI_DBG_DEBUG, "write LUT index:%d value:%#x\n", i, *(data + i));
            }
			...
			for (i = 0; i < length; ++i) {
				...
				_reg_write(reg_base + REG_SCL_GOP0_256LUT1(inst), ~BIT(17));
				CVI_TRACE_VPSS(CVI_DBG_DEBUG, "read LUT index:%d value:%#x\n",
            ...
            CVI_TRACE_VPSS(CVI_DBG_DEBUG, "After update LUT, gop_cfg ctrl:%#x  fmt:%#x\n",
                           _reg_read(reg_base + REG_SCL_GOP0_CFG(inst)),
                           _reg_read(reg_base + REG_SCL_GOP0_FMT(inst, layer)));
        }
        else if (layer == 1)
    ...
    else
    {
        ...
        CVI_TRACE_VPSS(CVI_DBG_DEBUG, "[cvi-vip][sc] update 256LUT in gop1 of display.\n");
        CVI_TRACE_VPSS(CVI_DBG_DEBUG, "layer(%d), sc(%d). Length is %d.\n", layer, inst, length);
        for (i = 0; i < length; ++i)
        {
        ...
        CVI_TRACE_VPSS(CVI_DBG_DEBUG, "After update LUT, gop_cfg ctrl:%#x\n",
...
int sclr_gop_update_256LUT(u8 inst, u8 layer, u16 index, u16 data)
{
    ...
    if (layer == 0)
        CVI_TRACE_VPSS(CVI_DBG_DEBUG, "before update LUT, gop_cfg ctrl:%#x\n",
                       _reg_read(reg_base + REG_SCL_GOP0_CFG(inst)));
    else if (layer == 1)
        CVI_TRACE_VPSS(CVI_DBG_DEBUG, "before update LUT, gop_cfg ctrl:%#x\n",
                       _reg_read(reg_base + REG_SCL_GOP1_CFG(inst)));
    ...
    if (inst < SCL_MAX_INST)
    {
        CVI_TRACE_VPSS(CVI_DBG_DEBUG, "[cvi-vip][sc] update 256LUT in gop%d of sc_%d. Index is %d.\n",
                       layer, inst, index);
        if (layer == 0)
        {
            //Disable OW enable in gop ctrl register
            ...
            //Enable original OW enable in gop ctrl register
            _reg_write(reg_base + REG_SCL_GOP0_CFG(inst), gop_cfg.gop_ctrl.raw);
            CVI_TRACE_VPSS(CVI_DBG_DEBUG, "After upadte LUT, gop_cfg ctrl:%#x\n",
                           _reg_read(reg_base + REG_SCL_GOP0_CFG(inst)));
        }
        else if (layer == 1)
        {
    ...
    else
    {
        ...
        CVI_TRACE_VPSS(CVI_DBG_DEBUG, "[cvi-vip][sc] update 256LUT in gop1 of display.\n");
        CVI_TRACE_VPSS(CVI_DBG_DEBUG, "layer(%d), sc(%d), Index is %d.\n", layer, inst, index);
        _reg_write(reg_base + REG_SCL_DISP_GOP_256LUT0,
                   (index << 16) | data);
        _reg_write(reg_base + REG_SCL_DISP_GOP_256LUT1, BIT(16));
        _reg_write(reg_base + REG_SCL_DISP_GOP_256LUT1, ~BIT(16));
...
int sclr_gop_setup_16LUT(u8 inst, u8 layer, u8 length, u16 *data)
{
    ...
    if (inst < SCL_MAX_INST)
    {
        CVI_TRACE_VPSS(CVI_DBG_DEBUG, "[cvi-vip][sc] update 16LUT in gop%d of sc_%d. Length is %d.\n",
                       layer, inst, length);
        if (layer == 0)
        {
    ...
    else
    {
        ...
        CVI_TRACE_VPSS(CVI_DBG_DEBUG, "[cvi-vip][sc] update 16LUT in gop1 of display. Length is %d.\n", length);
        for (i = 0; i <= length; i += 2)
        {
...
int sclr_gop_update_16LUT(u8 inst, u8 layer, u8 index, u16 data)
{
    ...
    if (inst < SCL_MAX_INST)
    {
        CVI_TRACE_VPSS(CVI_DBG_DEBUG, "[cvi-vip][sc] update 16LUT in gop%d of sc_%d. Index is %d.\n",
                       layer, inst, index);
        if (layer == 0)
        {
            if (index % 2 == 0)
    ...
    else
    {
        ...
        CVI_TRACE_VPSS(CVI_DBG_DEBUG, "[cvi-vip][sc] update 16LUT in gop1 of display. Index is %d.\n", index);
        if (index % 2 == 0)
        {
...
void sclr_gop_ow_set_cfg(u8 inst, u8 layer, u8 ow_inst, struct sclr_gop_ow_cfg *ow_cfg, bool update)
{
    ...
    CVI_TRACE_VPSS(CVI_DBG_DEBUG, "[cvi-vip][sc]: inst:%d layer:%d ow_inst:%d ow_cfg->fmt:%d\n",
                   inst, layer, ow_inst, ow_cfg->fmt);
    if (inst < SCL_MAX_INST)
    {
        if (layer == 0)
        {
...
void sclr_gop_ow_get_addr(u8 inst, u8 layer, u8 ow_inst, u64 *addr)
{
    ...
    if (inst < SCL_MAX_INST)
    {
        if (layer == 0)
        {
            *addr = _reg_read(reg_base + REG_SCL_GOP0_ADDR_L(inst, ow_inst)) | ((u64)_reg_read(reg_base + REG_SCL_GOP0_ADDR_H(inst, ow_inst)) << 32);
        }
        else if (layer == 1)
        {
            *addr = _reg_read(reg_base + REG_SCL_GOP1_ADDR_L(inst, ow_inst)) | ((u64)_reg_read(reg_base + REG_SCL_GOP1_ADDR_H(inst, ow_inst)) << 32);
        }
        else
        {
            CVI_TRACE_VPSS(CVI_DBG_ERR, "[%s]Invalid layer(%d), sc gop only has 2 layer(0 & 1)",
                           __func__, layer);
            return;
        }
    }
    ...
    {
        *addr = _reg_read(reg_base + REG_SCL_DISP_GOP_ADDR_L(ow_inst)) | ((u64)_reg_read(reg_base + REG_SCL_DISP_GOP_ADDR_H(ow_inst)) << 32);
    }
}
...
void sclr_gop_fb_set_cfg(u8 inst, u8 layer, u8 fb_inst, struct sclr_gop_fb_cfg *fb_cfg)
{
    ...
    if (inst < SCL_MAX_INST)
    {
        ...
        else if (layer == 1)
        {
            ...
            CVI_TRACE_VPSS(CVI_DBG_ERR, "[cvi-vip][sc]: only 0 or 1 layer, no such layer(%d).\n", layer);
    }
    else
    {
        sclr_disp_reg_set_shadow_mask(true);
...
{
    if ((rect.y > disp_timing.vfde_end) || (rect.x > disp_timing.hfde_end) || ((disp_timing.vfde_start + rect.y + rect.h - 1) > disp_timing.vfde_end) || ((disp_timing.hfde_start + rect.x + rect.w - 1) > disp_timing.hfde_end))
    {
        CVI_TRACE_VPSS(CVI_DBG_ERR, "[cvi-vip][sc]: me's pos(%d, %d) size(%d, %d).\n",
                       rect.x, rect.y, rect.w, rect.h);
        CVI_TRACE_VPSS(CVI_DBG_ERR, " out of range(%d, %d).\n",
...
{
    _reg_write(reg_base + REG_SCL_DISP_IN_CSC0, BIT(31) | (cfg->coef[0][1] << 16) | (cfg->coef[0][0]));
...
void sclr_disp_set_out_csc(enum sclr_csc csc)
{
    ...
    else if (csc < SCL_CSC_MAX)
    {
        ...
        _reg_write(reg_base + REG_SCL_DISP_OUT_CSC0, BIT(31) | (cfg->coef[0][1] << 16) | (cfg->coef[0][0]));
                           ...
                           enum sclr_disp_pat_color color, const u16 *rgb)
{
    ...
    default:
        CVI_TRACE_VPSS(CVI_DBG_ERR, " - unacceptiable pattern-type(%d)\n", type);
        break;
    }
}
...
void sclr_dsi_clr_mode(void)
{
    ...
    CVI_TRACE_VPSS(CVI_DBG_DEBUG, " mac_en reg(%#x)\n", mode);
...
int sclr_dsi_long_packet_raw(const u8 *data, u8 count)
{
    ...
    CVI_TRACE_VPSS(CVI_DBG_DEBUG, " count(%d)\n", count);
    while (count != 0)
    {
        ...
        _reg_write(reg_base + REG_SCL_DSI_ESC, val | 0x10000);
        CVI_TRACE_VPSS(CVI_DBG_DEBUG, " esc reg(%#x)\n", val);
...
int sclr_dsi_long_packet(u8 di, const u8 *data, u8 count, bool sw_mode)
{
    ...
    if (count > 128 - 6)
    {
        CVI_TRACE_VPSS(CVI_DBG_ERR, " count(%d) invalid\n", count);
        return -1;
    }
...
int sclr_dsi_dcs_write_buffer(u8 di, const void *data, size_t len, bool sw_mode)
{
    if (len == 0)
    {
        CVI_TRACE_VPSS(CVI_DBG_ERR, "[cvi_mipi_tx]: 0 param unacceptable.\n");
        return -1;
    }
    ...
    if ((di == 0x06) || (di == 0x05) || (di == 0x04) || (di == 0x03))
    {
        if (len != 1)
        {
            CVI_TRACE_VPSS(CVI_DBG_ERR, "[cvi_mipi_tx]: cmd(0x%02x) should has 1 param.\n", di);
            return -1;
        }
    ...
    if ((di == 0x15) || (di == 0x37) || (di == 0x13) || (di == 0x14))
    {
        if (len != 2)
        {
            CVI_TRACE_VPSS(CVI_DBG_ERR, "[cvi_mipi_tx]: cmd(0x%02x) should has 2 param.\n", di);
            return -1;
        }
        return sclr_dsi_short_packet(di, data, len, sw_mode);
    }
...
int sclr_dsi_dcs_read_buffer(u8 di, const u16 data_param, u8 *data, size_t len)
{
    ...
    {
        CVI_TRACE_VPSS(CVI_DBG_ERR, "[cvi_mipi_tx]: not work in HS.\n");
        return -1;
    }

    _reg_write(reg_base + REG_SCL_DSI_ESC, 0x10004);
    ...
    {
        CVI_TRACE_VPSS(CVI_DBG_ERR, "[cvi_mipi_tx]: BTA error.\n");
        return ret;
    }
    ...
    case DCS_READ_LP:
        ...
        break;
    case ACK_WR:
        CVI_TRACE_VPSS(CVI_DBG_ERR, "[cvi_mipi_tx]: dcs read, ack with error(%#x %#x).\n", (rx_data >> 8) & 0xff, (rx_data >> 16) & 0xff);
        ret = -1;
        break;
    default:
        CVI_TRACE_VPSS(CVI_DBG_ERR, "[cvi_mipi_tx]: unknown DT, %#x.", rx_data);
        ret = -1;
        break;
    }

    //CVI_TRACE_VPSS(CVI_DBG_DEBUG, " %#x %#x\n", rx_data0, rx_data1);
    return ret;
}
...
void i80_trig(void)
{
    ...
    if (cnt == 10)
        CVI_TRACE_VPSS(CVI_DBG_ERR, "[I80] %s: hw  mcu cmd not ready.\n", __func__);
...
void sclr_i80_packet(u32 cmd)
{
    ...
    if (cnt == 10)
        CVI_TRACE_VPSS(CVI_DBG_ERR, "[cvi_vip]: cmd(%#x) not ready.\n", cmd);
}
...
#define I80_CLEAR_TRAIL        3
    ...
    for (i = 0; i < I80_CLEAR_TRAIL; ++i)
    {
        ...
        CVI_TRACE_VPSS(CVI_DBG_ERR, " waiting idle failed. sw try to clear it.\n");
        _reg_write_mask(reg_base + REG_SCL_DISP_MCU_IF_CTRL, BIT(10), BIT(10));
    }

    CVI_TRACE_VPSS(CVI_DBG_ERR, " failed.\n");
    return false;
}
...
u8 sclr_tile_cal_size(u8 inst, bool is_online_from_isp, struct sc_cfg_cb *post_para)
{
    ...
    else
    {
        ...
        {
            out_l_width           = (((src_l_last_pixel_max - cfg->crop.x - ((cfg->mir_enable) ? 0 : 1)) << 13) / h_sc_fac + 1) & ~0x01;
            L_last_phase          = (out_l_width - 1) * h_sc_fac;
            L_last_pixel          = (L_last_phase >> 13) + ((cfg->mir_enable) ? 0 : 1);
            cfg->tile.src_l_width = L_last_pixel + 2
                                    + ((L_last_phase & 0x1fff) ? 1 : 0);
    ...
    if (g_bd_cfg[inst].cfg.b.enable)
    {
        ...
        if ((g_odma_cfg[inst].flip == SCL_FLIP_HFLIP) || (g_odma_cfg[inst].flip == SCL_FLIP_HVFLIP))
            cfg->tile.dma_r_x = g_odma_cfg[inst].frame_size.w - out_size.w - (g_bd_cfg[inst].start.x & ~0x01);
    ...
    CVI_TRACE_VPSS(CVI_DBG_DEBUG, "L last phase(%d) last pixel(%d)\n", L_last_phase, L_last_pixel);
    CVI_TRACE_VPSS(CVI_DBG_DEBUG, "R first phase(%d) first pixel(%d)\n", R_first_phase, R_first_pixel);
...
void sclr_set_opencv_scale(u8 inst)
{
    ...
    if (!use_2tap)
    {
        ...
        CVI_TRACE_VPSS(CVI_DBG_DEBUG, "h_fac(%#x) v_fac(%#x)\n", h_fac, v_fac);
        CVI_TRACE_VPSS(CVI_DBG_DEBUG, "h_pos(%#x) v_pos(%#x)\n", h_pos, v_pos);
    }
    else
    {
...
void sclr_ctrl_set_scale(u8 inst, struct sclr_scale_cfg *cfg)
{
    if (inst >= SCL_MAX_INST)
    {
        CVI_TRACE_VPSS(CVI_DBG_ERR, "[cvi-vip][sc] %s: no enough sclr-instance ", __func__);
        CVI_TRACE_VPSS(CVI_DBG_ERR, "for the requirement(%d)\n", inst);
        return;
    }
                         ...
                         enum sclr_format fmt)
{
    ...
    if (cfg->mode == SCL_OUT_CSC)
    {
        ...
        if (!IS_YUV_FMT(fmt) || ((cfg->csc_type < SCL_CSC_601_LIMIT_RGB2YUV) && (cfg->csc_type != SCL_CSC_NONE)))
            return -1;
    }
    else
    {
...
int sclr_clk_disable()
{
    u32 temp  = 0;
    temp      = _reg_read(VPSS_CLK_CTRL_REG);
    temp     &= ~VPSS_CLK_CTRL_MASK;
    _reg_write(VPSS_CLK_CTRL_REG, temp);
...
int sclr_img_clk_ctrl(enum sclr_img_in img_type, int onoff)
{
    u32 temp = 0;
    temp     = _reg_read(VPSS_CLK_CTRL_REG);
    if (onoff)
    {
        ...
        {
            temp |= (1 << VPSS_CLK_CTRL_IMGD_BIT);
        }
        else
        {
            temp |= (1 << VPSS_CLK_CTRL_IMGV_BIT);
        }
    }
    else
    {
...
int sclr_sc_clk_ctrl(int inst, int onoff)
{
    u32 temp = 0;
    temp     = _reg_read(VPSS_CLK_CTRL_REG);
    if (onoff)
    {
        if (inst == 0)
        {
            temp |= (1 << VPSS_CLK_CTRL_SCD_BIT);
        }
        else if (inst == 1)
        {
            temp |= (1 << VPSS_CLK_CTRL_SCV1_BIT);
        }
        else if (inst == 2)
        {
            temp |= (1 << VPSS_CLK_CTRL_SCV2_BIT);
        }
        else
        {
            temp |= (1 << VPSS_CLK_CTRL_SCV3_BIT);
        }
    }
    else
    {
...
int sclr_clk_disp_disable()
{
    u32 temp = 0;

    temp  = _reg_read(VO_CLK_CTRL_REG);
    temp &= ~VO_CLK_CTRL_MASK;
    _reg_write(VO_CLK_CTRL_REG, temp);

    return 0;
}

int sclr_clk_disp_enable()
{
    u32 temp = 0;
                     ...
                     u16 cmd_idx)
{
    ...
    cmdQ_set_package(&cmd_start[cmd_idx++].reg, REG_SCL_IMG_CFG(img_inst),
                     (img_cfg.burst << 8) | (img_cfg.fmt << 4) | img_cfg.src);
    ...
    cmdQ_set_package(&cmd_start[cmd_idx++].reg, REG_SCL_IMG_SIZE(img_inst),
                     ((img_cfg.mem.height - 1) << 16) | (img_cfg.mem.width - 1));
    cmdQ_set_package(&cmd_start[cmd_idx++].reg,
                    ...
                    union cmdq_set *cmd_start, u16 cmd_idx)
{
    ...
    cmdQ_set_package(&cmd_start[cmd_idx++].reg, REG_SCL_ODMA_CFG(inst),
                     (odma_cfg.flip << 16) | (odma_cfg.fmt << 8) | odma_cfg.burst);
                           ...
                           u16 cmd_idx)
{
    ...
    cmdQ_set_package(&cmd_start[cmd_idx++].reg,
                     ...
                     (img_cfg.csc_en << 12) | (img_cfg.burst << 8) | (img_cfg.fmt << 4) | img_cfg.src);
    cmdQ_set_package(&cmd_start[cmd_idx++].reg,
...
#endif
#endif
    csi_dcache_clean_range((CVI_U64 *)cmdq_addr, 0x2000);

    cmdQ_intr_ctrl(reg_base + REG_SCL_CMDQ_BASE, 0x02);
    cmdQ_engine(reg_base + REG_SCL_CMDQ_BASE, cmdq_addr,
...
static void _gop_tile_shift(struct sclr_gop_ow_cfg *gop_ow_cfg, u16 left_width, u16 total_width, bool is_right_tile)
{
    s16 byte_offset;
    s8  pixel_offset = 0;
    u8  bpp          = (gop_ow_cfg->fmt == SCL_GOP_FMT_ARGB8888) ? 4 : (gop_ow_cfg->fmt == SCL_GOP_FMT_256LUT) ? 1
                                                                                                               : 2;
    ...
    if (byte_offset)
    {
        ...
        CVI_TRACE_VPSS(CVI_DBG_ERR, "gop tile wa: byte_offset(%d) pixoffset(%d)\n", byte_offset, pixel_offset);
    }
    ...
    CVI_TRACE_VPSS(CVI_DBG_ERR, "gop_tile(%s): addr(%#lx), startx(%d), endx(%d) img_width(%d) mem_width(%d)\n",
...
bool sclr_left_tile(u8 inst, u16 src_l_w)
{
    ...
    CVI_TRACE_VPSS(CVI_DBG_DEBUG, "gop_cfg of layer %d:%#x\n", layer, gop_cfg.gop_ctrl.raw);
...
void sclr_set_sclr_to_vc_sb(u8 inst, u8 sb_mode, u8 sb_size, u8 sb_nb, u8 sb_wr_ctrl_idx)
{
    ...
    if (inst >= SCL_MAX_INST || sb_mode >= 3 || (sb_mode && (sb_size >= 2 || sb_wr_ctrl_idx >= 2)))
    {
...
void sclr_show_core_status(int inst)
{
    ...
    CVI_TRACE_VPSS(CVI_DBG_INFO, "    s2ip_rdy=%d              ip2s_req=%d\n",
                   (val >> 0) & 1, (val >> 16) & 1);
    CVI_TRACE_VPSS(CVI_DBG_INFO, "    sc_aff2d_rdy=%d          d2sc_aff_req=%d\n",
                   (val >> 1) & 1, (val >> 17) & 1);
    CVI_TRACE_VPSS(CVI_DBG_INFO, "    s2crop_rdy=%d            crop2s_req=%d\n",
                   (val >> 2) & 1, (val >> 18) & 1);
    CVI_TRACE_VPSS(CVI_DBG_INFO, "    s2sc_rdy=%d              sc2s_req=%d\n",
                   (val >> 3) & 1, (val >> 19) & 1);
    CVI_TRACE_VPSS(CVI_DBG_INFO, "    s2cir_rdy=%d             cir2s_req=%d\n",
                   (val >> 4) & 1, (val >> 20) & 1);
    CVI_TRACE_VPSS(CVI_DBG_INFO, "    pri_sp_tvalid_i=%d       pri_sp_tready_o=%d\n",
                   (val >> 5) & 1, (val >> 21) & 1);
    CVI_TRACE_VPSS(CVI_DBG_INFO, "    s2border_rdy=%d          border2s_req=%d\n",
                   (val >> 6) & 1, (val >> 22) & 1);
    CVI_TRACE_VPSS(CVI_DBG_INFO, "    front2vgop_rdy=%d        vgop2front_req=%d\n",
                   (val >> 7) & 1, (val >> 23) & 1);
    CVI_TRACE_VPSS(CVI_DBG_INFO, "    front2vgop_b_rdy=%d      vgop_b2front_req=%d\n",
                   (val >> 8) & 1, (val >> 24) & 1);
    CVI_TRACE_VPSS(CVI_DBG_INFO, "    s_tvalid_sl_csc_in=%d    s_tready_sl_csc_in=%d\n",
                   (val >> 9) & 1, (val >> 25) & 1);
    CVI_TRACE_VPSS(CVI_DBG_INFO, "    s2csc_rdy=%d             csc2s_req=%d\n",
                   (val >> 10) & 1, (val >> 26) & 1);
    CVI_TRACE_VPSS(CVI_DBG_INFO, "    s2hsv_rdy=%d             hsv2s_req=%d\n",
                   (val >> 11) & 1, (val >> 27) & 1);
    CVI_TRACE_VPSS(CVI_DBG_INFO, "    s2dma_rdy=%d             dma2s_req=%d\n",
                   (val >> 12) & 1, (val >> 28) & 1);
...
void sclr_dump_odma_register(int inst)
{
    ...
    CVI_TRACE_VPSS(CVI_DBG_DEBUG, "    sc_odma_v_buf_empty=%d, sc_odma_v_buf_full=%d\n",
                   (val >> 4) & 1, (val >> 5) & 1);
    CVI_TRACE_VPSS(CVI_DBG_DEBUG, "    sc_odma_u_buf_empty=%d, sc_odma_u_buf_empty=%d\n",
                   (val >> 6) & 1, (val >> 7) & 1);
    CVI_TRACE_VPSS(CVI_DBG_DEBUG, "    sc_odma_y_buf_empty=%d, sc_odma_y_buf_empty=%d\n",
                   (val >> 8) & 1, (val >> 9) & 1);
    ...
    CVI_TRACE_VPSS(CVI_DBG_DEBUG, "    reg_dma_int_cycle_line_target=%d, reg_dma_int_cycle_line_target_sel=%d\n",
                   ...
                   (val & SC_ODMA_REG_DMA_INT_CYCLE_LINE_TARGET_SEL_MASK) >> SC_ODMA_REG_DMA_INT_CYCLE_LINE_TARGET_SEL_OFFSET);
...
void sclr_show_odma_status(u8 inst)
{
    ...
    CVI_TRACE_VPSS(CVI_DBG_INFO, "    sc_odma_v_buf_empty=%d, sc_odma_v_buf_full=%d\n",
                   (val >> 4) & 1, (val >> 5) & 1);
    CVI_TRACE_VPSS(CVI_DBG_INFO, "    sc_odma_u_buf_empty=%d, sc_odma_u_buf_empty=%d\n",
                   (val >> 6) & 1, (val >> 7) & 1);
    CVI_TRACE_VPSS(CVI_DBG_INFO, "    sc_odma_y_buf_empty=%d, sc_odma_y_buf_empty=%d\n",
                   (val >> 8) & 1, (val >> 9) & 1);
