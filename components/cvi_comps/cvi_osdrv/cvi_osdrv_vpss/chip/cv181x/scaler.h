...
#define IS_YUV_FMT(x) \
    ((x == SCL_FMT_YUV420) || (x == SCL_FMT_YUV422) || (x == SCL_FMT_Y_ONLY) || (x >= SCL_FMT_NV12))
#define IS_PACKED_FMT(x) \
    ((x == SCL_FMT_RGB_PACKED) || (x == SCL_FMT_BGR_PACKED) || (x == SCL_FMT_YVYU) || (x == SCL_FMT_YUYV) || (x == SCL_FMT_VYUY) || (x == SCL_FMT_UYVY))
...
void sclr_engine_cmdq(struct sclr_ctrl_cfg *cfg, u8 cnt, uintptr_t cmdq_addr);
...
int sclr_clk_enable();
/*onoff: 0 for disable, 1 for enable*/
