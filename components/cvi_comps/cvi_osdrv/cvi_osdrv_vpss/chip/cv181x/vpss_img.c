                      ...
                      enum cvi_input_type input_type, enum sclr_input *input)
{
    if (img_type == SCL_IMG_D)
    {
        ...
        *input = (input_type == CVI_VIP_INPUT_ISP || input_type == CVI_VIP_INPUT_ISP_POST) ? SCL_INPUT_ISP : SCL_INPUT_MEM;
    }
    else
    {
        *input = (input_type == CVI_VIP_INPUT_ISP || input_type == CVI_VIP_INPUT_ISP_POST) ? SCL_INPUT_ISP : (input_type == CVI_VIP_INPUT_DWA) ? SCL_INPUT_DWA
                                                                                                                                               : SCL_INPUT_MEM;
    }
...
int cvi_img_streamoff(struct cvi_img_vdev *idev)
{
    int rc = 0, count = 10;
    ...
    if (count == 0)
    {
        ...
        CVI_TRACE_VPSS(CVI_DBG_ERR,
                       "err_fwr_yuv(%d%d%d)\terr_erd_yuv(%d%d%d)\tlb_full_yuv(%d%d%d)\tlb_empty_yuv(%d%d%d)\n", status.b.err_fwr_y, status.b.err_fwr_u, status.b.err_fwr_v, status.b.err_erd_y, status.b.err_erd_u, status.b.err_erd_v, status.b.lb_full_y, status.b.lb_full_u, status.b.lb_full_v, status.b.lb_empty_y, status.b.lb_empty_u, status.b.lb_empty_v);
    ...
    //idev->is_online_from_isp = false;   //only for sys to set and change vpss mode
 ...
 *	General functions
 *************************************************************************/
int img_create_instance(struct cvi_vip_dev *pdev)
{
    int                  rc = 0;
    struct cvi_vip_dev  *bdev;
    ...
    for (i = 0; i < CVI_VIP_IMG_MAX; ++i)
    {
        ...
        idev->is_cmdq            = false;
        idev->IntMask            = 0;
        //idev->frame_number[0] = idev->frame_number[1] = 0;
        ...
        CVI_TRACE_VPSS(CVI_DBG_INFO, "img %d registered \n", i);
    }
...
int img_destroy_instance(struct cvi_vip_dev *pdev)
{
    return 0;
}
...
void img_irq_handler(union sclr_intr intr_status, u8 cmdq_intr_status, struct cvi_vip_dev *bdev)
{
    ...
    for (img_idx = 0; img_idx < CVI_VIP_IMG_MAX; ++img_idx)
    {
        ...
        // check if frame_done
        if (((img_idx == CVI_VIP_IMG_D) && (intr_status.b.img_in_d_frame_end == 0 || bdev->img_vdev[img_idx].is_cmdq == true)) || ((img_idx == CVI_VIP_IMG_V) && (intr_status.b.img_in_v_frame_end == 0)))
            continue;

        idev             = &bdev->img_vdev[img_idx];
        idev->job_flags &= ~(TRANS_RUNNING);
        currentTick      = rt_tick_get();
        idev->ts_end_us  = (CVI_U64)currentTick * (1000000 / RT_TICK_PER_SECOND);
        // clock_gettime(CLOCK_MONOTONIC,&idev->ts_end);
        // idev->ts_end_us = rt_tick_get();
        idev->hw_duration_us = idev->ts_end_us - idev->ts_start_us;
