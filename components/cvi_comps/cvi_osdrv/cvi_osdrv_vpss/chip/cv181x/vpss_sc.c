...
int cvi_sc_buf_queue(struct cvi_sc_vdev *vdev, struct vpss_sc_buffer *list_buf)
{
    ...
    b = malloc(sizeof(struct vpss_sc_buffer));
    if (b == NULL)
    {
...
void cvi_sc_device_run(void *priv, bool is_tile, bool is_work_on_r_tile, u8 grp_id)
{
    ...
    CVI_TRACE_VPSS(CVI_DBG_DEBUG, "update sc%d-buf for grp(%d): 0x%lx-0x%lx-0x%lx\n",
...
int cvi_sc_streamoff(struct cvi_sc_vdev *sdev)
{
    int                 rc = 0, count = 10;
    int                 i    = 0;
 ...
 *	General functions
 *************************************************************************/
int sc_create_instance(struct cvi_vip_dev *pdev)
{
    int                 rc = 0;
    struct cvi_vip_dev *bdev;
    ...
    for (i = 0; i < CVI_VIP_SC_MAX; ++i)
    {
        ...
        sdev->tile_mode = 0;
        sdev->is_cmdq   = false;
        spin_lock_init(&sdev->rdy_lock); //for linux
        atomic_set(&sdev->job_state, CVI_VIP_IDLE);
...
int sc_destroy_instance(struct cvi_vip_dev *pdev)
{
    struct cvi_vip_dev *bdev;
...
void sc_irq_handler(union sclr_intr intr_status, struct cvi_vip_dev *bdev)
{
    ...
    for (i = 0; i < CVI_VIP_SC_MAX; ++i)
    {
        if ((i == 0 && (intr_status.b.scl0_frame_end == 0 || bdev->sc_vdev[i].is_cmdq == true)) || (i == 1 && intr_status.b.scl1_frame_end == 0) || (i == 2 && intr_status.b.scl2_frame_end == 0) || (i == 3 && intr_status.b.scl3_frame_end == 0))
            continue;
...
void cvi_sc_update(struct cvi_sc_vdev *sdev, const struct cvi_vpss_chn_cfg *chn_cfg)
{
        ...
        for (i = 0; i < SCL_MAX_GOP_INST; ++i)
        {
            ...
            if (rgn_cfg->odec.enable)
            {
                ...
                CVI_TRACE_VPSS(CVI_DBG_DEBUG,
                               "ow%d:%11s(%4d)%10s(%4d %4d %4d %4d)%10s(%6d)%10s(0x0x%lx)\n", ow_idx,
                ...
                for (ow_idx = 0; ow_idx < rgn_cfg->num_of_rgn; ++ow_idx)
                    CVI_TRACE_VPSS(CVI_DBG_DEBUG,
                                   "ow%d:%11s(%4d)%10s(%4d %4d %4d %4d)%10s(%4d)%10s(0x%lx)\n", ow_idx,
                                   "fmt", rgn_cfg->param[ow_idx].fmt,
    ...
    {
        sclr_update_coef(sdev->dev_idx, CVI_SC_SCALING_COEF_DOWNSCALE_SMOOTH, NULL);
    }
...
CVI_S32 vpss_sc_qbuf(struct cvi_vip_dev *dev, struct cvi_buffer *buf, MMF_CHN_S chn)
{
    ...
    CVI_TRACE_VPSS(CVI_DBG_DEBUG, "chn(s32DevId=%d, s32ChnId=%d), 0x%lx-0x%lx-0x%lx\n",
    ...
    if (idev->is_online_from_isp)
    {
        ...
        CVI_TRACE_VPSS(CVI_DBG_DEBUG, "sc(%d), grp_id(%d), num_rdy=%d , phy:(%#lx)\n", sdev->dev_idx,
...
CVI_VOID vpss_sc_sb_qbuf(struct cvi_vip_dev *dev, struct cvi_buffer *buf, MMF_CHN_S chn)
{
    ...
    if (buf)
    {
        CVI_TRACE_VPSS(CVI_DBG_DEBUG, "sc(%d), sb enabled 0x%lx-0x%lx-0x%lx\n",
                       sdev->dev_idx,
                       buf->phy_addr[0], buf->phy_addr[1], buf->phy_addr[2]);
