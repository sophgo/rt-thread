...
CVI_VOID vpss_notify_wkup_evt(CVI_U8 u8VpssDev)
{
    ...
    handler_ctx[u8VpssDev].events |= CTX_EVENT_WKUP;
    spin_unlock_irqrestore(&handler_ctx[u8VpssDev].lock, flags_job);

    sem_post(&handler_ctx[u8VpssDev].wait);
}
...
CVI_VOID vpss_notify_isr_evt(CVI_U8 img_idx)
{
    ...
    {
        if (handler_ctx[i].enHdlState == HANDLER_STATE_RUN && handler_ctx[i].img_idx == img_idx)
        {
...
CVI_VOID vpss_notify_vi_err_evt(CVI_U8 img_idx)
{
    ...
    // ret = wait_event_interruptible(vip_dev->vi_reset_wait, vip_dev->reset_done);   //for linux
    // pthread_mutex_lock(&vip_dev->reset_lock);
    // pthread_cond_wait(&vip_dev->vi_reset_wait, &vip_dev->reset_lock);
    // pthread_mutex_unlock(&vip_dev->reset_lock);
    sem_wait(&ctx->vi_reset_wait);

    CVI_TRACE_VPSS(CVI_DBG_WARN, "reset all\n");
}
...
CVI_VOID vpss_print_vb_info(CVI_U8 grp_id, CVI_U8 sc_idx)
{
    ...
    {
        pool_id = _find_vb_pool(vpssCtx[grp_id]->stChnCfgs[chn_id].blk_size);
        if (pool_id == VB_INVALID_POOLID)
        {
    ...
    // hashmapForEach(vbHashmap, _hash_print_cb, &pool_id);
...
#if 0
static CVI_S32 _gdc_begin_job_cb(CVI_U64 *handle)
{
	...
	exe_cb.callee = E_MODULE_LDC;
	exe_cb.caller = E_MODULE_VPSS;
	exe_cb.cmd_id = LDC_CB_BEGIN_JOB;
	exe_cb.data   = &cfg;
...
static CVI_S32 _gdc_end_job_cb(CVI_U64 handle)
{
	struct gdc_handle_data cfg;
	struct base_exe_m_cb exe_cb;
	...
	exe_cb.callee = E_MODULE_LDC;
	exe_cb.caller = E_MODULE_VPSS;
	exe_cb.cmd_id = LDC_CB_END_JOB;
	exe_cb.data   = &cfg;

	return base_exe_module_cb(&exe_cb);
}

static CVI_S32 _gdc_add_rotation_job_cb(CVI_U64 handle,
		GDC_TASK_ATTR_S *pstTask, ROTATION_E enRotation)
{
	struct gdc_task_attr cfg;
	struct base_exe_m_cb exe_cb;

	memset(&cfg, 0, sizeof(cfg));
	cfg.handle = handle;
	memcpy(&cfg.stImgIn, &pstTask->stImgIn, sizeof(cfg.stImgIn));
	memcpy(&cfg.stImgOut, &pstTask->stImgOut, sizeof(cfg.stImgOut));
	cfg.enRotation = enRotation;

	exe_cb.callee = E_MODULE_LDC;
	exe_cb.caller = E_MODULE_VPSS;
	exe_cb.cmd_id = LDC_CB_ADD_ROT_TASK;
	exe_cb.data   = &cfg;

	return base_exe_module_cb(&exe_cb);
}

static VB_BLK _rgb_rotation(ROTATION_E enRotation, struct vb_s *vb)
{
	CVI_S32 s32Ret = 0, i;
	CVI_U64 hHandle;
	GDC_TASK_ATTR_S stTask;
	VB_BLK vbInBlk = (VB_BLK)vb;
	VB_BLK vbOutBlk = VB_INVALID_HANDLE;
	struct vb_s *vb_out;
	CVI_U32 u32BlkSizeOut;
	SIZE_S stSizeOut;
	CVI_U64 u64OutPhyAddr;
	VB_CAL_CONFIG_S stVbOutCalConfig;
	CVI_S32 s16SrcOffsetRight, s16SrcOffsetBottom;
	VIDEO_FRAME_INFO_S *pstFrameIn = &stTask.stImgIn;
	VIDEO_FRAME_INFO_S *pstFrameOut = &stTask.stImgOut;

	stSizeOut.u32Width = ALIGN(vb->buf.size.u32Height, DEFAULT_ALIGN);
	stSizeOut.u32Height = ALIGN(vb->buf.size.u32Width, DEFAULT_ALIGN);
	s16SrcOffsetRight = stSizeOut.u32Height - vb->buf.size.u32Width;
	s16SrcOffsetBottom = stSizeOut.u32Width - vb->buf.size.u32Height;
	...
	*************************************************/
	...
	vbOutBlk = vb_get_block_with_id(VB_INVALID_POOLID, u32BlkSizeOut, CVI_ID_VPSS);
	if (vbOutBlk == VB_INVALID_HANDLE) {
		CVI_TRACE_VPSS(CVI_DBG_ERR, "vb_get_block failed.\n");
		goto EXIT1;
	}

	u64OutPhyAddr = vb_handle2PhysAddr(vbOutBlk);
	if (0 == u64OutPhyAddr) {
		CVI_TRACE_VPSS(CVI_DBG_ERR, "CVI_VB_Handle2PhyAddr fail, u32OutPhyAddr:(%#llx)\n", u64OutPhyAddr);
		goto EXIT2;
	}
	COMMON_GetPicBufferConfig(stSizeOut.u32Width, stSizeOut.u32Height, PIXEL_FORMAT_RGB_888_PLANAR,
		DATA_BITWIDTH_8, COMPRESS_MODE_NONE, DEFAULT_ALIGN, &stVbOutCalConfig);

	vb_out = (struct vb_s *)vbOutBlk;
	vb_out->buf.size.u32Width = stSizeOut.u32Width;
	vb_out->buf.size.u32Height = stSizeOut.u32Height;
	vb_out->buf.enPixelFormat = PIXEL_FORMAT_RGB_888_PLANAR;
	vb_out->buf.phy_addr[0] = u64OutPhyAddr;
	vb_out->buf.stride[0] = stVbOutCalConfig.u32MainStride;
	vb_out->buf.length[0] = stVbOutCalConfig.u32MainYSize;
	vb_out->buf.phy_addr[1] = u64OutPhyAddr + stVbOutCalConfig.u32MainYSize;
	vb_out->buf.stride[1] = stVbOutCalConfig.u32CStride;
	vb_out->buf.length[1] = stVbOutCalConfig.u32MainCSize;
	vb_out->buf.phy_addr[2] = u64OutPhyAddr + stVbOutCalConfig.u32MainYSize + stVbOutCalConfig.u32MainCSize;
	vb_out->buf.stride[2] = stVbOutCalConfig.u32CStride;
	vb_out->buf.length[2] = stVbOutCalConfig.u32MainCSize;
	if (enRotation == ROTATION_90) {
		vb_out->buf.s16OffsetLeft = s16SrcOffsetBottom;
		vb_out->buf.s16OffsetRight = 0;
		vb_out->buf.s16OffsetTop = 0;
		vb_out->buf.s16OffsetBottom = s16SrcOffsetRight;
	} else if (enRotation == ROTATION_270) {
		vb_out->buf.s16OffsetLeft = 0;
		vb_out->buf.s16OffsetRight = s16SrcOffsetBottom;
		vb_out->buf.s16OffsetTop = s16SrcOffsetRight;
		vb_out->buf.s16OffsetBottom = 0;
	}

	memset(&stTask, 0, sizeof(stTask));
	pstFrameIn->stVFrame.enPixelFormat = PIXEL_FORMAT_YUV_400;
	pstFrameIn->stVFrame.u32Width = vb->buf.size.u32Width;
	pstFrameIn->stVFrame.u32Height = vb->buf.size.u32Height;

	pstFrameOut->stVFrame.enPixelFormat = PIXEL_FORMAT_YUV_400;
	pstFrameOut->stVFrame.u32Width = stSizeOut.u32Width;
	pstFrameOut->stVFrame.u32Height = stSizeOut.u32Height;

	for (i = 0; i < 3; i++) {
		pstFrameIn->stVFrame.u64PhyAddr[0] = vb->buf.phy_addr[i];
		pstFrameIn->stVFrame.u32Length[0] = vb->buf.length[i];
		pstFrameIn->stVFrame.u32Stride[0] = vb->buf.stride[i];

		pstFrameOut->stVFrame.u64PhyAddr[0]	= vb_out->buf.phy_addr[i];
		pstFrameOut->stVFrame.u32Stride[0]	= vb_out->buf.stride[i];
		pstFrameOut->stVFrame.u32Length[0]	= vb_out->buf.length[i];

		/************************************************
		 * step2:  Create GDC job
		*************************************************/
		s32Ret = _gdc_begin_job_cb(&hHandle);
		if (s32Ret != CVI_SUCCESS) {
			CVI_TRACE_VPSS(CVI_DBG_ERR, "_gdc_begin_job_cb failed, s32Ret:0x%x", s32Ret);
			goto EXIT2;
		}
		...
		*************************************************/
		...
		s32Ret = _gdc_add_rotation_job_cb(hHandle, &stTask, enRotation);
		if (s32Ret != CVI_SUCCESS) {
			CVI_TRACE_VPSS(CVI_DBG_ERR, "_gdc_add_rotation_job_cb failed, s32Ret:0x%x\n", s32Ret);
			goto EXIT2;
		}

		s32Ret = _gdc_end_job_cb(hHandle);
		if (s32Ret != CVI_SUCCESS) {
			CVI_TRACE_VPSS(CVI_DBG_ERR, "_gdc_end_job_cb failed, s32Ret:0x%x\n", s32Ret);
			goto EXIT2;
		}
	}

	vb_release_block(vbInBlk);
	return vbOutBlk;

EXIT2:
	vb_release_block(vbOutBlk);
...
CVI_VOID vpss_gdc_callback(CVI_VOID *pParam, VB_BLK blk)
{
    ...
    pmesh = &mesh[_pParam->chn.s32DevId][_pParam->chn.s32ChnId];
...
static VPSS_GRP findNextEnGrp(VPSS_GRP workingGrp, CVI_U8 u8VpssDev)
{
    ...
    do {
        ...
        if (vpssCtx[i] && vpssCtx[i]->isCreated && vpssCtx[i]->isStarted && (vpssCtx[i]->u8DevId == u8VpssDev))
        {
            chn.s32DevId = i;
...
static CVI_VOID _vpss_fill_cvi_buffer(MMF_CHN_S chn, struct vb_s *grp_vb_in, CVI_U64 phy_addr, struct cvi_buffer *buf, struct cvi_vpss_ctx *ctx)
{
    SIZE_S   size;
    CVI_BOOL ldc_wa = CVI_FALSE;

    // workaround for ldc 64-align for width/height.
    if (ctx->stChnCfgs[chn.s32ChnId].enRotation != ROTATION_0 || ctx->stChnCfgs[chn.s32ChnId].stLDCAttr.bEnable)
        ldc_wa = CVI_TRUE;
    ...
    base_get_frame_info(ctx->stChnCfgs[chn.s32ChnId].stChnAttr.enPixelFormat, size, buf, phy_addr, ctx->stChnCfgs[chn.s32ChnId].align);
    buf->s16OffsetTop = 0;
    buf->s16OffsetBottom =
        size.u32Height - ctx->stChnCfgs[chn.s32ChnId].stChnAttr.u32Height;
    buf->s16OffsetLeft = 0;
...
static CVI_S32 vpss_qbuf(MMF_CHN_S chn, struct vb_s *grp_vb_in, VB_BLK chn_vb_blk, struct cvi_vpss_ctx *ctx)
{
    VB_BLK             blk = chn_vb_blk;
    struct vb_s       *vb;
    struct cvi_buffer *buf;

    _vpss_fill_cvi_buffer(chn, grp_vb_in, vb_handle2PhysAddr(blk), &((struct vb_s *)blk)->buf, ctx);
...
static CVI_S32 vpss_online_qbuf(MMF_CHN_S chn)
{
    ...
    ctx = handler_ctx[u8VpssDev].online_from_isp ? &handler_ctx[u8VpssDev].vpssCtxWork[chn.s32DevId] : &handler_ctx[u8VpssDev].vpssCtxWork[0];
...
static void release_buffers(VPSS_GRP VpssGrp, struct cvi_vpss_ctx *ctx)
{
    ...
    if (!handler_ctx[ctx->u8DevId].online_from_isp && !vpssExtCtx[VpssGrp].sbm_cfg.sb_mode)
    {
        chn.s32ChnId = 0;
...
static CVI_S32 hw_start(VPSS_GRP VpssGrp, CVI_BOOL onoff, struct cvi_vpss_ctx *ctx)
{
    MMF_CHN_S chn = {.enModId = CVI_ID_VPSS, .s32DevId = VpssGrp, .s32ChnId = 0};
    CVI_S32   dev_idx;
    ...
    else
    {
        ...
        for (VpssChn = 0; VpssChn < ctx->chnNum; ++VpssChn)
        {
            chn.s32ChnId = VpssChn;
            dev_idx      = get_dev_info_by_chn(chn, CHN_TYPE_OUT);
            s32Ret       = cvi_sc_streamoff(&vip_dev->sc_vdev[dev_idx]);
            if (s32Ret != CVI_SUCCESS)
                return s32Ret;
        }

        dev_idx = get_dev_info_by_chn(chn, CHN_TYPE_IN);
        s32Ret  = cvi_img_streamoff(&vip_dev->img_vdev[dev_idx]);
        if (s32Ret != CVI_SUCCESS)
            return s32Ret;
    }
...
static void hw_reset(VPSS_GRP VpssGrp, struct cvi_vpss_ctx *ctx, CVI_BOOL bToggleReset)
{
    ...
    hw_start(VpssGrp, CVI_FALSE, ctx);
}
...
static RECT_S _vpss_get_union_crop(RECT_S cropA, RECT_S cropB)
{
    ...
    // check if no union
    if ((cropA.s32X >= cropB.s32X + (CVI_S32)cropB.u32Width) || (cropA.s32Y >= cropB.s32Y + (CVI_S32)cropB.u32Height) || (cropB.s32X >= cropA.s32X + (CVI_S32)cropA.u32Width) || (cropB.s32Y >= cropA.s32Y + (CVI_S32)cropA.u32Height))
        return cropA;

    union_crop.s32X = (cropA.s32X > cropB.s32X) ? cropA.s32X : cropB.s32X;
    union_crop.s32Y = (cropA.s32Y > cropB.s32Y) ? cropA.s32Y : cropB.s32Y;
    union_crop.u32Width =
        ((cropA.s32X + (CVI_S32)cropA.u32Width) < (cropB.s32X + (CVI_S32)cropB.u32Width)) ? (CVI_U32)(cropA.s32X + (CVI_S32)cropA.u32Width - union_crop.s32X) : (CVI_U32)(cropB.s32X + (CVI_S32)cropB.u32Width - union_crop.s32X);
    union_crop.u32Height =
        ((cropA.s32Y + (CVI_S32)cropA.u32Height) < (cropB.s32Y + (CVI_S32)cropB.u32Height)) ? (CVI_U32)(cropA.s32Y + (CVI_S32)cropA.u32Height - union_crop.s32Y) : (CVI_U32)(cropB.s32Y + (CVI_S32)cropB.u32Height - union_crop.s32Y);
...
static CVI_BOOL _is_frame_crop_changed(VPSS_GRP VpssGrp, struct cvi_vpss_ctx *ctx)
{
    ...
    if ((ctx->s16OffsetLeft != vb_in->buf.s16OffsetLeft) || (ctx->s16OffsetTop != vb_in->buf.s16OffsetTop) || (ctx->s16OffsetRight != vb_in->buf.s16OffsetRight) || (ctx->s16OffsetBottom != vb_in->buf.s16OffsetBottom))
    {
    ...
    {
        CVI_BOOL chk_width_even  = IS_FMT_YUV420(ctx->stGrpAttr.enPixelFormat) || IS_FMT_YUV422(ctx->stGrpAttr.enPixelFormat);
...
{
    return (ctx->frame_crop.end_x > ctx->frame_crop.start_x && ctx->frame_crop.end_y > ctx->frame_crop.start_y && ctx->frame_crop.end_x <= ctx->stGrpAttr.u32MaxW && ctx->frame_crop.end_y <= ctx->stGrpAttr.u32MaxH && !((CVI_U32)(ctx->frame_crop.end_x - ctx->frame_crop.start_x) == ctx->stGrpAttr.u32MaxW && (CVI_U32)(ctx->frame_crop.end_y - ctx->frame_crop.start_y) == ctx->stGrpAttr.u32MaxH));
}

static void _vpss_over_crop_resize(struct VPSS_GRP_HW_CFG *pstGrpHwCfg, RECT_S crop_rect, RECT_S *resize_rect)
{
    ...
    CVI_TRACE_VPSS(CVI_DBG_DEBUG, "rect before resize(%d, %d, %d, %d)\n", resize_rect->s32X, resize_rect->s32Y, resize_rect->u32Width, resize_rect->u32Height);
    ...
    if (crop_rect.s32X + crop_rect.u32Width > pstGrpHwCfg->rect_crop.width)
    {
        ...
        //	/ (crop_rect.u32Width);
        // resize_rect->u32Width -= (CVI_U32)(resize_rect->u32Width * ratio + 0.5);
        ratio                  = (crop_rect.s32X + crop_rect.u32Width - pstGrpHwCfg->rect_crop.width) * (crop_rect.u32Height);
        resize_rect->u32Width -= (CVI_U32)(resize_rect->u32Width * ratio + scale / 2) / scale;
    }
    ...
    if (crop_rect.s32Y + crop_rect.u32Height > pstGrpHwCfg->rect_crop.height)
    {
        ...
        //	/ (crop_rect.u32Height);
        // resize_rect->u32Height -= (CVI_U32)(resize_rect->u32Height * ratio + 0.5);
        ratio                   = (crop_rect.s32Y + crop_rect.u32Height - pstGrpHwCfg->rect_crop.height) * (crop_rect.u32Width);
        resize_rect->u32Height -= (CVI_U32)(resize_rect->u32Height * ratio + scale / 2) / scale;
    }
                             ...
                             struct VPSS_GRP_HW_CFG *pstGrpHwCfg, bool online_from_isp)
{
    ...
    COMMON_GetPicBufferConfig(stChnCfg->stChnAttr.u32Width, stChnCfg->stChnAttr.u32Height,
                              stChnCfg->stChnAttr.enPixelFormat, DATA_BITWIDTH_8, COMPRESS_MODE_NONE, stChnCfg->align, &stVbCalConfig);
    ...
    {
        pstHwCfg->rect_crop.left = pstHwCfg->rect_crop.top = chnCrop.s32X = chnCrop.s32Y = 0;
    ...
    CVI_TRACE_VPSS(CVI_DBG_INFO, "grp(%d) chn(%d) rect(%d %d %d %d)\n", VpssGrp, VpssChn, pstHwCfg->rect_crop.left, pstHwCfg->rect_crop.top, pstHwCfg->rect_crop.width, pstHwCfg->rect_crop.height);
    ...
    if (stChnCfg->stChnAttr.stAspectRatio.enMode == ASPECT_RATIO_AUTO)
    {
        ...
        is_border_enabled = stChnCfg->stChnAttr.stAspectRatio.bEnableBgColor && ((rect.u32Width != stChnCfg->stChnAttr.u32Width) || (rect.u32Height != stChnCfg->stChnAttr.u32Height));
        ...
        if (IS_FMT_YUV420(stChnCfg->stChnAttr.enPixelFormat) || IS_FMT_YUV422(stChnCfg->stChnAttr.enPixelFormat))
        {
        ...
        pstHwCfg->rect_output.height * VPSS_MAX_ZOOMOUT < pstHwCfg->rect_crop.height)
    {
        ...
        CVI_TRACE_VPSS(CVI_DBG_ERR, "Modify to sc in(w:%d, h:%d), sc out(w:%d, h:%d)\n",
                       pstHwCfg->rect_crop.width, pstHwCfg->rect_crop.height, pstHwCfg->rect_output.width, pstHwCfg->rect_output.height);
    }
    ...
    if (stChnCfg->stChnAttr.stNormalize.bEnable)
    {
        ...
        pstHwCfg->quantCfg.rounding = stChnCfg->stChnAttr.stNormalize.rounding;
    }
    else
    {
#if 0
        ...
        CVI_TRACE_VPSS(CVI_DBG_DEBUG, "coef[0][0]: %#4x coef[0][1]: %#4x coef[0][2]: %#4x\n", pstHwCfg->csc_cfg.coef[0][0], pstHwCfg->csc_cfg.coef[0][1], pstHwCfg->csc_cfg.coef[0][2]);
        CVI_TRACE_VPSS(CVI_DBG_DEBUG, "coef[1][0]: %#4x coef[1][1]: %#4x coef[1][2]: %#4x\n", pstHwCfg->csc_cfg.coef[1][0], pstHwCfg->csc_cfg.coef[1][1], pstHwCfg->csc_cfg.coef[1][2]);
        CVI_TRACE_VPSS(CVI_DBG_DEBUG, "coef[2][0]: %#4x coef[2][1]: %#4x coef[2][2]: %#4x\n", pstHwCfg->csc_cfg.coef[2][0], pstHwCfg->csc_cfg.coef[2][1], pstHwCfg->csc_cfg.coef[2][2]);
        CVI_TRACE_VPSS(CVI_DBG_DEBUG, "sub[0]: %3d sub[1]: %3d sub[2]: %3d\n", pstHwCfg->csc_cfg.sub[0], pstHwCfg->csc_cfg.sub[1], pstHwCfg->csc_cfg.sub[2]);
        CVI_TRACE_VPSS(CVI_DBG_DEBUG, "add[0]: %3d add[1]: %3d add[2]: %3d\n", pstHwCfg->csc_cfg.add[0], pstHwCfg->csc_cfg.add[1], pstHwCfg->csc_cfg.add[2]);
    }
...
void _vpss_grp_hw_cfg_update(VPSS_GRP VpssGrp, struct cvi_vpss_ctx *ctx, struct VPSS_GRP_HW_CFG *pstGrpHwCfg)
{
    ...
    COMMON_GetPicBufferConfig(ctx->stGrpAttr.u32MaxW, ctx->stGrpAttr.u32MaxH,
                              ctx->stGrpAttr.enPixelFormat, DATA_BITWIDTH_8, COMPRESS_MODE_NONE, DEFAULT_ALIGN, &stVbCalConfig);
    ...
    else
    {
        ...
        if (ctx->stGrpCropInfo.bEnable)
        {
            ...
            pstGrpHwCfg->rect_crop.left   = ctx->stGrpCropInfo.stCropRect.s32X + ctx->s16OffsetLeft;
            pstGrpHwCfg->rect_crop.top    = ctx->stGrpCropInfo.stCropRect.s32Y + ctx->s16OffsetTop;
    ...
    CVI_TRACE_VPSS(CVI_DBG_INFO, "grp(%d) Offset(left:%d top:%d right:%d bottom:%d) rect(%d %d %d %d)\n", VpssGrp, ctx->s16OffsetLeft, ctx->s16OffsetTop, ctx->s16OffsetRight, ctx->s16OffsetBottom, pstGrpHwCfg->rect_crop.left, pstGrpHwCfg->rect_crop.top, pstGrpHwCfg->rect_crop.width, pstGrpHwCfg->rect_crop.height);

    memcpy(&pstGrpHwCfg->csc_cfg, &vpssExtCtx[VpssGrp].csc_cfg, sizeof(pstGrpHwCfg->csc_cfg));

    CVI_TRACE_VPSS(CVI_DBG_DEBUG, "coef[0][0]: %#4x coef[0][1]: %#4x coef[0][2]: %#4x\n",
                                   pstGrpHwCfg->csc_cfg.coef[0][0], pstGrpHwCfg->csc_cfg.coef[0][1], pstGrpHwCfg->csc_cfg.coef[0][2]);
    CVI_TRACE_VPSS(CVI_DBG_DEBUG, "coef[1][0]: %#4x coef[1][1]: %#4x coef[1][2]: %#4x\n",
                                   pstGrpHwCfg->csc_cfg.coef[1][0], pstGrpHwCfg->csc_cfg.coef[1][1], pstGrpHwCfg->csc_cfg.coef[1][2]);
    CVI_TRACE_VPSS(CVI_DBG_DEBUG, "coef[2][0]: %#4x coef[2][1]: %#4x coef[2][2]: %#4x\n",
                                   pstGrpHwCfg->csc_cfg.coef[2][0], pstGrpHwCfg->csc_cfg.coef[2][1], pstGrpHwCfg->csc_cfg.coef[2][2]);
    CVI_TRACE_VPSS(CVI_DBG_DEBUG, "sub[0]: %3d sub[1]: %3d sub[2]: %3d\n",
                                   pstGrpHwCfg->csc_cfg.sub[0], pstGrpHwCfg->csc_cfg.sub[1], pstGrpHwCfg->csc_cfg.sub[2]);
    CVI_TRACE_VPSS(CVI_DBG_DEBUG, "add[0]: %3d add[1]: %3d add[2]: %3d\n",
                                   pstGrpHwCfg->csc_cfg.add[0], pstGrpHwCfg->csc_cfg.add[1], pstGrpHwCfg->csc_cfg.add[2]);
}
...
static void _notify_dwa_sbm_cfg_done(VPSS_GRP VpssGrp)
{
    ...
    exe_cb.callee = E_MODULE_AVS;
    exe_cb.caller = E_MODULE_VPSS;
    exe_cb.cmd_id = LDC_CB_VPSS_SBM_DONE;
    exe_cb.data   = NULL;
...
static CVI_S32 commitHWSettings(VPSS_GRP VpssGrp, bool online_from_isp, struct cvi_vpss_ctx *ctx)
{
    ...
    for (VpssChn = 0; VpssChn < ctx->chnNum; ++VpssChn)
    {
        ...
        CVI_TRACE_VPSS(CVI_DBG_DEBUG, "grp(%d) chn(%d) size(%d %d) rect(%d %d %d %d)\n",
                       VpssGrp, VpssChn, stChnCfg->stChnAttr.u32Width, stChnCfg->stChnAttr.u32Height,
		...
		if (vo_ctx.is_dev_enable[0] && (vo_ctx.fb_on_vpss || (vo_ctx.stPubAttr.enIntfType == VO_INTF_I80)))
			if (CVI_SYS_GetBindbySrc(&chn, &stBindDest) == CVI_SUCCESS) {
				for (CVI_U32 i = 0; i < stBindDest.u32Num; ++i)
					if (stBindDest.astMmfChn[i].enModId == CVI_ID_VO) {
						bind_fb = CVI_TRUE;
						CVI_TRACE_VPSS(CVI_DBG_INFO,
							"grp(%d) chn(%d) use fb's gop. fb_on_vpss(%d) Vo-Intf(%d)\n",
							VpssGrp, VpssChn, vo_ctx.fb_on_vpss,
							vo_ctx.stPubAttr.enIntfType);
						break;
					}
			}
		sc_set_vpss_chn_bind_fb(d->fd, bind_fb);
#endif
    }
    return CVI_SUCCESS;
}
...
static CVI_VOID _update_vpss_chnRealFrameRate(void)
{
    ...
    rt_tick_t       currentTick;

    currentTick = rt_tick_get();
    curTimeUs   = (CVI_U64)currentTick * (1000000 / RT_TICK_PER_SECOND);
        ...
        if (vpssCtx[i] && vpssCtx[i]->isCreated)
        {
            for (j = 0; j < vpssCtx[i]->chnNum; ++j)
            {
                if (vpssCtx[i]->stChnCfgs[j].isEnabled)
                {
                    ...
                    if (duration >= 1000000)
                    {
                        vpssCtx[i]->stChnCfgs[j].stChnWorkStatus.u32RealFrameRate = vpssCtx[i]->stChnCfgs[j].stChnWorkStatus.u32FrameNum;
...
static CVI_VOID _vpss_chl_frame_rate_ctrl(struct cvi_vpss_ctx *ctx)
{
    ...
    for (VpssChn = 0; VpssChn < ctx->chnNum; ++VpssChn)
    {
        if (!ctx->stChnCfgs[VpssChn].isEnabled || !ctx->stChnCfgs[VpssChn].stChnWorkStatus.u32SendOk || ctx->stChnCfgs[VpssChn].is_cfg_changed)
            continue;
...
static CVI_VOID _update_vpss_grp_proc(VPSS_GRP VpssGrp, CVI_U32 duration, CVI_U32 HwDuration)
{
    ...
    if (vpssCtx[VpssGrp]->stGrpWorkStatus.u32MaxCostTime < vpssCtx[VpssGrp]->stGrpWorkStatus.u32CostTime)
    {
        vpssCtx[VpssGrp]->stGrpWorkStatus.u32MaxCostTime = vpssCtx[VpssGrp]->stGrpWorkStatus.u32CostTime;
    }
    vpssCtx[VpssGrp]->stGrpWorkStatus.u32HwCostTime = HwDuration;
    if (vpssCtx[VpssGrp]->stGrpWorkStatus.u32HwMaxCostTime < vpssCtx[VpssGrp]->stGrpWorkStatus.u32HwCostTime)
    {
        vpssCtx[VpssGrp]->stGrpWorkStatus.u32HwMaxCostTime = vpssCtx[VpssGrp]->stGrpWorkStatus.u32HwCostTime;
    }
}
...
static CVI_BOOL _vpss_check_gdc_job(MMF_CHN_S chn, VB_BLK blk, struct cvi_vpss_ctx *vpss_ctx)
{
    ...
    if (atomic_cmpxchg(&pmesh->gdc_flag, 0, 1) == 0)
    {
        // why use handler_ctx->vpssCtxWork? Replace with vpssCtx
        vpss_ctx = vpssCtx[chn.s32DevId];
        if (vpss_ctx->stChnCfgs[chn.s32ChnId].stLDCAttr.bEnable)
        {
            ...
            if (_mesh_gdc_do_op_cb(GDC_USAGE_LDC,
                ...
                vb, vpss_ctx->stChnCfgs[chn.s32ChnId].stChnAttr.enPixelFormat,
                pmesh->paddr, CVI_FALSE, &cb_param, sizeof(cb_param), CVI_ID_VPSS,
...
#if 0
			if (vpss_ctx->stChnCfgs[chn.s32ChnId].stChnAttr.enPixelFormat == PIXEL_FORMAT_RGB_888_PLANAR) {
				VB_BLK rgb_blk_out;
				rgb_blk_out = _rgb_rotation(vpss_ctx->stChnCfgs[chn.s32ChnId].enRotation, vb);
				if (rgb_blk_out == VB_INVALID_HANDLE) {
					//atomic_set(&pmesh->gdc_flag, 0);
					CVI_TRACE_VPSS(CVI_DBG_ERR, "rgb gdc rotation failed.\n");
					return CVI_FALSE;
				} else {
					vb_done_handler(chn, CHN_TYPE_OUT, rgb_blk_out);
					atomic_set(&pmesh->gdc_flag, 0);
				}
			} else if (_mesh_gdc_do_op_cb(GDC_USAGE_ROTATION
				, NULL
				, vb
...
#else
            if (_mesh_gdc_do_op_cb(GDC_USAGE_ROTATION, NULL, vb, vpss_ctx->stChnCfgs[chn.s32ChnId].stChnAttr.enPixelFormat, pmesh->paddr, CVI_FALSE, &cb_param, sizeof(cb_param), CVI_ID_VPSS, vpss_ctx->stChnCfgs[chn.s32ChnId].enRotation) != CVI_SUCCESS)
            {
                // atomic_set(&pmesh->gdc_flag, 0);
                CVI_TRACE_VPSS(CVI_DBG_ERR, "gdc rotation failed.\n");
...
static CVI_BOOL _is_vc_sbm_enabled(VPSS_GRP VpssGrp, struct cvi_vpss_ctx *ctx)
{
    ...
    for (VpssChn = 0; VpssChn < ctx->chnNum; ++VpssChn)
    {
        if (ctx->stChnCfgs[VpssChn].isEnabled && ctx->stChnCfgs[VpssChn].stBufWrap.bEnable)
            return CVI_TRUE;
    }
                                  ...
                                  CVI_U32 cmd_id, CVI_BOOL online_from_isp)
{
    ...
    for (VpssChn = 0; VpssChn < ctx->chnNum; ++VpssChn)
    {
        ...
        if (base_get_bindbysrc(pchn, &stBindDest) == CVI_SUCCESS)
        {
...
static CVI_U64 get_diff_time_us(CVI_U64 t1, CVI_U64 t2)
{
    CVI_U64 diff_time;
...
static CVI_S32 vpss_try_schedule_online(struct vpss_handler_ctx *ctx)
{
    ...
    for (workingGrp = 0; workingGrp < VPSS_ONLINE_NUM; workingGrp++)
    {
        ...
        pthread_mutex_lock(&vpssCtx[workingGrp]->lock);
        ...
        if (commitHWSettings(workingGrp, ctx->online_from_isp, vpss_ctx) != CVI_SUCCESS)
        {
            ...
            pthread_mutex_unlock(&vpssCtx[workingGrp]->lock);
            break;
        }
        ...
        if (!(vpssExtCtx[workingGrp].grp_state & GRP_STATE_HW_STARTED))
        {
            ...
            if (vc_sbm_enabled)
            {
                ...
                if (_notify_vc_sb_mode(workingGrp, vpss_ctx, VCODEC_CB_SEND_FRM, CVI_TRUE) != CVI_SUCCESS)
                {
                    pthread_mutex_unlock(&vpssCtx[workingGrp]->lock);
                    ...
                    hw_start(workingGrp, CVI_TRUE, vpss_ctx);
                    break;
                }
                fill_sb_buffers(workingGrp, vpss_ctx, true);
            }

            if (hw_start(workingGrp, CVI_TRUE, vpss_ctx) != CVI_SUCCESS)
            {
...
static CVI_VOID vpss_handle_online_frame_done(struct vpss_handler_ctx *ctx, VPSS_GRP workingGrp)
{
    ...
    do {
        if (vpss_ctx->stChnCfgs[VpssChn].stBufWrap.bEnable && vpss_ctx->stChnCfgs[VpssChn].bufWrapPhyAddr)
        {
        ...
        vb = (struct vb_s *)blk;
        CVI_TRACE_VPSS(CVI_DBG_DEBUG, "grp(%d) chn(%d) dqbuf phy(%#lx)\n",
                       workingGrp, VpssChn, (uint64_t)((struct vb_s *)blk)->phy_addr);
        ...
        pts_time = vip_dev->img_vdev[ctx->img_idx].ts_end;
        u64PTS   = pts_time.tv_sec * 1000000.0 + pts_time.tv_nsec / 1000.0;
        ...
        //	pthread_mutex_lock(&handler_ctx[0].rgnex_jobs.lock);
        ...
        //		pthread_mutex_unlock(&handler_ctx[0].rgnex_jobs.lock);
        //		CVI_VPSS_PostJob(0);
    ...
    // job done.
    time_us       = rt_tick_get() * 1000000 / RT_TICK_PER_SECOND;
    duration      = time_us - ctx->time_us;
    HwDuration_us = vip_dev->img_vdev[ctx->img_idx].hw_duration_us;

    // Update vpss proc info
    _update_vpss_grp_proc(workingGrp, duration, HwDuration_us);
}
...
static CVI_VOID vpss_handle_online(struct vpss_handler_ctx *ctx)
{
    ...
    if (events & CTX_EVENT_VI_ERR)
    {
        ...
        CVI_TRACE_VPSS(CVI_DBG_ERR, "img(%d:%d:%d) grp(%d) events(%d) vi err\n",
                       dev_idx,
                       vip_dev->img_vdev[dev_idx].img_type,
                       vip_dev->img_vdev[dev_idx].input_type,
                       workingGrp, events);
        ...
        hw_start(workingGrp, CVI_TRUE, vpss_ctx);
        // pthread_mutex_lock(&vip_dev->reset_lock);
        // pthread_cond_signal(&vip_dev->vi_reset_wait);
        // pthread_mutex_unlock(&vip_dev->reset_lock);
        CVI_TRACE_VPSS(CVI_DBG_WARN, "vpss reset done\n");
        sem_post(&ctx->vi_reset_wait);
        // return;
    }
    ...
    {
        time_us       = rt_tick_get() * 1000000 / RT_TICK_PER_SECOND;
        duration64_us = get_diff_time_us(time_us, ctx->time_us);
        duration64_us = duration64_us / 1000;

        CVI_TRACE_VPSS(CVI_DBG_DEBUG, "ctxVpssDev[%d] event=0x%x, workingGrp(%d) state=0x%x vpssCtx[%d].isStarted:%d  \n",
                       ctx->u8VpssDev, events, workingGrp,
                       vpssExtCtx[workingGrp].grp_state, workingGrp, vpssCtx[workingGrp]->isStarted);
        ...
        {
            if (duration64_us > EOF_WAIT_TIMEOUT_MS)
            {
                /* timeout */
                if (duration64_us > EOF_WAIT_TIMEOUT_MS && duration64_us < (2 * EOF_WAIT_TIMEOUT_MS))
                {
                    CVI_TRACE_VPSS(CVI_DBG_NOTICE, "event timeout on grp(%d)\n", workingGrp);
                    sclr_check_register();
                }
            }
            else
    ...
    for (i = 0; i < VPSS_ONLINE_NUM; i++)
    {
        if (vpssCtx[i] && vpssCtx[i]->isStarted && !(vpssExtCtx[i].grp_state & GRP_STATE_BUF_FILLED))
        {
            ...
            ctx->time_us = rt_tick_get() * 1000000 / RT_TICK_PER_SECOND;
            break;
        }
    }

    CVI_TRACE_VPSS(CVI_DBG_DEBUG, "ctx[%d] evt=0x%x, Grp(%d) state 0x%x->0x%x\n",
                   ctx->u8VpssDev, ctx->events, workingGrp, state,
                   vpssExtCtx[workingGrp].grp_state);
}
...
static CVI_S32 vpss_try_schedule_offline(struct vpss_handler_ctx *ctx)
{
    ...
    if (hw_start(workingGrp, CVI_FALSE, vpss_ctx) != CVI_SUCCESS)
    {
...
static void vpss_handle_offline(struct vpss_handler_ctx *ctx)
{
    ...
    CVI_TRACE_VPSS(CVI_DBG_DEBUG, "ctx[%d] evt=0x%x, mask=0x%x, Grp(%d) state 0x%x->0x%x\n",
                   ctx->u8VpssDev, events, ctx->workingMask, workingGrp, state,
                   vpssExtCtx[workingGrp].grp_state);
}
...
void *vpss_event_handler(void *arg)
{
    struct vpss_handler_ctx *ctx     = (struct vpss_handler_ctx *)arg;
    CVI_U64                  timeout = IDLE_TIMEOUT_MS;
    ...
    while (ctx->v_thread)
    {
        ...
        {
            /* Adjust timeout */
            timeout = vpss_handler_is_idle(ctx->u8VpssDev) ? IDLE_TIMEOUT_MS : EOF_WAIT_TIMEOUT_MS;

            current_tick      = rt_tick_get();
            tick_per_second   = RT_TICK_PER_SECOND;
            cond_time.tv_sec  = current_tick / tick_per_second;
            cond_time.tv_nsec = (current_tick % tick_per_second) * (1000000000 / tick_per_second);

            timeout_s  = timeout / 1000;
            timeout   %= 1000;

            cond_time.tv_nsec += timeout * 1000000;               // 毫秒转换为纳秒
            cond_time.tv_sec  += timeout_s + (cond_time.tv_nsec / 1000000000L);
            cond_time.tv_nsec  = cond_time.tv_nsec % 1000000000L; // 确保纳秒在有效范围内

            ret = sem_timedwait(&ctx->wait, &cond_time);
            ...
            // pthread_mutex_lock(&vip_dev->wait_lock);
            // pthread_cond_timedwait(&vip_dev->wait, &vip_dev->wait_lock, &cond_time);
            // pthread_mutex_unlock(&vip_dev->wait_lock);
            ret = sem_timedwait(&ctx->wait, &cond_time);
#endif
        }
        ...
        /* timeout */
        if (ret < 0 && vpss_handler_is_idle(ctx->u8VpssDev))
            continue;
        ...
        else
        {
            ...
            if (!ctx->workingMask)
            {
                ctx->workingGrp = findNextEnGrp(ctx->workingGrp, ctx->u8VpssDev);
...
void _os_vpss_GrpParamInit(VPSS_GRP VpssGrp)
{
    CVI_U8 i, j, k;
    ...
    for (i = 0; i < vpssCtx[VpssGrp]->chnNum; ++i)
    {
        ...
        atomic_set(&mesh[VpssGrp][i].gdc_flag, 0);
...
static CVI_S32 _vpss_update_rotation_mesh(VPSS_GRP VpssGrp, VPSS_CHN VpssChn, ROTATION_E enRotation)
{
    ...
    pthread_mutex_lock(&vpssCtx[VpssGrp]->lock);
    vpssCtx[VpssGrp]->stChnCfgs[VpssChn].enRotation = enRotation;
    pthread_mutex_unlock(&vpssCtx[VpssGrp]->lock);
    return CVI_SUCCESS;
}

CVI_S32 _os_vpss_update_ldc_mesh(VPSS_GRP VpssGrp, VPSS_CHN VpssChn,
                            ...
                            const VPSS_CHN_ATTR_S *pstChnAttrOld, const VPSS_CHN_ATTR_S *pstChnAttrNew)
{
    ...
    if ((pstChnAttrOld->u32Width != pstChnAttrNew->u32Width) || (pstChnAttrOld->u32Height != pstChnAttrNew->u32Height) || (pstChnAttrOld->enVideoFormat != pstChnAttrNew->enVideoFormat) || (pstChnAttrOld->enPixelFormat != pstChnAttrNew->enPixelFormat) || (pstChnAttrOld->bMirror != pstChnAttrNew->bMirror) || (pstChnAttrOld->bFlip != pstChnAttrNew->bFlip) || memcmp(&pstChnAttrOld->stAspectRatio, &pstChnAttrNew->stAspectRatio, sizeof(ASPECT_RATIO_S)) || memcmp(&pstChnAttrOld->stNormalize, &pstChnAttrNew->stNormalize, sizeof(VPSS_NORMALIZE_S)))
    {
 ...
 */
void vpss_post_job(CVI_S32 dev_id)
{
    VPSS_GRP VpssGrp = (VPSS_GRP)dev_id;
    CVI_S32  ret;
 ...
 *   Public APIs.
 **************************************************************************/
CVI_S32 _os_vpss_create_grp(VPSS_GRP VpssGrp, const VPSS_GRP_ATTR_S *pstGrpAttr)
{
    ...
    {
        if ((stVIVPSSMode.aenMode[VpssGrp] == VI_OFFLINE_VPSS_ONLINE || stVIVPSSMode.aenMode[VpssGrp] == VI_ONLINE_VPSS_ONLINE) && enInput != VPSS_INPUT_ISP)
        {
    ...
    if (!vpssCtx[VpssGrp])
    {
        CVI_TRACE_VPSS(CVI_DBG_ERR, "vpssCtx malloc fail.\n");
        return CVI_ERR_VPSS_NOMEM;
    }
    vpssCtx[VpssGrp]->hw_cfgs = calloc(sizeof(struct VPSS_GRP_HW_CFG), 1);
    if (!vpssCtx[VpssGrp]->hw_cfgs)
    {
        CVI_TRACE_VPSS(CVI_DBG_ERR, "hw_cfgs malloc fail.\n");
    ...
    pthread_mutex_init(&vpssCtx[VpssGrp]->lock, NULL);

    vpssCtx[VpssGrp]->u8DevId = u8DevUsed;
    vpssCtx[VpssGrp]->chnNum  = (stVPSSMode.enMode == VPSS_MODE_SINGLE) ? VPSS_MAX_CHN_NUM : (u8DevUsed == 0) ? 1
                                                                                                              : VPSS_MAX_CHN_NUM - 1;

    // handler_ctx[u8DevUsed].online_from_isp = (stVPSSMode.aenInput[u8DevUsed] == VPSS_INPUT_ISP);

    _os_vpss_GrpParamInit(VpssGrp);
...
CVI_S32 _os_vpss_destroy_grp(VPSS_GRP VpssGrp)
{
    CVI_U8    VpssChn;
    MMF_CHN_S chn = {.enModId = CVI_ID_VPSS, .s32DevId = VpssGrp, .s32ChnId = 0};
    CVI_S32   ret;
    ...
    if (vpssCtx[VpssGrp]->isCreated)
    {
        ...
        for (VpssChn = 0; VpssChn < vpssCtx[VpssGrp]->chnNum; ++VpssChn)
        {
            ...
            if (mesh[VpssGrp][VpssChn].paddr)
            {
                ...
                // 	aos_free_align((void *)mesh[VpssGrp][VpssChn].paddr);
                // }
                mesh[VpssGrp][VpssChn].paddr = 0;
                mesh[VpssGrp][VpssChn].vaddr = 0;
            }
            pthread_mutex_destroy(&mesh[VpssGrp][VpssChn].lock);
        }
        pthread_mutex_unlock(&vpssCtx[VpssGrp]->lock);
        pthread_mutex_destroy(&vpssCtx[VpssGrp]->lock);
    }
    free(vpssCtx[VpssGrp]->hw_cfgs);
...
VPSS_GRP _os_vpss_get_available_grp(void)
{
    VPSS_GRP grp = 0;
    CVI_U8   i;

    for (i = 0; i < VI_MAX_PIPE_NUM; i++)
        if ((stVIVPSSMode.aenMode[i] == VI_ONLINE_VPSS_ONLINE) || (stVIVPSSMode.aenMode[i] == VI_OFFLINE_VPSS_ONLINE))
        {
            grp = VPSS_ONLINE_NUM;
            break;
        }
...
CVI_S32 _os_vpss_start_grp(VPSS_GRP VpssGrp)
{
    CVI_U8        u8VpssDev;
    CVI_S32       ret;
    unsigned long flags_job;
    ...
    if (handler_ctx[u8VpssDev].online_from_isp)
    {
        ...
        handler_ctx[u8VpssDev].workingGrp      = VpssGrp;
    }
    ...
    if (handler_ctx[u8VpssDev].enHdlState == HANDLER_STATE_STOP)
    {
        ...
        handler_ctx[u8VpssDev].events      = 0;
...
CVI_S32 _os_vpss_stop_grp(VPSS_GRP VpssGrp)
{
    CVI_S32            ret;
    CVI_U8             u8VpssDev;
    CVI_BOOL           enabled;
    ...
    if (!enabled && state == HANDLER_STATE_RUN)
    {
        ...
        handler_ctx[u8VpssDev].workingMask = 0;
        hw_reset(VpssGrp, vpssCtx[VpssGrp], CVI_FALSE);
        spin_lock_irqsave(&handler_ctx[u8VpssDev].lock, flags_job);
        handler_ctx[u8VpssDev].events     = 0;
        handler_ctx[u8VpssDev].isr_evt[0] = 0;
        handler_ctx[u8VpssDev].isr_evt[1] = 0;
        spin_unlock_irqrestore(&handler_ctx[u8VpssDev].lock, flags_job);
    }
...
CVI_S32 _os_vpss_reset_grp(VPSS_GRP VpssGrp)
{
    CVI_S32 ret;
...
CVI_S32 _os_vpss_get_grp_attr(VPSS_GRP VpssGrp, VPSS_GRP_ATTR_S *pstGrpAttr)
{
    CVI_S32 ret;
...
CVI_S32 _os_vpss_set_grp_attr(VPSS_GRP VpssGrp, const VPSS_GRP_ATTR_S *pstGrpAttr)
{
    CVI_U32      u32MinHeight;
    CVI_S32      ret;
    CVI_U8       u8DevUsed;
    ...
    if (pstGrpAttr->stFrameRate.s32SrcFrameRate != pstGrpAttr->stFrameRate.s32DstFrameRate)
        CVI_TRACE_VPSS(CVI_DBG_WARN, "Grp(%d) FrameRate ctrl, src(%d) dst(%d), not support yet.\n", VpssGrp, pstGrpAttr->stFrameRate.s32SrcFrameRate, pstGrpAttr->stFrameRate.s32DstFrameRate);
...
CVI_S32 _os_vpss_set_grp_csc(struct vpss_grp_csc_cfg *cfg)
{
    CVI_S32  ret;
...
CVI_S32 _os_vpss_set_chn_attr(VPSS_GRP VpssGrp, VPSS_CHN VpssChn, const VPSS_CHN_ATTR_S *pstChnAttr)
{
    VB_CAL_CONFIG_S stVbCalConfig;
    CVI_U32         u32MinHeight;
    CVI_S32         ret;
    ...
    if (pstChnAttr->stAspectRatio.enMode == ASPECT_RATIO_MANUAL)
    {
        ...
        if (!pstChnAttr->stAspectRatio.bEnableBgColor)
        {
            ...
            if (ret != CVI_SUCCESS)
                return ret;
            if ((IS_FMT_YUV420(pstChnAttr->enPixelFormat) || IS_FMT_YUV422(pstChnAttr->enPixelFormat)) && (rect->s32X & 0x01))
            {
        ...
        if ((rect->s32X + rect->u32Width > pstChnAttr->u32Width) || (rect->s32Y + rect->u32Height > pstChnAttr->u32Height))
        {
            CVI_TRACE_VPSS(CVI_DBG_ERR, "ASPECT_RATIO_MANUAL invalid.\n");
            CVI_TRACE_VPSS(CVI_DBG_ERR, "Grp(%d) Chn(%d) rect(%d %d %d %d) output-size(%d %d).\n", VpssGrp, VpssChn, rect->s32X, rect->s32Y, rect->u32Width, rect->u32Height, pstChnAttr->u32Width, pstChnAttr->u32Height);
            return CVI_ERR_VPSS_ILLEGAL_PARAM;
        }
    }
    ...
    {
        CVI_TRACE_VPSS(CVI_DBG_WARN, "Grp(%d) Chn(%d) FrameRate ctrl, src(%d) < dst(%d), not support\n", VpssGrp, VpssChn, pstChnAttr->stFrameRate.s32SrcFrameRate, pstChnAttr->stFrameRate.s32DstFrameRate);
        return CVI_ERR_VPSS_ILLEGAL_PARAM;
    }

    COMMON_GetPicBufferConfig(pstChnAttr->u32Width, pstChnAttr->u32Height,
                              pstChnAttr->enPixelFormat, DATA_BITWIDTH_8, COMPRESS_MODE_NONE, DEFAULT_ALIGN, &stVbCalConfig);
...
CVI_S32 _os_vpss_get_chn_attr(VPSS_GRP VpssGrp, VPSS_CHN VpssChn, VPSS_CHN_ATTR_S *pstChnAttr)
{
    CVI_S32 ret;
...
CVI_S32 _os_vpss_enable_chn(VPSS_GRP VpssGrp, VPSS_CHN VpssChn)
{
    ...
    if (handler_ctx[u8VpssDev].online_from_isp && vpssCtx[VpssGrp]->isStarted)
    {
        vpss_notify_wkup_evt(u8VpssDev);
    }
...
CVI_S32 _os_vpss_disable_chn(VPSS_GRP VpssGrp, VPSS_CHN VpssChn)
{
    MMF_CHN_S                      chn = {.enModId = CVI_ID_VPSS, .s32DevId = VpssGrp, .s32ChnId = VpssChn};
    CVI_S32                        ret;
    struct VPSS_CHN_WORK_STATUS_S *pstChnStatus;
    ...
    {
        base_ion_free(vpssCtx[VpssGrp]->stChnCfgs[VpssChn].bufWrapPhyAddr);
...
CVI_S32 _os_vpss_set_chn_crop(VPSS_GRP VpssGrp, VPSS_CHN VpssChn, const VPSS_CROP_INFO_S *pstCropInfo)
{
    CVI_S32 ret;
    ...
    if (pstCropInfo->bEnable)
    {
        if ((pstCropInfo->stCropRect.u32Width < 4) || (pstCropInfo->stCropRect.u32Height < 1))
        {
            CVI_TRACE_VPSS(CVI_DBG_ERR, "Grp(%d) Chn(%d) crop size(%d %d) can't smaller than 4x1\n",
                           VpssGrp, VpssChn, pstCropInfo->stCropRect.u32Width, pstCropInfo->stCropRect.u32Height);
            return CVI_ERR_VPSS_ILLEGAL_PARAM;
        }
        ...
        {
            CVI_TRACE_VPSS(CVI_DBG_ERR, "Grp(%d) Chn(%d) crop rect(%d %d %d %d) can't smaller than 4x1\n",
        ...
        {
            if ((CVI_S32)vpssCtx[VpssGrp]->stGrpCropInfo.stCropRect.u32Height - pstCropInfo->stCropRect.s32Y < 1 ||
                (CVI_S32)vpssCtx[VpssGrp]->stGrpCropInfo.stCropRect.u32Width - pstCropInfo->stCropRect.s32X < 4)
            {
                CVI_TRACE_VPSS(CVI_DBG_ERR, "Grp(%d) Chn(%d) crop rect(%d %d %d %d) can't smaller than 4x1\n",
                               VpssGrp, VpssChn, pstCropInfo->stCropRect.s32X, pstCropInfo->stCropRect.s32Y,
                               pstCropInfo->stCropRect.u32Width, pstCropInfo->stCropRect.u32Height);
                CVI_TRACE_VPSS(CVI_DBG_ERR, "grp crop size(%d %d)\n",
                               vpssCtx[VpssGrp]->stGrpCropInfo.stCropRect.u32Width, vpssCtx[VpssGrp]->stGrpCropInfo.stCropRect.u32Height);
                return CVI_ERR_VPSS_ILLEGAL_PARAM;
            }
        }
        else
        {
            if ((CVI_S32)vpssCtx[VpssGrp]->stGrpAttr.u32MaxH - pstCropInfo->stCropRect.s32Y < 1 || (CVI_S32)vpssCtx[VpssGrp]->stGrpAttr.u32MaxW - pstCropInfo->stCropRect.s32X < 4)
            {
                CVI_TRACE_VPSS(CVI_DBG_ERR, "Grp(%d) Chn(%d) crop rect(%d %d %d %d) can't smaller than 4x1\n",
                               VpssGrp, VpssChn, pstCropInfo->stCropRect.s32X, pstCropInfo->stCropRect.s32Y,
                               pstCropInfo->stCropRect.u32Width, pstCropInfo->stCropRect.u32Height);
...
CVI_S32 _os_vpss_get_chn_crop(VPSS_GRP VpssGrp, VPSS_CHN VpssChn, VPSS_CROP_INFO_S *pstCropInfo)
{
    CVI_S32 ret;
...
CVI_S32 _os_vpss_show_chn(VPSS_GRP VpssGrp, VPSS_CHN VpssChn)
{
    CVI_S32 ret;
...
CVI_S32 _os_vpss_hide_chn(VPSS_GRP VpssGrp, VPSS_CHN VpssChn)
{
    CVI_S32 ret;
...
CVI_S32 _os_vpss_get_grp_crop(VPSS_GRP VpssGrp, VPSS_CROP_INFO_S *pstCropInfo)
{
    CVI_S32 ret;
...
CVI_S32 _os_vpss_set_grp_crop(VPSS_GRP VpssGrp, const VPSS_CROP_INFO_S *pstCropInfo)
{
    CVI_S32 ret;
    ...
    if (pstCropInfo->bEnable)
    {
        ...
        if ((pstCropInfo->stCropRect.u32Width < 4) || (pstCropInfo->stCropRect.u32Height < u32MinHeight))
        {
            CVI_TRACE_VPSS(CVI_DBG_ERR, "Grp(%d) crop size(%d %d) can't smaller than 4x%d\n",
                           VpssGrp, pstCropInfo->stCropRect.u32Width, pstCropInfo->stCropRect.u32Height, u32MinHeight);
            return CVI_ERR_VPSS_ILLEGAL_PARAM;
        }

        if ((pstCropInfo->stCropRect.s32Y + pstCropInfo->stCropRect.u32Height) > vpssCtx[VpssGrp]->stGrpAttr.u32MaxH ||
            (pstCropInfo->stCropRect.s32X + pstCropInfo->stCropRect.u32Width) > vpssCtx[VpssGrp]->stGrpAttr.u32MaxW)
        {
...
CVI_S32 _os_vpss_send_frame(VPSS_GRP VpssGrp, const VIDEO_FRAME_INFO_S *pstVideoFrame, CVI_S32 s32MilliSec)
{
    MMF_CHN_S chn = {.enModId = CVI_ID_VPSS, .s32DevId = VpssGrp, .s32ChnId = 0};
    VB_BLK    blk;
    CVI_S32   ret;
    ...
    if ((vpssCtx[VpssGrp]->stGrpAttr.u32MaxW != pstVideoFrame->stVFrame.u32Width) || (vpssCtx[VpssGrp]->stGrpAttr.u32MaxH != pstVideoFrame->stVFrame.u32Height))
    {
    ...
    {
        if ((pstVideoFrame->stVFrame.u32Width - pstVideoFrame->stVFrame.s16OffsetLeft - pstVideoFrame->stVFrame.s16OffsetRight) & 0x01)
        {
        ...
        if ((pstVideoFrame->stVFrame.u32Height - pstVideoFrame->stVFrame.s16OffsetTop - pstVideoFrame->stVFrame.s16OffsetBottom) & 0x01)
        {
    ...
    if (blk == VB_INVALID_HANDLE)
    {
        blk = vb_create_block(pstVideoFrame->stVFrame.u64PhyAddr[0], NULL, 0, CVI_TRUE);
        if (blk == VB_INVALID_HANDLE)
        {
...
CVI_S32 _os_vpss_send_chn_frame(VPSS_GRP VpssGrp, VPSS_CHN VpssChn, const VIDEO_FRAME_INFO_S *pstVideoFrame, CVI_S32 s32MilliSec)
{
    CVI_S32           ret;
    MMF_CHN_S         chn = {.enModId = CVI_ID_VPSS, .s32DevId = VpssGrp, .s32ChnId = VpssChn};
    VB_BLK            blk;
    ...
    if ((vpssCtx[VpssGrp]->stChnCfgs[VpssChn].stChnAttr.u32Width != pstVideoFrame->stVFrame.u32Width) || (vpssCtx[VpssGrp]->stChnCfgs[VpssChn].stChnAttr.u32Height != pstVideoFrame->stVFrame.u32Height))
    {
...
CVI_S32 _os_vpss_get_chn_frame(VPSS_GRP VpssGrp, VPSS_CHN VpssChn, VIDEO_FRAME_INFO_S *pstFrameInfo,
                               CVI_S32 s32MilliSec)
{
    CVI_S32      ret, i;
...
CVI_S32 _os_vpss_release_chn_frame(VPSS_GRP VpssGrp, VPSS_CHN VpssChn, const VIDEO_FRAME_INFO_S *pstVideoFrame)
{
    VB_BLK  blk;
    CVI_S32 ret;
...
CVI_S32 _os_vpss_set_chn_rotation(VPSS_GRP VpssGrp, VPSS_CHN VpssChn, ROTATION_E enRotation)
{
    CVI_S32 ret;
...
CVI_S32 _os_vpss_get_chn_rotation(VPSS_GRP VpssGrp, VPSS_CHN VpssChn, ROTATION_E *penRotation)
{
    CVI_S32 ret;
...
CVI_S32 _os_vpss_set_chn_align(VPSS_GRP VpssGrp, VPSS_CHN VpssChn, CVI_U32 u32Align)
{
    CVI_S32         ret;
    VB_CAL_CONFIG_S stVbCalConfig;
...
CVI_S32 _os_vpss_get_chn_align(VPSS_GRP VpssGrp, VPSS_CHN VpssChn, CVI_U32 *pu32Align)
{
    CVI_S32 ret;
 ...
 */
CVI_S32 _os_vpss_set_chn_yratio(VPSS_GRP VpssGrp, VPSS_CHN VpssChn, CVI_U32 YRatio)
{
    CVI_S32 ret;
...
CVI_S32 _os_vpss_get_chn_yratio(VPSS_GRP VpssGrp, VPSS_CHN VpssChn, CVI_U32 *pYRatio)
{
    CVI_S32 ret;
                              ...
                              const VPSS_LDC_ATTR_S *pstLDCAttr, CVI_U64 mesh_addr)
{
    ...
    return _os_vpss_update_ldc_mesh(VpssGrp, VpssChn, pstLDCAttr, enRotation, mesh_addr);
}
...
CVI_S32 _os_vpss_attach_vb_pool(VPSS_GRP VpssGrp, VPSS_CHN VpssChn, VB_POOL hVbPool)
{
    CVI_S32 ret;
...
CVI_S32 _os_vpss_detach_vb_pool(VPSS_GRP VpssGrp, VPSS_CHN VpssChn)
{
    CVI_S32 ret;
...
CVI_S32 vpss_set_chn_bufwrap_attr(VPSS_GRP VpssGrp, VPSS_CHN VpssChn, const VPSS_CHN_BUF_WRAP_S *pstVpssChnBufWrap)
{
    ...
    if (pstVpssChnBufWrap->bEnable)
    {
        ...
        if (!ion_paddr)
        {
            ...
            ret = base_ion_alloc(&ion_paddr, (void *)&ion_vaddr, ion_name, u32WrapBufferSize, true);
...
CVI_S32 _os_vpss_trigger_snap_frame(VPSS_GRP VpssGrp, VPSS_CHN VpssChn, CVI_U32 frame_cnt)
{
    CVI_S32                   ret;
    MMF_BIND_DEST_S           stBindDest;
    MMF_CHN_S                 chn = {.enModId = CVI_ID_VPSS, .s32DevId = VpssGrp, .s32ChnId = VpssChn};
    ...
    {
        CVI_TRACE_VPSS(CVI_DBG_WARN, "base_get_bindbysrc fails\n");
        return CVI_ERR_VPSS_NOT_PERM;
    }
    if (stBindDest.astMmfChn[0].enModId != CVI_ID_VENC)
    {
...
static void vpss_start_handler(void)
{
    ...
    sem_init(&vip_dev->wait_sem, 0, 0);
    pthread_cond_init(&vip_dev->wait, &cattr);
    pthread_cond_init(&vip_dev->vi_reset_wait, &cattr);

    pthread_mutex_init(&vip_dev->mutex, NULL);
    pthread_mutex_init(&vip_dev->wait_lock, NULL);  // for cond wait
    pthread_mutex_init(&vip_dev->reset_lock, NULL); // for cond reset_wait

    // set vpss thread attr
    tsk.sched_priority = 16;
    ...
    for (u8VpssDev = 0; u8VpssDev < VPSS_IP_NUM; u8VpssDev++)
    {
        ...
        sem_init(&handler_ctx[u8VpssDev].wait, 0, 0);
        sem_init(&handler_ctx[u8VpssDev].vi_reset_wait, 0, 0);
        spin_lock_init(&handler_ctx[u8VpssDev].lock);

        ret = pthread_create(&handler_ctx[u8VpssDev].v_thread, &attr, vpss_event_handler, (void *)&handler_ctx[u8VpssDev]);
        if (ret != 0)
        {
            CVI_TRACE_VPSS(CVI_DBG_ERR, "failed to create vpss pthread, u8VpssDev=%d\n", u8VpssDev);
        }
...
static void vpss_stop_handler(void)
{
    ...
    for (u8VpssDev = 0; u8VpssDev < VPSS_IP_NUM; u8VpssDev++)
    {
        ...
        ret = pthread_join(handler_ctx[u8VpssDev].thread, NULL);
        if (ret)
            CVI_TRACE_VPSS(CVI_DBG_ERR, "fail to stop vpss thread, err=%d\n", ret);

        handler_ctx[u8VpssDev].thread = 0;
    }
}
...
void vpss_mode_init(struct cvi_vip_dev *pdev)
{
    CVI_U8 i;
    ...
    vpss_set_mode_ex(pdev, &stVPSSMode);
}

void _os_vpss_init(struct cvi_vip_dev *pdev)
{
    if (!pdev)
        return;
...
void vpss_deinit(void)
{
    ...
    pthread_mutex_destroy(&vip_dev->mutex);
    pthread_mutex_destroy(&vip_dev->wait_lock);
    pthread_mutex_destroy(&vip_dev->reset_lock);
    sem_destroy(&vip_dev->wait_sem);
    for (int u8VpssDev = 0; u8VpssDev < VPSS_IP_NUM; u8VpssDev++)
    {
        sem_destroy(&handler_ctx[u8VpssDev].wait);
        sem_destroy(&handler_ctx[u8VpssDev].vi_reset_wait);
    }
    pthread_cond_destroy(&vip_dev->wait);
    pthread_cond_destroy(&vip_dev->vi_reset_wait);
}
...
int vpss_set_grp_sbm(struct vpss_grp_sbm_cfg *cb_cfg)
{
    ...
    if (!sb_mode)
    {
        if (sbm_cfg->ion_paddr)
            base_ion_free(sbm_cfg->ion_paddr);
    ...
    ret = base_ion_alloc(&ion_paddr, (void *)&ion_vaddr, ion_name, ion_size, true);
