...
CVI_S32 GetBmpInfo(const char *filename, OSD_BITMAPFILEHEADER *pBmpFileHeader,
                   OSD_BITMAPINFO *pBmpInfo)
{
    ...
    printf("bmp width(%d) height(%d) bpp(%d) compression(%d)\n", pBmpInfo->bmiHeader.biWidth,
           pBmpInfo->bmiHeader.biHeight, pBmpInfo->bmiHeader.biBitCount,
           pBmpInfo->bmiHeader.biCompression);

    return 0;
}

int LoadBMP(const char *filename, OSD_LOGO_T *pVideoLogo, OSD_COLOR_FMT_E enFmt,
            CVI_BOOL update_logo)
{
    FILE   *pFile;
    CVI_U16 i, j;
    ...
    h = ((bmpInfo.bmiHeader.biHeight > 0) ? bmpInfo.bmiHeader.biHeight
    ...
    if (update_logo) {
        ...
        pVideoLogo->stride = (enFmt == OSD_COLOR_FMT_8BIT_MODE) ? w
                             : (enFmt >= OSD_COLOR_FMT_RGB888)  ? w << 2
                                                                : w << 1;
    }
...
CVI_S32 CreateSurfaceByBitMap(const CVI_CHAR *pszFileName, OSD_SURFACE_S *pstSurface,
                              CVI_U8 *pu8Virt)
{
    OSD_LOGO_T stLogo;
...
CVI_S32 CreateSurfaceByCanvas(const CVI_CHAR *pszFileName, OSD_SURFACE_S *pstSurface,
                              CVI_U8 *pu8Virt, CVI_U32 u32Width, CVI_U32 u32Height,
                              CVI_U32 u32Stride)
{
    OSD_LOGO_T stLogo;
