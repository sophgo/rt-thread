...
__attribute__((weak)) void MEDIA_NightVision_GetAutoAttr(ISP_DEV             IspDev,
                                                         ISP_IR_AUTO_ATTR_S *pstIrAttr)
{
...
int MEDIA_NigthVision_RunAutoIr(int DevId)
{
    ...
    if (MEDIA_NightVision_IsRGBMode() == 0) {
        ...
        if (g_stIrAttr[DevId].enIrSwitch == ISP_IR_SWITCH_TO_IR) {
            // load rgb mono param
            if (pstViCfg->u32IspSceneNum >= 2 &&
                pstViCfg->pstIspCfg[1].astPQBinDes[0].pIspBinData) {
                CVI_BIN_LoadParamFromBinEx(pstViCfg->pstIspCfg[1].astPQBinDes[0].binID,
                                           pstViCfg->pstIspCfg[1].astPQBinDes[0].pIspBinData,
            ...
            // load rgb param
            if (pstViCfg->u32IspSceneNum >= 1 &&
                pstViCfg->pstIspCfg[0].astPQBinDes[0].pIspBinData) {
                CVI_BIN_LoadParamFromBinEx(pstViCfg->pstIspCfg[0].astPQBinDes[0].binID,
                                           pstViCfg->pstIspCfg[0].astPQBinDes[0].pIspBinData,
...
MSH_CMD_EXPORT_ALIAS(testMedia_NightVisionInit, testMedia_NightVisionInit, testMedia_NightVisionInit);
MSH_CMD_EXPORT_ALIAS(testMedia_NightVisionDeInit, testMedia_NightVisionDeInit, testMedia_NightVisionDeInit);
MSH_CMD_EXPORT_ALIAS(testMedia_NightVisionAutoRun, testMedia_NightVisionAutoRun, testMedia_NightVisionAutoRun);
#endif
...
MSH_CMD_EXPORT_ALIAS(cli_switch_led, cli_switch_led, cli_switch_led);