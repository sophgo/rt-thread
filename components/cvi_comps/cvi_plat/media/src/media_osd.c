...
static void APP_OSD_GetTimeStr(const struct tm *pstTime, char *pazStr, int s32MaxLen)
{
    ...
    snprintf(pazStr, s32MaxLen, "%04d-%02d-%02d %02d:%02d:%02d", pstTime->tm_year + 1900,
             pstTime->tm_mon + 1, pstTime->tm_mday, pstTime->tm_hour, pstTime->tm_min,
             pstTime->tm_sec);
}
    ...
    for (u32BmRow = 0; u32BmRow < u32CanvasHeight; ++u32BmRow) {
        ...
        for (u32BmCol = 0; u32BmCol < u32CanvasWidth; ++u32BmCol) {
            ...
            if (Rotation == ROTATION_90) {
                ...
                s32BmDataIdx = ((u32CanvasHeight - 1) - u32BmRow) * pstBitmap->u32Width +
                               ((u32CanvasWidth - 1) - u32BmCol);
...
CVI_VOID *APP_OSDTimeStamp_Task(CVI_VOID *args)
{
    ...
    stBitmap.u32Width =
        OSD_LIB_FONT_W * (strlen(szStr) - MediaOsd_GetNonASCNum(szStr, strlen(szStr)));
    ...
    while (pstAppOsd->s32Enable) {
        ...
        ret = APP_OSD_UpdateBitmap(pstAppOsd->s32Handle, szStr, &stBitmap, pu32Color,
                                   pstAppOsd->Rotation);
        if (ret != CVI_SUCCESS) {
...
CVI_S32 APP_OSD_Destory(APP_OSD_S *pstAppOsd)
{
    ...
    if (pstAppOsd->s32Handle == APP_MEDIAOSD_TIMESTAMP) pthread_join(pstAppOsd->PthreadId, NULL);
...
static CVI_S32 APP_OSD_GetRoationPos(APP_OSD_S *pstAppOsd, RECT_S *RotationPos)
{
    ...
    if (CVI_VPSS_GetChnAttr(pstAppOsd->stChn.s32DevId, pstAppOsd->stChn.s32ChnId, &stVpssAttr) !=
        CVI_SUCCESS) {
        printf("%s CVI_VPSS_GETCHNATTR Err\n", __func__);
        return CVI_FAILURE;
    }
    CVI_VPSS_GetChnRotation(pstAppOsd->stChn.s32DevId, pstAppOsd->stChn.s32ChnId,
                            &pstAppOsd->Rotation);
    ...
    if (pstAppOsd->s32Type == APP_OSDTYPE_CHAR) {
        ...
        if (pstAppOsd->Rotation == ROTATION_90) {
            ...
            RotationPos->s32Y =
                (CVI_S32)(proportiony * (1.0 - ((CVI_DOUBLE)pstAppOsd->Pos.s32X / proportionVideo) /
                                                   ((CVI_DOUBLE)stVpssAttr.u32Width)) -
            ...
            RotationPos->s32Y =
                stVpssAttr.u32Height - pstAppOsd->Pos.u32Height - pstAppOsd->Pos.s32Y;
...
CVI_S32 APP_OSD_SetString(APP_OSD_S *pstAppOsd, CVI_S32 s32Handle, CVI_CHAR *String)
{
    ...
    stBitmap.u32Width =
        OSD_LIB_FONT_W * (strlen(String) - MediaOsd_GetNonASCNum(String, strlen(String)));
    stBitmap.u32Height = OSD_LIB_FONT_H;
    stBitmap.pData     = malloc(2 * stBitmap.u32Width * stBitmap.u32Height);
    if (!stBitmap.pData) {
    ...
    ret = APP_OSD_UpdateBitmap(pstTempAppOsd->s32Handle, String, &stBitmap, pu32Color,
                               pstTempAppOsd->Rotation);
    if (ret != CVI_SUCCESS) {
...
static CVI_S32 APP_OSD_MST_UpdateCanvas(const CVI_CHAR *filename, BITMAP_S *pstBitmap,
                                        CVI_BOOL bFil, CVI_U32 u16FilColor, SIZE_S *pstSize,
                                        CVI_U32 u32Stride, PIXEL_FORMAT_E enPixelFormat)
{
    ...
    CreateSurfaceByCanvas(filename, &Surface, (CVI_U8 *)(pstBitmap->pData), pstSize->u32Width,
    ...
    if (bFil) {
        ...
        for (i = 0; i < pstBitmap->u32Height; i++) {
            for (j = 0; j < pstBitmap->u32Width; j++) {
                if (u16FilColor == *pu16Temp) *pu16Temp &= 0x7FFF;
...
CVI_S32 APP_OSD_Init(void)
{
    for (CVI_S32 i = 0; i < APP_MEDIAOSD_MAX; i++) {
        ...
        if (APP_OSD_Create(&s_APP_OSD_Manager[i]) == CVI_FAILURE) continue;
