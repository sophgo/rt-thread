...
static int start_isp(ISP_PUB_ATTR_S stPubAttr, VI_PIPE ViPipe)
{
    ...
    stsCfg.unKey.bit1FEAeGloStat = stsCfg.unKey.bit1FEAeLocStat = stsCfg.unKey.bit1AwbStat1 =
        stsCfg.unKey.bit1AwbStat2 = stsCfg.unKey.bit1FEAfStat = 1;
    // LDG
...
static int _meida_sensor_init(PARAM_VI_CFG_S *pstViCtx, CVI_U8 *devNum)
{
    ...
    for (CVI_U8 i = 0; i < *devNum; ++i) {
        ViDev = pstViCtx->pstDevInfo[i].u8AttachDev > 0
                    ? VI_MAX_PHY_DEV_NUM + pstViCtx->pstDevInfo[i].u8AttachDev - 1
                    : i;
...
static void dumpFrame(MMF_CHN_S *pstChn, CVI_U32 u32FrameCnt)
{
    ...
    while (u32Cnt--) {
        ...
        for (i = 0; i < 3; ++i) {
            ...
            memcpy(image_buf + image_offset, (void *)stFrameInfo.stVFrame.u64PhyAddr[i],
                   u32DataLen);
            image_offset += u32DataLen;
            // aos_write(fd, (CVI_U8 *)stFrameInfo.stVFrame.u64PhyAddr[i], u32DataLen);
        }

        if (_releaseFrame(pstChn, &stFrameInfo) != CVI_SUCCESS) printf("_releaseFrame fail\n");
    }

    if (bFlag) {
        /* make file name */
        getFmtName(stFrameInfo.stVFrame.enPixelFormat, szPixFrm);
        snprintf(szFrameName, 128, SD_FATFS_MOUNTPOINT "/pipe%d_chn%d_%dx%d_%s_%d.yuv",
                 pstChn->s32DevId, pstChn->s32ChnId, stFrameInfo.stVFrame.u32Width,
                 stFrameInfo.stVFrame.u32Height, szPixFrm, u32FrameCnt);
...
CVI_S32 dual_sns_sync_task_callback(VI_SYNC_TASK_DATA_S *data)
{
    ...
    if (data->ViPipe == data->value) return 0;
...
// TODO: current don't support ldc
...
//     CVI_GDC_LoadMeshWithBuf(&stMeshDumpAttr, &stViLdcCfg.stLDCAttr.stAttr, pMeshBuf,
//     u32MeshSize);
...
int MEDIA_VIDEO_ViInit(PARAM_VI_CFG_S *pstViCfg)
{
    ...
    for (int i = 0; i < devNum; i++) {
        ...
        if (pstViCfg->bFastConverge == CVI_TRUE) setFastConvergeAttr(ViDev, CVI_TRUE);
    }
    ...
    for (int i = 0; i < devNum; i++) {
        ...
        stPubAttr.stWndRect.u32Width = stPubAttr.stSnsSize.u32Width =
            stViDevAttr[ViDev].stSize.u32Width;
        stPubAttr.stWndRect.u32Height = stPubAttr.stSnsSize.u32Height =
            stViDevAttr[ViDev].stSize.u32Height;
    ...
    for (int i = 0; i < devNum; i++) {
        ...
        if (pstViCfg->pstSensorCfg[i].u8Rotation != ROTATION_0) {
            MEDIA_CHECK_RET(
                CVI_VI_SetChnRotation(ViDev, ViChn, pstViCfg->pstSensorCfg[i].u8Rotation),
    ...
    for (int i = 0; i < devNum; i++) {
        if (pstViCfg->pstIspCfg[scene_mode].astPQBinDes[i].pIspBinData &&
            pstViCfg->pstIspCfg[scene_mode].astPQBinDes[i].u32IspBinDataLen > 0) {
            MEDIA_CHECK_RET(CVI_BIN_LoadParamFromBinEx(
                                pstViCfg->pstIspCfg[scene_mode].astPQBinDes[i].binID,
                                pstViCfg->pstIspCfg[scene_mode].astPQBinDes[i].pIspBinData,
                                pstViCfg->pstIspCfg[scene_mode].astPQBinDes[i].u32IspBinDataLen),
                            "CVI_BIN_LoadParamFromBinEx(%d) fail",
                            pstViCfg->pstIspCfg[scene_mode].astPQBinDes[i].binID);
        }
    }
...
int MEDIA_VIDEO_SysInit()
{
    ...
    // vip init
    cvi_cif_init();
    // rt_kprintf("media video snsr i2c probe\n");
    // cvi_snsr_i2c_probe();
    vi_core_init();
    vpss_core_init();
...
int MEDIA_VIDEO_SysVbInit(PARAM_SYS_CFG_S *pstSysCtx)
{
    ...
    for (i = 0; i < stVbConfig.u32MaxPoolCnt; i++) {
        ...
        u32BlkSize    = COMMON_GetPicBufferSize(width, height, pstSysCtx->pstVbPool[i].fmt,
                                                pstSysCtx->pstVbPool[i].enBitWidth,
                                                pstSysCtx->pstVbPool[i].enCmpMode, DEFAULT_ALIGN);
    ...
    for (i = 0; i < pstVpssCtx->u8GrpCnt; i++) {
        ...
        for (j = 0; j < pstVpssCtx->pstVpssGrpCfg[i].u8ChnCnt; j++) {
            ...
            if (pstVpssCtx->pstVpssGrpCfg[i].pstChnCfg[j].u8Rotation != ROTATION_0) {
                CVI_VPSS_SetChnRotation(VpssGrp, j,
                                        pstVpssCtx->pstVpssGrpCfg[i].pstChnCfg[j].u8Rotation);
            }
            // Set VPSS Grp[i] Chn[j] Crop Configuration
            if (pstVpssCtx->pstVpssGrpCfg[i].pstChnCfg[j].stVpssChnCropInfo.bEnable == CVI_TRUE) {
                MEDIA_CHECK_RET(
                    CVI_VPSS_SetChnCrop(
                        VpssGrp, j, &pstVpssCtx->pstVpssGrpCfg[i].pstChnCfg[j].stVpssChnCropInfo),
                    "CVI_VPSS_SetChnCrop failed\n");
            }
        }
        MEDIA_CHECK_RET(CVI_VPSS_StartGrp(VpssGrp), "CVI_VPSS_StartGrp failed\n");
        if (pstVpssCtx->pstVpssGrpCfg[i].bBindMode == CVI_TRUE) {
            MEDIA_CHECK_RET(CVI_SYS_Bind(&pstVpssCtx->pstVpssGrpCfg[i].astChn[0],
                                         &pstVpssCtx->pstVpssGrpCfg[i].astChn[1]),
                            "CVI_SYS_Bind err");
        }
    }
    return CVI_SUCCESS;
...
int MEDIA_VIDEO_VpssDeinit(PARAM_VPSS_CFG_S *pstVpssCtx)
{
    ...
    for (i = 0; i < pstVpssCtx->u8GrpCnt; i++) {
        ...
        if (pstVpssCtx->pstVpssGrpCfg[i].bBindMode == CVI_TRUE) {
            MEDIA_CHECK_RET(CVI_SYS_UnBind(&pstVpssCtx->pstVpssGrpCfg[i].astChn[0],
                                           &pstVpssCtx->pstVpssGrpCfg[i].astChn[1]),
                            "CVI_SYS_UnBind fail");
        }
...
int _MEDIA_VIDEO_DSIInit(int devno, const struct dsc_instr *cmds, int size)
{
    ...
    for (int i = 0; i < size; i++) {
        ...
        struct cmd_info_s       cmd_info = {.devno     = devno,
                                            .cmd_size  = instr->size,
...
void *_MEDIA_VIDEO_PanelInit(void *data)
{
    ...
    for (i = 0; i < 3; ++i) {
        struct get_cmd_info_s get_cmd_info = {.devno         = 0,
                                              .data_type     = 0x06,
                                              .data_param    = param[i],
                                              .get_data_size = 0x01,
                                              .get_data      = buf};
...
int MEDIA_VIDEO_VoInit(PARAM_VO_CFG_S *pstVoCtx)
{
    ...
    for (i = 0; i < pstVoCtx->u8VoCnt; i++) {
        ...
        MEDIA_CHECK_RET(CVI_VO_SetPubAttr(pstVoConfig->VoDev, &pstVoConfig->stVoPubAttr),
                        "CVI_VO_SetPubAttr failed!\n");
        MEDIA_CHECK_RET(CVI_VO_Enable(pstVoConfig->VoDev), "CVI_VO_Enable failed!\n");
        MEDIA_CHECK_RET(
            CVI_VO_SetDisplayBufLen(pstVoCtx->pstVoCfg[i].VoLayer, pstVoConfig->u32DisBufLen),
        ...
        MEDIA_CHECK_RET(
            CVI_SYS_Bind(&pstVoCtx->pstVoCfg[i].stSrcChn, &pstVoCtx->pstVoCfg[i].stDestChn),
...
int MEDIA_VIDEO_VoDeinit(PARAM_VO_CFG_S *pstVoCtx)
{
    ...
    for (i = 0; i < pstVoCtx->u8VoCnt; i++) {
        ...
        MEDIA_CHECK_RET(
            CVI_SYS_UnBind(&pstVoCtx->pstVoCfg[i].stSrcChn, &pstVoCtx->pstVoCfg[i].stDestChn),
            "CVI_SYS_UnBind(VPSS-VO) fail");
        for (j = 0; j < pstVoCtx->pstVoCfg[i].u8ChnCnt; j++) {
            VoChn = j;
...
int MEDIA_VIDEO_VencChnInit(PARAM_VENC_CFG_S *pstVencCfg, int VencChn)
{
    ...
    if (pstVecncChnCtx->stChnParam.u8ModId == CVI_ID_VPSS &&
        pstVecncChnCtx->stChnParam.u16Width == 0 && pstVecncChnCtx->stChnParam.u16Height == 0) {
        MEDIA_CHECK_RET(CVI_VPSS_GetChnAttr(pstVecncChnCtx->stChnParam.u8DevId,
                                            pstVecncChnCtx->stChnParam.u8DevChnid, &stVpssChnAttr),
                        "CVI_VPSS_GetChnAttr");
        // if VPSS enable rotation, need exchange width and height
        MEDIA_CHECK_RET(CVI_VPSS_GetChnRotation(pstVecncChnCtx->stChnParam.u8DevId,
                                                pstVecncChnCtx->stChnParam.u8DevChnid, &enRotation),
                        "CVI_VPSS_GetChnRotation");
    ...
    case PT_MJPEG: {
        if (stAttr.stRcAttr.enRcMode == VENC_RC_MODE_MJPEGCBR) {
            ...
            stAttr.stRcAttr.stMjpegFixQp.fr32DstFrameRate =
                pstVecncChnCtx->stRcParam.u8DstFrameRate;
    ...
    case PT_H264: {
        if (stAttr.stRcAttr.enRcMode == VENC_RC_MODE_H264CBR) {
            ...
            stRcParam.stParamH264Cbr.s32MaxReEncodeTimes =
                pstVecncChnCtx->stRcParam.u8MaxReEncodeTimes;
            ...
            stRcParam.stParamH264Vbr.s32MaxReEncodeTimes =
                pstVecncChnCtx->stRcParam.u8MaxReEncodeTimes;
    ...
    case PT_H265: {
        if (stAttr.stRcAttr.enRcMode == VENC_RC_MODE_H265CBR) {
            ...
            stRcParam.stParamH265Cbr.s32MaxReEncodeTimes =
                pstVecncChnCtx->stRcParam.u8MaxReEncodeTimes;
            ...
            stRcParam.stParamH265Vbr.s32MaxReEncodeTimes =
                pstVecncChnCtx->stRcParam.u8MaxReEncodeTimes;
    ...
    if (pstVecncChnCtx->stChnParam.u16EnType == PT_H265) {
...
#if !CONFIG_DISABLE_VENC_H264
        MEDIA_CHECK_RET(CVI_VENC_GetH264Entropy(VencChn, &stH264EntropyEnc),
        ...
        MEDIA_CHECK_RET(CVI_VENC_SetH264Entropy(VencChn, &stH264EntropyEnc),
...
int MEDAI_VIDEO_VencChnDeinit(PARAM_VENC_CFG_S *pstVencCfg, int VencChn)
{
    ...
    MEDIA_CHECK_RET(CVI_VENC_StopRecvFrame(pstVecncChnCtx->stChnParam.u8VencChn),
                    "CVI_VENC_StopRecvFrame");

    MEDIA_CHECK_RET(CVI_VENC_ResetChn(pstVecncChnCtx->stChnParam.u8VencChn), "CVI_VENC_ResetChn");

    MEDIA_CHECK_RET(CVI_VENC_DestroyChn(pstVecncChnCtx->stChnParam.u8VencChn),
                    "CVI_VENC_DestroyChn");
    return CVI_SUCCESS;
}
...
int MEDIA_VIDEO_VencGetStream(int VencChn, VENC_STREAM_S *pstStreamFrame, unsigned int blocktimeMs)
{
    ...
    if (pstVecncChnCtx->stChnParam.u8ModId != CVI_ID_VPSS &&
        pstVecncChnCtx->stChnParam.u8ModId != CVI_ID_VI) {
        MEDIA_CHECK_RET(
            CVI_VPSS_GetChnFrame(pstVecncChnCtx->stChnParam.u8DevId,
                                 pstVecncChnCtx->stChnParam.u8DevChnid, &stSrcFrame, 2000),
...
EXIT:
    if (pstVecncChnCtx->stChnParam.u8ModId != CVI_ID_VPSS &&
        pstVecncChnCtx->stChnParam.u8ModId != CVI_ID_VI) {
        MEDIA_CHECK_RET(
            CVI_VPSS_ReleaseChnFrame(pstVecncChnCtx->stChnParam.u8DevId,
                                     pstVecncChnCtx->stChnParam.u8DevChnid, &stSrcFrame),
...
MSH_CMD_EXPORT_ALIAS(testMedia_switch_vpss_scale_mode, testMedia_switch_vpss_scale_mode,
                     switch vpss scale coefficient level);
MSH_CMD_EXPORT_ALIAS(testMedia_video_Deinit, testMedia_video_Deinit, testMedia_video_Deinit);
MSH_CMD_EXPORT_ALIAS(testMedia_video_init, testMedia_video_init, testMedia_video_init);
MSH_CMD_EXPORT_ALIAS(testMedia_switch_pipeline, testMedia_switch_pipeline,
                     testMedia_switch_pipeline);
MSH_CMD_EXPORT_ALIAS(testMedia_switch_frame_rate_ratio, testMedia_switch_frame_rate_ratio,
                     swtich frame rate ratio in dual_switch mode);
MSH_CMD_EXPORT_ALIAS(testMedia_multiplex_switch_pipeline, testMedia_multiplex_switch_pipeline,
...
MSH_CMD_EXPORT_ALIAS(testMedia_sensor_switch, testMedia_sensor_switch, testMedia_sensor_switch);
#endif
