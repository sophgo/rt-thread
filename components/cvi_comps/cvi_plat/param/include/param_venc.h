...
// #include "cvi_venc.h"
// 多级别结构 确定使用的多级数组加number作标定 (多级限定场景 串行结构
// 不允许存在同一级别有多个多级存在) 大结构必须精简尽量使用指针
...
typedef struct _PARAM_RC_PARAM_S {
    ...
    CVI_U16 u16ThrdLv;    /*RW; Range:[0, 4]; Mad threshold for controlling the macroblock-level bit
                             rate */
...
typedef struct _PARAM_ROI_PARAM_S {
    CVI_U32 u8Index;  /* RW; Range:[0, 7]; Index of an ROI. The system supports indexes ranging from
                         0 to 7 */
    CVI_BOOL bEnable; /* RW; Range:[0, 1]; Whether to enable this ROI */
    CVI_BOOL bAbsQp;  // RW; Range:[0, 1]; QP mode of an ROI. CVI_FALSE: relative QP. CVI_TRUE:
                      // absolute QP
