...
static CVI_S32 cmos_get_ae_default(VI_PIPE ViPipe, AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
    ...
    case WDR_MODE_NONE: /*linear mode*/
        ...
        pstAeSnsDft->u32InitExposure =
            g_au32InitExposure[ViPipe] ? g_au32InitExposure[ViPipe] : pstMode->stExp.u16Def;
...
static CVI_U32 regValTable[29][4] = {
    {0x00, 0x00, 0x01, 0x00}, {0x00, 0x10, 0x01, 0x0c}, {0x00, 0x20, 0x01, 0x1b},
    {0x00, 0x30, 0x01, 0x2c}, {0x00, 0x40, 0x01, 0x3f}, {0x00, 0x50, 0x02, 0x16},
    {0x00, 0x60, 0x02, 0x35}, {0x00, 0x70, 0x03, 0x16}, {0x00, 0x80, 0x04, 0x02},
    {0x00, 0x90, 0x04, 0x31}, {0x00, 0xa0, 0x05, 0x32}, {0x00, 0xb0, 0x06, 0x35},
    {0x00, 0xc0, 0x08, 0x04}, {0x00, 0x5a, 0x09, 0x19}, {0x00, 0x83, 0x0b, 0x0f},
    {0x00, 0x93, 0x0d, 0x12}, {0x00, 0x84, 0x10, 0x00}, {0x00, 0x94, 0x12, 0x3a},
    {0x01, 0x2c, 0x1a, 0x02}, {0x01, 0x3c, 0x1b, 0x20}, {0x00, 0x8c, 0x20, 0x0f},
    {0x00, 0x9c, 0x26, 0x07}, {0x02, 0x64, 0x36, 0x21}, {0x02, 0x74, 0x37, 0x3a},
    {0x00, 0xc6, 0x3d, 0x02}, {0x00, 0xdc, 0x3f, 0x3f}, {0x02, 0x85, 0x3f, 0x3f},
    {0x02, 0x95, 0x3f, 0x3f}, {0x00, 0xce, 0x3f, 0x3f},
};

static CVI_U32 gain_table[29] = {
    64 * 16,   76 * 16,   90 * 16,   108 * 16,  127 * 16,  148 * 16,  180 * 16,  216 * 16,
    255 * 16,  300 * 16,  361 * 16,  422 * 16,  504 * 16,  593 * 16,  722 * 16,  850 * 16,
    1008 * 16, 1182 * 16, 1408 * 16, 1689 * 16, 2021 * 16, 2391 * 16, 2850 * 16, 3369 * 16,
    3985 * 16, 4805 * 16, 5768 * 16, 6806 * 16, 7723 * 16,
};
...
static CVI_S32 cmos_gains_update(VI_PIPE ViPipe, CVI_U32 *pu32Again, CVI_U32 *pu32Dgain)
{
    ...
    temperature_value =
        (temperature_value0 + temperature_value1 + temperature_value2 + temperature_value3) >> 2;

    u32Again = pu32Again[0];
    if (temperature_value >= 0x78) {
        u32Again = u32Again - 1;
        if (u32Again > 12) u32Again = 12;  // 最低模拟增益档位
    }
    flag_num++;
...
static CVI_S32 cmos_get_isp_default(VI_PIPE ViPipe, ISP_CMOS_DEFAULT_S *pstDef)
{
    ...
    memcpy(pstDef->stNoiseCalibration.CalibrationCoef, &g_stIspNoiseCalibratio,
...
static CVI_U32 sensor_cmp_wdr_size(ISP_SNS_ISP_INFO_S *pstWdr1, ISP_SNS_ISP_INFO_S *pstWdr2)
{
    ...
    if (pstWdr1->frm_num != pstWdr2->frm_num) goto _mismatch;
    for (i = 0; i < 2; i++) {
    ...
    if ((pstSnsState->bSyncInit == CVI_FALSE) || (pstSnsRegsInfo->bConfig == CVI_FALSE)) {
        ...
        for (i = 0; i < pstCfg0->snsCfg.u32RegNum; i++) {
            if (pstCfg0->snsCfg.astI2cData[i].u32Data == pstCfg1->snsCfg.astI2cData[i].u32Data) {
                ...
                if (i >= LINEAR_AGAIN_H) gainsUpdate = 1;
...
static CVI_S32 sensor_patch_rx_attr(RX_INIT_ATTR_S *pstRxInitAttr)
{
    ...
    if (pstRxInitAttr->stMclkAttr.bMclkEn) pstRxAttr->mclk.cam = pstRxInitAttr->stMclkAttr.u8Mclk;

    if (pstRxInitAttr->MipiDev >= 2) return CVI_SUCCESS;
...
{
    if (GC2053_I2C_ADDR_IS_VALID(s32I2cAddr)) gc2053_i2c_addr = s32I2cAddr;
}
...
static CVI_S32 sensor_register_callback(VI_PIPE ViPipe, ALG_LIB_S *pstAeLib, ALG_LIB_S *pstAwbLib)
{
    ...
    if (s32Ret != CVI_SUCCESS) return CVI_FAILURE;
