...
CVI_U8 sc035hgs_1L_i2c_addr = 0x30;        /* I2C Address of SC035HGS_1L */
CVI_U32 sc035hgs_1L_addr_byte = 2;
CVI_U32 sc035hgs_1L_data_byte = 1;
// static int g_fd[VI_MAX_PIPE_NUM] = {[0 ... (VI_MAX_PIPE_NUM - 1)] = -1};

int sc035hgs_1L_i2c_init(VI_PIPE ViPipe)
{
	CVI_U8 i2c_id = (CVI_U8)g_aunSC035HGS_1L_BusInfo[ViPipe].s8I2cDev;
...
int sc035hgs_1L_read_register(VI_PIPE ViPipe, int addr)
{
	...
	return sensor_i2c_read(i2c_id, sc035hgs_1L_i2c_addr, (CVI_U32)addr, sc035hgs_1L_addr_byte, sc035hgs_1L_data_byte);
...
int sc035hgs_1L_write_register(VI_PIPE ViPipe, int addr, int data)
{
	...
	// 	, __func__, __LINE__, i2c_id, sc035hgs_1L_i2c_addr, addr, sc035hgs_1L_addr_byte, sc035hgs_1L_data_byte, data);
	return sensor_i2c_write(i2c_id, sc035hgs_1L_i2c_addr, (CVI_U32)addr, sc035hgs_1L_addr_byte,
				(CVI_U32)data, sc035hgs_1L_data_byte);
...
#define SC035HGS_1L_CHIP_ID_ADDR_H    0x3108
#define SC035HGS_1L_CHIP_ID_ADDR_L    0x3109
#define SC035HGS_1L_CHIP_ID           0x310B

int sc035hgs_1L_probe(VI_PIPE ViPipe)
{
	int nVal;
	int nVal2;

	usleep(50);
	if (sc035hgs_1L_i2c_init(ViPipe) != CVI_SUCCESS)
		return CVI_FAILURE;

	nVal  = sc035hgs_1L_read_register(ViPipe, SC035HGS_1L_CHIP_ID_ADDR_H);
	nVal2 = sc035hgs_1L_read_register(ViPipe, SC035HGS_1L_CHIP_ID_ADDR_L);
	if (nVal < 0 || nVal2 < 0) {
		CVI_TRACE_SNS(CVI_DBG_ERR, "read sensor id error.\n");
		return nVal;
	}

	if ((((nVal & 0xFF) << 8) | (nVal2 & 0xFF)) != SC035HGS_1L_CHIP_ID) {
...
static void sc035hgs_1L_linear_1296P30_init(VI_PIPE ViPipe)
{
	...
	printf("ViPipe:%d,===SC035HGS_1L 480P 120fps 12bit LINE Init OK!===\n", ViPipe);
}
