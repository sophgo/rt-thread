...
{
    if ((p->ifmap->start_address % ctx->info.lmem_size) % 2 == 0 && p->ifmap->shape.c % 2 == 0 && p->ifmap->shape.c >= 4 && p->weight->start_address % 2 == 0)
        return 1;
...
void cvkcv180x_tiu_convolution(cvk_context_t *ctx, const param_t *p)
{
    int8_t status = 0;
    ...
    if (status)
    {
        printf("cvkcv180x tiu conv: wrong parameter\n");
        return;
    }
