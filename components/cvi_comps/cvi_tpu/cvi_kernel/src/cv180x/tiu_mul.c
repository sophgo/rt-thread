    ...
    const cvk_tiu_mul_param_t *p)
{
    ...
    CHECK(status, (
                      p->b_is_const || (!reg.opt_opd1_sign && !reg.opt_opd0_sign && !reg.opt_shift_typ) || ((reg.opt_opd1_sign || reg.opt_opd0_sign) && reg.opt_shift_typ)));

    reg.layer_info = p->layer_id;

    if (status)
    {
        printf("cvkcv180x tiu mul: wrong parameter\n");
        return;
    }
