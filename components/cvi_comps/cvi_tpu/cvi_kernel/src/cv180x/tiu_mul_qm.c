    ...
    const cvk_tiu_mul_qm_param_t *p)
{
    ...
    CHECK(status, (
                      (!reg.opt_opd1_sign && !reg.opt_opd0_sign && !reg.opt_shift_typ) || ((reg.opt_opd1_sign || reg.opt_opd0_sign) && reg.opt_shift_typ)));
    ...
    if (status)
    {
        printf("cvkcv180x tiu mul qm: wrong parameter\n");
    }
