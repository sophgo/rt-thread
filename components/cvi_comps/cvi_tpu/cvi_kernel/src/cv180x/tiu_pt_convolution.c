...
static int can_do_double_conv(cvk_context_t *ctx, const cvk_tiu_pt_convolution_param_t *p)
{
    uint8_t bf16_enable = (p->ifmap->fmt == CVK_FMT_BF16) ? 1 : 0;

    if (((p->ifmap->start_address % ctx->info.lmem_size) % 2 == 0 && p->ifmap->shape.c % 2 == 0 && p->ifmap->shape.c >= 4 && p->weight->start_address % 2 == 0) && !bf16_enable)
        return 1;

    return 0;
}

static int8_t check_conv_param(cvk_context_t *ctx, const cvk_tiu_pt_convolution_param_t *p)
{
    int8_t   status      = 0;
...
void cvkcv180x_tiu_pt_convolution(cvk_context_t *ctx, const cvk_tiu_pt_convolution_param_t *p)
{
    int8_t status = 0;
    ...
    reg.opd0_ins_val     = bf16_enable ? (uint32_t)p->ins_fp : (uint32_t)p->ins_val;
    ...
    if (status)
    {
        printf("cvkcv180x tiu_pt_conv: wrong parameter\n");
        return;
    }
