...
static int8_t check_matrix(cvk_context_t *ctx, const cvk_ml_t *m)
{
    ...
    t.stride.w      = 1 * (m->fmt == CVK_FMT_BF16 ? 2 : 1);
...
static int is_arith_shift(const cvk_tiu_matrix_multiplication_param_t *p)
{
...
void cvkcv181x_tiu_matrix_multiplication(cvk_context_t *ctx, const cvk_tiu_matrix_multiplication_param_t *p)
{
    int8_t          status      = 0;
    ...
    int             bf16_enable = (res->fmt == CVK_FMT_BF16) ? 1 : 0;
    ...
    if (status)
    {
        printf("cvkcv181x tiu matrix: wrong parameter");
        return;
    }
