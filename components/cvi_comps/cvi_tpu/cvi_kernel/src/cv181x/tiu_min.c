...
void cvkcv181x_tiu_min(
    cvk_context_t             *ctx,
    const cvk_tiu_min_param_t *p)
{
    int8_t status      = 0;
    int    bf16_enable = (p->a->fmt == CVK_FMT_BF16) ? 1 : 0;

    status |= check_tiu_tensor_2(p->min, p->a);
    status |= check_same_shape(p->min, p->a);
    ...
    else if (!p->b_is_const)
    {
        ...
        status |= check_same_shape(p->min, p->b);
    ...
    if (status)
    {
        printf("cvkcv181x tiu min: wrong parameter\n");
        return;
    }
