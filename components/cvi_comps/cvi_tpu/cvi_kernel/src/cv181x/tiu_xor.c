...
void cvkcv181x_tiu_xor_int8(
    cvk_context_t                  *ctx,
    const cvk_tiu_xor_int8_param_t *p)
{
    int8_t status  = 0;
    ...
    if (status)
    {
        printf("cvkcv181x tiu xor: wrong parameter\n");
        return;
    }
...
void cvkcv181x_tiu_xor_int16(
    cvk_context_t                   *ctx,
    const cvk_tiu_xor_int16_param_t *p)
{
    int8_t status  = 0;
