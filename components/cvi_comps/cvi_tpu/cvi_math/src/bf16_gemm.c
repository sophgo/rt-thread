...
static int get_fmt_byte_sz(cvk_fmt_t fmt)
{
                                        ...
                                        size_t row_slice_num, size_t col_slice_num, cvk_fmt_t fmt)
{
    ...
    {
        slice_offset_row = (row_num % row_slice_num) * (row_num / row_slice_num + 1) + (row_slice_idx - (row_num % row_slice_num)) * (row_num / row_slice_num);
    }
    ...
    {
        slice_offset_col = (col_num % col_slice_num) * (col_num / col_slice_num + 1) + (col_slice_idx - (col_num % col_slice_num)) * (col_num / col_slice_num);
    }
                                 ...
                                 size_t *slice_num, cvk_fmt_t fmt)
{
    ...
    while (true)
    {
        ...
        bool slicing_success = (local_size_A <= bank_size_half) && (local_size_C <= bank_size * outputs_nr) && (local_size_B <= bank_size_half);
...
int cvm_combin_gemm_i8(size_t *slice_num, uint8_t *i8_C, uint32_t *i32_C, int M, int N)
{
        ...
        for (int tiling = 0; tiling < chunks; tiling++)
        {
            ...
            for (int m = 0; m < (int)M; m++)
            {
                for (int n = 0; n < (int)N_slice; n++)
                {
                    ...
                    i32_C[shift] = (i8_C[shift + bstride * 0]) | (i8_C[shift + bstride * 1] << 8) | (i8_C[shift + bstride * 2] << 16) | (i8_C[shift + bstride * 3] << 24);
                }
            }
            pack_shift  += size;
            N_slice_cnt += N_slice;
    ...
    else if (strategy == 1)
    {
        ...
        for (int tiling = 0; tiling < chunks; tiling++)
        {
            ...
            {
                i32_C[pack_shift + i] = (i8_C[pack_shift * sizeof(int) + i + bstride * 0]) | (i8_C[pack_shift * sizeof(int) + i + bstride * 1] << 8) | (i8_C[pack_shift * sizeof(int) + i + bstride * 2] << 16) | (i8_C[pack_shift * sizeof(int) + i + bstride * 3] << 24);
            }
            pack_shift += size;
        }
    }
