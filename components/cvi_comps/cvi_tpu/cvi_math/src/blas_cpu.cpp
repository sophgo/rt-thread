                             ...
                             const uint32_t k)
{
    ...
    for (uint32_t i = 0; i < data_num; i++)
    {
        buffer[i] = dot_i8(feature, precached + i * data_length, data_length) / (unit_feature * unit_precached_arr[i]);
    }
                             ...
                             const uint32_t k)
{
    ...
    for (uint32_t i = 0; i < data_num; i++)
    {
        buffer[i] = dot(feature, precached + i * data_length, data_length) / (unit_feature * unit_precached_arr[i]);
    }
    k_selection_sort_index(buffer, k_index, k_value, data_num, k);
}
