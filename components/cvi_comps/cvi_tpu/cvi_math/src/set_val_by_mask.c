                               ...
                               cvk_tl_t *tl_kernel, cvk_tl_t *tl_bias, cvk_tl_t *tl_ofmap)
{
    ...
    if (!check_same_fmt(tl_ifmap, tl_ifmap, tl_mask) && !check_same_fmt(tl_ifmap, tl_ifmap, tl_ofmap))
    {
        printf("all tensor's fmt shoud be equal\n");
        return -2;
    }
                           ...
                           uint8_t w1, uint8_t w2, cvk_tl_t *tl_ofmap)
{
    ...
    if (!check_same_fmt(tl_ifmap, tl_buf, tl_update_tbl) && !check_same_fmt(tl_buf2, tl_update_tbl, tl_ofmap))
    {
        printf("all tensor's fmt shoud be equal\n");
        return -2;
    }
