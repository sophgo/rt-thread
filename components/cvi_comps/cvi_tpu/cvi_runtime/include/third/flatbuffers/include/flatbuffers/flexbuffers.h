...
class Reference
{
   public:
    Reference() :
        data_(nullptr), parent_width_(0), byte_width_(BIT_WIDTH_8), type_(FBT_NULL)
    {
    }
    ...
    void ToString(bool strings_quoted, bool keys_quoted, std::string &s) const
    {
        ...
        {
            auto blob = AsBlob();
            flatbuffers::EscapeString(reinterpret_cast<const char *>(blob.data()), blob.size(), &s,
                                      true, false);
        }
        else
        {
...
class Builder FLATBUFFERS_FINAL_CLASS
{
   public:
    ...
    {
        auto start = StartMap();
        for (auto it = map.begin(); it != map.end(); ++it) Add(it->first.c_str(), it->second);
        EndMap(start);
    }
