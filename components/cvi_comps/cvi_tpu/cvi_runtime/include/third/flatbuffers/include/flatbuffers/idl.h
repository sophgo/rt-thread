...
{
    Value() :
        constant("0"), offset(static_cast<voffset_t>(~(static_cast<voffset_t>(0U))))
    {
    }
    Type        type;
    std::string constant;
...
inline bool operator<(const Namespace &a, const Namespace &b)
{
    ...
    {
        if (a.components[i] != b.components[i]) return a.components[i] < b.components[i];
    }
...
class Parser : public ParserState
{
   ...
   private:
    ...
    FLATBUFFERS_CHECKED_ERROR TryTypedValue(const std::string *name, int dtoken, bool check,
                                            Value &e, BaseType req, bool *destmatch);
