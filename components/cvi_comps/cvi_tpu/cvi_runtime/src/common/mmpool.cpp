...
static void free_existing_block(pool_addr_t ptr, struct pool_struct *P, int index)
{
    ...
    for (i = 0;; i++)
    {
        assert((i < P->block_set[index].nr_of_entries) && (quad_block[i].mem_blocks != MEM_POOL_ADDR_INVALID));
                                      ...
                                      int                   *piblockindex)
{
    ...
    do {
        ...
        if (status != 0x0)
        {
            ...
            found = (u64)pfraglevelinfo->quad_block[i].mem_blocks + (u64)(OCTET_TO_SIZEOFUNIT(free_bit * (u64)pfraglevelinfo->block_size));
