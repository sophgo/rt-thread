...
#define CHECK_FREED_MEM(offset, size, P)                                                                                                                        \
    do {                                                                                                                                                        \
        ...
        {                                                                                                                                                       \
            if (((*it).first <= offset && (*it).first + (*it).second > offset) || ((*it).first < offset + size && (*it).first + (*it).second >= offset + size)) \
            {                                                                                                                                                   \
...
{
   public:
    offset_prev_finder(pool_addr_t offset_prev) :
        offset_prev_(offset_prev)
    {
    }
...
{
   public:
    offset_next_finder(pool_addr_t offset_next) :
        offset_next_(offset_next)
    {
    }
    bool operator()(const pool_pair_t &pair_)
    {
