    ...
    _baseMemArray(baseMemArray)
{
    ...
    auto pixel_format  = tensor->pixel_format() ? tensor->pixel_format()->str() : std::string("");
...
bool Neuron::isPacked()
{
    ...
    {
        pixel_format = (shape[3] == 3) ? CVI_NN_PIXEL_PACKED : CVI_NN_PIXEL_PLANAR;
    }
    if (pixel_format == CVI_NN_PIXEL_BGR_PACKED || pixel_format == CVI_NN_PIXEL_RGB_PACKED || pixel_format == CVI_NN_PIXEL_PACKED)
    {
        return true;
    }
    return false;
