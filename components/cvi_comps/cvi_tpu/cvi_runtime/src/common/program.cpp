...
void Program::skipPreProcess()
{
    for (int i = 0; i < (int)in_tensors.size(); ++i)
    {
        if (in_tensors[i]->fmt != CVI_FMT_FP32)
        {
            continue;
        }
        for (auto rt = _routines.begin(); rt != _routines.end();)
        {
            if ((*rt)->inputs[0]->name == in_tensors[i]->name)
            {
                neuron_map.erase(in_tensors[i]->name);
                in_tensors[i] = (*rt)->outputs[0];
                rt            = _routines.erase(rt);
            }
            else
            {
                ++rt;
...
{
    if (param.get<std::string>("from") == "NONE" && param.get<std::string>("to") == "INT8")
    {
        float scale = param.has("threshold") ? (128.0 / param.get<float>("threshold"))
