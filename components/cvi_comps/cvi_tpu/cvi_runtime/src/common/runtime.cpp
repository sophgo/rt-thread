    ...
    CVI_VIDEO_FRAME_INFO *video_frame_info)
{
  ...
  if (tensor->pixel_format != video_frame_info->type) {
    ...
    // check shape
    if (tensor->shape.dim[1] != video_frame_info->shape.dim[1] || tensor->shape.dim[2] != video_frame_info->shape.dim[2] || tensor->shape.dim[3] != video_frame_info->shape.dim[3])
    {
