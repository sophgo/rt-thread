...
void print_memory(uint64_t address, size_t num_bytes)
{
    uint8_t *ptr = (uint8_t *)(uintptr_t)address;

    rt_kprintf("Memory at 0x%p:\n", ptr);
                                     ...
                                     cvi_array_base *p_array_base)
{
    ...
    //submit
    // print_memory(device_mem->p_addr, device_mem->size);
    bmerr_t ret = submit_dmabuf(ctx->dev, device_mem->p_addr, ctx->seq_no);
    *seq_no     = ctx->seq_no++;
    BMDEV_UNLOCK(ctx->dev);
