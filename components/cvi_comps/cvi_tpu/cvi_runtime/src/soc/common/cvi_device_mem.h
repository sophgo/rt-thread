...
class CviDeviceMem
{
   public:
    ...
    virtual bmerr_t  load_cmdbuf(bmctx_t ctx, uint8_t *cmdbuf, size_t sz, uint64_t neuron_gaddr,
                                 uint64_t weight_gaddr, bool enable_pmu,
                                 bmmem_device_t *cmdbuf_mem)     = 0;
    virtual bmerr_t  load_cmdbuf_tee(bmctx_t ctx, uint8_t *cmdbuf, size_t sz, uint64_t neuron_gaddr,
                                     uint64_t weight_gaddr, uint32_t weight_len,
                                     bmmem_device_t *cmdbuf_mem) = 0;
    virtual bmerr_t  load_dmabuf(bmctx_t ctx, bmmem_device_t in_mem, size_t sz,
                                 uint64_t neuron_gaddr, uint64_t weight_gaddr, bool enable_pmu,
                                 bmmem_device_t *dmabuf_mem)     = 0;
    ...
    virtual bmerr_t  parse_pmubuf(bmmem_device_t cmdbuf_mem, uint8_t **buf_start,
                                  uint32_t *buf_len)                = 0;
   ...
   public:
    ...
    virtual bmerr_t mem_flush_addr(bmdev_t dev, uint64_t paddr, size_t size);
    virtual bmerr_t mem_invld_addr(bmdev_t dev, uint64_t paddr, size_t size);
    virtual bmerr_t submit_dmabuf(bmdev_t dev, uint64_t p_addr, uint32_t seq_no);
    virtual bmerr_t wait_dmabuf(bmdev_t dev, uint32_t seq_no);
    virtual bmerr_t mem_alloc(bmdev_t dev, size_t size, uint64_t *paddr, uint8_t **vaddr,
                              uint64_t *ori_addr);
    virtual bmerr_t mem_free(uint64_t ori_addr, size_t size);
    ...
    virtual bmerr_t run_tee(bmctx_t ctx, uint32_t seq_no, uint64_t dmabuf_addr,
                            uint64_t array_base2, uint64_t array_base3, uint64_t array_base4,
                            uint64_t array_base5, uint64_t array_base6, uint64_t array_base7);
   ...
   public:
    ...
    static std::mutex        root_daemon_lock;
