...
CVI_RC CviRTSoc::Submit(CVI_RT_KHANDLE rt_khandle)
{
    ...
    cvi_device->send_cmdbuf(submit_handle->rt_ctx, cmdbuf, (size_t)len, &seq_no);
    cvk_context->ops->reset(cvk_context);
    return CVI_SUCCESS;
}
...
CVI_RC CviRTSoc::MemFlush(CVI_RT_HANDLE rt_handle, CVI_RT_MEM mem)
{
    ...
    TPU_ASSERT(cvi_device->mem_flush_ext(ctx->dev, device_mem->dma_fd,
                                         device_mem->p_addr, device_mem->size)
                   == BM_SUCCESS,
               NULL);
    return CVI_SUCCESS;
}
...
CVI_RC CviRTSoc::MemFlushAddr(CVI_RT_HANDLE rt_handle, uint64_t paddr, uint64_t len)
{
    bmctx_t ctx = (bmctx_t)rt_handle;
    TPU_ASSERT(cvi_device->mem_flush_addr(ctx->dev, paddr, len) == BM_SUCCESS, NULL);
    return CVI_SUCCESS;
}

CVI_RC CviRTSoc::MemInvldAddr(CVI_RT_HANDLE rt_handle, uint64_t paddr, uint64_t len)
{
    bmctx_t ctx = (bmctx_t)rt_handle;
    TPU_ASSERT(cvi_device->mem_invld_addr(ctx->dev, paddr, len) == BM_SUCCESS, NULL);
    return CVI_SUCCESS;
}
