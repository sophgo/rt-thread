...
class CviRTBase
{
   public:
    ...
    virtual CVI_RC         LoadCmdbuf(CVI_RT_HANDLE rt_handle, uint8_t *cmdbuf, uint64_t cmdbuf_sz,
                                      uint64_t gaddr_base0, uint64_t gaddr_base1, bool enable_pmu,
                                      CVI_RT_MEM *cmdbuf_mem)                                         = 0;
    virtual CVI_RC         LoadDmabuf(CVI_RT_HANDLE rt_handle, CVI_RT_MEM dmabuf, uint64_t dmabuf_sz,
                                      uint64_t gaddr_base0, uint64_t gaddr_base1, bool enable_pmu,
                                      CVI_RT_MEM *dmabuf_mem)                                         = 0;
    virtual CVI_RC         RunCmdbuf(CVI_RT_HANDLE rt_handle, CVI_RT_MEM cmdbuf_mem, uint64_t gaddr_base2,
                                     uint64_t gaddr_base3)                                            = 0;
    virtual CVI_RC         RunCmdbufEx(CVI_RT_HANDLE rt_handle, CVI_RT_MEM cmdbuf_mem,
                                       CVI_RT_ARRAYBASE *p_array_base)                                = 0;
    virtual CVI_RC         LoadCmdbufTee(CVI_RT_HANDLE rt_handle, uint8_t *cmdbuf, size_t sz,
                                         uint64_t neuron_gaddr, uint64_t weight_gaddr, uint32_t weight_len,
                                         CVI_RT_MEM *cmdbuf_mem)                                      = 0;
    ...
    virtual CVI_RC         MemFlushAddr(CVI_RT_HANDLE rt_handle, uint64_t paddr, uint64_t len)        = 0;
    virtual CVI_RC         MemInvldAddr(CVI_RT_HANDLE rt_handle, uint64_t paddr, uint64_t len)        = 0;
    virtual CVI_RC         MemCopyS2D(CVI_RT_HANDLE rt_handle, CVI_RT_MEM dst, uint8_t *src)          = 0;
    virtual CVI_RC         MemCopyS2DEx(CVI_RT_HANDLE rt_handle, CVI_RT_MEM dst, uint64_t offset,
                                        uint64_t len, uint8_t *src)                                   = 0;
    virtual CVI_RC         MemCopyD2S(CVI_RT_HANDLE rt_handle, uint8_t *dst, CVI_RT_MEM src)          = 0;
    virtual CVI_RC         ParsePmuBuf(CVI_RT_MEM cmdbuf_mem, uint8_t **buf_start, uint32_t *buf_len) = 0;
    virtual CVI_RC         SetBaseReg(CVI_RT_HANDLE rt_handle, uint32_t inx, uint64_t base_addr)      = 0;
...
class CviRTSoc : public CviRTBase
{
   public:
    ...
    virtual CVI_RC         LoadDmabuf(CVI_RT_HANDLE rt_handle, CVI_RT_MEM dmabuf, uint64_t dmabuf_sz,
                                      uint64_t gaddr_base0, uint64_t gaddr_base1, bool enable_pmu,
                                      CVI_RT_MEM *dmabuf_mem) override;
    virtual CVI_RC         RunCmdbuf(CVI_RT_HANDLE rt_handle, CVI_RT_MEM cmdbuf_mem, uint64_t gaddr_base2,
                                     uint64_t gaddr_base3) override;
    virtual CVI_RC         RunCmdbufEx(CVI_RT_HANDLE rt_handle, CVI_RT_MEM cmdbuf_mem,
    ...
    virtual CVI_RC         MemFlushAddr(CVI_RT_HANDLE rt_handle, uint64_t paddr, uint64_t len) override;
    virtual CVI_RC         MemInvldAddr(CVI_RT_HANDLE rt_handle, uint64_t paddr, uint64_t len) override;
    ...
    virtual CVI_RC         SetBaseReg(CVI_RT_HANDLE rt_handle, uint32_t inx, uint64_t base_addr) override;
