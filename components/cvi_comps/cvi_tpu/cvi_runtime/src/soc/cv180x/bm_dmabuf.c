...
static DESC *traverse_start(uint8_t *cmdbuf)
{
    ...
    TPU_ASSERT(desc->hdr.magic == CMDBUF_HDR_MAGIC_180X, NULL);
    return desc;
}
...
static DESC *traverse_next(DESC *desc, uint8_t *cmdbuf, size_t size)
{
    ...
    if ((uint8_t *)next_desc >= cmdbuf + size)
        return NULL;
    TPU_ASSERT(next_desc->hdr.magic == CMDBUF_HDR_MAGIC_180X, NULL);
    return next_desc;
}
...
void cvi180x_arraybase_set(uint8_t *dmabuf, uint32_t arraybase0L, uint32_t arraybase1L, uint32_t arraybase0H, uint32_t arraybase1H)
{
...
uint64_t cvi180x_get_pmusize(uint8_t *dmabuf)
{
