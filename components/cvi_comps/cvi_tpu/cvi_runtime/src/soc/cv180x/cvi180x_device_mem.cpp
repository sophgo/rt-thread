                                      ...
                                      bool enable_pmu, bmmem_device_t *cmdbuf_mem)
{
    ...
    cvi180x_arraybase_set(dmabuf_mem->v_addr, (u32)neuron_gaddr, (u32)weight_gaddr, 0, 0);

    cvi180x_dmabuf_relocate(dmabuf_mem->v_addr, dmabuf_devaddr, dmabuf_size, pmubuf_size);
                                      ...
                                      bool enable_pmu, bmmem_device_t *dmabuf_mem)
{
    ...
    if (enable_pmu)
    {
        pmubuf_size = cvi180x_get_pmusize(dmabuf->v_addr);
    ...
    cvi180x_dmabuf_relocate((*dmabuf_mem)->v_addr, dmabuf_devaddr, sz,
                            pmubuf_size);
    TPU_ASSERT(mem_flush_ext(ctx->dev, (*dmabuf_mem)->dma_fd,
                             (*dmabuf_mem)->p_addr, sz)
                   == BM_SUCCESS,
               NULL);
    // if (0) {
    //cvi180x_dmabuf_dump(dmabuf);
