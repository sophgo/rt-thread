...
static void tpu_pmu_fill_cmdbuf(uint8_t *v_dma_buf)
{
    ...
    while (p_element[index].pmuEvent.type)
    {
        ...
        { //tdma
            ...
            if ((p_element[index].pmuEvent.desID != tdma_id_previous) || (p_element[index].pmuEvent.startTime != tdma_start_pre) || (p_element[index].pmuEvent.endTime != tdma_end_pre))
            {
            ...
            { //tdma g2g case, copy 1st to 2nd tdma descriptor
...
static void tpu_pmu_fwrite_des()
{
    ...
    while (p_element[index].pmuEvent.type)
    {
        ...
        {
            srcAddr = ((uint64_t)(p_element[index].tdmaReg.src_base_addr_high) << 32) | (uint64_t)(p_element[index].tdmaReg.src_base_addr_low);
            dstAddr = ((uint64_t)(p_element[index].tdmaReg.dst_base_addr_high) << 32) | (uint64_t)(p_element[index].tdmaReg.dst_base_addr_low);
...
static void tpu_pmu_getlayerInfo(void)
{
    ...
    while (p_element[index].pmuEvent.type)
    {
        if (!curLayID)
        {
            ...
            curLayID = p_element[index].pmuEvent.type == TPU_PMUTYPE_TIU ? p_element[index].tiuReg.layer_info : p_element[index].tdmaReg.layer_ID;
        ...
        else
        {
            ...
            if (curLayID == (p_element[index].pmuEvent.type == TPU_PMUTYPE_TIU ? p_element[index].tiuReg.layer_info : p_element[index].tdmaReg.layer_ID))
            {
                p_layer[layIDIndex].endTime = (p_element[index].pmuEvent.endTime > p_layer[layIDIndex].endTime) ? (p_element[index].pmuEvent.endTime) : (p_layer[layIDIndex].endTime);
...
static int tpu_pmu_time(uint8_t *v_dma_buf, uint64_t p_dma_buf, uint8_t all_info)
{
    ...
    while (*(uint32_t *)pCurrent)
    {
        ...
        if (pCurrent->type == TPU_PMUTYPE_TIU)
        { //tiu case
        ...
        { //tdma case
            ...
            if ((pCurrent->desID != tdma_id_previous) || (pCurrent->startTime != tdma_start_pre) || (pCurrent->endTime != tdma_end_pre))
            {
                if (pCurrent->endTime > pCurrent->startTime)
                {
                    diff = pCurrent->endTime - pCurrent->startTime;
    ...
    printf("%-20s %8dMhz, %-20s %9.2fMB, %-20s %7.2fMB/s\n",
           "181x_tpu_clock:", tpu_clk_rate / 1000000, "inferece_data:", load_mb + store_mb, "inference_bw:", bandwidth);
