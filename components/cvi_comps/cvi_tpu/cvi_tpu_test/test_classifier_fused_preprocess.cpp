...
static void load_file(const char *file_name, void **ptr, size_t *size)
{
    ...
    struct stat label_stat;
    int         ret = fstat(file_fd, &label_stat);
    if (ret < 0)
    {
    ...
    size_t r_size = read(file_fd, *ptr, *size);
    if (r_size != *size)
    {
...
MSH_CMD_EXPORT(test_classifier, test tpu sdk);
