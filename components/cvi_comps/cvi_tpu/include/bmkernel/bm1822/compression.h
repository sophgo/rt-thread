...
static inline void compute_compressed_range(uint32_t bit_length, int is_signed, int *min, int *max)
{
    if (is_signed)
    {
        switch (bit_length)
...
static inline uint64_t count_non_zero_results(uint8_t buf[], uint64_t size, int is_signed, int max,
                                              int min)
{
    uint64_t n = 0;
...
static inline void parse_header(uint32_t header, int *is_signed, uint32_t *compress_md,
                                uint32_t *nz_num)
{
...
static inline void fill_header(uint32_t *hdr, compression_info_t *info)
{
    ...
    {
        *hdr = (info->is_signed << 29) | (1 << 28) | (info->compress_md << 24) | info->non_zero_data_num;
    }
    else
    {
        *hdr = (info->is_signed << 29) | (1 << 28) | (info->compress_md << 24);
    }
}
...
static inline void fill_map(uint8_t map[], uint8_t buf[], compression_info_t *info)
{
    ...
    for (uint64_t i = 0; i < clear_map; i++) map[i] = 0;

    for (uint64_t i = 0; i < info->total_data_num; i++)
    {
        int val = info->is_signed ? (int8_t)buf[i] : buf[i];
        int res = saturate(val, max, min);
        if (res != 0) set_map_bit(map, i);
    }
}

static inline void compress_one_data(uint8_t data[], uint64_t i, uint8_t val,
...
static inline uint8_t decompress_one_data(uint8_t data[], uint64_t i, compression_info_t *info)
{
    uint32_t bit_len       = info->bit_length;
    uint32_t data_per_byte = 8 / bit_len;
    ...
    if (info->is_signed) val = sign_extend(val, bit_len);
...
static inline void fill_data(uint8_t data[], uint8_t buf[], compression_info_t *info)
{
    ...
    for (uint64_t i = 0; i < info->data_bytes; i++) data[i] = 0;
...
static inline compression_info_t make_compression_info(uint8_t buf[], uint64_t size,
                                                       uint32_t compress_md, int is_signed)
{
...
static inline compression_info_t parse_compression_info(uint8_t compressed_buf[], uint64_t max_size,
                                                        uint64_t total_data_num)
{
...
static inline uint8_t *compress(uint8_t buf[], uint64_t size, uint32_t compress_md, int is_signed,
                                compress_addr_info *compressed_data)
{
    compression_info_t info = make_compression_info(buf, size, compress_md, is_signed);
    ...
    if (info.bit_length != 1) fill_data(data, buf, &info);
...
static inline void decompress(uint8_t buf[], uint64_t size, uint8_t compressed_buf[],
                              uint64_t max_size)
{
    compression_info_t info = parse_compression_info(compressed_buf, max_size, size);
    assert(info.total_bytes <= max_size);
