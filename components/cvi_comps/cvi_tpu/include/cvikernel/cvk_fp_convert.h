...
static inline void cvk_f32_integer(void *if32, void *o_integer, int integer_size, int accumulate,
                                   int int8_signed, int int8_rnd_md);
...
static inline uint16_t cvk_convert_fp32_bf16(float fp32)
{
    if (cvk_float_isnan(fp32)) return NAN_VALUE;
    convert_int_float convert_val;
...
static inline void cvk_flt2int_flt(float x, unsigned long long *integer_part, float *sub_part,
                                   uint8_t sign)
{
