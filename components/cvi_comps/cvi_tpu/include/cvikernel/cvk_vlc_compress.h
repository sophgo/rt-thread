...
static inline void init_stream(StreamBuffer *bs, const uint8_t *buf, int buf_size,
                               uint8_t read_only);

static inline void cvk_vlc_est_weight_bias(const uint8_t *ibuf, size_t isz, uint8_t signedness,
                                           uint8_t isBfloat16, CommandInfo *cmd_info);
static inline void cvk_vlc_enc_int8(const uint8_t *ibuf, size_t isz, uint8_t *obuf, size_t *osz,
                                    CommandInfo *cmd_info);
static inline void cvk_vlc_dec_int8_ext(const uint8_t *ibuf, size_t isz, uint8_t *obuf,
                                        size_t *bs_size);
static inline void cvk_vlc_dec_int8(const uint8_t *ibuf, size_t isz, uint8_t *obuf);
static inline void cvk_vlc_enc_bf16(const uint16_t *ibuf, size_t isz, uint8_t *obuf, size_t *osz,
                                    CommandInfo *cmd_info);
static inline void cvk_vlc_dec_bf16_ext(const uint8_t *ibuf, size_t isz, uint16_t *obuf,
                                        size_t *bs_size);
...
static inline void dispatch_bf16_data(const uint16_t *bf16_in, uint8_t *exp, uint8_t *frac,
                                      size_t isz)
{
...
static inline void merge_bf16_data(const uint8_t *exp_in, const uint8_t *frac_in,
                                   uint16_t *bf16_out, size_t isz)
{
                               ...
                               uint8_t read_only)
{
    ...
    if (!read_only) memset((uint8_t *)buf, 0, sizeof(uint8_t) * buf_size);
}
...
static inline void vlc_enc_header(StreamBuffer *bs_header, CommandInfo *cmd_info,
                                  size_t blk_bs_size)
{
...
static inline void vlc_dec_header_ext(StreamBuffer *bs_header, CommandInfo *cmd_info,
                                      size_t *blk_bs_size)
{
    ...
    parse_stream(bs_header, (uint8_t *)&cmd_info->bias0,
                 8);                                                 // bit[39:32] bias0 for symbol remapping
    parse_stream(bs_header, (uint8_t *)&cmd_info->bias1,
                 7);                                                 // bit[46:40] bias1 for symbol remapping
...
static inline uint8_t inv_center_shift(uint8_t val, uint8_t bias, uint8_t zero_guard)
{
    ...
    {
        return (unsign_data_i > (range << 1)) ? (range + bias - val + zero_guard)
...
static inline int8_t inv_two_side_circular_shift(int8_t val, uint8_t bias0, uint8_t bias1)
{
    ...
    uint8_t abs_val_lsb    = ((abs_val_minus <= 0) ? (uint8_t)abs_val : (uint8_t)abs_val_minus) & 0xFF;
    return (sign) ? -abs_val_lsb : abs_val_lsb;
}

static inline void symbol_remapping(uint8_t *blk_in, uint8_t *blk_out, uint8_t bias0, uint8_t bias1,
                                    uint8_t signedness, uint8_t is_bf16_exp, uint8_t zero_guard)
{
...
static inline void inv_symbol_remapping(uint8_t *blk_in, uint8_t *blk_out, uint8_t bias0,
                                        uint8_t bias1, uint8_t signedness, uint8_t is_bf16_exp,
                                        uint8_t zero_guard)
{
    if (is_bf16_exp == false && signedness == false)
    {
...
static inline uint8_t vlc_gr_enc_block_data(uint8_t *blk_in, StreamBuffer *bs, int order_k,
                                            uint8_t bf16_zvc_en)
{
    // uncompressed mode
    if (order_k == -1)
    ...
    if (bf16_zvc_en && order_k > 0)
    {
        ...
        for (int i = 0; i < 16; i++)
        {
            if (blk_in[i] == 0) zero_num++;
        }
        // assert(zero_num < 16);
        if (zero_num >= 16) return 0;
    ...
    if (unary_field_len > MAX_UNARY_FIELD_SIZE) return 0;
...
static inline void vlc_gr_dec_block_data(StreamBuffer *bs, uint8_t bs_size, uint8_t *rec,
                                         int order_k, uint8_t bf16_zvc_en)
{
    // assert(bs_size <= 128);
    if (bs_size > 128) return;
                                        ...
                                        size_t *bs_size)
{
    ...
    *bs_size           = 0;
    ...
    if (*bs_size > bs_buf_size || cmd_info.is_bfloat16) return;
    ...
    for (size_t blk_idx = 0; blk_idx < blk_num; blk_idx++)
    {
        ...
        inv_symbol_remapping(blk_data, blk_sr_data, cmd_info.bias0, cmd_info.bias1,
                             cmd_info.signedness, false, false);
                                    ...
                                    CommandInfo *cmd_info)
{
    ...
    for (size_t blk_idx = 0; blk_idx < blk_num; blk_idx++)
    {
        ...
        // exp: BGR encode
        symbol_remapping(blk_data, blk_sr_data, cmd_info->bias0, cmd_info->bias1, false, true,
                                        ...
                                        size_t *bs_size)
{
    ...
    *bs_size           = 0;
    ...
    if (*bs_size > bs_buf_size || cmd_info.is_bfloat16 != 1) return;
    ...
    for (size_t blk_idx = 0; blk_idx < blk_num; blk_idx++)
    {
        ...
        inv_symbol_remapping(blk_data, blk_sr_data, cmd_info.bias0, cmd_info.bias1, false, true,
                             cmd_info.zero_guard_en);
                                           ...
                                           uint8_t isBfloat16, CommandInfo *cmd_info)
{
    ...
    if (isBfloat16 == false && signedness == true)
    {
        ...
        int8_t neg_v    = -1;
        // while (neg_v >= (-128)) // comparison is always   true due to limited range of data type
        // [-Werror=type-limits]
        while (true)
        {
