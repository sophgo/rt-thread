...
static inline int cvm_exp_start()
{
    return -62;
}
static inline int cvm_exp_end()
{
    return 63;
}
static inline int cvm_table_h()
{
    return 32;
}
static inline int cvm_table_w()
{
    return 8;
}
static inline int cvm_table_hw()
{
    return cvm_table_h() * cvm_table_w();
}
static inline int half_h_table()
{
    return cvm_table_h() * cvm_table_w() / 2;
}
static inline bool is_1880v2_tbl_shape(cvk_tl_shape_t *s)
{
    // FIXME: h could be reduce less than 32
    assert(s->h == (uint32_t)cvm_table_h() && s->w == (uint32_t)cvm_table_w() && "table h/w should be 32/8");
...
{
    assert(src->fmt == CVK_FMT_BF16 && (fmt == CVK_FMT_I8 || fmt == CVK_FMT_U8) && "only support bf16->i8/uint8_t, plz check fmt\n");
