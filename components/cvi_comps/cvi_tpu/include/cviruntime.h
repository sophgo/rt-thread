 ...
 */
CVI_RC CVI_NN_GetInputOutputTensors(CVI_MODEL_HANDLE model, CVI_TENSOR **inputs, int32_t *input_num,
 ...
 */
CVI_RC CVI_NN_SetTensorWithVideoFrame(CVI_MODEL_HANDLE model, CVI_TENSOR *tensor,
 ...
 * WARNNING, this API is DEPRECATED.
 */
CVI_RC CVI_NN_FeedTensorWithFrames(CVI_MODEL_HANDLE model, CVI_TENSOR *tensor, CVI_FRAME_TYPE type,
                                   CVI_FMT format, int32_t channel_num, uint64_t *channel_paddrs,
                                   int32_t height, int32_t width, uint32_t height_stride);

/*
 * Fill frames data from vpss to tensor.
 */
CVI_RC CVI_NN_SetTensorWithAlignedFrames(CVI_TENSOR *tensor, uint64_t frame_paddrs[],
