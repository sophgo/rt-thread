...
#define CVI_RT_TRACE(level, fmt, ...)                                          \
    do {                                                                       \
        if (level <= CONFIG_CVI_LOG_TRACE_LEVEL)                               \
            printf("[%s-%s] " fmt, "CVIRT", level_name[level], ##__VA_ARGS__); \
    } while (0)

#define TPU_LOG_FATAL(...) CVI_RT_TRACE(CVI_DBG_CRIT, __VA_ARGS__);

#define TPU_LOG_ERROR(...) CVI_RT_TRACE(CVI_DBG_ERR, __VA_ARGS__);

#define TPU_LOG_WARNING(...) CVI_RT_TRACE(CVI_DBG_WARN, __VA_ARGS__);

#define TPU_LOG_NOTICE(...) CVI_RT_TRACE(CVI_DBG_NOTICE, __VA_ARGS__);

#define TPU_LOG_INFO(...) CVI_RT_TRACE(CVI_DBG_INFO, __VA_ARGS__);

#define TPU_LOG_DEBUG(...) CVI_RT_TRACE(CVI_DBG_DEBUG, __VA_ARGS__);
...
typedef enum _CVI_TPU_ERRCODE
{
    ...
    CVI_ERR_TPU_AGAIN =
        CVI_TPU_ERR(CVI_TPU_RUNTIME, TPU_EN_ERR_LEVEL_ERROR, TPU_EN_ERR_SYS_NOTREADY),
    CVI_ERR_TPU_FAILURE = -1,
    CVI_ERR_TPU_TIMEOUT = CVI_TPU_ERR(CVI_TPU_RUNTIME, TPU_EN_ERR_LEVEL_ERROR, TPU_EN_ERR_TIMEOUT),
    CVI_ERR_TPU_UNINIT =
        CVI_TPU_ERR(CVI_TPU_RUNTIME, TPU_EN_ERR_LEVEL_ERROR, TPU_EN_ERR_NOT_CONFIG),
    CVI_ERR_TPU_INVALID_ARG =
        CVI_TPU_ERR(CVI_TPU_RUNTIME, TPU_EN_ERR_LEVEL_ERROR, TPU_EN_ERR_ILLEGAL_PARAM),
    ...
    CVI_ERR_TPU_UNSUPPORT =
        CVI_TPU_ERR(CVI_TPU_RUNTIME, TPU_EN_ERR_LEVEL_ERROR, TPU_EN_ERR_NOT_SUPPORT),
} CVI_TPU_ERRCODE;
