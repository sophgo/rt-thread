...
int dfs_mnt_unref(struct dfs_mnt* mnt)
{
    ...
    if (mnt)
    {
        ...
        if (rt_atomic_load(&(mnt->ref_count)) == 0)
        {
            ...
            /*
            2025-07-10, wangliang.wang
            disable this code temporarily, it will cause a pointer error in dfs_mnt_lookup() function when mounting /dev/sdx.
            calltrace:
            mmcsd_detect -> init_sd -> rt_mmcsd_blk_probe  -> mbr_device_probe -> rt_device_register ->
            dfs_devfs_device_add  -> access -> stat -> dfs_file_stat -> dfs_file_realpath ->
            _try_readlink -> dfs_dentry_unref -> dfs_mnt_unref -> rt_free
            */
            #if 0
            /* free full path */
